...
void cache_map(char *name, struct map_data *m) {
	...
	// Fill the map header
	if (strlen(name) > MAP_NAME_LENGTH)	 // It does not hurt to warn that there are maps with name
										 // longer than allowed.
		ShowWarning("Map name '%s' size '%" PRIuPTR "' is too long. Truncating to '%d'.\n", name,
					strlen(name), MAP_NAME_LENGTH);
...
char *remove_extension(char *mapname) {
	...
	if (ptr) {														 // Check and remove extension.
		while (ptr[1] && (ptr2 = strchr(ptr + 1, '.'))) ptr = ptr2;	 // Skip to the last dot.
		if (strcmp(ptr, ".gat") == 0)
