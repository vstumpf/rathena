...
int parse_console(const char* buf) { return 1; }

std::thread svr_thr;

/// Msg_conf tayloring
int web_msg_config_read(char* cfgName) { return _msg_config_read(cfgName, WEB_MAX_MSG, msg_table); }
const char* web_msg_txt(int msg_number) { return _msg_txt(msg_number, WEB_MAX_MSG, msg_table); }
void web_do_final_msg(void) { _do_final_msg(WEB_MAX_MSG, msg_table); }
...
void web_set_defaults() {
	...
	safestrncpy(web_config.msgconf_name, "conf/msg_conf/web_msg.conf",
				sizeof(web_config.msgconf_name));
...
int web_sql_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(login_handle, login_server_id.c_str(), login_server_pw.c_str(),
								 login_server_ip.c_str(), login_server_port,
								 login_server_db.c_str())) {
		ShowError(
			"Couldn't connect with uname='%s',passwd='%s',host='%s',port='%d',database='%s'\n",
			login_server_id.c_str(), login_server_pw.c_str(), login_server_ip.c_str(),
	...
	if (SQL_ERROR == Sql_Connect(char_handle, char_server_id.c_str(), char_server_pw.c_str(),
								 char_server_ip.c_str(), char_server_port,
								 char_server_db.c_str())) {
		ShowError(
			"Couldn't connect with uname='%s',passwd='%s',host='%s',port='%d',database='%s'\n",
			char_server_id.c_str(), char_server_pw.c_str(), char_server_ip.c_str(),
	...
	if (SQL_ERROR == Sql_Connect(web_handle, web_server_id.c_str(), web_server_pw.c_str(),
								 web_server_ip.c_str(), web_server_port, web_server_db.c_str())) {
		ShowError(
			"Couldn't connect with uname='%s',passwd='%s',host='%s',port='%d',database='%s'\n",
			web_server_id.c_str(), web_server_pw.c_str(), web_server_ip.c_str(), web_server_port,
			web_server_db.c_str());
...
// Is this still used ??
void set_server_type(void) { SERVER_TYPE = ATHENA_SERVER_WEB; }
...
int do_init(int argc, char** argv) {
	...
	svr_thr =
		std::thread([] { http_server->listen(web_config.web_ip.c_str(), web_config.web_port); });
	...
	ShowStatus("The web-server is " CL_GREEN "ready" CL_RESET
