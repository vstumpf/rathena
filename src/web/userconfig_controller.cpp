...
HANDLER_FUNC(userconfig_save) {
	...
	if (SQL_SUCCESS !=
		...
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (void *)world_name.c_str(),
										 world_name.length()) ||
	...
	if (SQL_SUCCESS !=
			SqlStmt_Prepare(
				stmt, "REPLACE INTO `%s` (`account_id`, `world_name`, `data`) VALUES (?, ?, ?)",
				user_configs_table) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_INT, &account_id, sizeof(account_id)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (void *)world_name.c_str(),
										 world_name.length()) ||
		SQL_SUCCESS !=
			SqlStmt_BindParam(stmt, 2, SQLDT_STRING, (void *)data_str.c_str(), data_str.length()) ||
		SQL_SUCCESS != SqlStmt_Execute(stmt)) {
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
		sl.unlock();
...
HANDLER_FUNC(userconfig_load) {
	...
	if (SqlStmt_NumRows(stmt) <= 0) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(
					stmt, "INSERT INTO `%s` (`account_id`, `world_name`, `data`) VALUES (?, ?, ?)",
					user_configs_table) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_INT, &account_id, sizeof(account_id)) ||
			SQL_SUCCESS !=
				SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (void *)world_name, strlen(world_name)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 2, SQLDT_STRING, (void *)data.c_str(),
											 strlen(data.c_str())) ||
			SQL_SUCCESS != SqlStmt_Execute(stmt)) {
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
