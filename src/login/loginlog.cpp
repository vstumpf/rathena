...
static char global_db_hostname[64] =
	"127.0.0.1";  // Doubled to reflect the change on commit #0f2dd7f
static uint16 global_db_port = 3306;
static char global_db_username[32] = "ragnarok";
static char global_db_password[32] = "";  // empty by default since mysql is empty by default as
										  // well
...
unsigned long loginlog_failedattempts(uint32 ip, unsigned int minutes) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT count(*) FROM `%s` WHERE `ip` = '%s' AND (`rcode` = '0' OR "
							   "`rcode` = '1') AND `time` > NOW() - INTERVAL %d MINUTE",
							   log_login_db, ip2str(ip, NULL),
							   minutes))  // how many times failed account? in one ip.
