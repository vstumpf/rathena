...
static bool account_db_sql_load_num(AccountDB* self, struct mmo_account* acc,
									const uint32 account_id);
...
AccountDB* account_db_sql(void) {
	...
	safestrncpy(db->global_acc_reg_num_table, "global_acc_reg_num",
				sizeof(db->global_acc_reg_num_table));
	safestrncpy(db->global_acc_reg_str_table, "global_acc_reg_str",
				sizeof(db->global_acc_reg_str_table));
...
static void account_db_sql_destroy(AccountDB* self) {
	...
	if (SQL_ERROR ==
		Sql_Query(db->accounts, "UPDATE `%s` SET `web_auth_token` = NULL", db->account_db)) {
 ...
 */
static bool account_db_sql_get_property(AccountDB* self, const char* key, char* buf,
										size_t buflen) {
	AccountDB_SQL* db = (AccountDB_SQL*)self;
...
static bool account_db_sql_create(AccountDB* self, struct mmo_account* acc) {
	...
	if (acc->account_id != -1) {  // caller specifies it manually
		...
		if (SQL_SUCCESS !=
			Sql_Query(sql_handle, "SELECT MAX(`account_id`)+1 FROM `%s`", db->account_db)) {
			Sql_ShowDebug(sql_handle);
			return false;
		}
 ...
 * fetching. Filled data structure is done by delegation to account_db_sql_load_num.
 * @param self: pointer to db
 * @param acc: pointer of mmo_account to fill
...
static bool account_db_sql_iter_next(AccountDBIterator* self, struct mmo_account* acc) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `account_id` FROM `%s` WHERE `account_id` > '%d' ORDER BY "
							   "`account_id` ASC LIMIT 1",
...
static bool mmo_auth_fromsql(AccountDB_SQL* db, struct mmo_account* acc, uint32 account_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(
			...
			"`account_id`,`userid`,`user_pass`,`sex`,`email`,`group_id`,`state`,`unban_time`,`"
			"expiration_time`,`logincount`,`lastlogin`,`last_ip`,`birthdate`,`character_slots`,`"
			"pincode`, `pincode_change` FROM `%s` WHERE `account_id` = %d",
#endif
			db->account_db, account_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
...
static bool mmo_auth_tosql(AccountDB_SQL* db, const struct mmo_account* acc, bool is_new,
	...
	do {
		...
		if (is_new) {  // insert into account table
			if (SQL_SUCCESS !=
				...
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 5, SQLDT_INT, (void*)&acc->group_id,
												 sizeof(acc->group_id)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 6, SQLDT_UINT, (void*)&acc->state,
												 sizeof(acc->state)) ||
			...
			if (SQL_SUCCESS !=
					SqlStmt_Prepare(
						...
						"`userid`=?,`user_pass`=?,`sex`=?,`email`=?,`group_id`=?,`state`=?,`unban_"
						"time`=?,`expiration_time`=?,`logincount`=?,`lastlogin`=?,`last_ip`=?,`"
						"birthdate`=?,`character_slots`=?,`pincode`=?, `pincode_change`=? WHERE "
						"`account_id` = '%d'",
#endif
				...
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 3, SQLDT_STRING, (void*)acc->email,
												 strlen(acc->email)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 4, SQLDT_INT, (void*)&acc->group_id,
												 sizeof(acc->group_id)) ||
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 5, SQLDT_UINT, (void*)&acc->state,
												 sizeof(acc->state)) ||
		...
		if (acc->sex != 'S' && login_config.use_web_auth_token && refresh_token) {
			...
			do {
				if (SQL_SUCCESS == Sql_Query(sql_handle, query, db->account_db,
...
bool account_db_sql_disable_webtoken(AccountDB* self, const uint32 account_id) {
	...
	add_timer(gettick() + login_config.disable_webtoken_delay, account_disable_webtoken_timer,
			  account_id, reinterpret_cast<intptr_t>(db));
