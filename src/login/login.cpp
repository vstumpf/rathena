...
// early declaration
bool login_check_password(const char* md5key, int passwdenc, const char* passwd,
						  const char* refpass);

/// Accessors
AccountDB* login_get_accounts_db(void) { return accounts; }
...
void login_remove_auth_node(uint32 account_id) { auth_db.erase(account_id); }
...
int login_mmo_auth_new(const char* userid, const char* pass, const char sex, const char* last_ip) {
	...
	acc.expiration_time =
		(login_config.start_limited_time != -1) ? time(NULL) + login_config.start_limited_time : 0;
...
int login_mmo_auth(struct login_session_data* sd, bool isServer) {
	...
	if (login_config.use_dnsbl) {
		...
		for (dnsbl_serv = strtok(login_config.dnsbl_servs, ","); dnsbl_serv != NULL;
			 dnsbl_serv = strtok(NULL, ",")) {
	...
	if (login_config.new_account_flag) {
			...
			sd->userid[len - 2] == '_' && memchr("FfMm", sd->userid[len - 1], 4))  // _M/_F suffix
		{
			...
			if (sd->passwdenc != 0) {
				ShowError(
					"Account '%s' could not be created because client side password encryption is "
					"enabled.\n",
					sd->userid);
	...
	if (login_config.client_hash_check && !isServer) {
		...
		if (!match) {
			...
			for (i = 0; i < 16; i++) sprintf(&smd5[i * 2], "%02x", sd->client_hash[i]);
...
bool login_check_password(const char* md5key, int passwdenc, const char* passwd,
	...
	if (passwdenc == 0) {
		...
		// password mode set to 1 -> md5(md5key, refpass) enable with
		// <passwordencrypt></passwordencrypt> password mode set to 2 -> md5(refpass, md5key) enable
		// with <passwordencrypt2></passwordencrypt2>
...
const char* login_msg_txt(int msg_number) { return _msg_txt(msg_number, LOGIN_MAX_MSG, msg_table); }
void login_do_final_msg(void) { _do_final_msg(LOGIN_MAX_MSG, msg_table); }
...
bool login_config_read(const char* cfgName, bool normal) {
	...
	while (fgets(line, sizeof(line), fp)) {
			...
			if (!strcmpi(w1, "bind_ip")) {
				...
				if (login_config.login_ip) {
					...
					ShowStatus("Login server binding IP address : %s -> %s\n", w2,
							   ip2str(login_config.login_ip, ip_str));
				}
		...
		if (!strcmpi(w1, "timestamp_format"))
			...
			login_config.ip_sync_interval =
				(unsigned int)1000 * 60 * atoi(w2);	 // w2 comes in minutes.
			...
			if (login_config.vip_sys.char_increase >
				(unsigned int)MAX_CHARS - login_config.char_per_account) {
				ShowWarning(
					"vip_char_increase too high, can only go up to %d, according to your "
					"char_per_account config %d\n",
...
void login_set_defaults() {
	...
	// other default conf
	safestrncpy(login_config.loginconf_name, "conf/login_athena.conf",
				sizeof(login_config.loginconf_name));
	safestrncpy(login_config.lanconf_name, "conf/subnet_athena.conf",
				sizeof(login_config.lanconf_name));
	safestrncpy(login_config.msgconf_name, "conf/msg_conf/login_msg.conf",
				sizeof(login_config.msgconf_name));
}
...
// Is this still used ??
void set_server_type(void) { SERVER_TYPE = ATHENA_SERVER_LOGIN; }
...
int do_init(int argc, char** argv) {
	...
	ShowStatus("The login-server is " CL_GREEN "ready" CL_RESET
			   " (Server is listening on the port %u).\n\n",
			   login_config.login_port);
