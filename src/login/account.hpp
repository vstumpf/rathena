...
struct mmo_account {
	...
	uint8 char_slots;	 // this accounts maximum character slots (maximum is limited to MAX_CHARS
						 // define in char server)
	...
	char web_auth_token[WEB_AUTH_TOKEN_LENGTH];	 // web authentication token (randomized on each
												 // login)
#ifdef VIP_ENABLE
	int old_group;
	time_t vip_time;
