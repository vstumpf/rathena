 ...
 * S 2717 aid.W email.40B exp_time.L group_id.B char_slot.B birthdate.11B pincode.5B
 * pincode_change.L isvip.1B char_vip.1B max_billing.1B (tot 75)
...
int logchrif_parse_reqchangemail(int fd, int id, char* ip) {
	if (RFIFOREST(fd) < 86)
		...
		if (e_mail_check(actual_email) == 0)
			ShowNotice(
				"Char-server '%s': Attempt to modify an e-mail on an account (@email GM command), "
				"but actual email is invalid (account: %d, ip: %s)\n",
				ch_server[id].name, account_id, ip);
		else if (e_mail_check(new_email) == 0)
			ShowNotice(
				"Char-server '%s': Attempt to modify an e-mail on an account (@email GM command) "
				"with a invalid new e-mail (account: %d, ip: %s)\n",
				ch_server[id].name, account_id, ip);
		else if (strcmpi(new_email, "a@a.com") == 0)
			ShowNotice(
				"Char-server '%s': Attempt to modify an e-mail on an account (@email GM command) "
				"with a default e-mail (account: %d, ip: %s)\n",
				ch_server[id].name, account_id, ip);
		else if (!accounts->load_num(accounts, &acc, account_id))
			ShowNotice(
				"Char-server '%s': Attempt to modify an e-mail on an account (@email GM command), "
				"but account doesn't exist (account: %d, ip: %s).\n",
				ch_server[id].name, account_id, ip);
		else if (strcmpi(acc.email, actual_email) != 0)
			ShowNotice(
				"Char-server '%s': Attempt to modify an e-mail on an account (@email GM command), "
				"but actual e-mail is incorrect (account: %d (%s), actual e-mail: %s, proposed "
				"e-mail: %s, ip: %s).\n",
				ch_server[id].name, account_id, acc.userid, acc.email, actual_email, ip);
		else {
			safestrncpy(acc.email, new_email, 40);
			ShowNotice(
				"Char-server '%s': Modify an e-mail on an account (@email GM command) (account: %d "
				"(%s), new e-mail: %s, ip: %s).\n",
...
int logchrif_parse_requpdaccstate(int fd, int id, char* ip) {
	if (RFIFOREST(fd) < 10)
		...
		if (!accounts->load_num(accounts, &acc, account_id))
			ShowNotice(
				"Char-server '%s': Error of Status change (account: %d not found, suggested status "
				"%d, ip: %s).\n",
				ch_server[id].name, account_id, state, ip);
		else if (acc.state == state)
			ShowNotice(
				"Char-server '%s':  Error of Status change - actual status is already the good "
	...
	if (RFIFOREST(fd) < 10)
		...
		if (!accounts->load_num(accounts, &acc, account_id))
			...
			if (timestamp == -1)
				...
				ShowNotice(
					"Char-server '%s': Error of ban request (account: %d, new date unbans the "
					"account, ip: %s).\n",
					ch_server[id].name, account_id, ip);
			else {
				uint8 buf[11];
				...
				ShowNotice(
					"Char-server '%s': Ban request (account: %d, new final date of banishment: %s, "
					"ip: %s).\n",
...
int logchrif_parse_reqchgsex(int fd, int id, char* ip) {
	if (RFIFOREST(fd) < 6)
		...
		if (!accounts->load_num(accounts, &acc, account_id))
			...
			ShowNotice(
				"Char-server '%s': Error of sex change - account to change is a Server account "
				"(account: %d, ip: %s).\n",
				ch_server[id].name, account_id, ip);
		else {
			unsigned char buf[7];
...
int logchrif_parse_upd_global_accreg(int fd, int id, char* ip) {
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
		...
		if (!accounts->load_num(accounts, &acc, account_id))
			ShowStatus(
				"Char-server '%s': receiving (from the char-server) of account_reg2 (account: %d "
				"not found, ip: %s).\n",
				ch_server[id].name, account_id, ip);
		else
...
int logchrif_parse_requnbanacc(int fd, int id, char* ip) {
	if (RFIFOREST(fd) < 6)
		...
		if (!accounts->load_num(accounts, &acc, account_id))
			...
			ShowNotice(
				"Char-server '%s': Error of UnBan request (account: %d, no change for unban date, "
				"ip: %s).\n",
				ch_server[id].name, account_id, ip);
		else {
	...
	if (RFIFOREST(fd) < 15)
		...
		if (accounts->load_num(accounts, &acc, aid)) {
			...
			if (now < vip_time) {  // isvip
				if (acc.group_id !=
					login_config.vip_sys.group) {  // only upd this if we're not vip already
				...
				vip_time = 0;
				if (acc.group_id ==
					login_config.vip_sys.group) {  // prevent alteration in case account wasn't
												   // registered as vip yet
			...
			accounts->save(accounts, &acc, false);
			if (flag & 1)
				logchrif_sendvipdata(fd, &acc, ((isvip) ? 0x1 : 0) | ((flag & 0x8) ? 0x4 : 0),
									 mapfd);
		}
	}
#endif
...
int logchrif_parse_accinfo(int fd) {
	if (RFIFOREST(fd) < 19)
		...
		int map_fd = RFIFOL(fd, 2), u_fd = RFIFOL(fd, 6), u_aid = RFIFOL(fd, 10),
			account_id = RFIFOL(fd, 14);
...
int logchrif_parse(int fd) {
	...
	while (RFIFOREST(fd) >= 2) {
		int next = 1;  // 0: avoid processing followup packets (prev was probably incomplete)
					   // packet, 1: Continue parsing
		...
		switch (command) {
			...
			default:
				ShowError(
					"logchrif_parse: Unknown packet 0x%x from a char-server! Disconnecting!\n",
					command);
				set_eof(fd);
				return 0;
		}  // switch
...
void do_init_loginchrif(void) {
	int i;
	for (i = 0; i < ARRAYLENGTH(ch_server); ++i) logchrif_server_init(i);
	...
	if (login_config.ip_sync_interval) {
		...
		add_timer_interval(gettick() + login_config.ip_sync_interval, logchrif_sync_ip_addresses, 0,
						   0, login_config.ip_sync_interval);
	}
}
...
void do_shutdown_loginchrif(void) {
	int id;
	for (id = 0; id < ARRAYLENGTH(ch_server); ++id) logchrif_server_reset(id);
}
...
void do_final_loginchrif(void) {
	int i;
	for (i = 0; i < ARRAYLENGTH(ch_server); ++i) logchrif_server_destroy(i);
}
