...
static void logclif_auth_ok(struct login_session_data* sd) {
	...
	if (login_config.group_id_to_connect >= 0 && sd->group_id != login_config.group_id_to_connect) {
		ShowStatus(
			"Connection refused: the required group id for connection is %d (account: %s, group: "
			"%d).\n",
			login_config.group_id_to_connect, sd->userid, sd->group_id);
		logclif_sent_auth_result(fd, 1);  // server closed
		return;
	} else if (login_config.min_group_id_to_connect >= 0 &&
			   login_config.group_id_to_connect == -1 &&
			   sd->group_id < login_config.min_group_id_to_connect) {
		ShowStatus(
			"Connection refused: the minimum group id required for connection is %d (account: %s, "
			"group: %d).\n",
	...
	if (server_num == 0) {	// if no char-server, don't send void list of servers, just disconnect
							// the player with proper message
...

	{
		...
		if (data) {	 // account is already marked as online!
			if (data->char_server >
				-1) {  // Request char servers to kick this account out. [Skotlex]
				uint8 buf[6];
				...
				if (data->waiting_disconnect == INVALID_TIMER)
					data->waiting_disconnect =
						add_timer(gettick() + AUTH_TIMEOUT, login_waiting_disconnect_timer,
								  sd->account_id, 0);
				logclif_sent_auth_result(fd, 8);  // 08 = Server still recognizes your last login
				return;
			} else if (data->char_server ==
					   -1) {  // client has authed but did not access char-server yet
	...
	// memcpy(WFIFOP(fd,20), sd->lastlogin, 24); // in old version, that was for name (not more
	// used)
	memset(WFIFOP(fd, 20), 0, 24);
	WFIFOW(fd, 44) = 0;	 // unknown
	WFIFOB(fd, 46) = sex_str2num(sd->sex);
#if PACKETVER >= 20170315
	safestrncpy(WFIFOCP(fd, 47), sd->web_auth_token,
				WEB_AUTH_TOKEN_LENGTH);	 // web authentication token
#endif
	for (i = 0, n = 0; i < ARRAYLENGTH(ch_server); ++i) {
		...
		WFIFOW(fd, header + n * size + 4) =
			ntows(htons(ch_server[i].port));  // [!] LE byte order here [!]
 ...
 * @param result: nb (msg define in conf)
	...
	101 = Account has been locked for a hacking investigation. Please contact the GM Team for more
 information 102 = This account has been temporarily prohibited from login due to a bug-related
 investigation 103 = This character is being deleted. Login is temporarily unavailable for the time
 being 104 = This character is being deleted. Login is temporarily unavailable for the time being
 ...
 *  S 0277 <version>.L <username>.24B <password>.24B <clienttype>.B <ip address>.16B <adapter
 * address>.13B S 02b0 <version>.L <username>.24B <password>.24B <clienttype>.B <ip address>.16B
 * <adapter address>.13B <g_isGravityID>.B S 01dd <version>.L <username>.24B <password hash>.16B
 * <clienttype>.B S 01fa <version>.L <username>.24B <password hash>.16B <clienttype>.B <?>.B(index
 * of the connection in the clientinfo file (+10 if the command-line contains "pc")) S 027c
 * <version>.L <username>.24B <password hash>.16B <clienttype>.B <?>.13B(junk) S 0825 <packetsize>.W
 * <version>.L <clienttype>.B <userid>.24B <password>.27B <mac>.17B <ip>.15B <token>.(packetsize -
 * 0x5C)B
 * @param fd: fd to parse from (client fd)
...
static int logclif_parse_reqauth(int fd, struct login_session_data* sd, int command, char* ip) {
	...
	if ((command == 0x0064 && packet_len < 55) || (command == 0x0277 && packet_len < 84) ||
		(command == 0x02b0 && packet_len < 85) || (command == 0x01dd && packet_len < 47) ||
		(command == 0x01fa && packet_len < 48) || (command == 0x027c && packet_len < 60) ||
		...
		uint8 clienttype;
		bool israwpass =
			(command == 0x0064 || command == 0x0277 || command == 0x02b0 || command == 0x0825);
		...
		if (command == 0x0825) {
			...
			if (uAccLen > NAME_LENGTH - 1 || uAccLen == 0 || uTokenLen > NAME_LENGTH - 1 ||
				uTokenLen == 0) {
...
static int logclif_parse_reqcharconnec(int fd, struct login_session_data* sd, char* ip) {
	if (RFIFOREST(fd) < 86)
		...
		ShowInfo(
			"Connection request of the char-server '%s' @ %u.%u.%u.%u:%u (account: '%s', ip: "
			"'%s')\n",
			server_name, CONVIP(server_ip), server_port, sd->userid, ip);
		sprintf(message, "charserver - %s@%u.%u.%u.%u:%u", server_name, CONVIP(server_ip),
				server_port);
		login_log(session[fd]->client_addr, sd->userid, 100, message);

		result = login_mmo_auth(sd, true);
		if (runflag == LOGINSERVER_ST_RUNNING && result == -1 && sd->sex == 'S' &&
			sd->account_id < ARRAYLENGTH(ch_server) &&
			!session_isValid(ch_server[sd->account_id].fd)) {
			ShowStatus("Connection of the char-server '%s' accepted.\n", server_name);
			safestrncpy(ch_server[sd->account_id].name, server_name,
						sizeof(ch_server[sd->account_id].name));
...
int logclif_parse(int fd) {
	...
	while (RFIFOREST(fd) >= 2) {
		...
		switch (command) {
			...
			case 0x0277:  // S 0277 <version>.L <username>.24B <password>.24B <clienttype>.B <ip
						  // address>.16B <adapter address>.13B
			case 0x02b0:  // S 02b0 <version>.L <username>.24B <password>.24B <clienttype>.B <ip
						  // address>.16B <adapter address>.13B <g_isGravityID>.B
			// request client login (md5-hashed password)
			case 0x01dd:  // S 01dd <version>.L <username>.24B <password hash>.16B <clienttype>.B
			case 0x01fa:  // S 01fa <version>.L <username>.24B <password hash>.16B <clienttype>.B
						  // <?>.B(index of the connection in the clientinfo file (+10 if the
						  // command-line contains "pc"))
			case 0x027c:  // S 027c <version>.L <username>.24B <password hash>.16B <clienttype>.B
						  // <?>.13B(junk)
			case 0x0825:  // S 0825 <packetsize>.W <version>.L <clienttype>.B <userid>.24B
						  // <password>.27B <mac>.17B <ip>.15B <token>.(packetsize - 0x5C)B
 ...
 */
void do_init_loginclif(void) { return; }

/**
 * loginclif destructor
 *  dealloc..., function called at exit of the login-serv
 */
void do_final_loginclif(void) { return; }
