...
const std::string PetDatabase::getDefaultLocation() { return std::string(db_path) + "/pet_db.yml"; }
...
uint64 PetDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Fullness")) {
		...
		if (fullness > 100) {
			this->invalidWarning(node["Fullness"],
								 "Fullness %hu exceeds maximum of 100. Capping...\n", fullness);
			fullness = 100;
		}
	...
	if (this->nodeExists(node, "IntimacyStart")) {
		...
		if (start > 10000) {
			this->invalidWarning(node["IntimacyStart"],
								 "IntimacyStart %hu exceeds maximum of 10000. Capping...\n", start);
			start = 10000;
		}
	...
	if (this->nodeExists(node, "CaptureRate")) {
		...
		if (rate > 10000) {
			this->invalidWarning(node["CaptureRate"],
								 "CaptureRate %hu exceeds maximum of 10000. Capping...\n", rate);
			rate = 10000;
		}
	...
	if (this->nodeExists(node, "AttackRate")) {
		...
		if (rate > 10000) {
			this->invalidWarning(node["AttackRate"],
								 "AttackRate %hu exceeds maximum of 10000. Capping...\n", rate);
			rate = 10000;
		}
	...
	if (this->nodeExists(node, "RetaliateRate")) {
		...
		if (rate > 10000) {
			this->invalidWarning(node["RetaliateRate"],
								 "RetaliateRate %hu exceeds maximum of 10000. Capping...\n", rate);
			rate = 10000;
		}
	...
	if (this->nodeExists(node, "ChangeTargetRate")) {
		...
		if (rate > 10000) {
			this->invalidWarning(node["ChangeTargetRate"],
								 "ChangeTargetRate %hu exceeds maximum of 10000. Capping...\n",
								 rate);
			rate = 10000;
		}
	...
	if (this->nodeExists(node, "SupportScript")) {
		...
		pet->pet_support_script = parse_script(script.c_str(), this->getCurrentFile().c_str(),
											   this->getLineNumber(node["SupportScript"]),
	...
	if (this->nodeExists(node, "Evolution")) {
		...
		for (const auto &evolutionNode : evolutionsNode) {
			...
			if (mob == nullptr) {
				this->invalidWarning(evolutionNode["Target"],
									 "Evolution target %s does not exist.\n", target_name.c_str());
				return 0;
			}
			...
			if (!this->nodeExists(evolutionNode, "ItemRequirements")) {
				this->invalidWarning(evolutionNode,
									 "Missing required node \"ItemRequirements\".\n");
				return 0;
			}

			std::shared_ptr<s_pet_evo_data> evolution =
				util::umap_find(pet->evolution_data, targetId);
			...
			for (const auto &requirementNode : requirementsNode) {
				...
				if (item == nullptr) {
					this->invalidWarning(requirementNode["Item"],
										 "Evolution requirement item %s does not exist.\n",
										 item_name.c_str());
					return 0;
				}
				...
				if (amount > MAX_AMOUNT) {
					this->invalidWarning(requirementNode["Amount"],
										 "Amount %u exceeds the maximum of %d.\n", amount,
										 MAX_AMOUNT);
					return 0;
				}
...
int pet_attackskill(struct pet_data *pd, int target_id) {
	...
	if (rnd() % 100 < (pd->a_skill->rate + pd->pet.intimate * pd->a_skill->bonusrate /
											   1000)) {	 // Skotlex: Use pet's skill
		int inf;
		...
		if (inf & INF_GROUND_SKILL)
			...
			unit_skilluse_id(&pd->bl, (inf & INF_SELF_SKILL ? pd->bl.id : bl->id), pd->a_skill->id,
							 pd->a_skill->lv);
...
int pet_target_check(struct pet_data *pd, struct block_list *bl, int type) {
	...
	if (bl == NULL || bl->type != BL_MOB || bl->prev == NULL ||
		...
		pd->pet.hungry <= PET_HUNGRY_NONE || pd->pet.class_ == status_get_class(bl))
		return 0;
...
int pet_sc_check(struct map_session_data *sd, int type) {
	...
	if (pd == NULL || (battle_config.pet_equip_required && pd->pet.equip == 0) ||
		pd->recovery == NULL || pd->recovery->timer != INVALID_TIMER || pd->recovery->type != type)
		return 1;

	pd->recovery->timer =
		add_timer(gettick() + pd->recovery->delay * 1000, pet_recovery_timer, sd->bl.id, 0);
...
int pet_birth_process(struct map_session_data *sd, struct s_pet *pet) {
	...
	if (save_settings & CHARSAVE_PET)
		chrif_save(
			sd,
			CSAVE_INVENTORY);  // is it REALLY Needed to save the char for hatching a pet? [Skotlex]
...
int pet_select_egg(struct map_session_data *sd, short egg_index) {
	...
	std::shared_ptr<s_pet_db> pet =
		pet_db_search(sd->inventory.u.items_inventory[egg_index].nameid, PET_EGG);
	if (!pet) {
	...
	if (sd->inventory.u.items_inventory[egg_index].card[0] == CARD0_PET)
		intif_request_petdata(sd->status.account_id, sd->status.char_id,
							  MakeDWord(sd->inventory.u.items_inventory[egg_index].card[1],
										sd->inventory.u.items_inventory[egg_index].card[2]));
	else
...
int pet_catch_process2(struct map_session_data *sd, int target_id) {
	...
	if (!md || md->bl.type != BL_MOB ||
		md->bl.prev == NULL) {	// Invalid inputs/state, abort capture.
	...
	// FIXME: delete taming item here, if this was an item-invoked capture and the item was flagged
	// as delay-consume [ultramage]

	std::shared_ptr<s_pet_db> pet = pet_db.find(md->mob_id);

	// If the target is a valid pet, we have a few exceptions
	if (pet) {
		// catch_target_class == PET_CATCH_UNIVERSAL is used for universal lures (except bosses for
		// now). [Skotlex]
		if (sd->catch_target_class == PET_CATCH_UNIVERSAL &&
			!status_has_mode(&md->status, MD_STATUSIMMUNE)) {
			sd->catch_target_class = md->mob_id;
			// catch_target_class == PET_CATCH_UNIVERSAL_ITEM is used for catching any monster
			// required the lure item used
		} else if (sd->catch_target_class == PET_CATCH_UNIVERSAL_ITEM &&
				   sd->itemid == pet->itemID) {
			sd->catch_target_class = md->mob_id;
		}
	}
	...
	if (battle_config.pet_distance_check &&
		distance_bl(&sd->bl, &md->bl) > battle_config.pet_distance_check) {
		clif_pet_roulette(sd, 0);
		sd->catch_target_class = PET_CATCH_FAIL;
	...
	if (battle_config.pet_legacy_formula) {
		pet_catch_rate =
			(pet->capture + (sd->status.base_level - md->level) * 30 + sd->battle_status.luk * 20) *
			(200 - get_percentage(md->status.hp, md->status.max_hp)) / 100;
	} else {
		pet_catch_rate =
			pet->capture +
			((100 - get_percentage(md->status.hp, md->status.max_hp)) * pet->capture) / 100;
	}
	...
	if (rnd() % 10000 < pet_catch_rate) {
		...
		intif_create_pet(sd->status.account_id, sd->status.char_id, pet->class_, mdb->lv,
...
bool pet_get_egg(uint32 account_id, short pet_class, int pet_id) {
	...
	tmp_item.card[3] |=
		pet_get_card3_intimacy(pet->intimate);	// Store intimacy status based on initial intimacy
...
int pet_equipitem(struct map_session_data *sd, int index) {
	...
	if (battle_config.pet_equip_required) {	 // Skotlex: start support timers if need
		...
		if (pd->s_skill && pd->s_skill->timer == INVALID_TIMER) {
			if (pd->s_skill->id)
				pd->s_skill->timer = add_timer(tick + pd->s_skill->delay * 1000,
											   pet_skill_support_timer, sd->bl.id, 0);
			else
				pd->s_skill->timer =
					add_timer(tick + pd->s_skill->delay * 1000, pet_heal_timer, sd->bl.id, 0);
		}

		if (pd->bonus && pd->bonus->timer == INVALID_TIMER)
			pd->bonus->timer =
				add_timer(tick + pd->bonus->delay * 1000, pet_skill_bonus_timer, sd->bl.id, 0);
	}
...
static int pet_ai_sub_hard(struct pet_data *pd, struct map_session_data *sd, t_tick tick) {
	...
	if (pd->ud.attacktimer != INVALID_TIMER || pd->ud.skilltimer != INVALID_TIMER ||
		pd->bl.m != sd->bl.m)
		return 0;
	...
	if (!target && pd->loot && pd->loot->count < pd->loot->max &&
		...
		map_foreachinallrange(pet_ai_sub_hard_lootsearch, &pd->bl, pd->db->range2 / 2, BL_ITEM, pd,
							  &target);
	}
	...
	if (!target) {	// Just walk around.
		...
		if (pd->ud.walktimer != INVALID_TIMER &&
			check_distance_blxy(&sd->bl, pd->ud.to_x, pd->ud.to_y, 3))
			return 0;  // Already walking to him
...
TIMER_FUNC(pet_skill_bonus_timer) {
	...
	if (pd->state.skillbonus && pd->bonus->delay > 0) {
		bonus = 0;
		timer =
			pd->bonus->delay * 1000;  // the duration until pet bonuses will be reactivated again
...
TIMER_FUNC(pet_heal_timer) {
	...
	if (pc_isdead(sd) || (rate = get_percentage(status->sp, status->max_sp)) > pd->s_skill->sp ||
		...
		pd->s_skill->timer =
			add_timer(gettick() + (rate > 10 ? rate : 10) * 100, pet_heal_timer, sd->bl.id, 0);
		return 0;
	}
...
TIMER_FUNC(pet_skill_support_timer) {
	...
	if (pc_isdead(sd) || (rate = get_percentage(status->sp, status->max_sp)) > pd->s_skill->sp ||
		...
		pd->s_skill->timer =
			add_timer(tick + (rate > 10 ? rate : 10) * 100, pet_skill_support_timer, sd->bl.id, 0);
		return 0;
	}
	...
	pet_stop_walking(pd, 1);
	pd->s_skill->timer =
		add_timer(tick + pd->s_skill->delay * 1000, pet_skill_support_timer, sd->bl.id, 0);
...
int pet_egg_search(struct map_session_data *sd, int pet_id) {
	for (int i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].card[0] == CARD0_PET &&
			pet_id == MakeDWord(sd->inventory.u.items_inventory[i].card[1],
								sd->inventory.u.items_inventory[i].card[2]))
			return i;
	}
	return -1;
}
 ...
 * @return True on success or false otherwise
 */
bool pet_addautobonus(std::vector<std::shared_ptr<s_petautobonus>> &bonus,
					  const std::string &script, int16 rate, uint32 dur, uint16 flag,
 ...
 */
void pet_delautobonus(map_session_data &sd, std::vector<std::shared_ptr<s_petautobonus>> &bonus,
					  bool restore) {
 ...
 */
void pet_exeautobonus(map_session_data &sd, std::vector<std::shared_ptr<s_petautobonus>> *bonus,
	...
	autobonus->timer =
		add_timer(gettick() + autobonus->duration, pet_endautobonus, sd.bl.id, (intptr_t)bonus);
...
TIMER_FUNC(pet_endautobonus) {
	...
	std::vector<std::shared_ptr<s_petautobonus>> *bonus =
		(std::vector<std::shared_ptr<s_petautobonus>> *)data;
...
void do_init_pet(void) {
	...
	item_drop_list_ers =
		ers_new(sizeof(struct item_drop_list), "pet.cpp::item_drop_list_ers", ERS_OPT_NONE);
