...
static int cashshop_read_db_sql(void) {
	...
	for (fi = 0; fi < ARRAYLENGTH(cash_db_name); ++fi) {
		...
		if (SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `tab`, `item_id`, `price` FROM `%s`",
								   cash_db_name[fi])) {
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			...
			if (!cashshop_parse_dbrow(str, 3, lines)) {
				ShowError(
					"cashshop_read_db_sql: Cannot process table '%s' at line '%d', skipping...\n",
...
static bool sale_parse_dbrow(char* fields[], int columns, int current) {
	...
	ARR_FIND(0, cash_shop_items[CASHSHOP_TAB_SALE].count, i,
			 cash_shop_items[CASHSHOP_TAB_SALE].item[i]->nameid == nameid);

	if (i == cash_shop_items[CASHSHOP_TAB_SALE].count) {
		ShowWarning(
			"sale_parse_dbrow: ID %u is not registered in the limited tab in line '%d', "
			"skipping...\n",
			nameid, current);
		return false;
	}
	...
	if (start >= end) {
		ShowWarning(
			"sale_parse_dbrow: Sale for item %u was ignored, because the timespan was not "
			"correct.\n",
			nameid);
		return false;
	}
...
static void sale_read_db_sql(void) {
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle,
							   "SELECT `nameid`, UNIX_TIMESTAMP(`start`), UNIX_TIMESTAMP(`end`), "
							   "`amount` FROM `%s` WHERE `end` > now()",
							   sales_table)) {
		Sql_ShowDebug(mmysql_handle);
		return;
	}
...
static TIMER_FUNC(sale_start_timer) {
	...
	// Init sale end
	sale_item->timer_end = add_timer(gettick() + (unsigned int)(sale_item->end - time(NULL)) * 1000,
									 sale_end_timer, 0, (intptr_t)sale_item);
...
enum e_sale_add_result sale_add_item(t_itemid nameid, int32 count, time_t from, time_t to) {
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle,
							   "INSERT INTO `%s`(`nameid`,`start`,`end`,`amount`) VALUES ( '%u', "
							   "FROM_UNIXTIME(%d), FROM_UNIXTIME(%d), '%d' )",
	...
	sale_item->timer_start = add_timer(gettick() + (unsigned int)(from - time(NULL)) * 1000,
									   sale_start_timer, 0, (intptr_t)sale_item);
	sale_item->timer_end = INVALID_TIMER;
...
bool sale_remove_item(t_itemid nameid) {
	...
	if (SQL_ERROR ==
		Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `nameid` = '%u'", sales_table, nameid)) {
		Sql_ShowDebug(mmysql_handle);
		return false;
	}
...
static void cashshop_read_db(void) {
	...
	if (SQL_ERROR == Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `end` < FROM_UNIXTIME(%d)",
							   sales_table, (uint32)now)) {
		Sql_ShowDebug(mmysql_handle);
	}
	...
	for (i = 0; i < sale_items.count; i++) {
		...
		if (it->start > now) {
			it->timer_start = add_timer(gettick() + (unsigned int)(it->start - time(NULL)) * 1000,
										sale_start_timer, 0, (intptr_t)it);
		} else {
 ...
 * Checks if the transaction is valid and if the user has enough inventory space to receive the
 * item. If yes, take cashpoints and give items; else return clif_error.
 ...
 */
bool cashshop_buylist(struct map_session_data* sd, uint32 kafrapoints, int n,
					  struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST_sub* item_list) {
	uint32 totalcash = 0;
	...
	for (i = 0; i < n; ++i) {
		...
		ARR_FIND(0, cash_shop_items[tab].count, j,
				 nameid == cash_shop_items[tab].item[j]->nameid ||
					 nameid == itemdb_viewid(cash_shop_items[tab].item[j]->nameid));
		...
		nameid = item_list[i].itemId =
			cash_shop_items[tab].item[j]->nameid;  // item_avail replacement
		...
		if (quantity > 99) {
			// Client blocks buying more than 99 items of the same type at the same time, this means
			// someone forged a packet with a higher quantity
			clif_cashshop_result(sd, nameid, CASHSHOP_RESULT_ERROR_UNKNOWN);
			return false;
		}
			...
			if (!pet_create_egg(sd, nameid)) {
				...
				switch (pc_additem(sd, &item_tmp, get_amt, LOG_TYPE_CASH)) {
					...
					case ADDITEM_OVERAMOUNT:
						clif_cashshop_result(sd, nameid,
											 CASHSHOP_RESULT_ERROR_OVER_PRODUCT_TOTAL_CNT);
						return false;
