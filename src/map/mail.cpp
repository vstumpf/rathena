...
void mail_getattachment(struct map_session_data *sd, struct mail_message *msg, int zeny,
						struct item *item) {
	int i;
	...
	for (i = 0; i < MAIL_MAX_ITEM; i++) {
		if (item[i].nameid > 0 && item[i].amount > 0) {
			...
			// If it is a pet egg and the card data does not contain a pet id or other special ids
			// are set
			if (pet != nullptr && item[i].card[0] == 0) {
				...
				if (pet_create_egg(sd, item[i].nameid)) {
					...
					// Do not send receive packet so that the mail is still displayed with item
					// attachment
					item_received = false;
 ...
 */
void mail_send(struct map_session_data *sd, const char *dest_name, const char *title,
			   const char *body_msg, int body_len) {
	...
	if (battle_config.mail_daily_count) {
		...
		if (sd->sc.data[SC_DAILYSENDMAILCNT] == NULL ||
			sd->sc.data[SC_DAILYSENDMAILCNT]->val2 >= battle_config.mail_daily_count) {
			clif_Mail_send(sd, WRITE_MAIL_FAILED_CNT);
			return;
		} else {
			sc_start2(&sd->bl, &sd->bl, SC_DAILYSENDMAILCNT, 100, date_get_dayofyear(),
