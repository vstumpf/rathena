...
enum e_CASHSHOP_ACK : uint8_t {
	...
	ERROR_TYPE_ITEM_ID =
		5,					///< The Purchase has failed because the Item Information was incorrect.
...
enum e_party_invite_reply {
	...
	PARTY_REPLY_DUAL,			 ///< result=4 : "Character in the same account already joined." ->
								 ///< MsgStringTable[608]
	...
	PARTY_REPLY_INVALID_MAPPROPERTY_ME,	 ///< return=9 : !TODO "Cannot join a party in this map" ->
										 ///< MsgStringTable[1871] (since 20110205)
};
...
enum map_property : uint8_t {  // clif_map_property
	...
	MAPPROPERTY_PKSERVERZONE = 4,  // message "You are in a PK area. Please beware of sudden
								   // attacks." in color 0x9B9BFF (light red)
...
enum useskill_fail_cause : uint8_t {  // clif_skill_fail
	...
	// CAUTION: client uses unidentified display name for displaying the required item. Still broken
	// on 2017-05-31 [Lemongrass]
...
enum clif_messages : uint16_t {
	...
	ITEM_PARTY_NO_MEMBER_IN_MAP =
		0x4c6,	///< "There is no party member to summon in the current map."
	MERC_MSG_BASE = 0x4f2,
	...
	PARTY_MASTER_CHANGE_SAME_MAP =
		0x82e,	///< "It is only possible to change the party leader while on the same map."
	...
	GUILD_MASTER_DELAY =
		0xb94,	/// <"You have to wait for one day before delegating a new Guild leader"
...
enum e_damage_type : uint8_t {
	DMG_NORMAL = 0,		   /// damage [ damage: total damage, div: amount of hits, damage2: assassin
						   /// dual-wield damage ]
...
int clif_damage(struct block_list *src, struct block_list *dst, t_tick tick, int sdelay, int ddelay,
				int64 sdamage, int div, enum e_damage_type type, int64 sdamage2,
				bool spdamage);	 // area
...
void clif_refreshlook(struct block_list *bl, int id, int type, int val,
					  enum send_target target);				  // area specified in 'target'
...
void clif_spiritball(struct block_list *bl, struct block_list *target = nullptr,
					 enum send_target send_target = AREA);
void clif_soulball(struct map_session_data *sd, struct block_list *target = nullptr,
				   enum send_target send_target = AREA);
void clif_servantball(struct map_session_data &sd, struct block_list *target = nullptr,
					  enum send_target send_target = AREA);
void clif_abyssball(struct map_session_data &sd, struct block_list *target = nullptr,
					enum send_target send_target = AREA);
...
void clif_showvendingboard(struct map_session_data &sd, enum send_target target = AREA_WOS,
...
void clif_specialeffect(struct block_list *bl, int type,
						enum send_target target);  // special effects [Valaris]
...
#define clif_messagecolor(bl, color, msg, rgb2bgr, type) \
	clif_messagecolor_target(bl, color, msg, rgb2bgr, type, NULL)  // Mob/Npc color talk [SnakeDrak]
...
#define clif_menuskill_clear(sd) \
...
void clif_camerainfo(struct map_session_data *sd, bool show, float range = 0.0f,
					 float rotation = 0.0f, float latitude = 0.0f);
...
void clif_broadcast_refine_result(struct map_session_data &sd, t_itemid itemId, int8 level,
								  bool success);
