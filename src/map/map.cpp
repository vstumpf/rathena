...
static DBMap *nick_db =
	NULL;  /// uint32 char_id -> struct charid2nick* (requested names of offline characters)
...
int console = 0;
int enable_spy = 0;	 // To enable/disable @spy commands, which consume too much cpu time when
					 // sending packets. [Skotlex]
int enable_grf =
	0;	// To enable/disable reading maps from GRF files, bypassing mapcache [blackhole89]
 ...
 *------------------------------------------*/
void map_setusers(int users) { map_users = users; }

int map_getusers(void) { return map_users; }

/*==========================================
 * server player count (this mapserver only)
 *------------------------------------------*/
int map_usercount(void) { return pc_db->size(pc_db); }
 ...
 *------------------------------------------*/
int map_freeblock_lock(void) { return ++block_free_lock; }
...
static void map_addblcell(struct block_list *bl) {
	...
	if (bl->m < 0 || bl->x < 0 || bl->x >= mapdata->xs || bl->y < 0 || bl->y >= mapdata->ys ||
		!(bl->type & BL_CHAR))
		return;
...
int map_moveblock(struct block_list *bl, int x1, int y1, t_tick tick) {
	...
	if (bl->type & BL_CHAR) {
		...
		if (bl->type == BL_PC && ((TBL_PC *)bl)->shadowform_id) {  // Shadow Form Target Moving
			...
			if ((d_bl = map_id2bl(((TBL_PC *)bl)->shadowform_id)) == NULL ||
		...
		if (sc && sc->count) {
			if (sc->data[SC_DANCING])
				skill_unit_move_unit_group(skill_id2group(sc->data[SC_DANCING]->val2), bl->m,
										   x1 - x0, y1 - y0);
			else {
				if (sc->data[SC_CLOAKING] && sc->data[SC_CLOAKING]->val1 < 3 &&
					!skill_check_cloaking(bl, NULL))
					status_change_end(bl, SC_CLOAKING);
				if (sc->data[SC_WARM])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_WARM]->val4), bl->m,
											   x1 - x0, y1 - y0);
				if (sc->data[SC_BANDING])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_BANDING]->val4), bl->m,
											   x1 - x0, y1 - y0);

				if (sc->data[SC_NEUTRALBARRIER_MASTER])
					skill_unit_move_unit_group(
						skill_id2group(sc->data[SC_NEUTRALBARRIER_MASTER]->val2), bl->m, x1 - x0,
						y1 - y0);
				else if (sc->data[SC_STEALTHFIELD_MASTER])
					skill_unit_move_unit_group(
						skill_id2group(sc->data[SC_STEALTHFIELD_MASTER]->val2), bl->m, x1 - x0,
						y1 - y0);

				if (sc->data[SC__SHADOWFORM]) {	 // Shadow Form Caster Moving
					struct block_list *d_bl;
					if ((d_bl = map_id2bl(sc->data[SC__SHADOWFORM]->val2)) == NULL ||
						!check_distance_bl(bl, d_bl, 10))
				...
				if (sc->data[SC_PROPERTYWALK] &&
					sc->data[SC_PROPERTYWALK]->val3 <
						skill_get_maxcount(sc->data[SC_PROPERTYWALK]->val1,
										   sc->data[SC_PROPERTYWALK]->val2) &&
					map_find_skill_unit_oncell(bl, bl->x, bl->y, SO_ELECTRICWALK, NULL, 0) ==
						NULL &&
					map_find_skill_unit_oncell(bl, bl->x, bl->y, NPC_ELECTRICWALK, NULL, 0) ==
						NULL &&
					...
					skill_unitsetting(bl, sc->data[SC_PROPERTYWALK]->val1,
									  sc->data[SC_PROPERTYWALK]->val2, x0, y0, 0)) {
					sc->data[SC_PROPERTYWALK]->val3++;
				}
			}
			/* Guild Aura Moving */
			if (bl->type == BL_PC && ((TBL_PC *)bl)->state.gmaster_flag) {
				if (sc->data[SC_LEADERSHIP])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_LEADERSHIP]->val4), bl->m,
											   x1 - x0, y1 - y0);
				if (sc->data[SC_GLORYWOUNDS])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_GLORYWOUNDS]->val4),
											   bl->m, x1 - x0, y1 - y0);
				if (sc->data[SC_SOULCOLD])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_SOULCOLD]->val4), bl->m,
											   x1 - x0, y1 - y0);
				if (sc->data[SC_HAWKEYES])
					skill_unit_move_unit_group(skill_id2group(sc->data[SC_HAWKEYES]->val4), bl->m,
											   x1 - x0, y1 - y0);
			}
		}
	} else if (bl->type == BL_NPC)
	...
	if (type & ~BL_MOB)
		for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != NULL; bl = bl->next)
			if (bl->x == x && bl->y == y && bl->type & type) {
				if (bl->type ==
					BL_NPC) {  // Don't count hidden or invisible npc. Cloaked npc are counted
					npc_data *nd = BL_CAST(BL_NPC, bl);
					if (nd->bl.m < 0 || nd->sc.option & (OPTION_HIDE | OPTION_INVISIBLE) ||
 ...
 */
struct skill_unit *map_find_skill_unit_oncell(struct block_list *target, int16 x, int16 y,
											  uint16 skill_id, struct skill_unit *out_unit,
											  int flag) {
	int16 bx, by;
	struct block_list *bl;
 ...
 *------------------------------------------*/
int map_foreachinrangeV(int (*func)(struct block_list *, va_list), struct block_list *center,
						int16 range, int type, va_list ap, bool wall_check) {
	int bx, by, m;
...
#endif
						&& (!wall_check || path_search_long(NULL, center->m, center->x, center->y,
															bl->x, bl->y, CELL_CHKWALL)) &&
	...
	for (i = blockcount; i < bl_list_count; i++) {
		if (bl_list[i]->prev) {	 // func() may delete this bl_list[] slot, checking for prev ensures
								 // it wasn't queued for deletion.
			va_copy(ap_copy, ap);
...
int map_foreachinrange(int (*func)(struct block_list *, va_list), struct block_list *center,
					   int16 range, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
	returnCount =
		map_foreachinrangeV(func, center, range, type, ap, battle_config.skill_wall_check > 0);
	va_end(ap);
	return returnCount;
}

int map_foreachinallrange(int (*func)(struct block_list *, va_list), struct block_list *center,
						  int16 range, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
...
/*==========================================
 * Same as foreachinrange, but there must be a shoot-able range between center and target to be
 *counted in. [Skotlex]
 *------------------------------------------*/
int map_foreachinshootrange(int (*func)(struct block_list *, va_list), struct block_list *center,
							int16 range, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
 ...
 *------------------------------------------*/
int map_foreachinareaV(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
					   int16 x1, int16 y1, int type, va_list ap, bool wall_check) {
	int bx, by, cx, cy;
...
int map_foreachinallarea(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
						 int16 x1, int16 y1, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
...
int map_foreachinshootarea(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
						   int16 x1, int16 y1, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
...
int map_foreachinarea(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
					  int16 x1, int16 y1, int type, ...) {
	int returnCount = 0;
	va_list ap;
	va_start(ap, type);
	returnCount =
		map_foreachinareaV(func, m, x0, y0, x1, y1, type, ap, battle_config.skill_wall_check > 0);
	va_end(ap);
	return returnCount;
}
 ...
 *------------------------------------------*/
int map_forcountinrange(int (*func)(struct block_list *, va_list), struct block_list *center,
						int16 range, int count, int type, ...) {
	int bx, by, m;
	int returnCount = 0;  // total sum of returned values of func() [Skotlex]
	struct block_list *bl;
	int blockcount = bl_list_count, i;
...
int map_forcountinarea(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
					   int16 x1, int16 y1, int count, int type, ...) {
	int bx, by;
	int returnCount = 0;  // total sum of returned values of func() [Skotlex]
	struct block_list *bl;
	int blockcount = bl_list_count, i;
 ...
 *------------------------------------------*/
int map_foreachinmovearea(int (*func)(struct block_list *, va_list), struct block_list *center,
						  int16 range, int16 dx, int16 dy, int type, ...) {
	int bx, by, m;
	int returnCount = 0;  // total sum of returned values of func() [Skotlex]
	struct block_list *bl;
	int blockcount = bl_list_count, i;
	...
	if (dx == 0 || dy == 0) {
		...
		for (by = y0 / BLOCK_SIZE; by <= y1 / BLOCK_SIZE; by++) {
			for (bx = x0 / BLOCK_SIZE; bx <= x1 / BLOCK_SIZE; bx++) {
				...
				if (type & BL_MOB) {
					for (bl = mapdata->block_mob[bx + by * mapdata->bxs]; bl != NULL;
						 bl = bl->next) {
						if (bl->x >= x0 && bl->x <= x1 && bl->y >= y0 && bl->y <= y1 &&
							bl_list_count < BL_LIST_MAX)
							bl_list[bl_list_count++] = bl;
...
//
int map_foreachincell(int (*func)(struct block_list *, va_list), int16 m, int16 x, int16 y,
					  int type, ...) {
	int bx, by;
	int returnCount = 0;  // total sum of returned values of func() [Skotlex]
	struct block_list *bl;
 ...
 *------------------------------------------------------------*/
int map_foreachinpath(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
					  int16 x1, int16 y1, int16 range, int length, int type, ...) {
	int returnCount = 0;  // total sum of returned values of func() [Skotlex]
	//////////////////////////////////////////////////////////////
	//
			...
			for (bx = mx0 / BLOCK_SIZE; bx <= mx1 / BLOCK_SIZE; bx++) {
				for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != NULL; bl = bl->next) {
					if (bl->prev && bl->type & type && bl_list_count < BL_LIST_MAX) {
						...
						if (k < 0 || k > len_limit)	 // Since more skills use this, check for ending
													 // point as well.
							continue;

						if (k > magnitude2 &&
							!path_search_long(NULL, m, x0, y0, xi, yi, CELL_CHKWALL))
							continue;  // Targets beyond the initial ending point need the wall
									   // check.

						// All these shifts are to increase the precision of the intersection point
						// and distance considering how it's int math.
 ...
 * The path goes into one of the eight directions and the direction is determined by the given
 *coordinates. The path has a length, a width and an offset. The cost for diagonal movement is the
 *same as for horizontal/vertical movement.
 * @param m: ID of map
 * @param x0: Start X
 * @param y0: Start Y
 ...
 * @param type: Type of bl to search for
 *------------------------------------------*/
int map_foreachindir(int (*func)(struct block_list *, va_list), int16 m, int16 x0, int16 y0,
					 int16 x1, int16 y1, int16 range, int length, int offset, int type, ...) {
	int returnCount = 0;  // Total sum of returned values of func()
				...
				for (bl = mapdata->block[bx + by * mapdata->bxs]; bl != NULL; bl = bl->next) {
					if (bl->prev && bl->type & type && bl_list_count < BL_LIST_MAX) {
						...
						if (battle_config.skill_eightpath_same_cell == 0 && rx == 0 && ry == 0)
							continue;
						// This turns it so that the area that is hit is always with positive rx and
						// ry
						rx *= dx;
						ry *= dy;
	...
	for (free_cell = 0, i = -1; i <= 1; i++) {
		...
		for (j = -1; j <= 1; j++) {
			...
			if (map_getcell(m, j + *x, i + *y, CELL_CHKNOPASS) &&
				!map_getcell(m, j + *x, i + *y, CELL_CHKICEWALL))
				continue;
 ...
 *------------------------------------------*/
int map_search_freecell(struct block_list *src, int16 m, int16 *x, int16 *y, int16 rx, int16 ry,
						int flag) {
	int tries, spawn = 0;
	int bx, by;
	int rx2 = 2 * rx + 1;
	...
	if (!src && (!(flag & 1) || flag & 2)) {
		ShowDebug(
			"map_search_freecell: Incorrect usage! When src is NULL, flag has to be &1 and can't "
			"have &2\n");
		return 0;
	}
		...
		if (map_getcell(m, *x, *y, CELL_CHKREACH)) {
			...
			if (flag & 4) {
				...
				if (spawn++ < battle_config.no_spawn_on_player &&
					map_foreachinallarea(map_count_sub, m, *x - AREA_SIZE, *y - AREA_SIZE,
 ...
 * @param first_charid : 1st player that could loot the item (only charid that could loot for
 *first_get_tick duration)
 * @param second_charid :  2nd player that could loot the item (2nd charid that could loot for
 *second_get_charid duration)
 * @param third_charid : 3rd player that could loot the item (3rd charid that could loot for
 *third_get_charid duration)
 ...
 *------------------------------------------*/
int map_addflooritem(struct item *item, int amount, int16 m, int16 x, int16 y, int first_charid,
					 int second_charid, int third_charid, int flags, unsigned short mob_id,
					 bool canShowEffect) {
	int r;
	...
	fitem->first_get_tick = gettick() + (flags & 1 ? battle_config.mvp_item_first_get_time
												   : battle_config.item_first_get_time);
	fitem->second_get_charid = second_charid;
	fitem->second_get_tick =
	...
	fitem->cleartimer = add_timer(gettick() + battle_config.flooritem_lifetime,
								  map_clearflooritem_timer, fitem->bl.id, 0);
...
void map_delnickdb(int charid, const char *name) {
	...
	if (!nick_db->remove(nick_db, db_i2key(charid), &data) ||
		(p = (struct charid2nick *)db_data2ptr(&data)) == NULL)
		return;
...
int map_quit(struct map_session_data *sd) {
	...
	if (!sd->state.active) {  // Removing a player that is not active.
		...
		// Non-active players should not have loaded any data yet (or it was cleared already) so no
		// additional cleanups are needed.
		return 0;
	}
	...
	if (sd->sc.count) {
		for (const auto &it : status_db) {
			...
			if (flag[SCF_NOSAVEINFINITE] && sd->sc.data[it.first] &&
				sd->sc.data[it.first]->val4 > 0) {
	...
	if (sd->ed)	 // Remove effects here rather than unit_remove_map_pc so we don't clear on
				 // Teleport/map change.
 ...
 **/
bool map_blid_exists(int id) { return (idb_exists(id_db, id)); }
...
struct mob_data *map_getmob_boss(int16 m) {
	...
	for (md = (struct mob_data *)dbi_first(iter); dbi_exists(iter);
		 md = (struct mob_data *)dbi_next(iter)) {
		if (md->bl.m == m) {
			found = true;
			break;
		}
...
void map_foreachpc(int (*func)(struct map_session_data *sd, va_list args), ...) {
	...
	for (sd = (struct map_session_data *)dbi_first(iter); dbi_exists(iter);
...
void map_foreachnpc(int (*func)(struct npc_data *nd, va_list args), ...) {
	...
	for (bl = (struct block_list *)dbi_first(iter); dbi_exists(iter);
...
struct block_list *mapit_first(struct s_mapiterator *mapit) {
	...
	for (bl = (struct block_list *)dbi_first(mapit->dbi); bl != NULL;
...
struct block_list *mapit_last(struct s_mapiterator *mapit) {
	...
	for (bl = (struct block_list *)dbi_last(mapit->dbi); bl != NULL;
...
static int map_instancemap_clean(struct block_list *bl, va_list ap) {
	...
	switch (bl->type) {
		/*case BL_PC:
		// BL_PET, BL_HOM, BL_MER, and BL_ELEM are moved when BL_PC warped out in
		map_instancemap_leave map_quit((struct map_session_data *) bl); break;*/
		case BL_NPC:
...
void map_removemobs(int16 m) {
	...
	mapdata->mob_delete_timer =
		add_timer(gettick() + battle_config.mob_remove_delay, map_removemobs_timer, m, 0);
}
...
int map_random_dir(struct block_list *bl, int16 *x, int16 *y) {
	...
	do {
		...
		short segment =
			1 + (rnd() % dist);	 // Pick a random interval from the whole vector in that direction
		xi = bl->x + segment * dirx[j];
		segment = (short)sqrt(
			(float)(dist2 - segment * segment));  // The complement of the previously picked segment
		yi = bl->y + segment * diry[j];
	} while ((map_getcell(bl->m, xi, yi, CELL_CHKNOPASS) ||
			  !path_search(NULL, bl->m, bl->x, bl->y, xi, yi, 1, CELL_CHKNOREACH)) &&
			 (++i) < 100);
...
bool map_iwall_exist(const char *wall_name) { return strdb_exists(iwall_db, wall_name); }
...
bool map_iwall_set(int16 m, int16 x, int16 y, int size, int8 dir, bool shootable,
...
void map_iwall_get(struct map_session_data *sd) {
	...
	for (iwall = (struct iwall_data *)dbi_first(iter); dbi_exists(iter);
		 iwall = (struct iwall_data *)dbi_next(iter)) {
		if (iwall->m != sd->bl.m)
			continue;

		for (i = 0; i < iwall->size; i++) {
			map_iwall_nextxy(iwall->x, iwall->y, iwall->dir, i, &x1, &y1);
			clif_changemapcell(sd->fd, iwall->m, x1, y1,
							   map_getcell(iwall->m, x1, y1, CELL_GETTYPE), SELF);
		}
	}
	dbi_destroy(iter);
}
...
bool map_iwall_remove(const char *wall_name) {
	...
	for (i = 0; i < iwall->size; i++) {
		...
		clif_changemapcell(0, iwall->m, x1, y1, map_getcell(iwall->m, x1, y1, CELL_GETTYPE),
						   ALL_SAMEMAP);
	}
...
int map_setipport(unsigned short mapindex, uint32 ip, uint16 port) {
	...
	mdos = (struct map_data_other_server *)uidb_ensure(map_db, (unsigned int)mapindex,
													   create_map_data_other_server);

	if (mdos->cell)	 // Local map,Do nothing. Give priority to our own local maps over ones from
					 // another server. [Skotlex]
		return 0;
	if (ip == clif_getip() && port == clif_getport()) {
		// That's odd, we received info that we are the ones with this map, but... we don't have it.
		ShowFatalError(
			"map_setipport : received info that this map-server SHOULD have map '%s', but it is "
			"not loaded.\n",
...
int map_readfromcache(struct map_data *m, char *buffer, char *decode_buffer) {
	...
	if (info && i < header->map_count) {
		...
		// TO-DO: Maybe handle the scenario, if the decoded buffer isn't the same size as expected?
		// [Shinryo]
		decode_zip(decode_buffer, &size, p + sizeof(struct map_cache_map_info), info->len);

		CREATE(m->cell, struct mapcell, size);

		for (xy = 0; xy < size; ++xy) m->cell[xy] = map_gat2cell(decode_buffer[xy]);
...
int map_addmap(char *mapname) {
	...
	if (map_num >= MAX_MAP_PER_SERVER - 1) {
		ShowError("Could not add map '" CL_WHITE "%s" CL_RESET
				  "', the limit of maps has been reached.\n",
				  mapname);
		return 1;
	}
...
static void map_delmapid(int id) {
	...
	for (int i = id; i < map_num - 1; i++) map[i] = map[i + 1];
	map_num--;
}
 ...
 *--------------------------------------*/
void map_addmap2db(struct map_data *m) { uidb_put(map_db, (unsigned int)m->index, m); }

void map_removemapdb(struct map_data *m) { uidb_remove(map_db, (unsigned int)m->index); }
...
int parse_console(const char *buf) {
	...
	if (strcmpi("admin", type) == 0) {
		...
		if (strcmpi("shutdown", command) == 0 || strcmpi("exit", command) == 0 ||
		...
		ShowInfo(
			"\t admin:@<atcommand> => Uses an atcommand. Do NOT use commands requiring an attached "
			"player.\n");
		ShowInfo(
			"\t admin:map:<map> <x> <y> => Changes the map from which console commands are "
			"executed.\n");
...
int map_sql_init(void) {
	...
	if (SQL_ERROR == Sql_Connect(mmysql_handle, map_server_id.c_str(), map_server_pw.c_str(),
								 map_server_ip.c_str(), map_server_port, map_server_db.c_str()) ||
		SQL_ERROR == Sql_Connect(qsmysql_handle, map_server_id.c_str(), map_server_pw.c_str(),
								 map_server_ip.c_str(), map_server_port, map_server_db.c_str())) {
		ShowError("Couldn't connect with uname='%s',host='%s',port='%d',database='%s'\n",
				  map_server_id.c_str(), map_server_ip.c_str(), map_server_port,
				  map_server_db.c_str());
...
int log_sql_init(void) {
	...
	ShowInfo("" CL_WHITE "[SQL]" CL_RESET ": Connecting to the Log Database " CL_WHITE "%s" CL_RESET
			 " At " CL_WHITE "%s" CL_RESET "...\n",
			 log_db_db.c_str(), log_db_ip.c_str());
	if (SQL_ERROR == Sql_Connect(logmysql_handle, log_db_id.c_str(), log_db_pw.c_str(),
	...
	ShowStatus("" CL_WHITE "[SQL]" CL_RESET ": Successfully '" CL_GREEN "connected" CL_RESET
			   "' to Database '" CL_WHITE "%s" CL_RESET "'.\n",
			   log_db_db.c_str());
	...
	switch (mapflag) {
		...
		case MF_PVP:
			...
			if (!status) {
				...
				if (mapdata->flag[MF_GVG]) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set GvG and PvP flags for the same map! "
						"Removing GvG flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_TE]) {
					mapdata->flag[MF_GVG_TE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG TE and PvP flags for the same map! "
						"Removing GvG TE flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_DUNGEON]) {
					mapdata->flag[MF_GVG_DUNGEON] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG Dungeon and PvP flags for the same map! "
						"Removing GvG Dungeon flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_CASTLE]) {
					mapdata->flag[MF_GVG_CASTLE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and PvP flags for the same map! "
						"Removing GvG Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_TE_CASTLE]) {
					mapdata->flag[MF_GVG_TE_CASTLE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG TE Castle and PvP flags for the same "
						"map! Removing GvG TE Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_BATTLEGROUND]) {
					mapdata->flag[MF_BATTLEGROUND] = false;
					ShowWarning(
						"map_setmapflag: Unable to set Battleground and PvP flags for the same "
						"map! Removing Battleground flag from %s.\n",
						mapdata->name);
				}
			}
			break;
		...
		case MF_GVG_TE:
			...
			if (!status) {
				...
				if (mapdata->flag[MF_PVP]) {
					...
					if (!battle_config.pk_mode)
						ShowWarning(
							"map_setmapflag: Unable to set PvP and GvG flags for the same map! "
							"Removing PvP flag from %s.\n",
							mapdata->name);
				}
				if (mapdata->flag[MF_BATTLEGROUND]) {
					mapdata->flag[MF_BATTLEGROUND] = false;
					ShowWarning(
						"map_setmapflag: Unable to set Battleground and GvG flags for the same "
						"map! Removing Battleground flag from %s.\n",
						mapdata->name);
				}
			}
		...
		case MF_GVG_TE_CASTLE:
			if (status) {
				if (mapflag == MF_GVG_CASTLE && mapdata->flag[MF_GVG_TE_CASTLE]) {
					mapdata->flag[MF_GVG_TE_CASTLE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG TE Castle and GvG Castle flags for the "
						"same map! Removing GvG TE Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapflag == MF_GVG_TE_CASTLE && mapdata->flag[MF_GVG_CASTLE]) {
					mapdata->flag[MF_GVG_CASTLE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and GvG TE Castle flags for the "
						"same map! Removing GvG Castle flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_PVP]) {
					mapdata->flag[MF_PVP] = false;
					if (!battle_config.pk_mode)
						ShowWarning(
							"npc_parse_mapflag: Unable to set PvP and GvG%s Castle flags for the "
							"same map! Removing PvP flag from %s.\n",
		...
		case MF_GVG_DUNGEON:
			if (status && mapdata->flag[MF_PVP]) {
				...
				if (!battle_config.pk_mode)
					ShowWarning(
						"map_setmapflag: Unable to set PvP and GvG Dungeon flags for the same map! "
						"Removing PvP flag from %s.\n",
						mapdata->name);
			}
			mapdata->flag[mapflag] = status;
		...
		case MF_NOJOBEXP:
			if (status) {
				if (mapflag == MF_NOBASEEXP && mapdata->flag[MF_BEXP] != 100) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set BEXP and No Base EXP flags for the same "
						"map! Removing BEXP flag from %s.\n",
						mapdata->name);
				}
				if (mapflag == MF_NOJOBEXP && mapdata->flag[MF_JEXP] != 100) {
					mapdata->flag[MF_JEXP] = false;
					ShowWarning(
						"map_setmapflag: Unable to set JEXP and No Job EXP flags for the same map! "
						"Removing JEXP flag from %s.\n",
						mapdata->name);
				}
			}
			mapdata->flag[mapflag] = status;
		...
		case MF_PVP_NIGHTMAREDROP:
			if (status) {
				...
				if (mapdata->drop_list.size() == MAX_DROP_PER_MAP) {
					ShowWarning(
						"map_setmapflag: Reached the maximum number of drop list items for mapflag "
						"pvp_nightmaredrop on %s. Skipping.\n",
						mapdata->name);
					break;
				}
		...
		case MF_BEXP:
			if (status) {
				...
				if (mapflag == MF_JEXP && mapdata->flag[MF_NOJOBEXP]) {
					...
					ShowWarning(
						"map_setmapflag: Unable to set No Job EXP and JEXP flags for the same map! "
						"Removing No Job EXP flag from %s.\n",
						mapdata->name);
				}
				if (mapflag == MF_BEXP && mapdata->flag[MF_NOBASEEXP]) {
					mapdata->flag[MF_NOBASEEXP] = false;
					ShowWarning(
						"map_setmapflag: Unable to set No Base EXP and BEXP flags for the same "
						"map! Removing No Base EXP flag from %s.\n",
						mapdata->name);
				}
		...
		case MF_BATTLEGROUND:
			if (status) {
				...
				if (mapdata->flag[MF_PVP]) {
					...
					if (!battle_config.pk_mode)
						ShowWarning(
							"map_setmapflag: Unable to set PvP and Battleground flags for the same "
							"map! Removing PvP flag from %s.\n",
							mapdata->name);
				}
				if (mapdata->flag[MF_GVG]) {
					mapdata->flag[MF_GVG] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG and Battleground flags for the same "
						"map! Removing GvG flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_DUNGEON]) {
					mapdata->flag[MF_GVG_DUNGEON] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG Dungeon and Battleground flags for the "
						"same map! Removing GvG Dungeon flag from %s.\n",
						mapdata->name);
				}
				if (mapdata->flag[MF_GVG_CASTLE]) {
					mapdata->flag[MF_GVG_CASTLE] = false;
					ShowWarning(
						"map_setmapflag: Unable to set GvG Castle and Battleground flags for the "
						"same map! Removing GvG Castle flag from %s.\n",
						mapdata->name);
				}
				mapdata->flag[mapflag] =
					((args->flag_val <= 0 || args->flag_val > 2) ? 1 : args->flag_val);
			} else
				mapdata->flag[mapflag] = false;
			break;
		case MF_NOLOOT:
		...
		case MF_SKILL_DAMAGE:
			if (!status) {
				...
				if (!args->flag_val) {	// Signifies if it's a single skill or global damage
										// adjustment
					if (args->skill_damage.caster == 0) {
						ShowError(
							"map_setmapflag: Skill damage adjustment without casting type for map "
							"%s.\n",
							mapdata->name);
						return false;
					}
		...
		case MF_SKILL_DURATION:
			if (!status)
				...
				map_skill_duration_add(mapdata, args->skill_duration.skill_id,
									   args->skill_duration.per);
			}
			mapdata->flag[mapflag] = status;
			break;
		default:
...
void do_final(void) {
	...
	for (struct map_session_data *sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter);
...
void do_abort(void) {
	...
	if (run) {
		ShowFatalError(
			"Server has crashed while trying to save characters. Character data can't be saved!\n");
		return;
	}
	run = 1;
	if (!chrif_isconnected()) {
		if (pc_db->size(pc_db))
			ShowFatalError(
				"Server has crashed without a connection to the char-server, %u characters can't "
				"be saved!\n",
				pc_db->size(pc_db));
		return;
	}
 ...
 *------------------------------------------------------*/
void set_server_type(void) { SERVER_TYPE = ATHENA_SERVER_MAP; }
...
void map_do_final_msg(void) {
	...
	for (mdb = (struct msg_data *)dbi_first(iter); dbi_exists(iter);
		 mdb = (struct msg_data *)dbi_next(iter)) {
...
int do_init(int argc, char *argv[]) {
	...
	if (save_settings == CHARSAVE_NONE)
		ShowWarning(
			"Value of 'save_settings' is not set, player's data only will be saved every "
			"'autosave_time' (%d seconds).\n",
	...
	iwall_db =
		strdb_alloc(DB_OPT_RELEASE_DATA, 2 * NAME_LENGTH + 2 + 1);	// [Zephyrus] Invisible Walls
...
#ifndef MAP_GENERATOR
	ShowStatus("Server is '" CL_GREEN "ready" CL_RESET "' and listening on port '" CL_WHITE
			   "%d" CL_RESET "'.\n\n",
			   map_port);
#else
	...
	if (console) {	// start listening
		...
		add_timer_interval(gettick() + 1000, parse_console_timer, 0, 0,
