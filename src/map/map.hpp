...
#define MAX_MAP_SIZE \
	512 * 512  // Wasn't there something like this already? Can't find it.. [Shinryo]
...
// Super Novices are considered the 2-1 version of the novice! Novices are considered a first class
// type, too...
enum e_mapid : uint64 {
	// Novice And 1-1 Jobs
	MAPID_NOVICE = 0x0,
...
// meant to make it easier to check for multiple types at a time on invocations such as map_foreach*
// calls [Skotlex]
enum bl_type : uint16 {
	BL_NUL = 0x000,
	BL_PC = 0x001,
...
static std::unordered_map<std::string, e_element> um_eleid2elename{
	{"Neutral", ELE_NEUTRAL}, {"Water", ELE_WATER},	  {"Earth", ELE_EARTH}, {"Fire", ELE_FIRE},
	{"Wind", ELE_WIND},		  {"Poison", ELE_POISON}, {"Holy", ELE_HOLY},	{"Dark", ELE_DARK},
	{"Ghost", ELE_GHOST},	  {"Undead", ELE_UNDEAD},
};
...
struct mapcell {
	...
	// dynamic flags
	unsigned char npc : 1, basilica : 1, landprotector : 1, novending : 1, nochat : 1,
		maelstrom : 1, icewall : 1, nobuyingstore : 1;
...
struct map_data {
	...
	struct mapcell *cell;  // Holds the information of each map cell (NULL if the map is not on this
						   // map-server).
	...
	std::unordered_map<uint16, s_skill_damage>
		skill_damage;  // Used for single skill damage adjustment
	...
	/* speeds up clif_updatestatus processing by causing hpmeter to run only when someone with the
...
inline bool mapdata_flag_vs(struct map_data *mapdata) {
	...
	if (mapdata->flag[MF_PVP] || mapdata->flag[MF_GVG_DUNGEON] || mapdata->flag[MF_GVG] ||
		((agit_flag || agit2_flag) && mapdata->flag[MF_GVG_CASTLE]) || mapdata->flag[MF_GVG_TE] ||
		(agit3_flag && mapdata->flag[MF_GVG_TE_CASTLE]) || mapdata->flag[MF_BATTLEGROUND])
		return true;
...
inline bool mapdata_flag_vs2(struct map_data *mapdata) {
	...
	if (mapdata->flag[MF_PVP] || mapdata->flag[MF_GVG_DUNGEON] || mapdata->flag[MF_GVG] ||
		mapdata->flag[MF_GVG_CASTLE] || mapdata->flag[MF_GVG_TE] ||
		mapdata->flag[MF_GVG_TE_CASTLE] || mapdata->flag[MF_BATTLEGROUND])
		return true;
...
inline bool mapdata_flag_gvg(struct map_data *mapdata) {
	...
	if (mapdata->flag[MF_GVG] || ((agit_flag || agit2_flag) && mapdata->flag[MF_GVG_CASTLE]) ||
		mapdata->flag[MF_GVG_TE] || (agit3_flag && mapdata->flag[MF_GVG_TE_CASTLE]))
		return true;
...
inline bool mapdata_flag_gvg2(struct map_data *mapdata) {
	...
	if (mapdata->flag[MF_GVG] || mapdata->flag[MF_GVG_TE] || mapdata->flag[MF_GVG_CASTLE] ||
		mapdata->flag[MF_GVG_TE_CASTLE])
		return true;
...
inline bool mapdata_flag_ks(struct map_data *mapdata) {
	...
	if (mapdata->flag[MF_TOWN] || mapdata->flag[MF_PVP] || mapdata->flag[MF_GVG] ||
		mapdata->flag[MF_GVG_TE] || mapdata->flag[MF_BATTLEGROUND])
		return true;
...
/**
 * Check if map is GVG maps exclusion for item, skill, and status restriction check (regardless of
 * agit_flag status)
 * @param mapdata: Map Data
 * @return True on success or false otherwise
 * @author Cydh
...
struct skill_unit *map_find_skill_unit_oncell(struct block_list *, int16 x, int16 y,
											  uint16 skill_id, struct skill_unit *, int flag);
// search and creation
...
#define BL_CAST(type_, bl)                                                           \
	(((bl) == (struct block_list *)NULL || (bl)->type != (type_)) ? (T##type_ *)NULL \
																  : (T##type_ *)(bl))
