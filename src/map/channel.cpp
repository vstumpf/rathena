...
struct Channel *channel_create(struct Channel *tmp_chan) {
	...
	safestrncpy(channel->name, tmp_chan->name,
				CHAN_NAME_LENGTH);	// Store channel cname without '#'
	safestrncpy(channel->alias, tmp_chan->alias[0] ? tmp_chan->alias : tmp_chan->name,
				CHAN_NAME_LENGTH);
	if (!tmp_chan->pass[0])
		channel->pass[0] = '\0';
	else
 ...
 * @return NULL on failure or Channel on success
 */
struct Channel *channel_create_simple(char *name, char *pass, enum Channel_Type chantype,
									  unsigned int owner) {
...
int channel_delete(struct Channel *channel, bool force) {
	if (!channel)
		return -1;
	if (!force && channel->type == CHAN_TYPE_PUBLIC &&
		runflag == MAPSERVER_ST_RUNNING)  // only delete those serv stop
		return -2;
	if (db_size(channel->users)) {
		...
		for (sd = (struct map_session_data *)dbi_first(iter); dbi_exists(iter);
			 sd = (struct map_session_data *)dbi_next(iter)) {	// for all users
...
int channel_join(struct Channel *channel, struct map_session_data *sd) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) &&
		!channel_pccheckgroup(channel, sd->group_id))
		return -2;

	if (channel_haspcbanned(channel, sd) == 1) {
		char output[CHAT_SIZE_MAX];
		sprintf(output, msg_txt(sd, 1438),
				channel->name);	 // You're currently banned from the '%s' channel.
		clif_displaymessage(sd->fd, output);
		return -3;
	}

	if (channel->type == CHAN_TYPE_PRIVATE &&
		db_size(channel->users) >= channel_config.private_channel.max_member) {
		char output[CHAT_SIZE_MAX];
		sprintf(output, msg_txt(sd, 760), channel->name,
				channel_config.private_channel
	...
	if (sd->stealth) {
		...
		char output[CHAT_SIZE_MAX];
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd, 761), channel->alias,
					 sd->status.name);	// %s %s has joined.
...
int channel_mjoin(struct map_session_data *sd) {
	...
	if (mapdata->channel->opt & CHAN_OPT_ANNOUNCE_SELF) {
		sprintf(mout, msg_txt(sd, 1435), mapdata->channel->name,
				mapdata->name);	 // You're now in the '#%s' channel for '%s'.
	...
	for (i = 0; i < MAX_GUILDALLIANCE; i++) {
		...
		if (ga->guild_id && (ga->opposition == 0) && (ag = guild_search(ga->guild_id))) {
			for (j = 0; j < ag->max_member; j++) {	// load all guildmember
				...
				if (channel_haspc(ag->channel, pl_sd) ==
					1)	// only if they are in their own guildchan
	...
	if (flag & 2) {
		...
		for (i = 0; i < MAX_GUILDALLIANCE; i++) {
			...
			if (ga->guild_id && (ga->opposition == 0) &&
				(ag = guild_search(ga->guild_id)))	// only join allies
 ...
 * @param sd: Player data
 * @param type: Quit type (1 - Quit guild channel, 2 - Quit ally channel, 4 - Quit map channel, 8 -
 * Quit all users in channel)
 * @return
 *  0: Success
 * -1: Invalid player
 */
int channel_pcquit(struct map_session_data *sd, int type) {
	...
	// he quit a map_server, not call in map_quit cause we need to cleanup when we switch map-server
	// as well
	if (!sd)
		return -1;

	// Leave all chat channels.
	if (type & (1 | 2) && channel_config.ally_tmpl.name[0] &&
		sd->guild) {  // quit guild and ally chan
	...
	if (type & 4 && channel_config.map_tmpl.name[0] &&
		channel_haspc(mapdata->channel, sd) == 1) {	 // quit map chan
...
int channel_send(struct Channel *channel, struct map_session_data *sd, const char *msg) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN) && channel->msg_delay != 0 &&
		...
		if ((channel->opt & CHAN_OPT_COLOR_OVERRIDE) && sd->fontcolor &&
			sd->fontcolor < channel_config.colors_count && channel_config.colors[sd->fontcolor])
 ...
 * @param type: Check types (1 - Check name # + length, 2 - Check if channel already exists, 4 -
...
int channel_chk(char *chname, char *chpass, int type) {
	if (type & 1) {	 // check name
		...
		if (strlen(chname) < 3 || strlen(chname) > CHAN_NAME_LENGTH)
			return -2;
		if ((type & 2) && (strcmpi(chname + 1, channel_config.map_tmpl.name) == 0 ||
 ...
 * @param flag: Lookup types (1 - Create channel if it does not exist (map or ally only), 2 - Join
 * the channel if not joined yet (map or ally only))
...
struct Channel *channel_name2channel(char *chname, struct map_session_data *sd, int flag) {
	...
	if (sd && strcmpi(chname + 1, channel_config.map_tmpl.name) == 0) {
		...
		if (flag & 1 && !sd->guild->channel)
			sd->guild->channel =
				channel_create_simple(NULL, NULL, CHAN_TYPE_ALLY, sd->guild->guild_id);
		if (flag & 2 && channel_pc_haschan(sd, mapdata->channel) < 1)
			channel_gjoin(sd, 3);
	...
	if (options[0] != '\0' && strcmpi(options, "colors") == 0) {
		...
		if (!sd->channel_count)
			...
			for (k = 0; k < sd->channel_count; k++) {
				...
				sprintf(output, msg_txt(sd, 1409), channel->name,
						db_size(channel->users));  // - #%s (%d users)
				clif_displaymessage(sd->fd, output);
			}
		}
		...
		if (channel_config.map_tmpl.name[0] && mapdata->channel) {
			sprintf(output, msg_txt(sd, 1409), mapdata->channel->name,
					db_size(mapdata->channel->users));	// - #%s (%d users)
			clif_displaymessage(sd->fd, output);
		}
		if (channel_config.ally_tmpl.name[0] && sd->status.guild_id) {
			struct guild *g = sd->guild;
			if (g && g->channel) {
				sprintf(output, msg_txt(sd, 1409), g->channel->name,
						db_size(((struct Channel *)g->channel)->users));  // - #%s (%d users)
				clif_displaymessage(sd->fd, output);
			}
		}
...
int channel_pccreate(struct map_session_data *sd, char *chname, char *chpass) {
	...
	if (res == 0) {	 // success
		struct Channel *channel =
			channel_create_simple(chname, chpass, CHAN_TYPE_PRIVATE, sd->status.char_id);
...
int channel_pcleave(struct map_session_data *sd, char *chname) {
	...
	if (!channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE)) {
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd, 763), channel->alias,
...
int channel_pcjoin(struct map_session_data *sd, char *chname, char *pass) {
	...
	if (channel) {
		...
		if (channel_haspc(channel, sd) == 1) {
			...
			if (strcmp(channel->pass, pass) != 0) {	 // wrong pass entry
				if (pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
					...
					sprintf(output, msg_txt(sd, 1401), chname,
							"@join");  // Channel '%s' is password-protected (usage: %s
		...
		sprintf(output, msg_txt(sd, 1400), chname,
...
int channel_pccolor(struct map_session_data *sd, char *chname, char *color) {
	...
	ARR_FIND(0, channel_config.colors_count, k,
			 (strcmpi(color, channel_config.colors_name[k]) == 0));
	...
	sprintf(output, msg_txt(sd, 1413), chname,
			channel_config.colors_name[k]);	 // '%s' channel color updated to '%s'.
	clif_displaymessage(sd->fd, output);
	return 0;
}
 ...
 * @param sd: Player data
 * @param chname: Channel name
 * @return 0 on success, -1 on incorrect channel name, or -2 channel doesn't exist or player didn't
 * join
 */
int channel_pcbind(struct map_session_data *sd, char *chname) {
	...
	sd->gcbind = channel;
	sprintf(output, msg_txt(sd, 1431),
			chname);  // Your global chat is now binded to the '%s' channel.
	clif_displaymessage(sd->fd, output);
	return 0;
}
...
int channel_pcunbind(struct map_session_data *sd) {
	...
	if (sd->gcbind == NULL) {
		clif_displaymessage(sd->fd,
							msg_txt(sd, 1432));	 // Your global chat is not binded to any channel.
		return -1;
	}
	sprintf(output, msg_txt(sd, 1433),
			sd->gcbind->name);	// Your global chat is now unbinded from the '#%s' channel.
	clif_displaymessage(sd->fd, output);
	sd->gcbind = NULL;
	return 0;
}
...
int channel_pcban(struct map_session_data *sd, char *chname, char *pname, int flag) {
	...
	if (flag != 2 && flag != 3) {
		...
		if (!flag && banned == 1) {
			sprintf(output, msg_txt(sd, 1465),
					tsd->status.name);	// Player '%s' is already banned from this channel.
			clif_displaymessage(sd->fd, output);
			return -1;
		} else if (flag == 1 && banned == 0) {
			sprintf(output, msg_txt(sd, 1440),
					tsd->status.name);	// Player '%s' is not banned from this channel.
			clif_displaymessage(sd->fd, output);
			return -1;
		}
	} else {
	...
	switch (flag) {
		case 0: {
			...
			sprintf(output, msg_txt(sd, 1437), tsd->status.name,
					chname);  // Player '%s' is banned from the '%s' channel.
			break;
		}
		case 1:
			if (!tsd)
			...
			sprintf(output, msg_txt(sd, 1441), tsd->status.name,
					chname);  // Player '%s' is unbanned from the '%s' channel.
			break;
		case 2:
...
int channel_pckick(struct map_session_data *sd, char *chname, char *pname) {
	...
	if (!pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		if (channel->char_id != sd->status.char_id) {
			...
			sprintf(output, msg_txt(sd, 766),
					chname);  // You cannot kick a player from channel '%s'.
			clif_displaymessage(sd->fd, output);
		}
		return -1;
	}
	...
	if (!channel_config.closing && (channel->opt & CHAN_OPT_ANNOUNCE_LEAVE)) {
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(sd, 768), channel->alias,
					 tsd->status.name);	 // %s %s has been kicked.
		clif_channel_msg(channel, output, channel->color);
	}
 ...
 * @return 0 on success or -1 on failure
 */
int channel_pcsetopt(struct map_session_data *sd, char *chname, const char *option,
					 const char *val) {
	struct Channel *channel;
	char output[CHAT_SIZE_MAX];
	int k, s = 0, opt;
	...
	if (sd && channel->char_id != sd->status.char_id &&
	...
	if (val[0] == '\0') {
		if (opt == CHAN_OPT_MSG_DELAY) {
			sprintf(output, msg_txt(sd, 1466),
					opt_str[k]);  // Input the number of seconds (0-10) for the '%s' option.
			clif_displaymessage(sd->fd, output);
			return -1;
		} else if (channel->opt & opt) {
			sprintf(output, msg_txt(sd, 1449), opt_str[k],
					opt_str[k]);  // Option '%s' is already enabled (use '@channel setopt %s 0' to
								  // disable).
			clif_displaymessage(sd->fd, output);
			return -1;
		} else {
			channel->opt |= opt;
			sprintf(output, msg_txt(sd, 1450), opt_str[k],
					channel->name);	 // Option '%s' is enabled for channel '#%s'.
			clif_displaymessage(sd->fd, output);
		}
	} else {
		int v = atoi(val);
		if (opt == CHAN_OPT_MSG_DELAY) {
			if (v < 0 || v > 10) {
				sprintf(output, msg_txt(sd, 1451), v,
						opt_str[k]);  // Value '%d' for option '%s' is out of range (limit 0-10).
				clif_displaymessage(sd->fd, output);
				return -1;
			}
			if (v == 0) {
				channel->opt &= ~opt;
				channel->msg_delay = 0;
				sprintf(output, msg_txt(sd, 1453), opt_str[k], channel->name,
						v);	 // Option '%s' is disabled for channel '#%s'.
				clif_displaymessage(sd->fd, output);
			} else {
				channel->opt |= opt;
				channel->msg_delay = v * 1000;
				sprintf(output, msg_txt(sd, 1452), opt_str[k], channel->name,
						v);	 // Option '%s' is enabled for channel '#%s' at %d seconds.
				clif_displaymessage(sd->fd, output);
			}
		} else {
			if (v) {
				if (channel->opt & opt) {
					sprintf(output, msg_txt(sd, 1449), opt_str[k],
							opt_str[k]);  // Option '%s' is already enabled (use '@channel setopt %s
										  // 0' to disable).
					clif_displaymessage(sd->fd, output);
					return -1;
				} else {
				...
				if (!(channel->opt & opt)) {
					...
					channel->opt &= ~opt;
					sprintf(output, msg_txt(sd, 1453), opt_str[k],
							channel->name);	 // Option '%s' is disabled for channel '#%s'.
					clif_displaymessage(sd->fd, output);
				}
			}
		}
...
bool channel_read_sub(config_setting_t *chan, struct Channel *tmp_chan, uint8 i) {
	...
	int delay = 1000, autojoin = 0, leave = 1, chat = 1, color_override = 0, self_notif = 1,
		join_notif = 0, leave_notif = 0;
	int64 type = CHAN_TYPE_PUBLIC;
	int group_count = 0;
	const char *name = NULL, *password = NULL, *alias = NULL, *color_str = "Default",
			   *type_str = NULL;
	...
	if (config_setting_lookup_string(chan, "type", &type_str) &&
		!script_get_constant(type_str, &type)) {
	...
	tmp_chan->opt =
		(autojoin ? CHAN_OPT_AUTOJOIN : 0) | (leave ? CHAN_OPT_CAN_LEAVE : 0) |
	...
	if (chan_setting != NULL) {
		...
		if (colors != NULL) {
			...
			for (i = 0; i < color_count; i++) {
				...
				safestrncpy(channel_config.colors_name[i], config_setting_name(color),
							CHAN_NAME_LENGTH);
				channel_config.colors[i] =
					strtoul(config_setting_get_string_elem(colors, i), NULL, 0);
				channel_config.colors[i] =
					(channel_config.colors[i] & 0x0000FF) << 16 |
		...
		if (private_channel != NULL) {
			int allow = 1, ban = 1, kick = 1, color_override = 0, change_delay = 0, self_notif = 1,
				join_notif = 0, leave_notif = 0, delay = 1000, max_member = 1000;
...
void do_init_channel(void) {
	channel_db = stridb_alloc(static_cast<DBOptions>(DB_OPT_DUP_KEY | DB_OPT_RELEASE_DATA),
							  CHAN_NAME_LENGTH);
