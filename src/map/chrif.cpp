...
static bool char_init_done =
	false;	// server already initialized? Used for InterInitOnce and vending loadings
...
// 2b02: Outgoing, chrif_charselectreq -> 'player returns from ingame to charserver to select
// another char.., this packets includes sessid etc' ? (not 100% sure) 2b03: Incoming,
// clif_charselectok -> '' (i think its the packet after enterworld?) (not sure) 2b04: Incoming,
// chrif_recvmap -> 'getting maps from charserver of other mapserver's' 2b05: Outgoing,
// chrif_changemapserver -> 'Tell the charserver the mapchange / quest for ok...' 2b06: Incoming,
// chrif_changemapserverack -> 'awnser of 2b05, ok/fail, data: dunno^^' 2b07: Outgoing,
// chrif_removefriend -> 'Tell charserver to remove friend_id from char_id friend list' 2b08:
// Outgoing, chrif_searchcharid -> '...' 2b09: Incoming, map_addchariddb -> 'Adds a name to the nick
// db' 2b0a: Outgoing, chrif_skillcooldown_request -> requesting the list of skillcooldown for char
...
// 2b1e: Incoming, chrif_update_ip -> 'Reqest forwarded from char-server for interserver IP sync.'
// [Lance] 2b1f: Incoming, chrif_disconnectplayer -> 'disconnects a player (aid X) with the message
// XY ... 0x81 ..' [Sirius] 2b20: Incoming, chrif_removemap -> 'remove maps of a server (sample: its
// going offline)' [Sirius] 2b21: Incoming, chrif_save_ack. Returned after a character has been
// "final saved" on the char-server. [Skotlex] 2b22: Incoming, chrif_updatefamelist_ack. Updated one
// position in the fame list. 2b23: Outgoing, chrif_keepalive. charserver ping. 2b24: Incoming,
// chrif_keepalive_ack. charserver ping reply. 2b25: Incoming, chrif_deadopt -> 'Removes baby from
// Father ID and Mother ID' 2b26: Outgoing, chrif_authreq -> 'client authentication request' 2b27:
// Incoming, chrif_authfail -> 'client authentication failed' 2b28: Outgoing, chrif_req_charban ->
// 'ban a specific char ' 2b29: FREE 2b2a: Outgoing, chrif_req_charunban -> 'unban a specific char '
...
int other_mapserver_count =
	0;	// Holds count of how many other map servers are online (apart of this instance) [Skotlex]
...
static bool chrif_auth_logout(TBL_PC *sd, enum sd_state state) {
	if (sd->fd &&
		state ==
			ST_LOGOUT) {  // Disassociate player, and free it after saving ack returns. [Skotlex]
		// fd info must not be lost for ST_MAPCHANGE as a final packet needs to be sent to the
		// player.
		if (session[sd->fd])
...
void chrif_setuserid(char *id) { safestrncpy(userid, id, NAME_LENGTH); }

// sets char-server's password
void chrif_setpasswd(char *pwd) { safestrncpy(passwd, pwd, NAME_LENGTH); }
...
void chrif_checkdefaultlogin(void) {
	...
	if (runflag != CORE_ST_STOP && strcmp(userid, "s1") == 0 && strcmp(passwd, "p1") == 0) {
		...
		ShowNotice(
			...
			"'S')\n");
		ShowNotice(
			"and then edit your user/password in conf/map_athena.conf (or "
...
void chrif_setport(uint16 port) { char_port = port; }

// says whether the char-server is connected or not
int chrif_isconnected(void) { return (session_isValid(char_fd) && chrif_state == 2); }
...
int chrif_save(struct map_session_data *sd, int flag) {
	...
	WFIFOB(char_fd, 12) =
		(flag & CSAVE_QUIT) ? 1 : 0;  // Flag to tell char-server this character is quitting.
...
int chrif_sendmap(int fd) {
	...
	WFIFOW(fd, 0) = 0x2afa;
	for (int i = 0; i < instance_start; i++) WFIFOW(fd, 4 + i * 4) = map[i].index;
...
int chrif_removemap(int fd) {
	...
	for (i = 10, j = 0; i < RFIFOW(fd, 2); i += 4, j++) map_eraseipport(RFIFOW(fd, i), ip, port);
...
/// R 2b06 <account_id>.L <login_id1>.L <login_id2>.L <char_id>.L <map_index>.W <x>.W <y>.W <ip>.L
/// <port>.W
int chrif_changemapserverack(uint32 account_id, int login_id1, int login_id2, uint32 char_id,
							 short map_index, short x, short y, uint32 ip, uint16 port) {
	struct auth_node *node;
...
int chrif_connectack(int fd) {
	if (RFIFOB(fd, 2)) {
		ShowFatalError(
			"Connection to char-server failed %d, please check conf/import/map_conf userid and "
			"passwd.\n",
			RFIFOB(fd, 2));
		exit(EXIT_FAILURE);
	}
	...
	ShowStatus(
		"Successfully logged on to Char Server (Connection: '" CL_WHITE "%d" CL_RESET "').\n", fd);
	chrif_state = 1;
	chrif_connected = 1;
...
static int chrif_reconnect(DBKey key, DBData *data, va_list ap) {
	...
	switch (node->state) {
		case ST_LOGIN:
			if (node->sd &&
				node->char_dat ==
					NULL) {	 // Since there is no way to request the char auth, make it fail.
				pc_authfail(node->sd);
				chrif_char_offline(node->sd);
...
void chrif_authok(int fd) {
	...
	// Causes problems if the currently connected player tries to quit or this data belongs to an
	// already connected player which is trying to re-auth.
	...
	if (runflag == MAPSERVER_ST_RUNNING && node->char_dat == NULL &&
	...
	chrif_char_offline(sd);	 // Set him offline, the char server likely has it set as online
							 // already.
...
void chrif_authfail(
	int fd) { /* HELLO WORLD. ip in RFIFOL 15 is not being used (but is available) */
	...
	if (node != NULL && node->account_id == account_id && node->char_id == char_id &&
		node->login_id1 == login_id1 && node->sex == sex &&
 ...
 */
int chrif_req_login_operation(int aid, const char *character_name, enum chrif_req_op operation_type,
							  int32 timediff, int val1, int val2) {
	chrif_check(-1);
...
int chrif_changesex(struct map_session_data *sd, bool change_account) {
	...
	clif_displaymessage(sd->fd,
						msg_txt(sd, 408));	//"Need disconnection to perform change-sex request..."
 ...
 * NB: That ack is received just after the char has sent the request to login and therefore didn't
 * have login reply yet
...
static void chrif_ack_login_req(int aid, const char *player_name, uint16 type, uint16 answer) {
	...
	switch (type) {
		...
		case CHRIF_OP_LOGIN_UNBAN:
			snprintf(action, 25, "%s",
					 msg_txt(sd, 427 + type));	// block|ban|unblock|unban|change the sex of
			break;
...
int chrif_changedsex(int fd) {
	...
	if (sd) {  // Normally there should not be a char logged on right now!
		...
		// save character
		sd->login_id1++;  // change identify, because if player come back in char within the 5
						  // seconds, he can change its characters do same modify in login-server
						  // for the account, but no in char-server (it ask again login_id1 to
						  // login, and don't remember it)
		clif_displaymessage(
			sd->fd,
			msg_txt(sd, 409));	//"Your sex has been changed (need disconnection by the server)..."
...
/*==========================================
 * Disconnection of a player (account has been banned of has a status, from login/char-server) by
 *[Yor]
 *------------------------------------------*/
int chrif_ban(int fd) {
	...
	sd->login_id1++;  // change identify, because if player come back in char within the 5 seconds,
					  // he can change its characters
	if (res == 0) {
		...
		if (0 < ret_status && ret_status <= 9)
			...
			clif_displaymessage(sd->fd,
								msg_txt(sd, 420));	//"Your account has not more authorised."
	} else if (res == 1 || res == 2) {
		time_t timestamp;
		char tmpstr[256];
		char strtime[25];
		...
		safesnprintf(tmpstr, sizeof(tmpstr), msg_txt(sd, 423), res == 2 ? "char" : "account",
					 strtime);	//"Your %s has been banished until %s "
...
int chrif_save_scdata(struct map_session_data *sd) {  // parses the sc_data of the player and sends
													  // it to the char-server for saving. [Skotlex]
#ifdef ENABLE_SC_SAVING
	int i, count = 0;
	t_tick tick;
	...
	for (i = 0; i < SC_MAX; i++) {
		...
		memcpy(WFIFOP(char_fd, 14 + count * sizeof(struct status_change_data)), &data,
...
int chrif_skillcooldown_save(struct map_session_data *sd) {
	...
	for (i = 0; i < MAX_SKILLCOOLDOWN; i++) {
		...
		memcpy(WFIFOP(char_fd, 14 + count * sizeof(struct skill_cooldown_data)), &data,
...
int chrif_load_scdata(int fd) {
	...
	if (sd->status.char_id != cid) {
		ShowError(
			"chrif_load_scdata: Receiving data for account %d, char id does not matches (%d != "
			"%d)!\n",
	...
	for (i = 0; i < count; i++) {
		...
		status_change_start(
			NULL, &sd->bl, (sc_type)data->type, 10000, data->val1, data->val2, data->val3,
			data->val4, data->tick,
...
int chrif_skillcooldown_load(int fd) {
	...
	if (sd->status.char_id != cid) {
		ShowError(
			"chrif_skillcooldown_load: Receiving data for account %d, char id does not matches (%d "
			"!= %d)!\n",
			aid, sd->status.char_id, cid);
		return -1;
	}
...
void chrif_on_disconnect(void) {
	...
	other_mapserver_count =
		0;	// Reset counter. We receive ALL maps from all map-servers on reconnect.
	map_eraseallipport();
	...
	if ((flag & 0x2))  // isgm
		...
		if ((flag & 0x1)) {	 // isvip
			...
			if (sd->storage.max_amount > MAX_STORAGE) {
				ShowError(
					"intif_parse_ack_vipActive: Storage size for player %s (%d:%d) is larger than "
					"MAX_STORAGE. Storage size has been set to MAX_STORAGE.\n",
...
int chrif_bsdata_save(struct map_session_data *sd, bool quit) {
	...
	if (sd->bonus_script.count) {
		...
		for (node = sd->bonus_script.head; node && i < MAX_PC_BONUS_SCRIPT; node = node->next) {
			...
			safestrncpy(bs.script_str, StringBuf_Value(entry->script_buf),
						StringBuf_Length(entry->script_buf) + 1);
			bs.tick = DIFF_TICK(timer->tick, tick);
			bs.flag = entry->flag;
			bs.type = entry->type;
			bs.icon = entry->icon;
			memcpy(WFIFOP(char_fd, 9 + i * sizeof(struct bonus_script_data)), &bs,
				   sizeof(struct bonus_script_data));
			i++;
		}

		if (i != sd->bonus_script.count && sd->bonus_script.count > MAX_PC_BONUS_SCRIPT)
			ShowWarning(
				"Only allowed to save %d (mmo.hpp::MAX_PC_BONUS_SCRIPT) bonus script each "
				"player.\n",
				MAX_PC_BONUS_SCRIPT);
	}
...
int chrif_bsdata_received(int fd) {
	...
	if ((count = RFIFOB(fd, 8))) {
		...
		for (i = 0; i < count; i++) {
			...
			if (!(entry = pc_bonus_script_add(sd, bs->script_str, bs->tick,
											  (enum efst_type)bs->icon, bs->flag, bs->type)))
				continue;
...
int chrif_parse(int fd) {
	...
	if (session[fd]->flag.eof) {
		...
		if (DIFF_TICK(last_tick, session[fd]->rdata_tick) >
			(stall_time * 2)) { /* we can't wait any longer */
			set_eof(fd);
			return 0;
	...
	while (RFIFOREST(fd) >= 2) {
		int cmd = RFIFOW(fd, 0);
		if (cmd < 0x2af8 || cmd >= 0x2af8 + ARRAYLENGTH(packet_len_table) ||
			...
			ShowWarning(
				"chrif_parse: session #%d, intif_parse failed (unrecognized command 0x%.4x).\n", fd,
				cmd);
			set_eof(fd);
			return 0;
		}
		...
		switch (cmd) {
			...
			case 0x2b04:
				...
				break;
			case 0x2b06:
				chrif_changemapserverack(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10),
										 RFIFOL(fd, 14), RFIFOW(fd, 18), RFIFOW(fd, 20),
										 RFIFOW(fd, 22), RFIFOL(fd, 24), RFIFOW(fd, 28));
				break;
			case 0x2b09:
		...
		if (fd == char_fd)	// There's the slight chance we lost the connection during parse, in
							// which case this would segfault if not checked [Skotlex]
...
void do_init_chrif(void) {
	if (sizeof(struct mmo_charstatus) > 0xFFFF) {
		ShowError("mmo_charstatus size = %" PRIuPTR
				  " is too big to be transmitted. (must be below 0xFFFF)\n",
				  sizeof(struct mmo_charstatus));
		exit(EXIT_FAILURE);
	}

	if (sizeof(struct s_storage) > 0xFFFF) {
		ShowError("s_storage size = %" PRIuPTR
				  " is too big to be transmitted. (must be below 0xFFFF)\n",
	...
	if ((sizeof(struct bonus_script_data) * MAX_PC_BONUS_SCRIPT) > 0xFFFF) {
		ShowError(
			"bonus_script_data size = %d is too big, please reduce MAX_PC_BONUS_SCRIPT (%d) size. "
			"(must be below 0xFFFF).\n",
