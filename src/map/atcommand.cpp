...
#define ACMD_FUNC(x)                                                                         \
	static int atcommand_##x(const int fd, struct map_session_data* sd, const char* command, \
							 const char* message)
...
enum e_atcmd_restict {
	...
	ATCMD_NOSCRIPT = 0x2,	  /// Cannot be used via script command 'atcommand' or 'useatcmd'
							  /// (is_atcommand type 0 and 3)
	ATCMD_NOAUTOTRADE = 0x4,  /// Like ATCMD_NOSCRIPT, but if the player is autotrader. Example:
...
static void atcommand_get_suggestions(struct map_session_data* sd, const char* name,
									  bool atcommand);	// @help
static void warp_get_suggestions(struct map_session_data* sd,
								 const char* name);	 // @rura, @warp, @mapmove
...
ACMD_FUNC(send) {
	...
	if (!message || !*message ||
		...
		int i;
		for (i = 900; i <= 903; ++i) clif_displaymessage(fd, msg_txt(sd, i));
		return -1;
	}
...
#define GET_VALUE(p, num)                                                             \
	...
	if (type > 0 && type < MAX_PACKET_DB) {
		...
		if (len) {	// show packet length
			sprintf(atcmd_output, msg_txt(sd, 904), type,
					packet_db[type].len);  // Packet 0x%x length: %d
			clif_displaymessage(fd, atcmd_output);
			return 0;
		}
					...
					if (*message == '\\') {
						...
						switch (*message) {
							...
							case 'x':  // Hexadecimal
							{
								...
								CHECK_EOS(message);
								if (!ISXDIGIT(*message)) {
									PARSE_ERROR(msg_txt(sd, 907),
												message);  // Not a hexadecimal digit:
									return -1;
								}
								num = (ISDIGIT(*message) ? *message - '0'
														 : TOLOWER(*message) - 'a' + 10);
								if (ISXDIGIT(*message)) {
									++message;
									CHECK_EOS(message);
									num <<= 8;
...
ACMD_FUNC(mapmove) {
	...
	if ((x || y) &&
		map_getcell(
			m, x, y,
			CELL_CHKNOPASS)) {	// This is to prevent the pc_setpos call from printing an error.
	...
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARP) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(
			fd, msg_txt(sd, 248));	// You are not authorized to warp from your current map.
		return -1;
	}
...
ACMD_FUNC(where) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 910));	// Please enter a player name (usage: @where <char name>).
		return -1;
	}
	...
	snprintf(atcmd_output, sizeof atcmd_output, "%s %s %d %d", pl_sd->status.name,
...
ACMD_FUNC(jumpto) {
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name, true)) == NULL &&
		(pl_sd = map_charid2sd(atoi(atcmd_player_name))) == NULL) {
		clif_displaymessage(fd, msg_txt(sd, 3));  // Character not found.
		return -1;
	}

	if (pl_sd->bl.m >= 0 && map_getmapflag(pl_sd->bl.m, MF_NOWARPTO) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(fd, msg_txt(sd, 247));	// You are not authorized to warp to this map.
		return -1;
	}
...
ACMD_FUNC(who) {
	...
	if (strstr(command, "map") != NULL) {
		char map_name[MAP_NAME_LENGTH_EXT] = "";
		if (sscanf(message, "%15s %23s", map_name, player_name) < 1 ||
			(map_id = map_mapname2mapid(map_name)) < 0)
			map_id = sd->bl.m;
	} else {
			...
			switch (display_type) {
				case 2: {
					...
					if (pc_get_group_id(pl_sd) > 0)	 // Player title, if exists
						StringBuf_Printf(&buf, msg_txt(sd, 344),
										 pl_sd->group->name.c_str());  // "(%s) "
					StringBuf_Printf(&buf, msg_txt(sd, 347), pl_sd->status.base_level,
				...
				case 3: {
					if (pc_has_permission(sd, PC_PERM_WHO_DISPLAY_AID))
						StringBuf_Printf(&buf, msg_txt(sd, 912), pl_sd->status.char_id,
										 pl_sd->status.account_id);	 // "(CID:%d/AID:%d) "
					StringBuf_Printf(&buf, msg_txt(sd, 343), pl_sd->status.name);  // "Name: %s "
					if (pc_get_group_id(pl_sd) > 0)	 // Player title, if exists
						StringBuf_Printf(&buf, msg_txt(sd, 344),
										 pl_sd->group->name.c_str());  // "(%s) "
					StringBuf_Printf(&buf, msg_txt(sd, 348), mapindex_id2name(pl_sd->mapindex),
				...
				default: {
					...
					if (pc_get_group_id(pl_sd) > 0)	 // Player title, if exists
						StringBuf_Printf(&buf, msg_txt(sd, 344),
										 pl_sd->group->name.c_str());  // "(%s) "
					if (p != NULL)
						StringBuf_Printf(&buf, msg_txt(sd, 345),
										 p->party.name);  // " | Party: '%s'"
					if (g != NULL)
	...
	if (map_id < 0) {
		...
		if (count == 0)
			...
			StringBuf_Printf(&buf, msg_txt(sd, 56), count,
							 mapdata->name);  // %d players found in map '%s'.
	}
	clif_displaymessage(fd, StringBuf_Value(&buf));
...
ACMD_FUNC(whogm) {
	...
	if (sscanf(message, "%255[^\n]", match_text) < 1)
		strcpy(match_text, "");
	for (j = 0; match_text[j]; j++) match_text[j] = TOLOWER(match_text[j]);
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		...
		if (match_text[0]) {
			...
			for (j = 0; player_name[j]; j++) player_name[j] = TOLOWER(player_name[j]);
		...
		sprintf(atcmd_output, msg_txt(sd, 914),	 // Name: %s (GM:%d) | Location: %s %d %d
				pl_sd->status.name, pl_level, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x,
				pl_sd->bl.y);
		clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(load) {
	...
	if (m >= 0 && map_getmapflag(m, MF_NOWARPTO) && !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(fd,
							msg_txt(sd, 249));	// You are not authorized to warp to your save map.
		return -1;
	}
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARP) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
	...
	pc_setpos(sd, sd->status.save_point.map, sd->status.save_point.x, sd->status.save_point.y,
			  CLR_OUTSIGHT);
...
ACMD_FUNC(speed) {
	...
	if (!message || !*message || sscanf(message, "%6hd", &speed) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 918), MIN_WALK_SPEED,
...
ACMD_FUNC(storage) {
	...
	if (storage_storageopen(sd) == 1) {	 // Already open.
		clif_displaymessage(
			fd, msg_txt(sd, 250));	// You have already opened your storage. Close it first.
		return -1;
	}
...
ACMD_FUNC(guildstorage) {
	...
	switch (storage_guild_storageopen(sd)) {
		...
		case GSTORAGE_ALREADY_OPEN:
			clif_displaymessage(
				fd,
				msg_txt(sd, 251));	// You have already opened your guild storage. Close it first.
			return -1;
		...
		case GSTORAGE_NO_PERMISSION:
			clif_displaymessage(
				fd, msg_txt(sd, 787));	// You do not have permission to use the guild storage.
			return -1;
	}
	return 0;
}
...
ACMD_FUNC(option) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d %11d", &param1, &param2, &param3) < 1 ||
		param1 < 0 || param2 < 0 ||
		param3 < 0) {  // failed to match the parameters so inform the user of the options
		const char* text;
...
ACMD_FUNC(hide) {
	...
	if (pc_isinvisible(sd)) {
		...
		if (!battle_config.pk_mode && map_getmapflag(sd->bl.m, MF_PVP) &&
			!map_getmapflag(sd->bl.m,
							MF_PVP_NOCALCRANK)) {  // register the player for ranking calculations
			sd->pvp_timer = add_timer(gettick() + 200, pc_calc_pvprank_timer, sd->bl.id, 0);
		}
		// bugreport:2266
		map_foreachinmovearea(clif_insight, &sd->bl, AREA_SIZE, sd->bl.x, sd->bl.y, BL_ALL,
							  &sd->bl);
	} else {
		sd->sc.option |= OPTION_INVISIBLE;
...
ACMD_FUNC(jobchange) {
	...
	if (job == JOB_KNIGHT2 || job == JOB_CRUSADER2 || job == JOB_WEDDING || job == JOB_XMAS ||
		job == JOB_SUMMER || job == JOB_HANBOK || job == JOB_OKTOBERFEST ||
		...
		job == JOB_BABY_STAR_GLADIATOR2 || job == JOB_STAR_EMPEROR2 ||
		job == JOB_BABY_STAR_EMPEROR2 || job == JOB_SUMMER2 ||
...
ACMD_FUNC(kami) {
	...
	if (*(command + 5) != 'c' && *(command + 5) != 'C') {
		if (!message || !*message) {
			clif_displaymessage(
				fd, msg_txt(sd, 980));	// Please enter a message (usage: @kami <message>).
			return -1;
		}

		sscanf(message, "%255[^\n]", atcmd_output);
		if (strstr(command, "l") != NULL)
			clif_broadcast(&sd->bl, atcmd_output, strlen(atcmd_output) + 1, BC_DEFAULT,
						   ALL_SAMEMAP);
		else
			intif_broadcast(
...
ACMD_FUNC(heal) {
	...
	if (hp > 0 && sp >= 0) {
		if (!status_heal(&sd->bl, hp, sp, 0))
			clif_displaymessage(fd,
								msg_txt(sd, 157));	// HP and SP are already with the good value.
		else
			clif_displaymessage(fd, msg_txt(sd, 17));  // HP, SP recovered.
		return 0;
	}
...
/*==========================================
 * @item command (usage: @item <itemdid1:itemid2:itemname:..> <quantity>) (modified by [Yor] for
 *pet_egg)
...
ACMD_FUNC(item) {
	...
	if (!strcmpi(parent_cmd, "itembound")) {
		if (!message || !*message ||
			...
			clif_displaymessage(fd,
								msg_txt(sd, 295));	// Please enter an item name or ID (usage: @item
													// <item name/ID> <quantity> <bound_type>).
...
ACMD_FUNC(item2) {
	...
	if (!strcmpi(parent_cmd + 1, "itembound2")) {
		if (!message || !*message ||
			(sscanf(message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name,
					&number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &bound) < 10 &&
			 sscanf(message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d %11d", item_name,
					&number, &identify, &refine, &attr, &c1, &c2, &c3, &c4, &bound) < 10)) {
			clif_displaymessage(
				fd,
				msg_txt(
			...
			clif_displaymessage(
				fd, msg_txt(sd, 297));	//   <identify_flag> <refine> <attribute> <card1> <card2>
										//   <card3> <card4> <bound_type>).
			clif_displaymessage(fd, msg_txt(sd, 298));	// Invalid bound type
			return -1;
		}
		if (bound <= BOUND_NONE || bound >= BOUND_MAX) {
			...
			return -1;
			   ...
			   (sscanf(message, "\"%99[^\"]\" %11d %11d %11d %11d %11d %11d %11d %11d", item_name,
					   &number, &identify, &refine, &attr, &c1, &c2, &c3, &c4) < 9 &&
				sscanf(message, "%99s %11d %11d %11d %11d %11d %11d %11d %11d", item_name, &number,
...
ACMD_FUNC(baselevelup) {
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, msg_txt(sd, 986));	// Please enter a level adjustment (usage:
													// @lvup/@blevel/@baselvlup <number of levels>).
		return -1;
	}
	...
	if (level > 0) {
		...
		if ((unsigned int)level > pc_maxbaselv(sd) ||
			(unsigned int)level >
				pc_maxbaselv(sd) - sd->status.base_level)  // fix positive overflow
...
ACMD_FUNC(joblevelup) {
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd,
							msg_txt(sd, 987));	// Please enter a level adjustment (usage:
												// @joblvup/@jlevel/@joblvlup <number of levels>).
		return -1;
	}
	if (level > 0) {
...
ACMD_FUNC(help) {
	...
	if (text == nullptr) {
		sprintf(atcmd_output, msg_txt(sd, 988), atcommand_symbol,
	...
	sprintf(atcmd_output, msg_txt(sd, 989), atcommand_symbol,
...
ACMD_FUNC(model) {
	...
	if (!message || !*message ||
		sscanf(message, "%11d %11d %11d", &hair_style, &hair_color, &cloth_color) < 1) {
		sprintf(atcmd_output,
				msg_txt(sd, 991),  // Please enter at least one value (usage: @model <hair ID:
								   // %d-%d> <hair color: %d-%d> <clothes color: %d-%d>).
				MIN_HAIR_STYLE, MAX_HAIR_STYLE, MIN_HAIR_COLOR, MAX_HAIR_COLOR, MIN_CLOTH_COLOR,
				MAX_CLOTH_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(bodystyle) {
	...
	if ((sd->class_ & JOBL_FOURTH) || !(sd->class_ & JOBL_THIRD) ||
		...
		(sd->class_ & MAPID_THIRDMASK) == MAPID_STAR_EMPEROR ||
	...
	if (!message || !*message || sscanf(message, "%d", &body_style) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 739), MIN_BODY_STYLE,
				MAX_BODY_STYLE);  // Please enter a body style (usage: @bodystyle <body ID: %d-%d>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(dye) {
	...
	if (!message || !*message || sscanf(message, "%11d", &cloth_color) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 992), MIN_CLOTH_COLOR,
				MAX_CLOTH_COLOR);  // Please enter a clothes color (usage: @dye/@ccolor <clothes
								   // color: %d-%d>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(hair_style) {
	...
	if (!message || !*message || sscanf(message, "%11d", &hair_style) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 993), MIN_HAIR_STYLE,
				MAX_HAIR_STYLE);  // Please enter a hair style (usage: @hairstyle/@hstyle <hair ID:
								  // %d-%d>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(hair_color) {
	...
	if (!message || !*message || sscanf(message, "%11d", &hair_color) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 994), MIN_HAIR_COLOR,
				MAX_HAIR_COLOR);  // Please enter a hair color (usage: @haircolor/@hcolor <hair
								  // color: %d-%d>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(go) {
	...
	for (i = 0; map_name[i]; i++) map_name[i] = TOLOWER(map_name[i]);
	...
	if (town >= 0 && town < ARRAYLENGTH(data)) {
		...
		if (m >= 0 && map_getmapflag(m, MF_NOWARPTO) &&
			!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
			clif_displaymessage(fd,
								msg_txt(sd, 247));	// You are not authorized to warp to this map.
			return -1;
		}
		if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARP) &&
			!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		...
		if (pc_setpos(sd, mapindex_name2id(data[town].map), data[town].x, data[town].y,
					  CLR_TELEPORT) == SETPOS_OK) {
			clif_displaymessage(fd, msg_txt(sd, 0));  // Warped.
		} else {
			clif_displaymessage(fd, msg_txt(sd, 1));  // Map not found.
			return -1;
...
ACMD_FUNC(monster) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd,
							msg_txt(sd, 80));  // Give the display name or monster name/id please.
		return -1;
	}
	if (sscanf(message, "\"%23[^\"]\" %23s %11d", name, monster, &number) > 1 ||
		...
		clif_displaymessage(fd,
							msg_txt(sd, 80));  // Give a display name and monster name/id please.
		return -1;
	}

	if ((mob_id = mobdb_searchname(monster)) ==
		0)	// check name first (to avoid possible name begining by a number)
	...
	// If value of atcommand_spawn_quantity_limit directive is greater than or equal to 1 and
	// quantity of monsters is greater than value of the directive
	...
	if (battle_config.etc_log)
		ShowInfo("%s monster='%s' name='%s' id=%d count=%d (%d,%d)\n", command, monster, name,
				 mob_id, number, sd->bl.x, sd->bl.y);
	...
	for (i = 0; i < number; i++) {
		...
		if (k) {
			// mapreg_setreg(reference_uid(add_str("$@mobid"), i),k); //retain created mobid in
			// array uncomment if needed
			count++;
		}
	}
...
ACMD_FUNC(refine) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &refine) < 2) {
		clif_displaymessage(fd, msg_txt(sd, 996));	// Please enter a position and an amount (usage:
	...
	for (j = 0; j < EQI_MAX; j++) {
		...
		if (sd->inventory.u.items_inventory[i].refine != final_refine) {
			...
			if (sd->inventory_data[i]->type == IT_WEAPON) {
				achievement_update_objective(sd, AG_ENCHANT_SUCCESS, 2,
...
ACMD_FUNC(grade) {
	...
	if (!message || !*message || sscanf(message, "%11d %11d", &position, &grade) < 2) {
		clif_displaymessage(fd,
							msg_txt(sd, 1519));	 // Please enter a position and an amount (usage:
	...
	for (j = 0; j < EQI_MAX; j++) {
		...
		final_grade =
			cap_value(sd->inventory.u.items_inventory[i].enchantgrade + grade, 0, MAX_ENCHANTGRADE);
...
ACMD_FUNC(produce) {
	...
	if (!message || !*message ||
		...
		clif_displaymessage(
			fd, msg_txt(sd, 1007));	 // Please enter at least one item name/ID (usage: @produce
									 // <equip name/ID> <element> <# of very's>).
		return -1;
	}
	...
	if (itemdb_isequip2(item_data.get())) {
		...
		sprintf(atcmd_output, msg_txt(sd, 169), item_id,
				item_data->name.c_str());  // The item (%u: '%s') is not equipable.
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(memo) {
	...
	if (!message || !*message || sscanf(message, "%11d", &position) < 1) {
		...
		for (i = 0; i < MAX_MEMOPOINTS; i++) {
			if (sd->status.memo_point[i].map)
				sprintf(atcmd_output, "%d - %s (%d,%d)", i,
	...
	if (position < 0 || position >= MAX_MEMOPOINTS) {
		sprintf(atcmd_output, msg_txt(sd, 1008), 0,
				MAX_MEMOPOINTS -
					1);	 // Please enter a valid position (usage: @memo <memo_position:%d-%d>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(displaystatus) {
	...
	if (!message || !*message ||
		(i = sscanf(message, "%11d %11d %11d %11d %11d %11d", &type, &flag, &tick, &val1, &val2,
					&val3)) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1009));	 // Please enter a status type/flag (usage: @displaystatus
									 // <status type> <flag> <tick> {<val1> {<val2> {<val3>}}}).
		return -1;
	}
	if (i < 2)
		flag = 1;
...
ACMD_FUNC(statuspoint) {
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1010));	 // Please enter a number (usage: @stpoint <number of points>).
		return -1;
	}
...
ACMD_FUNC(traitpoint) {
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 820));	// Please enter a number (usage: @trpoint <number of points>).
		return -1;
	}
...
ACMD_FUNC(skillpoint) {
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1011));	 // Please enter a number (usage: @skpoint <number of points>).
		return -1;
	}
...
ACMD_FUNC(zeny) {
	...
	if (!message || !*message || (zeny = atoi(message)) == 0) {
		clif_displaymessage(fd,
							msg_txt(sd, 1012));	 // Please enter an amount (usage: @zeny <amount>).
		return -1;
	}
...
ACMD_FUNC(param) {
	...
	if (!message || !*message || sscanf(message, "%11d", &value) < 1 || value == 0) {
		clif_displaymessage(fd,
							msg_txt(sd, 1013));	 // Please enter a valid value (usage:
												 // @str/@agi/@vit/@int/@dex/@luk <+/-adjustment>).
		return -1;
	}

	ARR_FIND(0, ARRAYLENGTH(parameter_names), stat,
			 strcmpi(command + 1, parameter_names[stat]) == 0);
	...
	if (stat < PARAM_POW) {
		...
		if (!(sd->class_ & JOBL_FOURTH)) {
			clif_displaymessage(
				fd, msg_txt(sd, 797));	// This command is unavailable to non - 4th class.
			return -1;
		}
...
ACMD_FUNC(stat_all) {
	...
	for (i = PARAM_STR; i < PARAM_POW; i++) status[i] = pc_getstat(sd, SP_STR + i);
	...
	if (!message || !*message || sscanf(message, "%11d", &value) < 1 || value == 0) {
		...
		if (pc_has_permission(sd, PC_PERM_BYPASS_MAX_STAT)) {
			for (i = PARAM_STR; i < PARAM_POW; i++) max_status[i] = SHRT_MAX;
		} else {
			for (i = PARAM_STR; i < PARAM_POW; i++)
				max_status[i] = pc_maxparameter(sd, static_cast<e_params>(i));
		}
...
ACMD_FUNC(trait_all) {
	...
	if (!(sd->class_ & JOBL_FOURTH)) {
		clif_displaymessage(fd,
							msg_txt(sd, 797));	// This command is unavailable to non - 4th class.
		return -1;
	}
	...
	for (i = PARAM_POW; i < PARAM_MAX; i++) status[i] = pc_getstat(sd, SP_POW + i - PARAM_POW);
	...
	if (!message || !*message || sscanf(message, "%11d", &value) < 1 || value == 0) {
		...
		if (pc_has_permission(sd, PC_PERM_BYPASS_MAX_STAT)) {
			for (i = PARAM_POW; i < PARAM_MAX; i++) max_status[i] = SHRT_MAX;
		} else {
			for (i = PARAM_POW; i < PARAM_MAX; i++)
				max_status[i] = pc_maxparameter(sd, static_cast<e_params>(i));
		}
...
ACMD_FUNC(guildlevelup) {
	...
	if (!message || !*message || sscanf(message, "%11d", &level) < 1 || level == 0) {
		clif_displaymessage(fd, msg_txt(sd, 1014));	 // Please enter a valid level (usage:
													 // @guildlvup/@guildlvlup <# of levels>).
		return -1;
	}
	...
	if (level > 0 &&
		(level > MAX_GUILDLEVEL ||
		 added_level > ((short)MAX_GUILDLEVEL - guild_info->guild_lv)))	 // fix positive overflow
		added_level = (short)MAX_GUILDLEVEL - guild_info->guild_lv;
	else if (level < 0 && (level < -MAX_GUILDLEVEL ||
						   added_level < (1 - guild_info->guild_lv)))  // fix negative overflow
...
ACMD_FUNC(makeegg) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1015));	 // Please enter a monster/egg name/ID (usage: @makeegg <pet>).
		return -1;
	}
	...
	if (pet != nullptr) {
		...
		if (mdb) {
			...
			if (intif_create_pet(sd->status.account_id, sd->status.char_id, pet->class_, mdb->lv,
								 pet->EggID, 0, pet->intimate, 100, 0, 1, mdb->jname.c_str())) {
				res = 0;
			} else {
...
ACMD_FUNC(petfriendly) {
	...
	if (!message || !*message || (friendly = atoi(message)) < 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1016));	 // Please enter a valid value (usage: @petfriendly <0-1000>).
		return -1;
	}
...
ACMD_FUNC(pethungry) {
	...
	if (!message || !*message || (hungry = atoi(message)) < 0) {
		clif_displaymessage(
			fd, msg_txt(sd, 1017));	 // Please enter a valid number (usage: @pethungry <0-100>).
		return -1;
	}
...
ACMD_FUNC(recall) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1018));	 // Please enter a player name (usage: @recall <char name/ID>).
		return -1;
	}
	...
	if (pc_get_group_level(sd) < pc_get_group_level(pl_sd)) {
		clif_displaymessage(fd, msg_txt(sd, 81));  // Your GM level doesn't authorize you to perform
												   // this action on the specified player.
		return -1;
	}

	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARPTO) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(
			fd, msg_txt(sd, 1019));	 // You are not authorized to warp someone to this map.
		return -1;
	}
	if (pl_sd->bl.m >= 0 && map_getmapflag(pl_sd->bl.m, MF_NOWARP) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(
			fd, msg_txt(sd, 1020));	 // You are not authorized to warp this player from their map.
		return -1;
	}
	...
	if (pc_setpos(pl_sd, sd->mapindex, sd->bl.x, sd->bl.y, CLR_RESPAWN) == SETPOS_AUTOTRADE) {
		clif_displaymessage(
			fd, msg_txt(sd, 1025));	 // The player is currently autotrading and cannot be recalled.
		return -1;
	}
...
ACMD_FUNC(char_block) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 1021),
				command);  // Please enter a player name (usage: %s <char name>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(char_ban) {
	...
	if (!message || !*message ||
		sscanf(message, "%255s %23[^\n]", atcmd_output, atcmd_player_name) < 2) {
		sprintf(
			output, msg_txt(sd, 1022),
			command);  // Please enter ban time and a player name (usage: %s <time> <char name>).
	...
	if (timediff == 0) {  // allow negative ?
		safesnprintf(output, sizeof(output), msg_txt(sd, 85), command,
					 timediff);	 // Invalid time for %s command (time=%d)
		clif_displaymessage(fd, output);
		clif_displaymessage(
			fd, msg_txt(sd, 702));	// Time parameter format is +/-<value> to alter. y/a = Year, m =
	...
	if (timediff < 0 &&
		...
		clif_displaymessage(fd,
							msg_txt(sd, 1023));	 // You are not allowed to alter the time of a ban.
		return -1;
	}
	...
	safesnprintf(output, sizeof(output), msg_txt(sd, 88),
				 bantype == CHRIF_OP_BAN ? "char" : "login");  // Sending request to %s server...
	clif_displaymessage(fd, output);
...
ACMD_FUNC(char_unblock) {
	...
	chrif_req_login_operation(sd->status.account_id, atcmd_player_name, CHRIF_OP_LOGIN_UNBLOCK, 0,
							  0, 0);
	sprintf(atcmd_output, msg_txt(sd, 88), "login");  // Sending request to %s server...
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(char_unban) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 435),
				command);  // Please enter a player name (usage: %s <char name>).
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	sprintf(atcmd_output, msg_txt(sd, 88),
			unbantype == CHRIF_OP_UNBAN ? "char" : "login");  // Sending request to %s server...
	clif_displaymessage(fd, atcmd_output);
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		...
		if (pl_sd->fd != fd && pc_get_group_level(sd) >= pc_get_group_level(pl_sd)) {
			...
			clif_displaymessage(pl_sd->fd,
								msg_txt(sd, 61));  // The holy messenger has given judgement.
		}
	}
	mapit_free(iter);
...
ACMD_FUNC(doommap) {
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		 pl_sd = (TBL_PC*)mapit_next(iter)) {
		if (pl_sd->fd != fd && sd->bl.m == pl_sd->bl.m &&
...
ACMD_FUNC(kick) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1026));	 // Please enter a player name (usage: @kick <char name/ID>).
		return -1;
	}
...
ACMD_FUNC(kickall) {
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		...
		if (pc_get_group_level(sd) >=
			pc_get_group_level(pl_sd)) {  // you can kick only lower or same gm level
...
ACMD_FUNC(questskill) {
	...
	if (!skill_get_inf2(skill_id, INF2_ISQUEST)) {
		clif_displaymessage(
			fd, msg_txt(sd, 197));	// This skill number doesn't exist or isn't a quest skill.
		return -1;
	}
...
ACMD_FUNC(spiritball) {
	...
	if (!message || !*message || (number = atoi(message)) < 0 || number > max_spiritballs) {
		...
		safesnprintf(msg, sizeof(msg), msg_txt(sd, 1028),
					 max_spiritballs);	// Please enter a party name (usage: @party <party_name>).
...
ACMD_FUNC(party) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1029));	 // Please enter a party name (usage: @party <party_name>).
		return -1;
	}
...
ACMD_FUNC(guild) {
	...
	if (sd->clan) {
		clif_displaymessage(
			fd, msg_txt(sd, 1498));	 // You cannot create a guild because you are in a clan.
		return -1;
	}

	if (!message || !*message || sscanf(message, "%23[^\n]", guild) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1030));	 // Please enter a guild name (usage: @guild <guild_name>).
		return -1;
	}
	...
	if (sd->status.guild_id) {	// Check if the player has a guild
		...
		if (g) {						   // Check if guild was found
			if (sd->state.gmaster_flag) {  // Check if player is guild master
				...
				clif_displaymessage(
					fd, msg_txt(sd, 1181));	 // You need to be a Guild Master to use this command.
				return -1;
			}
...
ACMD_FUNC(agitstart2) {
	...
	if (guild_agit2_start()) {
		...
		clif_displaymessage(fd,
							msg_txt(sd, 404));	// "War of Emperium SE is currently in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitstart3) {
	...
	if (guild_agit3_start()) {
		...
		clif_displaymessage(fd,
							msg_txt(sd, 750));	// "War of Emperium TE is currently in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitend2) {
	...
	if (guild_agit2_end()) {
		...
		clif_displaymessage(
			fd, msg_txt(sd, 406));	// "War of Emperium SE is currently not in progress."
		return -1;
	}
}
...
ACMD_FUNC(agitend3) {
	...
	if (guild_agit3_end()) {
		...
		clif_displaymessage(fd,
							msg_txt(sd, 752));	// War of Emperium TE is currently not in progress.
		return -1;
	}
}
...
ACMD_FUNC(idsearch) {
	...
	if (!message || !*message || sscanf(message, "%99s", item_name) < 0) {
		clif_displaymessage(fd, msg_txt(sd, 1031));	 // Please enter part of an item name (usage:
...
ACMD_FUNC(recallall) {
	...
	if (sd->bl.m >= 0 && map_getmapflag(sd->bl.m, MF_NOWARPTO) &&
		!pc_has_permission(sd, PC_PERM_WARP_ANYWHERE)) {
		clif_displaymessage(
			fd, msg_txt(sd, 1032));	 // You are not authorized to warp someone to your current map.
		return -1;
	}
	...
	if (count) {
		sprintf(atcmd_output, msg_txt(sd, 1033),
				count);	 // Because you are not authorized to warp from some maps, %d player(s) have
						 // not been recalled.
		clif_displaymessage(fd, atcmd_output);
	}
...
ACMD_FUNC(guildrecall) {
	...
	if ((g = guild_searchname(guild_name)) ==
			NULL &&	 // name first to avoid error when name begin with a number
		(g = guild_search(atoi(message))) == NULL) {
		clif_displaymessage(
			fd, msg_txt(sd, 94));  // Incorrect name/ID, or no one from the guild is online.
		return -1;
	}
	...
	sprintf(atcmd_output, msg_txt(sd, 93),
...
ACMD_FUNC(partyrecall) {
	...
	if ((p = party_searchname(party_name)) ==
		...
		clif_displaymessage(
			fd, msg_txt(sd, 96));  // Incorrect name or ID, or no one from the party is online.
		return -1;
	}
	...
	sprintf(atcmd_output, msg_txt(sd, 95), p->party.name);	// All online characters of the %s party
															// have been recalled to your position.
	clif_displaymessage(fd, atcmd_output);
	if (count) {
		sprintf(atcmd_output, msg_txt(sd, 1033),
				count);	 // Because you are not authorized to warp from some maps, %d player(s) have
...
ACMD_FUNC(partysharelvl) {
	...
	if (intif_party_sharelvlupdate(share_lvl))	// Successfully updated
		clif_displaymessage(
			fd, msg_txt(sd, 1478));	 // Party share level range has been changed successfully.
	else							 // Char server offline
		clif_displaymessage(
			fd, msg_txt(sd, 1479));	 // Failed to update configuration. Character server is offline.
...
ACMD_FUNC(mapinfo) {
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(fd, msg_txt(sd, 1038));	 // Please enter at least one valid list number
													 // (usage: @mapinfo <0-3> <map>).
		return -1;
	}
	...
	m_index = mapindex_name2id(mapname);  // This one shouldn't fail since the previous seek did
										  // not.
	...
	for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		...
		if (pl_sd->mapindex == m_index) {
			if (pl_sd->state.vending)
				vend_num++;
			else if ((cd = (struct chat_data*)map_id2bl(pl_sd->chatID)) != NULL &&
					 cd->usersd[0] == pl_sd)
				chat_num++;
		}
	}
	...
	sprintf(atcmd_output, msg_txt(sd, 1040), mapname, mapdata->users, mapdata->npc_num, chat_num,
			vend_num);	// Map: %s | Players: %d | NPCs: %d | Chats: %d | Vendings: %d
	clif_displaymessage(fd, atcmd_output);
	...
	if (map_getmapflag(m_id, MF_BATTLEGROUND)) {
		sprintf(atcmd_output, msg_txt(sd, 1045),
				map_getmapflag(m_id, MF_BATTLEGROUND));	 // Battlegrounds ON (type %d)
		clif_displaymessage(fd, atcmd_output);
	}
	...
	if (mapdata->flag[MF_SKILL_DAMAGE]) {
		...
		sprintf(atcmd_output, msg_txt(sd, 1053),	 // > [Map] %d%%, %d%%, %d%%, %d%% | Caster:%d
				...
				mapdata->damage_adjust.rate[SKILLDMG_OTHER], mapdata->damage_adjust.caster);
		clif_displaymessage(fd, atcmd_output);
		if (!mapdata->skill_damage.empty()) {
			...
			for (auto skilldmg : mapdata->skill_damage) {
				sprintf(atcmd_output, "     %s : %d%%, %d%%, %d%%, %d%% | %d",
						skill_get_name(skilldmg.first), skilldmg.second.rate[SKILLDMG_PC],
						skilldmg.second.rate[SKILLDMG_MOB], skilldmg.second.rate[SKILLDMG_BOSS],
						skilldmg.second.rate[SKILLDMG_OTHER], skilldmg.second.caster);
				clif_displaymessage(fd, atcmd_output);
			}
		}
	}
	...
	sprintf(atcmd_output, msg_txt(sd, 1065),  // No Exp Penalty: %s | No Zeny Penalty: %s
			...
			(map_getmapflag(m_id, MF_NOZENYPENALTY)) ? msg_txt(sd, 1066)
													 : msg_txt(sd, 1067));	// On / Off
	clif_displaymessage(fd, atcmd_output);
	...
	if (map_getmapflag(m_id, MF_NOSAVE)) {
		if (!mapdata->save.map)
			...
			sprintf(atcmd_output, msg_txt(sd, 1069),
					mapindex_id2name(mapdata->save.map));  // No Save, Save Point: %s,Random
			clif_displaymessage(fd, atcmd_output);
		} else {
	...
	switch (list) {
		...
		case 1:
			...
			for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
				...
				if (pl_sd->mapindex == m_index) {
					sprintf(atcmd_output,
							msg_txt(sd, 481),  // Player '%s' (session #%d) | Location: %d,%d
		...
		case 3:
			...
			for (pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
				...
				if ((cd = (struct chat_data*)map_id2bl(pl_sd->chatID)) != NULL &&
					pl_sd->mapindex == m_index && cd->usersd[0] == pl_sd) {
					sprintf(atcmd_output,
							msg_txt(sd, 484),  // Chat: %s | Player: %s | Location: %d %d
							cd->title, pl_sd->status.name, cd->bl.x, cd->bl.y);
					clif_displaymessage(fd, atcmd_output);
					sprintf(atcmd_output,
							msg_txt(sd, 485),  //    Users: %d/%d | Password: %s | Public: %s
							cd->users, cd->limit, cd->pass,
							(cd->pub) ? msg_txt(sd, 486) : msg_txt(sd, 487));  // Yes / No
					clif_displaymessage(fd, atcmd_output);
				}
			}
			mapit_free(iter);
		...
		default:										// normally impossible to arrive here
			clif_displaymessage(fd, msg_txt(sd, 488));	// Please enter at least one valid list
														// number (usage: @mapinfo <0-3> <map>).
			return -1;
			break;
	}
...
ACMD_FUNC(mount_peco) {
	...
	if ((sd->class_ & MAPID_THIRDMASK) == MAPID_RUNE_KNIGHT &&
		pc_checkskill(sd, RK_DRAGONTRAINING) > 0) {
	...
	if (!pc_isriding(sd)) {	 // if actually no peco
		...
		if (!pc_checkskill(sd, KN_RIDING)) {
			clif_displaymessage(
				fd, msg_txt(sd, 213));	// You can not mount a Peco Peco with your current job.
			return -1;
		}
...
ACMD_FUNC(guildspy) {
	...
	if (!enable_spy) {
		clif_displaymessage(fd,
							msg_txt(sd, 1125));	 // The mapserver has spy command support disabled.
		return -1;
	}
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1126));	 // Please enter a guild name/ID (usage: @guildspy <guild_name/ID>).
		return -1;
	}
	...
	if ((g = guild_searchname(guild_name)) !=
		...
		clif_displaymessage(
			fd,
			msg_txt(sd, 94));  // Incorrect name/ID, or no one from the specified guild is online.
		return -1;
	}
...
ACMD_FUNC(partyspy) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1127));	 // Please enter a party name/ID (usage: @partyspy <party_name/ID>).
		return -1;
	}
	...
	if ((p = party_searchname(party_name)) !=
		...
		if (sd->partyspy == p->party.party_id) {
			sd->partyspy = 0;
			sprintf(atcmd_output, msg_txt(sd, 105),
		...
		clif_displaymessage(
			fd,
			msg_txt(sd, 96));  // Incorrect name/ID, or no one from the specified party is online.
		return -1;
	}
...
ACMD_FUNC(clanspy) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", clan_name) < 1) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1499));	 // Please enter a clan name/ID (usage: @clanspy <clan_name/ID>).
		return -1;
	}

	if ((c = clan_searchname(clan_name)) !=
			NULL ||	 // name first to avoid error when name begin with a number
...
ACMD_FUNC(nuke) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1128));	 // Please enter a player name (usage: @nuke <char name>).
		return -1;
	}

	if ((pl_sd = map_nick2sd(atcmd_player_name, false)) != NULL) {
		if (pc_get_group_level(sd) >=
			pc_get_group_level(pl_sd)) {  // you can kill only lower or same GM level
			skill_castend_nodamage_id(&pl_sd->bl, &pl_sd->bl, NPC_SELFDESTRUCTION, 99, gettick(),
									  0);
...
ACMD_FUNC(tonpc) {
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", npcname) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1129));	 // Please enter a NPC name (usage: @tonpc <NPC_name>).
		return -1;
	}
...
ACMD_FUNC(shownpc) {
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1130));	 // Please enter a NPC name (usage: @enablenpc <NPC_name>).
		return -1;
	}
...
ACMD_FUNC(hidenpc) {
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1131));	 // Please enter a NPC name (usage: @hidenpc <NPC_name>).
		return -1;
	}
...
ACMD_FUNC(loadnpc) {
	if (!message || !*message) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1132));	 // Please enter a script file name (usage: @loadnpc <file name>).
		return -1;
	}
...
ACMD_FUNC(unloadnpc) {
	...
	if (!message || !*message || sscanf(message, "%49[^\n]", NPCname) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1133));	 // Please enter a NPC name (usage: @unloadnpc <NPC_name>).
		return -1;
	}
...
ACMD_FUNC(reloadnpcfile) {
	if (!message || !*message) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 733));	// Please enter a NPC file name (usage: @reloadnpcfile <file name>).
		return -1;
	}

	if (npc_unloadfile(message))
		clif_displaymessage(fd, msg_txt(sd, 1386));	 // File unloaded. Be aware that mapflags and
													 // monsters spawned directly are not removed.
...
ACMD_FUNC(servertime) {
	...
	strftime(temp, sizeof(temp) - 1, msg_txt(sd, 230),
	...
	if (battle_config.night_duration == 0 && battle_config.day_duration == 0) {
		if (night_flag == 0)
			clif_displaymessage(fd,
								msg_txt(sd, 231));	// Game time: The game is in permanent daylight.
		else
			clif_displaymessage(fd,
								msg_txt(sd, 232));	// Game time: The game is in permanent night.
	} else if (battle_config.night_duration == 0)
		if (night_flag == 1) {	// we start with night
			if ((timer_data = get_timer(day_timer_tid)) != nullptr) {
				sprintf(temp, msg_txt(sd, 233),
						txt_time(DIFF_TICK(timer_data->tick, gettick()) /
								 1000));  // Game time: The game is in night for %s.
				clif_displaymessage(fd, temp);
				clif_displaymessage(
					fd,
					msg_txt(sd, 234));	// Game time: After, the game will be in permanent daylight.
			} else
				clif_displaymessage(
					fd, msg_txt(sd, 232));	// Game time: The game is in permanent night.
		} else
			clif_displaymessage(fd,
								msg_txt(sd, 231));	// Game time: The game is in permanent daylight.
		...
		if (night_flag == 0) {	// we start with day
			if ((timer_data = get_timer(night_timer_tid)) != nullptr) {
				sprintf(temp, msg_txt(sd, 235),
						txt_time(DIFF_TICK(timer_data->tick, gettick()) /
								 1000));  // Game time: The game is in daylight for %s.
				clif_displaymessage(fd, temp);
				clif_displaymessage(
					fd,
					msg_txt(sd, 236));	// Game time: After, the game will be in permanent night.
			} else
				clif_displaymessage(
					fd, msg_txt(sd, 231));	// Game time: The game is in permanent daylight.
		} else
			clif_displaymessage(fd,
								msg_txt(sd, 232));	// Game time: The game is in permanent night.
	else {
		...
		if (night_flag == 0) {
			if ((timer_data = get_timer(night_timer_tid)) != nullptr &&
				...
				if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0)
					sprintf(temp, msg_txt(sd, 237),
							txt_time(DIFF_TICK(timer_data->interval,
											   DIFF_TICK(timer_data->tick, timer_data2->tick)) /
									 1000));  // Game time: After, the game will be in night for %s.
				else
					sprintf(temp, msg_txt(sd, 237),
							txt_time(DIFF_TICK(timer_data2->tick, timer_data->tick) /
									 1000));  // Game time: After, the game will be in night for %s.
				clif_displaymessage(fd, temp);
				sprintf(temp, msg_txt(sd, 238),
						txt_time(timer_data->interval /
								 1000));  // Game time: A day cycle has a normal duration of %s.
				clif_displaymessage(fd, temp);
			} else
				clif_displaymessage(
					fd, msg_txt(sd, 231));	// Game time: The game is in permanent daylight.
			...
			if ((timer_data = get_timer(day_timer_tid)) != nullptr &&
				...
				if (DIFF_TICK(timer_data->tick, timer_data2->tick) > 0)
					sprintf(
						...
						txt_time((timer_data->interval -
								  DIFF_TICK(timer_data->tick, timer_data2->tick)) /
								 1000));  // Game time: After, the game will be in daylight for %s.
				else
					sprintf(
						temp, msg_txt(sd, 239),
						txt_time(DIFF_TICK(timer_data2->tick, timer_data->tick) /
...
ACMD_FUNC(jail) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1134));	 // Please enter a player name (usage: @jail <char_name>).
		return -1;
	}
...
ACMD_FUNC(jailfor) {
	...
	// Added by Coltaro
	if (pl_sd->sc.data[SC_JAILED] &&
		pl_sd->sc.data[SC_JAILED]->val1 != INT_MAX) {  // Update the player's jail time
		...
		if (jailtime <= 0) {
			...
			sprintf(
				atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1137), year, month, day, hour,
				minute);  // %s in jail for %d years, %d months, %d days, %d hours and %d minutes
			clif_displaymessage(pl_sd->fd, atcmd_output);
			sprintf(atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1138), year, month, day, hour,
					minute);  // This player is now in jail for %d years, %d months, %d days, %d
							  // hours and %d minutes
			clif_displaymessage(fd, atcmd_output);
	...
	sc_start4(NULL, &pl_sd->bl, SC_JAILED, 100, jailtime, m_index, x, y,
			  jailtime ? 60000 : 1000);	 // jailtime = 0: Time was reset to 0. Wait 1 second to warp
										 // player out (since it's done in status_change_timer).
	return 0;
}
...
ACMD_FUNC(jailtime) {
	...
	if (sd->sc.data[SC_JAILED]->val1 <=
		0) {  // Was not jailed with @jailfor (maybe @jail? or warped there? or got recalled?)
		clif_displaymessage(
			fd, msg_txt(sd, 1141));	 // You have been jailed for an unknown amount of time.
		return -1;
	}

	// Get remaining jail time
	split_time(sd->sc.data[SC_JAILED]->val1 * 60, &year, &month, &day, &hour, &minute, &second);
	sprintf(atcmd_output, msg_txt(sd, 402), msg_txt(sd, 1142), year, month, day, hour,
			minute);  // You will remain in jail for %d years, %d months, %d days, %d hours and %d
					  // minutes
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(disguise) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1143));	 // Please enter a Monster/NPC name/ID (usage: @disguise <name/ID>).
		return -1;
	}
	...
	if (sd->sc.data[SC_MONSTER_TRANSFORM] || sd->sc.data[SC_ACTIVE_MONSTER_TRANSFORM]) {
		clif_displaymessage(
			fd, msg_txt(sd, 730));	// Character cannot be disguised while in monster transform.
		return -1;
	}
...
ACMD_FUNC(disguiseall) {
	...
	if ((mob_id = mobdb_searchname(message)) ==
		0)	// check name first (to avoid possible name beginning by a number)
		mob_id = atoi(message);
...
ACMD_FUNC(disguiseguild) {
	...
	if (!message || !*message || sscanf(message, "%23[^,], %23[^\r\n]", monster, guild) < 2) {
		clif_displaymessage(
			fd, msg_txt(sd, 1146));	 // Please enter a mob name/ID and guild name/ID (usage:
									 // @disguiseguild <mob name/ID>, <guild name/ID>).
		return -1;
	}
...
ACMD_FUNC(exp) {
	...
	sprintf(output, msg_txt(sd, 1148), sd->status.base_level, nextb, sd->status.job_level,
			nextj);	 // Base Level: %d (%.3f%%) | Job Level: %d (%.3f%%)
	clif_displaymessage(fd, output);
	return 0;
}
...
ACMD_FUNC(broadcast) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1149));	 // Please enter a message (usage: @broadcast <message>).
		return -1;
	}
...
ACMD_FUNC(localbroadcast) {
	...
	if (!message || !*message) {
		clif_displaymessage(
			fd, msg_txt(sd, 1150));	 // Please enter a message (usage: @localbroadcast <message>).
		return -1;
	}
...
ACMD_FUNC(email) {
	...
	if (e_mail_check(actual_email) == 0) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 144));	// Invalid actual email. If you have default e-mail, give a@a.com.
		return -1;
	} else if (e_mail_check(new_email) == 0) {
		clif_displaymessage(fd,
							msg_txt(sd, 145));	// Invalid new email. Please enter a real e-mail.
		return -1;
		...
		clif_displaymessage(fd,
							msg_txt(sd, 147));	// New email must be different of the actual e-mail.
		return -1;
	}
...
ACMD_FUNC(effect) {
	...
	if (!message || !*message || sscanf(message, "%11d", &type) < 1) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1152));	 // Please enter an effect number (usage: @effect <effect number>).
		return -1;
	}

	if (type <= EF_NONE || type >= EF_MAX) {
		sprintf(atcmd_output, msg_txt(sd, 1152), EF_NONE + 1,
				EF_MAX - 1);  // Please enter a valid effect id in the range from %d to %d.
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
ACMD_FUNC(killable) {
	...
	if (sd->state.killable)
		clif_displaymessage(fd,
							msg_txt(sd, 242));	// You can now be attacked and killed by players.
	else {
...
ACMD_FUNC(dropall) {
	...
	if (message[0]) {
		type = atoi(message);
		if (type != -1 && type != IT_HEALING && type != IT_USABLE && type != IT_ETC &&
			type != IT_WEAPON && type != IT_ARMOR && type != IT_CARD && type != IT_PETEGG &&
			type != IT_PETARMOR && type != IT_AMMO) {
			clif_displaymessage(fd, msg_txt(sd, 1492));	 // Usage: @dropall {<type>}
			clif_displaymessage(
				fd,
				msg_txt(
					sd,
					1493));	 // Type List: (default) all = -1, healing = 0, usable = 2, etc = 3,
							 // armor = 4, weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
			return -1;
		}
	}
		...
		if (sd->inventory.u.items_inventory[i].amount) {
			...
			if (id == nullptr) {
				ShowDebug("Non-existant item %d on dropall list (account_id: %d, char_id: %d)\n",
						  sd->inventory.u.items_inventory[i].nameid, sd->status.account_id,
						  sd->status.char_id);
				continue;
			}
...
ACMD_FUNC(cleargstorage) {
	...
	if (sd->state.storage_flag == 2) {
		clif_displaymessage(
			fd, msg_txt(sd, 251));	// You have already opened your guild storage. Close it first.
		return -1;
	}
	...
	if (gstorage == NULL) {	 // Doesn't have opened @gstorage yet, so we skip the deletion since
							 // *shouldn't* have any item there.
		return -1;
	}

	j = gstorage->amount;
	gstorage->lock = true;	// Lock @gstorage: do not allow any item to be retrieved or stored from
							// any guild member
	for (i = 0; i < j; ++i) {
...
#define MAX_SKILLID_PARTIAL_RESULTS_LEN \
	74	// "skill " (6) + "%d:" (up to 5) + "%s" (up to 30) + " (%s)" (up to 33)
ACMD_FUNC(skillid) {
	...
	for (const auto& skill : skill_db) {
		...
		if (strnicmp(name, message, skillen) == 0 || strnicmp(desc, message, skillen) == 0) {
			...
			clif_displaymessage(fd, atcmd_output);
		} else if (found < MAX_SKILLID_PARTIAL_RESULTS &&
				   (stristr(name, message) || stristr(desc, message))) {
			snprintf(partials[found++], MAX_SKILLID_PARTIAL_RESULTS_LEN, msg_txt(sd, 1164),
					 skill_id, desc, name);	 // // skill %d: %s (%s)
		}
	}
...
ACMD_FUNC(useskill) {
	...
	if (!message || !*message ||
		sscanf(message, "%6hu %6hu %23[^\n]", &skill_id, &skill_lv, atcmd_player_name) != 3) {
		clif_displaymessage(
			fd, msg_txt(sd, 1165));	 // Usage: @useskill <skill ID> <skill level> <char name>
		return -1;
	}
	...
	if (SKILL_CHK_HOMUN(skill_id) &&
		hom_is_active(sd->hd))	// (If used with @useskill, put the homunc as dest)
		bl = &sd->hd->bl;
	else
		bl = &sd->bl;
...
ACMD_FUNC(displayskill) {
	...
	if (!message || !*message ||
		sscanf(message, "%6hu %6hu %6hu", &skill_id, &skill_lv, &type) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1166));	 // Usage: @displayskill <skill ID> {<skill level> <type>}
		clif_displaymessage(
			fd,
			msg_txt(
				sd,
	...
	tick = gettick();
	if (type == 0 || type == 1)
		clif_skill_damage(&sd->bl, &sd->bl, tick, status->amotion, status->dmotion, 1, 1, skill_id,
						  skill_lv, DMG_SINGLE);
	if (type == 0 || type == 2)
		clif_skill_damage(&sd->bl, &sd->bl, tick, status->amotion, status->dmotion, 1, 1, skill_id,
						  skill_lv, DMG_SPLASH);
...
ACMD_FUNC(displayskillcast) {
	...
	if (!message || !*message ||
		sscanf(message, "%6hu %6hu %6hu %6hu", &skill_id, &skill_lv, &target_type, &cast_time) <
			1) {
		clif_displaymessage(fd, msg_txt(sd, 824));	// Usage: @displayskillcast <skill ID> {<skill
													// level> <ground target flag> <cast time>}
		return -1;
	}

	if (target_type == 1)
		clif_skillcasting(&sd->bl, sd->bl.id, 0, sd->bl.x, sd->bl.y, skill_id, skill_lv, 0,
						  cast_time);
	else
...
ACMD_FUNC(displayskillunit) {
	...
	if (!message || !*message ||
		sscanf(message, "%6hu %6hu %6hu", &unit_id, &skill_lv, &range) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 826));	// Usage: @displayskillunit <unit ID> {<skill level> <range>}
		return -1;
	}
...
ACMD_FUNC(skilltree) {
	...
	sprintf(atcmd_output, msg_txt(sd, 1168), job_name(c),
			pc_checkskill(pl_sd, NV_BASIC));  // Player is using %s skill tree (%d basic points).
	clif_displaymessage(fd, atcmd_output);
	...
	for (const auto& it : entry->need) {
		if (pc_checkskill(sd, it.first) < it.second) {
			sprintf(atcmd_output, msg_txt(sd, 1170), it.second,
					skill_get_desc(it.first));	// Player requires level %d of skill %s.
			clif_displaymessage(fd, atcmd_output);
			meets = false;
		}
	}
	...
	if (meets) {
		clif_displaymessage(
			fd, msg_txt(sd, 1171));	 // The player meets all the requirements for that skill.
	}
...
ACMD_FUNC(marry) {
	...
	clif_displaymessage(
		fd,
		msg_txt(sd, 1174));	 // The two cannot wed because one is either a baby or already married.
	return -1;
}
...
ACMD_FUNC(divorce) {
	...
	sprintf(atcmd_output, msg_txt(sd, 1176),
...
ACMD_FUNC(changelook) {
	...
	if ((i = sscanf(message, "%11d %11d", &j, &k)) < 1) {
		...
		clif_displaymessage(fd, msg_txt(sd, 1178));	 // Position: 1-Top 2-Middle 3-Bottom 4-Weapon
													 // 5-Shield 6-Shoes 7-Robe 8-Body
		return -1;
	} else if (i == 2) {
		if (j < 1 || j > 8)
			j = 1;
...
ACMD_FUNC(autotrade) {
	...
	if (!sd->state.vending && !sd->state.buyingstore) {	 // check if player is vending or buying
		clif_displaymessage(fd,
							msg_txt(sd, 549));	// "You should have a shop open to use @autotrade."
		return -1;
	}
	...
	if (battle_config.at_timeout) {
		...
		status_change_start(
			NULL, &sd->bl, SC_AUTOTRADE, 10000, 0, 0, 0, 0,
			((timeout > 0) ? min(timeout, battle_config.at_timeout) : battle_config.at_timeout) *
				60000,
			SCSTART_NONE);
	}
...
ACMD_FUNC(changegm) {
	...
	if ((pl_sd = map_nick2sd(atcmd_player_name, false)) == NULL ||
		pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(
			fd, msg_txt(sd, 1184));	 // Target character must be online and be a guild member.
		return -1;
	}
...
#else
		clif_displaymessage(
			fd, msg_txt(sd, 1513));	 // Currently in WoE hours, unable to delegate Guild leader
#endif
		return -1;
	}
...
ACMD_FUNC(partyoption) {
	...
	if (sd->status.party_id == 0 || (p = party_search(sd->status.party_id)) == NULL) {
		clif_displaymessage(
			fd, msg_txt(sd, 282));	// You need to be a party leader to use this command.
		return -1;
	}
...
ACMD_FUNC(autoloot) {
	...
	if (sd->state.autoloot) {
		snprintf(atcmd_output, sizeof atcmd_output, msg_txt(sd, 1187),
				 ((double)sd->state.autoloot) /
					 100.);	 // Autolooting items with drop rates of %0.02f%% and below.
		clif_displaymessage(fd, atcmd_output);
	} else
	...
	switch (action) {
		case 1:
			...
			if (i != AUTOLOOTITEM_SIZE) {
				clif_displaymessage(fd,
									msg_txt(sd, 1190));	 // You're already autolooting this item.
				return -1;
			}
			ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] == 0);
			if (i == AUTOLOOTITEM_SIZE) {
				clif_displaymessage(
					fd, msg_txt(sd, 1191));	 // Your autolootitem list is full. Remove some items
											 // first with @autolootid -<item name or ID>.
				return -1;
			}
			sd->state.autolootid[i] = item_data->nameid;  // Autoloot Activated
			sprintf(atcmd_output, msg_txt(sd, 1192), item_data->name.c_str(),
					item_data->ename.c_str(),
					item_data->nameid);	 // Autolooting item: '%s'/'%s' {%u}
			clif_displaymessage(fd, atcmd_output);
		...
		case 2:
			...
			if (i == AUTOLOOTITEM_SIZE) {
				clif_displaymessage(
					fd, msg_txt(sd, 1193));	 // You're currently not autolooting this item.
				return -1;
			}
			sd->state.autolootid[i] = 0;
			sprintf(
				atcmd_output, msg_txt(sd, 1194), item_data->name.c_str(), item_data->ename.c_str(),
				item_data->nameid);	 // Removed item: '%s'/'%s' {%u} from your autolootitem list.
			clif_displaymessage(fd, atcmd_output);
			ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
			if (i == AUTOLOOTITEM_SIZE) {
			...
			break;
		case 3:
			sprintf(atcmd_output, msg_txt(sd, 1195),
					AUTOLOOTITEM_SIZE);	 // You can have %d items on your autolootitem list.
			clif_displaymessage(fd, atcmd_output);
			clif_displaymessage(
				fd,
				msg_txt(sd, 1196));	 // To add an item to the list, use "@alootid +<item name or
									 // ID>". To remove an item, use "@alootid -<item name or ID>".
			clif_displaymessage(
				fd, msg_txt(sd, 1197));	 // "@alootid reset" will clear your autolootitem list.
			ARR_FIND(0, AUTOLOOTITEM_SIZE, i, sd->state.autolootid[i] != 0);
			if (i == AUTOLOOTITEM_SIZE) {
				...
				for (i = 0; i < AUTOLOOTITEM_SIZE; i++) {
					...
					if (item_data == nullptr) {
						ShowDebug(
							"Non-existant item %d on autolootitem list (account_id: %d, char_id: "
							"%d)",
							sd->state.autolootid[i], sd->status.account_id, sd->status.char_id);
						continue;
					}

					sprintf(atcmd_output, "'%s'/'%s' {%u}", item_data->name.c_str(),
							item_data->ename.c_str(), item_data->nameid);
					clif_displaymessage(fd, atcmd_output);
				}
			}
			break;
	...
	switch (action) {
		case 1:
			if (sd->state.autoloottype & (1 << type)) {
				clif_displaymessage(
					fd, msg_txt(sd, 1481));	 // You're already autolooting this item type.
				return -1;
			}
			if (sd->state.autoloottype == ITEM_MAX) {
				clif_displaymessage(
					fd,
					msg_txt(sd, 1482));	 // Your autoloottype list has all item types. You can
										 // remove some items with @autoloottype -<type name or ID>.
				return -1;
			}
			sd->state.autoloottype |= (1 << type);	// Stores the type
			sprintf(atcmd_output, msg_txt(sd, 1483), itemdb_typename(type),
					type);	// Autolooting item type: '%s' {%u}
			clif_displaymessage(fd, atcmd_output);
			break;
		case 2:
			if (!(sd->state.autoloottype & (1 << type))) {
				clif_displaymessage(
					fd, msg_txt(sd, 1484));	 // You're currently not autolooting this item type.
				return -1;
			}
			sd->state.autoloottype &= ~(1 << type);
			sprintf(atcmd_output, msg_txt(sd, 1485), itemdb_typename(type),
					type);	// Removed item type: '%s' {%u} from your autoloottype list.
			clif_displaymessage(fd, atcmd_output);
			break;
		case 3:
			clif_displaymessage(
				...
				msg_txt(
					sd,
					1486));	 // To add an item type to the list, use "@aloottype +<type name or
							 // ID>". To remove an item type, use "@aloottype -<type name or ID>".
			clif_displaymessage(
				fd,
				msg_txt(sd, 1487));	 // Type List: healing = 0, usable = 2, etc = 3, armor = 4,
									 // weapon = 5, card = 6, petegg = 7, petarmor = 8, ammo = 10
			clif_displaymessage(
				fd, msg_txt(sd, 1488));	 // "@aloottype reset" will clear your autoloottype list.
			if (sd->state.autoloottype == 0)
				clif_displaymessage(fd, msg_txt(sd, 1489));	 // Your autoloottype list is empty.
			else {
				uint8 i = 0;
				clif_displaymessage(fd,
									msg_txt(sd, 1490));	 // Item types on your autoloottype list:
				while (i < IT_MAX) {
					if (sd->state.autoloottype & (1 << i)) {
						sprintf(atcmd_output, "  '%s' {%d}",
								itemdb_typename(static_cast<item_types>(i)), i);
						clif_displaymessage(fd, atcmd_output);
					}
					i++;
				}
...
ACMD_FUNC(clearweather) {
	...
	// map_setmapflag(sd->bl.m, MF_RAIN, false); // No longer available, keeping here just in case
	// it's back someday. [Ind]
	map_setmapflag(sd->bl.m, MF_SNOW, false);
	map_setmapflag(sd->bl.m, MF_SAKURA, false);
	map_setmapflag(sd->bl.m, MF_CLOUDS, false);
	map_setmapflag(sd->bl.m, MF_CLOUDS2, false);
...
ACMD_FUNC(sound) {
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", sound_file) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1217));	 // Please enter a sound filename (usage: @sound <filename>).
		return -1;
	}
...
ACMD_FUNC(mobsearch) {
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mob_name) < 1) {
		clif_displaymessage(
			fd,
			msg_txt(sd, 1218));	 // Please enter a monster name (usage: @mobsearch <monster name>).
		return -1;
	}
	...
	if (mob == nullptr || mobdb_checkid(mob_id) == 0) {
		snprintf(atcmd_output, sizeof atcmd_output, msg_txt(sd, 1219),
	...
	snprintf(atcmd_output, sizeof atcmd_output, msg_txt(sd, 1220), mob_name,
	...
	for (;;) {
		...
		if (md->spawn_timer == INVALID_TIMER)
			snprintf(atcmd_output, sizeof(atcmd_output), "%2d[%3d:%3d] %s", number, md->bl.x,
					 md->bl.y, md->name);
		else
...
ACMD_FUNC(cleanarea) {
	...
	if (!message || !*message || sscanf(message, "%6hd %6hd %6hd %6hd", &x0, &y0, &x1, &y1) < 1) {
		map_foreachinallarea(atcommand_cleanfloor_sub, sd->bl.m, sd->bl.x - (AREA_SIZE * 2),
							 sd->bl.y - (AREA_SIZE * 2), sd->bl.x + (AREA_SIZE * 2),
							 sd->bl.y + (AREA_SIZE * 2), BL_ITEM);
	} else if (sscanf(message, "%6hd %6hd %6hd %6hd", &x0, &y0, &x1, &y1) == 1) {
		map_foreachinallarea(atcommand_cleanfloor_sub, sd->bl.m, sd->bl.x - x0, sd->bl.y - x0,
							 sd->bl.x + x0, sd->bl.y + x0, BL_ITEM);
...
ACMD_FUNC(npctalk) {
	...
	if (!ifcolor) {
		if (!message || !*message || sscanf(message, "%49[^,], %99[^\n]", name, mes) < 2) {
			clif_displaymessage(fd, msg_txt(sd, 1222));	 // Please enter the correct parameters
														 // (usage: @npctalk <npc name>, <message>).
			return -1;
		}
	} else {
		if (!message || !*message ||
			sscanf(message, "%16lx %23[^,], %99[^\n]", &color, name, mes) < 3) {
			clif_displaymessage(fd,
								msg_txt(sd, 1223));	 // Please enter the correct parameters (usage:
													 // @npctalkc <color> <npc name>, <message>).
			return -1;
		}
	}
...
ACMD_FUNC(pettalk) {
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1224));	 // Please enter a message (usage: @pettalk <message>).
		return -1;
	}
	...
	if (message[0] == '/') {  // pet emotion processing
		const char* emo[] = {
			"/!",			"/?",		 "/ho",		   "/lv",	  "/swt",	   "/ic",
			"/an",			"/ag",		 "/$",		   "/...",	  "/scissors", "/rock",
			"/paper",		"/korea",	 "/lv2",	   "/thx",	  "/wah",	   "/sry",
			"/heh",			"/swt2",	 "/hmm",	   "/no1",	  "/??",	   "/omg",
			"/O",			"/X",		 "/hlp",	   "/go",	  "/sob",	   "/gg",
			"/kis",			"/kis2",	 "/pif",	   "/ok",	  "-?-",	   "/indonesia",
			"/bzz",			"/rice",	 "/awsm",	   "/meh",	  "/shy",	   "/pat",
			"/mp",			"/slur",	 "/com",	   "/yawn",	  "/grat",	   "/hp",
			"/philippines", "/malaysia", "/singapore", "/brazil", "/fsh",	   "/spin",
			"/sigh",		"/dum",		 "/crwd",	   "/desp",	  "/dice",	   "-dice2",
			"-dice3",		"-dice4",	 "-dice5",	   "-dice6",  "/india",	   "/love",
			"/russia",		"-?-",		 "/mobile",	   "/mail",	  "/chinese",  "/antenna1",
			"/antenna2",	"/antenna3", "/hum",	   "/abs",	  "/oops",	   "/spit",
			"/ene",			"/panic",	 "/whisp"};
		int i;
		ARR_FIND(0, ARRAYLENGTH(emo), i, stricmp(message, emo[i]) == 0);
		if (i == ET_DICE1)
			i = rnd() % 6 + ET_DICE1;  // randomize /dice
...
ACMD_FUNC(users) {
	...
	for (i = 0; i < MAX_MAPINDEX; ++i) {
		...
		safesnprintf(buf, sizeof(buf), "%s: %d (%.2f%%)", mapindex_id2name(i), users[i],
...
ACMD_FUNC(reset) {
	pc_resetstate(sd);
	pc_resetskill(sd, 1);
	sprintf(atcmd_output, msg_txt(sd, 208),
...
ACMD_FUNC(trade) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1230));	 // Please enter a player name (usage: @trade <char name>).
		return -1;
	}
...
ACMD_FUNC(setbattleflag) {
	...
	if (!message || !*message || sscanf(message, "%127s %127s %11d", flag, value, &reload) < 2) {
		clif_displaymessage(fd,
							msg_txt(sd, 1231));	 // Usage: @setbattleflag <flag> <value> {<reload>}
		return -1;
	}
...
ACMD_FUNC(unmute) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1234));	 // Please enter a player name (usage: @unmute <char name>).
		return -1;
	}
 ...
 *------------------------------------------*/
ACMD_FUNC(uptime) {
	unsigned long seconds = 0, day = 24 * 60 * 60, hour = 60 * 60, minute = 60, days = 0, hours = 0,
				  minutes = 0;
	nullpo_retr(-1, sd);
	...
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 245), days, hours, minutes,
			 seconds);	// Server Uptime: %ld days, %ld hours, %ld minutes, %ld seconds.
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(refreshall) {
	...
	iter = mapit_getallusers();
	for (iter_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter);
		 iter_sd = (TBL_PC*)mapit_next(iter))
...
ACMD_FUNC(mobinfo) {
	...
	unsigned char mrace[RC_ALL][11] = {"Formless", "Undead", "Beast", "Plant",
	...
	if (!message || !*message) {
		clif_displaymessage(fd, msg_txt(sd, 1239));	 // Please enter a monster name/ID (usage:
													 // @mobinfo <monster_name_or_monster_ID>).
		return -1;
	}
	...
	for (uint16 k = 0; k < count; k++) {
		...
		if (mob->get_bosstype() == BOSSTYPE_MVP)
			sprintf(atcmd_output, msg_txt(sd, 1240), mob->name.c_str(), mob->jname.c_str(),
					mob->sprite.c_str(), mob->id);	// MVP Monster: '%s'/'%s'/'%s' (%d)
		else
			sprintf(atcmd_output, msg_txt(sd, 1241), mob->name.c_str(), mob->jname.c_str(),
					mob->sprite.c_str(), mob->id);	// Monster: '%s'/'%s'/'%s' (%d)
		clif_displaymessage(fd, atcmd_output);
		sprintf(atcmd_output, msg_txt(sd, 1242), mob->lv, mob->status.max_hp, base_exp, job_exp,
				MOB_HIT(mob),
				MOB_FLEE(mob));	 //  Lv:%d  HP:%d  Base EXP:%llu  Job EXP:%llu  HIT:%d  FLEE:%d
		clif_displaymessage(fd, atcmd_output);
		sprintf(
			atcmd_output,
			msg_txt(sd, 1243),	//  DEF:%d  MDEF:%d  STR:%d  AGI:%d  VIT:%d  INT:%d  DEX:%d  LUK:%d
			mob->status.def, mob->status.mdef, mob->status.str, mob->status.agi, mob->status.vit,
		...
		sprintf(
			...
			mob->status.rhw.range, mob->range2, mob->range3, msize[mob->status.size],
			mrace[mob->status.race], melement[mob->status.def_ele], mob->status.ele_lv);
		clif_displaymessage(fd, atcmd_output);
#ifdef RENEWAL
		sprintf(atcmd_output, msg_txt(sd, 827), mob->status.res,
				mob->status.mres);	//  MDEF:%d  RES:%d  MRES:%d
		clif_displaymessage(fd, atcmd_output);
#endif
		// drops
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (id->slots)
				sprintf(atcmd_output2, " - %s[%d]  %02.02f%%", id->ename.c_str(), id->slots,
						(float)droprate / 100);
			else
				sprintf(atcmd_output2, " - %s  %02.02f%%", id->ename.c_str(),
						(float)droprate / 100);
		...
		if (mob->get_bosstype() == BOSSTYPE_MVP) {
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				// Because if there are 3 MVP drops at 50%, the first has a chance of 50%, the
				// second 25% and the third 12.5%
				...
				if (mvppercent > 0) {
					...
					if (j == 1) {
						if (id->slots)
							sprintf(atcmd_output2, " %s[%d]  %02.02f%%", id->ename.c_str(),
									id->slots, mvppercent);
						else
...
ACMD_FUNC(showmobs) {
	...
	if (mob == nullptr || mobdb_checkid(mob_id) == 0) {
		snprintf(atcmd_output, sizeof atcmd_output, msg_txt(sd, 1250),
				 mob_name);	 // Invalid mob id %s!
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
	...
	if (status_has_mode(&mob->status, MD_STATUSIMMUNE) &&
		!pc_has_permission(
			sd, PC_PERM_SHOW_BOSS)) {  // If player group does not have access to boss mobs.
...
ACMD_FUNC(homfriendly) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd, msg_txt(sd, 1258));	 // Please enter a friendly value (usage:
													 // @homfriendly <friendly value [0-1000]>).
		return -1;
	}
...
ACMD_FUNC(homtalk) {
	...
	if (!message || !*message || sscanf(message, "%99[^\n]", mes) < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1260));	 // Please enter a message (usage: @homtalk <message>).
		return -1;
	}
...
ACMD_FUNC(homstats) {
	...
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1266), lv,
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1267), hom->max_hp, min,
			 max);	// Max HP: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1268), hom->max_sp, min,
			 max);	// Max SP: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1269), hom->str / 10, min,
			 max);	// Str: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1270), hom->agi / 10, min,
			 max);	// Agi: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1271), hom->vit / 10, min,
			 max);	// Vit: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1272), hom->int_ / 10, min,
			 max);	// Int: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1273), hom->dex / 10, min,
			 max);	// Dex: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
	...
	;
	snprintf(atcmd_output, sizeof(atcmd_output), msg_txt(sd, 1274), hom->luk / 10, min,
			 max);	// Luk: %d (%d~%d)
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(iteminfo) {
	...
	for (const auto& result : item_array) {
		...
		sprintf(
			atcmd_output,
			msg_txt(sd, 1277),	// Item: '%s'/'%s'[%d] (%u) Type: %s | Extra Effect: %s
			...
			(item_data->script == NULL) ? msg_txt(sd, 1278)
										: msg_txt(sd, 1279)	 // None / With script
		);
		clif_displaymessage(fd, atcmd_output);

		sprintf(atcmd_output, msg_txt(sd, 1280), item_data->value_buy, item_data->value_sell,
				item_data->weight / 10.);  // NPC Buy:%dz, Sell:%dz | Weight: %.1f
		clif_displaymessage(fd, atcmd_output);
		...
		if (item_data->maxchance == -1) {
			...
			if (item_data->maxchance)
				sprintf(atcmd_output, msg_txt(sd, 1282),
						(float)item_data->maxchance /
							100);  //  - Maximal monsters drop chance: %02.02f%%
			else
...
ACMD_FUNC(whodrops) {
	...
	for (const auto& result : item_array) {
		...
		sprintf(atcmd_output, msg_txt(sd, 1285), id->ename.c_str(), id->slots,
				id->nameid);  // Item: '%s'[%d] (ID:%u)
		clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(whereis) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd, msg_txt(sd, 1288));	 // Please enter a monster name/ID (usage:
	...
	for (int i = 0; i < count; i++) {
		...
		if (!mob)
			continue;
		snprintf(atcmd_output, sizeof atcmd_output, msg_txt(sd, 1289),
				 mob->jname.c_str());  // %s spawns in:
		clif_displaymessage(fd, atcmd_output);
		...
		if (spawns.size() <= 0) {
			...
			for (auto& spawn : spawns) {
				...
				if (mapid < 0)
					continue;
				snprintf(atcmd_output, sizeof atcmd_output, "%s (%d)", map_getmapdata(mapid)->name,
						 spawn.qty);
				clif_displaymessage(fd, atcmd_output);
			}
		}
...
ACMD_FUNC(mutearea) {
	...
	if (!message || !*message) {
		clif_displaymessage(fd, msg_txt(sd, 1297));	 // Please enter a time in minutes (usage:
													 // @mutearea/@stfu <time in minutes>).
		return -1;
	}

	time = atoi(message);

	map_foreachinallarea(atcommand_mutearea_sub, sd->bl.m, sd->bl.x - AREA_SIZE,
						 sd->bl.y - AREA_SIZE, sd->bl.x + AREA_SIZE, sd->bl.y + AREA_SIZE, BL_PC,
						 sd->bl.id, time);
...
ACMD_FUNC(rates) {
	...
	snprintf(
		...
		(battle_config.base_exp_rate +
		 (pc_isvip(sd) ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100
					   : 0)) /
			100.,
		(battle_config.job_exp_rate +
		 (pc_isvip(sd) ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100
					   : 0)) /
			100.);
	clif_displaymessage(fd, buf);
	snprintf(
		buf, CHAT_SIZE_MAX, msg_txt(sd, 1299),	// Normal Drop Rates: Common %.2fx / Healing %.2fx /
												// Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_heal +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_use +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_equip +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_card +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card) / 100
					   : 0)) /
			100.);
	clif_displaymessage(fd, buf);
	snprintf(
		buf, CHAT_SIZE_MAX, msg_txt(sd, 1300),	// Boss Drop Rates: Common %.2fx / Healing %.2fx /
												// Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_boss +
		 (pc_isvip(sd)
			  ? (battle_config.vip_drop_increase * battle_config.item_rate_common_boss) / 100
			  : 0)) /
			100.,
		(battle_config.item_rate_heal_boss +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_boss) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_use_boss +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_boss) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_equip_boss +
		 (pc_isvip(sd)
			  ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_boss) / 100
			  : 0)) /
			100.,
		(battle_config.item_rate_card_boss +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card_boss) / 100
					   : 0)) /
			100.);
	clif_displaymessage(fd, buf);
	snprintf(
		buf, CHAT_SIZE_MAX, msg_txt(sd, 1024),	// MVP Drop Rates: Common %.2fx / Healing %.2fx /
												// Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_mvp +
		 (pc_isvip(sd)
			  ? (battle_config.vip_drop_increase * battle_config.item_rate_common_mvp) / 100
			  : 0)) /
			100.,
		(battle_config.item_rate_heal_mvp +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_mvp) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_use_mvp +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_mvp) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_equip_mvp +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_mvp) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_card_mvp +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card_mvp) / 100
					   : 0)) /
			100.);
	clif_displaymessage(fd, buf);
	snprintf(
		buf, CHAT_SIZE_MAX,
		msg_txt(sd, 1301),	// Other Drop Rates: MvP %.2fx / Card-Based %.2fx / Treasure %.2fx
		(battle_config.item_rate_mvp +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_mvp) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_adddrop +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_adddrop) / 100
					   : 0)) /
			100.,
		(battle_config.item_rate_treasure +
		 (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_treasure) / 100
					   : 0)) /
			100.);
	clif_displaymessage(fd, buf);
...
ACMD_FUNC(me) {
	...
	if (!message || !*message || sscanf(message, "%255[^\n]", tempmes) < 0) {
		clif_displaymessage(fd,
							msg_txt(sd, 1302));	 // Please enter a message (usage: @me <message>).
		return -1;
	}
...
ACMD_FUNC(fakename) {
	...
	if (strlen(message) < 2) {
		clif_displaymessage(sd->fd,
							msg_txt(sd, 1309));	 // Fake name must be at least two characters.
		return -1;
	}
...
ACMD_FUNC(mapflag) {
	...
	if (!message || !*message || (sscanf(message, "%49s %6hd", flag_name, &flag) < 1)) {
		...
		clif_displaymessage(sd->fd, " ");
		clif_displaymessage(
			sd->fd, msg_txt(sd, 1312));	 // Usage: "@mapflag monster_noteleport 1" (0=Off | 1=On)
		clif_displaymessage(
			sd->fd,
			msg_txt(sd, 1313));	 // Type "@mapflag available" to list the available mapflags.
		return 1;
	}
	...
	if (strcasecmp(flag_name, "available")) {
		...
		if (mapflag != MF_INVALID) {
			...
			if (flag > 0 && util::vector_exists(disabled_mf, mapflag)) {
				sprintf(atcmd_output,
						"[ @mapflag ] %s flag cannot be enabled as it requires unique values.",
						flag_name);
				clif_displaymessage(sd->fd, atcmd_output);
			} else {
				map_setmapflag(sd->bl.m, mapflag, flag != 0);
				sprintf(atcmd_output, "[ @mapflag ] %s flag has been set to %s value = %hd",
						flag_name, flag ? "On" : "Off", flag);
				clif_displaymessage(sd->fd, atcmd_output);
			}
			return 0;
		} else {
...
ACMD_FUNC(invite) {
	...
	if (battle_config.duel_only_on_same_map && target_sd->bl.m != sd->bl.m) {
		sprintf(
			atcmd_output, msg_txt(sd, 364),
			atcmd_player_name);	 // Duel: You can't invite %s because he/she isn't on the same map.
		clif_displaymessage(fd, atcmd_output);
		return 0;
	}
...
ACMD_FUNC(duel) {
	...
	if (!duel_checktime(sd)) {
		char output[CHAT_SIZE_MAX];
		sprintf(output, msg_txt(sd, 356),
				battle_config.duel_time_interval);	// "Duel: You can take part in duel only one
													// time per %d minutes."
		clif_displaymessage(fd, output);
		return 0;
	}
		...
		if (sscanf(message, "%11u", &maxpl) >= 1) {
			...
			if (target_sd != NULL) {
				...
				if ((newduel = duel_create(sd, 2)) != -1) {
					if (target_sd->duel_group > 0 || target_sd->duel_invite > 0) {
						clif_displaymessage(fd,
											msg_txt(sd, 353));	// "Duel: Player already in duel."
						return 0;
					}
...
ACMD_FUNC(cash) {
	...
	if (sd->state.cashshop_open) {
		clif_displaymessage(
			fd, msg_txt(sd, 1376));	 // Please close the cashshop before using this command.
		return -1;
	}
	...
	if (!strcmpi(parent_cmd, "cash")) {
		if (value > 0) {
			if ((ret = pc_getcash(sd, value, 0, LOG_TYPE_COMMAND)) >= 0) {
				...
				if (!battle_config.cashshop_show_points) {
					sprintf(output, msg_txt(sd, 505), ret,
							sd->cashPoints);  // Gained %d cash points. Total %d points.
					clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
				}
			} else
				clif_displaymessage(fd,
									msg_txt(sd, 149));	// Impossible to increase the number/value.
		} else {
			...
			if ((ret = pc_paycash(sd, -value, 0, LOG_TYPE_COMMAND)) >= 0) {
				// If this option is set, the message is already sent by pc function
				if (!battle_config.cashshop_show_points) {
					sprintf(output, msg_txt(sd, 410), ret,
							sd->cashPoints);  // Removed %d cash points. Total %d points.
					clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
				}
			} else
				clif_displaymessage(fd, msg_txt(sd, 41));  // Unable to decrease the number/value.
		...
		if (value > 0) {
			if ((ret = pc_getcash(sd, 0, value, LOG_TYPE_COMMAND)) >= 0) {
				sprintf(output, msg_txt(sd, 506), ret,
						sd->kafraPoints);  // Gained %d kafra points. Total %d points.
				clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			} else
				clif_displaymessage(fd,
									msg_txt(sd, 149));	// Impossible to increase the number/value.
			...
			if ((ret = pc_paycash(sd, 0, -value, LOG_TYPE_COMMAND)) >= 0) {
				sprintf(output, msg_txt(sd, 411), ret,
						sd->kafraPoints);  // Removed %d kafra points. Total %d points.
				clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			} else
				clif_displaymessage(fd, msg_txt(sd, 41));  // Unable to decrease the number/value.
		}
...
ACMD_FUNC(clone) {
	...
	if (pc_get_group_level(pl_sd) > pc_get_group_level(sd)) {
		clif_displaymessage(
			fd, msg_txt(sd, 126));	// Cannot clone a player of higher GM level than yourself.
		return 0;
	}
	...
	if ((x = mob_clone_spawn(pl_sd, sd->bl.m, x, y, "", master, MD_NONE, flag ? 1 : 0, 0)) > 0) {
		clif_displaymessage(
			fd,
			msg_txt(sd,
					128 + flag * 2));  // Evil Clone spawned. Clone spawned. Slave clone spawned.
		return 0;
	}
	clif_displaymessage(fd,
						msg_txt(sd, 129 + flag * 2));  // Unable to spawn evil clone. Unable to
													   // spawn clone. Unable to spawn slave clone.
	return 0;
}
...
ACMD_FUNC(request) {
	if (!message || !*message) {
		clif_displaymessage(sd->fd,
							msg_txt(sd, 277));	// Usage: @request <petition/message to online GMs>.
		return -1;
	}
...
ACMD_FUNC(feelreset) {
	if ((sd->class_ & MAPID_UPPERMASK) != MAPID_STAR_GLADIATOR) {
		clif_displaymessage(sd->fd,
							msg_txt(sd, 35));  // You can't use this command with this class.
		return -1;
	}
...
ACMD_FUNC(ksprotection) {
	...
	if (sd->state.noks) {
		...
		if (!message || !*message || !strcmpi(message, "party")) {	// Default is Party
			sd->state.noks = 2;
			clif_displaymessage(fd,
								msg_txt(sd, 1326));	 // [ K.S Protection Active - Option: Party ]
			...
			sd->state.noks = 3;
			clif_displaymessage(fd,
								msg_txt(sd, 1328));	 // [ K.S Protection Active - Option: Guild ]
		} else
...
ACMD_FUNC(itemlist) {
	...
	for (i = 0; i < size; ++i) {
		...
		if (count == 1) {
			StringBuf_Printf(&buf, msg_txt(sd, 1332), location,
		...
		if (it->refine)
			StringBuf_Printf(&buf, "%d %s %+d (%s, id: %u)", it->amount, itd->ename.c_str(),
							 it->refine, itd->name.c_str(), it->nameid);
		else
			StringBuf_Printf(&buf, "%d %s (%s, id: %u)", it->amount, itd->ename.c_str(),
							 itd->name.c_str(), it->nameid);
		...
		if (it->card[0] == CARD0_PET) {	 // pet egg
			if (it->card[3] & 1)
				StringBuf_Printf(
					&buf, msg_txt(sd, 1348),
					(unsigned int)MakeDWord(it->card[1],
											it->card[2]));	//  -> (pet egg, pet id: %u, named)
			else
				StringBuf_Printf(
					&buf, msg_txt(sd, 1349),
					(unsigned int)MakeDWord(it->card[1],
											it->card[2]));	//  -> (pet egg, pet id: %u, unnamed)
		} else if (it->card[0] == CARD0_FORGE) {			// forged item
			StringBuf_Printf(
				&buf, msg_txt(sd, 1350), (unsigned int)MakeDWord(it->card[2], it->card[3]),
				it->card[1] >> 8,
				it->card[1] &
					0x0f);	//  -> (crafted item, creator id: %u, star crumbs %d, element %d)
		} else if (it->card[0] == CARD0_CREATE) {  // created item
			StringBuf_Printf(&buf, msg_txt(sd, 1351),
							 (unsigned int)MakeDWord(
								 it->card[2], it->card[3]));  //  -> (produced item, creator id: %u)
		} else {											  // normal item
			int counter2 = 0;
			...
			for (j = 0; j < itd->slots; ++j) {
				...
				StringBuf_Printf(&buf, "#%d %s (id: %u)", counter2, card->ename.c_str(),
								 card->nameid);
			}
	...
	if (count == 0)
		...
		StringBuf_Printf(&buf, msg_txt(sd, 1354), counter, count,
...
ACMD_FUNC(delitem) {
	...
	if (!message || !*message ||
		...
		amount < 1) {
		clif_displaymessage(
			fd, msg_txt(sd, 1355));	 // Please enter an item name/ID, a quantity, and a player name
	...
	while (amount && (idx = pc_search_inventory(sd, nameid)) != -1) {
		...
		if (sd->inventory_data[idx]->type == IT_PETEGG &&
			...
			intif_delete_petdata(MakeDWord(sd->inventory.u.items_inventory[idx].card[1],
										   sd->inventory.u.items_inventory[idx].card[2]));
		}
	...
	if (amount == total) {
		...
		sprintf(atcmd_output, msg_txt(sd, 115), total - amount, total - amount,
				total);	 // %d item(s) removed. Player had only %d on %d items.
		clif_displaymessage(fd, atcmd_output);
	} else {
...
ACMD_FUNC(font) {
	...
	if (font_id == 0) {
		if (sd->status.font) {
			...
			clif_displaymessage(fd,
								msg_txt(sd, 1357));	 // Use @font <1-9> to change your message font.
			clif_displaymessage(
				fd, msg_txt(sd, 1358));	 // Use 0 or no parameter to return to normal font.
		}
...
static void atcommand_commands_sub(struct map_session_data* sd, const int fd, AtCommandType type) {
	...
	for (cmd = (AtCommandInfo*)dbi_first(iter); dbi_exists(iter);
		 cmd = (AtCommandInfo*)dbi_next(iter)) {
		unsigned int slen = 0;
...
ACMD_FUNC(accinfo) {
	...
	if (!message || !*message || strlen(message) > NAME_LENGTH ||
		...
		clif_displaymessage(
			fd, msg_txt(sd, 1365));	 // Usage: @accinfo/@accountinfo <account_id/char name>
		clif_displaymessage(
			fd,
			msg_txt(sd,
					1366));	 // You may search partial name by making use of '%' in the search, ex.
							 // "@accinfo %Mario%" lists all characters whose name contains "Mario".
		return -1;
	} else if (type != 0) {
...
ACMD_FUNC(set) {
	...
	if (!message || !*message || (toset = sscanf(message, "%45s %127[^\n]s", reg, val)) < 1) {
		...
		clif_displaymessage(
			fd, msg_txt(sd, 1369));	 // Usage: ex. "@set PoringCharVarSTR$ Super Duper String"
		clif_displaymessage(fd, msg_txt(sd, 1370));	 // Usage: ex. "@set PoringCharVarSTR$" outputs
													 // its value, Super Duper String.
		return -1;
	}

	/* disabled variable types (they require a proper script state to function, so allowing them
	...
	if (reg[0] == '.') {
		...
		clif_displaymessage(fd,
							msg_txt(sd, 1372));	 // Instance variables may not be used with @set.
		return -1;
	}
...
ACMD_FUNC(addperm) {
	...
	if (add && pc_has_permission(sd, pc_g_permission_name[i].permission)) {
		sprintf(atcmd_output, msg_txt(sd, 1381), sd->status.name,
				pc_g_permission_name[i].name);	// User '%s' already possesses the '%s' permission.
		clif_displaymessage(fd, atcmd_output);
		return -1;
	} else if (!add && !pc_has_permission(sd, pc_g_permission_name[i].permission)) {
		sprintf(atcmd_output, msg_txt(sd, 1382), sd->status.name,
				pc_g_permission_name[i].name);	// User '%s' doesn't possess the '%s' permission.
		clif_displaymessage(fd, atcmd_output);
...
#define MC_CART_MDFY(idx, x)                            \
	...
	if (!message || !*message || val < 0 || val > MAX_CARTS) {
		sprintf(atcmd_output, msg_txt(sd, 1390), command,
...
ACMD_FUNC(join) {
	...
	if (!message || !*message || sscanf(message, "%19s %19s", chname, pass) < 1) {
		sprintf(atcmd_output, msg_txt(sd, 1399),
...
static inline void atcmd_channel_help(struct map_session_data* sd, const char* command) {
	...
	// option create
	if (can_create) {
		sprintf(atcmd_output, msg_txt(sd, 1415),
				command);  // * %s create <#channel_name> <channel_password>
		clif_displaymessage(fd, atcmd_output);
	...
	if (can_create) {
		...
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(
			fd, msg_txt(sd, 1420));	 // -- Lists all available colors for custom channels.
	}

	// option setcolor
	if (can_create) {
		sprintf(atcmd_output, msg_txt(sd, 1421),
				command);  // * %s setcolor <#channel_name> <color_name>
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(
			fd,
			msg_txt(
	...
	// option join
	sprintf(atcmd_output, msg_txt(sd, 1473),
			command);  // * %s join <#channel_name> <channel_password>
	clif_displaymessage(fd, atcmd_output);
	...
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(fd,
						msg_txt(sd, 1428));	 // -- Binds your global chat to the specified channel,
											 // sending all global messages to that channel.

	// option unbind
	sprintf(atcmd_output, msg_txt(sd, 1429), command);	// * %s unbind
	clif_displaymessage(fd, atcmd_output);
	clif_displaymessage(
		fd, msg_txt(sd, 1430));	 // -- Unbinds your global chat from the attached channel, if any.
	...
	if (can_create) {
		...
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(fd,
							msg_txt(sd, 1457));	 // -- Bans the specified player from the channel.
		sprintf(atcmd_output, msg_txt(sd, 1458), command);	// * %s banlist <#channel_name>
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(
			fd, msg_txt(sd, 1459));	 // -- Lists all players banned from the specified channel.
		sprintf(atcmd_output, msg_txt(sd, 1460), command);	// * %s unban <#channel_name> <player>
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(fd,
							msg_txt(sd, 1461));	 // -- Unbans the specified player from the channel.
		sprintf(atcmd_output, msg_txt(sd, 1467), command);	// * %s unbanall <#channel_name>
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(fd,
							msg_txt(sd, 1468));	 // -- Clears all bans from the specified channel.
	}

	// option setopt
	if (can_create) {
		sprintf(atcmd_output, msg_txt(sd, 1462),
				command);  // * %s setopt <#channel_name> <option> <value>
		clif_displaymessage(fd, atcmd_output);
		clif_displaymessage(
			fd, msg_txt(sd, 1463));	 // -- Sets an option and value for the specified channel.
	}
...
ACMD_FUNC(channel) {
	...
	if (strcmpi(key, "delete") == 0 && pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN)) {
		...
		if (strcmpi(key, "create") == 0 && (channel_config.private_channel.allow ||
											pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN))) {
			if (sub4[0] != '\0') {
				clif_displaymessage(fd,
									msg_txt(sd, 1408));	 // Channel password may not contain spaces.
				return -1;
			}
...
ACMD_FUNC(fontcolor) {
	...
	if (strcmpi(message, "Normal") == 0) {
		...
		unsigned char k;
		ARR_FIND(0, channel_config.colors_count, k,
				 (strcmpi(message, channel_config.colors_name[k]) == 0));
...
ACMD_FUNC(langtype) {
	...
	if (sscanf(message, "%3s", langstr) >= 1) {
		...
		if (msg_checklangtype(lang, false) == 1) {	// Verify it's enabled and set it
			...
			sd->langtype = lang;
			sprintf(output, msg_txt(sd, 461),
					msg_langtype2langstr(lang));  // Language is now set to %s.
			clif_displaymessage(fd, output);
			return 0;
	...
	// wrong or no entry
	clif_displaymessage(
		fd, msg_txt(sd, 460));	// Please enter a valid language (usage: @langtype <language>).
...
ACMD_FUNC(vip) {
	...
	if (pl_sd->vip.time <= now) {
		...
		sprintf(atcmd_output, msg_txt(pl_sd, 705), year, month, day, hour,
				minute);  // Your VIP status is valid for %d years, %d months, %d days, %d hours and
						  // %d minutes.
		clif_displaymessage(pl_sd->fd, atcmd_output);
		...
		if (pl_sd != sd) {
			sprintf(atcmd_output, msg_txt(sd, 706), pl_sd->status.name, year, month, day, hour,
					minute);  // Player '%s' is now VIP for %d years, %d months, %d days, %d hours
							  // and %d minutes.
			clif_displaymessage(fd, atcmd_output);
	...
	chrif_req_login_operation(pl_sd->status.account_id, pl_sd->status.name, CHRIF_OP_LOGIN_VIP,
							  vipdifftime, 7, 0);
	return 0;
}
...
ACMD_FUNC(showrate) {
	...
	if (!sd->vip.disableshowrate) {
		safestrncpy(atcmd_output, msg_txt(sd, 718),
					CHAT_SIZE_MAX);	 // Personal rate information is not displayed now.
		sd->vip.disableshowrate = 1;
	} else {
		safestrncpy(atcmd_output, msg_txt(sd, 719),
					CHAT_SIZE_MAX);	 // Personal rate information will be shown.
...
ACMD_FUNC(fullstrip) {
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, msg_txt(sd, 349));	// Please enter a player name (usage:
													// @fullstrip/@warpto/@goto <char name/ID>).
		return -1;
	}

	if ((tsd = map_nick2sd(atcmd_player_name, false)) == NULL &&
		(tsd = map_id2sd(atoi(atcmd_player_name))) == NULL) {
		clif_displaymessage(fd, msg_txt(sd, 3));  // Character not found.
		return -1;
	}
...
ACMD_FUNC(costume) {
	...
	for (k = 0; k < len; k++) {
		if (sd->sc.data[name2id[k]]) {
			sprintf(atcmd_output, msg_txt(sd, 724),
					names[k]);	// You're already wearing a(n) '%s' costume, type '@costume' to
								// remove it.
			clif_displaymessage(sd->fd, atcmd_output);
			return -1;
		}
	...
	sc_start(&sd->bl, &sd->bl, (sc_type)name2id[k], 100, name2id[k] == SC_DRESSUP ? 1 : 0,
			 INFINITE_TICK);
...
ACMD_FUNC(cloneequip) {
	...
	if (!(pl_sd = map_nick2sd(atcmd_player_name, true)) &&
		!(pl_sd = map_charid2sd(atoi(atcmd_player_name)))) {
...
/**
 * Clone other player's statuses/parameters using method same like ACMD_FUNC(param), doesn't use
 * stat point Usage: @clonestat <char name/ID>
 * http://rathena.org/board/topic/95076-new-atcommands-suggestion/
 * @author [Cydh], [Antares]
 */
ACMD_FUNC(clonestat) {
	...
	switch (i) {
		case 0:
			if (quest_check(sd, quest_id, HAVEQUEST) >= 0) {
				sprintf(atcmd_output, msg_txt(sd, 1507),
						quest_id);	// Character already has quest %d.
				clif_displaymessage(fd, atcmd_output);
				return -1;
			}
		...
		case 1:
			if (quest_check(sd, quest_id, HAVEQUEST) < 0) {
				sprintf(atcmd_output, msg_txt(sd, 1508),
						quest_id);	// Character doesn't have quest %d.
				clif_displaymessage(fd, atcmd_output);
				return -1;
			}
		...
		case 3:
			...
			clif_displaymessage(fd, atcmd_output);
			sprintf(atcmd_output, msg_txt(sd, 1510),
					quest_check(sd, quest_id, HAVEQUEST));	// HAVEQUEST : %d
			clif_displaymessage(fd, atcmd_output);
			sprintf(atcmd_output, msg_txt(sd, 1511),
					quest_check(sd, quest_id, HUNTING));  // HUNTING   : %d
			clif_displaymessage(fd, atcmd_output);
			sprintf(atcmd_output, msg_txt(sd, 1512),
					quest_check(sd, quest_id, PLAYTIME));  // PLAYTIME  : %d
			clif_displaymessage(fd, atcmd_output);
			break;
	}
	return 0;
...
ACMD_FUNC(refineui) {
	...
	clif_displaymessage(
		fd, msg_txt(sd, 773));	// This command requires packet version 2016-10-12 or newer.
	return -1;
#else
...
ACMD_FUNC(stylist) {
	...
	clif_displaymessage(
		fd, msg_txt(sd, 798));	// This command requires packet version 2015-11-04 or newer.
	return -1;
#else
...
ACMD_FUNC(addfame) {
	...
	if (!pc_addfame(*sd, famepoint)) {
		sprintf(atcmd_output, msg_txt(sd, 1517),
				job_name(sd->status.class_));  // Cannot add fame to class '%s'.
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}

	sprintf(atcmd_output, msg_txt(sd, 1518), famepoint,
			sd->status.name);  // %d points were added to '%s'.
	clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(enchantgradeui) {
	...
	sprintf(atcmd_output, msg_txt(sd, 798),
			"2020-07-24");	// This command requires packet version %s or newer.
	clif_displaymessage(fd, atcmd_output);
	return -1;
#else
 ...
 **/
#define ACMD_DEF(x) \
	{ #x, atcommand_##x, NULL, NULL, 0 }
#define ACMD_DEF2(x2, x) \
	{ x2, atcommand_##x, NULL, NULL, 0 }
// Define with restriction
#define ACMD_DEFR(x, r) \
	{ #x, atcommand_##x, NULL, NULL, r }
#define ACMD_DEF2R(x2, x, r) \
	{ x2, atcommand_##x, NULL, NULL, r }
...
#include "../custom/atcommand_def.inc"
	...
	for (i = 0; i < ARRAYLENGTH(atcommand_base); i++) {
		if (atcommand_exists(
				atcommand_base[i].command)) {  // Should not happen if atcommand_base[] array is OK
 ...
 *------------------------------------------*/
bool atcommand_exists(const char* name) { return strdb_exists(atcommand_db, name); }
...
static void atcommand_get_suggestions(struct map_session_data* sd, const char* name,
	...
	for (command_info = (AtCommandInfo*)dbi_first(atcommand_iter); dbi_exists(atcommand_iter);
		...
		if (full_count < MAX_SUGGESTIONS && match != NULL && match != command_info->command &&
			can_use) {
	...
	if ((full_count + prefix_count) > 0) {
		...
		if (prefix_count < MAX_SUGGESTIONS) {
			memmove(&suggestions[prefix_count], full_match,
					sizeof(char*) * (MAX_SUGGESTIONS - prefix_count));
...
bool is_atcommand(const int fd, struct map_session_data* sd, const char* message, int type) {
		...
		if ((n = sscanf(message, "%255s \"%23[^\"]\" %255[^\n]", command, charname, params)) < 2 &&
			...
			if (pc_get_group_level(sd) == 0) {
				...
				if (!info || info->char_groups[sd->group->index] ==
								 0)	 // If we can't use or doesn't exist: don't even display the
									 // command failed message
					return false;
			}

			sprintf(output, msg_txt(sd, 1388),
					charcommand_symbol);  // Charcommand failed (usage: %c<command> <char name>
										  // <parameters>).
			clif_displaymessage(fd, output);
			return true;
		}
	...
	if (battle_config.hom_idle_no_share && sd->hd &&
		battle_config.idletime_hom_option & IDLE_ATCOMMAND)
		sd->idletime_hom = last_tick;
	if (battle_config.mer_idle_no_share && sd->md &&
		battle_config.idletime_mer_option & IDLE_ATCOMMAND)
	...
	// Grab the command information and check for the proper GM level required to use it or if the
	// command exists
	info = get_atcommandinfo_byname(atcommand_alias_db.checkAlias(command + 1));
	if (info == NULL) {
...
void atcommand_db_load_groups() {
	...
	for (cmd = (AtCommandInfo*)dbi_first(iter); dbi_exists(iter);
		...
		for (auto& it : player_group_db) {
			cmd->at_groups[it.second->index] =
				it.second->can_use_command(cmd->command, COMMAND_ATCOMMAND);
			cmd->char_groups[it.second->index] =
				it.second->can_use_command(cmd->command, COMMAND_CHARCOMMAND);
		}
	}
...
void atcommand_doload(void) {
	atcommand_db_clear();
	atcommand_db =
		stridb_alloc((DBOptions)(DB_OPT_DUP_KEY | DB_OPT_RELEASE_DATA), ATCOMMAND_LENGTH);
