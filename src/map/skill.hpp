...
enum e_skill_inf2 : uint8 {
	...
	INF2_ISENSEMBLE,
	INF2_ISTRAP,
	INF2_TARGETSELF,  // Refers to ground placed skills that will target the caster as well (like
					  // Grandcross)
	INF2_NOTARGETSELF,
	INF2_PARTYONLY,
	INF2_GUILDONLY,
	...
	INF2_IGNORECICADA,	// Skill is not blocked by SC_UTSUSEMI or SC_BUNSINJYUTSU (physical-skill
						// only)
...
/// This is to prevent hackers from sending a skill packet after cast but before a timer triggers
/// castend
const t_tick SECURITY_CASTTIME = 100;

/// Flags passed to skill_attack/skill_area_sub
enum e_skill_display {
	SD_LEVEL = 0x1000,	// skill_attack will send -1 instead of skill level (affects display of some
						// skills)
	SD_ANIMATION = 0x2000,	// skill_attack will use '5' instead of the skill's 'type' (this makes
							// skills show an animation). Also being used in skill_attack for splash
							// skill (NK_SPLASH) to check status_check_skilluse
	SD_SPLASH = 0x4000,		// skill_area_sub will count targets in skill_area_temp[2]
	SD_PREAMBLE = 0x8000,	// skill_area_sub will transmit a 'magic' damage packet (-30000 dmg) for
...
struct s_skill_db {
	...
	uint8
		delaynodex;	 ///< 1 - Not affected by dex, 2 - Not affected by SC, 4 - Not affected by item
	...
	int32 unit_layout_type[MAX_SKILL_LEVEL];  ///< Layout type. -1 is special layout, others are
											  ///< square with lenght*width: (val*2+1)^2
	...
	uint16
		unit_nonearnpc_range;  ///< Additional range for UF_NONEARNPC or INF2_DISABLENEARNPC [Cydh]
...
class SkillDatabase : public TypesafeCachedYamlDatabase<uint16, s_skill_db> {
   private:
	/// Skill ID to Index lookup: skill_index = skill_get_index(skill_id) - [FWI] 20160423 the whole
	/// index thing should be removed.
	uint16 skilldb_id2idx[(UINT16_MAX + 1)];
	/// Skill count, also as last index
	uint16 skill_num;

	template <typename T, size_t S>
	bool parseNode(const std::string &nodeName, const std::string &subNodeName,
...
#define skill_get_index(skill_id)                                 \
	skill_db.get_index((skill_id), false, __FUNCTION__, __FILE__, \
					   __LINE__)  /// Get skill index from skill_id (common usage on source)
...
int skill_break_equip(struct block_list *src, struct block_list *bl, unsigned short where, int rate,
					  int flag);
int skill_strip_equip(struct block_list *src, struct block_list *bl, unsigned short where, int rate,
					  int lv, int time);
// Skills unit
...
// (Epoque:) To-do: replace this macro with some sort of skill tree check (rather than hard-coded
// skill names)
...
int skill_elementalanalysis(struct map_session_data *sd, int n, uint16 skill_lv,
							unsigned short *item_list);	 // Sorcerer Four Elemental Analisys.
int skill_changematerial(struct map_session_data *sd, int n,
						 unsigned short *item_list);  // Genetic Change Material.
...
#define SKILL_CHK_HOMUN(skill_id) \
	((skill_id) >= HM_SKILLBASE && (skill_id) < HM_SKILLBASE + MAX_HOMUNSKILL)
#define SKILL_CHK_MERC(skill_id) \
	((skill_id) >= MC_SKILLBASE && (skill_id) < MC_SKILLBASE + MAX_MERCSKILL)
#define SKILL_CHK_ELEM(skill_id) \
	((skill_id) >= EL_SKILLBASE && (skill_id) < EL_SKILLBASE + MAX_ELEMENTALSKILL)
#define SKILL_CHK_ABR(skill_id) \
	((skill_id) >= ABR_SKILLBASE && (skill_id) < ABR_SKILLBASE + MAX_ABRSKILL)
#define SKILL_CHK_GUILD(skill_id) \
	((skill_id) >= GD_SKILLBASE && (skill_id) < GD_SKILLBASE + MAX_GUILDSKILL)
