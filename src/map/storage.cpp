 ...
 **/
bool storage_exists(uint8 id) { return util::umap_find(storage_db, (uint16)id) != nullptr; }
 ...
 **/
static enum e_storage_add storage_canAddItem(struct s_storage *stor, int idx, struct item items[],
											 int amount, int max_num) {
...
static enum e_storage_add storage_canGetItem(struct s_storage *stor, int idx, int amount) {
	// If last index check is sd->storage_size, if player isn't VIP anymore but there's item, player
	// can't take it Example the storage size when not VIP anymore is 350/300, player still can take
	// the 301st~349th item.
 ...
 */
static int storage_additem(struct map_session_data *sd, struct s_storage *stor, struct item *it,
						   int amount) {
		...
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_storage[i], it)) {	// existing items found, stack them
				if (amount > MAX_AMOUNT - stor->u.items_storage[i].amount ||
					(data->stack.storage &&
					 amount > data->stack.amount - stor->u.items_storage[i].amount))
					return 2;
 ...
 */
void storage_storageadd(struct map_session_data *sd, struct s_storage *stor, int index,
						int amount) {
	...
	result =
		storage_canAddItem(stor, index, sd->inventory.u.items_inventory, amount, MAX_INVENTORY);
 ...
 * @return 0:fail, 1:success
 */
void storage_storageget(struct map_session_data *sd, struct s_storage *stor, int index,
						int amount) {
	...
	if ((flag = pc_additem(sd, &stor->u.items_storage[index], amount, LOG_TYPE_STORAGE)) ==
		ADDITEM_SUCCESS)
 ...
 * @return 0:fail, 1:success
 */
void storage_storageaddfromcart(struct map_session_data *sd, struct s_storage *stor, int index,
								int amount) {
	enum e_storage_add result;
	nullpo_retv(sd);
 ...
 * @return 0:fail, 1:success
 */
void storage_storagegettocart(struct map_session_data *sd, struct s_storage *stor, int index,
							  int amount) {
	unsigned char flag = 0;
	enum e_storage_add result;
 ...
 */
void storage_guild_delete(int guild_id) { guild_storage_db.erase(guild_id); }
...
char storage_guild_storageopen(struct map_session_data *sd) {
	...
	if (!pc_can_give_items(
			sd)) {	// check is this GM level can open guild storage and store items [Lupus]
		clif_displaymessage(sd->fd, msg_txt(sd, 246));
...
void storage_guild_log(struct map_session_data *sd, struct item *item, int16 amount) {
	...
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`time`, `guild_id`, `char_id`, `name`, `nameid`, `amount`, "
					 "`identify`, `refine`, `attribute`, `unique_id`, `bound`, `enchantgrade`",
	...
	StringBuf_Printf(
		&buf, ") VALUES(NOW(),'%u','%u', '%s', '%u', '%d','%d','%d','%d','%" PRIu64 "','%d','%d'",
		sd->status.guild_id, sd->status.char_id, sd->status.name, item->nameid, amount,
		item->identify, item->refine, item->attribute, item->unique_id, item->bound,
		item->enchantgrade);

	for (i = 0; i < MAX_SLOTS; i++) StringBuf_Printf(&buf, ",'%u'", item->card[i]);
	for (i = 0; i < MAX_ITEM_RDM_OPT; i++)
		StringBuf_Printf(&buf, ",'%d','%d','%d'", item->option[i].id, item->option[i].value,
						 item->option[i].param);
	StringBuf_Printf(&buf, ")");
...
enum e_guild_storage_log storage_guild_log_read_sub(struct map_session_data *sd,
	...
	StringBuf_AppendStr(&buf,
						" , `nameid`, `identify`, `refine`, `attribute`, `expire_time`, `bound`, "
	...
	StringBuf_Printf(&buf, " FROM `%s` WHERE `guild_id`='%u'", guild_storage_log_table,
					 sd->status.guild_id);
	...
	for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
		SqlStmt_BindColumn(stmt, 12 + MAX_SLOTS + j * 3, SQLDT_SHORT, &entry.item.option[j].id, 0,
						   NULL, NULL);
		SqlStmt_BindColumn(stmt, 12 + MAX_SLOTS + j * 3 + 1, SQLDT_SHORT,
						   &entry.item.option[j].value, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, 12 + MAX_SLOTS + j * 3 + 2, SQLDT_CHAR,
						   &entry.item.option[j].param, 0, NULL, NULL);
	}
...
enum e_guild_storage_log storage_guild_log_read(struct map_session_data *sd) {
	...
	enum e_guild_storage_log ret =
		storage_guild_log_read_sub(sd, log, MAX_GUILD_STORAGE_LOG_PACKET);
 ...
 */
bool storage_guild_additem(struct map_session_data *sd, struct s_storage *stor,
	...
	if ((item_data->bound == BOUND_ACCOUNT || item_data->bound > BOUND_GUILD) &&
		...
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_guild[i], item_data)) {
				if (amount > MAX_AMOUNT - stor->u.items_guild[i].amount ||
					(id->stack.guild_storage &&
					 amount > id->stack.amount - stor->u.items_guild[i].amount))
					return false;
	...
	if (itemdb_isstackable2(id.get())) {  // Stackable
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_guild[i], item)) {
				...
				amount = min(amount, ((id->stack.guild_storage) ? id->stack.amount : MAX_AMOUNT) -
										 stor->u.items_guild[i].amount);
				if (amount != item->amount)
					ShowWarning(
						"storage_guild_additem2: Stack limit reached! Altered amount of item "
						"\"" CL_WHITE "%s" CL_RESET "\" (%u). '" CL_WHITE "%d" CL_RESET
						"' -> '" CL_WHITE "%d" CL_RESET "'.\n",
...
void storage_guild_storageclose(struct map_session_data *sd) {
	...
	if (stor->status) {
		if (save_settings & CHARSAVE_STORAGE)
			chrif_save(sd,
					   CSAVE_INVENTORY | CSAVE_CART);  // This one also saves the storage. [Skotlex]
		else
