...
const char *macro_allowed_answer_chars =
...
uint64 AttendanceDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Rewards")) {
		...
		for (const auto &rewardNode : rewardsNode) {
			...
			std::shared_ptr<s_attendance_reward> reward =
				util::map_find(attendance_period->rewards, day);
			...
			if (this->nodeExists(rewardNode, "Amount")) {
				...
				if (amount == 0) {
					ShowWarning(
						"pc_attendance_load: Invalid reward count %hu for day %d. Defaulting to "
						"1...\n",
						amount, day + 1);
					amount = 1;
				} else if (amount > MAX_AMOUNT) {
					ShowError(
						"pc_attendance_load: Reward count %hu above maximum %hu for day %d. "
						"Defaulting to %hu...\n",
	...
	if (this->nodeExists(node, "LevelDifferences")) {
		for (const auto &levelNode : node["LevelDifferences"]) {
			...
			if (std::abs(difference) > MAX_LEVEL) {
				this->invalidWarning(levelNode["Difference"],
...
void PenaltyDatabase::loadingFinished() {
	for (const auto &pair : *this) {
		for (int i = MAX_LEVEL - 1, max = ARRAYLENGTH(pair.second->rate), last_rate = 100; i < max;
			 i++) {
...
const struct sg_data sg_info[MAX_PC_FEELHATE] = {
	...
	{SG_MOON_ANGER, SG_MOON_BLESS, SG_MOON_COMFORT, "PC_FEEL_MOON", "PC_HATE_MOB_MOON",
	 is_day_of_moon},
	{SG_STAR_ANGER, SG_STAR_BLESS, SG_STAR_COMFORT, "PC_FEEL_STAR", "PC_HATE_MOB_STAR",
	 is_day_of_star}};

void pc_set_reg_load(bool val) { reg_load = val; }
 ...
 * @return Group ID
 */
int pc_get_group_id(struct map_session_data *sd) { return sd->group_id; }

/** Get player's group Level
 * @param sd
 * @return Group Level
 */
int pc_get_group_level(struct map_session_data *sd) { return sd->group->level; }
...
int pc_addsoulball(map_session_data *sd, int max) {
	...
	sd->soulball++;
	sc_start(&sd->bl, &sd->bl, SC_SOULENERGY, 100, sd->soulball,
			 skill_get_time2(SP_SOULCOLLECT, 1));
	clif_soulball(sd);
...
/**
 * Check whether a player ID is in the fame rankers list of its job, returns his/her position if so,
 * 0 else
 * @param sd
...
void pc_inventory_rentals(struct map_session_data *sd) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {  // Check for Rentals on Inventory
		...
		if (sd->inventory.u.items_inventory[i].expire_time <= time(NULL)) {
			...
			unsigned int expire_tick =
				(unsigned int)(sd->inventory.u.items_inventory[i].expire_time - time(NULL));
	...
	if (c > 0)	// min(next_tick,3600000) 1 hour each timer to keep announcing to the owner, and to
				// avoid a but with rental time > 15 days
		sd->rental_timer =
			add_timer(gettick() + umin(next_tick, 3600000), pc_inventory_rental_end, sd->bl.id, 0);
	else
		sd->rental_timer = INVALID_TIMER;
}
...
void pc_inventory_rental_add(struct map_session_data *sd, unsigned int seconds) {
	...
	if (sd->rental_timer != INVALID_TIMER) {
		...
		if (DIFF_TICK(td->tick, gettick()) >
			tick) {	 // Update Timer as this one ends first than the current one
			pc_inventory_rental_clear(sd);
			sd->rental_timer = add_timer(gettick() + tick, pc_inventory_rental_end, sd->bl.id, 0);
		}
	} else
		sd->rental_timer =
			add_timer(gettick() + i64min(tick, 3600000), pc_inventory_rental_end, sd->bl.id, 0);
}
	...
	switch (shoptype) {
		case NPCTYPE_SHOP:
			if (item->bound && battle_config.allow_bound_sell & ISR_BOUND_SELLABLE &&
				(item->bound != BOUND_GUILD ||
				 ...
				 (item->bound == BOUND_GUILD &&
				  !(battle_config.allow_bound_sell & ISR_BOUND_GUILDLEADER_ONLY))))
				return true;
			break;
		case NPCTYPE_ITEMSHOP:
			if (item->bound && battle_config.allow_bound_sell & ISR_BOUND &&
				...
				if (itd && itd->flag.trade_restriction.sell &&
					battle_config.allow_bound_sell & ISR_SELLABLE)
					return true;
			}
			break;
	}
 ...
 */
bool pc_can_give_items(struct map_session_data *sd) { return pc_has_permission(sd, PC_PERM_TRADE); }
...
bool pc_can_trade_item(map_session_data *sd, int index) {
	if (sd && index >= 0) {
		return (
			...
			itemdb_cantrade(&sd->inventory.u.items_inventory[index], pc_get_group_level(sd),
...
#ifdef NEW_CARTS
	sd->status.option =
		sd->sc.option & (OPTION_INVISIBLE | OPTION_FALCON | OPTION_RIDING | OPTION_DRAGON |
						 OPTION_WUG | OPTION_WUGRIDER | OPTION_MADOGEAR);
#else
	sd->status.option =
		sd->sc.option & (OPTION_INVISIBLE | OPTION_CART | OPTION_FALCON | OPTION_RIDING |
 ...
 * Off init ? Connection?
 *------------------------------------------*/
void pc_setnewpc(struct map_session_data *sd, uint32 account_id, uint32 char_id, int login_id1,
				 t_tick client_tick, int sex, int fd) {
	nullpo_retv(sd);
 ...
 */
enum adopt_responses pc_try_adopt(struct map_session_data *p1_sd, struct map_session_data *p2_sd,
	...
	if (!p1_sd->status.partner_id || !p1_sd->status.party_id ||
		p1_sd->status.party_id != b_sd->status.party_id)
		return ADOPT_MARRIED_AND_PARTY;	 // You need to be married and in party with baby to adopt

	if (p1_sd->status.partner_id != p2_sd->status.char_id ||
		p2_sd->status.partner_id != p1_sd->status.char_id)
 ...
 * Adoption Process
 *------------------------------------------*/
bool pc_adoption(struct map_session_data *p1_sd, struct map_session_data *p2_sd,
				 struct map_session_data *b_sd) {
	int job, joblevel;
	t_exp jobexp;
	...
	if (job != -1 &&
		pc_jobchange(b_sd, job, 0)) {  // Success, proceed to configure parents and baby skills
...
static bool pc_isItemClass(struct map_session_data *sd, struct item_data *item) {
	while (1) {
		if (item->class_upper & ITEMJ_NORMAL &&
			!(sd->class_ &
			  (JOBL_UPPER | JOBL_BABY | JOBL_THIRD |
			   JOBL_FOURTH)))  // normal classes (no upper, no baby, no third, no fourth)
			break;
#ifndef RENEWAL
		// allow third classes to use trans. class items
		if (item->class_upper & ITEMJ_UPPER &&
			sd->class_ & (JOBL_UPPER | JOBL_THIRD))	 // trans. classes
			break;
		...
		if (item->class_upper & (ITEMJ_THIRD | ITEMJ_THIRD_UPPER | ITEMJ_THIRD_BABY) &&
			sd->class_ & JOBL_THIRD)
			break;
#else
		// trans. classes (exl. third-trans.)
		if (item->class_upper & ITEMJ_UPPER && sd->class_ & JOBL_UPPER &&
			!(sd->class_ & JOBL_THIRD))
			break;
		...
		if (item->class_upper & ITEMJ_THIRD && sd->class_ & JOBL_THIRD &&
			!(sd->class_ & (JOBL_UPPER | JOBL_BABY)) && !(sd->class_ & JOBL_FOURTH))
			break;
		// trans-third classes (exl. fourth)
		if (item->class_upper & ITEMJ_THIRD_UPPER && sd->class_ & JOBL_THIRD &&
			sd->class_ & JOBL_UPPER && !(sd->class_ & JOBL_FOURTH))
			break;
		// third-baby classes (exl. fourth)
		if (item->class_upper & ITEMJ_THIRD_BABY && sd->class_ & JOBL_THIRD &&
			sd->class_ & JOBL_BABY && !(sd->class_ & JOBL_FOURTH))
			break;
		// fourth classes
 ...
 * @return ITEM_EQUIP_ACK_OK(0) if can be equipped, or
 *ITEM_EQUIP_ACK_FAIL(1)/ITEM_EQUIP_ACK_FAILLEVEL(2) if can't
 *------------------------------------------------*/
uint8 pc_isequip(struct map_session_data *sd, int n) {
		...
		switch (item->subtype) {
			...
			case AMMO_SHELL:
				if (battle_config.ammo_check_weapon && sd->status.weapon != W_REVOLVER &&
					sd->status.weapon != W_RIFLE && sd->status.weapon != W_GATLING &&
			...
			case AMMO_CANNONBALL:
				if (!pc_ismadogear(sd) &&
					(sd->status.class_ == JOB_MECHANIC_T || sd->status.class_ == JOB_MECHANIC)) {
					clif_msg(
						sd,
						ITEM_NEED_MADOGEAR);  // Item can only be used when Mado Gear is mounted.
					return ITEM_EQUIP_ACK_FAIL;
				}
	...
	if (sd->sc.count) {
		if (item->equip & EQP_ARMS && item->type == IT_WEAPON &&
			sd->sc.data[SC_STRIPWEAPON])  // Also works with left-hand weapons [DracoRPG]
			return ITEM_EQUIP_ACK_FAIL;
 ...
 *------------------------------------------*/
bool pc_authok(struct map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id,
			   struct mmo_charstatus *st, bool changing_mapservers) {
	t_tick tick = gettick();
	...
	sd->status.clothes_color =
		cap_value(sd->status.clothes_color, MIN_CLOTH_COLOR, MAX_CLOTH_COLOR);
	...
	for (int i = 0; i < MAX_SPIRITBALL; i++) sd->spirit_timer[i] = INVALID_TIMER;
	...
	// Event Timers
	for (int i = 0; i < MAX_EVENTTIMER; i++) sd->eventtimer[i] = INVALID_TIMER;
	// Rental Timer
	sd->rental_timer = INVALID_TIMER;

	for (int i = 0; i < 3; i++) sd->hate_mob[i] = -1;
	...
	// warp player
	enum e_setpos setpos_result = pc_setpos(sd, sd->status.last_point.map, sd->status.last_point.x,
											sd->status.last_point.y, CLR_OUTSIGHT);
	...
	if (!changing_mapservers) {
		...
		for (int i = 0; i < MOTD_LINE_SIZE && motd_text[i][0]; i++) {
			if (battle_config.motd_type)
				clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], motd_text[i], false,
								  SELF);
			else
	...
	// Check EXP overflow, since in previous revision EXP on Max Level can be more than 'official'
	// Max EXP
...
void pc_reg_received(struct map_session_data *sd) {
	...
	sd->change_level_2nd =
		static_cast<unsigned char>(pc_readglobalreg(sd, add_str(JOBCHANGE2ND_VAR)));
	sd->change_level_3rd =
		static_cast<unsigned char>(pc_readglobalreg(sd, add_str(JOBCHANGE3RD_VAR)));
	sd->change_level_4th =
		static_cast<unsigned char>(pc_readglobalreg(sd, add_str(JOBCHANGE4TH_VAR)));
	...
	if ((sd->class_ & MAPID_BASEMASK) ==
		MAPID_TAEKWON) {  // Better check for class rather than skill to prevent "skill resets" from
						  // unsetting this
		sd->mission_mobid = static_cast<short>(pc_readglobalreg(sd, add_str(TKMISSIONID_VAR)));
		sd->mission_count =
			static_cast<unsigned char>(pc_readglobalreg(sd, add_str(TKMISSIONCOUNT_VAR)));
	}
	...
	for (i = 0; i < MAX_PC_FEELHATE; i++) {	 // for now - someone need to make reading from txt/sql
		...
		sd->hate_mob[i] =
			static_cast<short>(pc_readglobalreg(sd, add_str(sg_info[i].hate_var))) - 1;
	}

	if ((i = pc_checkskill(sd, RG_PLAGIARISM)) > 0) {
		unsigned short skid =
			static_cast<unsigned short>(pc_readglobalreg(sd, add_str(SKILL_VAR_PLAGIARISM)));
	...
	if ((i = pc_checkskill(sd, SC_REPRODUCE)) > 0) {
		unsigned short skid =
			static_cast<unsigned short>(pc_readglobalreg(sd, add_str(SKILL_VAR_REPRODUCE)));
	...
	sd->state.active = 1;
	sd->state.pc_loaded =
		false;	// Ensure inventory data and status data is loaded before we calculate player stats
	...
	if (sd->status.clan_id > 0)
	  ...
	  PACKETVER_ZERO_NUM >= 20190410)
	// Before those clients you could send out the instance info even when the client was still
	// loading the map, afterwards you need to send it later
	...
	sd->vip.time = 0;
	sd->vip.enabled = 0;
	chrif_req_login_operation(sd->status.account_id, sd->status.name, CHRIF_OP_LOGIN_VIP, 0, 1 | 8,
							  0);  // request VIP information
#endif
	intif_Mail_requestinbox(sd->status.char_id, 0,
							MAIL_INBOX_NORMAL);	 // MAIL SYSTEM - Request Mail Inbox
	...
	if (sd->state.connect_new == 0 &&
		sd->fd) {  // Character already loaded map! Gotta trigger LoadEndAck manually.
		sd->state.connect_new = 1;
		...
		if (sd->status.skill[i].id && sd->status.skill[i].lv > 0) {
			...
			if ((!skill->inf2[INF2_ISQUEST] || battle_config.quest_skill_learn) &&
				(!skill->inf2[INF2_ISWEDDING] ||
				 skill->inf2[INF2_ISSPIRIT])  // Do not count wedding/link skills. [Skotlex]
			) {
...
void pc_calc_skilltree(struct map_session_data *sd) {
	...
	if (class_ == -1) {	 // Unable to normalize job??
		ShowError("pc_calc_skilltree: Unable to normalize job %s(%d) for character %s (%d:%d)\n",
				  job_name(sd->status.class_), sd->status.class_, sd->status.name,
				  sd->status.account_id, sd->status.char_id);
		return;
	}
	...
	do {
		...
		for (const auto &skillsit : tree->skills) {
			...
			if (!battle_config.skillfree) {
					...
					for (const auto &it : entry->need) {
						...
						if (sk_need_idx > 0) {
							...
							if (sd->status.skill[sk_need_idx].id == 0 ||
								...
								sk_need =
									sd->status.skill[sk_need_idx].flag - SKILL_FLAG_REPLACED_LV_0;
							else
				...
				if (sd->status.base_level <
					entry->baselv) {  // We need to get the actual class in this case
				...
				if (sd->status.job_level <
					entry->joblv) {	 // We need to get the actual class in this case
					int c_ = pc_mapid2jobid(sd->class_, sd->status.sex);
			...
			if (!fail) {
				...
				if (!sd->status.skill[sk_idx].lv &&
					...
					continue;  // Cannot be learned via normal means. Note this check DOES allows
							   // raising already known skills.

				sd->status.skill[sk_idx].id = skid;

				if (skill->inf2[INF2_ISSPIRIT]) {  // Spirit skills cannot be learned, they will
												   // only show up on your tree when you get buffed.
	...
	if (class_ > 0 && sd->status.skill_point == 0 && pc_is_taekwon_ranker(sd)) {
		...
		- (sd->status.skill_point == 0) to wait until all skill points are assigned to avoid
		problems with Job Change quest. */
			...
			for (const auto &it : tree->skills) {
				...
				if (sd->status.skill[sk_idx].id == 0) {
					...
					sd->status.skill[sk_idx].flag =
						...
						sd->status.skill[sk_idx].lv;  // Remember original level
					...
					if (sk_need_id > 0) {
						...
						if (sd->status.skill[sk_need_idx].id == 0 ||
							...
							sk_need = 0;  // Not learned.
								 ...
								 SKILL_FLAG_REPLACED_LV_0)	// Real lerned level
...
void pc_clean_skilltree(struct map_session_data *sd) {
	...
	for (i = 0; i < MAX_SKILL; i++) {
		if (sd->status.skill[i].flag == SKILL_FLAG_TEMPORARY ||
...
int pc_disguise(struct map_session_data *sd, int class_) {
	...
	if (sd->bl.prev != NULL) {
		clif_spawn(&sd->bl);
		if (class_ == sd->status.class_ &&
			pc_iscarton(sd)) {	// It seems the cart info is lost on undisguise.
			clif_cartlist(sd);
 ...
 */
static void pc_bonus_autospell(std::vector<s_autospell> &spell, uint16 id, uint16 lv, short rate,
							   short battle_flag, t_itemid card_id, uint8 flag) {
	...
	if (!(battle_flag & BF_SKILLMASK)) {
		...
		if (battle_flag & BF_WEAPON)
			battle_flag |=
				BF_NORMAL;	// By default autospells should only trigger on normal weapon attacks.
	}
 ...
 *              &2: random skill level in [1..lv] is chosen
 */
static void pc_bonus_autospell_onskill(std::vector<s_autospell> &spell, uint16 src_skill, uint16 id,
									   uint16 lv, short rate, t_itemid card_id, uint8 flag) {
	if (spell.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_autospell_onskill: Reached max (%d) number of autospells per character!\n",
			MAX_PC_BONUS);
		return;
	}
 ...
 */
static void pc_bonus_addeff(std::vector<s_addeffect> &effect, enum sc_type sc, int rate,
							short arrow_rate, unsigned char flag, unsigned int duration) {
 ...
 * @param duration: Duration. If 0 use default duration lookup for associated skill with level 7
 */
static void pc_bonus_addeff_onskill(std::vector<s_addeffectonskill> &effect, enum sc_type sc,
									int rate, short skill_id, unsigned char target,
									unsigned int duration) {
	if (effect.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_addeff_onskill: Reached max (%d) number of add effects per character!\n",
			MAX_PC_BONUS);
		return;
	}
 ...
 */
static void pc_bonus_item_drop(std::vector<s_add_drop> &drop, t_itemid nameid, uint16 group,
							   int class_, short race, int rate) {
	if (!nameid && !group) {
	...
	if (drop.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_item_drop: Reached max (%d) number of added drops per character! (nameid: %u "
			"group: %d class_: %d race: %d rate: %d)\n",
	...
	for (auto &it : drop) {
		if (it.nameid == nameid && it.group == group && it.race == race && it.class_ == class_) {
			if ((rate < 0 && it.rate < 0) ||
				(rate > 0 && it.rate > 0))	// Adjust the rate if it has same classification
 ...
 */
bool pc_addautobonus(std::vector<std::shared_ptr<s_autobonus>> &bonus, const char *script,
					 short rate, unsigned int dur, uint16 flag, const char *other_script,
					 unsigned int pos, bool onskill) {
 ...
 */
void pc_delautobonus(struct map_session_data &sd, std::vector<std::shared_ptr<s_autobonus>> &bonus,
					 bool restore) {
	...
	while (it != bonus.end()) {
		...
		if (b->active != INVALID_TIMER && restore && b->bonus_script != nullptr) {
			...
			// Create a list of all equipped positions to see if all items needed for the autobonus
			// are still present [Playtester]
			for (uint8 j = 0; j < EQI_MAX; j++) {
 ...
 */
void pc_exeautobonus(struct map_session_data &sd, std::vector<std::shared_ptr<s_autobonus>> *bonus,
	...
	if (autobonus->other_script) {
		int j;
		unsigned int equip_pos_idx = 0;
		// Create a list of all equipped positions to see if all items needed for the autobonus are
		// still present [Playtester]
		for (j = 0; j < EQI_MAX; j++) {
			if (sd.equip_index[j] >= 0)
				equip_pos_idx |= sd.inventory.u.items_inventory[sd.equip_index[j]].equip;
		}
	...
	autobonus->active =
		add_timer(gettick() + autobonus->duration, pc_endautobonus, sd.bl.id, (intptr_t)bonus);
...
TIMER_FUNC(pc_endautobonus) {
	...
	std::vector<std::shared_ptr<s_autobonus>> *bonus =
		(std::vector<std::shared_ptr<s_autobonus>> *)data;
 ...
 */
static void pc_bonus_addele(struct map_session_data *sd, unsigned char ele, short rate,
							short flag) {
	nullpo_retv(sd);
	...
	if (wd->addele2.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_addele: Reached max (%d) number of add element damage bonuses per "
			"character!\n",
			MAX_PC_BONUS);
		return;
	}
 ...
 * @param flag: Battle flag
 */
static void pc_bonus_subele(struct map_session_data *sd, unsigned char ele, short rate,
							short flag) {
	nullpo_retv(sd);

	if (sd->subele2.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_subele: Reached max (%d) number of resist element damage bonuses per "
			"character!\n",
			MAX_PC_BONUS);
		return;
	}
 ...
 * @param flag: Battle flag
 */
static void pc_bonus_subrace(struct map_session_data *sd, unsigned char race, short rate,
							 short flag) {
	if (sd->subrace3.size() == MAX_PC_BONUS) {
		ShowWarning(
			"pc_bonus_subrace: Reached max (%d) number of resist race damage bonuses per "
			"character!\n",
			MAX_PC_BONUS);
		return;
	}
 ...
 * @param flag: Battle flag
 */
static void pc_bonus_addvanish(std::vector<s_vanish_bonus> &bonus, int16 rate, int16 per,
							   int flag) {
	if (bonus.size() == MAX_PC_BONUS) {
		ShowWarning(
...
void pc_bonus(struct map_session_data *sd, int type, int val) {
	...
	switch (type) {
		...
		case SP_NO_GEMSTONE:
			...
			break;
		case SP_INTRAVISION:  // Maya Purple Card effect allowing to see Hiding/Cloaking people
							  // [DracoRPG]
			if (sd->state.lr_flag != 2) {
		...
		case SP_SHORT_ATK_RATE:
			if (sd->state.lr_flag !=
				2)	//[Lupus] it should stack, too. As any other cards rate bonuses
		...
		default:
			if (current_equip_combo_pos > 0) {
				...
				ShowWarning("pc_bonus: unknown bonus type %d %d in item #%u\n", type, val,
							current_equip_card_id
								? current_equip_card_id
								: sd->inventory_data[current_equip_item_index]->nameid);
			} else {
	...
	switch (type) {
		...
		case SP_RESEFF:	 // bonus2 bResEff,eff,n;
			...
			if (sd->reseff.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_RESEFF: Reached max (%d) number of resistance bonuses per "
					"character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_DAMAGE_CLASS: {	 // bonus2 bAddDamageClass,mid,x;
			...
			if (wd->add_dmg.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_DAMAGE_CLASS: Reached max (%d) number of add class damage "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_MAGIC_DAMAGE_CLASS:	 // bonus2 bAddMagicDamageClass,mid,x;
			...
			if (sd->add_mdmg.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_MAGIC_DAMAGE_CLASS: Reached max (%d) number of add class "
					"magic dmg bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_DEF_MONSTER:  // bonus2 bAddDefMonster,mid,x;
			...
			if (sd->add_def.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_DEF_MONSTER: Reached max (%d) number of add class def "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_MDEF_MONSTER:  // bonus2 bAddMDefMonster,mid,x;
			...
			if (sd->add_mdef.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_MDEF_MONSTER: Reached max (%d) number of add class mdef "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_SKILL_ATK:	// bonus2 bSkillAtk,sk,n;
			...
			if (sd->skillatk.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_ATK: Reached max (%d) number of skills per character, "
					"bonus skill %d (+%d%%) lost.\n",
		...
		case SP_SKILL_HEAL:	 // bonus2 bSkillHeal,sk,n;
			if (sd->state.lr_flag == 2)
				break;
			if (sd->skillheal.size() == MAX_PC_BONUS) {	 // Better mention this so the array length
														 // can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: SP_SKILL_HEAL: Reached max (%d) number of skills per character, "
					"bonus skill %d (+%d%%) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SKILL_HEAL2:  // bonus2 bSkillHeal2,sk,n;
			if (sd->state.lr_flag == 2)
				break;
			if (sd->skillheal2.size() == MAX_PC_BONUS) {  // Better mention this so the array length
														  // can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: SP_SKILL_HEAL2: Reached max (%d) number of skills per character, "
					"bonus skill %d (+%d%%) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_ADD_SKILL_BLOW:	 // bonus2 bAddSkillBlow,sk,n;
			if (sd->state.lr_flag == 2)
				break;
			if (sd->skillblown.size() == MAX_PC_BONUS) {  // Better mention this so the array length
														  // can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: SP_ADD_SKILL_BLOW: Reached max (%d) number of skills per "
					"character, bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_ADD_ITEM_HEAL_RATE:	 // bonus2 bAddItemHealRate,iid,n;
			...
			if (sd->itemhealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEM_HEAL_RATE: Reached max (%d) number of item heal "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_ITEMGROUP_HEAL_RATE:  // bonus2 bAddItemGroupHealRate,ig,n;
			...
			if (!type2 || !itemdb_group.exists(type2)) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEMGROUP_HEAL_RATE: Invalid item group with id %d\n",
					type2);
				break;
			}
			if (sd->itemgrouphealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEMGROUP_HEAL_RATE: Reached max (%d) number of item heal "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_SKILL_USE_SP_RATE:	// bonus2 bSkillUseSPrate,sk,n;
			...
			if (sd->skillusesprate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_USE_SP_RATE: Reached max (%d) number of skills per "
					"character, bonus skill %d (+%d%%) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SKILL_DELAY:
			...
			if (sd->skilldelay.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_DELAY: Reached max (%d) number of skills per character, "
					"bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SKILL_COOLDOWN:	 // bonus2 bSkillCooldown,sk,t;
			...
			if (sd->skillcooldown.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_COOLDOWN: Reached max (%d) number of skills per "
					"character, bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SKILL_VARIABLECAST:	 // bonus2 bSkillVariableCast,sk,t;
			...
			if (sd->skillvarcast.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_VARIABLECAST: Reached max (%d) number of skills per "
					"character, bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SKILL_FIXEDCAST:  // bonus2 bSkillFixedCast,sk,t;
			...
			if (sd->skillfixcast.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_FIXEDCAST: Reached max (%d) number of skills per "
					"character, bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_VARCASTRATE:  // bonus2 bVariableCastrate,sk,n;
			...
			if (sd->skillcastrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_VARCASTRATE: Reached max (%d) number of skills per character, "
					"bonus skill %d (%d%%) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_FIXCASTRATE:  // bonus2 bFixedCastrate,sk,n;
			...
			if (sd->skillfixcastrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_FIXCASTRATE: Reached max (%d) number of skills per character, "
					"bonus skill %d (%d%%) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}

			pc_bonus_itembonus(sd->skillfixcastrate, type2, -val,
							   true);  // Send inversed value here
			break;
#else
		case SP_SKILL_FIXEDCAST:  // bonus2 bSkillFixedCast,sk,t;
		case SP_FIXCASTRATE:	  // bonus2 bFixedCastrate,sk,n;
		...
		case SP_CASTRATE:	  // bonus2 bCastrate,sk,n;
			if (sd->state.lr_flag == 2)
				break;
			if (sd->skillcastrate.size() == MAX_PC_BONUS) {	 // Better mention this so the array
															 // length can be updated. [Skotlex]
				ShowWarning(
					"pc_bonus2: %s: Reached max (%d) number of skills per character, bonus skill "
					"%d (%d%%) lost.\n",
					(type == SP_CASTRATE) ? "SP_CASTRATE" : "SP_VARCASTRATE", MAX_PC_BONUS, type2,
					val);
				break;
			}
		...
		case SP_SKILL_USE_SP:  // bonus2 bSkillUseSP,sk,n;
			...
			if (sd->skillusesp.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SKILL_USE_SP: Reached max (%d) number of skills per character, "
					"bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_SUB_SKILL:	// bonus2 bSubSkill,sk,n;
			if (sd->subskill.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_SUB_SKILL: Reached max (%d) number of skills per character, "
					"bonus skill %d (%d) lost.\n",
					MAX_PC_BONUS, type2, val);
				break;
			}
		...
		case SP_ADD_ITEM_SPHEAL_RATE:  // bonus2 bAddItemSPHealRate,iid,n;
			...
			if (sd->itemsphealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEM_SPHEAL_RATE: Reached max (%d) number of item SP heal "
					"bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		case SP_ADD_ITEMGROUP_SPHEAL_RATE:	// bonus2 bAddItemGroupSPHealRate,ig,n;
			...
			if (!type2 || !itemdb_group.exists(type2)) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEMGROUP_SPHEAL_RATE: Invalid item group with id %d\n",
					type2);
				break;
			}
			...
			if (sd->itemgroupsphealrate.size() == MAX_PC_BONUS) {
				ShowWarning(
					"pc_bonus2: SP_ADD_ITEMGROUP_SPHEAL_RATE: Reached max (%d) number of item SP "
					"heal bonuses per character!\n",
					MAX_PC_BONUS);
				break;
			}
		...
		default:
			if (current_equip_combo_pos > 0) {
				...
				ShowWarning(
					...
					current_equip_card_id ? current_equip_card_id
										  : sd->inventory_data[current_equip_item_index]->nameid);
			} else {
				ShowWarning(
					"pc_bonus2: unknown bonus type %d %d %d in unknown usage. Report this!\n", type,
					type2, val);
			}
			break;
	}
...
void pc_bonus3(struct map_session_data *sd, int type, int type2, int type3, int val) {
	...
	switch (type) {
		...
		case SP_AUTOSPELL:	// bonus3 bAutoSpell,sk,y,n;
			if (sd->state.lr_flag != 2) {
				int target = skill_get_inf(
					type2);	 // Support or Self (non-auto-target) skills should pick self.
				target = target & INF_SUPPORT_SKILL ||
						 (target & INF_SELF_SKILL && !skill_get_inf2(type2, INF2_NOTARGETSELF));
				pc_bonus_autospell(sd->autospell, type2, type3, val, 0, current_equip_card_id,
		...
		case SP_AUTOSPELL_WHENHIT:	// bonus3 bAutoSpellWhenHit,sk,y,n;
			if (sd->state.lr_flag != 2) {
				...
				target = target & INF_SUPPORT_SKILL ||
						 (target & INF_SELF_SKILL && !skill_get_inf2(type2, INF2_NOTARGETSELF));
				pc_bonus_autospell(sd->autospell2, type2, type3, val, BF_NORMAL | BF_SKILL,
								   current_equip_card_id,
								   target ? AUTOSPELL_FORCE_SELF : AUTOSPELL_FORCE_TARGET);
			}
			break;
		...
		case SP_ADDEFF_ONSKILL:	 // bonus3 bAddEffOnSkill,sk,eff,n;
			...
			if (sd->state.lr_flag != 2)
				pc_bonus_addeff_onskill(sd->addeff_onskill, (sc_type)type3, val, type2, ATF_TARGET,
										0);
			break;
...
void pc_bonus4(struct map_session_data *sd, int type, int type2, int type3, int type4, int val) {
	...
	switch (type) {
		case SP_AUTOSPELL:	// bonus4 bAutoSpell,sk,y,n,i;
			if (sd->state.lr_flag != 2)
				pc_bonus_autospell(sd->autospell, type2, type3, type4, 0, current_equip_card_id,
								   val & AUTOSPELL_FORCE_ALL);
			break;

		case SP_AUTOSPELL_WHENHIT:	// bonus4 bAutoSpellWhenHit,sk,y,n,i;
			if (sd->state.lr_flag != 2)
				pc_bonus_autospell(sd->autospell2, type2, type3, type4, BF_NORMAL | BF_SKILL,
								   current_equip_card_id, val & AUTOSPELL_FORCE_ALL);
			break;

		case SP_AUTOSPELL_ONSKILL:	// bonus4 bAutoSpellOnSkill,sk,x,y,n;
			if (sd->state.lr_flag != 2) {
				int target = skill_get_inf(
					type3);	 // Support or Self (non-auto-target) skills should pick self.
				target = target & INF_SUPPORT_SKILL ||
						 (target & INF_SELF_SKILL && !skill_get_inf2(type3, INF2_NOTARGETSELF));

				pc_bonus_autospell_onskill(sd->autospell3, type2, type3, type4, val,
										   current_equip_card_id,
 ...
 * @param val Value that usually for rate or fixed value
 */
void pc_bonus5(struct map_session_data *sd, int type, int type2, int type3, int type4, int type5,
			   int val) {
	nullpo_retv(sd);
	...
	switch (type) {
		case SP_AUTOSPELL:	// bonus5 bAutoSpell,sk,y,n,bf,i;
			if (sd->state.lr_flag != 2)
				pc_bonus_autospell(sd->autospell, type2, type3, type4, type5, current_equip_card_id,
								   val & AUTOSPELL_FORCE_ALL);
			break;

		case SP_AUTOSPELL_WHENHIT:	// bonus5 bAutoSpellWhenHit,sk,y,n,bf,i;
			if (sd->state.lr_flag != 2)
				pc_bonus_autospell(sd->autospell2, type2, type3, type4, type5,
								   current_equip_card_id, val & AUTOSPELL_FORCE_ALL);
			break;

		case SP_AUTOSPELL_ONSKILL:	// bonus5 bAutoSpellOnSkill,sk,x,y,n,i;
			if (sd->state.lr_flag != 2)
				pc_bonus_autospell_onskill(sd->autospell3, type2, type3, type4, type5,
										   current_equip_card_id, val & AUTOSPELL_FORCE_ALL);
			break;
		...
		case SP_ADDEFF_ONSKILL:	 // bonus5 bAddEffOnSkill,sk,eff,n,y,t;
			PC_BONUS_CHK_SC(type3, SP_ADDEFF_ONSKILL);
			if (sd->state.lr_flag != 2)
				pc_bonus_addeff_onskill(sd->addeff_onskill, (sc_type)type3, type4, type2, type5,
										val);
			break;
		...
		default:
			if (current_equip_combo_pos > 0) {
				...
				ShowWarning("pc_bonus5: unknown bonus type %d %d %d %d %d %d in item #%u\n", type,
							type2, type3, type4, type5, val,
							current_equip_card_id
								? current_equip_card_id
								: sd->inventory_data[current_equip_item_index]->nameid);
			} else {
				ShowWarning(
					"pc_bonus5: unknown bonus type %d %d %d %d %d %d in unknown usage. Report "
					"this!\n",
					type, type2, type3, type4, type5, val);
			}
			break;
...
bool pc_skill(struct map_session_data *sd, uint16 skill_id, int level, enum e_addskill_type type) {
	...
	if (type == ADDSKILL_TEMP_ADDLEVEL && sd->status.skill[idx].lv + level > MAX_SKILL_LEVEL) {
		ShowWarning(
			"pc_skill: Skill level bonus %d too high. Max lv supported is %d. Curr lv is %d. Set "
			"to max level.\n",
	...
	switch (type) {
		case ADDSKILL_PERMANENT:  // Set skill data overwriting whatever was there before.
			...
			if (!skill_get_inf(skill_id) || pc_checkskill_summoner(sd, SUMMONER_POWER_LAND) >= 20 ||
				pc_checkskill_summoner(sd, SUMMONER_POWER_SEA) >=
					20)	 // Only recalculate for passive skills.
				status_calc_pc(sd, SCO_NONE);
			break;

		case ADDSKILL_TEMP:	 // Item bonus skill.
			if (sd->status.skill[idx].id != 0) {
				if (sd->status.skill[idx].flag ==
					SKILL_FLAG_PERMANENT)  // Non-granted skill, store it's level.
					sd->status.skill[idx].flag =
						SKILL_FLAG_REPLACED_LV_0 + sd->status.skill[idx].lv;
			} else {
				sd->status.skill[idx].id = skill_id;
		...
		case ADDSKILL_TEMP_ADDLEVEL:  // Add skill bonus on top of what you had.
			if (sd->status.skill[idx].id != 0) {
				if (sd->status.skill[idx].flag == SKILL_FLAG_PERMANENT)
					sd->status.skill[idx].flag = SKILL_FLAG_REPLACED_LV_0 +
												 sd->status.skill[idx].lv;	// Store previous level.
			} else {
				sd->status.skill[idx].id = skill_id;
				sd->status.skill[idx].flag =
					SKILL_FLAG_TEMPORARY;  // Set that this is a bonus skill.
			}
			sd->status.skill[idx].lv += level;
			break;
...
int pc_insert_card(struct map_session_data *sd, int idx_card, int idx_equip) {
	...
	if (sd->inventory.u.items_inventory[idx_equip].nameid == 0 ||
		sd->inventory.u.items_inventory[idx_equip].amount < 1)
		return 0;  // target item missing
	if (sd->inventory.u.items_inventory[idx_card].nameid == 0 ||
		sd->inventory.u.items_inventory[idx_card].amount < 1)
	...
	if (item_eq->type == IT_ARMOR && (item_card->equip & EQP_ACC) &&
		((item_card->equip & EQP_ACC) != EQP_ACC) &&
...
char pc_checkadditem(struct map_session_data *sd, t_itemid nameid, int amount) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		// FIXME: This does not consider the checked item's cards, thus could check a wrong slot for
		// stackability.
		if (sd->inventory.u.items_inventory[i].nameid == nameid) {
			if (amount > MAX_AMOUNT - sd->inventory.u.items_inventory[i].amount ||
				(data->stack.inventory &&
				 amount > data->stack.amount - sd->inventory.u.items_inventory[i].amount))
				return CHKADDITEM_OVERAMOUNT;
			// If the item is in the inventory already, but the player is not allowed to use that
			// many slots anymore
 ...
 */
char pc_payzeny(struct map_session_data *sd, int zeny, enum e_log_pick_type type,
 ...
 */
char pc_getzeny(struct map_session_data *sd, int zeny, enum e_log_pick_type type,
				struct map_session_data *tsd) {
	nullpo_retr(-1, sd);
...
int pc_paycash(struct map_session_data *sd, int price, int points, e_log_pick_type type) {
	...
	if (sd->cashPoints < cash || sd->kafraPoints < points) {
		ShowError(
			"pc_paycash: Not enough points (cash=%d, kafra=%d) to cover the price (cash=%d, "
			"kafra=%d) (account_id=%d, char_id=%d).\n",
			sd->cashPoints, sd->kafraPoints, cash, points, sd->status.account_id,
			sd->status.char_id);
		return -1;
	}
	...
	if (battle_config.cashshop_show_points) {
		...
		sprintf(output, msg_txt(sd, 504), points, cash, sd->kafraPoints,
				sd->cashPoints);  // Used %d kafra points and %d cash points. %d kafra and %d cash
								  // points remaining.
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
	}
	return cash + points;
...
int pc_getcash(struct map_session_data *sd, int cash, int points, e_log_pick_type type) {
	...
	if (cash > 0) {
		if (cash > MAX_ZENY - sd->cashPoints) {
			ShowWarning(
				"pc_getcash: Cash point overflow (cash=%d, have cash=%d, account_id=%d, "
				"char_id=%d).\n",
	...
	if (points > 0) {
		if (points > MAX_ZENY - sd->kafraPoints) {
			ShowWarning(
				"pc_getcash: Kafra point overflow (points=%d, have points=%d, account_id=%d, "
				"char_id=%d).\n",
 ...
 *   7 = stack limitation
 */
enum e_additem_result pc_additem(struct map_session_data *sd, struct item *item, int amount,
	...
	if (itemdb_isstackable2(id) && item->expire_time == 0) {
		for (i = 0; i < MAX_INVENTORY; i++) {
			if (sd->inventory.u.items_inventory[i].nameid == item->nameid &&
				...
				if (amount > MAX_AMOUNT - sd->inventory.u.items_inventory[i].amount ||
					(id->stack.inventory &&
					 amount > id->stack.amount - sd->inventory.u.items_inventory[i].amount))
					return ADDITEM_OVERAMOUNT;
				// If the item is in the inventory already, but the player is not allowed to use
				// that many slots anymore
				if (i >= sd->status.inventory_slots) {
					return ADDITEM_OVERAMOUNT;
				}
	...
	if (i >= MAX_INVENTORY) {
		...
		memcpy(&sd->inventory.u.items_inventory[i], item,
 ...
 *------------------------------------------*/
char pc_delitem(struct map_session_data *sd, int n, int amount, int type, short reason,
				e_log_pick_type log_type) {
	nullpo_retr(1, sd);

	if (n < 0 || sd->inventory.u.items_inventory[n].nameid == 0 || amount <= 0 ||
		sd->inventory.u.items_inventory[n].amount < amount || sd->inventory_data[n] == NULL)
		return 1;
...
bool pc_dropitem(struct map_session_data *sd, int n, int amount) {
	...
	if (sd->inventory.u.items_inventory[n].nameid == 0 ||
		...
		sd->inventory.u.items_inventory[n].amount < amount || sd->state.trading ||
		sd->state.vending || !sd->inventory_data[n]	 // pc_delitem would fail on this case.
	)
		return false;
	...
	if (!map_addflooritem(&sd->inventory.u.items_inventory[n], amount, sd->bl.m, sd->bl.x, sd->bl.y,
						  0, 0, 0, 2, 0))
		return false;
...
bool pc_takeitem(struct map_session_data *sd, struct flooritem_data *fitem) {
	...
	if (fitem->first_get_charid > 0 && fitem->first_get_charid != sd->status.char_id) {
		...
		if (DIFF_TICK(tick, fitem->first_get_tick) < 0) {
			if (!(p && p->party.item & 1 && first_sd &&
				  first_sd->status.party_id == sd->status.party_id))
				return false;
			...
			if (DIFF_TICK(tick, fitem->second_get_tick) < 0) {
				if (!(p && p->party.item & 1 &&
					...
					return false;
			} else if (fitem->third_get_charid > 0 &&
					   fitem->third_get_charid != sd->status.char_id) {
	...
	// This function takes care of giving the item to whoever should have it, considering
	// party-share options.
	...
	if (fitem->mob_id && (itemdb_search(fitem->item.nameid))->flag.broadcast &&
		(!p || !(p->party.item &
				 2))  // Somehow, if party's pickup distribution is 'Even Share', no announcemet
	)
		intif_broadcast_obtain_special_item(sd, fitem->item.nameid, fitem->mob_id,
...
bool pc_isUseitem(struct map_session_data *sd, int n) {
	...
	if (DIFF_TICK(sd->canuseitem_tick, gettick()) > 0 ||
		(itemdb_group.item_exists(IG_CASH_FOOD, nameid) &&
		 DIFF_TICK(sd->canusecashfood_tick, gettick()) > 0))
		return false;

	if ((item->item_usage.sitting) && (pc_issit(sd) == 1) &&
		(pc_get_group_level(sd) < item->item_usage.override)) {
	...
	switch (nameid) {
			...
			if (nameid == ITEMID_GIANT_FLY_WING) {
				...
				if (pd) {
					...
					ARR_FIND(0, MAX_PARTY, i,
							 pd->data[i].sd && pd->data[i].sd != sd &&
								 pd->data[i].sd->bl.m == sd->bl.m && !pc_isdead(pd->data[i].sd));
		...
		case ITEMID_SIEGE_TELEPORT_SCROLL:
			...
			if (mapdata->flag[MF_NORETURN] && nameid != ITEMID_WING_OF_FLY &&
				nameid != ITEMID_GIANT_FLY_WING && nameid != ITEMID_N_FLY_WING)
				return false;
			break;
	...
	if (item->flag.group ||
		...
		if (!pc_inventoryblank(sd)) {
			clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 732), false,
							  SELF);  // Item cannot be open when inventory is full
			return false;
		}
	}
...
int pc_useitem(struct map_session_data *sd, int n) {
	...
	if (nameid != ITEMID_NAUTHIZ && sd->sc.opt1 > 0 && sd->sc.opt1 != OPT1_STONEWAIT &&
		sd->sc.opt1 != OPT1_BURNING)
		return 0;
	...
	// FIXME: Is this really needed here? It'll be checked in unit.cpp after all and this prevents
	// skill items using when silenced [Inkfish]
	if (id->flag.delay_consume > 0 &&
		(sd->ud.skilltimer !=
		 INVALID_TIMER /*|| !status_check_skilluse(&sd->bl, &sd->bl, ALL_RESURRECTION, 0)*/))
		return 0;
	...
	if (pet_db_search(id->nameid, PET_CATCH) != nullptr &&
		map_getmapflag(sd->bl.m, MF_NOPETCAPTURE)) {
	...
	if (id->flag.delay_consume > 0)
		...
		if (item.expire_time == 0 && nameid != ITEMID_REINS_OF_MOUNT) {
			...
			pc_delitem(sd, n, 1, 1, 0,
					   LOG_TYPE_CONSUME);  // Rental Usable Items are not deleted until expiration
		} else
 ...
 */
enum e_additem_result pc_cart_additem(struct map_session_data *sd, struct item *item, int amount,
									  e_log_pick_type log_type) {
	struct item_data *data;
	int i, w;
	...
	if (!itemdb_cancartstore(item, pc_get_group_level(sd)) ||
		(item->bound > BOUND_ACCOUNT &&
		 !pc_can_give_bounded_items(sd))) {	 // Check item trade restrictions	[Skotlex]
...
void pc_putitemtocart(struct map_session_data *sd, int idx, int amount) {
	...
	if (flag == ADDITEM_SUCCESS)
		...
		clif_cart_additem_ack(sd, (flag == ADDITEM_OVERAMOUNT) ? ADDITEM_TO_CART_FAIL_COUNT
															   : ADDITEM_TO_CART_FAIL_WEIGHT);
...
bool pc_steal_item(struct map_session_data *sd, struct block_list *bl, uint16 skill_lv) {
	...
	if (md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
		...
		sprintf(message, msg_txt(sd, 542), (sd->status.name[0]) ? sd->status.name : "GM",
				md->db->jname.c_str(), i_data->ename.c_str(),
				(float)md->db->dropitem[i].rate / 100);
...
int pc_steal_coin(struct map_session_data *sd, struct block_list *target) {
	...
	if (md->state.steal_coin_flag || md->sc.data[SC_STONE] || md->sc.data[SC_FREEZE] ||
		md->sc.data[SC_HANDICAPSTATE_FROSTBITE] || md->sc.data[SC_HANDICAPSTATE_SWOONING] ||
		md->sc.data[SC_HANDICAPSTATE_LIGHTNINGSTRIKE] ||
		md->sc.data[SC_HANDICAPSTATE_CRYSTALLIZATION] ||
		status_bl_has_mode(target, MD_STATUSIMMUNE) ||
		util::vector_exists(status_get_race2(&md->bl), RC2_TREASURE))
		return 0;
 ...
 *			SETPOS_NO_MAPSERVER	Map not in this map-server, and failed to locate alternate
 *map-server. SETPOS_AUTOTRADE	Player is in autotrade state
 *------------------------------------------*/
enum e_setpos pc_setpos(struct map_session_data *sd, unsigned short mapindex, int x, int y,
						clr_type clrtype) {
	...
	if (sd->state.changemap) {	// Misc map-changing settings
		...
		if (curr_map_instance_id != new_map_instance_id) {
			if (curr_map_instance_id > 0) {	 // Update instance timer for the map on leave
				...
				sd->instance_mode =
					util::umap_find(instances, curr_map_instance_id)
		...
		if (sd->bg_id && mapdata &&
			!mapdata->flag[MF_BATTLEGROUND])  // Moving to a map that isn't a Battlegrounds
			...
			for (const auto &it : status_db) {
				if (sc->data[it.first]) {
					...
					if (it.second->flag[SCF_RESTARTONMAPWARP] && it.second->skill_id > 0) {
						...
						sce->timer =
							add_timer(gettick() + skill_get_time(it.second->skill_id, sce->val1),
									  status_change_timer, sd->bl.id, it.first);
					}
				}
			}
		}
	...
	if (x == 0 && y == 0) {	 // pick a random walkable cell
		...
		do {
			...
			if (c > (mapdata->xs * mapdata->ys) * 3) {	// force out
				ShowError(
					"pc_setpos: couldn't found a valid coordinates for player '%s' (%d:%d) on "
					"(%s), preventing warp\n",
	...
	if (sd->status.guild_id > 0 &&
		mapdata->flag[MF_GVG_CASTLE]) {	 // Increased guild castle regen [Valaris]
...
bool pc_memo(struct map_session_data *sd, int pos) {
	...
	// check mapflags
	if (sd->bl.m >= 0 &&
		(map_getmapflag(sd->bl.m, MF_NOMEMO) || map_getmapflag(sd->bl.m, MF_NOWARPTO)) &&
	...
	if (pos == -1) {
		...
		memmove(&sd->status.memo_point[1], &sd->status.memo_point[0],
				(u8min(i, MAX_MEMOPOINTS - 1)) * sizeof(struct point));
		pos = 0;
	}
...
#else
	if ((idx = skill_db.get_index(skill_id, skill_id >= RK_ENCHANTBLADE, __FUNCTION__, __FILE__,
								  __LINE__)) == 0) {
	...
	switch (type) {
		case SUMMONER_POWER_SEA:
			count = pc_checkskill(sd, SU_TUNABELLY) + pc_checkskill(sd, SU_TUNAPARTY) +
					...
					pc_checkskill(sd, SU_GROOMING) + pc_checkskill(sd, SU_PURRING) +
					pc_checkskill(sd, SU_SHRIMPARTY);
			break;
		case SUMMONER_POWER_LAND:
			count = pc_checkskill(sd, SU_SV_STEMSPEAR) + pc_checkskill(sd, SU_CN_POWDERING) +
					...
					pc_checkskill(sd, SU_CHATTERING) + pc_checkskill(sd, SU_MEOWMEOW) +
					pc_checkskill(sd, SU_NYANGGRASS);
			break;
		case SUMMONER_POWER_LIFE:
			count = pc_checkskill(sd, SU_SCAROFTAROU) + pc_checkskill(sd, SU_PICKYPECK) +
					...
					pc_checkskill(sd, SU_HISS) + pc_checkskill(sd, SU_POWEROFFLOCK) +
					pc_checkskill(sd, SU_SVG_SPIRIT);
			break;
	}
...
uint8 pc_checkskill_imperial_guard(struct map_session_data *sd, short flag) {
	...
	if (flag & 2 && (sd->status.weapon == W_1HSWORD || sd->status.weapon == W_1HSPEAR ||
	...
	for (const auto &it : status_db) {
		...
		if (flag[SCF_REQUIREWEAPON]) {	// Skills requiring specific weapon types
			...
			if (sd->sc.data[status] &&
				!pc_check_weapontype(sd, skill_get_weapontype(it.second->skill_id)))
...
TIMER_FUNC(pc_follow_timer) {
	...
	if (sd->bl.prev != NULL && tbl->prev != NULL && sd->ud.skilltimer == INVALID_TIMER &&
	...
	sd->followtimer = add_timer(tick + 1000,  // increase time a bit to loosen up map's load
...
int pc_checkbaselevelup(struct map_session_data *sd) {
	...
	do {
		...
		// Kyoki pointed out that the max overcarry exp is the exp needed for the previous level -1.
		// [Skotlex]
		if ((!battle_config.multi_level_up ||
			 (battle_config.multi_level_up_base > 0 &&
			  sd->status.base_level >= battle_config.multi_level_up_base)) &&
	...
	if ((sd->class_ & MAPID_UPPERMASK) == MAPID_SUPER_NOVICE) {
		for (const auto &status : status_db) {
			if (status.second->flag[SCF_SUPERNOVICEANGEL])
				sc_start(&sd->bl, &sd->bl, status.second->type, 100, 1,
						 skill_get_time(status.second->skill_id, 1));
		}
	...
	for (i = 0; i < EQI_MAX; i++) {
		if (sd->equip_index[i] >= 0 && sd->inventory_data[sd->equip_index[i]]) {
			if (sd->inventory_data[sd->equip_index[i]]->elvmax &&
				sd->status.base_level >
					(unsigned int)sd->inventory_data[sd->equip_index[i]]->elvmax)
...
int pc_checkjoblevelup(struct map_session_data *sd) {
	...
	do {
		...
		// [Skotlex]
		if ((!battle_config.multi_level_up ||
			 (battle_config.multi_level_up_job > 0 &&
			  sd->status.job_level >= battle_config.multi_level_up_job)) &&
	...
	if (pc_checkskill(sd, SG_DEVIL) &&
		...
		clif_status_change(&sd->bl, EFST_DEVIL1, 1, 0, 0, 0,
						   1);	// Permanent blind effect from SG_DEVIL.
 ...
 */
static void pc_calcexp(struct map_session_data *sd, t_exp *base_exp, t_exp *job_exp,
	...
	if (sd->sc.data[SC_EXPBOOST]) {
		...
		if (battle_config.vip_bm_increase &&
			pc_isvip(sd))  // Increase Battle Manual EXP rate for VIP
 ...
 **/
void pc_gainexp_disp(struct map_session_data *sd, t_exp base_exp, t_exp next_base_exp,
					 t_exp job_exp, t_exp next_job_exp, bool lost) {
	char output[CHAT_SIZE_MAX];
	...
	sprintf(output, msg_txt(sd, 743),  // Experience %s Base:%ld (%0.2f%%) Job:%ld (%0.2f%%)
			...
			(base_exp / (float)next_base_exp * 100 * (lost ? -1 : 1)),
			(long)job_exp * (lost ? -1 : 1),
 ...
 * @return
 **/
void pc_gainexp(struct map_session_data *sd, struct block_list *src, t_exp base_exp, t_exp job_exp,
				uint8 exp_flag) {
	...
	flag = ((base_exp) ? 1 : 0) | ((job_exp) ? 2 : 0) | ((pc_is_maxbaselv(sd)) ? 4 : 0) |
...
#define PC_STATUS_POINT_COST(low) \
	(((low) < 100) ? (2 + ((low)-1) / 10) : (16 + 4 * (((low)-100) / 5)))
#else
...
int pc_need_status_point(struct map_session_data *sd, int type, int val) {
	...
	for (; low < high; low++) sp += PC_STATUS_POINT_COST(low);
...
bool pc_traitstatusup(struct map_session_data *sd, int type, int increase) {
	...
	increase = cap_value(increase, 0, max_increase);  // cap to the maximum status points available
	if (increase <= 0 ||
		current + increase > pc_maxparameter(sd, (enum e_params)(PARAM_POW + type - SP_POW))) {
		clif_statusupack(sd, type, 0, 0);
		return false;
	}
...
void pc_skillup(struct map_session_data *sd, uint16 skill_id) {
	...
	// Level up homunculus skill
		...
		if (sd->status.skill_point > 0 && sd->status.skill[idx].id &&
			sd->status.skill[idx].flag == SKILL_FLAG_PERMANENT &&  // Don't allow raising while you
																   // have granted skills. [Skotlex]
			...
			upgradable =
				(lv < skill_tree_get_max(sd->status.skill[idx].id, sd->status.class_)) ? 1 : 0;
			...
			if (pc_checkskill(sd, SG_DEVIL) &&
				...
				clif_status_change(&sd->bl, EFST_DEVIL1, 1, 0, 0, 0,
								   1);	// Permanent blind effect from SG_DEVIL.
			if (!pc_has_permission(sd,
								   PC_PERM_ALL_SKILL))	// may skill everything at any time anyways,
														// and this would cause a huge slowdown
	...
	if (!pc_grant_allskills(sd, true)) {
		...
		if (tree != nullptr && !tree->skills.empty()) {
			for (const auto &skillsit : tree->skills) {
				...
				sd->status.skill[sk_idx].lv =
					skill_tree_get_max(sk_id, sd->status.class_);  // celest
			}
		}
	}
	status_calc_pc(sd, SCO_NONE);
...
int pc_resetstate(struct map_session_data *sd) {
	...
	if (sd->status.base_level > pc_maxbaselv(sd)) {
		ShowError(
			"pc_resetstate: Capping the Level to %d to reset the stats of %d:%d, the base level "
			"(%d) is greater than the max level supported.\n",
...
int pc_resetskill(struct map_session_data *sd, int flag) {
	...
	if (!(flag & 2)) {	// Remove stuff lost when resetting skills.
		/**
		 * It has been confirmed on official servers that when you reset skills with a ranked
		 *Taekwon your skills are not reset (because you have all of them anyway)
		 **/
		...
		if (pc_checkskill(sd, SG_DEVIL) &&
			((sd->class_ & MAPID_THIRDMASK) == MAPID_STAR_EMPEROR || pc_is_maxjoblv(sd)))
			clif_status_load(&sd->bl, EFST_DEVIL1,
							 0);  // Remove perma blindness due to skill-reset. [Skotlex]
		i = sd->sc.option;
	...
	for (const auto &skill : skill_db) {
		...
		if (skill.second->inf2[INF2_ISQUEST] &&
			!battle_config.quest_skill_learn) {	 // Only handle quest skills in a special way when
												 // you can't learn them manually
...
void pc_respawn(struct map_session_data *sd, clr_type clrtype) {
	...
	if (pc_setpos(sd, sd->status.save_point.map, sd->status.save_point.x, sd->status.save_point.y,
				  clrtype) != SETPOS_OK)
 ...
 *------------------------------------------*/
void pc_damage(struct map_session_data *sd, struct block_list *src, unsigned int hp,
			   unsigned int sp, unsigned int ap) {
	if (ap)
...
int pc_dead(struct map_session_data *sd, struct block_list *src) {
	...
	if (hom_is_active(sd->hd) && battle_config.homunculus_auto_vapor &&
		get_percentage(sd->hd->battle_status.hp, sd->hd->battle_status.max_hp) >=
		...
		switch (src->type) {
			case BL_MOB: {
				...
				if (battle_config.mobs_level_up && md->status.hp &&
					(unsigned int)md->level < pc_maxbaselv(sd) && !md->guardian_data &&
					!md->special_state.ai  // Guardians/summons should not level. [Skotlex]
	...
	if (battle_config.bone_drop == 2 || (battle_config.bone_drop == 1 && mapdata->flag[MF_PVP])) {
	...
	if (battle_config.death_penalty_type &&
		...
		&& !sd->sc.cant.deathpenalty && !mapdata->flag[MF_NOEXPPENALTY] &&
	...
	if (mapdata->flag[MF_PVP_NIGHTMAREDROP]) {	// Moved this outside so it works when PVP isn't
												// enabled and during pk mode [Ancyker]
...
bool pc_setparam(struct map_session_data *sd, int64 type, int64 val_tmp) {
	...
	switch (type) {
			...
			if (val > sd->status.base_level) {
				for (int i = 0; i < (int)(val - sd->status.base_level); i++) {
					sd->status.status_point +=
						statpoint_db.pc_gets_status_point(sd->status.base_level + i);
					sd->status.trait_point +=
						statpoint_db.pc_gets_trait_point(sd->status.base_level + i);
				}
			}
		...
		case SP_MANNER:
			...
			return true;  // status_change_start/status_change_end already sends packets warning the
						  // client
		case SP_FAME:
			sd->status.fame = val;
			break;
		...
		case SP_CASHPOINTS:
			...
			if (!sd->state.connect_new)
				log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_CASH,
		...
		case SP_KAFRAPOINTS:
			if (val < 0)
				return false;
			if (!sd->state.connect_new)
				log_cash(sd, LOG_TYPE_SCRIPT, LOG_CASH_TYPE_KAFRA,
		...
		case SP_PCDIECOUNTER:
			...
			if (sd->die_counter == val)
				return true;
			sd->die_counter = val;
			if (!sd->state.connect_new && sd->die_counter == 1 &&
 ...
 *------------------------------------------*/
void pc_heal(struct map_session_data *sd, unsigned int hp, unsigned int sp, unsigned int ap,
			 int type) {  // Is there going to be a effect for gaining AP soon??? [Rytech]
	nullpo_retv(sd);
...
int pc_itemheal(struct map_session_data *sd, t_itemid itemid, int hp, int sp) {
	...
	if (hp) {
		bonus = 100 + (sd->battle_status.vit << 1) + pc_checkskill(sd, SM_RECOVERY) * 10 +
		...
		if (potion_flag == 2) {
			...
			if (sd->sc.data[SC_SPIRIT] && sd->sc.data[SC_SPIRIT]->val2 == SL_ROGUE)
				bonus +=
					bonus;	// Receive an additional +100% effect from ranked potions to HP only
		}
		// All item bonuses.
	...
	if (sp) {
		bonus = 100 + (sd->battle_status.int_ << 1) + pc_checkskill(sd, MG_SRECOVERY) * 10 +
				pc_checkskill(sd, AM_LEARNINGPOTION) * 5;
		// A potion produced by an Alchemist in the Fame Top 10 gets +50% effect [DracoRPG]
		if (potion_flag == 2)
			bonus += bonus * 50 / 100;
...
int pc_percentheal(struct map_session_data *sd, int hp, int sp) {
	...
	if (hp <= 0 && sp <= 0)	 // Damage (negative rates indicate % of max rather than current), and
							 // only kill target IF the specified amount is 100%
...
bool pc_jobchange(struct map_session_data *sd, int job, char upper) {
	...
	if ((b_class & MAPID_UPPERMASK) !=
		(sd->class_ & MAPID_UPPERMASK)) {  // Things to remove when changing class tree.
		status_db.changeSkillTree(sd);
	}

	if ((sd->class_ & MAPID_UPPERMASK) == MAPID_STAR_GLADIATOR &&
		(b_class & MAPID_UPPERMASK) != MAPID_STAR_GLADIATOR) {
		/* going off star glad lineage, reset feel and hate to not store no-longer-used vars in the
		 * database */
		pc_resetfeel(sd);
		pc_resethate(sd);
	}
	...
	if ((b_class & JOBL_UPPER) &&
		...
		clif_updatestatus(sd, SP_STATUSPOINT);
			   ...
			   (previous_class &
				JOBL_UPPER)) {	// Change from a t class to a non t class -> remove points
	...
	if ((b_class & JOBL_FOURTH) && !(previous_class & JOBL_FOURTH)) {  // Change to a 4th job.
		...
		clif_updatestatus(sd, SP_UCON);
		clif_updatestatus(sd, SP_UCRT);
	} else if (!(b_class & JOBL_FOURTH) &&
			   (previous_class & JOBL_FOURTH)) {  // Change to a non 4th job.
	...
	clif_changelook(&sd->bl, LOOK_BASE,
					sd->vd.class_);	 // move sprite update to prevent client crashes with
									 // incompatible equipment [Valaris]
#if PACKETVER >= 20151001
	clif_changelook(
		&sd->bl, LOOK_HAIR,
		sd->vd.hair_style);	 // Update player's head (only matters when switching to or from Doram)
#endif
		...
		case LOOK_HAIR:	 // Use the battle_config limits! [Skotlex]
			...
			if (sd->status.hair != val) {
				...
				if (sd->status.guild_id)  // Update Guild Window. [Skotlex]
					intif_guild_change_memberinfo(sd->status.guild_id, sd->status.account_id,
												  sd->status.char_id, GMI_HAIR, &sd->status.hair,
			...
			if (sd->status.hair_color != val) {
				...
				if (sd->status.guild_id)  // Update Guild Window. [Skotlex]
					intif_guild_change_memberinfo(
						sd->status.guild_id, sd->status.account_id, sd->status.char_id,
...
void pc_setoption(struct map_session_data *sd, int type, int subtype) {
	...
	// Option has to be changed client-side before the class sprite or it won't always work (eg:
	// Wedding sprite) [Skotlex]
	sd->sc.option = type;
	clif_changeoption(&sd->bl);

	if ((type & OPTION_RIDING && !(p_type & OPTION_RIDING)) ||
		(type & OPTION_DRAGON && !(p_type & OPTION_DRAGON) &&
		 pc_checkskill(sd, RK_DRAGONTRAINING) > 0)) {  // Mounting
		clif_status_load(&sd->bl, EFST_RIDING, 1);
		status_calc_pc(sd, SCO_NONE);
	} else if ((!(type & OPTION_RIDING) && p_type & OPTION_RIDING) ||
			   (!(type & OPTION_DRAGON) && p_type & OPTION_DRAGON &&
				pc_checkskill(sd, RK_DRAGONTRAINING) > 0)) {  // Dismount
...
int64 pc_readregistry(struct map_session_data *sd, int64 reg) {
	...
	if (!sd->vars_ok) {
		...
		// This really shouldn't happen, so it's possible the data was lost somewhere, we should
		// request it again. intif->request_registry(sd,type==3?4:type);
		set_eof(sd->fd);
		return 0;
	}
...
bool pc_setregistry_str(struct map_session_data *sd, int64 reg, const char *val) {
	...
	if (!script_check_RegistryVariableLength(1, val, &vlen)) {
		ShowError(
			"pc_check_RegistryVariableLength: Variable value length is too long (aid: %d, cid: "
			"%d): '%s' sz=%zu\n",
...
bool pc_setreg2(struct map_session_data *sd, const char *reg, int64 val) {
	...
	switch (prefix) {
		...
		case '#':
			return (reg[1] == '#') ? pc_setaccountreg2(sd, add_str(reg), val)
								   : pc_setaccountreg(sd, add_str(reg), val);
		default:
...
int64 pc_readreg2(struct map_session_data *sd, const char *reg) {
	...
	switch (prefix) {
		...
		case '#':
			return (reg[1] == '#') ? pc_readaccountreg2(sd, add_str(reg))
								   : pc_readaccountreg(sd, add_str(reg));
		default:
...
bool pc_addeventtimer(struct map_session_data *sd, int tick, const char *name) {
	...
	sd->eventtimer[i] =
		add_timer(gettick() + tick, pc_eventtimer, sd->bl.id, (intptr_t)aStrdup(name));
	sd->eventcount++;
				...
				if (itemdb_type(id) != IT_CARD) {
					...
					if (j > 0) {  // Check if this item not already used
						...
						for (size_t z = 0; z < nb_itemCombo - 1; z++) {
							if (combo_idx[z].idx == index &&
								combo_idx[z].nameid == id) {  // Index already recorded
								do_continue = true;
								break;
							}
						}
							...
							for (size_t c1 = 0; c1 < nb_itemCombo - 1; c1++) {
								if (combo_idx[c1].idx == index && combo_idx[c1].nameid == id) {
									for (uint8 c2 = 0; c2 < MAX_SLOTS; c2++) {
										if (combo_idx[c1].card[c2] ==
											id) {  // Card already recorded (at this same idx)
											do_continue = true;
											break;
										}
									}
	...
	for (int16 i = 0; i < EQI_MAX; i++) {
		...
		if (!itemdb_isspecial(sd->inventory.u.items_inventory[idx].card[0])) {
			for (uint8 j = 0; j < MAX_SLOTS; j++) {
				...
				std::shared_ptr<item_data> data =
					item_db.find(sd->inventory.u.items_inventory[idx].card[j]);
...
bool pc_equipitem(struct map_session_data *sd, short n, int req_pos, bool equipswitch) {
	...
	if (battle_config.battle_log && !equipswitch)
		ShowInfo("equip %u (%d) %x:%x\n", sd->inventory.u.items_inventory[n].nameid, n,
				 id ? id->equip : 0, req_pos);
	...
	if (sd->sc.count && (sd->sc.cant.equip || (sd->sc.data[SC_PYROCLASTIC] &&
											   sd->inventory_data[n]->type == IT_WEAPON))) {
		if (equipswitch) {
			clif_equipswitch_add(sd, n, req_pos, ITEM_EQUIP_ACK_FAIL);
		} else {
			clif_equipitemack(sd, n, 0, ITEM_EQUIP_ACK_FAIL);  // Fail
	...
	if (pos == EQP_ACC) {  // Accessories should only go in one of the two.
		...
		if (pos == EQP_ACC)	 // User specified both slots.
			pos =
				(equip_index[EQI_ACC_R] >= 0 && equip_index[EQI_ACC_L] < 0) ? EQP_ACC_L : EQP_ACC_R;

		for (i = 0; i < sd->inventory_data[n]->slots;
			 i++) {	 // Accessories that have cards that force equip location
			if (!sd->inventory.u.items_inventory[n].card[i])
				continue;

			std::shared_ptr<item_data> card_data =
				item_db.find(sd->inventory.u.items_inventory[n].card[i]);
...
#ifdef RENEWAL
			pos = (equip_index[EQI_HAND_R] >= 0 && equip_index[EQI_HAND_L] < 0) ? EQP_HAND_L
																				: EQP_HAND_R;
#else
		...
		if (pos == EQP_SHADOW_ACC)
			pos = (equip_index[EQI_SHADOW_ACC_L] >= 0 && equip_index[EQI_SHADOW_ACC_R] < 0)
					  ? EQP_SHADOW_ACC_R
					  : EQP_SHADOW_ACC_L;
	...
	if (battle_config.ammo_unequip && (pos & EQP_ARMS) && id->type == IT_WEAPON) {
		...
		if (idx >= 0) {
			switch (sd->inventory_data[idx]->subtype) {
				...
				case AMMO_BULLET:
				case AMMO_SHELL:
					if (id->subtype != W_REVOLVER && id->subtype != W_RIFLE &&
	...
	if (id) {
		...
		if (id->equip_script &&
			(pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(id, sd->bl.m)))
		...
		if (itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0]))
			...
			for (i = 0; i < MAX_SLOTS; i++) {
				...
				if (data != nullptr) {
					if (data->equip_script && (pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) ||
											   !itemdb_isNoEquip(data.get(), sd->bl.m)))
...
static void pc_unequipitem_sub(struct map_session_data *sd, int n, int flag) {
	...
	if (sd->sc.data[SC_SIGNUMCRUCIS] &&
		!battle_check_undead(sd->battle_status.race, sd->battle_status.def_ele))
...
bool pc_unequipitem(struct map_session_data *sd, int n, int flag) {
	...
	if (!(flag & 2) && sd->sc.count &&
		(sd->sc.cant.unequip || (sd->sc.data[SC_PYROCLASTIC] &&
	...
	if (sd->inventory_data[n]->type == IT_AMMO &&
		(sd->inventory_data[n]->nameid != ITEMID_SILVER_BULLET ||
...
void pc_checkitem(struct map_session_data *sd) {
	...
	if (sd->state.vending)	// Avoid reorganizing items when we are vending, as that leads to
							// exploits (pointed out by End of Exam)
		return;
...
void pc_check_available_item(struct map_session_data *sd, uint8 type) {
	...
	if (battle_config.item_check & ITMCHK_INVENTORY &&
		...
		for (i = 0; i < MAX_INVENTORY; i++) {
			...
			if (!itemdb_available(nameid)) {
				sprintf(output, msg_txt(sd, 709),
						nameid);  // Item %u has been removed from your inventory.
				clif_displaymessage(sd->fd, output);
				ShowWarning(
					"Removed invalid/disabled item (ID: %u, amount: %d) from inventory (char_id: "
					"%d).\n",
	...
	if (battle_config.item_check & ITMCHK_CART &&
		type & ITMCHK_CART) {  // Check for invalid(ated) items in cart.
		for (i = 0; i < MAX_CART; i++) {
			...
			if (!nameid)
				continue;
			if (!itemdb_available(nameid)) {
				sprintf(output, msg_txt(sd, 710),
						nameid);  // Item %u has been removed from your cart.
				clif_displaymessage(sd->fd, output);
				ShowWarning(
	...
	if (battle_config.item_check & ITMCHK_STORAGE &&
		...
		for (i = 0; i < sd->storage.max_amount; i++) {
			...
			if (!nameid)
				continue;
			if (!itemdb_available(nameid)) {
				sprintf(output, msg_txt(sd, 711),
						nameid);  // Item %u has been removed from your storage.
				clif_displaymessage(sd->fd, output);
				ShowWarning(
					"Removed invalid/disabled item (ID: %u, amount: %d) from storage (char_id: "
					"%d).\n",
...
TIMER_FUNC(pc_calc_pvprank_timer) {
	...
	if (pc_calc_pvprank(sd) > 0)
		sd->pvp_timer =
			add_timer(gettick() + PVP_CALCRANK_INTERVAL, pc_calc_pvprank_timer, id, data);
	return 0;
}
...
static int pc_daynight_timer_sub(struct map_session_data *sd, va_list ap) {
	if (sd->state.night != night_flag &&
		map_getmapflag(sd->bl.m, MF_NIGHTENABLED)) {	  // Night/day state does not match.
...
TIMER_FUNC(map_day_timer) {
	...
	strcpy(tmp_soutput,
		   (data == 0) ? msg_txt(NULL, 502) : msg_txt(NULL, 60));  // The day has arrived!
...
TIMER_FUNC(map_night_timer) {
	...
	map_foreachpc(pc_daynight_timer_sub);
	strcpy(tmp_soutput,
		   (data == 0) ? msg_txt(NULL, 503) : msg_txt(NULL, 59));  // The night has fallen...
	intif_broadcast(tmp_soutput, strlen(tmp_soutput) + 1, BC_DEFAULT);
	return 0;
}
 ...
 * @param sd
 * @param force Ignore the check, ask player to stand up. Used in some cases like pc_damage(),
 * pc_revive(), etc
 ...
 * @param sd Player map session data
 */
bool pc_should_log_commands(struct map_session_data *sd) { return sd->group->log_commands; }
...
static TIMER_FUNC(pc_spiritcharm_timer) {
	...
	if (i != sd->spiritcharm)
		memmove(sd->spiritcharm_timer + i, sd->spiritcharm_timer + i + 1,
				(sd->spiritcharm - i) * sizeof(int));
 ...
 * @return Penalty rate
 */
uint16 pc_level_penalty_mod(struct map_session_data *sd, e_penalty_type type,
							std::shared_ptr<s_mob_db> mob, mob_data *md) {
	...
	if ((type == PENALTY_DROP || type == PENALTY_MVP_DROP) &&
		status_has_mode(&mob->status, MD_FIXEDITEMDROP)) {
		return 100;
	}
	...
	if (this->nodeExists(node, "Tree")) {
		for (const auto &it : node["Tree"]) {
			...
			if (max_lv > MAX_SKILL_LEVEL) {
				this->invalidWarning(it["MaxLevel"],
									 "MaxLevel exceeds the maximum skill level of %d, skipping.\n",
									 MAX_SKILL_LEVEL);
				return 0;
			}
			...
			if (max_lv > skill_lv_max) {
				this->invalidWarning(it["MaxLevel"],
									 "Skill %s's level %hu exceeds the skill's max level %hu. "
									 "Capping skill level.\n",
			...
			// this->invalidWarning(it["Name"], "Failed to erase %s, the skill doesn't exist in for
			...
			if (this->nodeExists(it, "BaseLevel")) {
				...
				if (baselv > baselv_max) {
					this->invalidWarning(it["BaseLevel"],
										 "Skill %hu's base level requirement %hu exceeds job %s's "
										 "max base level %d. Capping skill base level.\n",
			...
			if (this->nodeExists(it, "JobLevel")) {
				...
				if (joblv > joblv_max) {
					this->invalidWarning(it["JobLevel"],
										 "Skill %hu's job level requirement %hu exceeds job %s's "
										 "max job level %d. Capping skill job level.\n",
			...
			if (this->nodeExists(it, "Requires")) {
				for (const auto &Requiresit : it["Requires"]) {
					...
					if (lv_req > MAX_SKILL_LEVEL) {
						this->invalidWarning(
							Requiresit["Level"],
							"Level exceeds the maximum skill level of %d, skipping.\n",
							MAX_SKILL_LEVEL);
						return 0;
					}
					...
					if (lv_req > lv_req_max) {
						this->invalidWarning(it["MaxLevel"],
											 "Required skill %s's level %hu exceeds the skill's "
											 "max level %hu. Capping skill level.\n",
					...
					if (lv_req == 0) {
						if (entry->need.erase(skill_id_req) == 0)
							this->invalidWarning(Requiresit["Name"],
												 "Failed to erase %s, the skill doesn't exist in "
												 "for job %s, skipping.\n",
		...
		for (const auto &inherit_job : data.second->inherit_job) {
			...
			for (const auto &it : tree->skills) {
				...
				if (skill->baselv > baselv_max) {
					ShowWarning(
						"SkillTreeDatabase: Skill %s (%hu)'s base level requirement %hu exceeds "
						"job %s's max base level %d. Capping skill base level.\n",
						skill_get_name(skill->skill_id), skill->skill_id, skill->baselv,
						job_name(data.first), baselv_max);
					skill->baselv = baselv_max;
				}
				if (skill->joblv > joblv_max) {
					ShowWarning(
						"SkillTreeDatabase: Skill %s (%hu)'s job level requirement %hu exceeds job "
						"%s's max job level %d. Capping skill job level.\n",
						skill_get_name(skill->skill_id), skill->skill_id, skill->joblv,
						job_name(data.first), joblv_max);
	...
	if (!job_tree.empty()) {
		for (auto &data : *this) {
			if (job_tree.count(data.first) == 0)
				continue;
			data.second->skills.insert(job_tree[data.first]->skills.begin(),
									   job_tree[data.first]->skills.end());
		}
	}
	...
	if (this->nodeExists(node, "Jobs")) {
		...
		for (const auto &jobit : jobsNode) {
			...
			if (!exists) {
				...
				std::fill(job->job_bonus.begin(), job->job_bonus.end(),
						  std::array<uint16, PARAM_MAX>{0});
			...
			if (this->nodeExists(node, "BaseASPD")) {
				...
				for (const auto &aspdit : aspdNode) {
					...
					if (!script_get_constant(weapon_constant.c_str(), &constant)) {
						this->invalidWarning(aspdNode["BaseASPD"],
											 "Unknown weapon type %s specified for %s, skipping.\n",
					...
					if (constant < W_FIST || constant > max) {
						this->invalidWarning(aspdNode["BaseASPD"],
											 "Invalid weapon type %s specified for %s, skipping.\n",
											 weapon.c_str(), job_name.c_str());
						continue;
					}
			...
			if (this->nodeExists(node, "MaxStats")) {
				...
				for (const auto &statit : statNode) {
					...
					if (!script_get_constant(stat_constant.c_str(), &constant)) {
						this->invalidWarning(statNode["Bonus"],
											 "Unknown max stat %s specified for %s, skipping.\n",
					...
					if (constant < PARAM_STR || constant >= PARAM_MAX) {
						this->invalidWarning(statNode["Bonus"],
											 "Invalid max stat %s specified for %s, skipping.\n",
											 stat.c_str(), job_name.c_str());
						continue;
					}
			...
			if (this->nodeExists(node, "BaseExp")) {
				for (const ryml::NodeRef &bexpNode : node["BaseExp"]) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(bexpNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
			...
			if (this->nodeExists(node, "MaxJobLevel")) {
				...
				if (level == 0 || level > MAX_LEVEL) {
					this->invalidWarning(
						node["MaxJobLevel"],
						"MaxJobLevel must be between 1~MAX_LEVEL for %s, capping to MAX_LEVEL.\n",
						job_name.c_str());
					level = MAX_LEVEL;
				}
			...
			if (this->nodeExists(node, "JobExp")) {
				for (const ryml::NodeRef &jexpNode : node["JobExp"]) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(jexpNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
			...
			if (this->nodeExists(node, "BonusStats")) {
				...
				for (const ryml::NodeRef &levelNode : bonusNode) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(levelNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
			...
			if (this->nodeExists(node, "BaseHp")) {
				for (const ryml::NodeRef &bhpNode : node["BaseHp"]) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(bhpNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
			...
			if (this->nodeExists(node, "BaseSp")) {
				for (const ryml::NodeRef &bspNode : node["BaseSp"]) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(bspNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
			...
			if (this->nodeExists(node, "BaseAp")) {
				for (const ryml::NodeRef &bapNode : node["BaseAp"]) {
					...
					if (level == 0 || level > MAX_LEVEL) {
						this->invalidWarning(bapNode["Level"],
											 "Level must be between 1~MAX_LEVEL for %s.\n",
											 job_name.c_str());
						return 0;
					}
...
void JobDatabase::loadingFinished() {
	...
	for (auto &jobIt : *this) {
		...
		if (!pcdb_checkid(job_id))
			continue;
		if (job_id == JOB_WEDDING || job_id == JOB_XMAS || job_id == JOB_SUMMER ||
			job_id == JOB_HANBOK || job_id == JOB_OKTOBERFEST || job_id == JOB_SUMMER2)
		...
		do {
			...
			// Extended classes
			if ((class_ & MAPID_UPPERMASK) == MAPID_KAGEROUOBORO ||
				(class_ & MAPID_UPPERMASK) == MAPID_REBELLION) {
...
void PlayerStatPointDatabase::loadingFinished() {
	...
	for (uint16 level = 2; level <= MAX_LEVEL; level++) {
		...
		if (!battle_config.use_traitpoint_table || !exists) {
			...
			if (level > trait_start_level) {
				entry->traitpoints =
					(level - trait_start_level) * 3 + (level - trait_start_level) / 5 * 4;
			} else {
				entry->traitpoints = 0;
			}
		}
...
void pc_readdb(void) {
	...
	for (i = 0; i < ARRAYLENGTH(dbsubpath); i++) {
		...
		sv_readdb(dbsubpath2, "job_noenter_map.txt", ',', 3, 3, CLASS_COUNT,
				  &pc_readdb_job_noenter_map, i > 0);
		aFree(dbsubpath1);
		aFree(dbsubpath2);
	}

	// Reset and read skilltree - needs to be read after pc_readdb_job_exp to get max base and job
	// levels
		...
		while (entries < MOTD_LINE_SIZE && fgets(buf, CHAT_SIZE_MAX, fp)) {
			...
			if (len) {
				...
				if ((ptr = strstr(buf, " :")) != NULL &&
					ptr - buf >= NAME_LENGTH)  // crashes newer clients
					ShowWarning("Found sequence '" CL_WHITE " :" CL_RESET "' on line '" CL_WHITE
								"%u" CL_RESET "' in '" CL_WHITE "%s" CL_RESET
								"'. This can cause newer clients to crash.\n",
								lines, motd_txt);
			} else {  // empty line
				buf[0] = ' ';
				buf[1] = 0;
 ...
 **/
uint8 pc_itemcd_add(struct map_session_data *sd, struct item_data *id, t_tick tick,
					unsigned short n) {
	int i;
		...
		if (sd->item_delay[i].nameid) {	 // found
			if (DIFF_TICK(sd->item_delay[i].tick, tick) > 0) {
				...
				if (e_tick > 99)
					sprintf(
						e_msg,
						msg_txt(sd, 379),  // Item Failed. [%s] is cooling down. Wait %.1f minutes.
						itemdb_ename(sd->item_delay[i].nameid), (double)e_tick / 60);
				else
					sprintf(
						e_msg,
						msg_txt(sd, 380),  // Item Failed. [%s] is cooling down. Wait %d seconds.
 ...
 *         1: Has delay, cancel consumption.
 **/
uint8 pc_itemcd_check(struct map_session_data *sd, struct item_data *id, t_tick tick,
					  unsigned short n) {
	...
	if (sc->data[id->delay.sc]) {
		...
		clif_msg_value(sd, ITEM_REUSE_LIMIT,
					   (int)(timer ? DIFF_TICK(timer->tick, tick) / 1000 : 99));
		return 1;
	}
...
void pc_scdata_received(struct map_session_data *sd) {
	pc_inventory_rentals(sd);  // Needed here to remove rentals that have Status Changes after
							   // chrif_load_scdata has finished

	clif_weight_limit(sd);

	if (pc_has_permission(sd, PC_PERM_ATTENDANCE) && pc_attendance_enabled() &&
		!pc_attendance_rewarded_today(sd)) {
	...
	if (pc_iscarton(sd)) {
		...
		status_calc_cart_weight(
			sd, (e_status_calc_weight_opt)(CALCWT_ITEM | CALCWT_MAXBONUS | CALCWT_CARTSTATE));
	}
...
void pc_check_expiration(struct map_session_data *sd) {
	...
	if (sd->expiration_time != 0) {	 // Don't display if it's unlimited or unknow value
		...
		strftime(tmpstr, sizeof(tmpstr) - 1, msg_txt(sd, 501),
				 localtime(&exp_time));	 // "Your account time limit is: %d-%m-%Y %H:%M:%S."
...
void pc_expire_check(struct map_session_data *sd) {
	...
	if (sd->expiration_time > (time(NULL) + ((60 * 60) * 24))) {
		...
		if (pc_expiration_tid == INVALID_TIMER) /* Starts in 1h, repeats every hour */
			pc_expiration_tid =
				add_timer_interval(gettick() + ((1000 * 60) * 60), pc_global_expiration_timer, 0, 0,
								   ((1000 * 60) * 60));
	...
	sd->expiration_tid =
		add_timer(gettick() + (unsigned int)(sd->expiration_time - time(NULL)) * 1000,
				  pc_expiration_timer, sd->bl.id, 0);
}
...
enum e_BANKING_WITHDRAW_ACK pc_bank_withdraw(struct map_session_data *sd, int money) {
	...
	if (money <= 0) {
		...
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 1495), false,
						  SELF);  // You can't withdraw that much money
		return BWA_UNKNOWN_ERROR;
	}
	...
	while (node) {
		...
		if ((entry = (struct s_bonus_script_entry *)node->data)) {
			...
			if (entry->tid == INVALID_TIMER) {
				...
				entry->tick += now;
				entry->tid =
					add_timer(entry->tick, pc_bonus_script_timer, sd->bl.id, (intptr_t)entry);
			}
 ...
 * @author [Cydh]
 **/
struct s_bonus_script_entry *pc_bonus_script_add(struct map_session_data *sd,
												 const char *script_str, t_tick dur,
												 enum efst_type icon, uint16 flag, uint8 type) {
...
static void inline pc_bonus_script_check_final(struct map_session_data *sd) {
	if (sd->bonus_script.count == 0) {
		if (sd->bonus_script.head && sd->bonus_script.head->data)
			pc_bonus_script_free_entry(sd,
									   (struct s_bonus_script_entry *)sd->bonus_script.head->data);
	...
	return ((sd->class_ & JOBL_THIRD) ? battle_config.max_third_aspd
									  : (((sd->class_ & MAPID_UPPERMASK) == MAPID_KAGEROUOBORO ||
										  (sd->class_ & MAPID_UPPERMASK) == MAPID_REBELLION)
											 ...
											 : ((sd->class_ & MAPID_BASEMASK) == MAPID_SUMMONER)
...
short pc_get_itemgroup_bonus(struct map_session_data *sd, t_itemid nameid,
							 std::vector<s_item_bonus> &bonuses) {
	if (bonuses.empty())
		return 0;
...
/**
 * Check if player's equip index in same specified position, like for 2-Handed weapon & Heagdear
 * (inc. costume)
 * @param eqi Item EQI of enum equip_index
 * @param *equip_index Player's equip_index[]
 * @param index Known index item in inventory from sd->equip_index[] to compare with specified EQI
 * in *equip_index
...
bool pc_is_same_equip_index(enum equip_index eqi, short *equip_index, short index) {
	...
	if (eqi == EQI_HEAD_TOP &&
		(equip_index[EQI_HEAD_MID] == index || equip_index[EQI_HEAD_LOW] == index))
		return true;
	// Headgear with Mid & Low location
...
bool pc_job_can_entermap(enum e_job jobid, int m, int group_lv) {
	...
	// !FIXME: Currently, a map-server doesn't recognized map's attributes on other server, so we
	// assume it's fine to warp.
	if (m < 0)
		return true;
	...
	if ((job->noenter_map.zone & 1 && !mapdata_flag_vs2(mapdata)) ||		// Normal
		...
		(job->noenter_map.zone & (mapdata->zone) &&
		 mapdata->flag[MF_RESTRICTED])	// Zone restriction
	)
		return false;
 ...
 */
void pc_macro_captcha_register(map_session_data &sd, uint16 image_size,
...
void pc_macro_captcha_register_upload(map_session_data &sd, uint16 upload_size, char *upload_data) {
	...
	memcpy(&sd.captcha_upload.cd->image_data[sd.captcha_upload.upload_size], upload_data,
		   upload_size);
...
TIMER_FUNC(pc_macro_detector_timeout) {
	...
	if (sd->macro_detect.retry == 0) {
		...
		chrif_req_login_operation(sd->macro_detect.reporter_aid, sd->status.name,
								  CHRIF_OP_LOGIN_BLOCK, 0, 0, 0);
	} else {
		// Update the client
		clif_macro_detector_request_show(*sd);

		// Start a new timer
		sd->macro_detect.timer = add_timer(gettick() + battle_config.macro_detection_timeout,
										   pc_macro_detector_timeout, sd->bl.id, 0);
	}
	return 0;
}
 ...
 */
void pc_macro_detector_process_answer(map_session_data &sd,
									  char captcha_answer[CAPTCHA_ANSWER_SIZE]) {
	nullpo_retv(captcha_answer);
	...
	if (strcmp(captcha_answer, cd->captcha_answer) == 0) {
		...
		pc_setreg(&sd, add_str("@captcha_retries"),
				  battle_config.macro_detection_retry - sd.macro_detect.retry);
 ...
 * @param radius: Area
 */
void pc_macro_reporter_area_select(map_session_data &sd, const int16 x, const int16 y,
								   const int8 radius) {
	std::vector<uint32> aid_list;

	map_foreachinarea(pc_macro_reporter_area_select_sub, sd.bl.m, x - radius, y - radius,
					  x + radius, y + radius, BL_PC, &aid_list);
...
void pc_macro_reporter_process(map_session_data &ssd, map_session_data &tsd) {
	...
	tsd.macro_detect.timer = add_timer(gettick() + battle_config.macro_detection_timeout,
									   pc_macro_detector_timeout, tsd.bl.id, 0);
}
 ...
 */
bool pc_macro_read_captcha_db_loadbmp(const std::string &filepath,
...
uint64 CaptchaDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Answer")) {
		...
		if (answer.length() < 4 || answer.length() > CAPTCHA_ANSWER_SIZE) {
			this->invalidWarning(node["Answer"],
								 "The captcha answer must be between 4~%d characters, skipping...",
								 CAPTCHA_ANSWER_SIZE);
			return 0;
		}
	...
	if (this->nodeExists(node, "Bonus")) {
		...
		cd->bonus_script =
			parse_script(script.c_str(), this->getCurrentFile().c_str(),
						 this->getLineNumber(node["Bonus"]), SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	} else {
		if (!exists)
			cd->bonus_script = parse_script(
				"specialeffect2 EF_BLESSING; sc_start SC_BLESSING,600000,10; specialeffect2 "
				"EF_INCAGILITY; sc_start SC_INCREASEAGI,600000,10;",
...
void do_init_pc(void) {
	...
	if (battle_config.day_duration > 0 && battle_config.night_duration > 0) {
		...
		day_timer_tid =
			add_timer_interval(gettick() + (night_flag ? 0 : day_duration) + night_duration,
							   map_day_timer, 0, 0, day_duration + night_duration);
		night_timer_tid =
			add_timer_interval(gettick() + day_duration + (night_flag ? night_duration : 0),
							   map_night_timer, 0, 0, day_duration + night_duration);
	}

	do_init_pc_groups();

	pc_sc_display_ers =
		ers_new(sizeof(struct sc_display_entry), "pc.cpp:pc_sc_display_ers", ERS_OPT_FLEX_CHUNK);
	num_reg_ers = ers_new(sizeof(struct script_reg_num), "pc.cpp:num_reg_ers",
						  (ERSOptions)(ERS_OPT_CLEAN | ERS_OPT_FLEX_CHUNK));
	str_reg_ers = ers_new(sizeof(struct script_reg_str), "pc.cpp:str_reg_ers",
						  (ERSOptions)(ERS_OPT_CLEAN | ERS_OPT_FLEX_CHUNK));
