...
static struct chat_data* chat_createchat(struct block_list* bl, const char* title, const char* pass,
										 int limit, bool pub, int trigger, const char* ev, int zeny,
...
int chat_createpcchat(struct map_session_data* sd, const char* title, const char* pass, int limit,
	...
	if (sd->chatID)
		return 0;  // Prevent people abusing the chat system by creating multiple chats, as pointed
				   // out by End of Exam. [Skotlex]
...
int chat_joinchat(struct map_session_data* sd, int chatid, const char* pass) {
	...
	if (cd == NULL || cd->bl.type != BL_CHAT || cd->bl.m != sd->bl.m || sd->state.vending ||
		sd->state.buyingstore || sd->chatID ||
		((cd->owner->type == BL_NPC) ? cd->users + 1 : cd->users) >= cd->limit) {
		clif_joinchatfail(sd, 0);
		return 0;
	}

	if (!cd->pub && strncmp(pass, cd->pass, sizeof(cd->pass)) != 0 &&
		!pc_has_permission(sd, PC_PERM_JOIN_ALL_CHAT)) {
...
int chat_leavechat(struct map_session_data* sd, bool kicked) {
	...
	for (i = leavechar; i < cd->users; i++) cd->usersd[i] = cd->usersd[i + 1];
	...
	if (cd->users == 0 && cd->owner->type == BL_PC) {  // Delete empty chatroom
		...
		skill_unit* unit =
			map_find_skill_unit_oncell(&sd->bl, sd->bl.x, sd->bl.y, AL_WARP, nullptr, 0);
 ...
 */
int chat_changechatstatus(struct map_session_data* sd, const char* title, const char* pass,
						  int limit, bool pub) {
	struct chat_data* cd;
 ...
 * @return 0
 */
int chat_createnpcchat(struct npc_data* nd, const char* title, int limit, bool pub, int trigger,
					   const char* ev, int zeny, int minLvl, int maxLvl) {
	struct chat_data* cd;
	...
	if (zeny > MAX_ZENY || maxLvl > MAX_LEVEL) {
		ShowError(
			"chat_createnpcchat: npc '%s' has a required lvl or amount of zeny over the max "
			"limit!\n",
			nd->exname);
		return 0;
	}
...
int chat_npckickall(struct chat_data* cd) {
	...
	while (cd->users > 0) chat_leavechat(cd->usersd[cd->users - 1], 0);
