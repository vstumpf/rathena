...
struct s_pet_db {
	...
	uint16 attack_rate;			///< Rate of which the pet will attack (requires at least
								///< pet_support_min_friendly intimacy).
	uint16
		defence_attack_rate;	///< Rate of which the pet will retaliate when master is being
								///< attacked (requires at least pet_support_min_friendly intimacy).
	...
	std::unordered_map<uint16, std::shared_ptr<s_pet_evo_data>>
		evolution_data;	 ///< Data for evolving the pet.
	struct script_code
...
struct pet_skill_attack {  // Attack Skill
	...
	unsigned short
		bonusrate;	// How being 100% loyal affects cast rate (10 = At 1000 intimacy->rate+10%
};
...
bool pet_addautobonus(std::vector<std::shared_ptr<s_petautobonus>> &bonus,
					  const std::string &script, int16 rate, uint32 dur, uint16 atk_type,
