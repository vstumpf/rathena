...
struct s_battleground_map {
	...
	bool isReserved;  ///< Reserve BG maps that are used so that the system won't create multiple BG
					  ///< instances on the same map
};

/// Enum for queue state tracking
enum e_queue_state : uint16 {
	QUEUE_STATE_SETUP =
		0,	///< The initial setup of a queue (a required amount of players hasn't been met)
	QUEUE_STATE_SETUP_DELAY,  ///< The initial setup of a queue but a required amount of players
							  ///< have accepted and the delay timer is active
	QUEUE_STATE_ACTIVE,	 ///< The queue is active script side and more players can join (players may
						 ///< or may not be on the field)
