...
export_constant2("Job_SuperNovice",
				 JOB_SUPER_NOVICE);	 // TODO: Refactor NPCs to use the new constant
...
export_deprecated_constant3("JOB_BABY_CROSS", JOB_BABY_GUILLOTINE_CROSS,
...
export_deprecated_constant3("EAJ_BABY_CROSS", MAPID_BABY_GUILLOTINE_CROSS,
...
export_deprecated_constant3("mf_sumstarmiracle", MF_NOSUNMOONSTARMIRACLE,
...
export_constant2(
	"VAR_HEAD_TOP",
	LOOK_HEAD_TOP);	 // This one was actually pointing to LOOK_HEAD_BOTTOM until now, so we might be
					 // safe to remove them anyway(since we got no bug report until today)
export_constant2(
	"VAR_HEAD_MID",
	LOOK_HEAD_MID);	 // This one was actually pointing to LOOK_HEAD_TOP until now, so we might be
					 // safe to remove them anyway(since we got no bug report until today)
export_constant2(
	"VAR_HEAD_BOTTOM",
	LOOK_HEAD_BOTTOM);	// This one was actually pointing to LOOK_HEAD_MID until now, so we might be
						// safe to remove them anyway(since we got no bug report until today)
