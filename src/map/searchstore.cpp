...
enum e_searchstore_failure {
	...
	SSI_FAILED_OVER_MAXCOUNT =
		1,	// "There are too many results. Please enter more detailed search term."
...
typedef bool (*searchstore_searchall_t)(struct map_session_data* sd,
										const struct s_search_store_search* s);
...
void searchstore_query(struct map_session_data* sd, unsigned char type, unsigned int min_price,
	...
	for (pl_sd = (struct map_session_data*)dbi_first(iter); dbi_exists(iter);
		 pl_sd = (struct map_session_data*)dbi_next(iter)) {
...
void searchstore_click(struct map_session_data* sd, uint32 account_id, int store_id,
	...
	if (!battle_config.feature_search_stores || !sd->searchstore.open ||
		sd->searchstore.items.empty())
		return;

	searchstore_clearremote(sd);

	ARR_FIND(0, sd->searchstore.items.size(), i,
			 sd->searchstore.items[i]->store_id == store_id &&
				 sd->searchstore.items[i]->account_id == account_id &&
				 sd->searchstore.items[i]->nameid == nameid);
	if (i == sd->searchstore.items.size()) {  // no such result, crafted
		ShowWarning(
			"searchstore_click: Received request with item %u of account %d, which is not part of "
			"current result set (account_id=%d, char_id=%d).\n",
	...
	if (!searchstore_hasstore(pl_sd, sd->searchstore.type) ||
		searchstore_getstoreid(pl_sd, sd->searchstore.type) !=
			store_id) {	 // no longer vending/buying or not same shop
		clif_search_store_info_failed(sd, SSI_FAILED_SSILIST_CLICK_TO_OPEN_STORE);
		return;
	}
...
bool searchstore_queryremote(struct map_session_data* sd, uint32 account_id) {
	return (bool)(sd->searchstore.open && !sd->searchstore.items.empty() &&
				  sd->searchstore.remote_id == account_id);
}
 ...
 * @param sd : player requesting
 */
void searchstore_clearremote(struct map_session_data* sd) { sd->searchstore.remote_id = 0; }
