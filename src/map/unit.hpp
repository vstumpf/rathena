...
struct unit_data {
	struct block_list *bl;	/// link to owner object
							/// BL_PC|BL_MOB|BL_PET|BL_NPC|BL_HOM|BL_MER|BL_ELEM
	...
	uint16 stepskill_id, stepskill_lv;	// Remembers skill that should be casted on step
										// [Playtester]
	...
	struct s_udState {
		...
		bool force_walk;  ///< Used with script commands unitwalk/unitwalkto. Disables monster idle
						  ///< and random walk.
	} state;
...
enum e_unit_stop_walking {
	...
	USW_MOVE_FULL_CELL =
		0x4,  /// Enable moving to the next cell when unit was already half-way there (may cause
