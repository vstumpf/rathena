	...
	if (this->nodeExists(node, "AdditionalMaps")) {
		...
		for (const auto &mapIt : mapNode) {
			...
			if (m == instance->enter.map) {
				this->invalidWarning(
					mapNode, "Additional Map %s is already listed as the EnterMap.\n", map.c_str());
				continue;
			}
...
static TIMER_FUNC(instance_subscription_timer) {
	...
	switch (mode) {
		case IM_NONE:
			break;
		case IM_CHAR:
			if (ret == 0 && (sd = map_charid2sd(
								 idata->owner_id)))	 // If no maps are created, tell player to wait
				clif_instance_changewait(instance_id, 0xffff);
			break;
		case IM_PARTY:
			if (ret == 0 &&
				(pd = party_search(idata->owner_id)))  // If no maps are created, tell party to wait
				clif_instance_changewait(instance_id, 0xffff);
			break;
		case IM_GUILD:
			if (ret == 0 &&
				(gd = guild_search(idata->owner_id)))  // If no maps are created, tell guild to wait
				clif_instance_changewait(instance_id, 0xffff);
			break;
		case IM_CLAN:
			if (ret == 0 &&
				(cd = clan_search(idata->owner_id)))  // If no maps are created, tell clan to wait
				clif_instance_changewait(instance_id, 0xffff);
			break;
		default:
			return 0;
	...
	if (!instance_wait.id.empty())
		instance_wait.timer =
			add_timer(gettick() + INSTANCE_INTERVAL, instance_subscription_timer, 0, 0);
	else
...
bool instance_startkeeptimer(std::shared_ptr<s_instance_data> idata, int instance_id) {
	...
	idata->keep_timer =
		add_timer(gettick() + db->limit * 1000, instance_delete_timer, instance_id, 0);
	...
	switch (idata->mode) {
		...
		case IM_CHAR:
			if (map_charid2sd(idata->owner_id))	 // Notify player of the added instance timer
				clif_instance_status(instance_id, static_cast<uint32>(idata->keep_limit),
									 static_cast<uint32>(idata->idle_limit));
			break;
		case IM_PARTY:
...
bool instance_startidletimer(std::shared_ptr<s_instance_data> idata, int instance_id) {
	...
	idata->idle_timer =
		add_timer(gettick() + db->timeout * 1000, instance_delete_timer, instance_id, 0);
	...
	switch (idata->mode) {
		...
		case IM_CHAR:
			if (map_charid2sd(idata->owner_id))	 // Notify the player
				clif_instance_changestatus(instance_id, IN_NOTIFY,
										   static_cast<uint32>(idata->idle_limit));
			break;
		case IM_PARTY:
...
void instance_addnpc(std::shared_ptr<s_instance_data> idata) {
	...
	for (const auto &it : idata->map) {
		...
		map_foreachinallarea(instance_addnpc_sub, it.src_m, 0, 0, mapdata->xs, mapdata->ys, BL_NPC,
							 it.m);
	}
 ...
 * @return -4 = no free instances | -3 = already exists | -2 = character/party/guild not found | -1
 * = invalid type | On success return instance_id
 */
...
int instance_addmap(int instance_id) {
	...
	for (const auto &it : db->maplist) {
		if ((m = map_addinstancemap(it, instance_id, db->nomapflag)) <
...
#error This algorithm is only safe for up to 9999 maps, change at your own risk.
	...
	safesnprintf(outname, MAP_NAME_LENGTH, "%0*u#%0*u", prefix_length, map_id % prefix_limit,
				 suffix_length, instance_id % suffix_limit);
}
...
void instance_destroy_command(map_session_data *sd) {
	...
	} else if (sd->instance_mode == IM_GUILD && sd->guild != nullptr &&
			   sd->guild->instance_id > 0) {
...
bool instance_destroy(int instance_id) {
	...
	if (idata->state == INSTANCE_IDLE) {
		for (auto instance_it = instance_wait.id.begin(); instance_it != instance_wait.id.end();
			 ++instance_it) {
			if (*instance_it == instance_id) {
				...
				for (int i = 0; i < instance_wait.id.size(); i++) {
					if (util::umap_find(instances, instance_wait.id[i])->state == INSTANCE_IDLE)
						if ((mode == IM_CHAR && sd) || (mode == IM_PARTY && pd) ||
							(mode == IM_GUILD && gd) || (mode == IM_CLAN && cd))
							clif_instance_changewait(instance_id, i + 1);
				}
		...
		for (const auto &it : idata->map) {
			...
			map_foreachinallarea(instance_npcdestroy, it.m, 0, 0, mapdata->xs, mapdata->ys, BL_NPC,
								 it.m);
	...
	ShowInfo("[Instance] Destroyed: %s (%d)\n", instance_db.find(idata->id)->name.c_str(),
			 instance_id);
 ...
 */
e_instance_enter instance_enter(struct map_session_data *sd, int instance_id, const char *name,
								short x, short y) {
	...
	// If one of the two coordinates was not given or is below zero, we use the entry point from the
	// database
	if (x < 0 || y < 0) {
		x = db->enter.x;
		y = db->enter.y;
...
bool instance_delusers(int instance_id) {
	...
	for (const auto &it : idata->map) users += max(map_getmapdata(it.m)->users, 0);
...
void do_reload_instance(void) {
	...
	for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC *)mapit_next(iter)) {
		...
		if (sd && mapdata->instance_id > 0) {
			...
			int instance_id;
			std::shared_ptr<s_instance_data> idata =
				util::umap_find(instances, map[sd->bl.m].instance_id);
			std::shared_ptr<s_instance_db> db = instance_db.find(idata->id);
			...
			switch (idata->mode) {
				...
				case IM_CHAR:
					if (sd->instance_id !=
						mapdata
							->instance_id)	// Someone who is not instance owner is on instance map
						continue;
					instance_id = sd->instance_id;
					break;
				case IM_PARTY:
					if ((!(pd = party_search(sd->status.party_id)) ||
						 pd->instance_id !=
							 mapdata->instance_id))	 // Someone not in party is on instance map
						continue;
					instance_id = pd->instance_id;
					break;
				case IM_GUILD:
					if (!(gd = guild_search(sd->status.guild_id)) ||
						gd->instance_id !=
							mapdata->instance_id)  // Someone not in guild is on instance map
						continue;
					instance_id = gd->instance_id;
					break;
				case IM_CLAN:
					if (!(cd = clan_search(sd->status.clan_id)) ||
						cd->instance_id !=
							mapdata->instance_id)  // Someone not in clan is on instance map
						continue;
					instance_id = cd->instance_id;
					break;
				default:
					ShowError(
						"do_reload_instance: Unexpected instance mode for instance %s (id=%d, "
						"mode=%u).\n",
						(db) ? db->name.c_str() : "Unknown", mapdata->instance_id,
						(uint8)idata->mode);
					continue;
			}
			if (db &&
				instance_enter(sd, instance_id, db->name.c_str(), -1, -1) == IE_OK) {  // All good
				clif_displaymessage(sd->fd, msg_txt(sd, 515));	// Instance has been reloaded
				instance_reqinfo(sd, instance_id);
			} else	// Something went wrong
				ShowError(
					"do_reload_instance: Error setting character at instance start: "
					"character_id=%d instance=%s.\n",
...
void do_final_instance(void) {
	for (const auto &it : instances) instance_destroy(it.first);
}
