...
void trade_traderequest(struct map_session_data *sd, struct map_session_data *target_sd) {
	...
	if (sd->trade_partner !=
		0) {  // If a character tries to trade to another one then cancel the previous one
	...
	if (!pc_can_give_items(sd) ||
		!pc_can_give_items(target_sd)) {  // check if both GMs are allowed to trade
...
void trade_tradeack(struct map_session_data *sd, int type) {
	...
	if (sd->npc_id || sd->state.vending || sd->state.buyingstore || sd->state.storage_flag ||
		tsd->npc_id || tsd->state.vending || tsd->state.buyingstore ||
		tsd->state.storage_flag) {	// Fail
		clif_tradestart(sd, 2);
...
int impossible_trade_check(struct map_session_data *sd) {
	...
	for (i = 0; i < 10; i++) {
		...
		if (inventory[index].amount <
			sd->deal.item[i].amount) {	// if more than the player have -> hack
			sprintf(message_to_gm, msg_txt(sd, 538), sd->status.name,
					sd->status.account_id);	 // Hack on trade: character '%s' (account: %d) try to
											 // trade more items that he has.
			intif_wis_message_to_gm(wisp_server_name, PC_PERM_RECEIVE_HACK_INFO, message_to_gm);
			sprintf(message_to_gm, msg_txt(sd, 539), inventory[index].amount,
					inventory[index].nameid,
					sd->deal.item[i].amount);  // This player has %d of a kind of item (id: %u), and
											   // try to trade %d of them.
			intif_wis_message_to_gm(wisp_server_name, PC_PERM_RECEIVE_HACK_INFO, message_to_gm);
			// if we block people
			if (battle_config.ban_hack_trade < 0) {
				chrif_req_login_operation(-1, sd->status.name, CHRIF_OP_LOGIN_BLOCK, 0, 0,
										  0);  // type: 1 - block
				...
				chrif_req_login_operation(
					...
					0);			  // type: 2 - ban (year, month, day, hour, minute, second)
				set_eof(sd->fd);  // forced to disconnect because of the hack
				// message about the ban
				sprintf(message_to_gm, msg_txt(sd, 507),
						battle_config
							.ban_hack_trade);  //  This player has been banned for %d minute(s).
			} else
				// message about the ban
				strcpy(
					message_to_gm,
					msg_txt(sd, 508));	//  This player hasn't been banned (Ban option is disabled).
...
int trade_check(struct map_session_data *sd, struct map_session_data *tsd) {
	...
	// check zeny value against hackers (Zeny was already checked on time of adding, but you never
	// know when you lost some zeny since then.
			...
			if (itemdb_isstackable2(data)) {  // Stackable item.
				for (i = 0; i < MAX_INVENTORY; i++)
					if (inventory2[i].nameid == inventory[n].nameid &&
						inventory2[i].card[0] == inventory[n].card[0] &&
		...
		if (itemdb_isstackable2(data)) {
			for (i = 0; i < MAX_INVENTORY; i++)
				if (inventory[i].nameid == inventory2[n].nameid &&
					inventory[i].card[0] == inventory2[n].card[0] &&
					inventory[i].card[1] == inventory2[n].card[1] &&
					inventory[i].card[2] == inventory2[n].card[2] &&
...
void trade_tradeadditem(struct map_session_data *sd, short index, short amount) {
	...
	if (!amount) {	// Why do this.. ~.~ just send an ack, the item won't display on the trade
					// window.
	...
	if (!itemdb_cantrade(item, src_lv, dst_lv) &&  // Can't trade
		(pc_get_partner(sd) != target_sd ||
		 !itemdb_canpartnertrade(item, src_lv, dst_lv))) {	// Can't partner-trade
	...
	ARR_FIND(0, 10, trade_i,
			 sd->deal.item[trade_i].index == index || sd->deal.item[trade_i].amount == 0);
...
void trade_tradeaddzeny(struct map_session_data *sd, int amount) {
	...
	if (amount < 0 || amount > sd->status.zeny ||
		amount >
			MAX_ZENY -
				target_sd->status.zeny) {  // invalid values, no appropriate packet for it => abort
		trade_tradecancel(sd);
		return;
	}
...
void trade_tradecancel(struct map_session_data *sd) {
	...
	for (trade_i = 0; trade_i < 10; trade_i++) {  // give items back (only virtual)
		if (!target_sd->deal.item[trade_i].amount)
			continue;
		clif_additem(target_sd, target_sd->deal.item[trade_i].index,
...
void trade_tradecommit(struct map_session_data *sd) {
	...
	if (!sd->state.trading ||
		!sd->state.deal_locked)	 // Locked should be 1 (pressed ok) before you can press trade.
		return;
	...
	// Now is a good time (to save on resources) to check that the trade can indeed be made and it's
	// not exploitable. check exploit (trade more items that you have)
	...
	for (trade_i = 0; trade_i < 10; trade_i++) {
		...
		if (sd->deal.item[trade_i].amount) {
			...
			flag = pc_additem(tsd, &sd->inventory.u.items_inventory[n],
							  sd->deal.item[trade_i].amount, LOG_TYPE_TRADE);
			if (flag == 0)
		...
		if (tsd->deal.item[trade_i].amount) {
			...
			flag = pc_additem(sd, &tsd->inventory.u.items_inventory[n],
							  tsd->deal.item[trade_i].amount, LOG_TYPE_TRADE);
			if (flag == 0)
	...
	// save both player to avoid crash: they always have no advantage/disadvantage between the 2
	// players
