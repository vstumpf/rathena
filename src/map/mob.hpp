...
// The number of drops all mobs have and the max drop-slot that the steal skill will attempt to
// steal from.
...
// Min time before mobs do a check to call nearby friends for help (or for slaves to support their
// master)
...
enum e_random_monster_flags {
	...
	RMF_DB_RATE = 0x01,	 ///< Apply the summon success chance found in the list (otherwise get any
						 ///< monster from the db)
	RMF_CHECK_MOB_LV = 0x02,   ///< Apply a monster level check
	RMF_MOB_NOT_BOSS = 0x04,   ///< Selected monster should not be a Boss type (except those from
							   ///< MOBG_BLOODY_DEAD_BRANCH)
...
class MobDatabase : public TypesafeCachedYamlDatabase<uint32, s_mob_db> {
   private:
	bool parseDropNode(std::string nodeName, const ryml::NodeRef &node, uint8 max,
					   s_mob_drop *drops);
...
class MapDropDatabase : public TypesafeYamlDatabase<uint16, s_map_drops> {
   ...
   private:
	bool parseDrop(const ryml::NodeRef &node,
...
struct mob_data {
	...
	struct status_data status,
		*base_status;  // Second one is in case of leveling up mobs, or tiny/large mobs.
	struct status_change sc;
	std::shared_ptr<s_mob_db>
		db;	 // For quick data access (saves doing mob_db(md->mob_id) all the time) [Skotlex]
	...
	struct s_MobState {
		unsigned int aggressive : 1;  // Signals whether the mob AI is in aggressive mode or
									  // reactive mode. [Skotlex]
		...
		unsigned int boss : 1;
		unsigned int
			copy_master_mode : 1;  ///< Whether the spawned monster should copy the master's mode.
		enum MobSkillState skillstate;
		unsigned char steal_flag;  // number of steal tries (to prevent steal exploit on mobs with
								   // few items) [Lupus]
	...
	short min_chase;
	unsigned char walktoxy_fail_count;	// Pathfinding succeeds but the actual walking failed (e.g.
										// Icewall lock)
...
struct item_drop_list {
	...
	int first_charid, second_charid,
		third_charid;		 // charid's of players with higher pickup priority
