...
static DBMap* NPCMarketDB;	/// Stock persistency! Temporary market stocks from `market` table.
							/// struct s_npc_market, key: NPC exname
...
static void npc_parsename(struct npc_data* nd, const char* name, const char* start,
						  const char* buffer, const char* filepath);

const std::string StylistDatabase::getDefaultLocation() {
	return std::string(db_path) + "/stylist.yml";
}

bool StylistDatabase::parseCostNode(std::shared_ptr<s_stylist_entry> entry, bool doram,
	...
	if (this->nodeExists(node, "Price")) {
		...
		if (price > MAX_ZENY) {
			this->invalidWarning(
				node["Price"],
				"stylist_parseCostNode: Price %u is too high, capping to MAX_ZENY...\n", price);
			price = MAX_ZENY;
		}
	...
	if (this->nodeExists(node, "RequiredItem")) {
		...
		if (id == nullptr) {
			this->invalidWarning(node["RequiredItem"],
								 "stylist_parseCostNode: Unknown item \"%s\"...\n", item.c_str());
			return false;
		}
	...
	if (this->nodeExists(node, "RequiredItemBox")) {
		...
		if (id == nullptr) {
			this->invalidWarning(node["RequiredItemBox"],
								 "stylist_parseCostNode: Unknown item \"%s\"...\n", item.c_str());
			return false;
		}
...
uint64 StylistDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	switch (constant) {
		...
		case LOOK_BODY2:
			break;
		default:
			this->invalidWarning(node["Look"],
								 "stylist_parseBodyNode: Unsupported look value \"%s\"...\n",
								 look_str.c_str());
			return 0;
	}
	...
	for (const ryml::NodeRef& optionNode : node["Options"]) {
		...
		if (index == 0) {
			this->invalidWarning(optionNode["Index"],
								 "stylist_parseBodyNode: Unsupported index value \"%hd\"...\n",
								 index);
			return 0;
		}
		...
		if (this->nodeExists(optionNode, "Value")) {
			...
			switch (list->look) {
				...
				case LOOK_HEAD_BOTTOM: {
					...
					if (id == nullptr) {
						this->invalidWarning(optionNode["Value"],
											 "stylist_parseBodyNode: Unknown item \"%s\"...\n",
											 item.c_str());
						return 0;
					}
				...
				case LOOK_HAIR:
					...
					if (value < MIN_HAIR_STYLE) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: hair style \"%u\" is too low...\n", value);
						return 0;
					} else if (value > MAX_HAIR_STYLE) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: hair style \"%u\" is too high...\n", value);
						return 0;
					}
					break;
				case LOOK_HAIR_COLOR:
					...
					if (value < MIN_HAIR_COLOR) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: hair color \"%u\" is too low...\n", value);
						return 0;
					} else if (value > MAX_HAIR_COLOR) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: hair color \"%u\" is too high...\n", value);
						return 0;
					}
					break;
				case LOOK_CLOTHES_COLOR:
					...
					if (value < MIN_CLOTH_COLOR) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: cloth color \"%u\" is too low...\n", value);
						return 0;
					} else if (value > MAX_CLOTH_COLOR) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: cloth color \"%u\" is too high...\n", value);
						return 0;
					}
					break;
				case LOOK_BODY2:
					...
					if (value < MIN_BODY_STYLE) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: body style \"%u\" is too low...\n", value);
						return 0;
					} else if (value > MAX_BODY_STYLE) {
						this->invalidWarning(
							optionNode["Value"],
							"stylist_parseBodyNode: body style \"%u\" is too high...\n", value);
						return 0;
					}
					break;
			}
...
const std::string BarterDatabase::getDefaultLocation() { return "npc/barters.yml"; }
...
uint64 BarterDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Map")) {
		...
		if (index == 0) {
			this->invalidWarning(
				node["Map"], "barter_parseBodyNode: Unknown mapname %s, skipping.\n", map.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "X")) {
		...
		if (mapdata == nullptr) {
			this->invalidWarning(
				node["X"],
				"barter_parseBodyNode: Barter NPC is not on a map. Ignoring X coordinate...\n");
			x = 0;
		} else if (x >= mapdata->xs) {
			this->invalidWarning(node["X"],
								 "barter_parseBodyNode: X coordinate %hu is out of bound %hu...\n",
								 x, mapdata->xs);
			return 0;
		}
	...
	if (this->nodeExists(node, "Y")) {
		...
		if (mapdata == nullptr) {
			this->invalidWarning(
				node["Y"],
				"barter_parseBodyNode: Barter NPC is not on a map. Ignoring Y coordinate...\n");
			y = 0;
		} else if (y >= mapdata->ys) {
			this->invalidWarning(node["Y"],
								 "barter_parseBodyNode: Y coordinate %hu is out of bound %hu...\n",
								 y, mapdata->ys);
			return 0;
		}
	...
	if (this->nodeExists(node, "Direction")) {
		...
		if (!script_get_constant(("DIR_" + direction_name).c_str(), &constant)) {
			this->invalidWarning(node["Direction"],
								 "barter_parseBodyNode: Unknown direction %s, skipping.\n",
		...
		if (constant < DIR_NORTH || constant >= DIR_MAX) {
			this->invalidWarning(
				node["Direction"],
				"barter_parseBodyNode: Invalid direction %s, defaulting to North.\n",
				direction_name.c_str());
			constant = DIR_NORTH;
		}
	...
	if (this->nodeExists(node, "Sprite")) {
		...
		if (!script_get_constant(sprite_name.c_str(), &constant)) {
			this->invalidWarning(node["Sprite"],
								 "barter_parseBodyNode: Unknown sprite name %s, skipping.\n",
								 sprite_name.c_str());
			return 0;
		}
		...
		if (constant != JT_FAKENPC && !npcdb_checkid(constant)) {
			this->invalidWarning(node["Sprite"],
								 "barter_parseBodyNode: Invalid sprite name %s, skipping.\n",
								 sprite_name.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "Items")) {
		for (const ryml::NodeRef& itemNode : node["Items"]) {
			...
			if (this->nodeExists(itemNode, "Item")) {
				...
				if (id == nullptr) {
					this->invalidWarning(itemNode["Item"],
										 "barter_parseBodyNode: Unknown item %s.\n",
										 aegis_name.c_str());
					return 0;
				}
			...
			if (this->nodeExists(itemNode, "Zeny")) {
				...
				if (zeny > MAX_ZENY) {
					this->invalidWarning(
						itemNode["Zeny"],
						"barter_parseBodyNode: Zeny price %u is above MAX_ZENY (%u), capping...\n",
						zeny, MAX_ZENY);
					zeny = MAX_ZENY;
				}
			...
			if (this->nodeExists(itemNode, "RequiredItems")) {
				for (const ryml::NodeRef& requiredItemNode : itemNode["RequiredItems"]) {
					...
					if (requirement_index >= MAX_BARTER_REQUIREMENTS) {
						this->invalidWarning(requiredItemNode["Index"],
											 "barter_parseBodyNode: Index %hu is out of bounds. "
											 "Barters support up to %d requirements.\n",
					...
					if (this->nodeExists(requiredItemNode, "Item")) {
						...
						std::shared_ptr<item_data> data =
							item_db.search_aegisname(aegis_name.c_str());

						if (data == nullptr) {
							this->invalidWarning(
								requiredItemNode["Item"],
								"barter_parseBodyNode: Unknown required item %s.\n",
								aegis_name.c_str());
							return 0;
						}
					...
					if (this->nodeExists(requiredItemNode, "Refine")) {
						...
						if (data->flag.no_refine) {
							this->invalidWarning(
								requiredItemNode["Refine"],
								"barter_parseBodyNode: Item %s is not refineable.\n",
								data->name.c_str());
							return 0;
						}
		...
		for (const auto& itemPair : barter->items) {
			if (itemPair.second->stockLimited) {
				if (Sql_Query(mmysql_handle,
							  "SELECT `amount` FROM `%s` WHERE `name` = '%s' AND `index` = '%hu'",
							  barter_table, barter->name.c_str(), itemPair.first) != SQL_SUCCESS) {
				...
				if (Sql_Query(
						...
						itemPair.second->stock) != SQL_SUCCESS) {
					Sql_ShowDebug(mmysql_handle);
				}
			} else {
				if (Sql_Query(mmysql_handle,
							  "DELETE FROM `%s` WHERE `name` = '%s' AND `index` = '%hu'",
							  barter_table, barter->name.c_str(), itemPair.first) != SQL_SUCCESS) {
					Sql_ShowDebug(mmysql_handle);
				}
			}
...
bool npc_isnear(struct block_list* bl) {
	if (battle_config.min_npc_vendchat_distance > 0 &&
		map_foreachinallrange(npc_isnear_sub, bl, battle_config.min_npc_vendchat_distance, BL_NPC,
							  0))
		return true;
...
bool npc_enable_target(npc_data& nd, uint32 char_id, e_npcv_status flag) {
	if (char_id > 0 && (flag & NPCVIEW_CLOAK)) {
		...
		if (flag & NPCVIEW_ENABLE) {
			...
			nd.sc.option |= OPTION_CLOAK;
		else {	// Can't change the view_data to invisible class because the view_data for all npcs
				// is shared! [Skotlex]
			nd.sc.option |= OPTION_INVISIBLE;
		...
		if (nd.class_ != JT_WARPNPC &&
			nd.class_ !=
				JT_GUILD_FLAG)	// Client won't display option changes for these classes [Toms]
		...
		map_foreachinmap(npc_cloaked_sub, nd.bl.m, BL_PC,
						 nd.bl.id);	 // Because npc option has been updated we remove the npc id
									 // from sd->cloaked_npc
	}
	...
	if (flag & NPCVIEW_VISIBLE) {  // check if player standing on a OnTouchArea
		...
		if (xs > -1 && ys > -1)
			map_foreachinallarea(npc_enable_sub, nd.bl.m, nd.bl.x - xs, nd.bl.y - ys, nd.bl.x + xs,
								 nd.bl.y + ys, BL_PC, &nd);
	}
...
TIMER_FUNC(npc_secure_timeout_timer) {
	...
	if ((sd = map_id2sd(id)) == NULL || !sd->npc_id || sd->state.ignoretimeout) {
		...
		return 0;  // Not logged in anymore OR no longer attached to a NPC OR using 'ignoretimeout'
				   // script command
	}
	...
	if (DIFF_TICK(cur_tick, sd->npc_idle_tick) > (timeout * 1000)) {
		...
		sd->npc_idle_timer = add_timer(cur_tick + (SECURE_NPCTIMEOUT_INTERVAL * 1000),
...
int npc_event_dequeue(struct map_session_data* sd, bool free_script_stack) {
	...
	if (!pc_addeventtimer(sd, 100, sd->eventqueue[0])) {  // Failed to dequeue, couldn't set a
														  // timer.
...
static int npc_event_export(struct npc_data* nd, int i) {
	...
	if ((lname[0] == 'O' || lname[0] == 'o') && (lname[1] == 'N' || lname[1] == 'n')) {
		...
		if (nd->bl.m > -1 &&
			map_getmapdata(nd->bl.m)->instance_id > 0) {  // Block script events in instances
			int j;

			for (j = 0; j < NPCE_MAX; j++) {
				if (strcmpi(npc_get_script_event_name(j), lname) == 0) {
					ShowWarning(
						"npc_event_export: attempting to duplicate a script event in an instance "
						"(%s::%s), ignoring\n",
						nd->name, lname);
					return 0;
				}
		...
		if (!strncasecmp(lname, script_config.onwhisper_event_name, NAME_LENGTH) &&
			...
			// The client only allows that many character so that NPC could not be whispered by
			// unmodified clients
			ShowWarning("Whisper event in npc '" CL_WHITE "%s" CL_RESET
						"' was ignored, because it's name is too long.\n",
						nd->exname);
			return 0;
		}
...
int npc_event_sub(struct map_session_data* sd, struct event_data* ev,
				  const char* eventname);  //[Lance]
...
int npc_event_doall_sub(DBKey key, DBData* data, va_list ap) {
	...
	if (p &&
		strcmpi(name, p) == 0 /* && !ev->nd->src_id */)	 // Do not run on duplicates. [Paradox924X]
	{
...
int npc_event_do(const char* name) { return npc_event_do_id(name, 0); }

// runs the specified event (global only)
int npc_event_doall(const char* name) { return npc_event_doall_id(name, 0); }

// runs the specified event(global only) and reports call count
void npc_event_runall(const char* eventname) {
	ShowStatus("Event '" CL_WHITE "%s" CL_RESET "' executed with '" CL_WHITE "%d" CL_RESET
			   "' NPCs.\n",
...
TIMER_FUNC(npc_event_do_clock) {
	...
	if (t->tm_min != ev_tm_b.tm_min) {
		...
		safesnprintf(buf, EVENT_NAME_LENGTH, "%s%02d", script_config.timer_minute_event_name,
					 t->tm_min);
		c += npc_event_doall(buf);

		safesnprintf(buf, EVENT_NAME_LENGTH, "%s%02d%02d", script_config.timer_clock_event_name,
					 t->tm_hour, t->tm_min);
		c += npc_event_doall(buf);
	...
	if (t->tm_hour != ev_tm_b.tm_hour) {
		safesnprintf(buf, EVENT_NAME_LENGTH, "%s%02d", script_config.timer_hour_event_name,
					 t->tm_hour);
		c += npc_event_doall(buf);
	}

	if (t->tm_mday != ev_tm_b.tm_mday) {
		safesnprintf(buf, EVENT_NAME_LENGTH, "%s%02d%02d", script_config.timer_day_event_name,
...
int npc_timerevent_export(struct npc_data* nd, int i) {
	...
	// Check if the label name starts with OnTimer(default) and then parse the seconds right after
	// it
	if (!strncmp(lname, script_config.timer_event_name, len) &&
		sscanf((lname += len), "%11d%n", &t, &k) == 1 && lname[k] == '\0') {
		// Timer event
		...
		if (te == NULL)
			...
			te = (struct npc_timerevent_list*)aRealloc(
				te, sizeof(struct npc_timerevent_list) * (k2 + 1));
		for (j = 0; j < k2; j++) {
			if (te[j].timer > t) {
...
struct timer_event_data {
	...
	int time;  // holds total time elapsed for the script from when timer was started to when last
			   // time the event triggered.
};
...
int npc_timerevent_start(struct npc_data* nd, int rid) {
	...
	if (nd->u.scr.rid > 0 &&
		!(sd = map_id2sd(nd->u.scr.rid))) {	 // Failed to attach timer to this player.
...
int npc_timerevent_stop(struct npc_data* nd) {
	...
	if (!sd && nd->u.scr.timertick) {
		nd->u.scr.timer +=
			DIFF_TICK(gettick(), nd->u.scr.timertick);	// Set 'timer' to the time that has passed
														// since the beginning of the timers
...
void npc_timerevent_quit(struct map_session_data* sd) {
	...
	if (nd && nd->bl.type == BL_NPC) {
		...
		if (ev && ev->nd != nd) {
			ShowWarning(
				"npc_timerevent_quit: Unable to execute \"%s\", two NPCs have the same event name "
				"[%s]!\n",
		...
		if (ev) {
			...
			old_rid = (nd->u.scr.rid == sd->bl.id
						   ? 0
						   : nd->u.scr.rid);  // Detach rid if the last attached player logged off.
			old_tick = nd->u.scr.timertick;
			old_timer = nd->u.scr.timer;
...
t_tick npc_gettimerevent_tick(struct npc_data* nd) {
	...
	// TODO: Get player attached timer's tick. Now we can just get it by using 'getnpctimer' inside
	// OnTimer event.
...
int npc_event_sub(struct map_session_data* sd, struct event_data* ev, const char* eventname) {
	...
	safesnprintf(ontouch_event_name, ARRAYLENGTH(ontouch_event_name), "%s::%s", ev->nd->exname,
				 script_config.ontouch_event_name);
	safesnprintf(ontouch2_event_name, ARRAYLENGTH(ontouch2_event_name), "%s::%s", ev->nd->exname,
	...
	if (strcmp(eventname, ontouch_event_name) == 0 || strcmp(eventname, ontouch2_event_name) == 0) {
		...
		if (x > 0 && y > 0 && (xs > -1 && ys > -1) &&
				((sd->bl.x < x - xs) || (sd->bl.x > x + xs) || (sd->bl.y < y - ys) ||
				 (sd->bl.y > y + ys)) ||
...
int npc_event(struct map_session_data* sd, const char* eventname, int ontouch) {
	...
	npc_event_sub(sd, ev, eventname);  // Don't return this value so npc_enable_sub doesn't attempt
									   // to "click" the NPC if OnTouch fails.
	return 0;
}
...
int npc_touchnext_areanpc(struct map_session_data* sd, bool leavemap) {
	...
	sd->npc_ontouch_.erase(
		std::remove_if(sd->npc_ontouch_.begin(), sd->npc_ontouch_.end(),
					   [&](const int& current_npc_id) {
						   ...
						   if (!nd) {
							   ...
							   if (sd->bl.m != nd->bl.m || sd->bl.x < nd->bl.x - xs ||
								   sd->bl.x > nd->bl.x + xs || sd->bl.y < nd->bl.y - ys ||
								   sd->bl.y > nd->bl.y + ys || leavemap) {
								   char name[EVENT_NAME_LENGTH];

								   if (nd->touching_id &&
									   nd->touching_id == sd->bl.id) {	// empty when reload script
									   found = true;
									   nd->touching_id = 0;
									   safesnprintf(name, ARRAYLENGTH(name), "%s::%s", nd->exname,
													script_config.ontouch_event_name);
									   map_forcountinarea(npc_touch_areanpc_sub, nd->bl.m,
														  nd->bl.x - xs, nd->bl.y - ys,
														  nd->bl.x + xs, nd->bl.y + ys, 1, BL_PC,
														  sd->bl.id, name);
								   }
...
int npc_touch_areanpc(struct map_session_data* sd, int16 m, int16 x, int16 y, struct npc_data* nd) {
	...
	switch (nd->subtype) {
		case NPCTYPE_WARP:
			if ((!nd->trigger_on_hidden &&
				 (pc_ishiding(sd) || (sd->sc.count && sd->sc.data[SC_CAMOUFLAGE]))) ||
				pc_isdead(sd))
				break;	// hidden or dead chars cannot use warps
			if (!pc_job_can_entermap((enum e_job)sd->status.class_,
									 map_mapindex2mapid(nd->u.warp.mapindex),
									 pc_get_group_level(sd)))
				break;
			if (sd->count_rewarp > 10) {
				ShowWarning(
					"Prevented infinite warp loop for player (%d:%d). Please fix NPC: '%s', path: "
					"'%s'\n",
		...
		case NPCTYPE_SCRIPT:
			...
			if (npc_ontouch_event(sd, nd) > 0 &&
				npc_ontouch2_event(sd, nd) >
					0) {  // failed to run OnTouch event, so just click the npc
				if (!util::vector_exists(sd->areanpc, nd->bl.id))
					sd->areanpc.push_back(nd->bl.id);
				if (sd->npc_id == 0)
					npc_click(sd, nd);
...
int npc_touch_area_allnpc(struct map_session_data* sd, int16 m, int16 x, int16 y) {
	...
	if (f == 1) {
		ShowError(
			"npc_touch_area_allnpc : stray NPC cell/NPC not found in the block on coordinates "
			"'%s',%d,%d\n",
			mapdata->name, x, y);
		return 1;
	}
		...
		if (x >= mapdata->npc[i]->bl.x - xs && x <= mapdata->npc[i]->bl.x + xs &&
			...
			switch (mapdata->npc[i]->subtype) {
				case NPCTYPE_WARP: {
					...
					if (unit_warp(&md->bl, warp_m, mapdata->npc[i]->u.warp.x,
								  mapdata->npc[i]->u.warp.y, CLR_OUTSIGHT) == 0)
						return 1;  // Warped
				} break;
				case NPCTYPE_SCRIPT:
					if (mapdata->npc[i]->bl.id == md->areanpc_id)
						break;	// Already touch this NPC
					safesnprintf(eventname, ARRAYLENGTH(eventname), "%s::%s",
								 mapdata->npc[i]->exname, script_config.ontouchnpc_event_name);
					if ((ev = (struct event_data*)strdb_get(ev_db, eventname)) == NULL ||
						ev->nd == NULL)
...
void run_tomb(struct map_session_data* sd, struct npc_data* nd) {
	...
	snprintf(buffer, sizeof(buffer), msg_txt(sd, 657),
			 nd->u.tomb.md->db->name.c_str());	// [ ^EE0000%s^000000 ]
	...
	snprintf(buffer, sizeof(buffer), msg_txt(sd, 661),
			 nd->u.tomb.killer_name[0] ? nd->u.tomb.killer_name : "Unknown");  // [^EE0000%s^000000]
	clif_scriptmes(*sd, nd->bl.id, buffer);
 ...
 **/
static enum e_CASHSHOP_ACK npc_cashshop_process_payment(struct npc_data* nd, int price, int points,
														struct map_session_data* sd) {
	int cost[2] = {0, 0};
	...
	switch (nd->subtype) {
		...
		case NPCTYPE_ITEMSHOP: {
			...
			if (!id) {	// Item Data is checked at script parsing but in case of item_db reload,
						// check again.
				ShowWarning("Failed to find sellitem %u for itemshop NPC '%s' (%s, %d, %d)!\n",
							nd->u.shop.itemshop_nameid, nd->exname, map_mapid2mapname(nd->bl.m),
							nd->bl.x, nd->bl.y);
			...
			if (cost[1] < points || cost[0] < (price - points)) {
				...
				sprintf(output, msg_txt(sd, 712), id->ename.c_str(),
						id->nameid);  // You do not have enough %s (%u).
			...
			for (i = 0; i < MAX_INVENTORY && delete_amount > 0; i++) {
				...
				if (sd->inventory.u.items_inventory[i].nameid == 0 ||
					sd->inventory_data[i] == NULL || !(it = &sd->inventory.u.items_inventory[i]) ||
					it->nameid != nd->u.shop.itemshop_nameid)
					continue;
				...
				if (pc_delitem(sd, i, amount, 0, 0, LOG_TYPE_NPC)) {
					ShowWarning(
						...
						nd->u.shop.itemshop_nameid, sd->status.name, nd->exname,
			...
			if (delete_amount > 0) {
				ShowError(
					"Item %u is not enough as payment at itemshop NPC '%s' (%s, %d, %d, AID=%d, "
					"CID=%d)!\n",
					nd->u.shop.itemshop_nameid, nd->exname, map_mapid2mapname(nd->bl.m), nd->bl.x,
					nd->bl.y, sd->status.account_id, sd->status.char_id);
				return ERROR_TYPE_PURCHASE_FAIL;
			}
		} break;
		case NPCTYPE_POINTSHOP: {
			...
			if (cost[1] < points || cost[0] < (price - points)) {
				sprintf(output, msg_txt(sd, 713),
						nd->u.shop.pointshop_str);	// You do not have enough '%s'.
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], output, false, SELF);
				return ERROR_TYPE_PURCHASE_FAIL;
			}
			pc_setreg2(sd, nd->u.shop.pointshop_str, cost[0] - (price - points));
			sprintf(output, msg_txt(sd, 716), nd->u.shop.pointshop_str,
					cost[0] - (price - points));  // Your '%s' is now: %d
 ...
 */
int npc_cashshop_buylist(struct map_session_data* sd, int points,
	...
	if (!nd || (nd->subtype != NPCTYPE_CASHSHOP && nd->subtype != NPCTYPE_ITEMSHOP &&
	...
	for (i = 0; i < item_list.size(); i++) {
		...
		ARR_FIND(0, nd->u.shop.count, j,
				 nd->u.shop.shop_item[j].nameid == nameid ||
					 itemdb_viewid(nd->u.shop.shop_item[j].nameid) == nameid);
		...
		if (nd->master_nd) {  // Script-controlled shops decide by themselves, what can be bought
							  // and for what price.
			continue;
		}
	...
	for (i = 0; i < item_list.size(); i++) {
		...
		if (!pet_create_egg(sd, nameid)) {
			...
			for (j = 0; j < amount; j += get_amt) pc_additem(sd, &item_tmp, get_amt, LOG_TYPE_NPC);
		}
	}
 ...
 */
void npc_shop_currency_type(struct map_session_data* sd, struct npc_data* nd, int cost[2],
							bool display) {
	nullpo_retv(sd);
	...
	switch (nd->subtype) {
		...
		case NPCTYPE_ITEMSHOP: {
			...
			if (id) {  // Item Data is checked at script parsing but in case of item_db reload,
				...
				if (display) {
					...
					sprintf(output, msg_txt(sd, 714), id->ename.c_str(),
							id->nameid);  // Item Shop List: %s (%u)
		...
		case NPCTYPE_POINTSHOP:
			if (display) {
				...
				sprintf(output, msg_txt(sd, 715),
						nd->u.shop.pointshop_str);	// Point Shop List: '%s'
				clif_broadcast(&sd->bl, output, strlen(output) + 1, BC_BLUE, SELF);
			}
...
int npc_cashshop_buy(struct map_session_data* sd, t_itemid nameid, int amount, int points) {
	...
	ARR_FIND(0, nd->u.shop.count, i,
			 nd->u.shop.shop_item[i].nameid == nameid ||
				 itemdb_viewid(nd->u.shop.shop_item[i].nameid) == nameid);
	if (i == nd->u.shop.count)
		return ERROR_TYPE_ITEM_ID;
	...
	if ((double)nd->u.shop.shop_item[i].value * amount > INT_MAX) {
		...
		ShowDebug("(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%d, amount:%d)\n", nd->exname,
				  map_mapid2mapname(nd->bl.m), nd->bl.x, nd->bl.y, sd->status.name,
				  sd->status.account_id, sd->status.char_id, nd->u.shop.shop_item[i].value, amount);
		return ERROR_TYPE_ITEM_ID;
	}
	...
	if (!pet_create_egg(sd, nameid)) {
		...
		for (int j = 0; j < amount; j += get_amt) pc_additem(sd, &item_tmp, get_amt, LOG_TYPE_NPC);
	}
 ...
 */
static int npc_buylist_sub(struct map_session_data* sd, std::vector<s_npc_buy_list>& item_list,
						   struct npc_data* nd) {
...
e_purchase_result npc_buylist(struct map_session_data* sd, std::vector<s_npc_buy_list>& item_list) {
	...
	for (int i = 0; i < item_list.size(); ++i) {
		...
		if (!itemdb_isstackable2(id.get()) &&
			amount > 1) {  // Exploit? You can't buy more than 1 of equipment types o.O
			ShowWarning(
				"Player %s (%d:%d) sent a hexed packet trying to buy %d of nonstackable item %u!\n",
				sd->status.name, sd->status.account_id, sd->status.char_id, amount, nameid);
			amount = item_list[i].qty = 1;
...
static int npc_selllist_sub(struct map_session_data* sd, int list_length,
							PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list, struct npc_data* nd) {
	char npc_ev[EVENT_NAME_LENGTH];
	...
	int key_option_id[MAX_ITEM_RDM_OPT], key_option_val[MAX_ITEM_RDM_OPT],
		key_option_param[MAX_ITEM_RDM_OPT];
	...
	for (i = 0; i < list_length; i++) {
		...
		script_setarray_pc(sd, "@sold_nameid", i, sd->inventory.u.items_inventory[idx].nameid,
						   &key_nameid);
		script_setarray_pc(sd, "@sold_quantity", i, item_list[i].amount, &key_amount);

		if (itemdb_isequip(sd->inventory.u.items_inventory[idx]
							   .nameid)) {	// process equipment based information into the arrays
			script_setarray_pc(sd, "@sold_refine", i, sd->inventory.u.items_inventory[idx].refine,
							   &key_refine);
			script_setarray_pc(sd, "@sold_attribute", i,
							   sd->inventory.u.items_inventory[idx].attribute, &key_attribute);
			script_setarray_pc(sd, "@sold_identify", i,
							   sd->inventory.u.items_inventory[idx].identify, &key_identify);
			script_setarray_pc(sd, "@sold_enchantgrade", i,
							   sd->inventory.u.items_inventory[idx].enchantgrade,
							   &key_enchantgrade);

			for (j = 0; j < MAX_SLOTS;
				 j++) {	 // store each of the cards from the equipment in the array
				snprintf(card_slot, sizeof(card_slot), "@sold_card%d", j + 1);
				script_setarray_pc(sd, card_slot, i, sd->inventory.u.items_inventory[idx].card[j],
								   &key_card[j]);
			}

			for (j = 0; j < MAX_ITEM_RDM_OPT; j++) {  // Store each of the item options in the array
				snprintf(option_id, sizeof(option_id), "@sold_option_id%d", j + 1);
				script_setarray_pc(sd, option_id, i,
								   sd->inventory.u.items_inventory[idx].option[j].id,
								   &key_option_id[j]);
				snprintf(option_val, sizeof(option_val), "@sold_option_val%d", j + 1);
				script_setarray_pc(sd, option_val, i,
								   sd->inventory.u.items_inventory[idx].option[j].value,
								   &key_option_val[j]);
				snprintf(option_param, sizeof(option_param), "@sold_option_param%d", j + 1);
				script_setarray_pc(sd, option_param, i,
								   sd->inventory.u.items_inventory[idx].option[j].param,
								   &key_option_param[j]);
			}
		}
	}
...
uint8 npc_selllist(struct map_session_data* sd, int list_length,
				   PACKET_CZ_PC_SELL_ITEMLIST_sub* item_list) {
	double z;
	int i, skill;
	struct npc_data* nd;
	...
	for (i = 0; i < list_length; i++) {
		...
		if (!nameid || !sd->inventory_data[idx] ||
			sd->inventory.u.items_inventory[idx].amount < amount) {
			return 1;
		}

		if (nd->master_nd) {  // Script-controlled shops decide by themselves, what can be sold and
							  // at what price.
			continue;
		}
	...
	for (i = 0; i < list_length; i++) {
		...
		if (sd->inventory_data[idx]->type == IT_PETEGG &&
			sd->inventory.u.items_inventory[idx].card[0] == CARD0_PET) {
...
e_purchase_result npc_barter_purchase(struct map_session_data& sd,
				...
				for (j = 0; j < MAX_INVENTORY; j++) {
					if (sd.inventory.u.items_inventory[j].nameid == requirement->nameid) {
						...
						if (battle_config.hide_fav_sell &&
							sd.inventory.u.items_inventory[j].favorite) {
							continue;
						}
				...
				for (int i = 0; i < (requirement->amount * amount); i++) {
					...
					for (j = 0; j < MAX_INVENTORY; j++) {
						if (sd.inventory.u.items_inventory[j].nameid == requirement->nameid) {
							...
							// Found a match, since it is not stackable, check if it was already
							// taken
					...
					if (j == MAX_INVENTORY) {
						...
						if (requirement->refine >= 0) {
							...
							// Try to find a higher refine level, going from the next lowest to the
							// highest possible
							for (refine = requirement->refine + 1; refine <= MAX_REFINE; refine++) {
								for (j = 0; j < MAX_INVENTORY; j++) {
									if (sd.inventory.u.items_inventory[j].nameid ==
										requirement->nameid) {
										// Equipped items are not taken into account
										if (sd.inventory.u.items_inventory[j].equip != 0) {
											continue;
										...
										// Found a match, since it is not stackable, check if it was
										// already taken
										if (requiredItems[j] > 0) {
											// Item was already taken, try to find another match
											continue;
	...
	for (s_barter_purchase& purchase : purchases) {
		if (purchase.item->stockLimited) {
			...
			if (Sql_Query(
					mmysql_handle,
					"REPLACE INTO `%s` (`name`,`index`,`amount`) VALUES ( '%s', '%hu', '%hu' )",
					barter_table, barter->name.c_str(), purchase.item->index,
					purchase.item->stock) != SQL_SUCCESS) {
				Sql_ShowDebug(mmysql_handle);
				return e_purchase_result::PURCHASE_FAIL_EXCHANGE_FAILED;
			}
		}
...
void npc_unload_duplicates(struct npc_data* nd) { map_foreachnpc(npc_unload_dup_sub, nd->bl.id); }
...
int npc_unload(struct npc_data* nd, bool single) {
	...
	if ((nd->subtype == NPCTYPE_SHOP || nd->subtype == NPCTYPE_CASHSHOP ||
...
static void npc_parsename(struct npc_data* nd, const char* name, const char* start,
	...
	if (p) {  // <Display name>::<Unique name>
		...
		if (len > NPC_NAME_LENGTH) {
			ShowWarning(
				"npc_parsename: Display name of '%s' is too long (len=%u) in file '%s', line'%d'. "
				"Truncating to %u characters.\n",
				name, (unsigned int)len, filepath, strline(buffer, start - buffer),
				NPC_NAME_LENGTH);
		...
		if (len > NPC_NAME_LENGTH)
			ShowWarning(
				"npc_parsename: Unique name of '%s' is too long (len=%u) in file '%s', line'%d'. "
				"Truncating to %u characters.\n",
				name, (unsigned int)len, filepath, strline(buffer, start - buffer),
				NPC_NAME_LENGTH);
		...
		if (len > NPC_NAME_LENGTH)
			ShowWarning(
				"npc_parsename: Name '%s' is too long (len=%u) in file '%s', line'%d'. Truncating "
				"to %u characters.\n",
				name, (unsigned int)len, filepath, strline(buffer, start - buffer),
				NPC_NAME_LENGTH);
		safestrncpy(nd->name, name, sizeof(nd->name));
	...
	if ((dnd = npc_name2id(nd->exname)) != NULL) {	// duplicate unique name, generate new one
		...
		strcpy(this_mapname, (nd->bl.m == -1 ? "(not on a map)"
											 : mapindex_id2name(map_getmapdata(nd->bl.m)->index)));
		strcpy(other_mapname,
			   (dnd->bl.m == -1 ? "(not on a map)"
								: mapindex_id2name(map_getmapdata(dnd->bl.m)->index)));
	...
	if (pid != nullptr && *pid != '\0') {
		...
		if (!script_get_constant(viewid, &val_tmp)) {
			...
			if (mob != nullptr)
				...
				ShowWarning(
					"npc_parseview: Invalid NPC constant '%s' specified in file '%s', line'%d'. "
 ...
 */
struct npc_data* npc_add_warp(char* name, short from_mapid, short from_x, short from_y, short xs,
							  short ys, unsigned short to_mapindex, short to_x, short to_y) {
	int i, flag = 0;
	struct npc_data* nd;
	...
	for (i = 0; npc_name2id(nd->exname) != NULL; ++i)
		snprintf(nd->exname, ARRAYLENGTH(nd->exname), "warp%d_%d_%d_%d", i, from_mapid, from_x,
				 from_y);
...
/**
 * Parses a warp npc.
 * Line definition <from mapname>,<fromX>,<fromY>,<facing>%TAB%warp%TAB%<warp
 ...
 */
static const char* npc_parse_warp(char* w1, char* w2, char* w3, char* w4, const char* start,
								  const char* buffer, const char* filepath) {
	int m;
	...
	if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
		ShowWarning(
			"npc_parse_warp: coordinates %d/%d are out of bounds in map %s(%dx%d), in file '%s', "
			"line '%d'\n",
 ...
 * Line definition :
 * <map name>,<x>,<y>,<facing>%TAB%shop%TAB%<NPC Name>%TAB%<sprite
 * id>,<itemid>:<price>{,<itemid>:<price>...} <map name>,<x>,<y>,<facing>%TAB%cashshop%TAB%<NPC
 * Name>%TAB%<sprite id>,<itemid>:<price>{,<itemid>:<price>...} <map
 * name>,<x>,<y>,<facing>%TAB%itemshop%TAB%<NPC Name>%TAB%<sprite
 ...
 * @param w2 : word 2 before tab (shop|cashshop|itemshop|pointshop|marketshop), keyword that sent us
 * in this parsing
 ...
 * @return new index for next parsing
 */
static const char* npc_parse_shop(char* w1, char* w2, char* w3, char* w4, const char* start,
								  const char* buffer, const char* filepath) {
	char *p, point_str[32];
	int m, is_discount = 0;
	uint16 dir;
	...
	if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
		ShowWarning(
			"npc_parse_shop: coordinates %d/%d are out of bounds in map %s(%dx%d), in file '%s', "
			"line '%d'\n",
			x, y, mapdata->name, mapdata->xs, mapdata->ys, filepath,
			strline(buffer, start - buffer));
	}
	...
	switch (type) {
		case NPCTYPE_ITEMSHOP: {
			if (sscanf(p, ",%u:%11d,", &nameid, &is_discount) < 1) {
				ShowError(
					"npc_parse_shop: Invalid item cost definition in file '%s', line '%d'. "
		...
		case NPCTYPE_POINTSHOP: {
			...
			switch (point_str[0]) {
				...
				case '\'':
					ShowWarning(
						"npc_parse_shop: Invalid item cost variable type (must be permanent "
						"character or account based) in file '%s', line '%d'. Ignoring the rest of "
						"the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
						filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
					return strchr(start, '\n');	 // skip and continue
					break;
			...
			if (point_str[strlen(point_str) - 1] == '$') {
				ShowWarning(
					"npc_parse_shop: Invalid item cost variable type (must be integer) in file "
					"'%s', line '%d'. Ignoring the rest of the line...\n * w1=%s\n * w2=%s\n * "
					"w3=%s\n * w4=%s\n",
					filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
				return strchr(start, '\n');	 // skip and continue
			}
...
#if PACKETVER < 20131223
			ShowError(
				"npc_parse_shop: (MARKETSHOP) Feature is disabled, need client 20131223 or newer. "
				"Ignoring file '%s', line '%d\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
			return strchr(start, '\n');	 // skip and continue
#else
			is_discount = 0;
	...
	while (p) {
		...
		switch (type) {
			case NPCTYPE_MARKETSHOP:
				...
				if (sscanf(p, ",%u:%11d:%11d", &nameid2, &value, &qty) != 3) {
					ShowError(
						"npc_parse_shop: (MARKETSHOP) Invalid item definition in file '%s', line "
						"'%d'. Ignoring the rest of the line...\n * w1=%s\n * w2=%s\n * w3=%s\n * "
						"w4=%s\n",
						filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
					skip = true;
				}
			...
			default:
				if (sscanf(p, ",%u:%11d", &nameid2, &value) != 2) {
					ShowError(
						"npc_parse_shop: Invalid item definition in file '%s', line '%d'. Ignoring "
		...
		if (value == 0 &&
			(type == NPCTYPE_SHOP || type == NPCTYPE_MARKETSHOP)) {	 // NPC selling items for free!
			ShowWarning(
		...
		if (type == NPCTYPE_SHOP &&
			value * 0.75 <
				id->value_sell * 1.24) {  // Exploit possible: you can buy and sell back with profit
			ShowWarning(
				"npc_parse_shop: Item %s [%u] discounted buying price (%d->%d) is less than "
				"overcharged selling price (%d->%d) at file '%s', line '%d'.\n",
				id->name.c_str(), nameid2, value, (int)(value * 0.75), id->value_sell,
				(int)(id->value_sell * 1.24), filepath, strline(buffer, start - buffer));
		}
		if (type == NPCTYPE_MARKETSHOP && qty < -1) {
			ShowWarning(
				"npc_parse_shop: Item %s [%u] is stocked with invalid value %hd, changed to "
				"unlimited (-1). File '%s', line '%d'.\n",
...
int npc_convertlabel_db(DBKey key, DBData* data, va_list ap) {
	...
	if (*label_list == NULL) {
		...
		*label_list = (struct npc_label_list*)aRealloc(
			*label_list, sizeof(struct npc_label_list) * (*label_list_num + 1));
...
static const char* npc_skip_script(const char* start, const char* buffer, const char* filepath) {
	...
	p = strchr(start, '{');
	if (p == NULL) {
		ShowError("npc_skip_script: Missing left curly in file '%s', line'%d'.", filepath,
				  strline(buffer, start - buffer));
 ...
 * <map name>,<x>,<y>,<facing>%TAB%script%TAB%<NPC Name>%TAB%<sprite
 * id>,<triggerX>,<triggerY>,{<code>} * @TODO missing cashshop line definition
 * @param w1 : word 1 before tab (<from map name>,<x>,<y>,<facing>)
 ...
 * @return new index for next parsing
 */
static const char* npc_parse_script(char* w1, char* w2, char* w3, char* w4, const char* start,
									const char* buffer, const char* filepath) {
	int16 dir = 0;
	...
	if (strcmp(w1, "-") == 0) {	 // floating npc
		...
		if (sscanf(w1, "%15[^,],%6hd,%6hd,%4hd", mapname, &x, &y, &dir) != 4) {
			ShowError(
				"npc_parse_script: Invalid placement format for a script in file '%s', line '%d'. "
				"Skipping the rest of file...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
	...
	if (strstr(w4, ",{") == NULL || script_start == NULL || (end != NULL && script_start > end)) {
		ShowError(
			"npc_parse_script: Missing left curly ',{' in file '%s', line '%d'. Skipping the rest "
			"of the file.\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
		return NULL;  // can't continue
	}
	...
	script = parse_script(script_start, filepath, strline(buffer, script_start - buffer),
						  SCRIPT_USE_LABEL_DB);
	label_list = NULL;
	label_list_num = 0;
	if (script) {
...
/// warp: <map name>,<x>,<y>,<facing>%TAB%duplicate(<name of target>)%TAB%<NPC
/// Name>%TAB%<spanx>,<spany> shop/cashshop/npc: -%TAB%duplicate(<name of target>)%TAB%<NPC
/// Name>%TAB%<sprite id> shop/cashshop/npc: <map name>,<x>,<y>,<facing>%TAB%duplicate(<name of
/// target>)%TAB%<NPC Name>%TAB%<sprite id> npc: -%TAB%duplicate(<name of target>)%TAB%<NPC
/// Name>%TAB%<sprite id>,<triggerX>,<triggerY> npc: <map name>,<x>,<y>,<facing>%TAB%duplicate(<name
/// of target>)%TAB%<NPC Name>%TAB%<sprite id>,<triggerX>,<triggerY>
const char* npc_parse_duplicate(char* w1, char* w2, char* w3, char* w4, const char* start,
								const char* buffer, const char* filepath,
								struct map_session_data* owner = nullptr) {
	...
	if (w2[length - 1] != ')' || length <= 11 ||
		length - 11 >=
			sizeof(srcname)) {	// does not match 'duplicate(%127s)', name is empty or too long
	...
	// get placement
	if ((type == NPCTYPE_SHOP || type == NPCTYPE_CASHSHOP || type == NPCTYPE_ITEMSHOP ||
		 type == NPCTYPE_POINTSHOP || type == NPCTYPE_SCRIPT || type == NPCTYPE_MARKETSHOP) &&
		...
		if (sscanf(w1, "%15[^,],%6hd,%6hd,%4hd", mapname, &x, &y, &dir) !=
			4) {  // <map name>,<x>,<y>,<facing>
			ShowError(
				"npc_parse_duplicate: Invalid placement format for duplicate in file '%s', line "
				"'%d'. Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
			return end;	 // next line, try to continue
		}
	...
	if (m != -1 && (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys)) {
		ShowError(
			"npc_parse_duplicate: coordinates %d/%d are out of bounds in map %s(%dx%d), in file "
			"'%s', line '%d'\n",
			x, y, mapdata->name, mapdata->xs, mapdata->ys, filepath,
			strline(buffer, start - buffer));
	}
	...
	if (type == NPCTYPE_WARP && sscanf(w4, "%6hd,%6hd", &xs, &ys) == 2)
		...
		ShowError(
			"npc_parse_duplicate: Invalid span format for duplicate warp in file '%s', line '%d'. "
			"Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
			filepath, strline(buffer, start - buffer), w1, w2, w3, w4);
		return end;	 // next line, try to continue
	}
	...
	switch (type) {
		...
		case NPCTYPE_MARKETSHOP:
			++npc_shop;
			safestrncpy(nd->u.shop.pointshop_str, dnd->u.shop.pointshop_str,
						strlen(dnd->u.shop.pointshop_str));
...
int npc_duplicate4instance(struct npc_data* snd, int16 m) {
	...
	if (npc_name2id(newname) != NULL) {	 // Name already in use
		ShowError(
			"npc_duplicate4instance: the npcname (%s) is already in use while trying to duplicate "
			"npc %s in instance %d.\n",
	...
	if (snd->subtype == NPCTYPE_WARP) {	 // Adjust destination, if instanced
		...
		if (imap == -1) {
			ShowError(
				"npc_duplicate4instance: warp (%s) leading to instanced map (%s), but instance map "
				"is not attached to current instance.\n",
		...
		if (snd->u.scr.xs >= 0 && snd->u.scr.ys >= 0)
			snprintf(w4, sizeof(w4), "%d,%d,%d", snd->class_, snd->u.scr.xs,
					 snd->u.scr.ys);  // Touch Area
		else
...
int npc_instanceinit(struct npc_data* nd) {
	...
	snprintf(evname, ARRAYLENGTH(evname), "%s::%s", nd->exname,
			 script_config.instance_init_event_name);
...
int npc_instancedestroy(struct npc_data* nd) {
	...
	snprintf(evname, ARRAYLENGTH(evname), "%s::%s", nd->exname,
			 script_config.instance_destroy_event_name);
...
void npc_market_tosql(const char* exname, struct npc_item_list* list) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt,
									 "REPLACE INTO `%s` (`name`,`nameid`,`price`,`amount`,`flag`) "
									 "VALUES ('%s','%u','%d','%d','%" PRIu8 "')",
									 market_table, exname, list->nameid, list->value, list->qty,
									 list->flag) ||
...
void npc_market_delfromsql_(const char* exname, t_itemid nameid, bool clear) {
	...
	if (clear) {
		if (SQL_ERROR ==
				SqlStmt_Prepare(stmt, "DELETE FROM `%s` WHERE `name`='%s'", market_table, exname) ||
			SQL_ERROR == SqlStmt_Execute(stmt))
			SqlStmt_ShowDebug(stmt);
	} else {
		if (SQL_ERROR == SqlStmt_Prepare(
							 stmt, "DELETE FROM `%s` WHERE `name`='%s' AND `nameid`='%u' LIMIT 1",
							 market_table, exname, nameid) ||
			SQL_ERROR == SqlStmt_Execute(stmt))
			SqlStmt_ShowDebug(stmt);
	}
	SqlStmt_Free(stmt);
...
/**
 * Loads persistent NPC Market Data from SQL, use the records after NPCs init'd to reuse the stock
 *values.
 **/
static void npc_market_fromsql(void) {
	uint32 count = 0;
	...
	if (SQL_ERROR ==
		Sql_Query(mmysql_handle,
				  "SELECT `name`,`nameid`,`price`,`amount`,`flag` FROM `%s` ORDER BY `name`",
				  market_table)) {
		Sql_ShowDebug(mmysql_handle);
		return;
	}
	...
	ShowStatus("Done loading '" CL_WHITE "%d" CL_RESET "' entries for '" CL_WHITE "%d" CL_RESET
			   "' NPC Markets from '" CL_WHITE "%s" CL_RESET "' table.\n",
...
void npc_unsetcells(struct npc_data* nd) {
	...
	for (i = y - ys; i <= y + ys; i++)
		for (j = x - xs; j <= x + xs; j++) map_setcell(m, j, i, CELL_NPC, false);
...
int npc_do_atcmd_event(struct map_session_data* sd, const char* command, const char* message,
	...
	for (i = 0; i < (strlen(message) + 1) && k < 127; i++) {
		if (message[i] == ' ' || message[i] == '\0') {
			if (i > 0 && message[i - 1] == ' ') {
				continue;  // To prevent "@atcmd [space][space]" and .@atcmd_numparameters return 1
...
static const char* npc_parse_function(char* w1, char* w2, char* w3, char* w4, const char* start,
									  const char* buffer, const char* filepath) {
	DBMap* func_db;
	DBData old_data;
	struct script_code* script;
	...
	if (*w4 != '{' || script_start == NULL || (end != NULL && script_start > end)) {
		ShowError(
			"npc_parse_function: Missing left curly '%%TAB%%{' in file '%s', line '%d'. Skipping "
	...
	script = parse_script(script_start, filepath, strline(buffer, start - buffer),
						  SCRIPT_RETURN_EMPTY_SCRIPT);
...
static const char* npc_parse_mob(char* w1, char* w2, char* w3, char* w4, const char* start,
								 const char* buffer, const char* filepath) {
	...
	if ((w1count = sscanf(w1, "%15[^,],%6hd,%6hd,%6hd,%6hd", mapname, &x, &y, &xs, &ys)) < 1 ||
		...
		sscanf(w4, "%23[^,],%11d,%11u,%11u,%77[^,],%11d,%11d[^\t\r\n]", sprite, &num, &mob.delay1,
			   &mob.delay2, mob.eventname, &size, &ai) < 2) {
		ShowError(
	...
	if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
		ShowError(
			"npc_parse_mob: Spawn coordinates out of range: %s (%d,%d), map size is (%d,%d) - %s "
			"%s (file '%s', line '%d').\n",
			mapdata->name, x, y, (mapdata->xs - 1), (mapdata->ys - 1), w1, w3, filepath,
			strline(buffer, start - buffer));
		return strchr(start, '\n');	 // skip and continue
	}
	...
	if (num < 1 || num > 1000) {
		ShowError(
			"npc_parse_mob: Invalid number of monsters %d, must be inside the range [1,1000] (file "
			"'%s', line '%d').\n",
	...
	if (mob.xs < 0) {
		if (w1count > 3) {
			ShowWarning(
				"npc_parse_mob: Negative x-span %hd for mob ID %d (file '%s', line '%d').\n",
	...
	if (mob.ys < 0) {
		if (w1count > 4) {
			ShowWarning(
				"npc_parse_mob: Negative y-span %hd for mob ID %d (file '%s', line '%d').\n",
	...
	if (battle_config.force_random_spawn ||
		(mob.x == 0 && mob.y == 0)) {  // Force a random spawn anywhere on the map.
		mob.x = mob.y = 0;
		mob.xs = mob.ys = -1;
	}
	...
	if (mob.delay1 > 0xfffffff || mob.delay2 > 0xfffffff) {
		ShowError("npc_parse_mob: Invalid spawn delays %u %u (file '%s', line '%d').\n", mob.delay1,
				  mob.delay2, filepath, strline(buffer, start - buffer));
		return strchr(start, '\n');	 // skip and continue
	}
	...
	// Now that all has been validated. We allocate the actual memory that the re-spawn data will
	// use.
 ...
 *------------------------------------------*/
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start,
									 const char* buffer, const char* filepath) {
	int16 m;
	char mapname[MAP_NAME_LENGTH_EXT];
	bool state = true;
	...
	switch (mapflag) {
		...
		case MF_NOSAVE: {
			...
			if (state && !strcmpi(w4, "SavePoint")) {
				...
				args.nosave.x = -1;
				args.nosave.y = -1;
			} else if (state && sscanf(w4, "%15[^,],%6hd,%6hd", savemap, &args.nosave.x,
									   &args.nosave.y) == 3) {
				args.nosave.map = mapindex_name2id(savemap);
				if (!args.nosave.map) {
					ShowWarning(
						"npc_parse_mapflag: Specified save point map '%s' for mapflag 'nosave' not "
						"found (file '%s', line '%d'), using 'SavePoint'.\n * w1=%s\n * w2=%s\n * "
						"w3=%s\n * w4=%s\n",
		...
		case MF_PVP_NIGHTMAREDROP: {
			...
			if (sscanf(w4, "%15[^,],%15[^,],%11d", drop_arg1, drop_arg2,
					   &args.nightmaredrop.drop_per) == 3) {
				if (!strcmpi(drop_arg1, "random"))
					args.nightmaredrop.drop_id = -1;
				else if (!item_db.exists(
							 (args.nightmaredrop.drop_id = strtol(drop_arg1, nullptr, 10)))) {
					args.nightmaredrop.drop_id = 0;
					ShowWarning(
						"npc_parse_mapflag: Invalid item ID '%d' supplied for mapflag "
						"'pvp_nightmaredrop' (file '%s', line '%d'), removing.\n * w1=%s\n * "
						"w2=%s\n * w3=%s\n * w4=%s\n",
						args.nightmaredrop.drop_id, filepath, strline(buffer, start - buffer), w1,
						w2, w3, w4);
					break;
				}
		...
		case MF_RESTRICTED:
			if (state) {
				...
				else  // Could not be read, no value defined; don't remove as other restrictions may
					  // be set on the map
					ShowWarning(
						"npc_parse_mapflag: Zone value not set for the restricted mapflag! Skipped "
						"flag from %s (file '%s', line '%d').\n",
		...
		case MF_SKILL_DAMAGE: {
			...
			if (!state)
				...
				if (sscanf(w4, "%30[^,],%23[^,],%11d,%11d,%11d,%11d[^\n]", skill_name,
						   caster_constant, &args.skill_damage.rate[SKILLDMG_PC],
						   &args.skill_damage.rate[SKILLDMG_MOB],
						   &args.skill_damage.rate[SKILLDMG_BOSS],
						   &args.skill_damage.rate[SKILLDMG_OTHER]) >= 3) {
					if (ISDIGIT(caster_constant[0]))
						...
						if (!script_get_constant(caster_constant, &val_tmp)) {
							ShowError(
								"npc_parse_mapflag: Unknown constant '%s'. Skipping (file '%s', "
								"line '%d').\n",
					...
					for (int i = SKILLDMG_PC; i < SKILLDMG_MAX; i++)
						args.skill_damage.rate[i] =
							cap_value(args.skill_damage.rate[i], -100, 100000);
					...
					if (strcmp(skill_name, "all") == 0)	 // Adjust damage for all skills
						...
						ShowWarning(
							"npc_parse_mapflag: Invalid skill name '%s' for Skill Damage mapflag. "
			...
			if (!state)
				...
				if (sscanf(w4, "%30[^,],%5hu[^\n]", skill_name, &args.skill_duration.per) == 2) {
					...
					if (!args.skill_duration.skill_id)
						ShowError(
							"npc_parse_mapflag: skill_duration: Invalid skill name '%s' for Skill "
							"Duration mapflag. Skipping (file '%s', line '%d')\n",
							skill_name, filepath, strline(buffer, start - buffer));
					else {
...
int npc_parsesrcfile(const char* filepath) {
	...
	if ((unsigned char)buffer[0] == 0xEF && (unsigned char)buffer[1] == 0xBB &&
		...
		// - It's unlikely that the user wants to use UTF-8 data here, since we don't really support
		// it, nor does the client by default.
		// - If the user really wants to use UTF-8 (instead of latin1, EUC-KR, SJIS, etc), then they
		// can still do it <without BOM>. More info at http://unicode.org/faq/utf_bom.html#bom5 and
		// http://en.wikipedia.org/wiki/Byte_order_mark#UTF-8
		ShowError(
			"npc_parsesrcfile: Detected unsupported UTF-8 BOM in file '%s'. Stopping (please "
			"consider using another character set).\n",
			filepath);
		aFree(buffer);
		return 0;
	}
	...
	for (p = skip_space(buffer); p && *p; p = skip_space(p)) {
		...
		count = sv_parse(p, len + buffer - p, 0, '\t', pos, ARRAYLENGTH(pos),
		...
		if (strcmp(w1, "-") != 0 &&
			strcasecmp(w1, "function") != 0) {	// w1 = <map name>,<x>,<y>,<facing>
			char mapname[MAP_NAME_LENGTH_EXT];
			int count2;
			...
			if (count2 < 1) {
				ShowError(
					"npc_parsesrcfile: Invalid script definition in file '%s', line '%d'. Skipping "
					"line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
					filepath, strline(buffer, p - buffer), w1, w2, w3, w4);
			...
			if (!mapindex_name2id(mapname)) {  // Incorrect map, we must skip the script info...
				ShowError(
					"npc_parsesrcfile: Unknown map '%s' in file '%s', line '%d'. Skipping "
					"line...\n",
			...
			m = map_mapname2mapid(mapname);
			if (m <
				0) {  // "mapname" is not assigned to this server, we must skip the script info...
				if (strcasecmp(w2, "script") == 0 && count > 3) {
					if ((p = npc_skip_script(p, buffer, filepath)) == NULL) {
						break;
					}
			...
			if (x < 0 || x >= mapdata->xs || y < 0 || y >= mapdata->ys) {
				ShowError(
					"npc_parsesrcfile: Unknown coordinates ('%d', '%d') for map '%s' in file '%s', "
		...
		if ((strcasecmp(w2, "warp") == 0 || strcasecmp(w2, "warp2") == 0) && count > 3)
		...
		else if ((!strcasecmp(w2, "shop") || !strcasecmp(w2, "cashshop") ||
				  !strcasecmp(w2, "itemshop") || !strcasecmp(w2, "pointshop") ||
				  !strcasecmp(w2, "marketshop")) &&
				 count > 3)
			...
			ShowError(
				"npc_parsesrcfile: Unable to parse, probably a missing or extra TAB in file '%s', "
				"line '%d'. Skipping line...\n * w1=%s\n * w2=%s\n * w3=%s\n * w4=%s\n",
				filepath, strline(buffer, p - buffer), w1, w2, w3, w4);
			p = strchr(p, '\n');  // skip and continue
		}
 ...
 */
npc_data* npc_duplicate_npc(npc_data& nd, char name[NPC_NAME_LENGTH + 1], int16 mapid, int16 x,
							int16 y, int class_, uint8 dir, int16 xs, int16 ys,
							struct map_session_data* owner) {
	static char w1[128], w2[128], w3[128], w4[128];
	...
	npc_parse_duplicate(w1, w2, w3, w4, stat_buf, stat_buf, "DUPLICATE",
						owner);	 // DUPLICATE means nothing for now.
	...
	if (sd != nullptr) {
		...
		if (sd->npc_id == nd->bl.id || sd->npc_shopid == nd->bl.id) {
			...
			nd->dynamicnpc.removal_tid = add_timer(
				...
				npc_dynamicnpc_removal_timer, nd->bl.id, NULL);
			return 0;
		}

		// Last interaction is not long enough in the past
		if (DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction) <
			battle_config.feature_dynamicnpc_timeout) {
			nd->dynamicnpc.removal_tid =
				add_timer(nd->dynamicnpc.last_interaction +
							  DIFF_TICK(gettick(), nd->dynamicnpc.last_interaction),
						  npc_dynamicnpc_removal_timer, nd->bl.id, NULL);
			return 0;
		}
...
struct npc_data* npc_duplicate_npc_for_player(struct npc_data& nd, struct map_session_data& sd) {
	...
	if (!map_search_freecell(&sd.bl, 0, &new_x, &new_y, battle_config.feature_dynamicnpc_rangex,
							 battle_config.feature_dynamicnpc_rangey, 0)) {
		ShowError(
			"npc_duplicate_npc_for_player: Unable to find a free cell to duplicate NPC \"%s\" for "
			"player \"%s\"(AID: %u, CID: %u)\n",
	...
	struct npc_data* dnd = npc_duplicate_npc(nd, nd.name, sd.bl.m, new_x, new_y, nd.class_, dir,
											 nd.u.scr.xs, nd.u.scr.ys, &sd);
	...
	dnd->dynamicnpc.removal_tid =
		add_timer(dnd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout,
				  npc_dynamicnpc_removal_timer, dnd->bl.id, NULL);
...
const char* npc_get_script_event_name(int npce_index) {
	switch (npce_index) {
		...
		default:
			ShowError(
				"npc_get_script_event_name: npce_index is outside the array limits: %d (max: "
				"%d).\n",
				npce_index, NPCE_MAX);
			return NULL;
	}
...
int npc_reload(void) {
	...
	ShowInfo("Done loading '" CL_WHITE "%d" CL_RESET "' NPCs:" CL_CLL
			 ...
			 npc_id - npc_new_min, npc_warp, npc_shop, npc_script, npc_mob, npc_cache_mob,
			 npc_delay_mob);
...
bool npc_unloadfile(const char* path) {
	...
	for (nd = (struct npc_data*)dbi_first(iter); dbi_exists(iter);
		 nd = (struct npc_data*)dbi_next(iter)) {
		if (nd->path && strcasecmp(nd->path, path) == 0) {
			found = true;
			npc_unload_duplicates(
				nd); /* unload any npcs which could duplicate this but be in a different file */
			npc_unload(nd, true);
		}
	}
...
static void npc_debug_warps_sub(struct npc_data* nd) {
	...
	if (map_getcell(m, nd->u.warp.x, nd->u.warp.y, CELL_CHKNPC)) {
		ShowWarning("Warp %s at %s(%d,%d) warps directly on top of an area npc at %s(%d,%d)\n",
					nd->name, mapdata_nd->name, nd->bl.x, nd->bl.y, mapdata->name, nd->u.warp.x,
					nd->u.warp.y);
	}
	if (map_getcell(m, nd->u.warp.x, nd->u.warp.y, CELL_CHKNOPASS)) {
		ShowWarning("Warp %s at %s(%d,%d) warps to a non-walkable tile at %s(%d,%d)\n", nd->name,
					mapdata_nd->name, nd->bl.x, nd->bl.y, mapdata->name, nd->u.warp.x,
					nd->u.warp.y);
	}
}
...
void do_init_npc(void) {
	...
	for (i = 1; i < MAX_NPC_CLASS; i++) npc_viewdb[i].class_ = i;
	...
	timer_event_ers =
		ers_new(sizeof(struct timer_event_data), "npc.cpp::timer_event_ers", ERS_OPT_NONE);
	npc_sc_display_ers =
		ers_new(sizeof(struct sc_display_entry), "npc.cpp:npc_sc_display_ers", ERS_OPT_NONE);
	...
	ShowInfo("Done loading '" CL_WHITE "%d" CL_RESET "' NPCs:" CL_CLL
			 ...
			 "' Mobs Cached\n"
			 "\t-'" CL_WHITE "%d" CL_RESET "' Mobs Not Cached\n",
			 npc_id - START_NPC_NUM, npc_warp, npc_shop, npc_script, npc_mob, npc_cache_mob,
			 npc_delay_mob);
