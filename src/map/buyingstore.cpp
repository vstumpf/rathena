...
DBMap *buyingstore_getdb(void) { return buyingstore_db; }

/// failure constants for clif functions
enum e_buyingstore_failure {
	BUYINGSTORE_CREATE = 1,				// "Failed to open buying store."
	BUYINGSTORE_CREATE_OVERWEIGHT = 2,	// "Total amount of then possessed items exceeds the weight
										// limit by %d. Please re-enter."
	...
	BUYINGSTORE_TRADE_SELLER_COUNT = 6,	 // "The trade failed, because the entered amount of item %s
										 // is higher, than the buyer is willing to buy."
	BUYINGSTORE_TRADE_SELLER_ZENY =
...
static const t_itemid buyingstore_blankslots[MAX_SLOTS] = {
	0};	 // used when checking whether or not an item's card slots are blank

/// Returns unique buying store id
static unsigned int buyingstore_getuid(void) { return ++buyingstore_nextid; }

/**
 * Attempt to setup buying store fast check before create new one
 * @param sd
 * @param slots Number of item on the list
 * @return 0 If success, 1 - Cannot open, 2 - Manner penalty, 3 - Mapflag restiction, 4 - Cell
 * restriction
 */
int8 buyingstore_setup(struct map_session_data *sd, unsigned char slots) {
	nullpo_retr(1, sd);

	if (!battle_config.feature_buying_store || sd->state.vending || sd->state.buyingstore ||
		sd->state.trading || slots == 0) {
		return 1;
	}

	if (sd->sc.data[SC_NOCHAT] &&
		(sd->sc.data[SC_NOCHAT]->val1 & MANNER_NOROOM)) {  // custom: mute limitation
		return 2;
	}
	...
	if (map_getcell(sd->bl.m, sd->bl.x, sd->bl.y,
					CELL_CHKNOBUYINGSTORE)) {			// custom: no buyingstore cells
 ...
 * @return 0 If success, 1 - Cannot open, 2 - Manner penalty, 3 - Mapflag restiction, 4 - Cell
 * restriction, 5 - Invalid count/result, 6 - Cannot give item, 7 - Will be overweight
 */
int8 buyingstore_create(struct map_session_data *sd, int zenylimit, unsigned char result,
						const char *storename,
						const struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub *itemlist,
						unsigned int count, struct s_autotrader *at) {
	...
	for (i = 0; i < count; i++) {
		...
		if (i) {
			...
			if (listidx != i) {
				ShowWarning(
					"buyingstore_create: Found duplicate item on buying list (nameid=%u, "
					"amount=%hu, account_id=%d, char_id=%d).\n",
	...
	if ((sd->max_weight * 90) / 100 <
		weight) {  // not able to carry all wanted items without getting overweight (90%)
		sd->buyingstore.slots = 0;
	...
	if (Sql_Query(mmysql_handle,
				  "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, "
				  "`title`, `limit`, `autotrade`, `body_direction`, `head_direction`, `sit`) "
				  "VALUES( %d, %d, %d, '%c', '%s', %d, %d, '%s', %d, %d, '%d', '%d', '%d' );",
				  buyingstores_table, sd->buyer_id, sd->status.account_id, sd->status.char_id,
				  sd->status.sex == 0 ? 'F' : 'M', map_getmapdata(sd->bl.m)->name, sd->bl.x,
				  sd->bl.y, message_sql, sd->buyingstore.zenylimit, sd->state.autotrade,
				  at ? at->dir : sd->ud.dir, at ? at->head_dir : sd->head_dir,
				  at ? at->sit : pc_issit(sd)) != SQL_SUCCESS) {
		Sql_ShowDebug(mmysql_handle);
	}

	StringBuf_Init(&buf);
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s`(`buyingstore_id`,`index`,`item_id`,`amount`,`price`) VALUES",
					 buyingstore_items_table);
	for (i = 0; i < sd->buyingstore.slots; i++) {
		StringBuf_Printf(&buf, "(%d,%d,%u,%d,%d)", sd->buyer_id, i, sd->buyingstore.items[i].nameid,
						 sd->buyingstore.items[i].amount, sd->buyingstore.items[i].price);
...
void buyingstore_close(struct map_session_data *sd) {
	...
	if (sd->state.buyingstore) {
		if (Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE buyingstore_id = %d;",
					  buyingstore_items_table, sd->buyer_id) != SQL_SUCCESS ||
			Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `id` = %d;", buyingstores_table,
...
void buyingstore_open(struct map_session_data *sd, uint32 account_id) {
	...
	if ((pl_sd = map_id2sd(account_id)) == NULL ||
		!pl_sd->state.buyingstore) {  // not online or not buying
		return;
	}

	if (!searchstore_queryremote(sd, account_id) &&
		(sd->bl.m != pl_sd->bl.m ||
		 !check_distance_bl(&sd->bl, &pl_sd->bl, AREA_SIZE))) {	 // out of view range
		return;
	}
 ...
 * @param count Number of item on the itemlist
 */
void buyingstore_trade(struct map_session_data *sd, uint32 account_id, unsigned int buyer_id,
	...
	for (int i = 0; i < count; i++) {
		...
		for (int k = 0; k < i; k++) {
			...
			if (itemlist[k].index == item->index && k != i) {
				ShowWarning(
					"buyingstore_trade: Found duplicate item on selling list (prevnameid=%u, "
					"prevamount=%hu, nameid=%u, amount=%hu, account_id=%d, char_id=%d).\n",
					itemlist[k].itemId, itemlist[k].amount, item->itemId, item->amount,
					sd->status.account_id, sd->status.char_id);
				clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED,
													 item->itemId);
				return;
			}
		}
		...
		if (index < 0 || index >= ARRAYLENGTH(sd->inventory.u.items_inventory) ||
			...
			sd->inventory.u.items_inventory[index].nameid != item->itemId ||
		...
		if (sd->inventory.u.items_inventory[index].expire_time ||
			...
			memcmp(sd->inventory.u.items_inventory[index].card, buyingstore_blankslots,
		...
		ARR_FIND(0, pl_sd->buyingstore.slots, listidx,
				 pl_sd->buyingstore.items[listidx].nameid == item->itemId);
		...
		if (item->amount * (unsigned int)sd->inventory_data[index]->weight >
			pl_sd->max_weight - weight) {
			clif_buyingstore_trade_failed_seller(sd, BUYINGSTORE_TRADE_SELLER_FAILED, item->itemId);
			return;
		}
		...
		if (item->amount * pl_sd->buyingstore.items[listidx].price >
	...
	for (int i = 0; i < count; i++) {
		...
		// move item
		pc_additem(pl_sd, &sd->inventory.u.items_inventory[index], item->amount,
				   LOG_TYPE_BUYING_STORE);
		...
		if (pl_sd->buyingstore.items[listidx].amount > 0) {
			if (Sql_Query(
					mmysql_handle,
					"UPDATE `%s` SET `amount` = %d WHERE `buyingstore_id` = %d AND `index` = %d;",
					buyingstore_items_table, pl_sd->buyingstore.items[listidx].amount,
					pl_sd->buyer_id, listidx) != SQL_SUCCESS) {
				Sql_ShowDebug(mmysql_handle);
			}
		} else {
			if (Sql_Query(mmysql_handle,
						  "DELETE FROM `%s` WHERE `buyingstore_id` = %d AND `index` = %d;",
						  buyingstore_items_table, pl_sd->buyer_id, listidx) != SQL_SUCCESS) {
				Sql_ShowDebug(mmysql_handle);
			}
		}
		...
		// notify clients
		clif_buyingstore_delete_item(sd, index, item->amount,
	...
	if (i == pl_sd->buyingstore.slots) {  // everything was bought
		...
		if (Sql_Query(mmysql_handle, "UPDATE `%s` SET `limit` = %d WHERE `id` = %d;",
					  buyingstores_table, pl_sd->buyingstore.zenylimit,
					  pl_sd->buyer_id) != SQL_SUCCESS) {
			Sql_ShowDebug(mmysql_handle);
		}
...
bool buyingstore_search(struct map_session_data *sd, t_itemid nameid) {
	...
	ARR_FIND(0, sd->buyingstore.slots, i,
			 sd->buyingstore.items[i].nameid == nameid && sd->buyingstore.items[i].amount);
...
bool buyingstore_searchall(struct map_session_data *sd, const struct s_search_store_search *s) {
	...
	for (idx = 0; idx < s->item_count; idx++) {
		ARR_FIND(0, sd->buyingstore.slots, i,
				 sd->buyingstore.items[i].nameid == s->itemlist[idx].itemId &&
		...
		if (s->search_sd->searchstore.items.size() >=
			(unsigned int)battle_config.searchstore_maxresults) {
			return false;
		}

		std::shared_ptr<s_search_store_info_item> ssitem =
			std::make_shared<s_search_store_info_item>();
...
void buyingstore_reopen(struct map_session_data *sd) {
	...
	if ((at = (struct s_autotrader *)uidb_get(buyingstore_autotrader_db, sd->status.char_id)) &&
		at->count && at->entries) {
		...
		if ((fail = buyingstore_setup(sd, (unsigned char)at->count)) == 0 &&
			...
			ShowInfo("Buyingstore loaded for '" CL_WHITE "%s" CL_RESET "' with '" CL_WHITE
					 "%d" CL_RESET "' items at " CL_WHITE "%s (%d,%d)" CL_RESET "\n",
					 sd->status.name, at->count, mapindex_id2name(sd->mapindex), sd->bl.x,
					 sd->bl.y);
		}
		aFree(data);
	}
	...
	if (at) {
		...
		if (db_size(buyingstore_autotrader_db) == 0)
			buyingstore_autotrader_db->clear(buyingstore_autotrader_db,
											 buyingstore_autotrader_free);
	}

	if (fail != 0) {
		ShowError("buyingstore_reopen: (Error:%d) Load failed for autotrader '" CL_WHITE
...
void do_init_buyingstore_autotrade(void) {
	if (battle_config.feature_autotrade) {
		if (Sql_Query(mmysql_handle,
					  ...
					  "FROM `%s` "
					  "WHERE `autotrade` = 1 AND `limit` > 0 AND (SELECT COUNT(`buyingstore_id`) "
					  "FROM `%s` WHERE `buyingstore_id` = `id`) > 0 "
					  "ORDER BY `id`;",
		...
		if (Sql_NumRows(mmysql_handle) > 0) {
			...
			ShowStatus("Done loading '" CL_WHITE "%d" CL_RESET
					   "' buyingstore autotraders with '" CL_WHITE "%d" CL_RESET "' items.\n",
...
void buyingstore_update(map_session_data &sd) {
	if (Sql_Query(mmysql_handle,
				  "UPDATE `%s` SET `map` = '%s', `x` = '%d', `y` = '%d', `body_direction` = '%d', "
				  "`head_direction` = '%d', `sit` = '%d', `autotrade` = '%d' WHERE `id` = '%d'",
				  buyingstores_table, map_getmapdata(sd.bl.m)->name, sd.bl.x, sd.bl.y, sd.ud.dir,
				  sd.head_dir, pc_issit(&sd), sd.state.autotrade, sd.buyer_id) != SQL_SUCCESS) {
		Sql_ShowDebug(mmysql_handle);
	}
}
