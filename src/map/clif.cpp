...
static inline uint32 client_tick(t_tick tick) { return (uint32)tick; }

#if PACKETVER >= 20170830
static inline int64 client_exp(t_exp exp) { return (int64)u64min(exp, MAX_EXP); }
#else
static inline int32 client_exp(t_exp exp) { return (int32)u64min(exp, MAX_EXP); }
#endif
 ...
 * @return item type. For IT_PETEGG will be displayed as IT_ARMOR. If Shadow Weapon of IT_SHADOWGEAR
 * as IT_WEAPON and else as IT_ARMOR
 */
static inline int itemtype(t_itemid nameid) {
	struct item_data *id =
		itemdb_search(nameid);	// Use itemdb_search, so non-existance item will use dummy data and
...
// TODO: doc
static inline uint16 client_index(uint16 server_index) { return server_index + 2; }

static inline uint16 server_index(uint16 client_index) { return client_index - 2; }

static inline uint16 client_storage_index(uint16 server_index) { return server_index + 1; }

static inline uint16 server_storage_index(uint16 client_index) { return client_index - 1; }
...
static inline void WBUFPOS2(uint8 *p, unsigned short pos, short x0, short y0, short x1, short y1,
...
static inline void WFIFOPOS2(int fd, unsigned short pos, short x0, short y0, short x1, short y1,
							 unsigned char sx0, unsigned char sy0) {
	WBUFPOS2(WFIFOP(fd, pos), 0, x0, y0, x1, y1, sx0, sy0);
}

static inline void RBUFPOS(const uint8 *p, unsigned short pos, short *x, short *y,
						   unsigned char *dir) {
	p += pos;
...
static inline void RBUFPOS2(const uint8 *p, unsigned short pos, short *x0, short *y0, short *x1,
							short *y1, unsigned char *sx0, unsigned char *sy0) {
	p += pos;
...
static inline void RFIFOPOS2(int fd, unsigned short pos, short *x0, short *y0, short *x1, short *y1,
							 unsigned char *sx0, unsigned char *sy0) {
...
// Guarantees that the given string does not exceeds the allowed size, as well as making sure it's
// null terminated. [Skotlex]
...
void clif_setbindip(const char *ip) {
	...
	if (bind_ip) {
		char ip_str[16];
		ShowInfo("Map Server Bind IP Address : '" CL_WHITE "%s" CL_RESET "' -> '" CL_WHITE
				 "%s" CL_RESET "'.\n",
 ...
 *------------------------------------------*/
void clif_setport(uint16 port) { map_port = port; }

/*==========================================
 * Returns map server IP
 *------------------------------------------*/
uint32 clif_getip(void) { return map_ip; }
 ...
 *------------------------------------------*/
uint16 clif_getport(void) { return map_port; }

#if PACKETVER >= 20071106
static inline unsigned char clif_bl_type(struct block_list *bl, bool walking) {
	switch (bl->type) {
		case BL_PC:
			return (disguised(bl) && !pcdb_checkid(status_get_viewdata(bl)->class_))
					   ? 0x1
					   : 0x0;  // PC_TYPE
		case BL_ITEM:
...
// Since walking NPCs are not supported on official servers, the client does not know how to handle
// it.
#if PACKETVER >= 20170726
			return (pcdb_checkid(status_get_viewdata(bl)->class_) && walking)
					   ? 0x0
					   : 0xC;  // New walking NPC type
#else
 ...
 * Called from a map_foreachinallarea (grabs all players in specific area and subjects them to this
 *function) In order to send area-wise packets, such as:
...
static int clif_send_sub(struct block_list *bl, va_list ap) {
	...
	if (WFIFOP(fd, 0) == buf) {
		...
		// NO. It is not ok. There is the chance WFIFOSET actually sends the buffer data, and shifts
		// elements around, which will corrupt the buffer.
		return 0;
	}
...
int clif_send(const void *buf, int len, struct block_list *bl, enum send_target type) {
	...
	switch (type) {
		...
		case AREA_WOC:
		case AREA_WOS:
			map_foreachinallarea(clif_send_sub, bl->m, bl->x - AREA_SIZE, bl->y - AREA_SIZE,
								 bl->x + AREA_SIZE, bl->y + AREA_SIZE, BL_PC, buf, len, bl, type);
			break;
		case AREA_CHAT_WOC:
			map_foreachinallarea(clif_send_sub, bl->m, bl->x - (AREA_SIZE - 5),
								 bl->y - (AREA_SIZE - 5), bl->x + (AREA_SIZE - 5),
			...
			if (g) {
				for (i = 0; i < g->max_member; i++) {
					if ((sd = g->member[i].sd) != nullptr) {
						...
						if (sd->bl.id == bl->id &&
							(type == GUILD_WOS || type == GUILD_SAMEMAP_WOS ||
							 type == GUILD_AREA_WOS))
							continue;

						if (type != GUILD && type != GUILD_NOBG && type != GUILD_WOS &&
							sd->bl.m != bl->m)
							continue;
			...
			if (sd && sd->bg_id > 0 && (bg = util::umap_find(bg_team_db, sd->bg_id))) {
				for (const auto &member : bg->members) {
					if ((sd = member.sd) == nullptr || !session_isActive(fd = sd->fd))
						continue;
					if (sd->bl.id == bl->id &&
						(type == BG_WOS || type == BG_SAMEMAP_WOS || type == BG_AREA_WOS))
						continue;
...
/// 009E <id>.L <name id>.W <identified>.B <x>.W <y>.W <subX>.B <subY>.B <amount>.W
/// (ZC_ITEM_FALL_ENTRY) 084B <id>.L <name id>.W <type>.W <identified>.B <x>.W <y>.W <subX>.B
/// <subY>.B <amount>.W (ZC_ITEM_FALL_ENTRY4) 0ADD <id>.L <name id>.W <type>.W <identified>.B <x>.W
/// <y>.W <subX>.B <subY>.B <amount>.W <show drop effect>.B <drop effect mode>.W
/// (ZC_ITEM_FALL_ENTRY5)
 ...
 *------------------------------------------*/
static void clif_set_unit_idle(struct block_list *bl, bool walking, send_target target,
	...
	if (tbl && tbl->type == BL_PC && bl->type == BL_NPC) {
		...
		if (!nd->vd.dead_sit) {
			if (std::find(sd->cloaked_npc.begin(), sd->cloaked_npc.end(), nd->bl.id) !=
				sd->cloaked_npc.end()) {
				option ^= OPTION_CLOAK;
			}
		}
	...
	p.head = vd->hair_style;
	p.weapon = vd->weapon;
#if PACKETVER < 7 || PACKETVER_MAIN_NUM >= 20181121 || PACKETVER_RE_NUM >= 20180704 || \
	...
	if (battle_config.monster_hp_bars_info && !map_getmapflag(bl->m, MF_HIDEMOBHPBAR) &&
		bl->type == BL_MOB && (status_get_hp(bl) < status_get_max_hp(bl))) {
...
#if PACKETVER >= 20091103
		p.objecttype =
			pcdb_checkid(status_get_viewdata(bl)->class_) ? 0x0 : 0x5;	// PC_TYPE : NPC_MOB_TYPE
#if PACKETVER >= 20131223
...
#endif
#if PACKETVER >= 20120221
	if (battle_config.monster_hp_bars_info && bl->type == BL_MOB &&
		!map_getmapflag(bl->m, MF_HIDEMOBHPBAR) && (status_get_hp(bl) < status_get_max_hp(bl))) {
		p.maxHP = status_get_max_hp(bl);
		p.HP = status_get_hp(bl);
	} else {
		p.maxHP = -1;
 ...
 *------------------------------------------*/
static void clif_set_unit_walking(struct block_list *bl, struct map_session_data *tsd,
...
/// type:
///     unused
void clif_class_change_target(struct block_list *bl, int class_, int type, enum send_target target,
							  struct map_session_data *sd) {
	nullpo_retv(bl);
...
void clif_servantball(struct map_session_data &sd, struct block_list *target,
...
void clif_abyssball(struct map_session_data &sd, struct block_list *target,
					enum send_target send_target) {
	struct PACKET_ZC_SPIRITS p = {};
...
static void clif_weather_check(struct map_session_data *sd) {
	...
	if (map_getmapflag(m, MF_SNOW) || map_getmapflag(m, MF_CLOUDS) || map_getmapflag(m, MF_FOG) ||
		map_getmapflag(m, MF_FIREWORKS) || map_getmapflag(m, MF_SAKURA) ||
		map_getmapflag(m, MF_LEAVES) || map_getmapflag(m, MF_CLOUDS2)) {
...
/**
 * Run when the weather on a map changes, throws all players in map id 'm' to clif_weather_check
 *function
 **/
void clif_weather(int16 m) {
	...
	iter = mapit_getallusers();
	for (sd = (struct map_session_data *)mapit_first(iter); mapit_exists(iter);
		 sd = (struct map_session_data *)mapit_next(iter)) {
		if (sd->bl.m == m)
...
/// 022e <name>.24B <modified>.B <level>.W <hunger>.W <intimacy>.W <equip id>.W <atk>.W <matk>.W
/// <hit>.W <crit>.W <def>.W <mdef>.W <flee>.W <aspd>.W <hp>.W <max hp>.W <sp>.W <max sp>.W <exp>.L
/// <max exp>.L <skill points>.W <atk range>.W	(ZC_PROPERTY_HOMUN) 09f7 <name>.24B <modified>.B
/// <level>.W <hunger>.W <intimacy>.W <equip id>.W <atk>.W <matk>.W <hit>.W <crit>.W <def>.W
/// <mdef>.W <flee>.W <aspd>.W <hp>.L <max hp>.L <sp>.W <max sp>.W <exp>.L <max exp>.L <skill
/// points>.W <atk range>.W (ZC_PROPERTY_HOMUN_2)
void clif_hominfo(struct map_session_data *sd, struct homun_data *hd, int flag) {
	...
	// Bit field, bit 0 : rename_flag (1 = already renamed), bit 1 : homunc vaporized (1 = true),
	// bit 2 : homunc dead (1 = true)
	p.flags = (!battle_config.hom_rename && hd->homunculus.rename_flag ? 0x1 : 0x0) |
			  (hd->homunculus.vaporize == HOM_ST_REST ? 0x2 : 0) |
			  (hd->homunculus.hp > 0 ? 0x4 : 0);
	p.level = hd->homunculus.level;
	p.hunger = hd->homunculus.hunger;
	p.intimacy = hd->homunculus.intimacy / 100;
#if !(PACKETVER_MAIN_NUM >= 20190619 || PACKETVER_RE_NUM >= 20190605 || \
	  PACKETVER_ZERO_NUM >= 20190626)
	p.itemId = 0;  // equip id
#endif
...
void clif_send_homdata(struct map_session_data *sd, int state, int param) {	 //[orn]
	...
	if ((state == SP_INTIMATE) && (param >= 910) &&
		(sd->hd->homunculus.class_ == sd->hd->homunculusDB->evo_class))
...
int clif_homskillinfoblock(struct map_session_data *sd) {  //[orn]
	...
	for (i = 0; i < MAX_HOMUNSKILL; i++) {
		...
		if (id != 0) {
			...
			WFIFOW(fd, len + 10) =
				skill_get_range2(&sd->hd->bl, id, hd->homunculus.hskill[idx].lv, false);
	...
	WFIFOB(fd, 10) =
		(hd->homunculus.level < hom_skill_get_min_level(hd->homunculus.class_, skill_id) ||
		 hd->homunculus.hskill[idx].lv >=
			 hom_skill_tree_get_max(hd->homunculus.hskill[idx].id, hd->homunculus.class_))
			? 0
			: 1;
...
static void clif_move2(struct block_list *bl, struct view_data *vd, struct unit_data *ud) {
	...
	if ((sc = status_get_sc(bl)) &&
		sc->option & (OPTION_HIDE | OPTION_CLOAK | OPTION_INVISIBLE | OPTION_CHASEWALK))
		clif_ally_only = true;
...
void clif_move(struct unit_data *ud) {
	...
	// This performance check is needed to keep GM-hidden objects from being notified to bots.
		...
		// If the player was disguised we still need to update the disguised unit, since the main
		// unit will be updated through clif_walkok
		if (disguised(bl)) {
			WBUFW(buf, 0) = 0x86;
			WBUFL(buf, 2) = disguised_bl_id(bl->id);
...
/// map-server. 0092 <map name>.16B <x>.W <y>.W <ip>.L <port>.W (ZC_NPCACK_SERVERMOVE) 0ac7 <map
/// name>.16B <x>.W <y>.W <ip>.L <port>.W <unknown>.128B (ZC_NPCACK_SERVERMOVE2)
void clif_changemapserver(struct map_session_data *sd, unsigned short map_index, int x, int y,
...
void clif_buylist(struct map_session_data *sd, struct npc_data *nd) {
	...
	struct PACKET_ZC_PC_PURCHASE_ITEMLIST *p =
		(struct PACKET_ZC_PC_PURCHASE_ITEMLIST *)WFIFOP(fd, 0);
	...
	p->packetLength = sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST) +
					  count * sizeof(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_sub);
...
void clif_npc_market_open(struct map_session_data *sd, struct npc_data *nd) {
	...
	WFIFOHEAD(fd, sizeof(struct PACKET_ZC_NPC_MARKET_OPEN) +
					  nd->u.shop.count * sizeof(struct PACKET_ZC_NPC_MARKET_OPEN_sub));
	...
	p->packetLength = sizeof(struct PACKET_ZC_NPC_MARKET_OPEN) +
					  count * sizeof(struct PACKET_ZC_NPC_MARKET_OPEN_sub);
	WFIFOSET(fd, p->packetLength);
...
/// 0x9d7 <packet len>.W <count>.B { <name id>.W <qty>.W <price>.L }*
/// (ZC_NPC_MARKET_PURCHASE_RESULT)
void clif_npc_market_purchase_ack(struct map_session_data *sd, e_purchase_result res,
								  std::vector<s_npc_buy_list> &list) {
#if PACKETVER >= 20131223
	nullpo_retv(sd);
	...
	struct PACKET_ZC_NPC_MARKET_PURCHASE_RESULT *p =
		(struct PACKET_ZC_NPC_MARKET_PURCHASE_RESULT *)packet_buffer;
...
void clif_parse_NPCMarketPurchase(int fd, struct map_session_data *sd) {
	...
	const struct PACKET_CZ_NPC_MARKET_PURCHASE *p =
		(struct PACKET_CZ_NPC_MARKET_PURCHASE *)RFIFOP(fd, 0);
...
void clif_scriptmenu(struct map_session_data *sd, int npcid, const char *mes) {
	...
	if (!sd->state.using_fake_npc &&
		(npcid == fake_nd->bl.id ||
		 ((bl = map_id2bl(npcid)) &&
		  (bl->m != sd->bl.m || bl->x < sd->bl.x - AREA_SIZE - 1 ||
		   bl->x > sd->bl.x + AREA_SIZE + 1 || bl->y < sd->bl.y - AREA_SIZE - 1 ||
		   bl->y > sd->bl.y + AREA_SIZE + 1))))
...
/// color:
///     0x00RRGGBB
void clif_viewpoint(struct map_session_data *sd, int npc_id, int type, int x, int y, int id,
					int color) {
	int fd;
...
static void clif_addcards(struct EQUIPSLOTINFO *buf, struct item *item) {
	...
	if (item->card[0] == CARD0_PET) {
		...
		buf->card[1] = 0;
		// Pet intimacy
		// No idea when this was added exactly, but older clients have no problem if we send it
		// anyway
...
/// 00a0 <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W <equip location>.W <item type>.B <result>.B (ZC_ITEM_PICKUP_ACK) 029a
/// <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W <equip location>.W <item type>.B <result>.B <expire time>.L
/// (ZC_ITEM_PICKUP_ACK2) 02d4 <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.W <item type>.B <result>.B
/// <expire time>.L <bindOnEquipType>.W (ZC_ITEM_PICKUP_ACK3) 0990 <index>.W <amount>.W <name id>.W
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.L
/// <item type>.B <result>.B <expire time>.L <bindOnEquipType>.W (ZC_ITEM_PICKUP_ACK_V5) 0a0c
/// <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W <equip location>.L <item type>.B <result>.B <expire time>.L
/// <bindOnEquipType>.W { <option id>.W <option value>.W <option param>.B }*5
/// (ZC_ITEM_PICKUP_ACK_V6) 0a37 <index>.W <amount>.W <name id>.W <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <equip location>.L <item type>.B <result>.B
/// <expire time>.L <bindOnEquipType>.W { <option id>.W <option value>.W <option param>.B }*5
/// <favorite>.B <view id>.W (ZC_ITEM_PICKUP_ACK_V7)
void clif_additem(struct map_session_data *sd, int n, int amount, unsigned char fail) {
	...
	if (fail) {
		...
		/* why restrict the flag to non-stackable? because this is the only packet allows stackable
		 * to, show the color, and therefore it'd be inconsistent with the rest (aka it'd show
		 * yellow, you relog/refresh and boom its gone)
		 */
		p.bindOnEquipType =
			sd->inventory.u.items_inventory[n].bound && !itemdb_isstackable2(sd->inventory_data[n])
				? 2
				: sd->inventory_data[n]->flag.bindOnEquip ? 1 : 0;
...
static void clif_item_equip(short idx, struct EQUIPITEM_INFO *p, struct item *it,
							struct item_data *id, int eqp_pos) {
	nullpo_retv(p);
	nullpo_retv(it);
	nullpo_retv(id);
...
static void clif_item_normal(short idx, struct NORMALITEM_INFO *p, struct item *i,
							 struct item_data *id) {
	nullpo_retv(p);
	nullpo_retv(i);
	nullpo_retv(id);
...
static void clif_inventoryStart(struct map_session_data *sd, e_inventory_type type,
								const char *name) {
...
// Unified inventory function which sends all of the inventory (requires two packets, one for
// equipable items and one for stackable ones. [Skotlex]
void clif_inventorylist(struct map_session_data *sd) {
	...
	for (int i = 0; i < MAX_INVENTORY; i++) {
		...
		if (!itemdb_isstackable2(sd->inventory_data[i])) {
			clif_item_equip(client_index(i), &itemlist_equip.list[equip++],
							&sd->inventory.u.items_inventory[i], sd->inventory_data[i],
							pc_equippoint(sd, i));

			if (equip == MAX_INVENTORY_ITEM_PACKET_NORMAL) {
				itemlist_equip.PacketType = inventorylistequipType;
				itemlist_equip.PacketLength =
					(sizeof(itemlist_equip) - sizeof(itemlist_equip.list)) +
	...
	if (normal) {
		itemlist_normal.PacketType = inventorylistnormalType;
		itemlist_normal.PacketLength = (sizeof(itemlist_normal) - sizeof(itemlist_normal.list)) +
									   (sizeof(struct NORMALITEM_INFO) * normal);
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
		itemlist_normal.invType = type;
#endif
	...
	if (equip) {
		itemlist_equip.PacketType = inventorylistequipType;
		itemlist_equip.PacketLength = (sizeof(itemlist_equip) - sizeof(itemlist_equip.list)) +
									  (sizeof(struct EQUIPITEM_INFO) * equip);
#if PACKETVER_RE_NUM >= 20180912 || PACKETVER_ZERO_NUM >= 20180919 || PACKETVER_MAIN_NUM >= 20181002
		itemlist_equip.invType = type;
#endif
...
void clif_storagelist(struct map_session_data *sd, struct item *items, int items_length,
	...
	for (int i = 0; i < items_length; i++) {
		...
		if (!itemdb_isstackable2(id)) {
			clif_item_equip(client_storage_index(i), &storage_itemlist_equip.list[equip++],
							&items[i], id, pc_equippoint_sub(sd, id));

			if (equip == MAX_STORAGE_ITEM_PACKET_EQUIP) {
				storage_itemlist_equip.PacketType = storageListEquipType;
				storage_itemlist_equip.PacketLength =
				...
				clif_send(&storage_itemlist_equip, storage_itemlist_equip.PacketLength, &sd->bl,
						  SELF);
			...
			clif_item_normal(client_storage_index(i), &storage_itemlist_normal.list[normal++],
							 &items[i], id);

			if (normal == MAX_STORAGE_ITEM_PACKET_NORMAL) {
				storage_itemlist_normal.PacketType = storageListNormalType;
				storage_itemlist_normal.PacketLength =
				...
				clif_send(&storage_itemlist_normal, storage_itemlist_normal.PacketLength, &sd->bl,
						  SELF);
...
void clif_cartlist(struct map_session_data *sd) {
	...
	for (int i = 0; i < MAX_CART; i++) {
		...
		if (!itemdb_isstackable2(id)) {
			clif_item_equip(client_index(i), &itemlist_equip.list[equip++],
							&sd->cart.u.items_cart[i], id, id->equip);
			// Stackable (Normal)
		} else {
			clif_item_normal(client_index(i), &itemlist_normal.list[normal++],
							 &sd->cart.u.items_cart[i], id);
		}
	}
...
static int clif_hpmeter_sub(struct block_list *bl, va_list ap) {
	...
	if (pc_has_permission(tsd, PC_PERM_VIEW_HPMETER)) {
		clif_hpmeter_single(*tsd, sd->status.account_id, sd->battle_status.hp,
...
/// (ZC_NOTIFY_CARTITEM_COUNTINFO) 013a <atk range>.W (ZC_ATTACK_RANGE) 0141 <status id>.L <base
/// status>.L <plus status>.L (ZC_COUPLESTATUS) 0acb <var id>.W <value>.Q (ZC_LONGPAR_CHANGE2)
...
void clif_updatestatus(struct map_session_data *sd, int type) {
	...
	switch (type) {
		...
		case SP_WEIGHT:
			...
			WFIFOHEAD(fd, 14);
			WFIFOW(fd, 0) =
				0xb0;  // Need to re-set as pc_updateweightstatus can alter the buffer. [Skotlex]
			WFIFOW(fd, 2) = type;
		...
		case SP_HP:
			...
			WFIFOL(fd, 4) = sd->battle_status.hp ? sd->battle_status.hp
...
void clif_sprite_change(struct block_list *bl, int id, int type, int val, int val2,
			...
			case LOOK_BASE:
				...
				if (sd) {
					...
					if (sd->sc.option & OPTION_XMAS && battle_config.xmas_ignorepalette)
						vd->cloth_color = 0;
					if (sd->sc.option & (OPTION_SUMMER | OPTION_SUMMER2) &&
...
///     1 = MsgStringTable[243]="You can't Attack or use Skills because your Weight Limit has been
///     exceeded." 2 = MsgStringTable[244]="You can't use Skills because Weight Limit has been
///     exceeded." 3 = assassin, baby_assassin, assassin_cross => MsgStringTable[1040]="You have
///     equipped throwing daggers."
...
void clif_arrow_create_list(struct map_session_data *sd) {
	...
	p->packetLength = sizeof(struct PACKET_ZC_MAKINGARROW_LIST) +
					  count * sizeof(struct PACKET_ZC_MAKINGARROW_LIST_sub);
...
void clif_changeoption_target(struct block_list *bl, struct block_list *target) {
	...
	if (target == nullptr) {
		...
		// There is probably an option for the provoke icon, but as we don't know it, we have to do
		// this for now
		if (sc->data[SC_PROVOKE]) {
			const struct TimerData *td = get_timer(sc->data[SC_PROVOKE]->timer);

			clif_status_change(bl, status_db.getIcon(SC_PROVOKE), 1,
							   (!td ? INFINITE_TICK : DIFF_TICK(td->tick, gettick())), 0, 0, 0);
		}
	} else {
		if (disguised(bl)) {
			p.AID = disguised_bl_id(p.AID);
...
void clif_dispchat(struct chat_data *cd, int fd) {
	...
	type = (cd->owner->type == BL_PC) ? (cd->pub) ? 1 : 0
									  : (cd->owner->type == BL_NPC) ? (cd->limit) ? 2 : 3 : 1;
...
/// 00e9 <amount>.L <nameid>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W
/// <card4>.W (ZC_ADD_EXCHANGE_ITEM) 080f <nameid>.W <item type>.B <amount>.L <identified>.B
/// <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W (ZC_ADD_EXCHANGE_ITEM2) 0a09
/// <nameid>.W <item type>.B <amount>.L <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W { <option id>.W <option value>.W <option param>.B }*5
/// (ZC_ADD_EXCHANGE_ITEM3)
void clif_tradeadditem(struct map_session_data *sd, struct map_session_data *tsd, int index,
					   int amount) {
	nullpo_retv(sd);
	nullpo_retv(tsd);
...
/// Notifies the client about the result of request to add an item to the current trade
/// (ZC_ACK_ADD_EXCHANGE_ITEM). 00ea <index>.W <result>.B result:
///     0 = success
///     1 = overweight
///     2 = trade canceled
...
/// 00f4 <index>.W <amount>.L <nameid>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W (ZC_ADD_ITEM_TO_STORE) 01c4 <index>.W <amount>.L <nameid>.W <type>.B
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W
/// (ZC_ADD_ITEM_TO_STORE2) 0a0a <index>.W <amount>.L <nameid>.W <type>.B <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W { <option id>.W <option value>.W <option
/// param>.B }*5 (ZC_ADD_ITEM_TO_STORE3)
...
/// 0b73 <id>.L <amount>.W
void clif_soulball(struct map_session_data *sd, struct block_list *target,
				   enum send_target send_target) {
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
...
static void clif_getareachar_pc(struct map_session_data *sd, struct map_session_data *dstsd) {
	...
	if ((sd->status.party_id &&
		 dstsd->status.party_id == sd->status.party_id) ||	// Party-mate, or hpdisp setting.
		(sd->bg_id && sd->bg_id == dstsd->bg_id) ||			// BattleGround
		pc_has_permission(sd, PC_PERM_VIEW_HPMETER))
		clif_hpmeter_single(*sd, dstsd->bl.id, dstsd->battle_status.hp,
	...
	if (dstsd->sc.data[SC_DEVOTION] &&
		(d_bl = map_id2bl(dstsd->sc.data[SC_DEVOTION]->val1)) != NULL)
...
void clif_getareachar_unit(struct map_session_data *sd, struct block_list *bl) {
	...
	switch (bl->type) {
		...
		case BL_MOB: {
			...
			if (battle_config.monster_hp_bars_info && !map_getmapflag(bl->m, MF_HIDEMOBHPBAR)) {
				int i;
				for (i = 0; i < DAMAGELOG_SIZE;
					 i++)  // must show hp bar to all char who already hit the mob.
...
static enum e_damage_type clif_calc_delay(enum e_damage_type type, int div, int64 damage,
										  int delay) {
 ...
 *------------------------------------------*/
static int clif_calc_walkdelay(struct block_list *bl, int delay, char type, int64 damage,
							   int div_) {
	...
	return (delay > 0) ? delay : 1;	 // Return 1 to specify there should be no noticeable delay, but
...
/// 008a <src ID>.L <dst ID>.L <server tick>.L <src speed>.L <dst speed>.L <damage>.W <div>.W
/// <type>.B <damage2>.W (ZC_NOTIFY_ACT) 02e1 <src ID>.L <dst ID>.L <server tick>.L <src speed>.L
/// <dst speed>.L <damage>.L <div>.W <type>.B <damage2>.L (ZC_NOTIFY_ACT2) 08c8 <src ID>.L <dst
/// ID>.L <server tick>.L <src speed>.L <dst speed>.L <damage>.L <IsSPDamage>.B <div>.W <type>.B
/// <damage2>.L (ZC_NOTIFY_ACT3) type:
///     0 = damage [ damage: total damage, div: amount of hits, damage2: assassin dual-wield damage
///     ] 1 = pick up item 2 = sit down 3 = stand up 4 = damage (endure) 5 = (splash?) 6 = (skill?)
///     7 = (repeat damage?)
///     8 = multi-hit damage
...
int clif_damage(struct block_list *src, struct block_list *dst, t_tick tick, int sdelay, int ddelay,
				int64 sdamage, int div, enum e_damage_type type, int64 sdamage2, bool spdamage) {
...
/// 01c9 <id>.L <creator id>.L <x>.W <y>.W <unit id>.B <visible>.B <has msg>.B <msg>.80B
/// (ZC_SKILL_ENTRY2)
...
/// 08c7 <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.B <range>.W <visible>.B
/// (ZC_SKILL_ENTRY3) 099f <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.L <range>.W
/// <visible>.B (ZC_SKILL_ENTRY4) 09ca <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.L
/// <range>.B <visible>.B <skill level>.B (ZC_SKILL_ENTRY5)
void clif_getareachar_skillunit(struct block_list *bl, struct skill_unit *unit,
								enum send_target target, bool visible) {
...
/// 09ca <lenght>.W <id> L <creator id>.L <x>.W <y>.W <unit id>.L <range>.B <visible>.B <skill
/// level>.B (ZC_SKILL_ENTRY5)
void clif_skill_unit_test(struct block_list *bl, short x, short y, int unit_id, short range,
						  short skill_lv) {
	unsigned char buf[128];
...
/// 010f <packet len>.W { <skill id>.W <type>.L <level>.W <sp cost>.W <attack range>.W <skill
/// name>.24B <upgradable>.B }*
void clif_skillinfoblock(struct map_session_data *sd) {
	...
	for (i = 0, len = 4; i < MAX_SKILL; i++) {
		if ((id = sd->status.skill[i].id) != 0) {
			...
			if (sd->status.skill[i].flag == SKILL_FLAG_PERMANENT)
				WFIFOB(fd, len + 36) =
					(sd->status.skill[i].lv < skill_tree_get_max(id, sd->status.class_)) ? 1 : 0;
			else
				WFIFOB(fd, len + 36) = 0;
			len += 37;
		}
	...
	// workaround for bugreport:5348; send the remaining skills one by one to bypass packet size
	// limit
	for (; i < MAX_SKILL; i++) {
...
/// 0111 <skill id>.W <type>.L <level>.W <sp cost>.W <attack range>.W <skill name>.24B
/// <upgradable>.B
void clif_addskill(struct map_session_data *sd, int skill_id) {
	...
	if (sd->status.skill[idx].flag == SKILL_FLAG_PERMANENT)
		WFIFOB(fd, 38) =
			(sd->status.skill[idx].lv < skill_tree_get_max(skill_id, sd->status.class_)) ? 1 : 0;
	else
		WFIFOB(fd, 38) = 0;
...
void clif_skill_scale(struct block_list *bl, int src_id, int x, int y, uint16 skill_id,
					  uint16 skill_lv, int casttime) {
...
/// 07fb <src id>.L <dst id>.L <x>.W <y>.W <skill id>.W <property>.L <delaytime>.L <is disposable>.B
/// (ZC_USESKILL_ACK2) property:
///     0 = Yellow cast aura
...
///     1 = no text
void clif_skillcasting(struct block_list *bl, int src_id, int dst_id, int dst_x, int dst_y,
					   uint16 skill_id, uint16 skill_lv, int property, int casttime) {
...
/// suggesting this is an ACK packet for the UseSkill packets and should be sent on success too
/// [FlavioJS]
void clif_skill_fail(struct map_session_data *sd, uint16 skill_id, enum useskill_fail_cause cause,
					 int btype, t_itemid itemId) {
	nullpo_retv(sd);
...
/// 0114 <skill id>.W <src id>.L <dst id>.L <tick>.L <src delay>.L <dst delay>.L <damage>.W
/// <level>.W <div>.W <type>.B (ZC_NOTIFY_SKILL) 01de <skill id>.W <src id>.L <dst id>.L <tick>.L
/// <src delay>.L <dst delay>.L <damage>.L <level>.W <div>.W <type>.B (ZC_NOTIFY_SKILL2)
int clif_skill_damage(struct block_list *src, struct block_list *dst, t_tick tick, int sdelay,
					  int ddelay, int64 sdamage, int div, uint16 skill_id, uint16 skill_lv,
	...
	// Because the damage delay must be synced with the client, here is where the can-walk tick must
	// be updated. [Skotlex]
	return clif_calc_walkdelay(dst, ddelay, type, damage, div);
}

/// Ground skill attack effect and damage (ZC_NOTIFY_SKILL_POSITION).
/// 0115 <skill id>.W <src id>.L <dst id>.L <tick>.L <src delay>.L <dst delay>.L <x>.W <y>.W
/// <damage>.W <level>.W <div>.W <type>.B
/*
int clif_skill_damage2(struct block_list *src,struct block_list *dst,t_tick tick,int sdelay,int
ddelay,int damage,int div,uint16 skill_id,uint16 skill_lv,enum e_damage_type type)
{
	unsigned char buf[64];
	struct status_change *sc;
	...
	//Because the damage delay must be synced with the client, here is where the can-walk tick must
be updated. [Skotlex] return clif_calc_walkdelay(dst,ddelay,type,damage,div);
}
*/
...
bool clif_skill_nodamage(struct block_list *src, struct block_list *dst, uint16 skill_id, int heal,
						 t_tick tick) {
...
void clif_skill_poseffect(struct block_list *src, uint16 skill_id, int val, int x, int y,
						  t_tick tick) {
	unsigned char buf[32];
...
void clif_skill_warppoint(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv,
						  unsigned short map1, unsigned short map2, unsigned short map3,
						  unsigned short map4) {
	int fd;
	nullpo_retv(sd);
	fd = sd->fd;
...
///     <water%>.B <earth%>.B <fire%>.B <wind%>.B <poison%>.B <holy%>.B <shadow%>.B <ghost%>.B
///     <undead%>.B
...
void clif_skill_produce_mix_list(struct map_session_data *sd, int skill_id, int trigger) {
	...
	WFIFOHEAD(fd, sizeof(struct PACKET_ZC_MAKABLEITEMLIST) +
					  MAX_SKILL_PRODUCE_DB * sizeof(struct PACKET_ZC_MAKABLEITEMLIST_sub));
	...
	p->packetLength = sizeof(struct PACKET_ZC_MAKABLEITEMLIST) +
					  count * sizeof(struct PACKET_ZC_MAKABLEITEMLIST_sub);
	WFIFOSET(fd, p->packetLength);
...
void clif_cooking_list(struct map_session_data *sd, int trigger, uint16 skill_id, int qty,
					   int list_type) {
	nullpo_retv(sd);
	...
	WFIFOHEAD(fd, sizeof(struct PACKET_ZC_MAKINGITEM_LIST) +
					  MAX_SKILL_PRODUCE_DB * sizeof(struct PACKET_ZC_MAKINGITEM_LIST_sub));
	...
	if (count > 0 || skill_id == AM_PHARMACY) {
		p->packetLength = sizeof(struct PACKET_ZC_MAKINGITEM_LIST) +
						  count * sizeof(struct PACKET_ZC_MAKINGITEM_LIST_sub);
		WFIFOSET(fd, p->packetLength);
...
/// 0196 <index>.W <id>.L <state>.B (ZC_MSG_STATE_CHANGE) [used for ending status changes and
/// starting them on non-pc units (when needed)] 043f <index>.W <id>.L <state>.B <remain msec>.L {
/// <val>.L }*3 (ZC_MSG_STATE_CHANGE2) [used exclusively for starting statuses on pcs] 0983
/// <index>.W <id>.L <state>.B <total msec>.L <remain msec>.L { <val>.L }*3 (ZC_MSG_STATE_CHANGE3)
/// (PACKETVER >= 20120618)
...
/// @param val2
/// @param val3
void clif_status_change_sub(struct block_list *bl, int id, int type, int flag, t_tick tick,
							int val1, int val2, int val3, enum send_target target_type) {
	unsigned char buf[32];
	...
	if (type == EFST_LUNARSTANCE || type == EFST_UNIVERSESTANCE || type == EFST_SUNSTANCE ||
		type == EFST_STARSTANCE)
		tick = 200;
	...
	if (flag && battle_config.display_status_timers) {
		...
		WBUFL(buf, 9) =
			client_tick(tick); /* at this stage remain and total are the same value I believe */
 ...
 * @param val3
 */
void clif_status_change(struct block_list *bl, int type, int flag, t_tick tick, int val1, int val2,
						int val3) {
	struct map_session_data *sd = NULL;
	...
	if (!(status_efst_get_bl_type((efst_type)type) &
		  bl->type))  // only send status changes that actually matter to the client
		return;

	clif_status_change_sub(bl, bl->id, type, flag, tick, val1, val2, val3,
						   ((sd ? (pc_isinvisible(sd) ? SELF : AREA) : AREA_WOS)));
}
...
void clif_efst_status_change_sub(struct block_list *tbl, struct block_list *bl,
	...
	for (i = 0; i < sc_display_count; i++) {
		...
		const struct TimerData *td =
			(sc && sc->data[type] ? get_timer(sc->data[type]->timer) : NULL);
		t_tick tick = 0;
		...
		clif_efst_status_change(tbl, bl->id, target, status_db.getIcon(type), tick,
								sc_display[i]->val1, sc_display[i]->val2, sc_display[i]->val3);
#else
		clif_status_change_sub(tbl, bl->id, status_db.getIcon(type), 1, tick, sc_display[i]->val1,
							   sc_display[i]->val2, sc_display[i]->val3, target);
#endif
	}
}
...
/// 0984 <id>.L <index>.W <total msec>.L <remain msec>.L { <val>.L }*3 (ZC_EFST_SET_ENTER2)
/// (PACKETVER >= 20120618)
void clif_efst_status_change(struct block_list *bl, int tid, enum send_target target, int type,
							 t_tick tick, int val1, int val2, int val3) {
...
void clif_displaymessage(const int fd, const char *mes) {
	...
	if (session_isActive(fd)) {
		...
		while (line != NULL) {
			...
			/** for some reason game client crashes depending on message pattern (only for this
			 * packet) **/
...
#else
			// Limit message to 255+1 characters (otherwise it causes a buffer overflow in the
			// client)
			int len = strnlen(line, CHAT_SIZE_MAX);

			if (len > 0) {	// don't send a void message (it's not displaying on the client chat).
							// @help can send void line.
				WFIFOHEAD(fd, 5 + len);
				WFIFOW(fd, 0) = 0x8e;
...
void clif_broadcast(struct block_list *bl, const char *mes, int len, int type,
					enum send_target target) {
	nullpo_retv(mes);
	if (len < 2)
		return;
	...
	if ((type & BC_BLUE) != 0) {
		...
		// If there's "blue" at the beginning of the message, game client will display it in blue
		// instead of yellow.
...
/// (ZC_BROADCAST2)
void clif_broadcast2(struct block_list *bl, const char *mes, int len, unsigned long fontColor,
					 short fontType, short fontSize, short fontAlign, short fontY,
					 enum send_target target) {
	nullpo_retv(mes);
	if (len < 2)
		return;
...
void clif_channel_msg(struct Channel *channel, const char *msg, unsigned long color) {
	...
	for (user = (struct map_session_data *)dbi_first(iter); dbi_exists(iter);
		 user = (struct map_session_data *)dbi_next(iter)) {
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t) {
	...
	WBUFL(buf, 4) =
		((mapdata->flag[MF_PVP] || (sd && sd->duel_group > 0))
		...
		((mapdata->flag[MF_FORCEMINEFFECT] || mapdata_flag_gvg2(mapdata))
		 << 3) |  // USE_SIMPLE_EFFECT - Forces simpler skill effects, like /mineffect command
		((mapdata->flag[MF_NOLOCKON] || mapdata_flag_vs(mapdata) || (sd && sd->duel_group > 0))
		 << 4) |  // DISABLE_LOCKON - Only allow attacks on other players with shift key or /ns
				  // active
		((mapdata->flag[MF_PVP]) << 5) |  // COUNT_PK - Show the PvP counter
		((mapdata->flag[MF_PARTYLOCK])
		 << 6) |  // NO_PARTY_FORMATION - Prevents party creation/modification (Might be used for
				  // instance dungeons)
		((mapdata->flag[MF_BATTLEGROUND])
...
///     2 = "cannot upgrade %s until you level up the upgrade weapon skill" MsgStringTable[913] in
///     rgb(255,200,200) 3 = "you lack the item %s to upgrade the weapon" MsgStringTable[914] in
///     rgb(255,200,200)
...
void clif_wis_message(struct map_session_data *sd, const char *nick, const char *mes, int mes_len,
					  int gmlvl) {
	int fd;
...
void clif_wis_end(int fd, int result) {
	struct map_session_data *sd =
		(session_isActive(fd) ? (struct map_session_data *)session[fd]->session_data : NULL);
#if PACKETVER < 20131223
	const int cmd = 0x98;
#else
	const int cmd = 0x9df;
...
void clif_use_card(struct map_session_data *sd, int idx) {
	...
	for (i = c = 0; i < MAX_INVENTORY; i++) {
		...
		if (sd->inventory_data[i]->type == IT_WEAPON &&
			ep == EQP_SHIELD)  // Shield card won't go on left weapon.
			continue;

		if (sd->inventory_data[i]->type == IT_ARMOR && (ep & EQP_ACC) &&
			((ep & EQP_ACC) != EQP_ACC) &&
			((sd->inventory_data[i]->equip & EQP_ACC) !=
			 (ep &
			  EQP_ACC)))  // specific accessory-card can only be inserted to specific accessory.
			continue;

		ARR_FIND(0, sd->inventory_data[i]->slots, j,
				 sd->inventory.u.items_inventory[i].card[j] == 0);
		if (j == sd->inventory_data[i]->slots)	// No room
			continue;

		if (sd->inventory.u.items_inventory[i].equip >
			0)	// Do not check items that are already equipped
			continue;
...
void clif_item_repair_list(struct map_session_data *sd, struct map_session_data *dstsd, int lv) {
	...
	int len =
		MAX_INVENTORY * sizeof(struct REPAIRITEM_INFO) + sizeof(struct PACKET_ZC_REPAIRITEMLIST);
	...
	for (int i = 0; i < MAX_INVENTORY; i++) {
		if (dstsd->inventory.u.items_inventory[i].nameid > 0 &&
			...
			!itemdb_ishatched_egg(
				&dstsd->inventory.u.items_inventory[i])) {	// && skill_can_repair(sd,nameid)){
			p->items[c].index = i;
...
void clif_item_damaged(struct map_session_data *sd, unsigned short position) {
	...
	WFIFOL(fd, 4) = sd->bl.id;	// TODO: the packet seems to be sent to other people as well,
								// probably party and/or guild.
	WFIFOSET(fd, packet_len(0x2bb));
}

/// Presents a list of weapon items that can be refined [Taken from jAthena]
/// (ZC_NOTIFY_WEAPONITEMLIST). 0221 <packet len>.W { <index>.W <name id>.W <refine>.B <card1>.W
/// <card2>.W <card3>.W <card4>.W }*
void clif_item_refine_list(struct map_session_data *sd) {
	nullpo_retv(sd);

	struct PACKET_ZC_NOTIFY_WEAPONITEMLIST *p =
		(struct PACKET_ZC_NOTIFY_WEAPONITEMLIST *)packet_buffer;
	...
	for (int i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid > 0 &&
			sd->inventory.u.items_inventory[i].refine < skill_lv &&
			sd->inventory_data[i] != nullptr && sd->inventory_data[i]->type == IT_WEAPON &&
			sd->inventory.u.items_inventory[i].identify &&
			sd->inventory_data[i]->weapon_level >= 1 &&
	...
	p->packetLength = sizeof(struct PACKET_ZC_NOTIFY_WEAPONITEMLIST) +
					  sizeof(struct PACKET_ZC_NOTIFY_WEAPONITEMLIST_sub) * count;
...
/// 0124 <index>.W <amount>.L <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W (ZC_ADD_ITEM_TO_CART) 01c5 <index>.W <amount>.L <name id>.W <type>.B
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W
/// (ZC_ADD_ITEM_TO_CART2) 0a0b <index>.W <amount>.L <name id>.W <type>.B <identified>.B <damaged>.B
/// <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W { <option id>.W <option value>.W <option
/// param>.B }*5 (ZC_ADD_ITEM_TO_CART3)
...
void clif_parse_BankOpen(int fd, struct map_session_data *sd) {
	...
	if (!battle_config.feature_banking) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 1496), false,
						  SELF);  // Banking is disabled
		return;
	} else {
...
void clif_showvendingboard(struct map_session_data &sd, enum send_target target,
						   struct block_list *tbl) {
	if (tbl == nullptr) {
		tbl = &sd.bl;
		target = AREA_WOS;
...
/// R 0133 <packet len>.W <owner id>.L { <price>.L <amount>.W <index>.W <type>.B <name id>.W
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }*
/// (ZC_PC_PURCHASE_ITEMLIST_FROMMC) R 0800 <packet len>.W <owner id>.L <unique id>.L { <price>.L
/// <amount>.W <index>.W <type>.B <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W
/// <card2>.W <card3>.W <card4>.W }* (ZC_PC_PURCHASE_ITEMLIST_FROMMC2)
void clif_vendinglist(struct map_session_data *sd, struct map_session_data *vsd) {
	...
	struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC *p =
		(struct PACKET_ZC_PC_PURCHASE_ITEMLIST_FROMMC *)WFIFOP(fd, 0);
...
/// 0136 <packet len>.W <owner id>.L { <price>.L <index>.W <amount>.W <type>.B <name id>.W
/// <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W }*
...
void clif_openvending(struct map_session_data *sd, int id, struct s_vending *vending) {
	...
	struct PACKET_ZC_PC_PURCHASE_MYITEMLIST *p =
		(struct PACKET_ZC_PC_PURCHASE_MYITEMLIST *)WFIFOP(fd, 0);
...
void clif_vendingreport(struct map_session_data *sd, int index, int amount, uint32 char_id,
						int zeny) {
...
/// 0104 <account id>.L <role>.L <x>.W <y>.W <state>.B <party name>.24B <char name>.24B <map
/// name>.16B (ZC_ADD_MEMBER_TO_GROUP) 01e9 <account id>.L <role>.L <x>.W <y>.W <state>.B <party
/// name>.24B <char name>.24B <map name>.16B <item pickup rule>.B <item share rule>.B
/// (ZC_ADD_MEMBER_TO_GROUP2) 0a43 <account id>.L <role>.L <class>.W <base level>.W <x>.W <y>.W
/// <state>.B <party name>.24B <char name>.24B <map name>.16B <item pickup rule>.B <item share
/// rule>.B (ZC_ADD_MEMBER_TO_GROUP3) role:
///     0 = leader
///     1 = normal
/// state:
///     0 = connected
...
/// 00fb <packet len>.W <party name>.24B { <account id>.L <nick>.24B <map name>.16B <role>.B
/// <state>.B }* role:
///     0 = leader
///     1 = normal
/// state:
///     0 = connected
///     1 = disconnected
/// 0a44 <packet len>.W <party name>.24B { <account id>.L <nick>.24B <map name>.16B <role>.B
/// <state>.B <class>.W <base level>.W }* <item pickup rule>.B <item share rule>.B <unknown>.L
...
/// result=8  : (%s) are currently in restricted map to join a party. -> MsgStringTable[1388] (since
/// 20080527) result=9  : Cannot join a party in this map. -> MsgStringTable[1871] (since 20110215)
/// result=10 : You cannot invite or withdraw while in memorial dungeon -> message: MSG_ID_BD3
/// (since 20161130) result=11 : The character is a level that can not join the party -> message:
/// MSG_ID_C9A (since 20170412)
void clif_party_invite_reply(struct map_session_data &sd, const char *nick,
							 enum e_party_invite_reply reply) {
 ...
 **/
void clif_party_withdraw(struct map_session_data &sd, uint32 account_id, const char *name,
						 enum e_party_member_withdraw result, enum send_target target) {
...
void clif_party_message(struct party_data &party, uint32 account_id, const char *mes, int len) {
	...
	if (len > CHAT_SIZE_MAX) {
		ShowWarning("clif_party_message: Truncated message '%s' (len=%d, max=%" PRIuPTR
					", party_id=%d).\n",
...
void clif_sendegg(struct map_session_data *sd) {
	...
	if (battle_config.pet_no_gvg &&
		map_flag_gvg2(
			sd->bl.m)) {  // Disable pet hatching in GvG grounds during Guild Wars [Skotlex]
...
#ifdef RENEWAL
	uint16 autospell_skill[][2] = {{MG_FIREBOLT, 0},   {MG_COLDBOLT, 0},	 {MG_LIGHTNINGBOLT, 0},
								   {MG_SOULSTRIKE, 3}, {MG_FIREBALL, 3},	 {WZ_EARTHSPIKE, 6},
								   {MG_FROSTDIVER, 6}, {MG_THUNDERSTORM, 9}, {WZ_HEAVENDRIVE, 9}};
	int count = 0;
...
void clif_devotion(struct block_list *src, struct map_session_data *tsd) {
	...
	if (src->type == BL_MER) {
		...
		WBUFW(buf, 26) =
			skill_get_range2(src, ML_DEVOTION, mercenary_checkskill(md, ML_DEVOTION), false);
	} else {
		int i;
...
/// 01d0 <id>.L <amount>.W (ZC_SPIRITS)
/// 01e1 <id>.L <amount>.W (ZC_SPIRITS2)
void clif_spiritball(struct block_list *bl, struct block_list *target,
					 enum send_target send_target) {
	nullpo_retv(bl);
...
void clif_mvp_exp(struct map_session_data *sd, t_exp exp) {
	...
	if (battle_config.mvp_exp_reward_message) {
		...
		clif_messagecolor(
			&sd->bl, color_table[COLOR_CYAN], e_msg, false,
...
/// 01f2 <account id>.L <char id>.L <status>.L <gender>.W <hair style>.W <hair color>.W
/// (ZC_UPDATE_CHARSTAT2) status:
///     0 = offline
///     1 = online
...
/// 0150 <guild id>.L <level>.L <member num>.L <member max>.L <exp>.L <max exp>.L <points>.L
/// <honor>.L <virtue>.L <emblem id>.L <name>.24B <master name>.24B <manage land>.16B
/// (ZC_GUILD_INFO) 01b6 <guild id>.L <level>.L <member num>.L <member max>.L <exp>.L <max exp>.L
/// <points>.L <honor>.L <virtue>.L <emblem id>.L <name>.24B <master name>.24B <manage land>.16B
/// <zeny>.L (ZC_GUILD_INFO2) 0a84 <guild id>.L <level>.L <member num>.L <member max>.L <exp>.L <max
/// exp>.L <points>.L <honor>.L <virtue>.L <emblem id>.L <name>.24B <manage land>.16B <zeny>.L
/// <master char id>.L (ZC_GUILD_INFO3)
...
/// 0154 <packet len>.W { <account>.L <char id>.L <hair style>.W <hair color>.W <gender>.W <class>.W
/// <level>.W <contrib exp>.L <state>.L <position>.L <memo>.50B <name>.24B }* (ZC_MEMBERMGR_INFO)
/// state:
///     0 = offline
///     1 = online
/// memo:
///     probably member's self-introduction (unused, no client UI/packets for editing it)
/// 0aa5 <packet len>.W { <account>.L <char id>.L <hair style>.W <hair color>.W <gender>.W <class>.W
/// <level>.W <contrib exp>.L <state>.L <position>.L <lastlogin>.L }* (ZC_MEMBERMGR_INFO2)
...
/// (ZC_ACK_CHANGE_GUILD_POSITIONINFO). 0174 <packet len>.W { <position id>.L <mode>.L <ranking>.L
/// <pay rate>.L <position name>.24B }* mode:
///     &0x01 = allow invite
///     &0x10 = allow expel
/// ranking:
///     TODO
...
/// (ZC_ACK_REQ_CHANGE_MEMBERS). 0156 <packet len>.W { <account id>.L <char id>.L <position id>.L }*
...
/// 0162 <packet len>.W <skill points>.W { <skill id>.W <type>.L <level>.W <sp cost>.W <atk range>.W
/// <skill name>.24B <upgradable>.B }*
void clif_guild_skillinfo(struct map_session_data *sd) {
	...
	for (i = 0, c = 0; i < MAX_GUILDSKILL; i++) {
		if (g->skill[i].id > 0 && guild_check_skill_require(g, g->skill[i].id)) {
			...
			WFIFOB(fd, p + 36) =
				(g->skill[i].lv < guild_skill_get_max(id) && sd == g->member[0].sd) ? 1 : 0;
			c++;
		}
	}
	WFIFOW(fd, 2) = 6 + c * 37;
...
void clif_guild_expulsion(struct map_session_data *sd, const char *name, const char *mes,
						  uint32 account_id) {
	unsigned char buf[128];
...
#if PACKETVER < 20100803
			memset(WFIFOP(fd, 4 + c * offset + 24), 0,
				   NAME_LENGTH);  // account name (not used for security reasons)
...
void clif_guild_message(
	struct guild *g, uint32 account_id, const char *mes,
	int len) {	// TODO: account_id is not used, candidate for deletion? [Ai4rei]
	struct map_session_data *sd;
	uint8 buf[256];
	...
	if (len == 0) {
		...
		ShowWarning("clif_guild_message: Truncated message '%s' (len=%d, max=%" PRIuPTR
					", guild_id=%d).\n",
...
void clif_disp_message(struct block_list *src, const char *mes, int len, enum send_target target) {
	...
	if (len == 0) {
		...
		ShowWarning("clif_disp_message: Truncated message '%s' (len=%d, max=%" PRIuPTR
					", aid=%d).\n",
...
/// Notifies the client about the result of a request to disconnect another player
/// (ZC_ACK_DISCONNECT_CHARACTER). 00cd <result>.L (unknown packet version or invalid information at
/// packet_len_table) 00cd <result>.B result:
///     0 = failure
///     1 = success
...
/// (ZC_NOTIFY_MANNER_POINT_GIVEN). 014b <type>.B <GM name>.24B type:
///     0 = positive (unmute)
///     1 = negative (mute)
...
/// (ZC_SETTING_WHISPER_PC). 00d1 <type>.B <result>.B type:
///     0 = /ex (deny)
///     1 = /in (allow)
/// result:
///     0 = success
...
/// (ZC_SETTING_WHISPER_STATE). 00d2 <type>.B <result>.B type:
///     0 = /exall (deny)
///     1 = /inall (allow)
/// result:
///     0 = success
...
void clif_specialeffect_remove(struct block_list *bl_src, int effect, enum send_target e_target,
...
void clif_messagecolor_target(struct block_list *bl, unsigned long color, const char *msg,
							  bool rgb2bgr, enum send_target type, struct map_session_data *sd) {
	...
	if (rgb2bgr)
		color =
			(color & 0x0000FF) << 16 | (color & 0x00FF00) | (color & 0xFF0000) >> 16;  // RGB to BGR
...
void clif_refresh_storagewindow(struct map_session_data *sd) {
	...
	if (sd->state.storage_flag == 3) {
		storage_sortitem(sd->premiumStorage.u.items_storage,
						 ARRAYLENGTH(sd->premiumStorage.u.items_storage));
		clif_storagelist(sd, sd->premiumStorage.u.items_storage,
						 ARRAYLENGTH(sd->premiumStorage.u.items_storage),
...
/// Needed because when you send a 0x95 packet, the client will not remove the cached party/guild
/// info that is not sent. 0095 <id>.L <char name>.24B (ZC_ACK_REQNAME) 0195 <id>.L <char name>.24B
/// <party name>.24B <guild name>.24B <position name>.24B (ZC_ACK_REQNAMEALL) 0a30 <id>.L <char
/// name>.24B <party name>.24B <guild name>.24B <position name>.24B <title ID>.L
/// (ZC_ACK_REQNAMEALL2)
	...
	switch (bl->type) {
		...
		case BL_MOB: {
			...
			if (md->guardian_data && md->guardian_data->guild_id) {
				...
				safestrncpy(packet.position_name, md->guardian_data->castle->castle_name,
							NAME_LENGTH);
				...
				if (battle_config.show_mob_info & 2) {
					str_p += sprintf(str_p, "HP: %u%% | ",
									 get_percentage(md->status.hp, md->status.max_hp));
				}

				// Even thought mobhp ain't a name, we send it as one so the client can parse it.
				// [Skotlex]
				if (str_p != mobhp) {
...
void clif_disp_overhead_(struct block_list *bl, const char *mes, enum send_target flag) {
	unsigned char buf[256];	 // This should be more than sufficient, the theorical max is CHAT_SIZE
							 // + 8 (pads and extra inserted crap)
...
void clif_starskill(struct map_session_data *sd, const char *mapname, int monster_id,
					unsigned char star, unsigned char result) {
	int fd = sd->fd;
 ...
 *------------------------------------------*/
void clif_hate_info(struct map_session_data *sd, unsigned char hate_level, int class_,
					unsigned char type) {
	if (pcdb_checkid(class_)) {
		...
		ShowWarning(
			"clif_hate_info: Received invalid class %d for this packet (char_id=%d, hate_level=%u, "
			"type=%u).\n",
 ...
 *------------------------------------------*/
void clif_feel_hate_reset(struct map_session_data *sd) { clif_starskill(sd, "", 0, 0, 30); }
...
/// 02d7 <packet len>.W <name>.24B <class>.W <hairstyle>.W <up-viewid>.W <mid-viewid>.W
/// <low-viewid>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.26B* (ZC_EQUIPWIN_MICROSCOPE)
/// 02d7 <packet len>.W <name>.24B <class>.W <hairstyle>.W <bottom-viewid>.W <mid-viewid>.W
/// <up-viewid>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.28B* (ZC_EQUIPWIN_MICROSCOPE,
/// PACKETVER >= 20100629) 0859 <packet len>.W <name>.24B <class>.W <hairstyle>.W <bottom-viewid>.W
/// <mid-viewid>.W <up-viewid>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.28B*
/// (ZC_EQUIPWIN_MICROSCOPE2, PACKETVER >= 20101124) 0859 <packet len>.W <name>.24B <class>.W
/// <hairstyle>.W <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W <robe>.W <haircolor>.W
/// <cloth-dye>.W <gender>.B {equip item}.28B* (ZC_EQUIPWIN_MICROSCOPE2, PACKETVER >= 20110111) 0997
/// <packet len>.W <name>.24B <class>.W <hairstyle>.W <bottom-viewid>.W <mid-viewid>.W <up-viewid>.W
/// <robe>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.31B* (ZC_EQUIPWIN_MICROSCOPE_V5,
/// PACKETVER >= 20120925) 0a2d <packet len>.W <name>.24B <class>.W <hairstyle>.W <bottom-viewid>.W
/// <mid-viewid>.W <up-viewid>.W <robe>.W <haircolor>.W <cloth-dye>.W <gender>.B {equip item}.57B*
/// (ZC_EQUIPWIN_MICROSCOPE_V6, PACKETVER >= 20150225)
void clif_viewequip_ack(struct map_session_data *sd, struct map_session_data *tsd) {
#if PACKETVER_AD_NUM >= 20071211 || PACKETVER_SAK_NUM >= 20071127 || \
	PACKETVER_MAIN_NUM >= 20071211 || defined(PACKETVER_RE)
	nullpo_retv(sd);
	nullpo_retv(tsd);
	...
	for (int i = 0; i < EQI_MAX; i++) {
		...
		if (k >= 0) {
			if (tsd->inventory.u.items_inventory[k].nameid == 0 ||
				tsd->inventory_data[k] == NULL) {  // Item doesn't exist
				continue;
			}
			...
			clif_item_equip(client_index(k), &p->list[equip++],
							&tsd->inventory.u.items_inventory[k], tsd->inventory_data[k],
							pc_equippoint(tsd, k));
		}
	}
...
static bool clif_process_message(struct map_session_data *sd, bool whisperFormat, char *out_name,
								 char *out_message, char *out_full_message) {
	...
	if (whisperFormat) {
		...
		if (inputLength < NAME_LENGTH + 1) {
			ShowWarning(
				"clif_process_message: Received malformed packet from player '%s' (packet length "
				"is incorrect)!\n",
				sd->status.name);
			return false;
		}
		...
		if (inputLength < nameLength + seperatorLength + 1) {
			ShowWarning(
				"clif_process_message: Received malformed packet from player '%s' (no username "
				"data)!\n",
				sd->status.name);
			return false;
		}
		...
		// validate name
		if (strncmp(name, sd->status.name,
					nameLength) ||	// the text must start with the speaker's name
			strncmp(name + nameLength, seperator, seperatorLength))	 // followed by the seperator
		{
			// Hacked message, or infamous "client desynch" issue where they pick one char while
			// loading another.
			ShowWarning(
				"clif_process_message: Player '%s' sent a message using an incorrect name! Forcing "
				"a relog...\n",
				sd->status.name);
	...
	if (messageLength != strnlen(message, messageLength) + 1) {
		ShowWarning(
			"clif_process_message: Received malformed packet from player '%s' (length is "
			"incorrect)!\n",
			sd->status.name);
		return false;
	}
	...
	if (messageLength > CHAT_SIZE_MAX - 1) {
		// Normally you can only enter CHATBOX_SIZE-1 letters into the chat box, but Frost Joke /
		// Dazzler's text can be longer. Also, the physical size of strings that use multibyte
		// encoding can go multiple times over the chatbox capacity. Neither the official client nor
		// server place any restriction on the length of the data in the packet, but we'll only
		// allow reasonably long strings here. This also makes sure that they fit into the `chatlog`
		// table.
	...
	// achievement_update_objective(sd, AG_CHATTING, 1, 1); // !TODO: Confirm how this achievement
	// is triggered
...
inline void clif_pk_mode_message(struct map_session_data *sd) {
	if (battle_config.pk_mode && battle_config.pk_mode_mes && sd &&
...
static int clif_parse_WantToConnection_sub(int fd) {
	...
	else if ((value = (int)RFIFOL(fd, packet_db[cmd].pos[0])) < START_ACCOUNT_NUM ||
			 value > END_ACCOUNT_NUM)
...
void clif_parse_WantToConnection(int fd, struct map_session_data *sd) {
	...
	if (err) {	// connection rejected
		ShowInfo(
			"clif_parse: Disconnecting session #%d with unknown connect packet "
			"0x%04x(length:%d)%s\n",
			fd, cmd, RFIFOREST(fd),
			(err == 1
				 ? "."
				 : err == 2
					   ? ", possibly for having an invalid account_id."
					   : err == 3
							 ? ", possibly for having an invalid char_id."
							 :
							 /* Uncomment when checks are added in clif_parse_WantToConnection_sub.
							 [FlavioJS] err == 4 ? ", possibly for having an invalid login_id1." :
	...
	if (bl && bl->type != BL_PC) {
		ShowError(
			"clif_parse_WantToConnection: a non-player object already has id %d, please increase "
			"the starting account number\n",
			account_id);
		WFIFOHEAD(fd, packet_len(0x6a));
		WFIFOW(fd, 0) = 0x6a;
	...
	if (bl ||
		((node = chrif_search(
			  account_id)) &&  // An already existing node is valid only if it is for this login.
		 !(node->account_id == account_id && node->char_id == char_id &&
		   node->state == ST_LOGIN))) {
	...
	sd->cryptKey = (((((clif_cryptKey[0] * clif_cryptKey[1]) + clif_cryptKey[2]) & 0xFFFFFFFF) *
					 clif_cryptKey[1]) +
					clif_cryptKey[2]) &
				   0xFFFFFFFF;
#endif
...
/// Notification from the client, that it has finished map loading and is about to display player's
/// character (CZ_NOTIFY_ACTORINIT). 007d
void clif_parse_LoadEndAck(int fd, struct map_session_data *sd) {
	...
	// Autotraders should ignore this entirely, clif_parse_LoadEndAck is always invoked manually for
	// them
	if (!sd->state.active ||
		(!sd->state.autotrade && !sd->state.pc_loaded)) {  // Character loading is not complete yet!
	...
	// item
	clif_inventorylist(sd);	 // inventory list first, otherwise deleted items in pc_checkitem show
							 // up as 'unknown item'
	pc_checkitem(sd);
	clif_equipswitch_list(sd);
	...
	if (!pc_isinvisible(sd) && mapdata->flag[MF_PVP]) {
		...
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE,
						  SELF);  // TODO: Figure out the real packet to send here.
	...
	map_foreachinallarea(clif_getareachar, sd->bl.m, sd->bl.x - AREA_SIZE, sd->bl.y - AREA_SIZE,
						 sd->bl.x + AREA_SIZE, sd->bl.y + AREA_SIZE, BL_ALL, sd);

	// pet
	if (sd->pd) {
		if (battle_config.pet_no_gvg &&
			mapdata_flag_gvg(mapdata)) {  // Return the pet to egg. [Skotlex]
	...
	if (hom_is_active(sd->hd)) {
		...
		clif_hominfo(sd, sd->hd, 0);  // for some reason, at least older clients want this sent
									  // twice
		clif_homskillinfoblock(sd);
		if (battle_config.hom_setting & HOMSET_COPY_SPEED)
			status_calc_bl(&sd->hd->bl,
						   {SCB_SPEED});  // Homunc mimic their master's speed on each map change
	...
	if (sd->md) {
		...
		clif_mercenary_info(sd);
		clif_mercenary_skillblock(sd);
		status_calc_bl(&sd->md->bl,
					   {SCB_SPEED});  // Mercenary mimic their master's speed on each map change
	}
	...
	if (sd->ed) {
		...
		clif_hpmeter_single(*sd, sd->ed->bl.id, sd->ed->battle_status.hp,
							sd->ed->battle_status.max_hp);
		clif_elemental_updatestatus(sd, SP_SP);
		status_calc_bl(&sd->ed->bl,
					   {SCB_SPEED});  // Elemental mimic their master's speed on each map change
	}
	...
	if (sd->state.connect_new) {
		...
		if ((lv = pc_checkskill(sd, SG_KNOWLEDGE)) > 0) {
			if (sd->bl.m == sd->feel_map[0].m || sd->bl.m == sd->feel_map[1].m ||
				sd->bl.m == sd->feel_map[2].m)
				sc_start(&sd->bl, &sd->bl, SC_KNOWLEDGE, 100, lv, skill_get_time(SG_KNOWLEDGE, lv));
		}

		if (sd->pd && sd->pd->pet.intimate > 900)
			clif_pet_emotion(sd->pd,
							 (sd->pd->pet.class_ - 100) * 100 + 50 + pet_hungry_val(sd->pd));
		...
		if (!sd->state.autotrade) {	 // Don't trigger NPC event or opening vending/buyingstore will
									 // be failed
			// Login Event
	...
	if (sd->state.changemap) {	// restore information that gets lost on map-change
		...
		if (battle_config.bg_flee_penalty != 100 || battle_config.gvg_flee_penalty != 100) {
			...
			if ((pmap != nullptr && (mapdata_flag_gvg(pmap) || pmap->flag[MF_BATTLEGROUND])) ||
				(mapdata != nullptr &&
				 (mapdata_flag_gvg(mapdata) || mapdata->flag[MF_BATTLEGROUND])))
				status_calc_bl(&sd->bl, {SCB_FLEE});  // Refresh flee penalty
		}

		if (night_flag && mapdata->flag[MF_NIGHTENABLED]) {	 // Display night.
			if (!sd->state.night) {
				sd->state.night = 1;
				clif_status_load(&sd->bl, EFST_SKE, 1);
			}
		...
		status_calc_pc(sd,
					   sd->state.autotrade
						   ? SCO_FIRST
						   : SCO_NONE);	 // Some conditions are map-dependent so we must recalculate
		...
		if (channel_config.map_tmpl.name[0] && (channel_config.map_tmpl.opt & CHAN_OPT_AUTOJOIN) &&
			!mapdata->instance_id && !mapdata->flag[MF_NOMAPCHANNELAUTOJOIN])
	...
	if ((sd->sc.data[SC_MONSTER_TRANSFORM] || sd->sc.data[SC_ACTIVE_MONSTER_TRANSFORM]) &&
		...
		clif_displaymessage(
			sd->fd, msg_txt(sd, 731));	// Transforming into monster is not allowed in Guild Wars.
	}
...
/// 0a00 <rotate>.B { <is skill>.B <id>.L <count>.W }*38 (ZC_SHORTCUT_KEY_LIST_V3, PACKETVER >=
/// 20141022)
...
void clif_parse_HotkeyRowShift(int fd, struct map_session_data *sd) {
#if PACKETVER_MAIN_NUM >= 20190522 || PACKETVER_RE_NUM >= 20190508 || PACKETVER_ZERO_NUM >= 20190605
	const struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE2 *p =
		(struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE2 *)RFIFOP(fd, 0);
	...
	const struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE1 *p =
		(struct PACKET_CZ_SHORTCUTKEYBAR_ROTATE1 *)RFIFOP(fd, 0);
...
void clif_parse_Hotkey(int fd, struct map_session_data *sd) {
#ifdef HOTKEY_SAVING
#if PACKETVER_MAIN_NUM >= 20190522 || PACKETVER_RE_NUM >= 20190508 || PACKETVER_ZERO_NUM >= 20190605
	const struct PACKET_CZ_SHORTCUT_KEY_CHANGE2 *p =
		(struct PACKET_CZ_SHORTCUT_KEY_CHANGE2 *)RFIFOP(fd, 0);
	...
	const struct PACKET_CZ_SHORTCUT_KEY_CHANGE1 *p =
		(struct PACKET_CZ_SHORTCUT_KEY_CHANGE1 *)RFIFOP(fd, 0);
...
void clif_parse_WalkToXY(int fd, struct map_session_data *sd) {
	...
	status_change_end(&sd->bl,
					  SC_ROLLINGCUTTER);  // If you move, you lose your counters. [malufett]
...
void clif_parse_QuitGame(int fd, struct map_session_data *sd) {
	...
	if (!sd->sc.data[SC_CLOAKING] && !sd->sc.data[SC_HIDING] && !sd->sc.data[SC_CHASEWALK] &&
		!sd->sc.data[SC_CLOAKINGEXCEED] && !sd->sc.data[SC_SUHIDE] && !sd->sc.data[SC_NEWMOON] &&
...
void clif_parse_GetCharNameRequest(int fd, struct map_session_data *sd) {
	...
	/* disabled due to false positives (network lag + request name of char that's about to hide =
	race condition) sc = status_get_sc(bl); if (sc && sc->option&OPTION_INVISIBLE && !disguised(bl)
	&& bl->type != BL_NPC && //Skip hidden NPCs which can be seen using Maya Purple
		pc_get_group_level(sd) < battle_config.hack_info_GM_level
	) {
		char gm_msg[256];
		sprintf(gm_msg, "Hack on NameRequest: character '%s' (account: %d) requested the name of an
	invisible target (id: %d).\n", sd->status.name, sd->status.account_id, id); ShowWarning(gm_msg);
...
void clif_parse_GlobalMessage(int fd, struct map_session_data *sd) {
	...
	if (sd->gcbind &&
		((sd->gcbind->opt & CHAN_OPT_CAN_CHAT) || pc_has_permission(sd, PC_PERM_CHANNEL_ADMIN))) {
	...
	map_foreachinallrange(npc_chat_sub, &sd->bl, AREA_SIZE, BL_NPC, output, strlen(output),
						  &sd->bl);
#endif

	// Chat logging type 'O' / Global Chat
	log_chat(LOG_CHAT_GLOBAL, 0, sd->status.char_id, sd->status.account_id,
			 mapindex_id2name(sd->mapindex), sd->bl.x, sd->bl.y, NULL, message);
	// achievement_update_objective(sd, AG_CHAT, 1, sd->bl.m); //! TODO: What's the official use of
...
void clif_parse_Emotion(int fd, struct map_session_data *sd) {
	...
	if (battle_config.basic_skill_check == 0 || pc_checkskill(sd, NV_BASIC) >= 2 ||
		...
		if (battle_config.idletime_option & IDLE_EMOTION)
			sd->idletime = last_tick;
		if (battle_config.hom_idle_no_share && sd->hd &&
			battle_config.idletime_hom_option & IDLE_EMOTION)
			sd->idletime_hom = last_tick;
		if (battle_config.mer_idle_no_share && sd->md &&
			battle_config.idletime_mer_option & IDLE_EMOTION)
			sd->idletime_mer = last_tick;
		...
		if (battle_config.client_reshuffle_dice && emoticon >= ET_DICE1 &&
			emoticon <= ET_DICE6) {	 // re-roll dice
...
void clif_parse_ActionRequest_sub(struct map_session_data *sd, int action_type, int target_id,
								  t_tick tick) {
	if (pc_isdead(sd)) {
		clif_clearunit_area(&sd->bl, CLR_DEAD);
		return;
	...
	switch (action_type) {
		...
		case 0x07:	// continuous attack
			...
			if (battle_config.idletime_option & IDLE_ATTACK)
				sd->idletime = last_tick;
			if (battle_config.hom_idle_no_share && sd->hd &&
				battle_config.idletime_hom_option & IDLE_ATTACK)
				sd->idletime_hom = last_tick;
			if (battle_config.mer_idle_no_share && sd->md &&
				battle_config.idletime_mer_option & IDLE_ATTACK)
				sd->idletime_mer = last_tick;
		...
		case 0x02:	// sitdown
			...
			if (sd->sc.count && (sd->sc.data[SC_DANCING] ||
								 (sd->sc.data[SC_GRAVITATION] &&
								  sd->sc.data[SC_GRAVITATION]->val3 ==
									  BCT_SELF)))  // No sitting during these states either.
				break;
			...
			if (battle_config.idletime_option & IDLE_SIT)
				sd->idletime = last_tick;
			if (battle_config.hom_idle_no_share && sd->hd &&
				battle_config.idletime_hom_option & IDLE_SIT)
				sd->idletime_hom = last_tick;
			if (battle_config.mer_idle_no_share && sd->md &&
				battle_config.idletime_mer_option & IDLE_SIT)
				sd->idletime_mer = last_tick;
	...
	switch (RFIFOB(fd, packet_db[RFIFOW(fd, 0)].pos[0])) {
		...
		case 0x01:
			...
			if (!sd->sc.data[SC_CLOAKING] && !sd->sc.data[SC_HIDING] &&
				...
				!sd->sc.data[SC_SUHIDE] && !sd->sc.data[SC_NEWMOON] &&
				(!battle_config.prevent_logout || sd->canlog_tick == 0 ||
				 DIFF_TICK(gettick(), sd->canlog_tick) >
					 battle_config
						 .prevent_logout)) {  // Send to char-server for character selection.
				pc_damage_log_clear(sd, 0);
...
void clif_parse_WisMessage(int fd, struct map_session_data *sd) {
	...
	log_chat(LOG_CHAT_WHISPER, 0, sd->status.char_id, sd->status.account_id,
			 mapindex_id2name(sd->mapindex), sd->bl.x, sd->bl.y, target, message);
	...
	if (target[0] && (strncasecmp(target, "NPC:", 4) == 0) && (strlen(target) > 4)) {
		...
		if ((npc = npc_name2id(str))) {
			...
			for (i = 0; i < NUM_WHISPER_VAR; ++i) {	 // Splits the message using '#' as separators
				...
				if (split == NULL) {  // use the remaining string
					...
					for (++i; i < NUM_WHISPER_VAR; ++i) split_data[i][0] = '\0';
					break;
				}
				*split = '\0';
				safestrncpy(split_data[i], str, ARRAYLENGTH(split_data[i]));
			...
			safesnprintf(event, sizeof(event), "%s::%s", npc->exname,
						 script_config.onwhisper_event_name);
		...
		if (channel &&
			...
			if (channel_pc_haschan(sd, channel) >= 0) {	 // we are in the chan
				...
				clif_displaymessage(
					fd,
					msg_txt(sd, 1402));	 // You're not in that channel, type '@join <#channel_name>'
			}
			return;
		}
	}
	...
	if (dstsd == NULL || strcmp(dstsd->status.name, target) != 0) {
		...
		// so, we send information to inter-server, which is the only one which decide (and copy
		// correct name).
	...
	if (pc_get_group_level(sd) <= pc_get_group_level(dstsd)) {
		...
		ARR_FIND(0, MAX_IGNORE_LIST, i,
				 dstsd->ignore[i].name[0] == '\0' ||
					 strcmp(dstsd->ignore[i].name, sd->status.name) == 0);
		if (i < MAX_IGNORE_LIST &&
			dstsd->ignore[i].name[0] != '\0') {	 // source char present in ignore list
...
void clif_parse_DropItem(int fd, struct map_session_data *sd) {
	...
	for (;;) {
		...
		if (battle_config.idletime_option & IDLE_DROPITEM)
			sd->idletime = last_tick;
		if (battle_config.hom_idle_no_share && sd->hd &&
			battle_config.idletime_hom_option & IDLE_DROPITEM)
			sd->idletime_hom = last_tick;
		if (battle_config.mer_idle_no_share && sd->md &&
			battle_config.idletime_mer_option & IDLE_DROPITEM)
			sd->idletime_mer = last_tick;
...
void clif_parse_UseItem(int fd, struct map_session_data *sd) {
	...
	if ((!sd->npc_id && pc_istrading(sd)) || sd->chatID ||
		(sd->state.block_action & PCBLOCK_USEITEM)) {
	...
	if (battle_config.idletime_option & IDLE_USEITEM)
		sd->idletime = last_tick;
	if (battle_config.hom_idle_no_share && sd->hd &&
		battle_config.idletime_hom_option & IDLE_USEITEM)
		sd->idletime_hom = last_tick;
	if (battle_config.mer_idle_no_share && sd->md &&
		battle_config.idletime_mer_option & IDLE_USEITEM)
		sd->idletime_mer = last_tick;
	...
	switch (bl->type) {
		...
		case BL_NPC:
			...
			if (sd->ud.skill_id < RK_ENCHANTBLADE &&
				sd->ud.skilltimer != INVALID_TIMER) {  // Should only show an error message for
													   // non-3rd job skills with a running timer
				clif_msg(sd, WORK_IN_PROGRESS);
				break;
			}
#endif
...
void clif_parse_NpcBuyListSend(int fd, struct map_session_data *sd) {
	const struct PACKET_CZ_PC_PURCHASE_ITEMLIST *p =
		(struct PACKET_CZ_PC_PURCHASE_ITEMLIST *)RFIFOP(fd, 0);
...
void clif_parse_NpcSellListSend(int fd, struct map_session_data *sd) {
	...
	n = (p->packetLength - 4) /
		sizeof(PACKET_CZ_PC_SELL_ITEMLIST_sub);	 // (pktlen-(cmd+len))/listsize
...
void clif_parse_CreateChatRoom(int fd, struct map_session_data *sd) {
	...
	safestrncpy(s_title, title,
				min(len + 1, CHATROOM_TITLE_SIZE));	 // NOTE: assumes that safestrncpy will not
													 // access the len+1'th byte
...
/// 00e3
void clif_parse_ChatLeave(int fd, struct map_session_data *sd) { chat_leavechat(sd, 0); }
...
/// 00eb
void clif_parse_TradeOk(int fd, struct map_session_data *sd) { trade_tradeok(sd); }

/// Request to cancel current trade (CZ_CANCEL_EXCHANGE_ITEM).
/// 00ed
void clif_parse_TradeCancel(int fd, struct map_session_data *sd) { trade_tradecancel(sd); }

/// Request to commit current trade (CZ_EXEC_EXCHANGE_ITEM).
/// 00ef
void clif_parse_TradeCommit(int fd, struct map_session_data *sd) { trade_tradecommit(sd); }
...
void clif_parse_PutItemToCart(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART *p =
		(struct PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART *)RFIFOP(fd, 0);
...
void clif_parse_RemoveOption(int fd, struct map_session_data *sd) {
	if (!(sd->sc.option & (OPTION_RIDING | OPTION_FALCON | OPTION_DRAGON | OPTION_MADOGEAR))
		...
		pc_setoption(
			sd, sd->sc.option & ~(OPTION_RIDING | OPTION_FALCON | OPTION_DRAGON | OPTION_MADOGEAR));
}
...
void clif_SelectCart(struct map_session_data *sd) {
	...
	// Right now we have 10-12, tested it you can also enable selection for all cart styles
	// here(1-12)
	for (i = 0; i < carts; i++) {
		WFIFOB(fd, 8 + i) = 10 + i;
	}
...
void clif_parse_ChangeCart(int fd, struct map_session_data *sd) {  // TODO: State tracking?
	...
	if (
		...
		(type == 3 && sd->status.base_level > 65) || (type == 2 && sd->status.base_level > 40) ||
		(type == 1))
		pc_setcart(sd, type);
}
...
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	struct PACKET_CZ_UNCONFIRMED_TSTATUS_UP *p =
		(struct PACKET_CZ_UNCONFIRMED_TSTATUS_UP *)RFIFOP(fd, 0);

	if (p->amount < 0) {
		ShowDebug(
			"clif_parse_traitstatus_up: Negative 'increase' value sent by client! %s (AID: %d, "
			"CID: %d, value: %d)\n",
...
static void clif_parse_UseSkillToId_homun(struct homun_data *hd, struct map_session_data *sd,
										  t_tick tick, uint16 skill_id, uint16 skill_lv,
										  int target_id) {
	int lv;
...
static void clif_parse_UseSkillToPos_homun(struct homun_data *hd, struct map_session_data *sd,
										   t_tick tick, uint16 skill_id, uint16 skill_lv, short x,
										   short y, int skillmoreinfo) {
	int lv;
	if (!hd)
		return;
	if (skill_isNotOk_hom(hd, skill_id, skill_lv)) {
...
static void clif_parse_UseSkillToId_mercenary(s_mercenary_data *md, struct map_session_data *sd,
											  t_tick tick, uint16 skill_id, uint16 skill_lv,
											  int target_id) {
	int lv;
...
static void clif_parse_UseSkillToPos_mercenary(s_mercenary_data *md, struct map_session_data *sd,
											   t_tick tick, uint16 skill_id, uint16 skill_lv,
											   short x, short y, int skillmoreinfo) {
	int lv;
	if (!md)
		return;
	if (skill_isNotOk_mercenary(skill_id, md))
...
void clif_parse_skill_toid(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv,
						   int target_id) {
	t_tick tick = gettick();
	...
	// This is done here, because homunculi and mercenaries can be triggered by AI and not by the
	// player itself
	if (battle_config.idletime_option & IDLE_USESKILLTOID)
		sd->idletime = last_tick;
	if (battle_config.hom_idle_no_share && sd->hd &&
		battle_config.idletime_hom_option & IDLE_USESKILLTOID)
		sd->idletime_hom = last_tick;
	if (battle_config.mer_idle_no_share && sd->md &&
		battle_config.idletime_mer_option & IDLE_USESKILLTOID)
		sd->idletime_mer = last_tick;
...
void clif_parse_UseSkillToId(int fd, struct map_session_data *sd) {
	...
	clif_parse_skill_toid(sd, RFIFOW(fd, info->pos[1]), RFIFOW(fd, info->pos[0]),
						  RFIFOL(fd, info->pos[2]));
}

/*==========================================
 * Client tells server he'd like to use AoE skill id 'skill_id' of level 'skill_lv' on 'x','y'
 *location
 *------------------------------------------*/
static void clif_parse_UseSkillToPosSub(int fd, struct map_session_data *sd, uint16 skill_lv,
										uint16 skill_id, short x, short y, int skillmoreinfo) {
	t_tick tick = gettick();
	...
	if (SKILL_CHK_MERC(skill_id)) {
		clif_parse_UseSkillToPos_mercenary(sd->md, sd, tick, skill_id, skill_lv, x, y,
										   skillmoreinfo);
		return;
	}
	...
	if (battle_config.idletime_option & IDLE_USESKILLTOPOS)
		sd->idletime = last_tick;
	if (battle_config.hom_idle_no_share && sd->hd &&
		battle_config.idletime_hom_option & IDLE_USESKILLTOPOS)
		sd->idletime_hom = last_tick;
	if (battle_config.mer_idle_no_share && sd->md &&
		battle_config.idletime_mer_option & IDLE_USESKILLTOPOS)
		sd->idletime_mer = last_tick;
...
void clif_parse_UseSkillToPos(int fd, struct map_session_data *sd) {
	...
	clif_parse_UseSkillToPosSub(fd, sd, RFIFOW(fd, info->pos[0]),  // skill lv
...
void clif_parse_Cooking(int fd, struct map_session_data *sd) {
	...
	if ((food_idx = skill_can_produce_mix(sd, p->itemId, sd->menuskill_val, amount)))
		skill_produce_mix(sd, (p->type > 1 ? sd->menuskill_id : 0), p->itemId, 0, 0, 0, amount,
						  food_idx - 1);
	clif_menuskill_clear(sd);
}
...
void clif_parse_NpcSelectMenu(int fd, struct map_session_data *sd) {
	...
	if ((select > sd->npc_menu && select != 0xff) || select == 0) {
		...
		ShowWarning(
			"Invalid menu selection on npc %d:'%s' - got %d, valid range is [%d..%d] (player "
			"AID:%d, CID:%d, name:'%s')!\n",
			npc_id, (nd) ? nd->name : "invalid npc id", select, 1, sd->npc_menu, sd->bl.id,
...
void clif_parse_PartyBookingRegisterReq(int fd, struct map_session_data *sd) {
	...
	for (i = 0; i < MAX_PARTY_BOOKING_JOBS; i++) job[i] = RFIFOB(fd, idxpbj + i * 2);
...
/// 0805 <packet len>.W <more results>.B { <index>.L <char name>.24B <expire time>.L <level>.W <map
/// id>.W { <job>.W }*6 }* more results:
///     0 = no
///     1 = yes
...
void clif_parse_PartyBookingUpdateReq(int fd, struct map_session_data *sd) {
	...
	for (i = 0; i < MAX_PARTY_BOOKING_JOBS; i++) job[i] = RFIFOW(fd, idxpbu + i * 2);
...
void clif_PartyBookingInsertNotify(struct map_session_data *sd,
								   struct party_booking_ad_info *pb_ad) {
	int i;
	...
	for (i = 0; i < MAX_PARTY_BOOKING_JOBS; i++) WBUFW(buf, 38 + i * 2) = pb_ad->p_detail.job[i];
...
void clif_PartyBookingUpdateNotify(struct map_session_data *sd,
								   struct party_booking_ad_info *pb_ad) {
	int i;
	...
	WBUFW(buf, 0) = 0x80a;
	WBUFL(buf, 2) = pb_ad->index;
	for (i = 0; i < MAX_PARTY_BOOKING_JOBS; i++) WBUFW(buf, 6 + i * 2) = pb_ad->p_detail.job[i];
...
/// 012e
void clif_parse_CloseVending(int fd, struct map_session_data *sd) { vending_closevending(sd); }
...
void clif_parse_PurchaseReq(int fd, struct map_session_data *sd) {
	struct PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC *p =
		(struct PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC *)RFIFOP(fd, 0);

	vending_purchasereq(sd, p->AID, sd->vended_id, (uint8 *)p->list,
...
#if PACKETVER >= 20100105
	struct PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC2 *p =
		(struct PACKET_CZ_PC_PURCHASE_ITEMLIST_FROMMC2 *)RFIFOP(fd, 0);

	vending_purchasereq(sd, p->AID, p->UniqueID, (uint8 *)p->list,
						(p->packetLength - sizeof(*p)) / sizeof(struct CZ_PURCHASE_ITEM_FROMMC));
...
/// 01b2 <packet len>.W <shop name>.80B <result>.B { <index>.W <amount>.W <price>.L }*
/// (CZ_REQ_OPENSTORE2) result:
///     0 = canceled
///     1 = open
...
void clif_parse_GuildChangePositionInfo(int fd, struct map_session_data *sd) {
	...
	for (i = idxgpos; i < len; i += 40) {
		guild_change_position(sd->status.guild_id, RFIFOL(fd, i), RFIFOL(fd, i + 4),
							  RFIFOL(fd, i + 12), RFIFOCP(fd, i + 16));
	}
}
...
void clif_parse_GuildChangeMemberPosition(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_REQ_CHANGE_MEMBERPOS *p =
		(struct PACKET_CZ_REQ_CHANGE_MEMBERPOS *)RFIFOP(fd, 0);
	...
	for (int16 i = 0; i < entries; i++) {
		...
		if (entry.position == 0) {
			...
			// No further entries will be processed - the requesting player lost his guild master
			// status
			return;
...
enum e_result_validate_emblem {	 // Used as Result for clif_validate_emblem
	...
	EMBVALIDATE_ERR_RAW_FILEFORMAT,	 // Invalid File Format (Error in zlib/decompression or
									 // malformed BMP header)
	EMBVALIDATE_ERR_TRANSPARENCY	 // uploaded emblem does not met the requirements of
									 // inter_config.emblem_transparency_limit
};

static enum e_result_validate_emblem clif_validate_emblem(const uint8 *emblem,
														  unsigned long emblem_len) {
	uint8 buf[1800];  // no well-formed emblem bitmap is larger than 1782 (24 bit) / 1654 (8 bit)
					  // bytes
	...
	if (!((decode_zip(buf, &buf_len, emblem, emblem_len) == 0 &&
		   buf_len >=
			   18)	// sizeof(BITMAPFILEHEADER) + sizeof(biSize) of the following info header struct
	...
	if (inter_config.emblem_transparency_limit != 100) {
		...
		if (((transcount * 300) / (buf_len - offset)) >
			inter_config.emblem_transparency_limit)	 // convert in % to chk
...
void clif_parse_GuildChangeEmblem(int fd, struct map_session_data *sd) {
	...
	if (!(inter_config.emblem_woe_change) && is_agit_start()) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 385), false,
	...
	if (emb_val == EMBVALIDATE_ERR_RAW_FILEFORMAT) {
		ShowWarning(
			"clif_parse_GuildChangeEmblem: Rejected malformed guild emblem (size=%lu, "
			"accound_id=%d, char_id=%d, guild_id=%d).\n",
			emblem_len, sd->status.account_id, sd->status.char_id, sd->status.guild_id);
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 386), false,
						  SELF);  //"The chosen emblem was detected invalid\n"
		return;
	} else if (emb_val == EMBVALIDATE_ERR_TRANSPARENCY) {
		char output[128];
		safesnprintf(output, sizeof(output), msg_txt(sd, 387),
					 inter_config.emblem_transparency_limit);
...
void clif_parse_GuildInvite2(int fd, struct map_session_data *sd) {
	struct map_session_data *t_sd =
		map_nick2sd(RFIFOCP(fd, packet_db[RFIFOW(fd, 0)].pos[0]), false);
...
void clif_parse_GuildLeave(int fd, struct map_session_data *sd) {
	...
	guild_leave(sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]), RFIFOL(fd, info->pos[2]),
...
void clif_parse_GuildExpulsion(int fd, struct map_session_data *sd) {
	...
	guild_expulsion(sd, RFIFOL(fd, info->pos[0]), RFIFOL(fd, info->pos[1]),
					RFIFOL(fd, info->pos[2]), RFIFOCP(fd, info->pos[3]));
}
...
void clif_parse_GMKick(int fd, struct map_session_data *sd) {
	...
	switch (target->type) {
		case BL_PC: {
			...
			safesnprintf(command, sizeof(command), "%ckick %s", atcommand_symbol,
						 status_get_name(target));
			is_atcommand(fd, sd, command, 1);
		} break;
		...
		case BL_MOB: {
			...
			safesnprintf(command, sizeof(command), "/kick %s (%d)", status_get_name(target),
...
/// 01bb <char name>.24B
void clif_parse_GMShift(int fd,
						struct map_session_data *sd) {	// FIXME: remove is supposed to receive
														// account name for clients prior 20100803RE
	char *player_name;
...
void clif_parse_GMRecall(
	...
	struct map_session_data
		*sd) {	// FIXME: recall is supposed to receive account name for clients prior 20100803RE
	char *player_name;
	char command[NAME_LENGTH + 8];
...
void clif_parse_GM_Item_Monster(int fd, struct map_session_data *sd) {
	...
	if (!str || str[0] == '\0')
		return;
	if (strcmpi(str, "SPPOINT") == 0 ||
		strcmpi(str, "JOBLEVEL") == 0)	//! TODO /sp sends these values
		return;
	trim(str);
...
void clif_parse_GMReqNoChat(int fd, struct map_session_data *sd) {
	...
	// Disabled because clients keep self-muting when you give players public @ commands...
	// [Skotlex]
	...
	safesnprintf(command, sizeof(command), "%cmute %d %s", atcommand_symbol, value,
				 dstsd->status.name);
	is_atcommand(fd, sd, command, 1);
}
...
void clif_parse_GMReqAccountName(int fd, struct map_session_data *sd) {
	...
	intif_request_accinfo(sd->fd, sd->bl.id, pc_get_group_level(sd), query, 1); //will call
	clif_account_name at return
	*/
...
void clif_parse_PMIgnore(int fd, struct map_session_data *sd) {
	...
	if (type == 0) {  // Add name to ignore list (block)
		...
		ARR_FIND(0, MAX_IGNORE_LIST, i,
				 sd->ignore[i].name[0] == '\0' || strcmp(sd->ignore[i].name, nick) == 0);
...
/// 00d3
void clif_parse_PMIgnoreList(int fd, struct map_session_data *sd) { clif_PMIgnoreList(sd); }
	...
	if ((sd->class_ & MAPID_UPPERMASK) == MAPID_SUPER_NOVICE) {
		...
		if (next) {
			...
			if (percent && (percent % 100) == 0) {	// 10.0%, 20.0%, ..., 90.0%
				sc_start(&sd->bl, &sd->bl, SC_EXPLOSIONSPIRITS, 100, 17,
						 skill_get_time(MO_EXPLOSIONSPIRITS,
										5));  // Lv17-> +50 critical (noted by Poki) [Skotlex]
				clif_skill_nodamage(&sd->bl, &sd->bl, MO_EXPLOSIONSPIRITS, 5,
									1);	 // prayer always shows successful Lv5 cast and disregards
										 // noskill restrictions
			}
		}
	}
}
...
///     0 = online
///     1 = offline
void clif_friendslist_toggle(struct map_session_data *sd, uint32 account_id, uint32 char_id,
							 int online) {
	int i, fd = sd->fd;

	// Seek friend.
	for (i = 0; i < MAX_FRIENDS && sd->status.friends[i].char_id &&
				(sd->status.friends[i].char_id != char_id ||
				 sd->status.friends[i].account_id != account_id);
		 i++)
		;
...
void clif_friendslist_send(struct map_session_data &sd) {
	...
	for (int i = 0; i < MAX_FRIENDS && sd.status.friends[i].char_id; i++) {
		if (map_charid2sd(sd.status.friends[i].char_id)) {
			clif_friendslist_toggle(&sd, sd.status.friends[i].account_id,
									sd.status.friends[i].char_id, 1);
		}
	}
}
...
void clif_friendlist_req(struct map_session_data *sd, uint32 account_id, uint32 char_id,
						 const char *name) {
	int fd = sd->fd;
...
void clif_parse_FriendsListReply(int fd, struct map_session_data *sd) {
	...
	if (reply == 0 ||
		!(sd->friend_req == f_sd->status.char_id && f_sd->friend_req == sd->status.char_id))
...
void clif_parse_FriendsListRemove(int fd, struct map_session_data *sd) {
	...
	// Search friend
	for (i = 0; i < MAX_FRIENDS && (sd->status.friends[i].char_id != char_id ||
									sd->status.friends[i].account_id != account_id);
		 i++)
		;
	...
	if ((f_sd = map_id2sd(account_id)) && f_sd->status.char_id == char_id) {
		for (i = 0;
			 i < MAX_FRIENDS && (f_sd->status.friends[i].char_id != sd->status.char_id ||
								 f_sd->status.friends[i].account_id != sd->status.account_id);
			 i++)
			;
		...
		if (i != MAX_FRIENDS) {
			...
			for (j = i + 1; j < MAX_FRIENDS; j++)
				memcpy(&f_sd->status.friends[j - 1], &f_sd->status.friends[j],
					   sizeof(f_sd->status.friends[0]));

			memset(&f_sd->status.friends[MAX_FRIENDS - 1], 0,
				   sizeof(f_sd->status.friends[MAX_FRIENDS - 1]));
...
/// 0292
void clif_parse_AutoRevive(int fd, struct map_session_data *sd) { pc_revive_item(sd); }
...
void clif_check(int fd, struct map_session_data *pl_sd) {
	...
	WFIFOW(fd, 14) =
		pl_sd->battle_status.batk + pl_sd->battle_status.rhw.atk + pl_sd->battle_status.lhw.atk;
...
void clif_parse_Check(int fd, struct map_session_data *sd) {
	...
	if ((pl_sd = map_nick2sd(charname, false)) == NULL ||
		pc_get_group_level(sd) < pc_get_group_level(pl_sd)) {
		return;
	}
...
///     0 = success
///     1 = failure
/// 0a05 <result>.B <index>.W <amount>.W <item id>.W <item type>.B <identified>.B <damaged>.B
/// <refine>.B (ZC_ACK_ADD_ITEM_TO_MAIL)
...
void clif_Mail_setattachment(struct map_session_data *sd, int index, int amount, uint8 flag) {
	...
	if (flag) {
		...
		for (int i = 0; i < MAIL_MAX_ITEM; i++) {
			...
			p.weight +=
				sd->mail.item[i].amount * (sd->inventory_data[sd->mail.item[i].index]->weight / 10);
		}
...
void clif_mail_getattachment(struct map_session_data *sd, struct mail_message *msg, uint8 result,
...
void clif_Mail_new(struct map_session_data *sd, int mail_id, const char *sender,
				   const char *title) {
#if PACKETVER < 20150513
	int fd = sd->fd;
...
void clif_Mail_refreshinbox(struct map_session_data *sd, enum mail_inbox_type type, int64 mailID) {
	...
	if (md->full) {	 // TODO: is this official?
		char output[100];
		safesnprintf(output, sizeof(output), "Inbox is full (Max %d). Delete some mails.",
					 MAIL_MAX_INBOX);
	...
	// Only count mails of the target type(before 2017-04-19) and those that should not have been
	// deleted already
	...
	for (amount = 0; i >= 0; i--) {
		...
		if (msg->scheduled_deletion > 0) {
			...
			// Sadly the client always displays the scheduled deletion after 24 hours no matter how
			// high this value gets [Lemongrass]
...
/// 09eb <packet len>.W <type>.B <mail id>.Q <message length>.W <zeny>.Q <item count>.B
/// (ZC_ACK_READ_MAIL)
///		{  }*n
...
void clif_Mail_Receiver_Ack(struct map_session_data *sd, uint32 char_id, short class_, uint32 level,
							const char *name) {
...
void clif_parse_Mail_getattach(int fd, struct map_session_data *sd) {
	...
	int attachment =
		packet_id == 0x9f1 ? MAIL_ATT_ZENY : packet_id == 0x9f3 ? MAIL_ATT_ITEM : MAIL_ATT_NONE;
#endif
...
#if PACKETVER_MAIN_NUM >= 20201104 || PACKETVER_RE_NUM >= 20211103 || PACKETVER_ZERO_NUM >= 20201118
	struct PACKET_CZ_UNCONFIRMED_RODEX_RETURN *p =
		(struct PACKET_CZ_UNCONFIRMED_RODEX_RETURN *)RFIFOP(fd, 0);
...
/// 09ec <packet len>.W <recipient>.24B <sender>.24B <zeny>.Q <title length>.W <body length>.W
/// <title>.?B <body>.?B (CZ_REQ_WRITE_MAIL) 0a6e <packet len>.W <recipient>.24B <sender>.24B
/// <zeny>.Q <title length>.W <body length>.W <char id>.L <title>.?B <body>.?B (CZ_REQ_WRITE_MAIL2)
void clif_parse_Mail_send(int fd, struct map_session_data *sd) {
	...
	mail_send(sd, RFIFOCP(fd, info->pos[1]), RFIFOCP(fd, info->pos[2]), RFIFOCP(fd, info->pos[4]),
			  RFIFOB(fd, info->pos[3]));
#else
...
/// 0252 <packet len>.W <pages>.L <count>.L { <auction id>.L <seller name>.24B <name id>.W <type>.L
/// <amount>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W <card3>.W <card4>.W <now
/// price>.L <max price>.L <buyer name>.24B <delete time>.L }*
...
void clif_parse_Auction_setitem(int fd, struct map_session_data *sd) {
	...
	if (amount != 1 || amount > sd->inventory.u.items_inventory[idx]
									.amount) {	// By client, amount is always set to 1. Maybe this
	...
	if (id != nullptr && !(id->type == IT_ARMOR || id->type == IT_PETARMOR ||
						   id->type == IT_WEAPON || id->type == IT_CARD || id->type == IT_ETC ||
						   id->type == IT_SHADOWGEAR)) {  // Consumable or pets are not allowed
...
void clif_Auction_close(int fd, unsigned char flag) {
	...
	WFIFOW(fd, 0) =
		0x25d;	// BUG: The client identifies this packet as 0x25d (CZ_AUCTION_REQ_MY_SELL_STOP)
	WFIFOW(fd, 2) = flag;
...
void clif_parse_Auction_register(int fd, struct map_session_data *sd) {
	...
	// Auction checks...
	if (sd->inventory.u.items_inventory[sd->auction.index].bound &&
	...
	std::shared_ptr<item_data> id =
		item_db.find(sd->inventory.u.items_inventory[sd->auction.index].nameid);
...
void clif_cashshop_list(struct map_session_data *sd) {
	...
	for (int tab = CASHSHOP_TAB_NEW; tab < CASHSHOP_TAB_MAX; tab++) {
		...
		int len =
			sizeof(struct PACKET_ZC_ACK_SCHEDULER_CASHITEM) +
			(cash_shop_items[tab].count * sizeof(struct PACKET_ZC_ACK_SCHEDULER_CASHITEM_sub));
		WFIFOHEAD(fd, len);
		struct PACKET_ZC_ACK_SCHEDULER_CASHITEM *p =
			(struct PACKET_ZC_ACK_SCHEDULER_CASHITEM *)WFIFOP(fd, 0);
...
/// }* 0287 <packet len>.W <cash point>.L <kafra point>.L { <sell price>.L <discount price>.L <item
/// type>.B <name id>.W }* (PACKETVER >= 20070711)
void clif_cashshop_show(struct map_session_data *sd, struct npc_data *nd) {
	...
	uint16 len = sizeof(struct PACKET_ZC_PC_CASH_POINT_ITEMLIST) +
				 nd->u.shop.count * sizeof(struct PACKET_ZC_PC_CASH_POINT_ITEMLIST_sub);
	WFIFOHEAD(fd, len);
	struct PACKET_ZC_PC_CASH_POINT_ITEMLIST *p =
		(struct PACKET_ZC_PC_CASH_POINT_ITEMLIST *)WFIFOP(fd, 0);
...
///     2 = The Purchase has failed because the Kafra Shop System is not working correctly.
///     (ERROR_TYPE_SYSTEM) 3 = You are over your Weight Limit. (ERROR_TYPE_INVENTORY_WEIGHT) 4 =
///     You cannot purchase items while you are in a trade. (ERROR_TYPE_EXCHANGE) 5 = The Purchase
///     has failed because the Item Information was incorrect. (ERROR_TYPE_ITEM_ID) 6 = You do not
///     have enough Kafra Credit Points. (ERROR_TYPE_MONEY) 7 = You can purchase up to 10 items.
///     (ERROR_TYPE_AMOUNT) 8 = Some items could not be purchased. (ERROR_TYPE_PURCHASE_FAIL)
...
/// 0288 <packet len>.W <kafra points>.L <count>.W { <amount>.W <name id>.W }.4B*count (PACKETVER >=
/// 20100803)
void clif_parse_npccashshop_buy(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM *p =
		(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM *)RFIFOP(fd, 0);
	...
	if (p->packetLength < sizeof(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM) ||
		...
		ShowWarning("Player %u sent incorrect cash shop buy packet (len %u:%" PRIdPTR ")!\n",
					sd->status.char_id, p->packetLength,
					sizeof(struct PACKET_CZ_PC_BUY_CASH_POINT_ITEM) + p->count * s_itl);
		return;
	}
...
/// Request to buy item(s) from cash shop.
/// 0848 <packet len>.W <count>.W <packet len>.W <kafra points>.L <count>.W { <amount>.W <name id>.W
/// <tab>.W }.6B*count (CZ_SE_PC_BUY_CASHITEM_LIST)
void clif_parse_cashshop_buy(int fd, struct map_session_data *sd) {
	struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST *p =
		(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST *)RFIFOP(fd, 0);
	...
	if (p->packetLength < sizeof(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST) ||
		...
		ShowWarning("Player %u sent incorrect cash shop buy packet (len %u:%" PRIdPTR ")!\n",
					sd->status.char_id, p->packetLength,
					sizeof(struct PACKET_CZ_SE_PC_BUY_CASHITEM_LIST) + p->count * s_itl);
		return;
	}
...
/// 0293 <infoType>.B <x>.L <y>.L <minHours>.W <minMinutes>.W <maxHours>.W <maxMinutes>.W <monster
/// name>.51B infoType:
...
void clif_parse_configuration(int fd, struct map_session_data *sd) {
	...
	switch (type) {
		...
		case CONFIG_PET_AUTOFEED:
			...
			if (sd->pd == nullptr || !battle_config.feature_pet_autofeed ||
				!sd->pd->get_pet_db()->allow_autofeed) {
				return;
			}
 ...
 **/
static void clif_quest_len(int def_len, int info_len, int avail_quests, int *limit_out,
						   int *len_out) {
...
/// 097a <packet len>.W <num>.L { <quest id>.L <active>.B <remaining time>.L <time>.L <count>.W {
/// <mob_id>.L <killed>.W <total>.W <mob name>.24B }*count }*num (ZC_ALL_QUEST_LIST2) 09f8 <packet
/// len>.W <num>.L { <quest id>.L <active>.B <remaining time>.L <time>.L <count>.W { <hunt
/// identification>.L <mob type>.L <mob_id>.L <min level>.W <max level>.W <killed>.W <total>.W <mob
/// name>.24B }*count }*num  (ZC_ALL_QUEST_LIST3)
		...
		if (!qi->objectives.empty()) {
			...
			for (int j = 0; j < qi->objectives.size(); j++) {
...
#endif
				WFIFOL(fd, offset) =
					((mob && qi->objectives[j]->mob_id > 0) ? qi->objectives[j]->mob_id
															: MOBID_PORING);
				offset += 4;
#if PACKETVER >= 20150513
				...
				if (mob && qi->objectives[j]->mob_id > 0)
					...
					safestrncpy(WFIFOCP(fd, offset), clif_quest_string(qi->objectives[j]).c_str(),
								NAME_LENGTH);
				offset += NAME_LENGTH;
			}
		}
...
/// 02b2 <packet len>.W <num>.L { <quest id>.L <start time>.L <expire time>.L <mobs>.W { <mob id>.L
/// <mob count>.W <mob name>.24B }*3 }*num
void clif_quest_send_mission(struct map_session_data *sd) {
	...
	clif_quest_len(8, 14 + ((6 + NAME_LENGTH) * MAX_QUEST_OBJECTIVES), sd->avail_quests, &limit,
				   &len);
	WFIFOHEAD(fd, len);
	WFIFOW(fd, 0) = 0x2b2;
	WFIFOW(fd, 2) = len;
	...
	for (int i = 0; i < limit; i++) {
		...
		for (int j = 0; j < qi->objectives.size(); j++) {
			...
			if (mob && qi->objectives[j]->mob_id > 0)
				...
				safestrncpy(WFIFOCP(fd, i * 104 + 28 + j * 30),
							clif_quest_string(qi->objectives[j]).c_str(), NAME_LENGTH);
		}
	}
...
/// 02b3 <quest id>.L <active>.B <start time>.L <expire time>.L <mobs>.W { <mob id>.L <mob count>.W
/// <mob name>.24B }*3 (ZC_ADD_QUEST) 08fe <packet len>.W  { <quest id>.L <mob id>.L <total count>.W
/// <current count>.W }*3 (ZC_HUNTING_QUEST_INFO) 09f9 <quest id>.L <active>.B <start time>.L
/// <expire time>.L <mobs>.W { <hunt identification>.L <mob type>.L <mob id>.L <min level>.W <max
/// level>.W <mob count>.W <mob name>.24B }*3 (ZC_ADD_QUEST_EX)
void clif_quest_add(struct map_session_data *sd, struct quest *qd) {
	...
	for (int i = 0, offset = 17; i < qi->objectives.size(); i++) {
		...
		offset += 4;
#endif
		WFIFOL(fd, offset) = ((mob && qi->objectives[i]->mob_id > 0)
								  ? qi->objectives[i]->mob_id
								  : MOBID_PORING);	// 0 can't be used as it displays "Novice" job
													// regardless of the clif_mobtype_name
		offset += 4;
#if PACKETVER >= 20150513
		...
		if (mob && qi->objectives[i]->mob_id > 0)
			...
			safestrncpy(WFIFOCP(fd, offset), clif_quest_string(qi->objectives[i]).c_str(),
						NAME_LENGTH);
		offset += NAME_LENGTH;
	}
...
/// (ZC_UPDATE_MISSION_HUNT) 09fa <packet len>.W <mobs>.W { <quest id>.L <hunt identification>.L
/// <total count>.W <current count>.W }*3 (ZC_UPDATE_MISSION_HUNT_EX)
...
///     2 = green
///     3 = purple
void clif_quest_show_event(struct map_session_data *sd, struct block_list *bl,
						   e_questinfo_types effect, e_questinfo_markcolor color) {
...
void clif_mercenary_updatestatus(struct map_session_data *sd, int type) {
	...
	switch (type) {
		case SP_ATK1: {
			int atk =
				rnd() % (status->rhw.atk2 - status->rhw.atk + 1) + status->batk + status->rhw.atk;
...
/// 029d <packet len>.W { <skill id>.W <type>.L <level>.W <sp cost>.W <attack range>.W <skill
/// name>.24B <upgradable>.B }*
...
void clif_bg_message(struct s_battleground_data *bg, int src_id, const char *name, const char *mes,
					 int len) {
...
void clif_parse_bg_queue_apply_request(int fd, struct map_session_data *sd) {
	...
	if (sd->bg_queue_id > 0) {
		// ShowWarning("clif_parse_bg_queue_apply_request: Received duplicate queue application: %d
		// from player %s (AID:%d CID:%d).\n", type, sd->status.name, sd->status.account_id,
		// sd->status.char_id);
		...
		ShowWarning(
			"clif_parse_bg_queue_apply_request: Received invalid queue type: %d from player %s "
			"(AID:%d CID:%d).\n",
			type, sd->status.name, sd->status.account_id, sd->status.char_id);
		clif_bg_queue_apply_result(BG_APPLY_INVALID_APP, name,
								   sd);	 // Someone sent an invalid queue type packet
		return;
	}
}
...
void clif_bg_queue_apply_result(e_bg_queue_apply_ack result, const char *name,
								struct map_session_data *sd) {
	nullpo_retv(sd);
...
/// Sends a notification at the start of the battlegrounds queue and is also used to update the
/// queue number. 0x8d9 <battleground name>.24B <queue number>.L (ZC_NOTIFY_ENTRY_QUEUE_APPLY)
...
void clif_parse_bg_queue_cancel_request(int fd, struct map_session_data *sd) {
	...
	if (sd->bg_queue_id > 0) {
		...
		if (queue && queue->state == QUEUE_STATE_SETUP_DELAY)
			return;	 // Make the cancel button do nothing if the entry window is open. Otherwise
					 // it'll crash the game when you click on both the queue status and entry
					 // status window.
		else
			success = bg_queue_leave(sd);
	} else {
		ShowWarning(
			"clif_parse_bg_queue_cancel_request: Player trying to request leaving non-existent "
...
void clif_bg_queue_ack_lobby(bool result, const char *name, const char *lobbyname,
							 struct map_session_data *sd) {
	nullpo_retv(sd);
...
/// Silently removes all the battlegrounds stuff client side so that you will open the first BG
/// window when you press battle on the interface. Send this when a player joins a battleground so
/// that it will remove all the queue stuff upon warping in. 0x90e (ZC_ENTRY_QUEUE_INIT)
...
void clif_instance_create(int instance_id, int num) {
	...
	std::shared_ptr<s_instance_db> db =
		instance_db.find(util::umap_find(instances, instance_id)->id);
...
/// 02b8 <account id>.L <name id>.W <identified>.B <damaged>.B <refine>.B <card1>.W <card2>.W
/// <card3>.W <card4>.W <equip location>.W <item type>.B (ZC_ITEM_PICKUP_PARTY)
...
void clif_displayexp(struct map_session_data *sd, t_exp exp, char type, bool quest, bool lost) {
	...
	WFIFOW(fd, 12 + offset) =
		(quest && type != SP_JOBEXP) ? 1 : 0;  // NOTE: Somehow JobEXP always in yellow color
	WFIFOSET(fd, packet_len(cmd));
}
...
///     1 = Incremental counter (1 tick/second), negated 'value' specifies start value (e.g. using
///     -10 lets the counter start at 10). 2 = Decremental counter (1 tick/second), negated 'value'
///     specifies start value (does not stop when reaching 0, but overflows). 3 = Decremental
///     counter (1 tick/second), 'value' specifies start value (stops when reaching 0, displays at
///     most 2 digits).
/// value:
///     Except for type 3 it is interpreted as seconds for displaying as DD:HH:MM:SS, HH:MM:SS,
///     MM:SS or SS (leftmost '00' is not displayed).
...
/// 0811 <packet len>.W <limit zeny>.L <result>.B <store name>.80B { <name id>.W <amount>.W
/// <price>.L }* result:
///     0 = cancel
///     1 = open
static void clif_parse_ReqOpenBuyingStore(int fd, struct map_session_data *sd) {
	const struct PACKET_CZ_REQ_OPEN_BUYING_STORE *p =
		(struct PACKET_CZ_REQ_OPEN_BUYING_STORE *)RFIFOP(fd, 0);
	...
	if (packet_len % sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub)) {
		ShowError(
			"clif_parse_ReqOpenBuyingStore: Unexpected item list size %u (account_id=%d, block "
			"size=%" PRIdPTR ")\n",
			packet_len, sd->bl.id, sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub));
		return;
	}

	buyingstore_create(sd, p->zenyLimit, p->result, storename, p->items,
					   packet_len / sizeof(struct PACKET_CZ_REQ_OPEN_BUYING_STORE_sub), NULL);
}

/// Notification, that the requested buying store could not be created
/// (ZC_FAILED_OPEN_BUYING_STORE_TO_BUYER). 0812 <result>.W <total weight>.L result:
///     1 = "Failed to open buying store." (0x6cd, MSI_BUYINGSTORE_OPEN_FAILED)
///     2 = "Total amount of then possessed items exceeds the weight limit by
///     <weight/10-maxweight*90%>. Please re-enter." (0x6ce, MSI_BUYINGSTORE_OVERWEIGHT) 8 = "No
///     sale (purchase) information available." (0x705) ? = nothing
void clif_buyingstore_open_failed(struct map_session_data *sd, unsigned short result,
								  unsigned int weight) {
	int fd = sd->fd;
...
void clif_buyingstore_myitemlist(struct map_session_data *sd) {
	...
	struct PACKET_ZC_MYITEMLIST_BUYING_STORE *p =
		(struct PACKET_ZC_MYITEMLIST_BUYING_STORE *)WFIFOP(fd, 0);
	p->packetType = 0x813;
	p->packetLength = length;
	p->AID = sd->bl.id;
...
/// 0818 <packet len>.W <account id>.L <store id>.L <limit zeny>.L { <price>.L <amount>.W <type>.B
/// <name id>.W }* (ZC_ACK_ITEMLIST_BUYING_STORE)
void clif_buyingstore_itemlist(struct map_session_data *sd, struct map_session_data *pl_sd) {
	...
	struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE *p =
		(struct PACKET_ZC_ACK_ITEMLIST_BUYING_STORE *)WFIFOP(fd, 0);
	p->packetType = 0x818;
	p->packetLength = length;
	p->AID = pl_sd->bl.id;
	...
	for (int i = 0; i < pl_sd->buyingstore.slots; i++) {
		...
		p->items[i].amount =
			pl_sd->buyingstore.items[i].amount;	 // TODO: Figure out, if no longer needed items
												 // (amount == 0) are listed on official.
...
static void clif_parse_ReqTradeBuyingStore(int fd, struct map_session_data *sd) {
	const struct PACKET_CZ_REQ_TRADE_BUYING_STORE *p =
		(struct PACKET_CZ_REQ_TRADE_BUYING_STORE *)RFIFOP(fd, 0);
	...
	if (packet_len % sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub)) {
		ShowError(
			"clif_parse_ReqTradeBuyingStore: Unexpected item list size %u (account_id=%d, "
			"buyer_id=%d, block size=%" PRIdPTR ")\n",
			packet_len, sd->bl.id, p->AID, sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub));
		return;
	}

	buyingstore_trade(sd, p->AID, p->storeId, p->items,
					  packet_len / sizeof(struct PACKET_CZ_REQ_TRADE_BUYING_STORE_sub));
}
...
///     3 = "All items within the buy limit were purchased." (0x6cf,
///     MSI_BUYINGSTORE_TRADE_OVERLIMITZENY) 4 = "All items were purchased." (0x6d0,
///     MSI_BUYINGSTORE_TRADE_BUYCOMPLETE) ? = nothing
...
/// 09e6 <name id>.W <amount>.W <zeny>.L <limit zeny>.L <GID>.L <Date>.L
/// (ZC_UPDATE_ITEM_FROM_BUYING_STORE2)
void clif_buyingstore_update_item(struct map_session_data *sd, t_itemid nameid,
								  unsigned short amount, uint32 char_id, int zeny) {
...
void clif_buyingstore_delete_item(struct map_session_data *sd, short index, unsigned short amount,
								  int price) {
	int fd = sd->fd;
...
///     6 = "The trade failed, because the entered amount of item %s is higher, than the buyer is
///     willing to buy." (0x6d3, MSI_BUYINGSTORE_TRADE_OVERCOUNT) 7 = "The trade failed, because the
///     buyer is lacking required balance." (0x6d1, MSI_BUYINGSTORE_TRADE_LACKBUYERZENY) ? = nothing
void clif_buyingstore_trade_failed_seller(struct map_session_data *sd, short result,
										  t_itemid nameid) {
...
/// 0835 <packet len>.W <type>.B <max price>.L <min price>.L <name id count>.B <card count>.B {
/// <name id>.W }* { <card>.W }* type:
///     0 = Vending
///     1 = Buying Store
///
...
static void clif_parse_SearchStoreInfo(int fd, struct map_session_data *sd) {
	const struct PACKET_CZ_SEARCH_STORE_INFO *p =
		(struct PACKET_CZ_SEARCH_STORE_INFO *)RFIFOP(fd, 0);
	...
	if (packet_len % sizeof(struct PACKET_CZ_SEARCH_STORE_INFO_item)) {
		ShowError(
			"clif_parse_SearchStoreInfo: Unexpected item list size %u (account_id=%d, block "
			"size=%" PRIdPTR ")\n",
	...
	if (count < p->itemsCount + p->cardsCount) {
		ShowError(
			"clif_parse_SearchStoreInfo: Malformed packet (expected count=%u, count=%u, "
			"account_id=%d).\n",
			p->itemsCount + p->cardsCount, count, sd->bl.id);
		return;
	}

	searchstore_query(sd, p->searchType, p->minPrice, p->maxPrice, &p->items[0], p->itemsCount,
					  &p->items[p->itemsCount], p->cardsCount);
}

/// Results for a store search request (ZC_SEARCH_STORE_INFO_ACK).
/// 0836 <packet len>.W <is first page>.B <is next page>.B <remaining uses>.B { <store id>.L
/// <account id>.L <shop name>.80B <nameid>.W <item type>.B <price>.L <amount>.W <refine>.B
/// <card1>.W <card2>.W <card3>.W <card4>.W }* is first page:
...
void clif_search_store_info_ack(struct map_session_data *sd) {
	...
	struct PACKET_ZC_SEARCH_STORE_INFO_ACK *p =
		(struct PACKET_ZC_SEARCH_STORE_INFO_ACK *)WFIFOP(fd, 0);
...
static void clif_parse_SearchStoreInfoListItemClick(int fd, struct map_session_data *sd) {
	const struct PACKET_CZ_SSILIST_ITEM_CLICK *p =
		(struct PACKET_CZ_SSILIST_ITEM_CLICK *)RFIFOP(fd, 0);
...
void clif_parse_debug(int fd, struct map_session_data *sd) {
	...
	if (sd) {
		...
		if (packet_len == 0) {	// unknown
			...
			packet_len =
				RFIFOW(fd, 2);	// clif_parse ensures, that this amount of data is already received
		}
		ShowDebug("Packet debug of 0x%04X (length %d), %s session #%d, %d/%d (AID/CID)\n", cmd,
				  packet_len, sd->state.active ? "authed" : "unauthed", fd, sd->status.account_id,
				  sd->status.char_id);
	} else {
		packet_len = RFIFOREST(fd);
 ...
 * Server populates the window with avilable elemental converter options according to player's
 *inventory
 *------------------------------------------*/
...
void clif_magicdecoy_list(struct map_session_data *sd, uint16 skill_lv, short x, short y) {
	...
	WFIFOHEAD(fd, sizeof(struct PACKET_ZC_MAKINGARROW_LIST) +
					  MAX_INVENTORY * sizeof(struct PACKET_ZC_MAKINGARROW_LIST_sub));
...
void clif_parse_SkillSelectMenu(int fd, struct map_session_data *sd) {
	struct s_packet_db *info = &packet_db[RFIFOW(fd, 0)];
	// int type = RFIFOL(fd,info->pos[0]); //WHY_LOWERVER_COMPATIBILITY =  0x0,
...
void __attribute__((unused)) clif_parse_dull(int fd, struct map_session_data *sd) { return; }
...
/// 0447
void clif_parse_blocking_playcancel(int fd, struct map_session_data *sd) { clif_loadConfirm(sd); }
...
void clif_sub_ranklist(unsigned char *buf, int idx, struct map_session_data *sd,
...
void clif_parse_Taekwon(int fd, struct map_session_data *sd) { clif_ranklist(sd, RANK_TAEKWON); }

/// Request for the killer ranklist.
/// /pk command sends this packet to the server.
/// 0237 (CZ_KILLER_RANK)
void clif_parse_RankingPk(int fd, struct map_session_data *sd) { clif_ranklist(sd, RANK_KILLER); }
...
void clif_update_rankingpoint(map_session_data &sd, int rankingtype, int point) {
	...
	switch (rankingtype) {
		...
		default:
			ShowError(
				"clif_update_rankingpoint: Unsupported ranking type '%d'. Please report this.\n",
				rankingtype);
			return;
	}
 ...
 * 0x0981 <packet len>.W <exp>.W <death>.W <drop>.W <activity rate>.W <DETAIL_EXP_INFO>13B
 * (ZC_PERSONAL_INFOMATION_CHN) <InfoType>.B <Exp>.L <Death>.L <Drop>.L (DETAIL_EXP_INFO 0x97b|0981)
 * FIXME!
 * - Find/decide for details of EXP, Drop, and Death penalty rates
 * - For now, we're assuming values for DETAIL_EXP_INFO are:
 *   0 - map adjustment (bexp mapflag), 1 - Premium/VIP adjustment, 2 - Server rate adjustment, 3 -
 * None
 */
#ifdef VIP_ENABLE
void clif_display_pinfo(struct map_session_data *sd, int cmdtype) {
	if (sd) {
		...
		// 1:Premium
		if (pc_isvip(sd)) {
			details_bexp[PINFO_PREMIUM] =
				battle_config.vip_base_exp_increase * battle_config.base_exp_rate / 100;
		...
		// 1:Premium
		if (pc_isvip(sd)) {
			details_drop[PINFO_PREMIUM] =
				(battle_config.vip_drop_increase * battle_config.item_rate_common) / 100;
		...
		if (pc_isvip(sd)) {
			...
			if (battle_config.death_penalty_base > battle_config.vip_exp_penalty_base)
				details_penalty[PINFO_PREMIUM] =
					battle_config.vip_exp_penalty_base - battle_config.death_penalty_base;
		} else
			details_penalty[PINFO_PREMIUM] = 0;
		...
		for (i = 0; i < maxinfotype; i++) {
			WFIFOB(fd, info->pos[4] + (i * szdetails)) =
				i;	// infotype //0 PCRoom, 1 Premium, 2 Server, 3 TPlus
 ...
 **/
void clif_party_leaderchanged(struct map_session_data *sd, int prev_leader_aid,
							  int new_leader_aid) {
	unsigned char buf[10];
...
void clif_clan_message(struct clan *clan, const char *mes, int len) {
	...
	if (len == 0) {
		...
		ShowWarning("clif_clan_message: Truncated message '%s' (len=%d, max=%" PRIuPTR
					", clan_id=%d).\n",
	...
	// Offially the sender name should also be filled here, but it is not required by the client and
	// since it's in the message too we do not fill it
 ...
 *      <antagonist count>.B { <alliance>.24B } * alliance count { <antagonist>.24B } * antagonist
 *count (ZC_CLANINFO)
 **/
 ...
 */
void clif_change_title_ack(struct map_session_data *sd, unsigned char result,
...
void DumpUnknown(int fd, TBL_PC *sd, int cmd, int packet_len) {
	...
	strftime(datestr, sizeof(datestr) - 1, "%A, %B %d %Y %X.",
	...
	if ((fp = fopen(packet_txt, "a")) != NULL) {
		if (sd) {
			fprintf(fp,
					...
					cmd, packet_len, sd->state.active ? "authed" : "unauthed", fd,
					sd->status.account_id, sd->status.char_id, datestr);
		} else {
...
/// 0A1A <result>.B <serial>.L <stage>.B <price index>.B <additional item id>.W <gold>.L <silver>.L
/// <bronze>.L (ZC_ACK_OPEN_ROULETTE)
...
void clif_parse_roulette_open(int fd, struct map_session_data *sd) {
	...
	if (!battle_config.feature_roulette) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 1497), false,
						  SELF);  // Roulette is disabled
		return;
	}
...
/// } * MAX_ROULETTE_LEVEL (ZC_ACK_ROULEITTE_INFO) 0A1C <length>.W <serial>.L { { <level>.W
/// <column>.W <item>.L <amount>.L } * MAX_ROULETTE_COLUMNS } * MAX_ROULETTE_LEVEL
...
static void clif_roulette_recvitem_ack(struct map_session_data *sd,
									   enum RECV_ROULETTE_ITEM_REQ type) {
#if PACKETVER >= 20141016
	nullpo_retv(sd);
...
static uint8 clif_roulette_getitem(struct map_session_data *sd) {
	...
	if ((res = pc_additem(sd, &it, rd.qty[sd->roulette.prizeStage][sd->roulette.prizeIdx],
						  LOG_TYPE_ROULETTE)) == 0) {
		;  // onSuccess
	}
...
/// 0A20 <result>.B <stage>.W <price index>.W <bonus item>.W <gold>.L <silver>.L <bronze>.L
/// (ZC_ACK_GENERATE_ROULETTE)
void clif_roulette_generate(struct map_session_data *sd, unsigned char result, short stage,
							short prizeIdx, t_itemid bonusItemID) {
	nullpo_retv(sd);
...
void clif_parse_roulette_generate(int fd, struct map_session_data *sd) {
	...
	if (!sd->roulette.stage && sd->roulette_point.bronze <= 0 && sd->roulette_point.silver < 10 &&
		...
		sd->roulette.tick =
			gettick() + i64max(1, (MAX_ROULETTE_COLUMNS - sd->roulette.prizeStage - 3)) * 1000;
	...
	clif_roulette_generate(sd, result, sd->roulette.prizeStage,
						   (sd->roulette.prizeIdx == -1 ? 0 : sd->roulette.prizeIdx), 0);
}
 ...
 * @param sd
 **/
void clif_merge_item_ack(struct map_session_data *sd, unsigned short index, unsigned short count,
						 enum MERGE_ITEM_ACK type) {
	unsigned char buf[9];
	struct s_packet_db *info = NULL;
	short cmd = 0;
...
static bool clif_merge_item_has_pair(struct map_session_data *sd, struct item *it) {
	...
	ARR_FIND(0, MAX_INVENTORY, i,
			 (it_ = &sd->inventory.u.items_inventory[i]) && it->nameid == it_->nameid &&
				 it->bound == it_->bound && memcmp(it_, it, sizeof(struct item)) != 0);
	if (i < MAX_INVENTORY)
		return true;
...
void clif_merge_item_open(struct map_session_data *sd) {
	...
	// Get entries
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (!clif_merge_item_check(sd->inventory_data[i],
...
void clif_parse_merge_item_req(int fd, struct map_session_data *sd) {
	...
	for (i = 0, j = 0; i < n; i++) {
		...
		if (!clif_merge_item_check((id = sd->inventory_data[idx]),
								   &sd->inventory.u.items_inventory[idx]))
			continue;
		indexes[j] = idx;
		if (j && id->nameid !=
					 sd->inventory_data[indexes[0]]->nameid) {	// Only can merge 1 kind at once
	...
	for (i = 1; i < n; i++) {
		...
		memset(&sd->inventory.u.items_inventory[idx], 0,
			   sizeof(sd->inventory.u.items_inventory[0]));
		sd->inventory_data[idx] = NULL;
 ...
 *(ZC_BROADCASTING_SPECIAL_ITEM_OBTAIN) 07fd <size>.W <type>.B <itemid>.W <charname_len>.B
 *<charname>.24B <source_len>.B <srcname>.24B (ZC_BROADCASTING_SPECIAL_ITEM_OBTAIN) type:
 *ITEMOBTAIN_TYPE_BOXITEM & ITEMOBTAIN_TYPE_MONSTER_ITEM "[playername] ... [sourcename] ...
 *[itemname]" -> MsgStringTable[1629] type: ITEMOBTAIN_TYPE_NPC "[playername] ... [itemname]" ->
 *MsgStringTable[1870]
 **/
void clif_broadcast_obtain_special_item(const char *char_name, t_itemid nameid, t_itemid container,
...
void clif_parse_Oneclick_Itemidentify(int fd, struct map_session_data *sd) {
	...
	// - Item is already identified
	if (idx < 0 || idx >= MAX_INVENTORY || sd->inventory.u.items_inventory[idx].nameid == 0 ||
		sd->inventory_data[idx] == NULL || sd->inventory.u.items_inventory[idx].identify)
		return;
...
void clif_navigateTo(struct map_session_data *sd, const char *mapname, uint16 x, uint16 y,
					 uint8 flag, bool hideWindow, uint16 mob_id) {
...
void clif_hat_effects(struct map_session_data *sd, struct block_list *bl, enum send_target target) {
	...
	p->packetLength =
		(int16)(sizeof(struct PACKET_ZC_EQUIPMENT_EFFECT) + sizeof(int16) * tsd->hatEffects.size());
	p->aid = tsd->bl.id;
	p->status = 1;
...
void clif_sale_start(struct sale_item_data *sale_item, struct block_list *bl,
					 enum send_target target) {
#if PACKETVER_SUPPORTS_SALES
...
void clif_sale_end(struct sale_item_data *sale_item, struct block_list *bl,
				   enum send_target target) {
#if PACKETVER_SUPPORTS_SALES
...
void clif_sale_amount(struct sale_item_data *sale_item, struct block_list *bl,
					  enum send_target target) {
#if PACKETVER_SUPPORTS_SALES
...
/// 09ae <account id>.L <item id>.W <amount>.L <start time>.L <hours on sale>.B
/// (CZ_REQ_APPLY_BARGAIN_SALE_ITEM) 0a3d <account id>.L <item id>.W <amount>.L <start time>.L
/// <hours on sale>.W (CZ_REQ_APPLY_BARGAIN_SALE_ITEM2)
void clif_parse_sale_add(int fd, struct map_session_data *sd) {
#if PACKETVER_SUPPORTS_SALES
	nullpo_retv(sd);

	struct PACKET_CZ_REQ_APPLY_BARGAIN_SALE_ITEM *p =
		(struct PACKET_CZ_REQ_APPLY_BARGAIN_SALE_ITEM *)RFIFOP(fd, 0);
...
void clif_parse_sale_remove(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM *p =
		(struct PACKET_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM *)RFIFOP(fd, 0);
...
void clif_achievement_list_all(struct map_session_data *sd) {
	...
	if (!battle_config.feature_achievement) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 772), false,
						  SELF);  // Achievements are disabled.
		return;
	}
	...
	WFIFOW(fd, 2) = len;
	WFIFOL(fd, 4) =
		count;	// Amount of achievements the player has in their list (started/completed)
	...
	for (int i = 0; i < count; i++) {
		...
		for (int j = 0; j < MAX_ACHIEVEMENT_OBJECTIVES; j++)
			WFIFOL(fd, (i * 50) + 27 + (j * 4)) =
				(uint32)sd->achievement_data.achievements[i].count[j];
	...
	switch (RFIFOB(fd, 2)) {
		case IN_UI_ATTENDANCE:
			if (!pc_has_permission(sd, PC_PERM_ATTENDANCE)) {
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 791), false,
...
/// 0A4A <response>.L
void clif_private_airship_response(struct map_session_data *sd,
								   enum e_private_airship_response response) {
...
void clif_parse_private_airship_request(int fd, struct map_session_data *sd) {
	...
	if (!battle_config.feature_privateairship) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 792), false,
						  SELF);  // The private airship system is disabled.
		return;
	}
	...
	const struct PACKET_CZ_PRIVATE_AIRSHIP_REQUEST *p =
		(struct PACKET_CZ_PRIVATE_AIRSHIP_REQUEST *)RFIFOP(fd, 0);
...
/// 09DA <size>.W <result>.W <count>.W { <id>.L <item id>.W <amount>.L <action>.B <refine>.L <unique
/// id>.Q <identify>.B <item type>.W
///      { <card item id>.W }*4 <name>.24B <time>.24B <attribute>.B }*count
///      (ZC_ACK_GUILDSTORAGE_LOG)
void clif_guild_storage_log(struct map_session_data *sd, std::vector<struct guild_log_entry> &log,
	...
	WFIFOHEAD(fd, size);
	struct PACKET_ZC_ACK_GUILDSTORAGE_LOG *p =
		(struct PACKET_ZC_ACK_GUILDSTORAGE_LOG *)WFIFOP(fd, 0);
...
void clif_camerainfo(struct map_session_data *sd, bool show, float range, float rotation,
					 float latitude) {
...
void clif_parse_camerainfo(int fd, struct map_session_data *sd) {
	...
	if (RFIFOB(fd, 2) == 1) {
		...
		safesnprintf(command, sizeof(command), "%ccamerainfo %03.03f %03.03f %03.03f",
					 atcommand_symbol, RFIFOF(fd, 3), RFIFOF(fd, 7), RFIFOF(fd, 11));
	}
...
void clif_refineui_info(struct map_session_data *sd, uint16 index) {
	...
	struct PACKET_ZC_REFINING_MATERIAL_LIST *p =
		(struct PACKET_ZC_REFINING_MATERIAL_LIST *)packet_buffer;
...
#if PACKETVER >= 20161012
	struct PACKET_CZ_REFINING_SELECT_ITEM *p =
		(struct PACKET_CZ_REFINING_SELECT_ITEM *)RFIFOP(fd, 0);
...
void clif_parse_refineui_refine(int fd, struct map_session_data *sd) {
	...
	if (pc_payzeny(sd, cost->zeny, LOG_TYPE_CONSUME, NULL)) {
		clif_npc_buy_result(
			sd, e_purchase_result::PURCHASE_FAIL_MONEY);  // "You do not have enough zeny."
		return;
	}
	...
	if (blacksmith_amount > 0 &&
		pc_delitem(sd, blacksmith_index, blacksmith_amount, 0, 0, LOG_TYPE_CONSUME)) {
		return;
	}
...
void clif_parse_stylist_buy(int fd, struct map_session_data *sd) {
	...
	if (p->BodyPalette != 0 &&
		!clif_parse_stylist_buy_sub(sd, LOOK_CLOTHES_COLOR, p->BodyPalette)) {
		clif_stylist_response(sd, true);
		return;
	}
	...
	if (p->BottomAccessory != 0 &&
		!clif_parse_stylist_buy_sub(sd, LOOK_HEAD_BOTTOM, p->BottomAccessory)) {
		clif_stylist_response(sd, true);
		return;
	}
...
void clif_inventory_expansion_response(struct map_session_data *sd,
									   e_inventory_expansion_response response) {
#if PACKETVER_MAIN_NUM >= 20181031 || PACKETVER_RE_NUM >= 20181031 || PACKETVER_ZERO_NUM >= 20181114
	nullpo_retv(sd);
...
void clif_parse_inventory_expansion_request(int fd, struct map_session_data *sd) {
	...
	static std::map<t_itemid, uint16> items = {
		...
		// The number of increased slots is "hardcoded" in the message of the client and cannot be
		// sent per item
...
void clif_inventory_expansion_result(struct map_session_data *sd,
									 e_inventory_expansion_result result) {
#if PACKETVER_MAIN_NUM >= 20181031 || PACKETVER_RE_NUM >= 20181031 || PACKETVER_ZERO_NUM >= 20181114
	nullpo_retv(sd);
...
void clif_barter_open(struct map_session_data &sd, struct npc_data &nd) {
	...
	struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO *p =
		(struct PACKET_ZC_NPC_BARTER_MARKET_ITEMINFO *)packet_buffer;
...
void clif_parse_barter_buy(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_NPC_BARTER_MARKET_PURCHASE *p =
		(struct PACKET_CZ_NPC_BARTER_MARKET_PURCHASE *)RFIFOP(fd, 0);
	...
	for (int i = 0; i < entries; i++) {
		std::shared_ptr<s_npc_barter_item> item =
			util::map_find(barter->items, (uint16)p->list[i].shopIndex);
		...
		for (int j = i + 1; j < entries; j++) {
			...
			std::shared_ptr<s_npc_barter_item> item2 =
				util::map_find(barter->items, (uint16)p->list[j].shopIndex);
...
void clif_barter_extended_open(struct map_session_data &sd, struct npc_data &nd) {
	...
	for (const auto &itemPair : barter->items) {
		...
		struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub *item =
			(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub *)(((uint8 *)p) +
																		 p->packetLength);
		std::shared_ptr<item_data> id = item_db.find(itemPair.second->nameid);
		...
		for (const auto &requirementPair : itemPair.second->requirements) {
			...
			struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2 *req =
				(struct PACKET_ZC_NPC_EXPANDED_BARTER_MARKET_ITEMINFO_sub2 *)(((uint8 *)p) +
																			  p->packetLength);
			std::shared_ptr<s_npc_barter_requirement> requirement = requirementPair.second;
...
void clif_parse_barter_extended_buy(int fd, struct map_session_data *sd) {
	...
	uint16 entries =
		(p->packetLength - sizeof(struct PACKET_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE)) /
...
void clif_laphine_synthesis_open(struct map_session_data *sd,
								 std::shared_ptr<s_laphine_synthesis> synthesis) {
...
void clif_laphine_synthesis_result(struct map_session_data *sd,
								   enum e_laphine_synthesis_result result) {
#if PACKETVER_MAIN_NUM >= 20160601 || PACKETVER_RE_NUM >= 20160525 || defined(PACKETVER_ZERO)
	nullpo_retv(sd);
...
void clif_parse_laphine_synthesis(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM *p =
		(struct PACKET_CZ_REQ_RANDOM_COMBINE_ITEM *)RFIFOP(fd, 0);
	...
	std::shared_ptr<s_laphine_synthesis> synthesis =
		laphine_synthesis_db.find(sd->state.laphine_synthesis);
...
void clif_laphine_upgrade_open(struct map_session_data *sd,
							   std::shared_ptr<s_laphine_upgrade> upgrade) {
...
void clif_parse_laphine_upgrade(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM *p =
		(struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM *)RFIFOP(fd, 0);
...
void clif_enchantgrade_add(struct map_session_data &sd, uint16 index = UINT16_MAX,
						   std::shared_ptr<s_enchantgradelevel> gradeLevel = nullptr) {
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
	struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST *p =
		(struct PACKET_ZC_GRADE_ENCHANT_MATERIAL_LIST *)packet_buffer;
...
void clif_parse_enchantgrade_add(int fd, struct map_session_data *sd) {
	...
	std::shared_ptr<s_enchantgrade> enchantgrade =
		enchantgrade_db.find(sd->inventory_data[index]->type);
	...
	std::shared_ptr<s_enchantgradelevel> enchantgradelevel =
		util::map_find(enchantgradelevels->second,
					   (e_enchantgrade)sd->inventory.u.items_inventory[index].enchantgrade);

	// Cannot increase enchantgrade any further - no answer, because client should have actually
	// prevented this request
...
/// </param>
void clif_enchantgrade_result(struct map_session_data &sd, uint16 index,
							  e_enchantgrade_result result) {
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
...
void clif_parse_enchantgrade_start(int fd, struct map_session_data *sd) {
	...
	struct PACKET_CZ_GRADE_ENCHANT_REQUEST *p =
		(struct PACKET_CZ_GRADE_ENCHANT_REQUEST *)RFIFOP(fd, 0);
	...
	if (rnd() % 10000 < totalChance) {
		...
		if (option->breaking_rate > 0 && (rnd() % 10000) < option->breaking_rate) {
			...
			sd->inventory.u.items_inventory[index].refine =
				cap_value(sd->inventory.u.items_inventory[index].refine - option->downgrade_amount,
						  0, MAX_REFINE);
...
void clif_reputation_type(struct map_session_data &sd, int64 type, int64 points) {
	...
	p->packetLength =
		sizeof(struct PACKET_ZC_REPUTE_INFO) + sizeof(struct PACKET_ZC_REPUTE_INFO_sub);
	p->success = true;
...
void clif_parse_item_reform_start(int fd, struct map_session_data *sd) {
	...
	std::shared_ptr<s_item_reform_base> base =
		util::umap_find(reform->base_items, selected_item.nameid);
	...
	if (sd->itemid == sd->state.item_reform &&
		pc_delitem(sd, sd->itemindex, 1, 0, 0, LOG_TYPE_REFORM) != 0) {
		return;
	}
...
bool clif_parse_enchant_basecheck(struct item &selected_item,
								  std::shared_ptr<s_item_enchant> enchant) {
	if (selected_item.equip != 0) {
		return false;
	}
...
void clif_parse_enchantwindow_general(int fd, struct map_session_data *sd) {
#if PACKETVER_RE_NUM >= 20211103 || PACKETVER_MAIN_NUM >= 20220330
	struct PACKET_CZ_REQUEST_RANDOM_ENCHANT *p =
		(struct PACKET_CZ_REQUEST_RANDOM_ENCHANT *)RFIFOP(fd, 0);
	...
	if (enchants_for_enchantgrade == nullptr) {
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], msg_txt(sd, 829), false,
...
void clif_parse_enchantwindow_perfect(int fd, struct map_session_data *sd) {
#if PACKETVER_RE_NUM >= 20211103 || PACKETVER_MAIN_NUM >= 20220330
	struct PACKET_CZ_REQUEST_PERFECT_ENCHANT *p =
		(struct PACKET_CZ_REQUEST_PERFECT_ENCHANT *)RFIFOP(fd, 0);
	...
	std::shared_ptr<s_item_enchant_perfect> perfect_enchant =
		util::umap_find(enchant_slot->perfect.enchants, p->itemId);
...
#if PACKETVER_RE_NUM >= 20211103 || PACKETVER_MAIN_NUM >= 20220330
	struct PACKET_CZ_REQUEST_UPGRADE_ENCHANT *p =
		(struct PACKET_CZ_REQUEST_UPGRADE_ENCHANT *)RFIFOP(fd, 0);
...
#if PACKETVER_RE_NUM >= 20211103 || PACKETVER_MAIN_NUM >= 20220330
	struct PACKET_CZ_REQUEST_RESET_ENCHANT *p =
		(struct PACKET_CZ_REQUEST_RESET_ENCHANT *)RFIFOP(fd, 0);
...
void clif_parse_itempackage_select(int fd, struct map_session_data *sd) {
	...
	for (const auto &entry : group->items) {
		...
		} else if (entry.second->amount > 1 &&
				   (!itemdb_isstackable(item.nameid) || item.expire_time > 0)) {
			for (int i = 0; i < entry.second->amount; i++) {
...
void clif_parse_captcha_register(int fd, map_session_data *sd) {
	...
	if (!pc_has_permission(sd, PC_PERM_MACRO_REGISTER)) {
		clif_displaymessage(
			sd->fd,
			msg_txt(sd, 246));	// Your GM level doesn't authorize you to perform this action.
		return;
	}
...
void clif_parse_captcha_upload(int fd, map_session_data *sd) {
	...
	PACKET_CZ_UPLOAD_MACRO_DETECTOR_CAPTCHA *p =
		(PACKET_CZ_UPLOAD_MACRO_DETECTOR_CAPTCHA *)RFIFOP(fd, 0);
...
void clif_parse_captcha_preview_request(int fd, map_session_data *sd) {
	...
	if (!(pc_has_permission(sd, PC_PERM_MACRO_REGISTER) &&
		  pc_has_permission(sd, PC_PERM_MACRO_DETECT))) {
		clif_displaymessage(
			sd->fd,
			msg_txt(sd, 246));	// Your GM level doesn't authorize you to perform this action.
		return;
	...
	if (cd != nullptr) {
		for (uint16 offset = 0; offset < cd->image_size;) {
			...
			PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA *p2 =
				(PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA *)packet_buffer;

			p2->PacketType = HEADER_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA;
			p2->PacketLength =
				(int16)(sizeof(PACKET_ZC_PREVIEW_MACRO_DETECTOR_CAPTCHA) + chunk_size);
...
void clif_macro_detector_request(map_session_data &sd) {
	...
	for (uint16 offset = 0; offset < cd->image_size;) {
		...
		PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA *p2 =
			(PACKET_ZC_APPLY_MACRO_DETECTOR_CAPTCHA *)packet_buffer;
...
void clif_parse_macro_detector_download_ack(int fd, map_session_data *sd) {
	...
	if (sd->macro_detect.retry != 0) {
		// PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA *p =
		// (PACKET_CZ_COMPLETE_APPLY_MACRO_DETECTOR_CAPTCHA *)RFIFOP(fd, 0);
...
void clif_macro_reporter_select(map_session_data &sd, const std::vector<uint32> &aid_list) {
	...
	p->PacketLength = static_cast<int16>(sizeof(PACKET_ZC_ACK_PLAYER_AID_IN_RANGE) +
										 sizeof(uint32) * aid_list.size());
	for (size_t i = 0; i < aid_list.size(); i++) p->AID[i] = aid_list[static_cast<int32>(i)];
...
static int clif_parse(int fd) {
	...
	for (pnum = 0; pnum < 3; ++pnum)  // Limit max packets per cycle to 3 (delay packet spammers)
									  // [FlavioJS]  -- This actually aids packet spammers, but
									  // stuff like /str+ gets slow without it [Ai4rei]
	{								  // begin main client packet processing loop
		...
		if (session[fd]->flag.eof) {
			if (sd) {
				if (sd->state.autotrade) {
					...
					sd->fd = 0;
					ShowInfo("Character '" CL_WHITE "%s" CL_RESET
							 "' logged off (using @autotrade).\n",
							 sd->status.name);
		...
		if (sd) {
			...
			cmd = (cmd ^
				   ((((clif_cryptKey[0] * clif_cryptKey[1]) + clif_cryptKey[2]) >> 16) & 0x7FFF));
		}
#endif
		...
		if (cmd > MAX_PACKET_DB || cmd < MIN_PACKET_DB || packet_db[cmd].len == 0) {
			ShowWarning(
				"clif_parse: Received unsupported packet (packet 0x%04x, %d bytes received), "
			...
			// If it is a connection attempt, check if the packet would have been valid without
			// decrypting it
			if (!sd) {
#ifdef PACKET_OBFUSCATION
		...
		if (packet_len == -1) {	 // variable-length packet
			...
			if (packet_len < 4 || packet_len > 32768) {
				ShowWarning(
					"clif_parse: Received packet 0x%04x specifies invalid packet_len (%d), "
					"disconnecting session #%d.\n",
					cmd, packet_len, fd);
#ifdef DUMP_INVALID_PACKET
				ShowDump(RFIFOP(fd, 0), RFIFOREST(fd));
		...
		if ((int)RFIFOREST(fd) < packet_len) {
			ShowWarning(
				"clif_parse: Received packet 0x%04x with expected packet length %d, but only %d "
				"bytes remaining, disconnecting session #%d.\n",
		...
		RFIFOW(fd, 0) = cmd;
		if (sd)
			sd->cryptKey = ((sd->cryptKey * clif_cryptKey[1]) + clif_cryptKey[2]) &
						   0xFFFFFFFF;	// Update key for the next packet
#endif
...
#ifdef PACKET_OBFUSCATION
	ShowStatus("Packet Obfuscation: " CL_GREEN "Enabled" CL_RESET ". Keys: " CL_WHITE
			   "0x%08X, 0x%08X, 0x%08X" CL_RESET "\n",
...
void do_init_clif(void) {
	...
	for (int i = 0; i < COLOR_MAX; i++) {
		color_table[i] = (colors[i] & 0x0000FF) << 16 | (colors[i] & 0x00FF00) |
						 (colors[i] & 0xFF0000) >> 16;	// RGB to BGR
	}
	...
	if (battle_config.ping_timer_interval > 0) {
		add_timer_interval(gettick() + battle_config.ping_timer_interval * 1000, clif_ping_timer, 0,
						   0, battle_config.ping_timer_interval * 1000);
	}
#endif

	delay_clearunit_ers =
		ers_new(sizeof(struct block_list), "clif.cpp::delay_clearunit_ers", ERS_OPT_CLEAR);
}

void do_final_clif(void) { ers_destroy(delay_clearunit_ers); }
