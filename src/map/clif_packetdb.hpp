...
parseable_packet(HEADER_CZ_MOVE_ITEM_FROM_BODY_TO_CART,
				 sizeof(PACKET_CZ_MOVE_ITEM_FROM_BODY_TO_CART), clif_parse_PutItemToCart, 0);
...
packet(0x018c, 29);
packet(0x018d, -1);
parseable_packet(HEADER_CZ_REQMAKINGITEM, sizeof(struct PACKET_CZ_REQMAKINGITEM),
				 clif_parse_ProduceMix, 0);
...
packet(0x01ac, 6);
packet(0x01ad, -1);
parseable_packet(HEADER_CZ_REQ_MAKINGARROW, sizeof(PACKET_CZ_REQ_MAKINGARROW),
				 clif_parse_SelectArrow, 0);
...
packet(0x01fb, 56);
packet(0x01fc, -1);
parseable_packet(HEADER_CZ_REQ_ITEMREPAIR1, sizeof(struct PACKET_CZ_REQ_ITEMREPAIR1),
				 clif_parse_RepairItem, 0);
packet(0x01fe, 5);
packet(0x01ff, 10);
packet(0x0200, 26);
packet(0x0201, -1);
...
packet(0x020e, 32);
packet(0x025a, -1);
parseable_packet(HEADER_CZ_REQ_MAKINGITEM, sizeof(struct PACKET_CZ_REQ_MAKINGITEM),
				 clif_parse_Cooking, 0);
#endif
...
packet(0x081b, 10);
packet(0x081c, 10);
packet(HEADER_ZC_FAILED_TRADE_BUYING_STORE_TO_SELLER,
	   sizeof(struct PACKET_ZC_FAILED_TRADE_BUYING_STORE_TO_SELLER));
#endif
...
parseable_packet(HEADER_CZ_SSILIST_ITEM_CLICK, sizeof(struct PACKET_CZ_SSILIST_ITEM_CLICK),
				 clif_parse_SearchStoreInfoListItemClick, 0);
packet(0x083D, 6);
#endif
...
parseable_packet(0x08a2, sizeof(struct PACKET_CZ_SSILIST_ITEM_CLICK),
				 clif_parse_SearchStoreInfoListItemClick, 2, 6, 10);
...
parseable_packet(0x0838, sizeof(struct PACKET_CZ_SSILIST_ITEM_CLICK),
				 clif_parse_SearchStoreInfoListItemClick, 2, 6, 10);
parseable_packet(0x0439, 8, clif_parse_UseItem, 2, 4);
...
ack_packet(ZC_PERSONAL_INFOMATION, 0x08cb, 10, 2, 4, 6, 8, 10, 11, 13,
		   15);	 // Still need further information
// Merge Item
...
parseable_packet(0x096A, 6, clif_parse_GetCharNameRequest, 2);
parseable_packet(0x0368, 6, clif_parse_SolveCharName, 2);
parseable_packet(0x08E5, 41, clif_parse_PartyBookingRegisterReq, 2, 4,
				 6);  // Added to prevent disconnections
packet(0x08d2, 10);
#endif
...
parseable_packet(0x0365, sizeof(struct PACKET_CZ_SSILIST_ITEM_CLICK),
				 clif_parse_SearchStoreInfoListItemClick, 2, 6, 10);
...
ack_packet(ZC_PERSONAL_INFOMATION, 0x097b, 16, 2, 4, 8, 12, 16, 17, 21,
		   25);	 // Still need further information
// ack_packet(ZC_PERSONAL_INFOMATION_CHN,0x0981,12,2,4,6,8,12,13,15,17,10); // Disabled until
...
packet(HEADER_ZC_ACK_CASH_BARGAIN_SALE_ITEM_INFO,
	   sizeof(PACKET_ZC_ACK_CASH_BARGAIN_SALE_ITEM_INFO));
parseable_packet(HEADER_CZ_REQ_APPLY_BARGAIN_SALE_ITEM,
				 sizeof(PACKET_CZ_REQ_APPLY_BARGAIN_SALE_ITEM), clif_parse_sale_add, 0);
packet(0x09AF, 4);
parseable_packet(HEADER_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM,
				 sizeof(PACKET_CZ_REQ_REMOVE_BARGAIN_SALE_ITEM), clif_parse_sale_remove, 0);
packet(0x09B1, 4);
...
packet(roulettgenerateackType,
	   sizeof(struct packet_roulette_generate_ack));	   // ZC_ACK_GENERATE_ROULETTE
...
parseable_packet(HEADER_CZ_REQ_STYLE_CHANGE, sizeof(PACKET_CZ_REQ_STYLE_CHANGE),
				 clif_parse_stylist_buy, 0);
parseable_packet(HEADER_CZ_REQ_STYLE_CLOSE, sizeof(PACKET_CZ_REQ_STYLE_CLOSE),
				 clif_parse_stylist_close, 0);
#endif
...
parseable_packet(HEADER_CZ_REQ_UPLOAD_MACRO_DETECTOR, sizeof(PACKET_CZ_REQ_UPLOAD_MACRO_DETECTOR),
				 clif_parse_captcha_register, 0);
...
parseable_packet(HEADER_CZ_ACK_ANSWER_MACRO_DETECTOR, sizeof(PACKET_CZ_ACK_ANSWER_MACRO_DETECTOR),
				 clif_parse_macro_detector_answer, 0);
parseable_packet(HEADER_CZ_REQ_APPLY_MACRO_DETECTOR, sizeof(PACKET_CZ_REQ_APPLY_MACRO_DETECTOR),
				 clif_parse_macro_reporter_ack, 0);
#endif
#if PACKETVER >= 20160323
parseable_packet(HEADER_CZ_REQ_PREVIEW_MACRO_DETECTOR, sizeof(PACKET_CZ_REQ_PREVIEW_MACRO_DETECTOR),
				 clif_parse_captcha_preview_request, 0);
#endif
#if PACKETVER >= 20160330
parseable_packet(HEADER_CZ_REQ_PLAYER_AID_IN_RANGE, sizeof(PACKET_CZ_REQ_PLAYER_AID_IN_RANGE),
				 clif_parse_macro_reporter_select, 0);
#endif

// 2016-03-30aRagexe
#if PACKETVER >= 20160330
parseable_packet(0x0A6E, -1, clif_parse_Mail_send, 2, 4, 28, 52, 60, 62, 64,
				 68);  // CZ_REQ_WRITE_MAIL2
#endif
...
parseable_packet(HEADER_CZ_REFINING_SELECT_ITEM, sizeof(struct PACKET_CZ_REFINING_SELECT_ITEM),
				 clif_parse_refineui_add, 0);
parseable_packet(HEADER_CZ_REQ_REFINING, sizeof(struct PACKET_CZ_REQ_REFINING),
				 clif_parse_refineui_refine, 0);
parseable_packet(HEADER_CZ_CLOSE_REFINING_UI, sizeof(struct PACKET_CZ_CLOSE_REFINING_UI),
				 clif_parse_refineui_close, 0);
#endif
...
parseable_packet(HEADER_CZ_REQ_RANDOM_UPGRADE_ITEM,
				 sizeof(struct PACKET_CZ_REQ_RANDOM_UPGRADE_ITEM), clif_parse_laphine_upgrade, 0);
#endif
...
parseable_packet(0x0A49, sizeof(struct PACKET_CZ_PRIVATE_AIRSHIP_REQUEST),
				 clif_parse_private_airship_request, 0);
packet(0x0A4A, 6);
packet(0x0A4B, 22);
packet(0x0A4C, 28);
#endif
...
parseable_packet(HEADER_CZ_REQ_STYLE_CHANGE2, sizeof(PACKET_CZ_REQ_STYLE_CHANGE2),
				 clif_parse_stylist_buy, 0);
#endif

#if PACKETVER_MAIN_NUM >= 20181002 || PACKETVER_RE_NUM >= 20181002 || PACKETVER_ZERO_NUM >= 20181010
parseable_packet(HEADER_CZ_USE_SKILL_START, sizeof(struct PACKET_CZ_USE_SKILL_START),
				 clif_parse_StartUseSkillToId, 0);
parseable_packet(HEADER_CZ_USE_SKILL_END, sizeof(struct PACKET_CZ_USE_SKILL_END),
				 clif_parse_StopUseSkillToId, 0);
#endif
...
parseable_packet(HEADER_CZ_NPC_BARTER_MARKET_CLOSE,
				 sizeof(struct PACKET_CZ_NPC_BARTER_MARKET_CLOSE), clif_parse_barter_close, 0);
#endif
...
parseable_packet(HEADER_CZ_GUILD_EMBLEM_CHANGE2, sizeof(PACKET_CZ_GUILD_EMBLEM_CHANGE2),
				 clif_parse_GuildChangeEmblem2, 0);
...
parseable_packet(HEADER_CZ_UNINSTALLATION, sizeof(PACKET_CZ_UNINSTALLATION),
				 clif_parse_RemoveOption, 0);
#endif
...
parseable_packet(HEADER_CZ_NPC_EXPANDED_BARTER_MARKET_PURCHASE, -1, clif_parse_barter_extended_buy,
				 0);
...
parseable_packet(HEADER_CZ_SE_CASHSHOP_OPEN2, sizeof(struct PACKET_CZ_SE_CASHSHOP_OPEN2),
				 clif_parse_cashshop_open_request, 0);
parseable_packet(HEADER_CZ_REQ_ITEMREPAIR2, sizeof(struct PACKET_CZ_REQ_ITEMREPAIR2),
				 clif_parse_RepairItem, 0);
#endif

#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
parseable_packet(HEADER_CZ_UNCONFIRMED_TSTATUS_UP, sizeof(PACKET_CZ_UNCONFIRMED_TSTATUS_UP),
				 clif_parse_traitstatus_up, 0);
...
parseable_packet(HEADER_CZ_GRADE_ENCHANT_REQUEST, sizeof(struct PACKET_CZ_GRADE_ENCHANT_REQUEST),
				 clif_parse_enchantgrade_start, 0);
parseable_packet(HEADER_CZ_GRADE_ENCHANT_CLOSE_UI, sizeof(struct PACKET_CZ_GRADE_ENCHANT_CLOSE_UI),
				 clif_parse_enchantgrade_close, 0);
#endif

#if PACKETVER_RE_NUM >= 20211103 || PACKETVER_ZERO_NUM >= 20210818 || PACKETVER_MAIN_NUM >= 20220330
parseable_packet(HEADER_CZ_CHECKNAME2, sizeof(struct PACKET_CZ_CHECKNAME2),
				 clif_parse_Mail_Receiver_Check, 0);
parseable_packet(HEADER_CZ_UNCONFIRMED_RODEX_RETURN,
				 sizeof(struct PACKET_CZ_UNCONFIRMED_RODEX_RETURN), clif_parse_Mail_return, 0);
parseable_packet(HEADER_CZ_REQ_TAKEOFF_EQUIP_ALL, sizeof(struct PACKET_CZ_REQ_TAKEOFF_EQUIP_ALL),
				 clif_parse_unequipall, 0);
parseable_packet(0xb93, 12, clif_parse_dull, 0);
parseable_packet(HEADER_CZ_CLOSE_REFORM_UI, sizeof(struct PACKET_CZ_CLOSE_REFORM_UI),
				 clif_parse_item_reform_close, 0);
parseable_packet(HEADER_CZ_ITEM_REFORM, sizeof(struct PACKET_CZ_ITEM_REFORM),
				 clif_parse_item_reform_start, 0);
parseable_packet(HEADER_CZ_REQUEST_RANDOM_ENCHANT, sizeof(struct PACKET_CZ_REQUEST_RANDOM_ENCHANT),
				 clif_parse_enchantwindow_general, 0);
parseable_packet(HEADER_CZ_REQUEST_PERFECT_ENCHANT,
				 sizeof(struct PACKET_CZ_REQUEST_PERFECT_ENCHANT), clif_parse_enchantwindow_perfect,
				 0);
parseable_packet(HEADER_CZ_REQUEST_UPGRADE_ENCHANT,
				 sizeof(struct PACKET_CZ_REQUEST_UPGRADE_ENCHANT), clif_parse_enchantwindow_upgrade,
				 0);
parseable_packet(HEADER_CZ_REQUEST_RESET_ENCHANT, sizeof(struct PACKET_CZ_REQUEST_RESET_ENCHANT),
				 clif_parse_enchantwindow_reset, 0);
parseable_packet(HEADER_CZ_CLOSE_UI_ENCHANT, sizeof(struct PACKET_CZ_CLOSE_UI_ENCHANT),
				 clif_parse_enchantwindow_close, 0);
#endif

#if PACKETVER_MAIN_NUM >= 20220216
parseable_packet(HEADER_CZ_APPROXIMATE_ACTOR, sizeof(struct PACKET_CZ_APPROXIMATE_ACTOR),
				 clif_parse_dull, 0);
#endif

#if PACKETVER_MAIN_NUM >= 20220216 || PACKETVER_ZERO_NUM >= 20220316
parseable_packet(HEADER_CZ_USE_PACKAGEITEM, sizeof(struct PACKET_CZ_USE_PACKAGEITEM),
				 clif_parse_itempackage_select, 0);
#endif
