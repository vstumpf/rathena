...
int elemental_create(map_session_data *sd, int class_, unsigned int lifetime) {
	...
	switch (db->class_) {
		...
		case ELEMENTALID_ARDOR:	 // ATK + (Summon Agni Skill Level x 20) / HIT + (Summon Agni Skill
								 // Level x 10)
			ele.atk += i * 20;
			ele.atk2 += i * 20;
			ele.hit += i * 10;
		...
		case ELEMENTALID_DILUVIO:  // MDEF + (Summon Aqua Skill Level x 10) / MATK + (Summon Aqua
								   // Skill Level x 20)
			ele.mdef += i * 10;
			ele.matk += i * 20;
			break;
		...
		case ELEMENTALID_PROCELLA:	// FLEE + (Summon Ventus Skill Level x 20) / MATK + (Summon
									// Ventus Skill Level x 10)
			ele.flee += i * 20;
			ele.matk += i * 10;
			break;
		...
		case ELEMENTALID_SERPENS:  // DEF + (Summon Tera Skill Level x 25) / ATK + (Summon Tera
								   // Skill Level x 5)
			ele.def += i * 25;
			ele.atk += i * 5;
			ele.atk2 += i * 5;
...
void elemental_summon_init(s_elemental_data *ed) {
	if (ed->summon_timer == INVALID_TIMER)
		ed->summon_timer = add_timer(gettick() + ed->elemental.life_time, elemental_summon_end,
									 ed->master->bl.id, 0);
...
int elemental_action(s_elemental_data *ed, block_list *bl, t_tick tick) {
	...
	std::shared_ptr<s_elemental_skill> skill = util::umap_find(
		ed->db->skill, EL_SKILLMODE_AGGRESSIVE);  // only one skill per mode is supported
	if (skill == nullptr)
		return 0;
	...
	if (!battle_check_range(&ed->bl, bl, skill_get_range(skill_id, skill_lv))) {
		...
		if (!unit_walktobl(&ed->bl, bl, skill_get_range(skill_id, skill_lv), 2))
			...
			if (skill_get_inf(skill_id) & INF_GROUND_SKILL)
				ed->ud.skilltimer = add_timer(tick + (t_tick)status_get_speed(&ed->bl) * walk_dist,
											  skill_castend_pos, ed->bl.id, 0);
			else
				ed->ud.skilltimer = add_timer(tick + (t_tick)status_get_speed(&ed->bl) * walk_dist,
											  skill_castend_id, ed->bl.id, 0);
		}
		return 1;
	}
...
bool elemental_skillnotok(uint16 skill_id, s_elemental_data *ed) {
	...
	return idx == 0
			   ? false
			   : skill_isNotOk(skill_id,
							   ed->master);	 // return false or check if it,s ok for master as well
}
...
static int elemental_ai_sub_timer_activesearch(block_list *bl, va_list ap) {
	...
	switch (bl->type) {
		...
		default:
			...
			if (((*target) == NULL || !check_distance_bl(&ed->bl, *target, dist)) &&
...
static int elemental_ai_sub_timer(s_elemental_data *ed, map_session_data *sd, t_tick tick) {
	...
	if (mode == EL_MODE_AGGRESSIVE) {
		...
		if (!target)
			map_foreachinallrange(elemental_ai_sub_timer_activesearch, &ed->bl, view_range, BL_CHAR,
								  ed, &target, status_get_mode(&ed->bl));
		...
		if (battle_check_range(&ed->bl, target, view_range) &&
			rnd() % 100 < 2) {	// 2% chance to cast attack skill.
		...
		if (battle_check_range(&ed->bl, target,
							   ed->base_status.rhw.range)) {  // Target within range, engage
...
uint64 ElementalDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (!((id >= ELEMENTALID_AGNI_S && id <= ELEMENTALID_TERA_L) ||
		...
		this->invalidWarning(node["Id"], "Invalid Id %d (valid ranges: %d-%d and %d-%d).\n", id,
							 ELEMENTALID_AGNI_S, ELEMENTALID_TERA_L, ELEMENTALID_DILUVIO,
							 ELEMENTALID_SERPENS);
		return 0;
	}
	...
	if (this->nodeExists(node, "Size")) {
		...
		if (!script_get_constant(size_constant.c_str(), &constant) || constant < SZ_SMALL ||
			constant > SZ_BIG) {
	...
	if (this->nodeExists(node, "ElementLevel")) {
		...
		if (!CHK_ELEMENT_LEVEL(level)) {
			this->invalidWarning(node["ElementLevel"],
								 "Invalid element level %hu, defaulting to 1.\n", level);
			level = 1;
		}
	...
	if (this->nodeExists(node, "Mode")) {
		...
		for (const auto &Modeit : ModeNode) {
			...
			if (!script_get_constant(mode_constant.c_str(), &constant) ||
				constant < EL_SKILLMODE_PASSIVE || constant > EL_SKILLMODE_AGGRESSIVE) {
			...
			if (!SKILL_CHK_ELEM(skill_id)) {
				this->invalidWarning(SkillNode["Skill"],
									 ...
									 skill_name.c_str(), skill_id, EL_SKILLBASE,
...
void do_final_elemental(void) { elemental_db.clear(); }
