...
#define calc_index(x, y) \
	(((x) + (y)*MAX_WALKPATH_NAVI) & (MAX_WALKPATH_NAVI * MAX_WALKPATH_NAVI - 1))
...
static int add_path(struct node_heap *heap, int16 x, int16 y, int g_cost, struct path_node *parent,
					int h_cost) {
 ...
 *------------------------------------------*/
bool navi_path_search(struct navi_walkpath_data *wpd, const struct navi_pos *from,
					  const struct navi_pos *dest, cell_chk cell) {
...
#define chk_dir(d) ((allowed_dirs & (d)) == (d))
		...
		if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_EAST) && !map_getcellp(mapdata, x + 1, y - 1, cell))
			e += add_path(&g_open_set, x + 1, y - 1, g_cost + MOVE_DIAGONAL_COST, current,
						  heuristic(x + 1, y - 1, dest->x, dest->y));  // (x+1, y-1) 5
		if (chk_dir(PATH_DIR_EAST))
			e += add_path(&g_open_set, x + 1, y, g_cost + MOVE_COST, current,
		...
		if (chk_dir(PATH_DIR_NORTH))
			e += add_path(&g_open_set, x, y + 1, g_cost + MOVE_COST, current,
...
void write_header(std::ostream &os, std::string name) { os << name << " = {\n"; }

void write_footer(std::ostream &os) { os << "}\n"; }
...
int map_type(const struct map_data *m) {
	...
	// this is n^2, yikes!
	if (std::find_if(
			m->navi.warps_outof.begin(), m->navi.warps_outof.end(), [&m](const navi_link *link) {
				return std::find_if(m->navi.warps_outof.begin(), m->navi.warps_outof.end(),
									[&link](const navi_link *link2) {
										// find if any two warps in a map cannot be reached
										return !navi_path_search(nullptr, &link->pos, &link2->pos,
																 CELL_CHKNOREACH);
...
void write_spawn(std::ostream &os, const struct map_data *m,
				 const std::shared_ptr<s_mob_db> mobinfo, int amount, int idx) {
	os << "\t{";
	os << "\"" << m->name << "\", ";
	os << "" << idx << ", ";
...
#endif
	os << "\"" << mobinfo->jname.c_str()
	   << "\", ";  // c_str'ed because the strings have been resized to 24
	os << "\"" << mobinfo->sprite.c_str() << "\", ";
	os << "" << mobinfo->lv << ", ";
	os << ""
	   << (((mobinfo->status.ele_lv * 20 + mobinfo->status.def_ele) << 16) |
		   (mobinfo->status.size << 8) | mobinfo->status.race);
	os << "},\n";
}
...
void write_npc_distance(std::ostream &os, const struct npc_data *nd, const struct map_data *msrc) {
	os << "\t\t{ " << nd->navi.id << ", -- (" << nd->name << " " << msrc->name << ", "
	   << nd->navi.pos.x << ", " << nd->navi.pos.y << ")\n";
	for (const auto warp : msrc->navi.warps_into) {
		...
		os << "\t\t\t{ \"" << mdest->name << "\", " << warp->id << ", "
		   << std::to_string(wpd.path_len) << "}, -- (" << msrc->name << ", " << warp->pos.x << ", "
		   << warp->pos.y << ")\n";
	}
	os << "\t\t\t{\"\", 0, 0}\n";
	os << "\t\t},\n";
...
void write_map_distance(std::ostream &os, const struct navi_link *warp1, const struct map_data *m) {
	...
	// 		continue;
	// 	os << "\t\t\t{ \"P\", " << warp2->id << ", " << std::to_string(wpd.path_len) << "}, --
	// ReachableFromSrc warp (" << m->name << ", " << warp2->pos.x << ", " << warp2->pos.y << ")\n";
	// }
	...
	for (const auto warp3 : map_getmapdata(warp1->warp_dest.m)->navi.warps_outof) {
		...
		os << "\t\t\t{ \"E\", " << warp3->id << ", " << std::to_string(wpd.path_len)
		   << "}, -- ReachableFromDst warp (" << map_getmapdata(warp3->pos.m)->name << ", "
		   << warp3->pos.x << ", " << warp3->pos.y << ")\n";
	}
