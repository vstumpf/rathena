...
#define APPLY_HOMUN_LEVEL_STATWEIGHT()                                                    \
	(hom->str_value = hom->agi_value = hom->vit_value = hom->int_value = hom->dex_value = \
		 hom->luk_value = hom->level / 10 - HOMUN_LEVEL_STATWEIGHT_VALUE)
#else
#define APPLY_HOMUN_LEVEL_STATWEIGHT()
#endif
...
enum homun_setting : uint8 {
	HOMSET_NO_SUPPORT_SKILL =
		0x01,  /// Cannot be targetted by support skills, except for their master
	...
	HOMSET_DISPLAY_LUK = 0x10,	/// They display luk/3+1 instead of their actual critical in the
								/// stat window, by default they don't crit
	HOMSET_SAME_MATK = 0x20,	/// Their Min-Matk is always the same as their max
	HOMSET_RESET_REUSESKILL_VAPORIZED =
...
#define hom_is_active(x) \
	((x) && (x)->homunculus.vaporize == HOM_ST_ACTIVE && (x)->battle_status.hp > 0)
