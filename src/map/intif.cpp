...
/**
 * Get sd from pc_db (map_id2db) or auth_db (in case if parsing packet from inter-server when sd not
 * added to pc_db yet)
 * @param account_id
 * @param char_id
 ...
 * @return
 */
int intif_create_pet(uint32 account_id, uint32 char_id, short pet_class, short pet_lv,
					 t_itemid pet_egg_id, t_itemid pet_equip, short intimate, short hungry,
					 char rename_flag, char incubate, const char *pet_name) {
 ...
 */
int intif_broadcast2(const char *mes, int len, unsigned long fontColor, short fontType,
					 short fontSize, short fontAlign, short fontY) {
	nullpo_ret(mes);
	if (len < 2)
		return 0;
...
int intif_main_message(struct map_session_data *sd, const char *message) {
	...
	log_chat(LOG_CHAT_MAINCHAT, 0, sd->status.char_id, sd->status.account_id,
 ...
 * @param flag : 0: success to send wisper, 1: target character is not loged in?, 2: ignored by
 * target
...
int intif_wis_reply(int id, int flag) {
	...
	WFIFOB(inter_fd, 6) = flag;	 // flag: 0: success to send wisper, 1: target character is not
								 // loged in?, 2: ignored by target
	WFIFOSET(inter_fd, 7);
...
int intif_saveregistry(struct map_session_data *sd) {
	...
	for (data = iter->first(iter, &key); iter->exists(iter); data = iter->next(iter, &key)) {
		...
		if (!lValid) {	// this is sql colum size, must be retrive from config
			ShowError(
				"intif_saveregistry: Variable name length is too long (aid: %d, cid: %d): '%s' "
				"sz=%" PRIuPTR "\n",
		...
		if (src->type) {
			...
			if (p->value) {
				...
				++len;
				if (!lValid) {	// error can't be higher; the column size is 254. (nb the
								// transmission limit with be fixed with protobuf revamp)
					ShowDebug(
						"intif_saveregistry: Variable value length is too long (aid: %d, cid: %d): "
						"'%s' sz=%" PRIuPTR
						" to be saved with current system and will be truncated\n",
						sd->status.account_id, sd->status.char_id, p->value, len);
					len = 254;
					p->value[len - 1] =
						'\0';  // this is backward for old char-serv but new one doesn't need this
				}
 ...
 */
int intif_party_leave(int party_id, uint32 account_id, uint32 char_id, const char *name,
					  enum e_party_member_withdraw type) {
	if (CheckForCharServer())
		return 0;
 ...
 * @return 0=error, 1=msg_sent
 */
int intif_guild_memberinfoshort(int guild_id, uint32 account_id, uint32 char_id, int online, int lv,
								int class_) {
	if (CheckForCharServer())
		return 0;
	WFIFOHEAD(inter_fd, 19);
 ...
 * @return 0=error, 1=msg_sent
 */
int intif_guild_change_memberinfo(int guild_id, uint32 account_id, uint32 char_id, int type,
 ...
 * @return  0=error, 1=msg_sent
 */
int intif_guild_alliance(int guild_id1, int guild_id2, uint32 account_id1, uint32 account_id2,
						 int flag) {
	if (CheckForCharServer())
		return 0;
	WFIFOHEAD(inter_fd, 19);
...
int intif_parse_WisMessage(int fd) {
	...
	for (i = 0; i < MAX_IGNORE_LIST && sd->ignore[i].name[0] != '\0' &&
	...
	clif_wis_message(sd, wisp_source, RFIFOCP(fd, 12 + 2 * NAME_LENGTH),
					 RFIFOW(fd, 2) - 12 + 2 * NAME_LENGTH, gmlvl);
...
int intif_parse_WisEnd(int fd) {
	...
	if (battle_config.etc_log)
		ShowInfo("intif_parse_wisend: player: %s, flag: %d\n", RFIFOP(fd, 2),
				 RFIFOB(fd, 26));  // flag: 0: success to send wisper, 1: target character is not
								   // loged in?, 2: ignored by target
...
void intif_parse_Registers(int fd) {
	...
	flag = (sd->vars_received & PRL_ACCG && sd->vars_received & PRL_ACCL &&
	...
	// have it not complain about insertion of vars before loading, and not set those vars as new or
	// modified
	...
	if (flag && sd->vars_received & PRL_ACCG && sd->vars_received & PRL_ACCL &&
		sd->vars_received & PRL_CHAR)
		pc_reg_received(
			sd);  // Received all registry values, execute init scripts and what-not. [Skotlex]
}
...
int intif_parse_LoadGuildStorage(int fd) {
	...
	if (gstor->status) {  // Already open.. lets ignore this update
		ShowWarning(
			"intif_parse_LoadGuildStorage: storage received for a client already open (User "
			"%d:%d)\n",
			flag ? sd->status.account_id : 1, flag ? sd->status.char_id : 1);
		return 0;
	}
	if (gstor->dirty) {	 // Already have storage, and it has been modified and not saved yet!
						 // Exploit! [Skotlex]
		ShowWarning(
			"intif_parse_LoadGuildStorage: received storage for an already modified non-saved "
			"storage! (User %d:%d)\n",
			flag ? sd->status.account_id : 1, flag ? sd->status.char_id : 1);
		return 0;
	}
...
int intif_parse_PartyInfo(int fd) {
	...
	if (RFIFOW(fd, 2) != 8 + sizeof(struct party))
		ShowError(
			"intif: party info : data size error (char_id=%d party_id=%d packet_len=%d "
			"expected_len=%" PRIuPTR ")\n",
...
int intif_parse_PartyOptionChanged(int fd) {
	party_optionchanged(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOW(fd, 10), RFIFOW(fd, 12),
						RFIFOB(fd, 14));
	return 1;
}
...
int intif_parse_PartyMemberWithdraw(int fd) {
	if (battle_config.etc_log)
		ShowInfo(
			...
			RFIFOB(fd, 14 + NAME_LENGTH), RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10),
			RFIFOCP(fd, 14));
	party_member_withdraw(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOCP(fd, 14),
...
int intif_parse_PartyMove(int fd) {
	party_recv_movemap(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOW(fd, 14), RFIFOB(fd, 16),
					   RFIFOW(fd, 17));
	return 1;
}
...
int intif_parse_GuildMemberWithdraw(int fd) {
	guild_member_withdraw(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14),
						  RFIFOCP(fd, 55), RFIFOCP(fd, 15));
	return 1;
}
...
int intif_parse_GuildMemberInfoShort(int fd) {
	guild_recv_memberinfoshort(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOB(fd, 14),
							   RFIFOW(fd, 15), RFIFOW(fd, 17));
	return 1;
}
...
int intif_parse_GuildAlliance(int fd) {
	guild_allianceack(RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), RFIFOB(fd, 18),
					  RFIFOCP(fd, 19), RFIFOCP(fd, 43));
	return 1;
}
...
int intif_parse_GuildEmblemVersionChanged(int fd) {
	guild_emblem_changed(0, RFIFOL(fd, 2), RFIFOL(fd, 6),
						 nullptr);	// Doesn't need emblem length and data
	return 1;
}
...
int intif_parse_ChangeNameOk(int fd) {
	struct map_session_data *sd = NULL;
	if ((sd = map_id2sd(RFIFOL(fd, 2))) == NULL || sd->status.char_id != RFIFOL(fd, 6))
		return 0;
	...
	if (num_received == 0) {
		...
		if (sd->avail_quests < k) {
			...
			if (k < num_received)  // Move the entries at the end to fill the gap
				memmove(&sd->quest_log[k], &sd->quest_log[sd->avail_quests],
						sizeof(struct quest) * (num_received - k));
			sd->quest_log =
				(struct quest *)aRealloc(sd->quest_log, sizeof(struct quest) * sd->num_quests);
		}
	}
...
void intif_parse_achievements(int fd) {
	...
	if (num_received == 0) {
		...
		for (i = 0; i < num_received; i++) {
			...
			if (received[i].completed == 0)	 // Insert at the beginning
				memcpy(&sd->achievement_data.achievements[sd->achievement_data.incompleteCount++],
					   &received[i], sizeof(struct achievement));
			else  // Insert at the end
				memcpy(&sd->achievement_data.achievements[--k], &received[i],
					   sizeof(struct achievement));
			sd->achievement_data.count++;
		}
		if (sd->achievement_data.incompleteCount < k) {
			// sd->achievement_data.incompleteCount and k didn't meet in the middle: some entries
			// were skipped
			if (k < num_received)  // Move the entries at the end to fill the gap
				memmove(&sd->achievement_data.achievements[k],
			...
			sd->achievement_data.achievements = (struct achievement *)aRealloc(
				sd->achievement_data.achievements,
	...
	achievement_level(
		sd, false);	 // Calculate level info but don't give any AG_GOAL_ACHIEVE achievements
	achievement_get_titles(
		sd->status.char_id);  // Populate the title list for completed achievements
...
int intif_achievement_save(struct map_session_data *sd) {
	...
	if (sd->achievement_data.count)
		memcpy(WFIFOP(inter_fd, 8), sd->achievement_data.achievements,
			   sizeof(struct achievement) * sd->achievement_data.count);
	WFIFOSET(inter_fd, len);
...
int intif_parse_Mail_inboxreceived(int fd) {
	...
	} else if (battle_config.mail_show_status &&
			   (battle_config.mail_show_status == 1 || sd->mail.inbox.unread)) {
		char output[128];
		sprintf(output, msg_txt(sd, 510), sd->mail.inbox.unchecked,
				sd->mail.inbox.unread + sd->mail.inbox.unchecked);
 ...
 * @return 0=error, 1=msg sent
 */
bool intif_mail_getattach(struct map_session_data *sd, struct mail_message *msg,
		...
		if (i < MAIL_MAX_INBOX) {
			...
			if (sd->mail.inbox.full || sd->mail.inbox.unchecked > 0)
				intif_Mail_requestinbox(sd->status.char_id, 1,
										type);	// Free space is available for new mails
		}
	}
...
bool intif_mail_checkreceiver(struct map_session_data *sd, char *name) {
	...
	if (tsd != NULL) {
		clif_Mail_Receiver_Ack(sd, tsd->status.char_id, tsd->status.class_, tsd->status.base_level,
							   name);
		return true;
	}
 ...
 * @param page : in case of huge result list display 5 entry per page, (kinda suck that we redo the
 * request atm)
 * @return 0=error, 1=msg sent
 */
int intif_Auction_requestlist(uint32 char_id, short type, int price, const char *searchtext,
							  short page) {
 ...
 * @return
 **/
int intif_broadcast_obtain_special_item(struct map_session_data *sd, t_itemid nameid,
										t_itemid sourceid, unsigned char type) {
	nullpo_retr(0, sd);
	...
	// Send local
	clif_broadcast_obtain_special_item(sd->status.name, nameid, sourceid,
...
void intif_parse_broadcast_obtain_special_item(int fd) {
	...
	clif_broadcast_obtain_special_item(name, RFIFOL(fd, 4), RFIFOW(fd, 8),
									   (enum BROADCASTING_SPECIAL_ITEM_OBTAIN)type);
}
...
void intif_itembound_guild_retrieve(uint32 char_id, uint32 account_id, int guild_id) {
	...
	ShowInfo("Request guild bound item(s) retrieval for CID = " CL_WHITE "%d" CL_RESET
...
void intif_parse_itembound_store2gstorage(int fd) {
	...
	ShowInfo("Retrieved '" CL_WHITE "%d" CL_RESET
			 "' (failed: %d) guild bound item(s) for Guild ID = " CL_WHITE "%d" CL_RESET ".\n",
...
static bool intif_parse_StorageReceived(int fd) {
	...
	if (!sd) {
		ShowError(
			"intif_parse_StorageReceived: No player online for receiving inventory/cart/storage "
			"data (AID: %d)\n",
			account_id);
		return false;
	}
	...
	if (stor->stor_id == p->stor_id) {
		if (stor->status) {	 // Already open.. lets ignore this update
			ShowWarning(
				"intif_parse_StorageReceived: storage received for a client already open (User "
				"%d:%d)\n",
				sd->status.account_id, sd->status.char_id);
			return false;
		}
		if (stor->dirty) {	// Already have storage, and it has been modified and not saved yet!
							// Exploit!
			ShowWarning(
				"intif_parse_StorageReceived: received storage for an already modified non-saved "
				"storage! (User %d:%d)\n",
				sd->status.account_id, sd->status.char_id);
			return false;
		}
	...
	switch (type) {
		case TABLE_INVENTORY: {
			...
			if (sd->status.party_id == 0 &&
				(j = pc_bound_chk(sd, BOUND_PARTY,
								  idxlist))) {	// Party was deleted while character offline
				int i;
				for (i = 0; i < j; i++)
					pc_delitem(sd, idxlist[i], sd->inventory.u.items_inventory[idxlist[i]].amount,
							   4, 1, LOG_TYPE_OTHER);
			}
#endif
			...
			status_calc_weight(sd, (e_status_calc_weight_opt)(
									   CALCWT_ITEM | CALCWT_MAXBONUS));	 // Refresh weight data
		...
		case TABLE_CART:
			...
			if (sd->state.autotrade) {
				...
				sd->autotrade_tid =
					add_timer(gettick() + battle_config.feature_autotrade_open_delay,
							  pc_autotrade_timer, sd->bl.id, 0);
...
void intif_parse_StorageInfo_recv(int fd) {
	...
	for (int i = 0; i < count; i++) {
		...
		std::shared_ptr<struct s_storage_table> storage =
			std::make_shared<struct s_storage_table>();
	...
	switch (cmd) {
		case 0x3800:
			if (RFIFOL(fd, 4) == 0xFF000000)  // Normal announce.
				...
				clif_broadcast2(NULL, RFIFOCP(fd, 16), packet_len - 16, RFIFOL(fd, 4),
								RFIFOW(fd, 8), RFIFOW(fd, 10), RFIFOW(fd, 12), RFIFOW(fd, 14),
								ALL_CLIENT);
			break;
		case 0x3801:
