...
#define GUILD_PAYEXP_INTERVAL \
	10000						// Interval (maximum survival time of the cache, in milliseconds)
...
uint64 GuildSkillTreeDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (!SKILL_CHK_GUILD(skill_id)) {
		this->invalidWarning(
			node["Id"],
			"Guild skill \"%s\" with Id %u is out of the guild skill range [%u-%u], skipping.\n",
	...
	if (this->nodeExists(node, "Required")) {
		...
		for (const auto &requiredNode : reqNode) {
			...
			if (!(requiredSkillId = skill_name2id(requiredName.c_str()))) {
				this->invalidWarning(requiredNode["Id"],
									 "Invalid required guild skill name \"%s\", skipping.\n",
									 requiredName.c_str());
				return 0;
			}
			...
			if (!SKILL_CHK_GUILD(requiredSkillId)) {
				this->invalidWarning(requiredNode["Id"],
									 "Required guild skill \"%s\" with Id %u is out of the guild "
									 "skill range [%u-%u], skipping.\n",
									 requiredName.c_str(), requiredSkillId, GD_SKILLBASE, GD_MAX);
				return 0;
			}

			std::shared_ptr<s_guild_skill_requirement> requirement =
				util::umap_find(skill->need, requiredSkillId);
...
struct guild *guild_searchname(char *str) {
	...
	for (g = (struct guild *)dbi_first(iter); dbi_exists(iter);
		 g = (struct guild *)dbi_next(iter)) {
...
int guild_payexp_timer_sub(DBKey key, DBData *data, va_list ap) {
	...
	intif_guild_change_memberinfo(g->guild_id, c->account_id, c->char_id, GMI_EXP,
								  &g->member[i].exp, sizeof(g->member[i].exp));
	c->exp = 0;
...
int guild_send_xy_timer_sub(DBKey key, DBData *data, va_list ap) {
	...
	for (i = 0; i < g->max_member; i++) {
		...
		if (sd != NULL && sd->fd && (sd->guild_x != sd->bl.x || sd->guild_y != sd->bl.y) &&
			!sd->bg_id) {
			clif_guild_xy(sd);
			sd->guild_x = sd->bl.x;
			sd->guild_y = sd->bl.y;
...
// Information request
int guild_request_info(int guild_id) { return intif_guild_request_info(guild_id); }
...
int guild_recv_info(struct guild *sg) {
	...
	if (g->max_member > MAX_GUILD) {
		ShowError(
			"guild_recv_info: Received guild with %d members, but MAX_GUILD is only %d. Extra "
			"guild-members have been lost!\n",
	...
	for (i = bm = m = 0; i < g->max_member; i++) {
		if (g->member[i].account_id > 0) {
			sd = g->member[i].sd =
				guild_sd_check(g->guild_id, g->member[i].account_id, g->member[i].char_id);
			if (sd)
				clif_name_area(&sd->bl);  // [LuzZza]
			m++;
		} else
	...
	for (i = 0; i < g->max_member; i++) {  // Transmission of information at all members
		...
		if (channel_config.ally_tmpl.name[0] &&
			(channel_config.ally_tmpl.opt & CHAN_OPT_AUTOJOIN)) {
 ...
 *----------------------------------------*/
int guild_leave(struct map_session_data *sd, int guild_id, uint32 account_id, uint32 char_id,
				const char *mes) {
	struct guild *g;
	...
	if (sd->status.account_id != account_id || sd->status.char_id != char_id ||
		sd->status.guild_id != guild_id || map_flag_gvg2(sd->bl.m))
		return 0;
 ...
 *----------------------------------------*/
int guild_expulsion(struct map_session_data *sd, int guild_id, uint32 account_id, uint32 char_id,
					const char *mes) {
	struct map_session_data *tsd;
	struct guild *g;
	int i, ps;
	...
	if (sd != NULL && sd->status.guild_id == guild_id) {
		...
		if (g->instance_id) {
			...
			if (mapdata->instance_id) {	 // User was on the instance map
				if (mapdata->save.map)
					pc_setpos(sd, mapdata->save.map, mapdata->save.x, mapdata->save.y,
							  CLR_TELEPORT);
				else
...
void guild_retrieveitembound(uint32 char_id, uint32 account_id, int guild_id) {
	...
	if (sd) {  // Character is online
		...
		if (j) {
			...
			for (i = 0; i < j;
				 i++) {	 // Loop the matching items, gstorage_additem takes care of opening storage
				if (stor)
					storage_guild_additem(sd, stor, &sd->inventory.u.items_inventory[idxlist[i]],
										  sd->inventory.u.items_inventory[idxlist[i]].amount);
				pc_delitem(sd, idxlist[i], sd->inventory.u.items_inventory[idxlist[i]].amount, 0, 4,
						   LOG_TYPE_GSTORAGE);
			}
...
int guild_send_memberinfoshort(struct map_session_data *sd, int online) {  // cleaned up [LuzZza]
	...
	intif_guild_memberinfoshort(g->guild_id, sd->status.account_id, sd->status.char_id, online,
								sd->status.base_level, sd->status.class_);
	...
	if (sd->state.connect_new) {  // Note that this works because it is invoked in parse_LoadEndAck
								  // before connect_new is cleared.
		clif_guild_belonginfo(*sd);
		sd->guild_emblem_id = g->emblem_id;
	}
	return 0;
...
int guild_recv_memberinfoshort(int guild_id, uint32 account_id, uint32 char_id, int online, int lv,
							   int class_) {  // cleaned up [LuzZza]
...
int guild_send_message(struct map_session_data *sd, const char *mes, int len) {
	...
	log_chat(LOG_CHAT_GUILD, sd->status.guild_id, sd->status.char_id, sd->status.account_id,
 ...
 * Change guild notice
 *---------------------------------------------------*/
int guild_change_notice(struct map_session_data *sd, int guild_id, const char *mes1,
						const char *mes2) {
	nullpo_ret(sd);
...
bool guild_check_emblem_change_condition(map_session_data *sd) {
	...
	if (battle_config.require_glory_guild && g != nullptr &&
	...
	{  // update guardians (mobs)
		for (const auto &it : castle_db) {
			...
			for (i = 0; i < ARRAYLENGTH(it.second->guardian); ++i) {
				TBL_MOB *md =
					(it.second->guardian[i].id ? map_id2md(it.second->guardian[i].id) : NULL);
			...
			for (i = 0; i < it.second->temp_guardians_max; ++i) {
				TBL_MOB *md =
					(it.second->temp_guardians[i] ? map_id2md(it.second->temp_guardians[i]) : NULL);
				if (md == NULL || md->guardian_data == NULL)
					continue;
				md->guardian_data->emblem_id = emblem_id;
				clif_guild_emblem_area(&md->bl);
...
t_exp guild_payexp(struct map_session_data *sd, t_exp exp) {
	...
	if (sd->status.guild_id == 0 || (g = sd->guild) == NULL || (per = guild_getposition(sd)) < 0 ||
	...
	c = (struct guild_expcache *)db_data2ptr(guild_expcache_db->ensure(
		guild_expcache_db, db_i2key(sd->status.char_id), create_expcache, sd));
...
void guild_guildaura_refresh(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	std::shared_ptr<s_skill_unit_group> group =
		skill_unitsetting(&sd->bl, skill_id, skill_lv, sd->bl.x, sd->bl.y, 0);

	if (group)
		sc_start4(NULL, &sd->bl, type, 100, (battle_config.guild_aura & 16) ? 0 : skill_lv, 0, 0,
				  group->group_id,
				  600000);	// duration doesn't matter these status never end with val4
	return;
}
...
void guild_block_skill(struct map_session_data *sd, int time) {
	...
	int i;
	for (i = 0; i < 4; i++) skill_blockpc_start(sd, skill_id[i], time);
}
...
int guild_check_alliance(int guild_id1, int guild_id2, int flag) {
	...
	ARR_FIND(0, MAX_GUILDALLIANCE, i,
			 g->alliance[i].guild_id == guild_id2 && g->alliance[i].opposition == flag);
...
int guild_reqalliance(struct map_session_data *sd, struct map_session_data *tsd) {
	...
	if (is_agit_start()) {	// Disable alliance creation during woe [Valaris]
		clif_displaymessage(sd->fd,
							msg_txt(sd, 676));	//"Alliances cannot be made during Guild Wars!"
		return 0;
...
int guild_reply_reqalliance(struct map_session_data *sd, uint32 account_id, int flag) {
	...
	if (sd->guild_alliance !=
		tsd->status.guild_id)  // proposed guild_id alliance doesn't match tsd guildid
		return 0;
	...
	if (flag == 1) {  // consent
		...
		intif_guild_alliance(sd->status.guild_id, tsd->status.guild_id, sd->status.account_id,
							 tsd->status.account_id, 0);
		return 0;
	} else {  // deny
		sd->guild_alliance = 0;
...
int guild_delalliance(struct map_session_data *sd, int guild_id, int flag) {
	...
	if (is_agit_start()) {	// Disable alliance breaking during woe [Valaris]
		clif_displaymessage(sd->fd,
							msg_txt(sd, 677));	//"Alliances cannot be broken during Guild Wars!"
		return 0;
	}  // end addition [Valaris]
...
int guild_allianceack(int guild_id1, int guild_id2, uint32 account_id1, uint32 account_id2,
	...
	for (i = 0; i < 2 - (flag & 1);
		 i++) {	 // Retransmission of the relationship list to all members
		if (g[i] != NULL)
			for (j = 0; j < g[i]->max_member; j++) {
...
void castle_guild_broken_sub(int guild_id) {
	for (const auto &it : castle_db) {
		...
		if (gc->guild_id == guild_id) {
			...
			safesnprintf(name, EVENT_NAME_LENGTH, "%s::%s", gc->castle_event,
						 script_config.guild_break_event_name);
			npc_event_do(name);
...
int guild_gm_changed(int guild_id, uint32 account_id, uint32 char_id, time_t time) {
	...
	for (pos = 0; pos < g->max_member &&
				  !(g->member[pos].account_id == account_id && g->member[pos].char_id == char_id);
		 pos++)
		;
	...
	if (g->member[pos].sd && g->member[pos].sd->fd) {
		clif_displaymessage(
			g->member[pos].sd->fd,
			msg_txt(g->member[pos].sd, 678));  //"You no longer are the Guild Master."
		g->member[pos].sd->state.gmaster_flag = 0;
		clif_name_area(&g->member[pos].sd->bl);
	}

	if (g->member[0].sd && g->member[0].sd->fd) {
		clif_displaymessage(g->member[0].sd->fd,
							msg_txt(g->member[pos].sd, 679));  //"You have become the Guild Master!"
...
void guild_castle_map_init(void) {
	...
	if (!castle_ids.empty() && intif_guild_castle_dataload(castle_ids)) {
		ShowStatus("Requested '" CL_WHITE "%" PRIdPTR CL_RESET
				   "' guild castles from char-server...\n",
				   castle_ids.size());
	}
}
...
int guild_castledatasave(int castle_id, int index, int value) {
	...
	switch (index) {
		case CD_GUILD_ID:  // The castle's owner has changed? Update or remove Guardians too.
						   // [Skotlex]
		{
			int i;
			gc->guild_id = value;
	...
	if (ev < 0) {  // No castles owned, invoke OnAgitInit as it is.
		...
		for (i = 0; i < n; i++, gc++) {
			...
			memcpy(&c->guild_id, &gc->guild_id,
				   sizeof(struct guild_castle) - offsetof(struct guild_castle, guild_id));
			...
			if (c->guild_id) {
				if (i != ev)
					...
					snprintf(event_name, EVENT_NAME_LENGTH, "::%s",
							 script_config.agit_init_event_name);
					guild_npc_request_info(c->guild_id, event_name);
					snprintf(event_name, EVENT_NAME_LENGTH, "::%s",
							 script_config.agit_init2_event_name);
					guild_npc_request_info(c->guild_id, event_name);
					snprintf(event_name, EVENT_NAME_LENGTH, "::%s",
							 script_config.agit_init3_event_name);
					guild_npc_request_info(c->guild_id, event_name);
				}
			}
		}
...
void do_init_guild(void) {
	...
	add_timer_interval(gettick() + GUILD_PAYEXP_INTERVAL, guild_payexp_timer, 0, 0,
					   GUILD_PAYEXP_INTERVAL);
	add_timer_interval(gettick() + GUILD_SEND_XY_INTERVAL, guild_send_xy_timer, 0, 0,
					   GUILD_SEND_XY_INTERVAL);
}
