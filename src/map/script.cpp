 ...
 **/
static bool script_accid2sd_(struct script_state *st, uint8 loc, struct map_session_data **sd,
							 const char *func) {
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_charid2sd_(struct script_state *st, uint8 loc, struct map_session_data **sd,
							  const char *func) {
	if (script_hasdata(st, loc)) {
		int id_ = script_getnum(st, loc);
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_nick2sd_(struct script_state *st, uint8 loc, struct map_session_data **sd,
							const char *func) {
	if (script_hasdata(st, loc)) {
 ...
 * @return True if `sd` is assigned, false otherwise
 **/
static bool script_mapid2sd_(struct script_state *st, uint8 loc, struct map_session_data **sd,
							 const char *func) {
	if (script_hasdata(st, loc)) {
		int id_ = script_getnum(st, loc);
		if (!(*sd = map_id2sd(id_))) {
 ...
 **/
static bool script_rid2bl_(struct script_state *st, uint8 loc, struct block_list **bl,
						   const char *func) {
	int unit_id;
...
struct Script_Config script_config = {
	...
	// NPC related
	"OnTouch_",	 // ontouch_event_name (runs on first visible char to enter area, picks another char
				 // if the first char leaves)
	...
	"OnInterIfInitOnce",  // inter_init_once_event_name (is only executed on the first inter server
						  // connection)
	// Guild related
	...
	"OnAgitInit3",	 // agit_init3_event_name (is executed after all castle owning guilds have been
					// loaded)
	...
	"OnSun",  // timer_sunday_event_name (is executed by a timer on sunday at the specific hour and
			  // minute)
	"OnMon",  // timer_monday_event_name (is executed by a timer on monday at the specific hour and
			  // minute)
	"OnTue",  // timer_tuesday_event_name (is executed by a timer on tuesday at the specific hour
			  // and minute)
	"OnWed",  // timer_wednesday_event_name (is executed by a timer on wednesday at the specific
			  // hour and minute)
	"OnThu",  // timer_thursday_event_name (is executed by a timer on thursday at the specific hour
			  // and minute)
	"OnFri",  // timer_friday_event_name (is executed by a timer on friday at the specific hour and
			  // minute)
	"OnSat",  // timer_saturday_event_name (is executed by a timer on saturday at the specific hour
			  // and minute)
	// Instance related
...
extern short current_equip_item_index;	// for New CARDS Scripts. It contains Inventory Index of the
										// EQUIP_SCRIPT caller item. [Lupus]
		...
		switch (data->type) {
			...
			case C_NAME:
				ShowMessage(" \"%s\" (id=%d ref=%p subtype=%s)\n", reference_getname(data),
							data->u.num, data->ref, script_op2name(str_data[data->u.num].type));
				break;
			...
			case C_RETINFO: {
				...
				ShowMessage(
					...
					ri, ri->scope.vars, ri->scope.arrays, ri->script, ri->pos, ri->nargs,
					ri->defsp);
			} break;
			default:
				ShowMessage("\n");
...
static void script_reportsrc(struct script_state *st) {
	...
	switch (bl->type) {
		case BL_NPC: {
			...
			if (bl->m >= 0)
				ShowDebug("Source (NPC): %s at %s (%d,%d)\n", nd->name, map_mapid2mapname(bl->m),
						  bl->x, bl->y);
			else
		...
		default:
			if (bl->m >= 0)
				ShowDebug("Source (Non-NPC type %d): name %s at %s (%d,%d)\n", bl->type,
						  status_get_name(bl), map_mapid2mapname(bl->m), bl->x, bl->y);
			else
...
static void script_reportfunc(struct script_state *st) {
	...
	if (!data_isreference(data) ||
		str_data[reference_getid(data)].type !=
...
static void check_event(struct script_state *st, const char *evt) {
	if (evt && evt[0] && !stristr(evt, "::On")) {
		ShowWarning(
			"NPC event parameter deprecated! Please use 'NPCNAME::OnEVENT' instead of '%s'.\n",
			evt);
		script_reportsrc(st);
	}
}
...
#else							// athena hash
	h = 0;
	while (*p) h = (h << 1) + (h >> 3) + (h >> 5) + (h >> 8) + (unsigned char)TOLOWER(*p++);
#endif
...
void set_label(int l, int pos, const char *script_pos_cur) {
	...
	if (str_data[l].type == C_INT || str_data[l].type == C_PARAM ||
		str_data[l].type == C_FUNC) {  // Prevent overwriting constants values, parameters and
									   // built-in functions [Skotlex]
			...
			for (;;) {
				if (*p == '\0') {
					disp_warning_message(
						"script:script->skip_space: end of file while parsing block comment. "
						"expected " CL_BOLD "*/" CL_NORM,
						p);
					return p;
				}
...
static int add_word(const char *p) {
	...
	if (len == 0)
		disp_error_message(
			"script:add_word: invalid word. A word consists of undercores and/or alphanumeric "
			"characters, and valid variable prefixes/postfixes.",
			p);
...
static const char *parse_callfunc(const char *p, int require_paren, int is_custom) {
	...
	if (str_data[func].type == C_FUNC) {
		...
		if (*arg == 0)
			disp_error_message(
				"parse_callfunc: callsub has no arguments, please review its definition", p);
		if (*arg != '*')
...
const char *parse_variable(const char *p) {
	...
	if (str_data[word].type == C_FUNC || str_data[word].type == C_USERFUNC ||
		str_data[word].type ==
			C_USERFUNC_POS)	 // cannot assign a variable which exists as a function or label
		disp_error_message(
			"Cannot modify a variable which has the same name as a function or label.", p);

	parse_variable_sub_push(word, p2);	// Push variable onto the stack

	if (type != C_EQ)
		parse_variable_sub_push(
			word, p2);	// Push variable onto the stack once again (first argument of setr)
...
const char *parse_simpleexpr(const char *p) {
	...
	if (*p == '(') {
		...
		if (i < SCRIPT_INT_MIN || (error && i == INT64_MIN)) {
			i = SCRIPT_INT_MIN;
			disp_warning_message(
				"parse_simpleexpr: underflow detected, capping value to SCRIPT_INT_MIN", p);
		} else if (i > SCRIPT_INT_MAX || (error && i == INT64_MAX)) {
			i = SCRIPT_INT_MAX;
			disp_warning_message(
				"parse_simpleexpr: overflow detected, capping value to SCRIPT_INT_MAX", p);
		}
		...
		while (*p && *p != '"') {
			if ((unsigned char)p[-1] <= 0x7e && *p == '\\') {
				...
				if (n != 1)
					ShowDebug(
						"parse_simpleexpr: unexpected length %d after unescape (\"%.*s\" -> "
						"%.*s)\n",
		...
		l = add_word(p);
		if (str_data[l].type == C_FUNC || str_data[l].type == C_USERFUNC ||
...
const char *parse_subexpr(const char *p, int limit) {
	...
	if ((op = C_ADD_PRE, p[0] == '+' && p[1] == '+') ||
		...
		p = parse_variable(p);
	else if ((op = C_NEG, *p == '-') || (op = C_LNOT, *p == '!') ||
			 (op = C_NOT, *p == '~')) {	 // Unary - ! ~ operators
	...
	while (((op = C_OP3, opl = 0, len = 1, *p == '?') ||
			...
			(op = C_OR, opl = 3, len = 1, *p == '|') || (op = C_XOR, opl = 4, len = 1, *p == '^') ||
	...
	switch (*p) {
			...
			if (p2 - p == 4 && !strncasecmp(p, "case", 4)) {
				...
				if (pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
					...
					if (syntax.curly[pos].count != 1) {
						// Jump for FALLTHRU
						sprintf(label, "goto __SW%x_%xJ;", syntax.curly[pos].index,
						...
						// You are here labeled
						sprintf(label, "__SW%x_%x", syntax.curly[pos].index,
								syntax.curly[pos].count);
						l = add_str(label);
						set_label(l, script_pos, p);
					}
					...
					if (is_number(p)) {
						...
						v = (int)strtol(p, &np, 0);
						if ((*p == '-' || *p == '+') &&
							ISDIGIT(p[1]))	// pre-skip because '-' can not skip_word
							p++;
						p = skip_word(p);
						if (np != p)
					...
					sprintf(label, "if(%" PRId64 " != $@__SW%x_VAL) goto __SW%x_%x;", v,
							syntax.curly[pos].index, syntax.curly[pos].index,
							syntax.curly[pos].count + 1);
					syntax.curly[syntax.curly_count++].type = TYPE_NULL;
					...
					if (syntax.curly[pos].count != 1) {
						...
						sprintf(label, "__SW%x_%xJ", syntax.curly[pos].index,
								syntax.curly[pos].count);
						l = add_str(label);
						set_label(l, script_pos, p);
					}
					// check duplication of case label [Rayce]
					if (linkdb_search(&syntax.curly[pos].case_label, (void *)__64BPRTSIZE(v)) !=
						NULL)
						disp_error_message("parse_syntax: dup 'case'", p);
					linkdb_insert(&syntax.curly[pos].case_label, (void *)__64BPRTSIZE(v),
								  (void *)1);
		...
		case 'D':
			if (p2 - p == 7 && !strncasecmp(p, "default", 7)) {
				...
				if (pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
					...
					sprintf(label, "goto __SW%x_%x;", syntax.curly[pos].index,
							syntax.curly[pos].count + 1);
					syntax.curly[syntax.curly_count++].type = TYPE_NULL;
					parse_line(label);
					syntax.curly_count--;
		...
		case 'F':
			if (p2 - p == 3 && !strncasecmp(p, "for", 3)) {
				...
				return p;
					   ...
					   strncasecmp(p, "function", 8) == 0) {  // internal script function
				const char *func_name;

				func_name = skip_space(p2);
				p = skip_word(func_name);
				if (p == func_name)
					disp_error_message("parse_syntax:function: function name is missing or invalid",
									   p);
				p2 = skip_space(p);
				if (*p2 == ';') {  // function <name> ;
					...
					if (str_data[l].type ==
						C_NOP)	// register only, if the name was not used by something else
						str_data[l].type = C_USERFUNC;
					else if (str_data[l].type == C_USERFUNC)
						;  // already registered
					else
						disp_error_message("parse_syntax:function: function name is invalid",
										   func_name);
					...
					if (str_data[l].type == C_NOP ||
						str_data[l].type == C_USERFUNC)	 // register only, if the name was not used
														 // by something else
					{
						str_data[l].type = C_USERFUNC;
						set_label(l, script_pos, p);
		...
		case 'I':
			if (p2 - p == 2 && !strncasecmp(p, "if", 2)) {
				...
				syntax.curly[syntax.curly_count].index = syntax.index++;
				syntax.curly[syntax.curly_count].flag = 0;
				sprintf(label, "__IF%x_%x", syntax.curly[syntax.curly_count].index,
						syntax.curly[syntax.curly_count].count);
				syntax.curly_count++;
				add_scriptl(add_str("jump_zero"));
				add_scriptc(C_ARG);
				p = parse_expr(p);
...
void script_set_constant_(const char *name, int64 value, const char *constant_name,
	...
	} else if (str_data[n].type == C_PARAM ||
			   str_data[n].type == C_INT) {	 // existing parameter or constant
		ShowError("script_set_constant: Attempted to overwrite existing %s '%s' (old value=%" PRId64
				  ", new value=%" PRId64 ").\n",
				  (str_data[n].type == C_PARAM) ? "parameter" : "constant", name, str_data[n].val,
				  value);
	} else {  // existing name
...
void script_errorwarning_sub(StringBuf *buf, const char *src, const char *file, int start_line,
							 const char *error_msg_cur, const char *error_pos_cur) {
...
void script_error(const char *src, const char *file, int start_line, const char *error_msg_cur,
...
void script_warning(const char *src, const char *file, int start_line, const char *error_msg_cur,
					const char *error_pos_cur) {
	StringBuf buf;
...
struct script_code *parse_script(const char *src, const char *file, int line, int options) {
	...
	// who called parse_script is responsible for clearing the database after using it, but just in
	// case... lets clear it here
	...
	if (setjmp(error_jump) != 0) {
		...
		for (j = 0; j < size; j++) linkdb_final(&syntax.curly[j].case_label);
		return NULL;
	}
	...
	if (options & SCRIPT_IGNORE_EXTERNAL_BRACKETS) {  // does not require brackets around the script
		if (*p == '\0' &&
			!(options & SCRIPT_RETURN_EMPTY_SCRIPT)) {	// empty script and can return NULL
			aFree(script_buf);
			script_pos = 0;
			script_size = 0;
			script_buf = NULL;
...
static bool script_rid2sd_(struct script_state *st, struct map_session_data **sd,
						   const char *func) {
	*sd = map_id2sd(st->rid);
 ...
 */
struct script_data *get_val_(struct script_state *st, struct script_data *data,
	...
	if (!reference_toconstant(data) && not_server_variable(prefix)) {
		if (sd == NULL && !script_rid2sd(sd)) {	 // needs player attached
			if (postfix == '$') {				 // string variable
				ShowWarning(
					"script:get_val: cannot access player variable '%s', defaulting to \"\"\n",
					name);
			...
			case '\'': {
				...
				if (data->ref)
					...
					std::shared_ptr<s_instance_data> idata =
						util::umap_find(instances, script_instancegetid(st));
				...
				if (n)
					...
					ShowWarning(
						"script:get_val: cannot access instance variable '%s', defaulting to "
						"\"\"\n",
						name);
					data->u.str = NULL;
				}
...
struct script_data *push_val2(struct script_stack *stack, enum c_op type, int64 val,
							  struct reg_db *ref);
...
const char *get_val2_str(struct script_state *st, int64 uid, struct reg_db *ref) {
	...
	// Do NOT remove the value from stack here, the pointer is returned here and will be used by the
	// caller [Lemongrass] script_removetop( st, -1, 0 );
...
/**
 * Because, currently, array members with key 0 are indifferenciable from normal variables, we
 *should ensure its actually in Will be gone as soon as undefined var feature is implemented
 **/
void script_array_ensure_zero(struct script_state *st, struct map_session_data *sd, int64 uid,
	...
	struct reg_db *src =
		script_array_src(st, sd ? sd : st->rid ? map_id2sd(st->rid) : NULL, name, ref);
	bool insert = false;
	...
	if (src && src->arrays) {
		struct script_array *sa =
			static_cast<script_array *>(idb_get(src->arrays, script_getvarid(uid)));
		if (sa) {
			unsigned int i;
...
/**
 * Returns array's highest key (for that awful getarraysize implementation that doesn't really gets
 *the array size)
 **/
 ...
 * @param ref:   variable container, in case of a npc/scope variable reference outside the current
 *scope.
 * @return: 0 failure, 1 success.
 *
 * TODO: return values are screwed up, have been for some time (reaad: years), e.g. some functions
 *return 1 failure and success.
 *------------------------------------------*/
bool set_reg_str(struct script_state *st, struct map_session_data *sd, int64 num, const char *name,
	...
	switch (prefix) {
		...
		case '#':
			return (name[1] == '#') ? pc_setaccountreg2str(sd, num, value)
									: pc_setaccountregstr(sd, num, value);
		case '.': {
			struct reg_db *n =
				(ref) ? ref : (name[1] == '@') ? &st->stack->scope : &st->script->local;
		...
		case '\'': {
			...
			if (src) {
				...
				ShowError(
					"script_set_reg: cannot write instance variable '%s', NPC not in a instance!\n",
					name);
				script_reportsrc(st);
			}
...
bool set_reg_num(struct script_state *st, struct map_session_data *sd, int64 num, const char *name,
	...
	switch (prefix) {
		...
		case '#':
			return (name[1] == '#') ? pc_setaccountreg2(sd, num, value)
									: pc_setaccountreg(sd, num, value);
		case '.': {
			struct reg_db *n =
				(ref) ? ref : (name[1] == '@') ? &st->stack->scope : &st->script->local;
 ...
 * @param sd
 */
const char *conv_str_(struct script_state *st, struct script_data *data,
					  struct map_session_data *sd) {
	char *p;
...
int64 conv_num_(struct script_state *st, struct script_data *data, struct map_session_data *sd) {
	...
	if (data_isint(data)) {			   // nothing to convert
		...
		num = strtoll(
			data->u.str, NULL,
			10);  // change radix to 0 to support octal numbers "o377" and hex numbers "0xFF"
		error = (errno == ERANGE);

		if (num < SCRIPT_INT_MIN || (error && num == INT64_MIN)) {
			num = SCRIPT_INT_MIN;
			ShowError("script:conv_num: underflow detected, capping value to %" PRId64 "",
					  SCRIPT_INT_MIN);
			script_reportdata(data);
			script_reportsrc(st);
		} else if (num > SCRIPT_INT_MAX || (error && num == INT64_MAX)) {
			num = SCRIPT_INT_MAX;
			ShowError("script:conv_num: overflow detected, capping value to %" PRId64 "",
					  SCRIPT_INT_MAX);
			script_reportdata(data);
			script_reportsrc(st);
		}
...
void stack_expand(struct script_stack *stack) {
	stack->sp_max += 64;
	stack->stack_data = (struct script_data *)aRealloc(
		stack->stack_data, stack->sp_max * sizeof(stack->stack_data[0]));
...
struct script_data *push_copy(struct script_stack *stack, int pos) {
	switch (stack->stack_data[pos].type) {
		...
		case C_RETINFO:
			...
			break;
		default:
			return push_val2(stack, stack->stack_data[pos].type, stack->stack_data[pos].u.num,
...
void pop_stack(struct script_state *st, int start, int end) {
	...
	if (stack->sp > end) {
		...
		for (i = start + stack->sp - end; i < stack->sp; ++i) stack->stack_data[i].type = C_NOP;
	}
...
void script_free_state(struct script_state *st) {
	if (idb_exists(st_db, st->id)) {
		...
		if (st->bk_st)	// backup was not restored
			ShowDebug(
				"script_free_state: Previous script state lost (rid=%d, oid=%d, state=%d, "
				"bk_npcid=%d).\n",
...
void op_2num(struct script_state *st, int op, int64 i1, int64 i2) {
	...
	switch (op) {
		...
		case C_MOD:
			if (i2 == 0) {
				ShowError("script:op_2num: division by zero detected op=%s i1=%" PRId64
						  " i2=%" PRId64 "\n",
		...
		default:
			...
			switch (op) {
				...
				case C_MUL:
					...
					break;
				default:
					ShowError("script:op_2num: unexpected number operator %s i1=%" PRId64
							  " i2=%" PRId64 "\n",
							  script_op2name(op), i1, i2);
					script_reportsrc(st);
					script_pushnil(st);
...
void op_2(struct script_state *st, int op) {
	...
	switch (op) {
		case C_ADD:
			if (data_isint(left) && data_isstring(right)) {	 // convert int-string to string-string
				conv_str(st, left);
			} else if (data_isstring(left) &&
					   data_isint(right)) {	 // convert string-int to string-string
				conv_str(st, right);
			}
			break;
	}
	...
	if (data_isstring(left) && data_isstring(right)) {	// ss => op_2str
		...
		script_removetop(st, leftref.type == C_NOP ? -3 : -2,
						 -1);  // pop the two values before the top one
...
static void script_check_buildin_argtype(struct script_state *st, int func) {
	...
	for (idx = 2; script_hasdata(st, idx); idx++) {
		...
		if (type == '?' || type == '*') {  // optional argument or unknown number of optional
										   // parameters ( no types are after this )
			break;
		} else if (type == 0) {	 // more arguments than necessary ( should not happen, as it is
								 // checked before )
			...
			if (data_isreference(
					data)) {  // get name for variables to determine the type they refer to
				name = reference_getname(data);
			}
			...
			switch (type) {
				case 'v':
					if (!data_isstring(data) && !data_isint(data) &&
						!data_isreference(data)) {	// variant
						ShowWarning(
							"Unexpected type for argument %d. Expected string, number or "
							"variable.\n",
							idx - 1);
						script_reportdata(data);
						invalid++;
				...
				case 's':
					if (!data_isstring(data) &&
						!(data_isreference(data) && is_string_variable(name))) {  // string
				...
				case 'i':
					if (!data_isint(data) &&
						!(data_isreference(data) &&
						  (reference_toparam(data) || reference_toconstant(data) ||
						   !is_string_variable(
							   name)))) {  // int ( params and constants are always int )
...
void run_script(struct script_code *rootscript, int pos, int rid, int oid) {
	...
	//      It is unclear how that can be triggered, so it needs the be traced/checked in more
	//      detail.
	// NOTE At the time of this change, this function wasn't capable of taking over the script state
	// because st->scriptroot was never set.
...
void script_stop_sleeptimers(int id) {
	for (;;) {
		struct script_state *st =
			(struct script_state *)linkdb_erase(&sleep_db, (void *)__64BPRTSIZE(id));
...
static void script_detach_state(struct script_state *st, bool dequeue_event) {
	...
	if (st->rid && (sd = map_id2sd(st->rid)) != NULL) {
		...
		ShowError(
			"script_detach_state: Found previous script state without attached player (rid=%d, "
			"oid=%d, state=%d, bk_npcid=%d)\n",
			st->bk_st->rid, st->bk_st->oid, st->bk_st->state, st->bk_npcid);
	...
	if (st->rid && (sd = map_id2sd(st->rid)) != NULL) {
		if (st != sd->st) {
			if (st->bk_st) {  // there is already a backup
				ShowDebug(
					"script_attach_state: Previous script state lost (rid=%d, oid=%d, state=%d, "
					"bk_npcid=%d).\n",
					st->bk_st->rid, st->bk_st->oid, st->bk_st->state, st->bk_npcid);
			}
			st->bk_st = sd->st;
...
void run_script_main(struct script_state *st) {
		...
		switch (c) {
			case C_EOL:
				if (stack->defsp > stack->sp)
					ShowError(
						"script:run_script_main: unexpected stack position (defsp=%d sp=%d). "
						"please report this!!!\n",
						stack->defsp, stack->sp);
				else
					pop_stack(st, stack->defsp,
							  stack->sp);  // pop unused stack data. (unused return value)
				break;
			case C_INT:
	...
	if (st->sleep.tick > 0) {
		...
		// Delay execution
		sd = map_id2sd(
			st->rid);  // Get sd since script might have attached someone while running. [Inkfish]
		st->sleep.charid = sd ? sd->status.char_id : 0;
		st->sleep.timer =
			add_timer(gettick() + st->sleep.tick, run_script_timer, st->sleep.charid, (intptr_t)st);
		...
		if (st->stack && st->stack->defsp >= 1 &&
			st->stack->stack_data[st->stack->defsp - 1].type == C_RETINFO) {
			for (int i = 0; i < st->stack->sp; i++) {
				if (st->stack->stack_data[i].type ==
					C_RETINFO) {  // Grab the first, aka the original
...
void script_run_autobonus(const char *autobonus, struct map_session_data *sd, unsigned int pos) {
	...
	if (script) {
		int j;
		ARR_FIND(0, EQI_MAX, j,
				 sd->equip_index[j] >= 0 &&
					 sd->inventory.u.items_inventory[sd->equip_index[j]].equip == pos);
		if (j < EQI_MAX) {
...
void script_add_petautobonus(const std::string &autobonus) {
	if (util::umap_find(pet_autobonuses, autobonus) == nullptr) {
		...
		if (script != nullptr) {
			std::shared_ptr<s_pet_autobonus_wrapper> bonus =
				std::make_shared<s_pet_autobonus_wrapper>();
...
void script_cleararray_pc(struct map_session_data *sd, const char *varname) {
	...
	if (!(sa = static_cast<script_array *>(
			  idb_get(src->arrays, key)))) /* non-existent array, nothing to empty */
		return;
...
/// @param refcache Pointer to an int variable, which keeps a copy of the reference to varname and
/// must be initialized to 0. Can be NULL if only one element is set.
...
void do_init_script(void) {
	...
	stack_ers =
		ers_new(sizeof(struct script_stack), "script.cpp::script_stack", ERS_OPT_FLEX_CHUNK);
...
#define BUILDIN_DEF(x, args) \
	{ buildin_##x, #x, args, NULL }
#define BUILDIN_DEF2(x, x2, args) \
	{ buildin_##x, x2, args, NULL }
#define BUILDIN_DEF_DEPRECATED(x, args, deprecationdate) \
	{ buildin_##x, #x, args, deprecationdate }
#define BUILDIN_DEF2_DEPRECATED(x, x2, args, deprecationdate) \
	{ buildin_##x, x2, args, deprecationdate }
...
BUILDIN_FUNC(menu) {
	...
	// TODO detect multiple scripts waiting for input at the same time, and what to do when that
	// happens
	if (sd->state.menu_or_input == 0) {
		struct StringBuf buf;

		if (script_lastdata(st) % 2 ==
			0) {  // argument count is not even (1st argument is at index 2)
		...
		for (i = 2; i < script_lastdata(st); i += 2) {
			...
			if (!data_islabel(data)) {	// not a label
				StringBuf_Destroy(&buf);
				ShowError(
					"buildin_menu: Argument #%d (from 1) is not a label or label not found.\n", i);
				script_reportdata(data);
				st->state = END;
				return SCRIPT_CMD_FAILURE;
			}
		...
		if (sd->npc_menu >=
			0xff) {	 // client supports only up to 254 entries; 0 is not used and 255 is reserved
					 // for cancel; excess entries are displayed but cause 'uint8' overflow
		...
		if (sd->npc_menu > 0) {	 // Invalid selection
			ShowDebug(
				"buildin_menu: Selection is out of range (%d pairs are missing?) - please report "
				"this\n",
				sd->npc_menu);
			st->state = END;
			return SCRIPT_CMD_FAILURE;
		}
		if (!data_islabel(script_getdata(
				st, i + 1))) {	// TODO remove this temporary crash-prevention code (fallback for
								// multiple scripts requesting user input)
	...
	if (script_hasdata(st, 2)) {  // return value
		...
		if (data_isreference(data)) {
			...
			if (name[0] == '.' && name[1] == '@') {	 // scope variable
				...
				if (data->ref &&
					data->ref->vars == st->stack->stack_data[st->stack->defsp - 1].u.ri->scope.vars)
...
BUILDIN_FUNC(warp) {
	...
	if (strcmp(str, "Random") == 0)
		...
		ret = pc_setpos(sd, sd->status.save_point.map, sd->status.save_point.x,
...
BUILDIN_FUNC(warpparty) {
	...
	enum e_warpparty_target type = (strcmp(str, "Random") == 0)
									   ? WARPPARTY_RANDOM
									   : (strcmp(str, "SavePointAll") == 0)
											 ? WARPPARTY_SAVEPOINTALL
											 : (strcmp(str, "SavePoint") == 0)
												   ? WARPPARTY_SAVEPOINT
												   : (strcmp(str, "Leader") == 0)
														 ? WARPPARTY_LEADER
														 : (strcmp(str, "RandomAll") == 0)
															   ? WARPPARTY_RANDOMALL
															   : WARPPARTY_RANDOMALLAREA;
		...
		switch (type) {
			...
			case WARPPARTY_SAVEPOINTALL:
				if (!mapdata->flag[MF_NORETURN])
					ret = pc_setpos(pl_sd, pl_sd->status.save_point.map, pl_sd->status.save_point.x,
									pl_sd->status.save_point.y, CLR_TELEPORT);
				break;
			case WARPPARTY_SAVEPOINT:
				if (!mapdata->flag[MF_NORETURN])
					ret = pc_setpos(pl_sd, sd->status.save_point.map, sd->status.save_point.x,
									sd->status.save_point.y, CLR_TELEPORT);
				break;
			case WARPPARTY_LEADER:
			...
			case WARPPARTY_RANDOMALL:
				...
				// Fall through
			case WARPPARTY_RANDOMALLAREA:
				if (!mapdata->flag[MF_NORETURN] && !mapdata->flag[MF_NOWARP] &&
					pc_job_can_entermap((enum e_job)pl_sd->status.class_, m,
										pc_get_group_level(pl_sd))) {
					if (rx || ry) {
						...
						if (attempts != 0) {  // Keep the original coordinates if fails to find a
											  // valid cell within the range
							x = nx;
							y = ny;
						}
					}
...
BUILDIN_FUNC(warpguild) {
	...
	type = (strcmp(str, "Random") == 0)
			   ? 0
			   : (strcmp(str, "SavePointAll") == 0) ? 1 : (strcmp(str, "SavePoint") == 0) ? 2 : 3;

	if (type == 2 &&
		!script_rid2sd(sd)) {  // "SavePoint" uses save point of the currently attached player
		return SCRIPT_CMD_SUCCESS;
	}
		...
		switch (type) {
			...
			case 1:	 // SavePointAll
				if (!map_getmapflag(pl_sd->bl.m, MF_NORETURN))
					pc_setpos(pl_sd, pl_sd->status.save_point.map, pl_sd->status.save_point.x,
							  pl_sd->status.save_point.y, CLR_TELEPORT);
				break;
			case 2:	 // SavePoint
				if (!map_getmapflag(pl_sd->bl.m, MF_NORETURN))
					pc_setpos(pl_sd, sd->status.save_point.map, sd->status.save_point.x,
							  sd->status.save_point.y, CLR_TELEPORT);
				break;
			case 3:	 // m,x,y
				if (!map_getmapflag(pl_sd->bl.m, MF_NORETURN) &&
					!map_getmapflag(pl_sd->bl.m, MF_NOWARP) &&
					pc_job_can_entermap((enum e_job)pl_sd->status.class_, m,
										pc_get_group_level(pl_sd)))
...
BUILDIN_FUNC(setr) {
	...
	if (!strcmp(command, "setr") &&
		script_hasdata(st, 4)) {  // Optional argument used by post-increment/post-decrement
								  // constructs to return the previous value
		if (is_string_variable(name))
...
BUILDIN_FUNC(setarray) {
	...
	if (is_string_variable(name)) {	 // string array
		for (i = 3; start < end; ++start, ++i)
			set_reg_str(st, sd, reference_uid(id, start), name, script_getstr(st, i),
						reference_getref(data));
	} else {  // int array
		for (i = 3; start < end; ++start, ++i)
			set_reg_num(st, sd, reference_uid(id, start), name, script_getnum64(st, i),
						reference_getref(data));
	}
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(copyarray) {
	...
	if (id1 == id2 && idx1 > idx2) {
		...
		for (i = count - 1; i >= 0; --i) {
			if (is_string) {
				const char *value =
					get_val2_str(st, reference_uid(id2, idx2 + i), reference_getref(data2));
				set_reg_str(st, sd, reference_uid(id1, idx1 + i), name1, value,
							reference_getref(data1));
				// Remove stack entry from get_val2_str
				script_removetop(st, -1, 0);
			} else {
				int64 value =
					get_val2_num(st, reference_uid(id2, idx2 + i), reference_getref(data2));
				set_reg_num(st, sd, reference_uid(id1, idx1 + i), name1, value,
							reference_getref(data1));
			}
		}
	} else {
...
BUILDIN_FUNC(getarraysize) {
	...
	script_pushint(
		st, script_array_highest_key(st, sd, reference_getname(data), reference_getref(data)));
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(deletearray) {
	...
	if (!(sa = static_cast<script_array *>(
			  idb_get(src->arrays, id)))) {	 // non-existent array, nothing to empty
	...
	if (script_hasdata(st, 3)) {
		...
		if (end - start < sa->size) {
			...
			for (; start + count < end; ++start) {
				...
				if (is_string) {
					const char *value =
						get_val2_str(st, reference_uid(id, start + count), reference_getref(data));
					set_reg_str(st, sd, reference_uid(id, start), name, value,
								reference_getref(data));
					// Remove stack entry from get_val2_str
					script_removetop(st, -1, 0);
				} else {
					int64 value =
						get_val2_num(st, reference_uid(id, start + count), reference_getref(data));
					set_reg_num(st, sd, reference_uid(id, start), name, value,
								reference_getref(data));
				}
			}
			for (; start < end; start++) {
			...
			for (; i < size && list[i] < end; i++) {
				...
				if (is_string) {
					const char *value =
						get_val2_str(st, reference_uid(id, list[i]), reference_getref(data));
					set_reg_str(st, sd, reference_uid(id, list[i] - count), name, value,
								reference_getref(data));
					// Remove stack entry from get_val2_str
					script_removetop(st, -1, 0);
				} else {
					int64 value =
						get_val2_num(st, reference_uid(id, list[i]), reference_getref(data));
					set_reg_num(st, sd, reference_uid(id, list[i] - count), name, value,
								reference_getref(data));
				}
...
BUILDIN_FUNC(countinarray) {
	...
	if (array_size1 < i || array_size2 < j) {  // To prevent unintended behavior
		ShowError(
			"buildin_countinarray: The given index of the array is higher than the array size.\n");
		script_reportdata(data1);
		script_reportdata(data2);
		st->state = END;
		return SCRIPT_CMD_FAILURE;
	...
	if (is_string_variable(name1) && is_string_variable(name2)) {
		...
		ShowError(
			"buildin_countinarray: Arrays do not match , You can't compare an int array to a "
			"string array.\n");
		script_reportdata(data1);
		script_reportdata(data2);
		st->state = END;
...
BUILDIN_FUNC(viewpointmap) {
	...
	map_foreachinmap(buildin_viewpointmap_sub, mapid, BL_PC, st->oid, script_getnum(st, 3),
					 script_getnum(st, 4), script_getnum(st, 5), script_getnum(st, 6),
					 script_getnum(st, 7));
	return SCRIPT_CMD_SUCCESS;
}
 ...
 **/
static bool script_getitem_randomoption(struct script_state *st, struct map_session_data *sd,
										struct item *it, const char *funcname, int x) {
	int i, opt_id_n;
	...
	if (not_server_variable(opt_id_var[0]) && sd == nullptr) {
		...
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_%s: variable \"%s\" was not a server variable, but no player was "
				"attached.\n",
				funcname, opt_id_var);
			return false;
		}
	...
	if (!data_isreference(opt_id) ||
		!script_array_src(st, sd, opt_id_var, reference_getref(opt_id))) {
	...
	if (!data_isreference(opt_val) ||
		!script_array_src(st, sd, opt_val_var, reference_getref(opt_val))) {
	...
	if (!data_isreference(opt_param) ||
		!script_array_src(st, sd, opt_param_var, reference_getref(opt_param))) {
	...
	for (i = 0; i < opt_id_n && i < MAX_ITEM_RDM_OPT; i++) {
		it->option[i].id =
			(short)get_val2_num(st, reference_uid(opt_id_id, opt_id_idx + i), opt_id_ref);
		it->option[i].value =
			(short)get_val2_num(st, reference_uid(opt_val_id, opt_val_idx + i), opt_val_ref);
		it->option[i].param =
			(char)get_val2_num(st, reference_uid(opt_param_id, opt_param_idx + i), opt_param_ref);
	}
	return true;
}
 ...
 */
static int script_countitem_sub(struct item *items, std::shared_ptr<item_data> id, int size,
								int expanded, struct script_state *st,
								struct map_session_data *sd = nullptr, bool rental = false) {
	nullpo_retr(-1, items);
	nullpo_retr(-1, st);
	...
	if (!expanded) {  // For non-expanded functions
		...
		for (int i = 0; i < size; i++) {
			...
			if (itm->nameid == nameid &&
				((rental && itm->expire_time > 0) || (!rental && itm->expire_time == 0)))
				count += itm->amount;
		}
		...
		for (int i = 0; i < size; i++) {
			...
			if (itm == nullptr || itm->nameid == 0 || itm->amount < 1)
				continue;
			if (itm->nameid != it.nameid || itm->identify != it.identify ||
				itm->refine != it.refine || itm->attribute != it.attribute ||
			...
			if (expanded & 2) {
				...
				for (j = 0; j < MAX_ITEM_RDM_OPT; j++) {
					if (itm->option[j].id != it.option[j].id ||
						itm->option[j].value != it.option[j].value ||
 ...
 * countitem2(<nameID>,<Identified>,<Refine>,<Attribute>,<Card0>,<Card1>,<Card2>,<Card3>{,<accountID>})
 * [Lupus] countitem3(<item
 * id>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem3("<item
 * name>",<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem4(<item
 * id>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<accountID>})
 * countitem4("<item
 * name>",<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<accountID>})
 */
BUILDIN_FUNC(countitem) {
	...
	int count =
		script_countitem_sub(sd->inventory.u.items_inventory, id, MAX_INVENTORY, expanded, st, sd);
	if (count < 0) {
		st->state = END;
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(rentalcountitem) {
	...
	int count = script_countitem_sub(sd->inventory.u.items_inventory, id, MAX_INVENTORY, expanded,
									 st, sd, true);
	if (count < 0) {
		st->state = END;
		return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(checkweight2) {
	...
	for (i = 0; i < nb_it; i++) {
		t_itemid nameid =
			(t_itemid)get_val2_num(st, reference_uid(id_it, idx_it + i), reference_getref(data_it));
		uint16 amount =
			(uint16)get_val2_num(st, reference_uid(id_nb, idx_nb + i), reference_getref(data_nb));
...
/*==========================================
 * getitem2 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>};
 * getitem2 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>};
 *
 * getitembound2 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound type>{,<account
 ID>};
 * getitembound2 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>{,<account ID>};
 *
 * getitem3 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
 ID>};
 * getitem3 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
 ID>};
 *
 * getitembound3 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound3 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 *
 * getitem4 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
 ID>};
 * getitem4 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account
 ID>};
 *
 * getitembound4 <item
 id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
 * getitembound4 "<item
 name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<bound
 type>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account ID>};
			...
			if (!pet_create_egg(sd, nameid)) {
				...
				if ((flag = pc_additem(sd, &item_tmp, get_count, LOG_TYPE_SCRIPT))) {
					...
					if (pc_candrop(sd, &item_tmp))
						map_addflooritem(&item_tmp, get_count, sd->bl.m, sd->bl.x, sd->bl.y, 0, 0,
										 0, 0, 0);
				}
			}
		}
 ...
 * rentitem2 <item
 * id>,<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account_id>};
 * rentitem2 "<item
 * name>",<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account_id>};
 *
 * rentitem3 <item
 * id>,<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account_id>};
 * rentitem3 "<item
 * name>",<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account_id>};
 *
 * rentitem4 <item
 * id>,<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account_id>};
 * rentitem4 "<item
 * name>",<time>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<account_id>};
 */
...
BUILDIN_FUNC(getnameditem) {
	...
	item_tmp.amount = 1;
	item_tmp.identify = 1;
	item_tmp.card[0] = CARD0_CREATE;  // we don't use 255! because for example SIGNED WEAPON
									  // shouldn't get TOP10 BS Fame bonus [Lupus]
...
BUILDIN_FUNC(grouprandomitem) {
	...
	FETCH(3, sub_group);
	std::shared_ptr<s_item_group_entry> entry =
		itemdb_group.get_random_entry(script_getnum(st, 2), sub_group);
	if (!entry) {
		ShowError(
...
/**
 * makeitem2 <item id>,<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<canShowEffect>};
 * makeitem2 "<item name>",<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<canShowEffect>};
 *
 * makeitem3 <item id>,<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 * makeitem3 "<item name>",<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 *
 * makeitem4 <item id>,<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 * makeitem4 "<item name>",<amount>,"<map
 * name>",<X>,<Y>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>,<grade>,<RandomIDArray>,<RandomValueArray>,<RandomParamArray>{,<canShowEffect>};
 */
...
static void buildin_delitem_delete(struct map_session_data *sd, int idx, int *amount, uint8 loc,
								   bool delete_items) {
	int delamount;
	...
	if (delete_items) {
		if (itemdb_type(itm->nameid) == IT_PETEGG &&
			itm->card[0] == CARD0_PET) {  // delete associated pet
...
static bool buildin_delitem_search(struct map_session_data *sd, struct item *it, uint8 exact_match,
								   uint8 loc) {
	...
	for (;;) {
		...
		for (i = 0; amount && i < size; i++) {
			...
			if (!&items[i] || !(itm = &items[i])->nameid ||
				itm->nameid != it->nameid) {  // wrong/invalid item
				continue;
			}
			...
			if (exact_match) {
				if ((exact_match & 0x1) &&
					(itm->identify != it->identify || itm->attribute != it->attribute ||
					 itm->enchantgrade != it->enchantgrade ||
					 memcmp(itm->card, it->card,
							sizeof(itm->card)))) {	// not matching exact attributes
					continue;
				}
				if (exact_match & 0x2) {
					uint8 j;
		...
		if (amount == 0 ||
			important == 0) {  // either everything was already consumed or no items were skipped
			;
		} else
			for (i = 0; amount && i < size; i++) {
				struct item *itm = NULL;
				...
				if (exact_match) {
					if ((exact_match & 0x1) &&
						(itm->refine != it->refine || itm->identify != it->identify ||
						 ...
						 memcmp(itm->card, it->card,
								sizeof(itm->card)))) {	// not matching attributes
						continue;
					}
					if (exact_match & 0x2) {
						uint8 j;
...
BUILDIN_FUNC(delitem) {
	...
	ShowError("buildin_%s: failed to delete %d items (AID=%d item_id=%u).\n", command, it.amount,
			  sd->status.account_id, it.nameid);
	st->state = END;
	st->mes_active = 0;
	clif_scriptclose(sd, st->oid);
	return SCRIPT_CMD_FAILURE;
...
/// cartdelitem2 "<Item
/// name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>}
/// storagedelitem2 <item
/// id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>}
/// storagedelitem2 "<Item
/// name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>}
/// guildstoragedelitem2 <item
/// id>,<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>}
/// guildstoragedelitem2 "<Item
/// name>",<amount>,<identify>,<refine>,<attribute>,<card1>,<card2>,<card3>,<card4>{,<account ID>}
/// delitem3 <item
...
BUILDIN_FUNC(delitemidx) {
	...
	if (sd->inventory_data[idx] == nullptr) {
		ShowWarning(
			"buildin_delitemidx: No item can be deleted from index %d of player %s (AID: %u, CID: "
			"%u).\n",
...
BUILDIN_FUNC(getpartymember) {
	...
	if (p != NULL) {
		...
		if (script_hasdata(st, 4)) {
			...
			if (!data_isreference(data)) {
				ShowError(
					"buildin_getpartymember: Error in argument! Please give a variable to store "
					"values in.\n");
				return SCRIPT_CMD_FAILURE;
			}
			...
			if (not_server_variable(*varname)) {
				...
				if (!script_rid2sd(sd)) {
					ShowError(
						"buildin_getpartymember: Cannot use a player variable '%s' if no player is "
						"attached.\n",
						varname);
					return SCRIPT_CMD_FAILURE;
				}
		...
		for (i = 0; i < MAX_PARTY; i++) {
			if (p->party.member[i].account_id) {
				switch (type) {
					case 2:
						if (data)
							setd_sub_num(st, NULL, varname, j, p->party.member[i].account_id,
										 data->ref);
						else
							mapreg_setreg(reference_uid(add_str("$@partymemberaid"), j),
										  p->party.member[i].account_id);
						break;
					case 1:
						if (data)
							setd_sub_num(st, NULL, varname, j, p->party.member[i].char_id,
										 data->ref);
						else
							mapreg_setreg(reference_uid(add_str("$@partymembercid"), j),
					...
					default:
						if (data)
							...
							mapreg_setregstr(reference_uid(add_str("$@partymembername$"), j),
...
BUILDIN_FUNC(getequippercentrefinery) {
	...
	if (i >= 0 && sd->inventory.u.items_inventory[i].nameid) {
		...
		std::shared_ptr<s_refine_cost> cost = util::umap_find(
			info->costs, (uint16)(enriched ? REFINE_COST_ENRICHED : REFINE_COST_NORMAL));
...
BUILDIN_FUNC(successrefitem) {
	...
	if (i >= 0) {
		...
		sd->inventory.u.items_inventory[i].refine =
			cap_value(sd->inventory.u.items_inventory[i].refine, 0, MAX_REFINE);
		...
		if (sd->inventory.u.items_inventory[i].refine ==
				battle_config.blacksmith_fame_refine_threshold &&
			sd->inventory.u.items_inventory[i].card[0] == CARD0_FORGE &&
			sd->status.char_id == (int)MakeDWord(sd->inventory.u.items_inventory[i].card[2],
			...
			switch (sd->inventory_data[i]->weapon_level) {
				case 1:
					pc_addfame(*sd,
							   battle_config.fame_refine_lv1);	// Success to refine to +10 a lv1
																// weapon you forged = +1 fame point
					break;
				case 2:
					pc_addfame(
						*sd, battle_config.fame_refine_lv2);  // Success to refine to +10 a lv2
															  // weapon you forged = +25 fame point
					break;
				case 3:
					pc_addfame(
						*sd,
						battle_config.fame_refine_lv3);	 // Success to refine to +10 a lv3 weapon
														 // you forged = +1000 fame point
					break;
			}
		}
		script_pushint(st, sd->inventory.u.items_inventory[i].refine);
...
BUILDIN_FUNC(failedrefitem) {
	...
	if (i >= 0) {
		...
		clif_refine(sd->fd, 1, i,
					sd->inventory.u.items_inventory[i].refine);	 // notify client of failure
...
BUILDIN_FUNC(downrefitem) {
	...
	if (i >= 0) {
		...
		achievement_update_objective(sd, AG_ENCHANT_FAIL, 1,
									 sd->inventory.u.items_inventory[i].refine);
		script_pushint(st, sd->inventory.u.items_inventory[i].refine);
		return SCRIPT_CMD_SUCCESS;
	}
		...
		case SP_SUB_SKILL:
			...
			if (script_isstring(st, 3)) {
				...
				if (!(val1 = skill_name2id(name))) {
					ShowError(
						"buildin_bonus: Invalid skill name %s passed to item bonus. Skipping.\n",
						name);
					return SCRIPT_CMD_FAILURE;
				}
			} else {
				...
				if (strcmpi(script_getfuncname(st), "bonus") &&
					!skill_get_index(
						val1)) {  // Only check skill ID for bonus2, bonus3, bonus4, or bonus5
					ShowError(
						"buildin_bonus: Invalid skill ID %d passed to item bonus. Skipping.\n",
						val1);
					return SCRIPT_CMD_FAILURE;
				}
			}
...
BUILDIN_FUNC(autobonus) {
	...
	if (pc_addautobonus(sd->autobonus, bonus_script, rate, dur, atk_type, other_script, pos,
						false)) {
...
BUILDIN_FUNC(autobonus2) {
	...
	if (pc_addautobonus(sd->autobonus2, bonus_script, rate, dur, atk_type, other_script, pos,
						false)) {
		script_add_autobonus(bonus_script);
		if (other_script)
			script_add_autobonus(other_script);
...
BUILDIN_FUNC(autobonus3) {
	...
	if (pc_addautobonus(sd->autobonus3, bonus_script, rate, dur, skill_id, other_script, pos,
						true)) {
		script_add_autobonus(bonus_script);
		if (other_script)
			script_add_autobonus(other_script);
...
BUILDIN_FUNC(petautobonus) {
	...
	if (pet_addautobonus(bonus2 ? sd->pd->autobonus2 : sd->pd->autobonus, bonus_script, rate, dur,
						 atk_type, other_script, false)) {
...
BUILDIN_FUNC(petautobonus3) {
	...
	if (script_isstring(st, 5)) {
		...
		if (!(skill_id = skill_name2id(name))) {
			ShowError(
				"buildin_petautobonus3: Invalid skill name %s passed to item bonus. Skipping.\n",
				name);
			return SCRIPT_CMD_FAILURE;
		}
	} else {
		...
		if (!skill_get_index(skill_id)) {
			ShowError(
				"buildin_petautobonus3: Invalid skill ID %d passed to item bonus. Skipping.\n",
				skill_id);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	if (pet_addautobonus(sd->pd->autobonus3, bonus_script, rate, dur, skill_id, other_script,
						 true)) {
		script_add_petautobonus(bonus_script);
		if (!other_script.empty())
			script_add_petautobonus(other_script);
...
BUILDIN_FUNC(guildskill) {
	...
	id = (script_isstring(st, 2) ? skill_name2id(script_getstr(st, 2)) : script_getnum(st, 2));
	level = script_getnum(st, 3);
	for (i = 0; i < level; i++) guild_skillup(sd, id);
...
BUILDIN_FUNC(getgdskilllv) {
	...
	guild_id = script_getnum(st, 2);
	skill_id =
		(script_isstring(st, 3) ? skill_name2id(script_getstr(st, 3)) : script_getnum(st, 3));
...
BUILDIN_FUNC(guildopenstorage_log) {
	...
	ShowError(
		"buildin_guildopenstorage_log: This command requires PACKETVER 2014-02-05 or newer.\n");
	return SCRIPT_CMD_FAILURE;
#else
	struct map_session_data *sd;
...
BUILDIN_FUNC(makepet) {
	...
	intif_create_pet(sd->status.account_id, sd->status.char_id, pet->class_, mdb->lv, pet->EggID, 0,
...
#ifdef RENEWAL
	if (base && sd->hd)
		hom_gainexp(sd->hd, base * battle_config.homunculus_exp_gain /
								100);  // Homunculus only receive 10% of EXP
#endif
 ...
 * *monster "<map name>",<x>,<y>,"<name to show>","<mob name>",<amount>{,"<event
 *label>",<size>,<ai>};
 *------------------------------------------*/
BUILDIN_FUNC(monster) {
	...
	if (script_hasdata(st, 10)) {
		...
		if (ai >= AI_MAX) {
			ShowWarning(
				"buildin_monster: Attempted to spawn non-existing ai %d for monster class %d\n", ai,
				class_);
			return SCRIPT_CMD_FAILURE;
		}
	}
...
BUILDIN_FUNC(getmobdrops) {
	...
	for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
		...
		mapreg_setreg(reference_uid(add_str("$@MobDrop_nosteal"), j),
					  mob->dropitem[i].steal_protected);
		mapreg_setreg(reference_uid(add_str("$@MobDrop_randomopt"), j),
					  mob->dropitem[i].randomopt_group);
...
BUILDIN_FUNC(areamonster) {
	...
	if (script_hasdata(st, 11)) {
		...
		if (size > 3) {
			ShowWarning(
				"buildin_areamonster: Attempted to spawn non-existing size %d for monster class "
				"%d\n",
				size, class_);
			return SCRIPT_CMD_FAILURE;
		}
...
/*==========================================
 * KillMonster subcheck, verify if mob to kill ain't got an even to handle, could be force kill by
 *allflag
 *------------------------------------------*/
static int buildin_killmonster_sub_strip(
	struct block_list *bl,
	va_list ap) {  // same fix but with killmonster instead - stripping events from mobs.
...
static int buildin_killmonsterall_sub_strip(
	struct block_list *bl,
	va_list ap) {  // Strips the event from the mob if it's killed the old method.
	struct mob_data *md;
...
BUILDIN_FUNC(clone) {
	...
	if (sd)	 // Return ID of newly crafted clone.
		script_pushint(st,
					   mob_clone_spawn(sd, m, x, y, event, master_id, mode, flag, 1000 * duration));
...
BUILDIN_FUNC(cmdothernpc)  // Added by RoVeRT
{
	...
	safesnprintf(event, EVENT_NAME_LENGTH, "%s::%s%s", npc, script_config.oncommand_event_name,
				 command);
	check_event(st, event);
...
BUILDIN_FUNC(announce) {
	...
	if (flag & (BC_TARGET_MASK | BC_SOURCE_MASK))  // Broadcast source or broadcast region defined
	{
		...
		if (fontColor)
			clif_broadcast2(bl, mes, (int)strlen(mes) + 1, strtol(fontColor, (char **)NULL, 0),
							fontType, fontSize, fontAlign, fontY, target);
		else
			clif_broadcast(bl, mes, (int)strlen(mes) + 1, flag & BC_COLOR_MASK, target);
	} else {
		if (fontColor)
			intif_broadcast2(mes, (int)strlen(mes) + 1, strtol(fontColor, (char **)NULL, 0),
...
static int buildin_announce_sub(struct block_list *bl, va_list ap) {
	...
	if (fontColor)
		clif_broadcast2(bl, mes, len, strtol(fontColor, (char **)NULL, 0), fontType, fontSize,
...
BUILDIN_FUNC(mapannounce) {
	...
	map_foreachinmap(buildin_announce_sub, m, BL_PC, mes, strlen(mes) + 1, flag & BC_COLOR_MASK,
...
BUILDIN_FUNC(areaannounce) {
	...
	map_foreachinallarea(buildin_announce_sub, m, x0, y0, x1, y1, BL_PC, mes, strlen(mes) + 1,
...
BUILDIN_FUNC(getmapguildusers) {
	...
	if ((m = map_mapname2mapid(str)) <
		0) {  // map id on this server (m == -1 if not in actual map-server)
		script_pushint(st, -1);
		return SCRIPT_CMD_SUCCESS;
	}
	g = guild_search(gid);
...
BUILDIN_FUNC(getunits) {
	...
	for (bl = (struct block_list *)mapit_first(iter); mapit_exists(iter);
		...
		if (m == -1 || (m == bl->m && !x0 && !y0 && !x1 && !y1) ||
			...
			if (data) {
				if (is_string_variable(name)) {
					set_reg_str(st, sd, reference_uid(id, idx + size), name, status_get_name(bl),
								reference_getref(data));
				} else {
					set_reg_num(st, sd, reference_uid(id, idx + size), name, bl->id,
								reference_getref(data));
				}
			}
	...
	if (!nd) {
		if (script_hasdata(st, 2))
			ShowError("buildin_%s: Attempted to %s a non-existing NPC '%s' (flag=%d).\n",
					  (flag & NPCVIEW_VISIBLE) ? "show" : "hide", command, script_getstr(st, 2),
					  flag);
		else
...
BUILDIN_FUNC(sc_start) {
	...
	if (nd && nd->bl.id == fake_nd->bl.id)
		flag = script_hasdata(st, 5 + start_type) ? script_getnum(st, 5 + start_type)
												  : SCSTART_NOTICKDEF;
	else
		flag = script_hasdata(st, 5 + start_type) ? script_getnum(st, 5 + start_type)
												  : SCSTART_NOAVOID;
	...
	rate =
		script_hasdata(st, 4 + start_type) ? min(script_getnum(st, 4 + start_type), 10000) : 10000;
	...
	if (potion_flag == 1 && potion_target) {  // skill.cpp set the flags before running the script,
											  // this is a potion-pitched effect.
...
BUILDIN_FUNC(getwaitingroomusers) {
	...
	if (nd != NULL && (cd = (struct chat_data *)map_id2bl(nd->chat_id)) != NULL) {
		for (i = 0; i < cd->users; ++i) {
			setd_sub_num(st, NULL, ".@waitingroom_users", j, cd->usersd[i]->status.account_id,
						 NULL);
			j++;
		}
...
BUILDIN_FUNC(addrid) {
	...
	switch (script_getnum(st, 2)) {
		case 0:
			for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter);
		...
		case 2:
			...
			for (sd = (TBL_PC *)mapit_first(iter); mapit_exists(iter);
				...
				if (!script_getnum(st, 3) || !sd->st)
					if ((sd->status.account_id != st->rid) &&
						(sd->status.party_id ==
						 script_getnum(st, 4)))	 // attached player already runs.
						run_script(st->script, st->pos, sd->status.account_id, st->oid);
			}
			break;
		case 3:
			...
			break;
		case 4:
			map_foreachinallarea(buildin_addrid_sub, bl->m, script_getnum(st, 4),
								 script_getnum(st, 5), script_getnum(st, 6), script_getnum(st, 7),
								 BL_PC, st, script_getnum(st, 3));	// 4-x0 , 5-y0 , 6-x1, 7-y1
			break;
		case 5:
			...
			map_foreachinmap(buildin_addrid_sub, map_mapname2mapid(script_getstr(st, 4)), BL_PC, st,
							 script_getnum(st, 3));
			break;
		default:
			mapit_free(iter);
	...
	switch (mf) {
		case MF_SKILL_DAMAGE:
			if (script_hasdata(st, 4) && script_hasdata(st, 5))
				...
				ShowWarning(
					"buildin_setmapflag: Unable to set skill_damage mapflag as flag data is "
					"missing.\n");
				return SCRIPT_CMD_FAILURE;
			}
			break;
		case MF_SKILL_DURATION:
			if (script_hasdata(st, 4) && script_hasdata(st, 5)) {
				...
				ShowWarning(
					"buildin_setmapflag: Unable to set skill_duration mapflag as flag data is "
					"missing.\n");
				return SCRIPT_CMD_FAILURE;
			}
			break;
...
/// and give them to the character. If any cards were removed in this manner, it will also show a
/// success effect. successremovecards <slot>;
BUILDIN_FUNC(successremovecards) {
	...
	for (c = sd->inventory_data[i]->slots - 1; c >= 0; --c) {
		if (sd->inventory.u.items_inventory[i].card[c] &&
			itemdb_type(sd->inventory.u.items_inventory[i].card[c]) ==
				IT_CARD) {	// extract this card from the item
			unsigned char flag = 0;
			struct item item_tmp;
			memset(&item_tmp, 0, sizeof(item_tmp));
			cardflag = 1;
			...
			if ((flag = pc_additem(sd, &item_tmp, 1,
								   LOG_TYPE_SCRIPT))) {	 // get back the cart in inventory
				clif_additem(sd, 0, 0, flag);
	...
	if (cardflag == 1) {  // if card was remove remplace item with no card
		...
		if ((flag = pc_additem(
				 sd, &item_tmp, 1,
				 LOG_TYPE_SCRIPT))) {  // chk if can be spawn in inventory otherwise put on floor
			clif_additem(sd, 0, 0, flag);
			map_addflooritem(&item_tmp, 1, sd->bl.m, sd->bl.x, sd->bl.y, 0, 0, 0, 0, 0);
		}
...
BUILDIN_FUNC(marriage) {
	...
	if (!script_rid2sd(sd) || !(p_sd = map_nick2sd(script_getstr(st, 2), false)) ||
		!pc_marriage(sd, p_sd)) {
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(petskillbonus) {
	...
	if (battle_config.pet_equip_required && pd->pet.equip == 0)
		...
		pd->bonus->timer =
			add_timer(gettick() + pd->bonus->delay * 1000, pet_skill_bonus_timer, sd->bl.id, 0);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(getinventorylist) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid > 0 &&
			...
			pc_setreg(sd, reference_uid(add_str("@inventorylist_id"), j),
					  sd->inventory.u.items_inventory[i].nameid);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_idx"), j), i);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_amount"), j),
					  sd->inventory.u.items_inventory[i].amount);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_equip"), j),
					  sd->inventory.u.items_inventory[i].equip);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_refine"), j),
					  sd->inventory.u.items_inventory[i].refine);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_identify"), j),
					  sd->inventory.u.items_inventory[i].identify);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_attribute"), j),
					  sd->inventory.u.items_inventory[i].attribute);
			for (k = 0; k < MAX_SLOTS; k++) {
				sprintf(card_var, "@inventorylist_card%d", k + 1);
				pc_setreg(sd, reference_uid(add_str(card_var), j),
						  sd->inventory.u.items_inventory[i].card[k]);
			}
			pc_setreg(sd, reference_uid(add_str("@inventorylist_expire"), j),
					  sd->inventory.u.items_inventory[i].expire_time);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_bound"), j),
					  sd->inventory.u.items_inventory[i].bound);
			pc_setreg(sd, reference_uid(add_str("@inventorylist_enchantgrade"), j),
			...
			for (k = 0; k < MAX_ITEM_RDM_OPT; k++) {
				...
				pc_setreg(sd, reference_uid(add_str(randopt_var), j),
						  sd->inventory.u.items_inventory[i].option[k].id);
				sprintf(randopt_var, "@inventorylist_option_value%d", k + 1);
				pc_setreg(sd, reference_uid(add_str(randopt_var), j),
						  sd->inventory.u.items_inventory[i].option[k].value);
				sprintf(randopt_var, "@inventorylist_option_parameter%d", k + 1);
				pc_setreg(sd, reference_uid(add_str(randopt_var), j),
						  sd->inventory.u.items_inventory[i].option[k].param);
			}
			pc_setreg(sd, reference_uid(add_str("@inventorylist_tradable"), j),
					  pc_can_trade_item(sd, i));
			pc_setreg(sd, reference_uid(add_str("@inventorylist_favorite"), j),
...
BUILDIN_FUNC(soundeffectall) {
	...
	// FIXME: enumerating map squares (map_foreach) is slower than enumerating the list of online
	// players (map_foreachpc?) [ultramage]
	...
	if (!script_hasdata(st, 4)) {  // area around
		...
		map_foreachinallarea(soundeffect_sub, map_mapname2mapid(mapname), x0, y0, x1, y1, BL_PC,
							 name, type);
	} else {
...
BUILDIN_FUNC(petskillattack) {
	...
	id = (script_isstring(st, 2) ? skill_name2id(script_getstr(st, 2))
								 : skill_get_index(script_getnum(st, 2)));
	if (!id) {
...
BUILDIN_FUNC(petskillsupport) {
	...
	if (battle_config.pet_equip_required && pd->pet.equip == 0)
		...
		pd->s_skill->timer =
			add_timer(gettick() + pd->s_skill->delay * 1000, pet_skill_support_timer, sd->bl.id, 0);
	return SCRIPT_CMD_SUCCESS;
}

static inline void script_skill_effect(block_list *bl, uint16 skill_id, uint16 skill_lv, int16 x,
									   int16 y) {
	nullpo_retv(bl);
	...
	switch (skill_get_casttype(skill_id)) {
		...
		case CAST_NODAMAGE:
			...
			break;
		case CAST_DAMAGE:
			clif_skill_damage(bl, bl, gettick(), status_get_amotion(bl), status_get_dmotion(bl), 0,
							  1, skill_id, skill_lv, skill_get_hit(skill_id));
			break;
	}
}
...
BUILDIN_FUNC(skilleffect) {
	...
	skill_id =
		(script_isstring(st, 2) ? skill_name2id(script_getstr(st, 2)) : script_getnum(st, 2));
	...
	/* Ensure we're standing because the following packet causes the client to virtually set the
	 * char to stand, which leaves the server thinking it still is sitting. */
...
BUILDIN_FUNC(removespecialeffect) {
	...
	send_target e_target =
		script_hasdata(st, 3) ? static_cast<send_target>(script_getnum(st, 3)) : AREA;
...
int atcommand_sub(struct script_state *st, int type) {
	...
	if (st->rid) {
		...
		if (st->oid) {
			...
			sd->mapindex =
				(bl->m > 0) ? map_id2index(bl->m) : mapindex_name2id(map_default.mapname);
		}
 ...
 * gmcommand [MouseJstr]
 *------------------------------------------*/
BUILDIN_FUNC(atcommand) { return atcommand_sub(st, 0); }
...
BUILDIN_FUNC(checkequipedcard) {
	...
	if (script_rid2sd(sd)) {
		...
		for (i = 0; i < MAX_INVENTORY; i++) {
			if (sd->inventory.u.items_inventory[i].nameid > 0 &&
				sd->inventory.u.items_inventory[i].amount && sd->inventory_data[i]) {
				if (itemdb_isspecial(sd->inventory.u.items_inventory[i].card[0]))
					continue;
...
BUILDIN_FUNC(getmapxy) {
	...
	switch (type) {
		...
		case BL_PET:  // Get Pet Position
			if (sd->pd &&
				((script_isstring(st, 6) && script_nick2sd(6, sd)) || script_mapid2sd(6, sd)))
				bl = &sd->pd->bl;
			break;
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_getmapxy: variable '%s' for mapname is not a server variable, but no "
				"player is attached!",
				name);
			return SCRIPT_CMD_FAILURE;
		}
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_getmapxy: variable '%s' for mapX is not a server variable, but no player "
				"is attached!",
				name);
			return SCRIPT_CMD_FAILURE;
		}
	...
	if (not_server_variable(prefix)) {
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_getmapxy: variable '%s' for mapY is not a server variable, but no player "
				"is attached!",
				name);
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(summon) {
	...
	md = mob_once_spawn_sub(&sd->bl, sd->bl.m, sd->bl.x, sd->bl.y, str, _class, event, SZ_SMALL,
							AI_NONE);
	if (md) {
		...
		md->deletetimer =
			add_timer(tick + (timeout > 0 ? timeout : 60000), mob_timer_delete, md->bl.id, 0);
...
BUILDIN_FUNC(isequipped) {
	...
	if (!script_rid2sd(sd)) {  // If the player is not attached it is a script error anyway... but
							   // better prevent the map server from crashing...
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
	...
	for (i = 0; id != 0; i++) {
		...
		for (j = 0; j < EQI_MAX; j++) {
			...
			if (itemdb_type(id) != IT_CARD) {
				...
				for (k = 0; k < MAX_SLOTS; k++) {  // New hash system which should support up to 4
												   // slots on any equipment. [Skotlex]
					unsigned int hash = 0;
					if (sd->inventory.u.items_inventory[index].card[k] != id)
						continue;
...
BUILDIN_FUNC(cardscnt) {
	...
	for (i = 0; id != 0; i++) {
		...
		index = current_equip_item_index;  // we get CURRENT WEAPON inventory index from status.cpp
										   // [Lupus]
		if (index < 0)
			continue;
...
BUILDIN_FUNC(setbattleflag) {
	...
	value = script_getstr(
		st, 3);	 // HACK: Retrieve number as string (auto-converted) for battle_set_value
	...
	if (battle_set_value(flag, value) == 0)
		...
		if (script_hasdata(st, 4) &&
			script_getnum(st, 4)) {	 // Only attempt to reload monster data if told to
...
BUILDIN_FUNC(explode) {
	...
	while (str[i] != '\0') {
		if (str[i] == delimiter &&
			start < SCRIPT_MAX_ARRAYSIZE -
						1) {  // break at delimiter but ignore after reaching last array index
			temp[j] = '\0';
	...
	*buf = 0;
	...
	while ((p = strchr(q, '%'))) {
		...
		if (buf_p[strlen(buf_p) - 1] == '$') {	// String
			...
			set_reg_str(st, sd, reference_uid(reference_getid(data), reference_getindex(data)),
						buf_p, ref_str, reference_getref(data));
		} else {  // Number
			if (sscanf(str, buf, &ref_int) == 0) {
				break;
			}
			set_reg_num(st, sd, reference_uid(reference_getid(data), reference_getindex(data)),
						buf_p, ref_int, reference_getref(data));
		}
		arg++;
		...
		for (f = 0; f <= findlen; f++) {
			if (f == findlen) {	 // complete match
				...
				if (usecase) {
					...
					if (((i + f) > inputlen || input[i + f] != find[f]) &&
						TOUPPER(input[i + f]) != TOUPPER(find[f])) {
						StringBuf_Printf(&output, "%c", input[i]);
						break;
					}
				}
...
BUILDIN_FUNC(query_sql) { return buildin_query_sql_sub(st, qsmysql_handle); }
...
BUILDIN_FUNC(callshop) {
	...
	if (!nd || nd->bl.type != BL_NPC ||
		(nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP &&
		 nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP &&
		 nd->subtype != NPCTYPE_MARKETSHOP && nd->subtype != NPCTYPE_BARTER)) {
...
BUILDIN_FUNC(npcshopitem) {
	...
	if (!nd || (nd->subtype != NPCTYPE_SHOP && nd->subtype != NPCTYPE_CASHSHOP &&
				nd->subtype != NPCTYPE_ITEMSHOP && nd->subtype != NPCTYPE_POINTSHOP &&
				nd->subtype != NPCTYPE_MARKETSHOP)) {  // Not found.
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
		...
		if (n < size) {
			if (n + 1 != size)
				memmove(&nd->u.shop.shop_item[n], &nd->u.shop.shop_item[n + 1],
						sizeof(nd->u.shop.shop_item[0]) * (size - n));
#if PACKETVER >= 20131223
			if (nd->subtype == NPCTYPE_MARKETSHOP)
...
BUILDIN_FUNC(addmonsterdrop) {
	...
	if (script_hasdata(st, 6)) {
		...
		if (itm->type != IT_WEAPON && itm->type != IT_ARMOR && itm->type != IT_SHADOWGEAR) {
			ShowError(
				"buildin_addmonsterdrop: Random option group can't be used with this type of item "
				"(item Id: %d).\n",
				item_id);
			script_pushint(st, false);
			return SCRIPT_CMD_FAILURE;
...
BUILDIN_FUNC(searchitem) {
	...
	for (i = 0; i < count; ++start, ++i) {	// Set array
		set_reg_num(st, sd, reference_uid(id, start), name, items[i]->nameid,
					reference_getref(data));
	}
...
BUILDIN_FUNC(getunitdata) {
	...
	if (!data_isreference(data)) {
		ShowWarning(
			"buildin_getunitdata: Error in argument! Please give a variable to store values in.\n");
		return SCRIPT_CMD_FAILURE;
	}
	...
	if (not_server_variable(*name) && !script_rid2sd(sd)) {
		ShowError(
			"buildin_getunitdata: Cannot use a player variable '%s' if no player is attached.\n",
			name);
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(setunitdata) {
	...
	if ((type == UMOB_MAPID || type == UHOM_MAPID || type == UPET_MAPID || type == UMER_MAPID ||
		 type == UELE_MAPID || type == UNPC_MAPID) &&
				...
				case UMOB_SLAVECPYMSTRMD:
					if (value > 0) {
						...
						if (!md->master_id || !(md2 = map_id2md(md->master_id))) {
							ShowWarning(
								"buildin_setunitdata: Trying to set UMOB_SLAVECPYMSTRMD on mob "
								"without master!\n");
							break;
						}
		...
		case BL_HOM:
			...
			switch (type) {
				...
				case UHOM_HUNGER:
					...
					clif_send_homdata(map_charid2sd(hd->homunculus.char_id), SP_HUNGRY,
									  hd->homunculus.hunger);
					break;
				case UHOM_INTIMACY:
					hom_increase_intimacy(hd, (unsigned int)value);
					clif_send_homdata(map_charid2sd(hd->homunculus.char_id), SP_INTIMATE,
...
BUILDIN_FUNC(unitwalk) {
	...
	if (ud != nullptr && ud->state.force_walk) {
		script_pushint(st, 0);
		ShowWarning(
			"buildin_%s: Unit has already been forced to walk and not reached it's destination "
			"yet.\n",
			cmd);
		return SCRIPT_CMD_FAILURE;
	}
	...
	if (!strcmp(cmd, "unitwalk")) {
		...
		if (script_pushint(st, unit_can_reach_pos(bl, x, y, 0))) {
			...
			add_timer(gettick() + 50, unit_delay_walktoxy_timer, bl->id,
					  (x << 16) | (y & 0xFFFF));  // Need timer to avoid mismatches
		}
	} else {
		...
		if (!tbl) {
			...
			script_pushint(st, 0);
			return SCRIPT_CMD_FAILURE;
		} else if (script_pushint(
					   st, unit_can_reach_bl(bl, tbl, distance_bl(bl, tbl) + 1, 0, NULL, NULL))) {
			if (ud != nullptr)
				ud->state.force_walk = true;
			add_timer(gettick() + 50, unit_delay_walktobl_timer, bl->id,
					  tbl->id);	 // Need timer to avoid mismatches
		}
		off = 4;
	}
...
BUILDIN_FUNC(unitattack) {
	...
	switch (unit_bl->type) {
		case BL_PC: {
			...
			clif_parse_ActionRequest_sub(sd, actiontype > 0 ? 0x07 : 0x00, target_bl->id,
										 gettick());
		...
		if (script_hasdata(st, 4)) {
			if (script_getnum(st, 4) == BC_SELF) {
				if (map_id2sd(bl->id) == NULL) {
					ShowWarning(
						"script: unittalk: bc_self can't be used for non-players objects.\n");
					return SCRIPT_CMD_FAILURE;
				}
				target = SELF;
			}
...
BUILDIN_FUNC(unitskilluseid) {
	...
	if (script_isstring(st, 3)) {
		...
		if (!(skill_id = skill_name2id(name))) {
			ShowError(
				"buildin_unitskilluseid: Invalid skill name %s passed to item bonus. Skipping.\n",
				name);
			return SCRIPT_CMD_FAILURE;
		}
	} else {
		...
		if (!skill_get_index(skill_id)) {
			ShowError(
				"buildin_unitskilluseid: Invalid skill ID %d passed to item bonus. Skipping.\n",
				skill_id);
			return SCRIPT_CMD_FAILURE;
		}
	}
	...
	bool cancel =
		(script_hasdata(st, 7) ? script_getnum(st, 7) > 0 : skill_get_castcancel(skill_id));
	...
	if (script_rid2bl(2, bl)) {
		...
		unit_skilluse_id2(bl, target_id, skill_id, skill_lv,
						  (casttime * 1000) + skill_castfix(bl, skill_id, skill_lv), cancel);
	}
...
///
/// unitskillusepos
/// <unit_id>,<skill_id>,<skill_lv>,<target_x>,<target_y>{,<casttime>,<cancel>,<Line_ID>};
/// unitskillusepos <unit_id>,"<skill
/// name>",<skill_lv>,<target_x>,<target_y>{,<casttime>,<cancel>,<Line_ID>};
BUILDIN_FUNC(unitskillusepos) {
	...
	bool cancel =
		(script_hasdata(st, 8) ? script_getnum(st, 8) > 0 : skill_get_castcancel(skill_id));
	...
	if (script_rid2bl(2, bl)) {
		...
		unit_skilluse_pos2(bl, skill_x, skill_y, skill_id, skill_lv,
						   (casttime * 1000) + skill_castfix(bl, skill_id, skill_lv), cancel);
	}
...
BUILDIN_FUNC(sleep) {
	...
	if (st->sleep.tick == 0) {
		...
		if (ticks <= 0) {
			ShowError(
				"buildin_sleep: negative or zero amount('%d') of milli seconds is not supported\n",
				ticks);
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(sleep2) {
	...
	if (st->sleep.tick == 0) {
		...
		if (ticks <= 0) {
			ShowError(
				"buildin_sleep2: negative or zero amount('%d') of milli seconds is not supported\n",
				ticks);
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(awake) {
	...
	// No need to keep the player attached if we are going to run other scripts now, where he might
	// get attached
	if (rid) {
		script_detach_rid(st);
	}
...
BUILDIN_FUNC(getvariableofnpc) {
	...
	nd = npc_name2id(script_getstr(st, 3));
	if (nd == NULL || nd->subtype != NPCTYPE_SCRIPT ||
		nd->u.scr.script == NULL) {	 // NPC not found or has no script
...
#else
	ShowError(
		"buildin_openmail: This command is not supported for PACKETVER 2015-05-13 or newer.\n");
...
BUILDIN_FUNC(setcell) {
	...
	for (y = y1; y <= y2; ++y)
		for (x = x1; x <= x2; ++x) map_setcell(m, x, y, type, flag);
	...
	if (not_server_variable(*name_x) || not_server_variable(*name_y)) {
		if (!script_rid2sd(sd)) {
			if (not_server_variable(*name_x))
				ShowError(
					"buildin_getfreecell: variable '%s' for mapX is not a server variable, but no "
					"player is attached!",
					name_x);
			else
				ShowError(
					"buildin_getfreecell: variable '%s' for mapY is not a server variable, but no "
					"player is attached!",
					name_y);
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(mercenary_delete) {
	...
	if (sd->md == nullptr) {
		ShowWarning(
			"buildin_mercenary_delete: Tried to delete a non existant mercenary from player '%s' "
			"(AID: %u, CID: %u)\n",
			sd->status.name, sd->status.account_id, sd->status.char_id);
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(questinfo) {
...
#else
	if (icon < QTYPE_QUEST ||
		icon > 7)  // TODO: check why 7 and not QTYPE_WARG, might be related to icon + 1 below
		icon = QTYPE_QUEST;
	else
		icon = icon + 1;
#endif
	...
	if (script_hasdata(st, 4)) {
		...
		if (str) {
			...
			script = parse_script(condition.c_str(), "questinfoparsing", 0,
...
BUILDIN_FUNC(showevent) {
	...
	clif_quest_show_event(sd, &nd->bl, static_cast<e_questinfo_types>(icon),
...
BUILDIN_FUNC(waitingroom2bg) {
	...
	if (strcmp(map_name, "-") != 0 &&
		(mapindex = mapindex_name2id(map_name)) == 0) {	 // Invalid Map
		script_pushint(st, 0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(bg_getareausers) {
	...
	for (const auto &member : bg->members) {
		if (member.sd->bl.m != m || member.sd->bl.x < x0 || member.sd->bl.y < y0 ||
			member.sd->bl.x > x1 || member.sd->bl.y > y1)
			continue;
		c++;
	}
	...
	switch (type) {
		...
		case BG_INFO_MAPS: {
			...
			for (i = 0; i < bg->maps.size(); i++)
				setd_sub_str(st, nullptr, ".@bgmaps$", i, mapindex_id2name(bg->maps[i].mapindex),
							 nullptr);
 ...
 *	IE_OTHER: Other errors (instance not in DB, instance doesn't match with character/party/guild,
 *etc.)
 *------------------------------------------*/
...
BUILDIN_FUNC(instance_warpall) {
	...
	if (instance_id <= 0 || (m = map_mapname2mapid(mapn)) < 0 ||
		(m = instance_mapid(m, instance_id)) < 0)
		return SCRIPT_CMD_FAILURE;
	...
	for (const auto &it : idata->map)
		map_foreachinmap(buildin_instance_warpall_sub, it.m, BL_PC, map_id2index(m), x, y,
						 instance_id);
 ...
 *
 * instance_announce <instance
 *id>,"<text>",<flag>{,<fontColor>{,<fontType>{,<fontSize>{,<fontAlign>{,<fontY>}}}}}; Using 0 for
 *<instance id> will auto-detect the id.
 *------------------------------------------*/
BUILDIN_FUNC(instance_announce) {
	...
	for (const auto &it : idata->map)
		map_foreachinmap(buildin_announce_sub, it.m, BL_PC, mes, strlen(mes) + 1,
						 flag & BC_COLOR_MASK, fontColor, fontType, fontSize, fontAlign, fontY);
...
BUILDIN_FUNC(instance_check_party) {
	...
	amount = script_hasdata(st, 3) ? script_getnum(st, 3)
								   : 1;	 // Amount of needed Partymembers for the Instance.
	min = script_hasdata(st, 4) ? script_getnum(st, 4)
								: 1;  // Minimum Level needed to join the Instance.
	max = script_hasdata(st, 5) ? script_getnum(st, 5)
								: MAX_LEVEL;  // Maxium Level allowed to join the Instance.
...
BUILDIN_FUNC(instance_check_guild) {
	...
	amount = script_hasdata(st, 3) ? script_getnum(st, 3)
								   : 1;	 // Amount of needed Guild members for the Instance.
	min = script_hasdata(st, 4) ? script_getnum(st, 4)
								: 1;  // Minimum Level needed to join the Instance.
	max = script_hasdata(st, 5) ? script_getnum(st, 5)
								: MAX_LEVEL;  // Maxium Level allowed to join the Instance.
...
BUILDIN_FUNC(instance_check_clan) {
	...
	amount = script_hasdata(st, 3) ? script_getnum(st, 3)
								   : 1;	 // Amount of needed Clan members for the Instance.
	min = script_hasdata(st, 4) ? script_getnum(st, 4)
								: 1;  // Minimum Level needed to join the Instance.
	max = script_hasdata(st, 5) ? script_getnum(st, 5)
								: MAX_LEVEL;  // Maxium Level allowed to join the Instance.
	...
	switch (type) {
		...
		case IIT_MAP:
			if (!script_hasdata(st, 4) || script_isstring(st, 4)) {
				ShowError(
					"buildin_instance_info: Type IIT_MAP requires a numeric index argument.\n");
				script_pushconststr(st, "");
				return SCRIPT_CMD_FAILURE;
			}
			...
			if (index < 0) {
				ShowError(
					"buildin_instance_info: Type IIT_MAP does not support a negative index "
					"argument.\n");
				script_pushconststr(st, "");
				return SCRIPT_CMD_FAILURE;
			}
			...
			if (index > UINT8_MAX) {
				ShowError(
					"buildin_instance_info: Type IIT_MAP does only support up to index %hu.\n",
					UINT8_MAX);
				script_pushconststr(st, "");
				return SCRIPT_CMD_FAILURE;
			}
...
BUILDIN_FUNC(instance_list) {
	...
	for (int i = instance_start; i < map_num; i++) {
		...
		if (mapdata->instance_src_map == src_id) {
			std::shared_ptr<s_instance_data> idata =
				util::umap_find(instances, mapdata->instance_id);
...
static int buildin_mobuseskill_sub(struct block_list *bl, va_list ap) {
	...
	switch (target) {
		...
		case 2:
			...
			break;
		default:
			tbl = battle_getenemy(&md->bl, DEFAULT_ENEMY_TYPE(md),
								  skill_get_range2(&md->bl, skill_id, skill_lv, true));
			break;
	}
...
/*==========================================
 * areamobuseskill "<Map Name>",<x>,<y>,<range>,"<Mob name>"/<Mob ID>,"<Skill Name>"/<Skill
 *ID>,<Skill Lv>,<Cast Time>,<Cancelable>,<Emotion>,<Target Type>;
 *------------------------------------------*/
BUILDIN_FUNC(areamobuseskill) {
	...
	if (script_isstring(st, 6)) {
		...
		if (mob == nullptr) {
			ShowWarning(
				"buildin_areamobuseskill: Attempted to use skill of non-existing monster \"%s\"\n",
				name);
			return SCRIPT_CMD_FAILURE;
		}
		...
		if (!mob_db.exists(mobid)) {
			ShowWarning(
				"buildin_areamobuseskill: Attempted to use skill of non-existing monster class "
				"%d\n",
				mobid);
			return SCRIPT_CMD_FAILURE;
		}
	...
	map_foreachinallrange(buildin_mobuseskill_sub, &center, range, BL_MOB, mobid, skill_id,
						  skill_lv, casttime, cancel, emotion, target);
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(progressbar_npc) {
	...
	if (!nd->progressbar.timeout) {
		...
		if (second < 0) {
			ShowError(
				"buildin_progressbar_npc: negative amount('%d') of seconds is not supported\n",
				second);
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(showdigit) {
	...
	switch (type) {
		...
		case 3:
			...
			if (value > 99) {
				ShowWarning(
					"buildin_showdigit: type 3 can display 2 digits at max. Capping value %d to "
					"99...\n",
					value);
				script_reportsrc(st);
				value = 99;
...
BUILDIN_FUNC(unbindatcmd) {
	...
	if (i < atcmd_binding_count) {
		...
		for (i = 0, cursor = 0; i < atcmd_binding_count; i++) {
			...
			if (cursor != i) {
				memmove(&atcmd_binding[cursor], &atcmd_binding[i],
						sizeof(struct atcmd_binding_data *));
			}
...
BUILDIN_FUNC(useatcmd) { return atcommand_sub(st, 3); }
	...
	for (i = 0; i < get_count; i++) {
		...
		if (!pet_create_egg(sd, entry->nameid)) {
			...
			if ((flag = pc_additem(sd, &item_tmp, item_tmp.amount, LOG_TYPE_SCRIPT))) {
				clif_additem(sd, 0, 0, flag);
				if (pc_candrop(sd, &item_tmp))
					map_addflooritem(&item_tmp, item_tmp.amount, sd->bl.m, sd->bl.x, sd->bl.y, 0, 0,
									 0, 0, 0);
			}
		}
	}
...
BUILDIN_FUNC(getgroupitem) {
	...
	if (itemdb_group.pc_get_itemgroup(
			group_id, (script_hasdata(st, 3) ? script_getnum(st, 3) != 0 : false), sd)) {
...
BUILDIN_FUNC(countbound) {
	...
	for (i = 0; i < MAX_INVENTORY; i++) {
		if (sd->inventory.u.items_inventory[i].nameid > 0 &&
			...
			pc_setreg(sd, reference_uid(add_str("@bound_items"), k),
					  sd->inventory.u.items_inventory[i].nameid);
			pc_setreg(sd, reference_uid(add_str("@bound_amount"), k),
					  sd->inventory.u.items_inventory[i].amount);
			k++;
		}
	}
 ...
 * @param char_id: Chara that will be leader of this new party. If no char_id specified, the invoker
...
/** Removes player from his/her party. If party_id and char_id is empty remove the invoker from
 *his/her party party_delmember {<char id>,<party_id>};
 * @param: char_id
 * @param: party_id
 * @return val: Result value
...
BUILDIN_FUNC(party_destroy) {
	...
	if (i >= MAX_PARTY || !party->data[i].sd) {	 // leader not online
		...
		for (j = 0; j < MAX_PARTY; j++) {
			...
			if (sd)
				party_member_withdraw(party->party.party_id, sd->status.account_id,
									  sd->status.char_id, sd->status.name,
									  PARTY_MEMBER_WITHDRAW_LEAVE);
			else if (party->party.member[j].char_id)
				intif_party_leave(party->party.party_id, party->party.member[j].account_id,
								  party->party.member[j].char_id, party->party.member[j].name,
								  PARTY_MEMBER_WITHDRAW_LEAVE);
		}
...
BUILDIN_FUNC(vip_time) {
	...
	chrif_req_login_operation(sd->status.account_id, sd->status.name, CHRIF_OP_LOGIN_VIP, viptime,
							  7, 0);
#endif
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(montransform) {
	...
	if (tick != 0) {
		if (battle_config.mon_trans_disable_in_gvg && map_flag_gvg2(sd->bl.m)) {
			clif_displaymessage(
				sd->fd,
				msg_txt(sd, 731));	// Transforming into monster is not allowed in Guild Wars.
			return SCRIPT_CMD_FAILURE;
		}

		if (sd->disguise) {
			clif_displaymessage(
				sd->fd, msg_txt(sd, 729));	// Cannot transform into monster while in disguise.
			return SCRIPT_CMD_FAILURE;
		}
...
BUILDIN_FUNC(getguildmember) {
	...
	if (g) {
		...
		if (script_hasdata(st, 4)) {
			...
			if (!data_isreference(data)) {
				ShowError(
					"buildin_getguildmember: Error in argument! Please give a variable to store "
					"values in.\n");
				return SCRIPT_CMD_FAILURE;
			}
			varname = reference_getname(data);
			...
			if (not_server_variable(*varname)) {
				...
				if (!script_rid2sd(sd)) {
					ShowError(
						"buildin_getguildmember: Cannot use a player variable '%s' if no player is "
						"attached.\n",
						varname);
					return SCRIPT_CMD_FAILURE;
				}
				...
				switch (type) {
					case 2:
						if (data)
							...
							mapreg_setreg(reference_uid(add_str("$@guildmemberaid"), j),
					...
					case 1:
						if (data)
							...
							mapreg_setreg(reference_uid(add_str("$@guildmembercid"), j),
										  g->member[i].char_id);
						break;
					default:
						if (data)
							...
							mapreg_setregstr(reference_uid(add_str("$@guildmembername$"), j),
											 g->member[i].name);
						break;
				}
				j++;
...
BUILDIN_FUNC(addspiritball) {
	...
	for (i = 0; i < count; i++) pc_addspiritball(sd, duration, 10);
	return SCRIPT_CMD_SUCCESS;
}
	...
	while (script_hasdata(st, i)) {
		...
		if (data_isint(data)) {
			...
			if (start < end) {
				...
				for (; start < end; start++) {
					value = func(
						value, get_val2_num(st, reference_uid(id, start), reference_getref(data)));
				}
			} else {
				value = func(value, 0);
			}
...
BUILDIN_FUNC(cap_value) {
	script_pushint64(
		st, cap_value(script_getnum64(st, 2), script_getnum64(st, 3), script_getnum64(st, 4)));
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(getrandomoptinfo) {
	...
	if (script_rid2sd(sd) && current_equip_item_index != -1 && current_equip_opt_index != -1 &&
		sd->inventory.u.items_inventory[current_equip_item_index]
			.option[current_equip_opt_index]
			.id) {
		int param = script_getnum(st, 2);

		switch (param) {
			case ROA_ID:
				val = sd->inventory.u.items_inventory[current_equip_item_index]
						  .option[current_equip_opt_index]
						  .id;
				break;
			case ROA_VALUE:
...
BUILDIN_FUNC(channel_ban) {
	...
	if (!idb_exists(ch->banned, char_id)) {
		...
		if (tsd) {
			...
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd, 769), ch->alias,
						 tsd->status.name);	 // %s %s has been banned.
		} else
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd, 769), ch->alias,
						 "****");  // %s %s has been banned.
...
BUILDIN_FUNC(channel_unban) {
	...
	if (idb_exists(ch->banned, char_id)) {
		...
		if (tsd)
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd, 770), ch->alias,
						 tsd->status.name);	 // %s %s has been unbanned
		else
			safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd, 770), ch->alias,
...
BUILDIN_FUNC(channel_kick) {
	...
	if (res == 0) {
		char output[CHAT_SIZE_MAX + 1];
		safesnprintf(output, CHAT_SIZE_MAX, msg_txt(tsd, 889), ch->alias,
					 tsd->status.name);	 // "%s %s is kicked"
		clif_channel_msg(ch, output, ch->color);
	}
...
BUILDIN_FUNC(achievementadd) {
	...
	if (!sd->state.pc_loaded) {
		// Simply ignore it on the first call, because the status will be recalculated after loading
		// anyway
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(round) {
	...
	if (precision <= 0) {
		ShowError(
			"buildin_round: Attempted to use zero or negative number as arbitrary precision.\n");
		return SCRIPT_CMD_FAILURE;
	}
...
static inline bool mail_sub(struct script_state *st, struct script_data *data,
							struct map_session_data *sd, int i, const char **out_name,
	...
	if (not_server_variable(*name) && sd == NULL) {
		...
		if (!script_rid2sd(sd)) {
			ShowError(
				"buildin_mail: variable \"%s\" was not a server variable, but no player was "
				"attached.\n",
				name);
			return false;
		}
...
// array>{,refine{,bound{,<item card0 array>{,<item card1 array>{,<item card2 array>{,<item card3
// array>{,<random option id0 array>, <random option value0 array>, <random option paramter0
// array>{,<random option id1 array>, <random option value1 array>, <random option paramter1
// array>{,<random option id2 array>, <random option value2 array>, <random option paramter2
// array>{,<random option id3 array>, <random option value3 array>, <random option paramter3
// array>{,<random option id4 array>, <random option value4 array>, <random option paramter4
// array>}}}}}}}}};
BUILDIN_FUNC(mail) {
	...
	while (script_hasdata(st, 7)) {
		...
		for (i = 0; i < num_items && start < end; i++, start++) {
			msg.item[i].nameid =
				(t_itemid)get_val2_num(st, reference_uid(id, start), reference_getref(data));
		...
		for (i = 0; i < num_items && start < end; i++, start++) {
			...
			msg.item[i].amount =
				(short)get_val2_num(st, reference_uid(id, start), reference_getref(data));
			...
			if (msg.item[i].amount <= 0) {
				...
				if (msg.item[i].amount > max) {
					ShowWarning(
						"buildin_mail: amount %d for item %u is exceeding the maximum of %d. "
						"Capping...\n",
		...
		for (i = 0; i < num_items && start < end; i++, start++) {
			...
			msg.item[i].refine =
				(char)get_val2_num(st, reference_uid(id, start), reference_getref(data));
		...
		for (i = 0, j = 11 + MAX_SLOTS; i < MAX_ITEM_RDM_OPT && script_hasdata(st, j) &&
										script_hasdata(st, j + 1) && script_hasdata(st, j + 2);
			 i++, j++) {
			// Option IDs
			data = script_getdata(st, j);
...
BUILDIN_FUNC(camerainfo) {
	...
	clif_camerainfo(sd, false, script_getnum(st, 2) / 100.0f, script_getnum(st, 3) / 100.0f,
					script_getnum(st, 4) / 100.0f);
...
BUILDIN_FUNC(getitempos) {
	...
	if (current_equip_item_index == -1) {
		ShowError(
			"buildin_getitempos: Invalid usage detected. This command should only be used inside "
			"item scripts.\n");
		script_pushint(st, 0);
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(laphine_synthesis) {
	...
	if (script_hasdata(st, 2)) {
		...
		if (sd->itemid == 0) {
			ShowError(
				"buildin_laphine_synthesis: Called outside of an item script without item id.\n");
			return SCRIPT_CMD_FAILURE;
		}

		if (sd->inventory_data[sd->itemindex]->flag.delay_consume == 0) {
			ShowError(
				"buildin_laphine_synthesis: Called from item %u, which is not a consumed "
				"delayed.\n",
				sd->itemid);
			return SCRIPT_CMD_FAILURE;
		}
	...
	if (sd->state.laphine_synthesis != 0) {
		ShowError(
			"buildin_laphine_synthesis: Laphine Synthesis window was already open. Player %s (AID: "
			"%u, CID: %u) with item id %u.\n",
...
BUILDIN_FUNC(laphine_upgrade) {
	...
	if (sd->inventory_data[sd->itemindex]->flag.delay_consume == 0) {
		ShowError(
			"buildin_laphine_upgrade: Called from item %u, which is not a consumed delayed.\n",
			sd->itemid);
		return SCRIPT_CMD_FAILURE;
	}
	...
	if (sd->state.laphine_upgrade != 0) {
		ShowError(
			"buildin_laphine_upgrade: Laphine Upgrade window was already open. Player %s (AID: %u, "
			"CID: %u) with item id %u.\n",
			sd->status.name, sd->status.account_id, sd->status.char_id, sd->itemid);
		return SCRIPT_CMD_FAILURE;
	}
...
BUILDIN_FUNC(item_reform) {
	...
	if (script_hasdata(st, 2)) {
		...
		if (sd->inventory_data[sd->itemindex]->flag.delay_consume == 0) {
			ShowError(
				"buildin_item_reform: Called from item %u, which is not a DelayConsume type.\n",
				sd->itemid);
			return SCRIPT_CMD_FAILURE;
		}
...
struct script_function buildin_func[] = {
	...
	BUILDIN_DEF(
		logmes,
		"s"),  // this command actls as MES but rints info into LOG file either SQL/TXT [Lupus]
	...
	BUILDIN_DEF(cardscnt,
				"i*"),	// check how many items/cards are being equipped in the same arm [Lupus]
	BUILDIN_DEF(getrefine, ""),	 // returns the refined number of the current item, or an item with
								 // index specified [celest]
	...
	BUILDIN_DEF(getequipcardid,
				"ii"),	//[Lupus] returns CARD ID or other info from CARD slot N of equipped item
	// [zBuffer] List of mathematics commands --->
	BUILDIN_DEF(sqrt, "i"),
	BUILDIN_DEF(pow, "ii"),
