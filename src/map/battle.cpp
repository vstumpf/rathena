...
// Early declaration
int battle_get_weapon_element(struct Damage *wd, struct block_list *src, struct block_list *target,
							  uint16 skill_id, uint16 skill_lv, short weapon_position,
...
struct block_list *battle_gettargeted(struct block_list *target) {
	...
	map_foreachinallrange(battle_gettargeted_sub, target, AREA_SIZE, BL_CHAR, bl_list, &c,
						  target->id);
	if (c == 0)
		return NULL;
	if (c > 24)
 ...
 * @return Target list
 */
struct block_list *battle_getenemyarea(struct block_list *src, int x, int y, int range, int type,
									   int ignore_id) {
	struct block_list *bl_list[24];
	int c = 0;

	memset(bl_list, 0, sizeof(bl_list));
	map_foreachinallarea(battle_getenemyarea_sub, src->m, x - range, y - range, x + range,
						 y + range, type, bl_list, &c, src, ignore_id);
 ...
 *------------------------------------------*/
void battle_damage(struct block_list *src, struct block_list *target, int64 damage, t_tick delay,
				   uint16 skill_lv, uint16 skill_id, enum damage_lv dmg_lv,
				   unsigned short attack_type, bool additional_effects, t_tick tick,
				   bool isspdamage) {
	map_freeblock_lock();
	if (isspdamage)
		...
		status_fix_damage(
			src, target, damage, delay,
			skill_id);	// We have to separate here between reflect damage and others [icescope]
	...
	// This is the last place where we have access to the actual damage type, so any monster events
	// depending on type must be placed here
	...
	if (dat) {
		...
		if (target && !status_isdead(target)) {
			if (src && target->m == src->m &&
				...
				check_distance_bl(
					src, target,
					dat->distance))	 // Check to see if you haven't teleported. [Skotlex]
			{
				// Deal damage
				battle_damage(src, target, dat->damage, dat->delay, dat->skill_lv, dat->skill_id,
							  dat->dmg_lv, dat->attack_type, dat->additional_effects, tick,
							  dat->isspdamage);
			} else if (!src &&
					   dat->skill_id == CR_REFLECTSHIELD) {	 // it was monster reflected damage, and
...
int battle_delay_damage(t_tick tick, int amotion, struct block_list *src, struct block_list *target,
						int attack_type, uint16 skill_id, uint16 skill_lv, int64 damage,
						enum damage_lv dmg_lv, t_tick ddelay, bool additional_effects,
						bool isspdamage) {
	...
	if (!battle_config.delay_battle_damage || amotion <= 1) {
		// Deal damage
		battle_damage(src, target, damage, ddelay, skill_lv, skill_id, dmg_lv, attack_type,
					  additional_effects, gettick(), isspdamage);
		return 0;
	}
	...
	if (src->type != BL_PC && amotion > 1000)
		amotion = 1000;	 // Aegis places a damage-delay cap of 1 sec to non player attacks.
						 // [Skotlex]
 ...
 * @return damage
 */
int64 battle_attr_fix(struct block_list *src, struct block_list *target, int64 damage, int atk_elem,
					  int def_type, int def_lv) {
 ...
 */
int battle_calc_cardfix(int attack_type, struct block_list *src, struct block_list *target,
						std::bitset<NK_MAX> nk, int rh_ele, int lh_ele, int64 damage, int left,
						int flag) {
	...
	switch (attack_type) {
		case BF_MAGIC:
			...
			if (tsd && !nk[NK_IGNOREDEFCARD]) {
				...
				for (const auto &raceit : s_race2) race_fix += tsd->indexed_bonus.subrace2[raceit];
				cardfix = cardfix * (100 - race_fix) / 100;
				race_fix =
					tsd->indexed_bonus.subrace[sstatus->race] + tsd->indexed_bonus.subrace[RC_ALL];
				...
				if (sd->state.arrow_atk) {	// Ranged attack
					...
					if (sd->status.weapon == W_KATAR &&
						(skill = pc_checkskill(sd, ASC_KATAR)) >
							0)	// Adv. Katar Mastery functions similar to a +%ATK card on official
								// [helvetica]
 ...
 **/
bool battle_status_block_damage(struct block_list *src, struct block_list *target,
								struct status_change *sc, struct Damage *d, int64 &damage,
	...
	if ((sce = sc->data[SC_GUARDIAN_S]) && damage > 0) {
		clif_specialeffect(
			target, EF_GUARD3,
			AREA);	// Not official but we gotta show some way the barrier is working. [Rytech]
		sce->val2 -= static_cast<int>(cap_value(damage, INT_MIN, INT_MAX));
		if (flag & BF_WEAPON) {
			if (sce->val2 >= 0)
				damage = 0;
	...
	if ((sc->data[SC_PNEUMA] && (flag & (BF_MAGIC | BF_LONG)) == BF_LONG) ||
		...
		sc->data[SC__MANHOLE] || sc->data[SC_KINGS_GRACE] || sc->data[SC_GRAVITYCONTROL]) {
		d->dmg_lv = ATK_BLOCK;
		return false;
	}

	if (sc->data[SC_WHITEIMPRISON]) {  // Gravitation and Pressure do damage without removing the
									   // effect
		if (skill_id == MG_NAPALMBEAT || skill_id == MG_SOULSTRIKE ||
			skill_id == WL_SOULEXPANSION || skill_id == AG_SOUL_VC_STRIKE ||
			...
			if (skill_id == WL_SOULEXPANSION)
				damage <<= 1;  // If used against a player in White Imprison, the skill deals double
							   // damage.
			status_change_end(target,
							  SC_WHITEIMPRISON);  // Those skills do damage and removes effect
		} else {
			d->dmg_lv = ATK_BLOCK;
			return false;
		}
	...
	// ATK_MISS Type
	if ((sce = sc->data[SC_AUTOGUARD]) && flag & BF_WEAPON && rnd() % 100 < sce->val2 &&
		...
		if (sce_d && (d_bl = map_id2bl(sce_d->val1)) &&
			...
			check_distance_bl(target, d_bl,
							  sce_d->val3)) {  // If player is target of devotion, show guard effect
											   // on the devotion caster rather than the target
	...
	// ATK_DEF Type
	if ((sce = sc->data[SC_LIGHTNINGWALK]) && !(flag & BF_MAGIC) && flag & BF_LONG &&
		rnd() % 100 < sce->val1) {
	...
	if ((sce = sc->data[SC_PARRYING]) && flag & BF_WEAPON && skill_id != WS_CARTTERMINATION &&
		rnd() % 100 < sce->val2) {
	...
	if ((sce = sc->data[SC_KAUPE]) &&
		(skill_id != NPC_EARTHQUAKE ||
		 (skill_id == NPC_EARTHQUAKE && flag & NPC_EARTHQUAKE_FLAG)) &&
		rnd() % 100 < sce->val2) {	// Kaupe blocks damage (skill or otherwise) from players, mobs,
									// homuns, mercenaries.
...
#endif
			if (--sce->val3 <=
				0)	// We make it work like Safety Wall, even though it only blocks 1 time.
 ...
 * @return damage
 */
int64 battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d,
						 int64 damage, uint16 skill_id, uint16 skill_lv) {
	...
	if (map_getcell(bl->m, bl->x, bl->y, CELL_CHKMAELSTROM) && skill_id &&
		skill_get_type(skill_id) != BF_MISC && skill_get_casttype(skill_id) == CAST_GROUND)
		return 0;
	...
	if (skill_id == SJ_NOVAEXPLOSING &&
		!(sc && (sc->data[SC_SAFETYWALL] || sc->data[SC_MILLENNIUMSHIELD]))) {
	...
	if (sc && sc->count) {
		if (!battle_status_block_damage(src, bl, sc, d, damage, skill_id,
		...
		if (sc->data[SC_AETERNA] && skill_id != PF_SOULBURN) {
			if (src->type != BL_MER || !skill_id)
				damage <<=
					1;	// Lex Aeterna only doubles damage of regular attacks from mercenaries

#ifndef RENEWAL
			if (skill_id != ASC_BREAKER || !(flag & BF_WEAPON))
#endif
				status_change_end(
					bl, SC_AETERNA);  // Shouldn't end until Breaker's non-weapon part connects.
		}
		...
		if (sc->data[SC_SHADOW_SCAR])	  // !TODO: Need official adjustment for this too.
			...
			// Damage reductions
			// Assumptio increases DEF on RE mode, otherwise gives a reduction on the final damage.
			// [Igniz]
#ifndef RENEWAL
		...
		if (sc->data[SC_DEFENDER] && skill_id != NJ_ZENYNAGE && skill_id != KO_MUCHANAGE &&
#ifdef RENEWAL
		...
		if (sc->data[SC_FOGWALL] && skill_id != RK_DRAGONBREATH &&
			skill_id != RK_DRAGONBREATH_WATER && skill_id != NPC_DRAGONBREATH) {
		...
		if (skill_id == RA_AIMEDBOLT &&
			(sc->data[SC_BITE] || sc->data[SC_ANKLE] || sc->data[SC_ELECTRICSHOCKER])) {
		...
		if (sc->data[SC_STYLE_CHANGE] && sc->data[SC_STYLE_CHANGE]->val1 == MH_MD_GRAPPLING) {
			TBL_HOM *hd =
				BL_CAST(BL_HOM, bl);  // We add a sphere for when the Homunculus is being hit
		...
		if (sc->data[SC_POISONINGWEAPON] && flag & BF_SHORT && damage > 0) {
			...
			if (rnd() % 100 < sc->data[SC_POISONINGWEAPON]->val3)
				sc_start4(src, bl, (sc_type)sc->data[SC_POISONINGWEAPON]->val2, 100,
						  ...
						  (sc->data[SC_POISONINGWEAPON]->val2 == SC_VENOMBLEED
	...
	if (sd && pc_ismadogear(sd)) {
		pc_overheat(*sd, (battle_get_weapon_element(d, src, bl, skill_id, skill_lv, EQI_HAND_R,
													false) == ELE_FIRE
							  ? 3
							  : 1));
	}
 ...
 */
bool battle_can_hit_bg_target(struct block_list *src, struct block_list *bl, uint16 skill_id,
							  int flag) {
 ...
 */
int64 battle_calc_bg_damage(struct block_list *src, struct block_list *bl, int64 damage,
 ...
 * @return Can be hit (true) or can't be hit (false)
 */
bool battle_can_hit_gvg_target(struct block_list *src, struct block_list *bl, uint16 skill_id,
							   int flag) {
	struct mob_data *md = BL_CAST(BL_MOB, bl);
	struct unit_data *ud = unit_bl2ud(bl);
	...
	if (md && (md->guardian_data || md->special_state.ai == AI_GUILD)) {
		...
		if (src->type != BL_MOB || mob_is_clone(((struct mob_data *)src)->mob_id)) {
			struct guild *g = src->type == BL_PC ? ((TBL_PC *)src)->guild
 ...
 * @return damage
 */
int64 battle_calc_gvg_damage(struct block_list *src, struct block_list *bl, int64 damage,
							 uint16 skill_id, int flag) {
 ...
 */
int64 battle_calc_pk_damage(block_list &src, block_list &bl, int64 damage, uint16 skill_id,
							int flag) {
 ...
 * @return damage
 */
int64 battle_addmastery(struct map_session_data *sd, struct block_list *target, int64 dmg,
						int type) {
	int64 damage;
	...
	damage += (15 * pc_checkskill(
						sd, NC_MADOLICENCE));  // Attack bonus is granted even without the Madogear
...
static void battle_add_weapon_damage(struct map_session_data *sd, int64 *damage, int lr_type) {
	...
	// Here we also apply the weapon_damage_rate bonus so it is correctly applied on left/right
	// hands.
...
#ifdef RENEWAL
static int battle_calc_sizefix(int64 damage, struct map_session_data *sd, unsigned char t_size,
							   unsigned char weapon_type, short flag) {
...
/**
 * Calculates renewal Variance, OverUpgradeBonus, and SizePenaltyMultiplier of weapon damage parts
 * for player
 ...
 */
static int battle_calc_base_weapon_attack(struct block_list *src, struct status_data *tstatus,
										  struct weapon_atk *wa, struct map_session_data *sd,
										  bool critical) {
 ...
 * &16: Arrow attack but BOW, REVOLVER, RIFLE, SHOTGUN, GATLING or GRENADE type weapon not equipped
 ...
 *	Refined and optimized by helvetica
 */
static int64 battle_calc_base_damage(struct block_list *src, struct status_data *status,
									 struct weapon_atk *wa, struct status_change *sc,
	...
	if (!sd) {	// Mobs/Pets
		...
		if (sc != nullptr && sc->data[SC_CHANGE] != nullptr)
			return status->matk_max;  // [Aegis] simply uses raw max matk for base damage when
									  // Mental Charge active
#endif
		if (flag & 4) {
		...
		if (!(flag & 1) || (flag & 2)) {  // Normal attacks
			...
			if (sd->equip_index[type] >= 0 && sd->inventory_data[sd->equip_index[type]] &&
				...
				atkmin = atkmin *
						 (80 + sd->inventory_data[sd->equip_index[type]]->weapon_level * 20) / 100;
...
static int battle_range_type(struct block_list *src, struct block_list *target, uint16 skill_id,
							 uint16 skill_lv) {
	...
	switch (skill_id) {
		...
		case GC_CROSSIMPACT:  // Cast range is 7 cells and player jumps to target but skill is
							  // considered melee
	...
	if (battle_config.skillrange_by_distance &&
		(src->type &
		 battle_config.skillrange_by_distance)) {  // based on distance between src/target [Skotlex]
 ...
 */
static int battle_skill_damage_skill(struct block_list *src, struct block_list *target,
									 uint16 skill_id) {
	...
	if ((damage->map & 1 && (!mapdata->flag[MF_PVP] && !mapdata_flag_gvg2(mapdata) &&
 ...
 * @return Skill damage rate
 */
static int battle_skill_damage_map(struct block_list *src, struct block_list *target,
								   uint16 skill_id) {
	map_data *mapdata = map_getmapdata(src->m);
...
int battle_calc_chorusbonus(struct map_session_data *sd) {
	...
	members =
		party_foreachsamemap(party_sub_count_class, sd, 0, MAPID_THIRDMASK, MAPID_MINSTRELWANDERER);
	...
	return members -
		   2;  // Effect bonus from additional Minstrels/Wanderers if not above the max possible.
}

struct Damage battle_calc_magic_attack(struct block_list *src, struct block_list *target,
									   uint16 skill_id, uint16 skill_lv, int mflag);
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_critical(struct Damage *wd, struct block_list *src, struct block_list *target,
							   int skill_id, int skill_lv, bool first_call) {
	if (!first_call)
 ...
 *	Refined and optimized by helvetica
 */
static int is_attack_piercing(struct Damage *wd, struct block_list *src, struct block_list *target,
							  int skill_id, int skill_lv, short weapon_position) {
	...
	if (src != NULL) {
		...
		if (skill_id != PA_SACRIFICE && skill_id != CR_GRANDCROSS &&
			skill_id != NPC_GRANDDARKNESS && skill_id != PA_SHIELDCHAIN && skill_id != KO_HAPPOKUNAI
#ifndef RENEWAL
			...
			if (sd &&
				(sd->left_weapon.def_ratio_atk_ele & (1 << tstatus->def_ele) ||
				 ...
				 sd->left_weapon.def_ratio_atk_class &
					 (1
					  << CLASS_ALL))) {	 // Pass effect onto right hand if configured so. [Skotlex]
 ...
 *	Refined and optimized by helvetica
 */
static bool is_attack_hitting(struct Damage *wd, struct block_list *src, struct block_list *target,
							  int skill_id, int skill_lv, bool first_call) {
	struct status_data *sstatus = status_get_status_data(src);
	struct status_data *tstatus = status_get_status_data(target);
	struct status_change *sc = status_get_sc(src);
	...
	if (skill_id) {
		switch (skill_id) {	 // Hit skill modifiers
			...
			case RL_SLUGSHOT: {
			   ...
			   wd->div_ ==
				   2)  // +1 hit per level of Double Attack on a successful double attack (making
					   // sure other multi attack skills do not trigger this) [helvetica]
 ...
 *	Refined and optimized by helvetica
 */
static bool attack_ignores_def(struct Damage *wd, struct block_list *src, struct block_list *target,
							   uint16 skill_id, uint16 skill_lv, short weapon_position) {
	struct status_data *tstatus = status_get_status_data(target);
	struct status_change *sc = status_get_sc(src);
	struct map_session_data *sd = BL_CAST(BL_PC, src);
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_element_damage(struct Damage *wd, struct block_list *src,
	...
	if (!nk[NK_IGNOREELEMENT] && (wd->damage > 0 || wd->damage2 > 0)) {
		...
		struct status_data *tstatus = status_get_status_data(target);
		int left_element =
			battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_L, true);
		int right_element =
			battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_R, true);
		...
		switch (skill_id) {
			...
			default:
				if (skill_id == 0 && (battle_config.attack_attr_none & src->type))
					return;	 // Non-player basic attacks are non-elemental, they deal 100% against
							 // all defense elements
#ifdef RENEWAL
				if (sd == nullptr) {  // Renewal player's elemental damage calculation is already
									  // done before this point, only calculate for everything else
#endif
					wd->damage = battle_attr_fix(src, target, wd->damage, right_element,
												 tstatus->def_ele, tstatus->ele_lv);
					if (is_attack_left_handed(src, skill_id))
		...
		// Skills forced to neutral gain benefits from weapon element but final damage is considered
		// "neutral" and resistances are applied again
		switch (skill_id) {
#ifdef RENEWAL
			case MO_INVESTIGATE:
			...
			case GN_FIRE_EXPANSION_ACID:
				wd->damage = battle_attr_fix(src, target, wd->damage, ELE_NEUTRAL, tstatus->def_ele,
											 tstatus->ele_lv);
				if (is_attack_left_handed(src, skill_id))
...
#ifdef RENEWAL
		if (sd == nullptr) {  // Only monsters have a single ATK for element, in pre-renewal we also
							  // apply element to entire ATK on players [helvetica]
#endif
			if (sc &&
				sc->data[SC_WATK_ELEMENT]) {  // Descriptions indicate this means adding a percent
											  // of a normal attack in another element [Skotlex]
				int64 damage =
					battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc, tstatus->size,
											(is_skill_using_arrow(src, skill_id) ? 2 : 0)) *
					sc->data[SC_WATK_ELEMENT]->val2 / 100;

				wd->damage += battle_attr_fix(src, target, damage, sc->data[SC_WATK_ELEMENT]->val1,
											  tstatus->def_ele, tstatus->ele_lv);
				if (is_attack_left_handed(src, skill_id)) {
					damage =
						battle_calc_base_damage(src, sstatus, &sstatus->lhw, sc, tstatus->size,
												(is_skill_using_arrow(src, skill_id) ? 2 : 0)) *
						sc->data[SC_WATK_ELEMENT]->val2 / 100;
					wd->damage2 +=
						battle_attr_fix(src, target, damage, sc->data[SC_WATK_ELEMENT]->val1,
										tstatus->def_ele, tstatus->ele_lv);
				}
			}
#ifdef RENEWAL
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_masteries(struct Damage *wd, struct block_list *src,
										 struct block_list *target, uint16 skill_id,
										 uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct status_change *sc = status_get_sc(src);
	...
	if (sd && battle_skill_stacks_masteries_vvs(skill_id) && skill_id != MO_INVESTIGATE &&
		...
		if (skill_id ==
			TF_POISON)	// Additional ATK from Envenom is treated as mastery type damage [helvetica]
		...
		if (skill_id == NJ_SYURIKEN &&
			(skill = pc_checkskill(sd, NJ_TOBIDOUGU)) > 0) {  // !TODO: Confirm new mastery formula
...
#ifdef RENEWAL
			if (sc->data[SC_NIBELUNGEN])  // With renewal, the level 4 weapon limitation has been
										  // removed
...
static void battle_calc_damage_parts(struct Damage *wd, struct block_list *src,
									 struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	struct status_data *sstatus = status_get_status_data(src);
	struct status_data *tstatus = status_get_status_data(target);
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	bool critical = false;
	...
	int right_element =
		battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_R, false);
	int left_element =
		battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_L, false);

	wd->statusAtk += battle_calc_status_attack(sstatus, EQI_HAND_R);
	wd->statusAtk2 += battle_calc_status_attack(sstatus, EQI_HAND_L);

	if (sd && sd->sc.data[SC_SEVENWIND]) {	// Mild Wind applies element to status ATK as well as
											// weapon ATK [helvetica]
		wd->statusAtk = battle_attr_fix(src, target, wd->statusAtk, right_element, tstatus->def_ele,
										tstatus->ele_lv);
		wd->statusAtk2 = battle_attr_fix(src, target, wd->statusAtk, left_element, tstatus->def_ele,
										 tstatus->ele_lv);
	} else {  // status atk is considered neutral on normal attacks [helvetica]
		wd->statusAtk = battle_attr_fix(src, target, wd->statusAtk, ELE_NEUTRAL, tstatus->def_ele,
										tstatus->ele_lv);
		wd->statusAtk2 = battle_attr_fix(src, target, wd->statusAtk, ELE_NEUTRAL, tstatus->def_ele,
										 tstatus->ele_lv);
	}
	...
	if (sd && sd->sc.data[SC_SUB_WEAPONPROPERTY]) {
		int64 bonus_atk =
			(int64)floor((float)(wd->weaponAtk * sd->sc.data[SC_SUB_WEAPONPROPERTY]->val2 / 100));
		int64 bonus_atk2 =
			(int64)floor((float)(wd->weaponAtk2 * sd->sc.data[SC_SUB_WEAPONPROPERTY]->val2 / 100));
		bonus_atk =
			battle_attr_fix(src, target, bonus_atk, sd->sc.data[SC_SUB_WEAPONPROPERTY]->val1,
							tstatus->def_ele, tstatus->ele_lv);
		bonus_atk2 =
			battle_attr_fix(src, target, bonus_atk2, sd->sc.data[SC_SUB_WEAPONPROPERTY]->val1,
							tstatus->def_ele, tstatus->ele_lv);
	...
	wd->weaponAtk = battle_attr_fix(src, target, wd->weaponAtk, right_element, tstatus->def_ele,
									tstatus->ele_lv);
	wd->weaponAtk2 = battle_attr_fix(src, target, wd->weaponAtk2, left_element, tstatus->def_ele,
									 tstatus->ele_lv);

	wd->equipAtk += battle_calc_equip_attack(src, skill_id);
	wd->equipAtk = battle_attr_fix(src, target, wd->equipAtk, right_element, tstatus->def_ele,
								   tstatus->ele_lv);

	wd->equipAtk2 += battle_calc_equip_attack(src, skill_id);
	wd->equipAtk2 = battle_attr_fix(src, target, wd->equipAtk2, left_element, tstatus->def_ele,
									tstatus->ele_lv);
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_skill_base_damage(struct Damage *wd, struct block_list *src,
										  struct block_list *target, uint16 skill_id,
										  uint16 skill_lv) {
	struct status_change *sc = status_get_sc(src);
	struct status_data *sstatus = status_get_status_data(src);
	...
	switch (skill_id) {	 // Calc base damage according to skill
				...
				int damagevalue =
					...
					(
						...
						2 *
							...
							sstatus->batk +
						sstatus->rhw.atk +
						(index >= 0 && sd->inventory_data[index] ? sd->inventory_data[index]->atk
																 : 0)) *
					(skill_lv + 5) / 5;
		...
		default:
			...
			if (sd)
				...
				wd->damage =
					battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc, tstatus->size, i);
				if (is_attack_left_handed(src, skill_id))
					wd->damage2 =
						battle_calc_base_damage(src, sstatus, &sstatus->lhw, sc, tstatus->size, i);
			}
#else
			i = (is_attack_critical(wd, src, target, skill_id, skill_lv, false) ? 1 : 0) |
				(is_skill_using_arrow(src, skill_id) ? 2 : 0) |
			...
			if (sd) {
				...
				if (sd->bonus.crit_atk_rate &&
					is_attack_critical(
						wd, src, target, skill_id, skill_lv,
						false)) {  // add +crit damage bonuses here in pre-renewal mode [helvetica]
...
static void battle_apply_div_fix(struct Damage *d, uint16 skill_id) {
	if (d->damage) {
		...
		// Min damage
		if (d->damage < d->div_ &&
			(skill_id == SU_LUNATICCARROTBEAT || skill_id == SU_LUNATICCARROTBEAT2 ||
			 skill_id == SU_CN_METEOR || skill_id == SU_CN_METEOR2 ||
...
static void battle_calc_multi_attack(struct Damage *wd, struct block_list *src,
	...
	if (sd && !skill_id) {	// if no skill_id passed, check for double attack [helvetica]
		...
		if (((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER) ||
			(pc_checkskill_flag(*sd, TF_DOUBLE) > SKILL_FLAG_PERMANENT &&
		...
		{  // Success chance is not added, the higher one is used [Skotlex]
			...
			if (sc && sc->data[SC_KAGEMUSYA])
				max_rate =
					sc->data[SC_KAGEMUSYA]->val1 * 10;	// Same rate as even levels of TF_DOUBLE
			else
#ifdef RENEWAL
			...
			if (rnd() % 100 < max_rate) {
				...
				wd->type = DMG_MULTI_HIT;
					...
					(sc && sc->count && sc->data[SC_E_CHAIN] &&
					 (skill_lv = sc->data[SC_E_CHAIN]->val1) > 0))	// Chain Action of ETERNAL_CHAIN
	...
	switch (skill_id) {
		...
		case RL_QD_SHOT:
			wd->div_ =
				1 + (sd ? sd->status.job_level : 1) / 20 + (tsc && tsc->data[SC_C_MARKER] ? 2 : 0);
			break;
 ...
 *	Refined and optimized by helvetica
 */
static int battle_calc_attack_skill_ratio(struct Damage *wd, struct block_list *src,
										  struct block_list *target, uint16 skill_id,
										  uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct map_session_data *tsd = BL_CAST(BL_PC, target);
	...
	if (sc && skill_id != PA_SACRIFICE) {
		...
		if (sc && sc->data[SC_VIGOR])  // Lacking info on how damage is increased. Guessing for now.
									   // [Rytech]
			skillratio += skillratio * 50 / 100;
#endif
		if (!skill_id || skill_id == KN_AUTOCOUNTER) {
			if (sc->data[SC_CRUSHSTRIKE]) {
				if (sd) {  // ATK [{Weapon Level * (Weapon Upgrade Level + 6) * 100} + (Weapon ATK)
						   // + (Weapon Weight)]%
					short index = sd->equip_index[EQI_HAND_R];

					if (index >= 0 && sd->inventory_data[index] &&
						sd->inventory_data[index]->type == IT_WEAPON)
						skillratio += -100 + sd->inventory_data[index]->weight / 10 +
	...
	switch (skill_id) {
		...
		case MS_MAGNUM:
			if (wd->miscflag == 1)
				skillratio +=
					20 * skill_lv;	// Inner 3x3 circle takes 100%+20%*level damage [Playtester]
			else
				skillratio +=
					10 * skill_lv;	// Outer 5x5 circle takes 100%+10%*level damage [Playtester]
			break;
		case MC_MAMMONITE:
		...
		case KN_PIERCE:
			...
			if (sc && sc->data[SC_CHARGINGPIERCE_COUNT] &&
...
#endif
			skillratio =
				min(500000, skillratio);  // We stop at roughly 50k SP for overflow protection
			break;
		...
		case RK_STORMBLAST:
			skillratio +=
				-100 + (((sd) ? pc_checkskill(sd, RK_RUNEMASTERY) : 0) + sstatus->str / 6) *
						   100;	 // ATK = [{Rune Mastery Skill Level + (Caster's STR / 6)} x 100] %
			RE_LVL_DMOD(100);
			break;
		case RK_PHANTOMTHRUST:	// ATK = [{(Skill Level x 50) + (Spear Master Level x 10)} x
								// Caster's Base Level / 150] %
		...
		case GC_COUNTERSLASH:
			// ATK [{(Skill Level x 150) + 300} x Caster's Base Level / 120]% + ATK [(AGI x 2) +
			// (Caster's Job Level x 4)]%
		...
		case NPC_DRAGONBREATH:
			if (skill_lv > 5)
				skillratio +=
					500 +
					500 * (skill_lv -
						   5);	// Level 6-10 is using water element, like RK_DRAGONBREATH_WATER
			else
				skillratio +=
					500 + 500 * skill_lv;  // Level 1-5 is using fire element, like RK_DRAGONBREATH
			break;
		case RA_ARROWSTORM:
			...
			if (sd) {
				...
				if (index >= 0 && sd->inventory_data[index] &&
					...
					skillratio += sd->inventory_data[index]->weight /
								  10;  // Weight is divided by 10 since 10 weight in coding make 1
									   // whole actual weight. [Rytech]
			}
			RE_LVL_DMOD(100);
			break;
		case NC_POWERSWING:	 // According to current sources, only the str + dex gets modified by
							 // level [Akinari]
		...
		case NC_AXETORNADO:
			skillratio +=
				-100 + 200 + 180 * skill_lv + sstatus->vit / 6;	 // !TODO: What's the VIT bonus?
			RE_LVL_DMOD(100);
		...
		case SC_FEINTBOMB:
			skillratio +=
				-100 + (skill_lv + 1) * sstatus->dex / 2 * ((sd) ? sd->status.job_level / 10 : 1);
			RE_LVL_DMOD(120);
			break;
		case LG_CANNONSPEAR:
		...
		case SR_KNUCKLEARROW:
			if (wd->miscflag &
				4) {  // ATK [(Skill Level x 150) + (1000 x Target current weight / Maximum weight)
		...
		case WM_GREAT_ECHO:
			...
			RE_LVL_DMOD(100);
			break;
		case GN_CART_TORNADO: {	 // ATK [( Skill Level x 200 ) + ( Cart Weight / ( 150 - Caster Base
								 // STR ))] + ( Cart Remodeling Skill Level x 50 )] %
			skillratio += -100 + 200 * skill_lv;
			if (sd && sd->cart_weight)
			...
			if (sd) {
				switch (sd->itemid) {
					...
					case ITEMID_BLACK_LUMP:
						skillratio +=
							-100 +
							(status_get_str(src) + status_get_agi(src) + status_get_dex(src)) / 3;
						break;
					case ITEMID_BLACK_HARD_LUMP:
						skillratio +=
							-100 +
							(status_get_str(src) + status_get_agi(src) + status_get_dex(src)) / 2;
						break;
					case ITEMID_VERY_HARD_LUMP:
						skillratio +=
							-100 + status_get_str(src) + status_get_agi(src) + status_get_dex(src);
						break;
				}
				RE_LVL_DMOD(100);
			}
		...
		case GN_HELLS_PLANT_ATK:
			skillratio += -100 + 100 * skill_lv +
						  sstatus->int_ * (sd ? pc_checkskill(sd, AM_CANNIBALIZE)
											  : 5);	 // !TODO: Confirm INT and Cannibalize bonus
			RE_LVL_DMOD(100);
			break;
		...
		case KO_HUUMARANKA:
			skillratio +=
				-100 + 150 * skill_lv + sstatus->str + (sd ? pc_checkskill(sd, NJ_HUUMA) * 100 : 0);
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_KAGEMUSYA])
				skillratio += skillratio * sc->data[SC_KAGEMUSYA]->val2 / 100;
			break;
		case KO_SETSUDAN:
			...
			if (tsc) {
				...
				if ((sce = tsc->data[SC_SPIRIT]) || (sce = tsc->data[SC_SOULGOLEM]) ||
					...
					(sce = tsc->data[SC_SOULFAIRY]))  // Bonus damage added when target is soul
													  // linked.
		...
		case MH_MAGMA_FLOW:
			skillratio +=
				-100 + (100 * skill_lv + 3 * status_get_lv(src)) * status_get_lv(src) / 120;
			break;
		case RL_MASS_SPIRAL:
			skillratio += -100 + 200 * skill_lv;
			break;
		...
		case IG_SHIELD_SHOOTING:
			...
			if (sd) {  // Damage affected by the shield's weight and refine. Need official formula.
					   // [Rytech]
				short index = sd->equip_index[EQI_HAND_L];

				if (index >= 0 && sd->inventory_data[index] &&
					sd->inventory_data[index]->type == IT_ARMOR)
					skillratio += sd->inventory_data[index]->weight / 20 *
		...
		case WH_HAWKRUSH:
			...
			RE_LVL_DMOD(100);
			break;
		case WH_HAWKBOOMERANG:	// Affected by trait stats??? CON for sure but the other one
								// unknown. Likely POW. [Rytech]
		...
		case WH_GALESTORM:
			...
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_CALAMITYGALE] &&
				(tstatus->race == RC_BRUTE || tstatus->race == RC_FISH))
				skillratio += skillratio * 50 / 100;
			break;
		case WH_CRESCIVE_BOLT:
			skillratio += -100 + 300 * skill_lv + 5 * sstatus->con;
			RE_LVL_DMOD(100);
			if (sc) {  // At level 10 the SP usage of 100 increased by 20 on each count. So maybe
					   // damage increase is 20%??? [Rytech]
		...
		case BO_ACIDIFIED_ZONE_FIRE_ATK:
			...
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_RESEARCHREPORT]) {  // Does this also affect skills like acid
													  // demo? [Rytech]
				skillratio += skillratio * 50 / 100;
				...
				// I need more info on this before allowing this part to work to avoid overpowered
				// issues. [Rytech]
				// skillratio += 5 * sstatus->pow;
				// RE_LVL_DMOD(100);
			}
		...
		case TR_ROSEBLOSSOM_ATK:  // Same damage formula? [Rytech]
			skillratio += -100 + 500 * skill_lv +
						  (sd ? pc_checkskill(sd, TR_STAGE_MANNER) : 5) * sstatus->con;
			RE_LVL_DMOD(100);
...
/*==================================================================================================
 * Constant skill damage additions are added before SC modifiers and after skill base ATK
 *calculation
 *--------------------------------------------------------------------------------------------------*
 * Credits:
 *	Original coder Skotlex
 ...
 *	Refined and optimized by helvetica
 */
static int64 battle_calc_skill_constant_addition(struct Damage *wd, struct block_list *src,
												 struct block_list *target, uint16 skill_id,
												 uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct map_session_data *tsd = BL_CAST(BL_PC, target);
	...
	if (sc) {
		...
		if (sc->data[SC_MAGICALBULLET]) {
			...
			if (sstatus->matk_min > tmdef && sstatus->matk_max > sstatus->matk_min) {
				ATK_ADD(
					...
					i64max((sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min)) -
							   tmdef,
						   0));
			} else {
		...
		if (sc->data[SC_SPIRIT]) {
			if (skill_id == AS_SONICBLOW && sc->data[SC_SPIRIT]->val2 == SL_ASSASIN) {
				ATK_ADDRATE(
					...
					map_flag_gvg2(src->m) ? 25 : 100);	//+25% dmg on woe/+100% dmg on nonwoe
				RE_ALLATK_ADDRATE(
					wd, map_flag_gvg2(src->m) ? 25 : 100);	//+25% dmg on woe/+100% dmg on nonwoe
		...
		if (sc->data[SC_DANCEWITHWUG]) {
			if (skill_get_inf2(skill_id, INF2_INCREASEDANCEWITHWUGDAMAGE)) {
				ATK_ADDRATE(wd->damage, wd->damage2,
							sc->data[SC_DANCEWITHWUG]->val1 * 10 * battle_calc_chorusbonus(sd));
				RE_ALLATK_ADDRATE(
					wd, sc->data[SC_DANCEWITHWUG]->val1 * 10 * battle_calc_chorusbonus(sd));
			}
			ATK_ADDRATE(wd->damage, wd->damage2,
						sc->data[SC_DANCEWITHWUG]->val1 * 2 * battle_calc_chorusbonus(sd));
#ifdef RENEWAL
		...
		if (sc->data[SC_MIRACLE])
			anger_id =
				2;	// Always treat all monsters as star flagged monster when in miracle state
	}
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_defense_reduction(struct Damage *wd, struct block_list *src,
										  struct block_list *target, uint16 skill_id,
										  uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct map_session_data *tsd = BL_CAST(BL_PC, target);
	...
	// Defense reduction
	short vit_def;
	defType def1 = status_get_def(target);	// Don't use tstatus->def1 due to skill timer
											// reductions.
	...
	if (battle_config.vit_penalty_type && battle_config.vit_penalty_target & target->type) {
		...
		target_count = min(unit_counttargeted(target), (100 / battle_config.vit_penalty_num) +
		...
		default:
			...
			ATK_ADD2(
				...
				is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
					? (def1 * battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv)) /
						  200
					: 0,
				is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
					? (def1 * battle_calc_attack_skill_ratio(wd, src, target, skill_id, skill_lv)) /
	...
	ATK_RATE2(
		...
		attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
			? 100
			: (is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R)
				   ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_R) *
						 (def1 + vit_def)
				   : (100 - def1)),
		attack_ignores_def(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
			? 100
			: (is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L)
				   ? (int64)is_attack_piercing(wd, src, target, skill_id, skill_lv, EQI_HAND_L) *
						 (def1 + vit_def)
				   : (100 - def1)));
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_post_defense(struct Damage *wd, struct block_list *src,
											struct block_list *target, uint16 skill_id,
											uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct status_change *sc = status_get_sc(src);
	...
	if (sc) {  // SC skill damages
		if (sc->data[SC_AURABLADE]
			...
			ATK_ADD(
				wd->damage, wd->damage2,
				(3 + sc->data[SC_AURABLADE]->val1) * status_get_lv(src));  // !TODO: Confirm formula
#else
...
static void battle_calc_attack_plant(struct Damage *wd, struct block_list *src,
	...
	if (attack_hits && target->type == BL_MOB) {
		...
		if (sc && !battle_status_block_damage(src, target, sc, wd, damage_dummy, skill_id,
											  skill_lv)) {	// Statuses that reduce damage to 0.
	...
	if (attack_hits && status_get_class(target) == MOBID_EMPERIUM) {
		if (target &&
			!battle_can_hit_gvg_target(src, target, skill_id, (skill_id) ? BF_SKILL : 0) &&
			map_flag_gvg2(target->m)) {
			wd->damage = wd->damage2 = 0;
			return;
		}

		const int right_element =
			battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_R, false);
		const int left_element =
			battle_get_weapon_element(wd, src, target, skill_id, skill_lv, EQI_HAND_L, false);

		if (wd->damage > 0) {
			wd->damage = battle_attr_fix(src, target, wd->damage, right_element, tstatus->def_ele,
										 tstatus->ele_lv);
			wd->damage = battle_calc_gvg_damage(src, target, wd->damage, skill_id, wd->flag);
		} else if (wd->damage2 > 0) {
			wd->damage2 = battle_attr_fix(src, target, wd->damage2, left_element, tstatus->def_ele,
										  tstatus->ele_lv);
	...
	// For plants we don't continue with the weapon attack code, so we have to apply DAMAGE_DIV_FIX
	// here
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_attack_left_right_hands(struct Damage *wd, struct block_list *src,
												struct block_list *target, uint16 skill_id,
												uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	...
	if (sd) {
		...
		if (!is_attack_right_handed(src, skill_id) && is_attack_left_handed(src, skill_id)) {
			...
			wd->damage2 = 0;
				   ...
				   !skill_id) {	 // Katars (offhand damage only applies to normal attacks, tested on
								 // Aegis 10.2)
			...
			wd->damage2 = (int64)wd->damage * (1 + (skill * 2)) / 100;
				   ...
				   sd->status.weapon != W_KATAR) {	// Dual-wield
#else
...
static void battle_calc_attack_gvg_bg(struct Damage *wd, struct block_list *src,
	...
	if (wd->damage + wd->damage2) {	 // There is a total damage value
		if (src != target &&		 // Don't reflect your own damage (Grand Cross)
			...
			if (rdamage >
				0) {  // Item reflect gets calculated before any mapflag reducing is applicated
				struct block_list *d_bl = battle_check_devotion(src);

				rdelay = clif_damage(src, (!d_bl) ? src : d_bl, tick, wd->amotion, sstatus->dmotion,
									 rdamage, 1, DMG_ENDURE, 0, false);
				if (tsd)
				...
				battle_delay_damage(tick, wd->amotion, target, (!d_bl) ? src : d_bl, 0,
									CR_REFLECTSHIELD, 0, rdamage, ATK_DEF, rdelay, true, false);
				skill_additional_effect(target, (!d_bl) ? src : d_bl, CR_REFLECTSHIELD, 1,
 ...
 *	Refined and optimized by helvetica
 */
static void battle_calc_weapon_final_atk_modifiers(struct Damage *wd, struct block_list *src,
												   struct block_list *target, uint16 skill_id,
												   uint16 skill_lv) {
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	struct map_session_data *tsd = BL_CAST(BL_PC, target);
	...
	if (tsc && tsc->data[SC_CRESCENTELBOW] && wd->flag & BF_SHORT &&
		...
		// ATK [{(Target HP / 100) x Skill Level} x Caster Base Level / 125] % + [Received damage x
		// {1 + (Skill Level x 0.2)}]
		int64 rdamage = 0;
		int ratio = (int64)(status_get_hp(src) / 100) * tsc->data[SC_CRESCENTELBOW]->val1 *
					status_get_lv(target) / 125;
		if (ratio > 5000)
			ratio = 5000;  // Maximum of 5000% ATK
		rdamage = battle_calc_base_damage(target, tstatus, &tstatus->rhw, tsc, sstatus->size, 0);
		rdamage = (int64)rdamage * ratio / 100 +
				  wd->damage * (10 + tsc->data[SC_CRESCENTELBOW]->val1 * 20 / 10) / 10;
		skill_blown(
			target, src,
			skill_get_blewcount(SR_CRESCENTELBOW_AUTOSPELL, tsc->data[SC_CRESCENTELBOW]->val1),
			unit_getdir(src), BLOWN_NONE);
		clif_skill_damage(target, src, gettick(), status_get_amotion(src), 0, rdamage, 1,
						  SR_CRESCENTELBOW_AUTOSPELL, tsc->data[SC_CRESCENTELBOW]->val1,
						  DMG_SINGLE);	// This is how official does
		clif_damage(src, target, gettick(), status_get_amotion(src) + 1000, 0, rdamage / 10, 1,
					DMG_NORMAL, 0, false);
 ...
 *	Refined and optimized by helvetica
 */
static struct Damage initialize_weapon_data(struct block_list *src, struct block_list *target,
											uint16 skill_id, uint16 skill_lv, int wflag) {
	struct status_data *sstatus = status_get_status_data(src);
	struct status_data *tstatus = status_get_status_data(target);
	struct status_change *sc = status_get_sc(src);
	struct map_session_data *sd = BL_CAST(BL_PC, src);
	...
	// counter attack DOES obey ASPD delay on official, uncomment if you want the old (bad) behavior
	// [helvetica]
	...
	wd.damage = wd.damage2 =
#ifdef RENEWAL
		wd.statusAtk = wd.statusAtk2 = wd.equipAtk = wd.equipAtk2 = wd.weaponAtk = wd.weaponAtk2 =
			wd.masteryAtk = wd.masteryAtk2 = wd.percentAtk = wd.percentAtk2 =
#endif
				0;
	...
	if (skill_id) {
		...
		switch (skill_id) {
			...
			case IG_OVERSLASH:
				wd.div_ =
					min(wd.div_ + wd.miscflag, 5);	// Number of hits doesn't appear to go above 5.
				break;
			case SHC_ETERNAL_SLASH:
 ...
 */
void battle_do_reflect(int attack_type, struct Damage *wd, struct block_list *src,
					   struct block_list *target, uint16 skill_id, uint16 skill_lv) {
	...
	if ((wd->damage + wd->damage2) && src && target && src != target &&
		...
		if (tsc->data[SC_MAXPAIN]) {
			...
			if (!tsc->data[SC_KYOMU] &&
				!(tsc->data[SC_DARKCROW] &&
				  (wd->flag & BF_SHORT)))  // SC_KYOMU invalidates reflecting ability. SC_DARKCROW
										   // also does, but only for short weapon attack.
				skill_castend_damage_id(target, src, NPC_MAXPAIN_ATK, tsc->data[SC_MAXPAIN]->val1,
										tick, ((wd->flag & 1) ? wd->flag - 1 : wd->flag));
		}
		...
		if (rdamage > 0) {
			...
			if (attack_type == BF_WEAPON &&
				tsc->data[SC_REFLECTDAMAGE])  // Don't reflect your own damage (Grand Cross)
				map_foreachinshootrange(battle_damage_area, target,
										skill_get_splash(LG_REFLECTDAMAGE, 1), BL_CHAR, tick,
										target, wd->amotion, sstatus->dmotion, rdamage, wd->flag);
 ...
 *	Refined and optimized by helvetica
 */
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target,
											   uint16 skill_id, uint16 skill_lv, int wflag) {
	...
	if (!is_attack_hitting(&wd, src, target, skill_id, skill_lv, true))
		...
		int64 bonus_damage = battle_calc_skill_constant_addition(&wd, src, target, skill_id,
																 skill_lv);	 // other skill bonuses
		...
		if (skill_id == HW_MAGICCRASHER) {	// Add weapon attack for MATK onto Magic Crasher
			...
			if (sstatus->matk_max > sstatus->matk_min) {
				ATK_ADD(wd.weaponAtk, wd.weaponAtk2,
						sstatus->matk_min + rnd() % (sstatus->matk_max - sstatus->matk_min));
			} else
		...
		if (sd) {
			wd.weaponAtk += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
												left_element, wd.weaponAtk, 2, wd.flag);
			wd.equipAtk += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
											   left_element, wd.equipAtk, 2, wd.flag);
			if (is_attack_left_handed(src, skill_id)) {
				wd.weaponAtk2 += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
													 left_element, wd.weaponAtk2, 3, wd.flag);
				wd.equipAtk2 += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
		...
		if (tsd && skill_id != NJ_ISSEN &&
			...
			wd.statusAtk += battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk, right_element,
												left_element, wd.statusAtk, 0, wd.flag);
			wd.weaponAtk += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
												left_element, wd.weaponAtk, 0, wd.flag);
			wd.equipAtk += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
											   left_element, wd.equipAtk, 0, wd.flag);
			wd.masteryAtk +=
				battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk, right_element,
									left_element, wd.masteryAtk, 0, wd.flag);
			if (is_attack_left_handed(src, skill_id)) {
				wd.statusAtk2 +=
					battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk, right_element,
										left_element, wd.statusAtk2, 1, wd.flag);
				wd.weaponAtk2 += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
				...
				wd.masteryAtk2 +=
					battle_calc_cardfix(BF_WEAPON, src, target, ignoreele_nk, right_element,
										left_element, wd.masteryAtk2, 1, wd.flag);
			}
		}
		...
		if (sd) {  // monsters, homuns and pets have their damage computed directly
			wd.damage =
				(int64)floor((float)((wd.statusAtk + wd.weaponAtk + wd.equipAtk + wd.percentAtk) *
										 (100 + sstatus->patk) / 100 +
									 wd.masteryAtk + bonus_damage));
			if (is_attack_left_handed(src, skill_id))
				wd.damage2 = (int64)floor(
					(float)((wd.statusAtk2 + wd.weaponAtk2 + wd.equipAtk2 + wd.percentAtk2) *
								(100 + sstatus->patk) / 100 +
			...
			if (wd.type == DMG_CRITICAL || wd.type == DMG_MULTI_HIT_CRITICAL) {
				if (skill_id > 0) {
					...
					if (is_attack_left_handed(src, skill_id))
						wd.damage2 +=
							(int64)floor((float)(wd.damage2 * sd->bonus.crit_atk_rate / 200));
				} else {
					wd.damage += (int64)floor((float)(wd.damage * sd->bonus.crit_atk_rate / 100));
					if (is_attack_left_handed(src, skill_id))
						wd.damage2 +=
							(int64)floor((float)(wd.damage2 * sd->bonus.crit_atk_rate / 100));
				}
			}
		...
		if (sd) {
			...
			if (sd->status.weapon == W_KATAR &&
				(katar_skill = pc_checkskill(sd, ASC_KATAR)) >
...
#ifdef RENEWAL
	if (is_attack_critical(&wd, src, target, skill_id, skill_lv, false)) {
		if (sd) {  // Check for player so we don't crash out, monsters don't have bonus crit rates
				   // [helvetica]
			wd.damage = (int64)floor((float)((wd.damage * (1.4f + (0.01f * sstatus->crate)))));
			if (is_attack_left_handed(src, skill_id))
				wd.damage2 =
					(int64)floor((float)((wd.damage2 * (1.4f + (0.01f * sstatus->crate)))));
		} else
		...
		case SR_GATEOFHELL: {
			...
			if (sc && sc->data[SC_COMBO] && sc->data[SC_COMBO]->val1 == SR_FALLENEMPIRE) {
				ATK_ADD(wd.damage, wd.damage2,
						static_cast<int64>(sstatus->max_sp * bonus) + 40 * status_get_lv(src));
			} else
				ATK_ADD(wd.damage, wd.damage2,
						static_cast<int64>(sstatus->sp * bonus) + 10 * status_get_lv(src));
		} break;
		case MH_TINDER_BREAKER:
			ATK_ADD(wd.damage, wd.damage2,
					2500 * skill_lv + status_get_lv(src));	// !TODO: Confirm base level bonus
			break;
		case MH_CBC:
			ATK_ADD(wd.damage, wd.damage2,
					4000 * skill_lv + status_get_lv(src));	// !TODO: Confirm base level bonus
			break;
		case MH_EQC:
			ATK_ADD(wd.damage, wd.damage2,
					6000 * skill_lv + status_get_lv(src));	// !TODO: Confirm base level bonus
			break;
		case NPC_MAXPAIN_ATK:
	...
	if (sd) {
		...
		if (skill_id !=
			CR_SHIELDBOOMERANG)	 // Only Shield boomerang doesn't takes the Star Crumbs bonus.
			ATK_ADD2(wd.damage, wd.damage2, ((wd.div_ < 1) ? 1 : wd.div_) * sd->right_weapon.star,
					 ((wd.div_ < 1) ? 1 : wd.div_) * sd->left_weapon.star);
		if (skill_id != MC_CARTREVOLUTION && pc_checkskill(sd, BS_HILTBINDING) > 0)
			ATK_ADD(wd.damage, wd.damage2, 4);
		if (skill_id == MO_FINGEROFFENSIVE) {  // The finger offensive spheres on moment of attack
											   // do count. [Skotlex]
...
#endif
		if (skill_id == CR_SHIELDBOOMERANG ||
			skill_id == PA_SHIELDCHAIN) {  // Refine bonus applies after cards and elements.
			short index = sd->equip_index[EQI_HAND_L];
		...
		switch (skill_id) {
			case RK_DRAGONBREATH:
			case RK_DRAGONBREATH_WATER:
				if (wd.flag & BF_LONG) {  // Add check here, because we want to apply the same
										  // behavior in pre-renewal [exneval]
			...
			default:
				...
				if (is_attack_left_handed(src, skill_id))
					wd.damage2 += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
...
#endif
		// Card Fix for target (tsd), 2 is not added to the "left" flag meaning "target cards only"
		wd.damage += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element, left_element,
										 wd.damage, 0, wd.flag);
		if (is_attack_left_handed(src, skill_id))
			wd.damage2 += battle_calc_cardfix(BF_WEAPON, src, target, nk, right_element,
...
struct Damage battle_calc_magic_attack(struct block_list *src, struct block_list *target,
	...
	ad.amotion = (skill_get_inf(skill_id) & INF_GROUND_SKILL
					  ? 0
					  : sstatus->amotion);	// Amotion should be 0 for ground skills.
	...
	switch (skill_id) {
		...
		case ABC_ABYSS_SQUARE:
			if (mflag == 2)
				ad.div_ = 2;
			break;
		case TR_METALIC_FURY:  // Deals up to 5 additional hits. But what affects the number of
							   // hits? [Rytech]
...
#define MATK_ADD(a) \
	{ ad.damage += a; }
		...
		switch (skill_id) {
			...
			case NPC_EARTHQUAKE:
				...
				if (src->type == BL_PC)
					ad.damage =
						sstatus->str * 2 +
						battle_calc_weapon_attack(src, target, skill_id, skill_lv, mflag).damage;
				else
					ad.damage =
						battle_calc_base_damage(src, sstatus, &sstatus->rhw, sc, tstatus->size, 0);
				...
				switch (skill_id) {
						...
						if (sc) {
							...
							if (sc->data[SC_SPELLFIST] && mflag & BF_SHORT) {
								skillratio += (sc->data[SC_SPELLFIST]->val3 * 100) +
											  (sc->data[SC_SPELLFIST]->val1 * 50 - 50) -
											  100;	// val3 = used bolt level, val1 = used spellfist
													// level. [Rytech]
								ad.div_ =
					...
					case LG_RAYOFGENESIS:
						skillratio += -100 + 230 * skill_lv +
									  sstatus->int_ / 6;  // !TODO: What's the INT bonus?
						if (sc && sc->data[SC_INSPIRATION])
							skillratio += 70 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					...
					case WM_METALICSOUND:
						skillratio +=
							-100 + 120 * skill_lv + 60 * ((sd) ? pc_checkskill(sd, WM_LESSON) : 1);
						if (tsc && tsc->data[SC_SLEEP])
					...
					case SO_EARTHGRAVE:
						skillratio += -100 + 2 * sstatus->int_ +
									  300 * pc_checkskill(sd, SA_SEISMICWEAPON) +
					...
					case SO_DIAMONDDUST:
						skillratio += -100 + 2 * sstatus->int_ +
									  300 * pc_checkskill(sd, SA_FROSTWEAPON) +
									  sstatus->int_ * skill_lv;
						RE_LVL_DMOD(100);
					...
					case SO_VARETYR_SPEAR:
						skillratio += -100 + (2 * sstatus->int_ +
											  150 * (pc_checkskill(sd, SO_STRIKING) +
													 pc_checkskill(sd, SA_LIGHTNINGLOADER)) +
					...
					case SU_CN_METEOR2:
						skillratio +=
							100 + 100 * skill_lv + sstatus->int_ * 5;  // !TODO: Confirm INT bonus
						RE_LVL_DMOD(100);
						break;
					case NPC_VENOMFOG:
						skillratio += 600 + 100 * skill_lv;
					...
					case IG_JUDGEMENT_CROSS:
						...
						if ((i = pc_checkskill_imperial_guard(sd, 3)) > 0)
							skillratio += skillratio * i / 100;
						break;
					case IG_CROSS_RAIN:	 // Need official damage increase from Spear and Sword
										 // Mastery. [Rytech]
						skillratio += -100 + 30 * skill_lv + 5 * sstatus->spl +
					...
					case CD_FRAMEN:
						...
						RE_LVL_DMOD(100);
						break;
					case AG_DESTRUCTIVE_HURRICANE_CLIMAX:  // Is this affected by BaseLV and SPL
														   // too??? [Rytech]
...
#ifdef RENEWAL
		ad.damage +=
			battle_calc_cardfix(BF_MAGIC, src, target, nk, s_ele, 0, ad.damage, 0, ad.flag);
#endif
		...
		if (sd) {
			...
			// Ignore Defense?
			if (!flag.imdef && (sd->bonus.ignore_mdef_ele & (1 << tstatus->def_ele) ||
		...
		if (!nk[NK_IGNOREELEMENT])
			ad.damage =
				battle_attr_fix(src, target, ad.damage, s_ele, tstatus->def_ele, tstatus->ele_lv);
	...
	// battle_do_reflect(BF_MAGIC,&ad, src, target, skill_id, skill_lv); //WIP [lighta] Magic skill
	// has own handler at skill_attack
	return ad;
}
...
struct Damage battle_calc_misc_attack(struct block_list *src, struct block_list *target,
	...
	s_ele = skill_get_ele(skill_id, skill_lv);
	if (s_ele == ELE_WEAPON || s_ele == ELE_ENDOWED)  // Attack that takes weapon's element for misc
	...
	switch (skill_id) {
		...
		case HT_CLAYMORETRAP:
			md.damage =
				(int64)(skill_lv * sstatus->dex * (3.0 + (float)status_get_lv(src) / 100.0) *
		...
		case GN_FIRE_EXPANSION_ACID:
			...
			// skill is a "forced neutral" type skill, it benefits from weapon element but final
			// damage 	is considered "neutral" for purposes of resistances
			{
				struct Damage atk = battle_calc_weapon_attack(src, target, skill_id, skill_lv, 0);
				struct Damage matk = battle_calc_magic_attack(src, target, skill_id, skill_lv, 0);
				md.damage =
					7 * ((atk.damage / skill_lv + matk.damage / skill_lv) * tstatus->vit / 100);

				// AD benefits from endow/element but damage is forced back to neutral
				md.damage = battle_attr_fix(src, target, md.damage, ELE_NEUTRAL, tstatus->def_ele,
											tstatus->ele_lv);
			}
			// Fall through
#else
		case CR_ACIDDEMONSTRATION:
			if (tstatus->vit + sstatus->int_)  // crash fix
				md.damage = (int)((int64)7 * tstatus->vit * sstatus->int_ * sstatus->int_ /
		...
		case NJ_ISSEN:
			...
			// final damage = base damage + ((mirror image count + 1) / 5 * base damage) - (edef +
			// sdef) modified def formula
			{
				short totaldef;
				struct Damage atk = battle_calc_weapon_attack(src, target, skill_id, skill_lv, 0);
				struct status_change *sc = status_get_sc(src);

				md.damage = (int64)sstatus->hp +
		...
		case RA_ICEBOUNDTRAP:
			...
			if (sd) {
				...
				if (researchskill_lv)
					md.damage =
						md.damage * 20 * researchskill_lv / (skill_id == RA_CLUSTERBOMB ? 50 : 100);
				else
					md.damage = 0;
			} else
		...
		case SJ_NOVAEXPLOSING:
			...
			// Additional Damage
			md.damage +=
				sstatus->max_hp / (6 - min(5, skill_lv)) + status_get_max_sp(src) * (2 * skill_lv);
			break;
	}
	...
	if (!nk[NK_IGNOREELEMENT])
		md.damage =
			battle_attr_fix(src, target, md.damage, s_ele, tstatus->def_ele, tstatus->ele_lv);
 ...
 *	Refined and optimized by helvetica
 */
struct Damage battle_calc_attack(int attack_type, struct block_list *bl, struct block_list *target,
								 uint16 skill_id, uint16 skill_lv, int flag) {
	struct Damage d;
	...
	if (d.damage + d.damage2 < 1) {	 // Miss/Absorbed
		...
		if (d.dmg_lv ==
			ATK_DEF /*&& attack_type&(BF_MAGIC|BF_MISC)*/)	// Isn't it that additional effects
															// don't apply if miss?
			d.dmg_lv = ATK_MISS;
		d.dmotion = 0;
		if (bl->type == BL_PC)
		...
		if (tsc != nullptr && tsc->data[SC_WEAPONBLOCKING]) {
			...
			if (attack_type == BF_WEAPON && rnd() % 100 < tsce->val2) {
				...
				sc_start(bl, target, SC_WEAPONBLOCK_ON, 100, bl->id,
						 skill_get_time2(GC_WEAPONBLOCKING, tsce->val1));
			}
		}
 ...
 *	Refined and optimized by helvetica
 */
int64 battle_calc_return_damage(struct block_list *tbl, struct block_list *src, int64 *dmg,
								int flag, uint16 skill_id, bool status_reflect) {
	...
	if (flag & BF_SHORT) {	// Bounces back part of the damage.
		if ((skill_get_inf2(skill_id, INF2_ISTRAP) || !status_reflect) && tsd &&
			...
			if (tsc->data[SC_REFLECTSHIELD]) {
				...
				if ((sce_d = tsc->data[SC_DEVOTION]) && (d_bl = map_id2bl(sce_d->val1)) &&
					...
					if ((!skill_id && battle_config.devotion_rdamage_skill_only &&
						 tsc->data[SC_REFLECTSHIELD]->val4) ||
						!check_distance_bl(tbl, d_bl, sce_d->val3))
						return 0;
				}
			}
			if (tsc->data[SC_REFLECTSHIELD] && skill_id != WS_CARTTERMINATION &&
				skill_id != NPC_MAXPAIN_ATK) {
				// Don't reflect non-skill attack if has SC_REFLECTSHIELD from Devotion bonus
				// inheritance
				if (!skill_id && battle_config.devotion_rdamage_skill_only &&
					tsc->data[SC_REFLECTSHIELD]->val4)
					rdamage = 0;
			...
			if (tsc->data[SC_DEATHBOUND] && skill_id != WS_CARTTERMINATION &&
				skill_id != GN_HELLS_PLANT_ATK && !status_bl_has_mode(src, MD_STATUSIMMUNE)) {
				if (distance_bl(src, tbl) <= 0 ||
					!map_check_dir(map_calc_dir(tbl, src->x, src->y), unit_getdir(tbl))) {
					int64 rd1 = i64min(damage, status_get_max_hp(tbl)) *
								tsc->data[SC_DEATHBOUND]->val2 / 100;  // Amplify damage.

					*dmg = rd1 * 30 / 100;	// Received damage = 30% of amplified damage.
					clif_skill_damage(src, tbl, gettick(), status_get_amotion(src), 0, -30000, 1,
									  RK_DEATHBOUND, tsc->data[SC_DEATHBOUND]->val1, DMG_SINGLE);
					skill_blown(tbl, src,
								skill_get_blewcount(RK_DEATHBOUND, tsc->data[SC_DEATHBOUND]->val1),
								unit_getdir(src), BLOWN_NONE);
					status_change_end(tbl, SC_DEATHBOUND);
					rdamage +=
						rd1 * 70 / 100;	 // Target receives 70% of the amplified damage. [Rytech]
				}
			}
		}
	} else {
 ...
 */
bool battle_vellum_damage(struct map_session_data *sd, struct block_list *target,
						  struct Damage *wd) {
	...
	struct status_data *tstatus = status_get_status_data(target);
	// bHPVanishRaceRate
	int16 vellum_rate_hp = cap_value(
		sd->hp_vanish_race[tstatus->race].rate + sd->hp_vanish_race[RC_ALL].rate, 0, INT16_MAX);
	int8 vellum_hp = cap_value(
		sd->hp_vanish_race[tstatus->race].per + sd->hp_vanish_race[RC_ALL].per, INT8_MIN, INT8_MAX);
	// bSPVanishRaceRate
	int16 vellum_rate_sp = cap_value(
		sd->sp_vanish_race[tstatus->race].rate + sd->sp_vanish_race[RC_ALL].rate, 0, INT16_MAX);
	int8 vellum_sp = cap_value(
		sd->sp_vanish_race[tstatus->race].per + sd->sp_vanish_race[RC_ALL].per, INT8_MIN, INT8_MAX);

	// The HP and SP damage bonus from these items don't stack because of the special damage display
	// for SP. Vellum damage overrides any other damage done as well.
 ...
 *-------------------------------------------*/
void battle_drain(struct map_session_data *sd, struct block_list *tbl, int64 rdamage, int64 ldamage,
				  int race, int class_) {
...
int battle_damage_area(struct block_list *bl, va_list ap) {
	...
	if (bl != src && battle_check_target(src, bl, BCT_ENEMY) > 0) {
		...
		skill_additional_effect(src, bl, CR_REFLECTSHIELD, 1, BF_WEAPON | BF_SHORT | BF_NORMAL,
								ATK_DEF, tick);
		map_freeblock_unlock();
	}
 ...
 */
void battle_autocast_aftercast(struct block_list *src, uint16 skill_id, uint16 skill_lv,
							   t_tick tick) {
 ...
 */
void battle_autocast_elembuff_skill(struct map_session_data *sd, struct block_list *target,
									uint16 skill_id, t_tick tick, int flag) {
 ...
 *------------------------------------------*/
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, t_tick tick,
									int flag) {
	...
	if (sd) {
		sd->state.arrow_atk = (sd->status.weapon == W_BOW ||
	...
	if (tsc && tsc->data[SC_BLADESTOP_WAIT] &&
		...
		if (sc_start4(src, src, SC_BLADESTOP, 100, sd ? pc_checkskill(sd, MO_BLADESTOP) : 5, 0, 0,
					  target->id, duration)) {	// Target locked.
			clif_damage(src, target, tick, sstatus->amotion, 1, 0, 1, DMG_NORMAL, 0,
						false);	 // Display MISS.
	...
	if (sd && (skillv = pc_checkskill(sd, MO_TRIPLEATTACK)) > 0) {
		...
		if (rnd() % 100 < triple_rate) {
			...
			sd->ud.canact_tick =
				i64max(tick + skill_delayfix(src, MO_TRIPLEATTACK, skillv), sd->ud.canact_tick);
	...
	if (sc) {
		if (sc->data[SC_SACRIFICE]) {
			 ...
			 */
			ret_val = (damage_lv)skill_attack(BF_WEAPON, src, src, target, PA_SACRIFICE, skill_lv,
											  tick, 0);
	...
	if (tsc && tsc->data[SC_GT_ENERGYGAIN]) {
		...
		if (tsd && rnd() % 100 < tsc->data[SC_GT_ENERGYGAIN]->val2)
			pc_addspiritball(
				tsd, skill_get_time2(SR_GENTLETOUCH_ENERGYGAIN, tsc->data[SC_GT_ENERGYGAIN]->val1),
				spheres);
	}

	if (tsc && tsc->data[SC_MTF_MLEATKED] && rnd() % 100 < tsc->data[SC_MTF_MLEATKED]->val2)
		clif_skill_nodamage(target, target, SM_ENDURE, tsc->data[SC_MTF_MLEATKED]->val1,
	...
	if (sc && sc->count) {
		...
		if (sc->data[SC_SPELLFIST] && !vellum_damage) {
			if (status_charge(src, 0, 20)) {
				if (!is_infinite_defense(target, wd.flag)) {
					struct Damage ad =
						battle_calc_attack(BF_MAGIC, src, target, sc->data[SC_SPELLFIST]->val2,
										   sc->data[SC_SPELLFIST]->val3, flag | BF_SHORT);
		...
		if (sc->data[SC_GIANTGROWTH] && (wd.flag & BF_SHORT) &&
			rnd() % 100 < sc->data[SC_GIANTGROWTH]->val2 && !is_infinite_defense(target, wd.flag) &&
			!vellum_damage)
		...
		if (sd && battle_config.arrow_decrement && sc->data[SC_FEARBREEZE] &&
			...
			if (idx >= 0 &&
				sd->inventory.u.items_inventory[idx].amount >= sc->data[SC_FEARBREEZE]->val4) {
	...
	if (damage > 0 && src != target) {
		if (sc && sc->data[SC_DUPLELIGHT] &&
			(wd.flag & BF_SHORT)) {	 // Activates only from regular melee damage. Success chance is
									 // seperate for both duple light attacks.
			uint16 duple_rate = 10 + 2 * sc->data[SC_DUPLELIGHT]->val1;

			if (rand() % 100 < duple_rate)
				skill_castend_damage_id(src, target, AB_DUPLELIGHT_MELEE,
										sc->data[SC_DUPLELIGHT]->val1, tick, flag | SD_LEVEL);

			if (rand() % 100 < duple_rate)
				skill_castend_damage_id(src, target, AB_DUPLELIGHT_MAGIC,
										sc->data[SC_DUPLELIGHT]->val1, tick, flag | SD_LEVEL);
		}
	}

	wd.dmotion = clif_damage(src, target, tick, wd.amotion, wd.dmotion, wd.damage, wd.div_, wd.type,
							 wd.damage2, wd.isspdamage);
	...
	if (!(tsc && tsc->data[SC_DEVOTION]) && !vellum_damage &&
		...
		battle_delay_damage(tick, wd.amotion, src, target, wd.flag, 0, 0, damage, wd.dmg_lv,
							wd.dmotion, true, wd.isspdamage);
	if (tsc) {
		if (tsc->data[SC_DEVOTION]) {
			...
			if (d_bl &&
				...
				if (damage > 0) {
					...
					clif_damage(d_bl, d_bl, gettick(), wd.amotion, wd.dmotion, devotion_damage, 1,
								DMG_NORMAL, 0, false);
		...
		if (target->type == BL_PC && (wd.flag & BF_SHORT) && tsc->data[SC_CIRCLE_OF_FIRE_OPTION]) {
			...
			if (ed) {
				clif_skill_damage(&ed->bl, target, tick, status_get_amotion(src), 0, -30000, 1,
								  EL_CIRCLE_OF_FIRE, tsc->data[SC_CIRCLE_OF_FIRE_OPTION]->val1,
								  DMG_SINGLE);
	...
	if (sd) {
		...
		if (wd.flag & BF_WEAPON && sc && sc->data[SC__AUTOSHADOWSPELL] &&
			...
			(sk_idx = skill_get_index(r_skill)) && sd->status.skill[sk_idx].id != 0 &&
			...
			if (r_skill != AL_HOLYLIGHT && r_skill != PR_MAGNUS) {
				...
				sd->ud.canact_tick =
					i64max(tick + skill_delayfix(src, r_skill, r_lv), sd->ud.canact_tick);
				clif_status_change(src, EFST_POSTDELAY, 1, skill_delayfix(src, r_skill, r_lv), 0, 0,
								   1);
			}
		}
	...
	if (sd && tsc && wd.flag & BF_LONG && tsc->data[SC_WINDSIGN] &&
...
int battle_check_target(struct block_list *src, struct block_list *target, int flag) {
	...
	int strip_enemy =
		1;	// Flag which marks whether to remove the BCT_ENEMY status if it's also friend/ally.
		...
		case BL_MOB: {
			...
			if (((md->special_state.ai == AI_SPHERE ||	// Marine Spheres
				...
				(md->special_state.ai == AI_FAW &&
				 (t_bl->id != s_bl->id ||
				  (s_bl->type == BL_PC && src->type != BL_MOB)))) {	 // Targettable by players
				state |= BCT_ENEMY;
				strip_enemy = 0;
			}
			break;
		...
		case BL_SKILL: {
			...
			if (skill_get_inf2(su->group->skill_id, INF2_ISTRAP) &&
				...
				if (!skill_id || su->group->skill_id == NPC_REVERBERATION ||
					su->group->skill_id == WM_POEMOFNETHERWORLD) {
					;
				} else if (skill_get_inf2(skill_id,
				...
				switch (skill_id) {
					...
					case HT_CLAYMORETRAP:
						...
						return 0;
					default:
						// Usually BCT_ALL stands for only hitting chars, but skills specifically
						// set to hit traps also hit icewall
				...
				state |= BCT_ENEMY;
				strip_enemy = 0;
			} else	// Excepting traps, Icewall, and Wall of Thorns, you should not be able to
					// target skills.
				return 0;
		} break;
		case BL_MER:
	...
	switch (s_bl->type) {  // Checks on source master
		case BL_PC: {
			...
			if (!sd->status.guild_id && t_bl->type == BL_MOB &&
				((TBL_MOB *)t_bl)->mob_id == MOBID_EMPERIUM && mapdata_flag_gvg(mapdata))
	...
	if ((flag & BCT_ALL) ==
		BCT_ALL) {	// All actually stands for all attackable chars, icewall and traps
	...
	if (mapdata_flag_vs(mapdata)) {	 // Check rivalry settings.
		...
		if (flag & (BCT_GUILD | BCT_ENEMY)) {
			...
			if (!(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOGUILD]) && s_guild && t_guild &&
				(s_guild == t_guild ||
		...
		if (state & BCT_ENEMY && battle_config.pk_mode && !mapdata_flag_gvg(mapdata) &&
			s_bl->type == BL_PC &&
...
bool battle_check_range(struct block_list *src, struct block_list *bl, int range) {
	...
	if (src->type == BL_PC) {  // Range for players' attacks and skills should always have a
							   // circular check. [Angezerus]
...
static const struct _battle_data {
	...
	{"natural_heal_weight_rate_renewal", &battle_config.natural_heal_weight_rate_renewal, 70, 0,
	 100},
	{
		"arrow_decrement",
	...
	},
	{
		...
		&battle_config.max_res_mres_reduction,
		625,
		1,
		SHRT_MAX,
	},
...
void battle_set_defaults() {
	int i;
	for (i = 0; i < ARRAYLENGTH(battle_data); i++) *battle_data[i].val = battle_data[i].defval;
}
...
void battle_adjust_conf() {
	...
	if (battle_config.feature_buying_store) {
		ShowWarning(
			"conf/battle/feature.conf:buying_store is enabled but it requires PACKETVER 2010-04-27 "
			"or newer, disabling...\n");
	...
	if (battle_config.feature_search_stores) {
		ShowWarning(
			"conf/battle/feature.conf:search_stores is enabled but it requires PACKETVER "
			"2010-08-03 or newer, disabling...\n");
	...
	if (battle_config.feature_bgqueue) {
		ShowWarning(
			"conf/battle/feature.conf:bgqueue is enabled but it requires PACKETVER 2012-01-01 or "
			"newer, disabling...\n");
	...
	if (battle_config.feature_auction) {
		ShowWarning(
			"conf/battle/feature.conf:feature.auction is enabled but it is not stable on "
			"PACKETVER " EXPAND_AND_QUOTE(PACKETVER) ", disabling...\n");
		ShowWarning(
			"conf/battle/feature.conf:feature.auction change value to '2' to silence this warning "
			"and maintain it enabled\n");
		battle_config.feature_auction = 0;
	}
#elif PACKETVER >= 20141112
	if (battle_config.feature_auction) {
		ShowWarning(
			"conf/battle/feature.conf:feature.auction is enabled but it is not available for "
			"clients from 2014-11-12 on, disabling...\n");
		ShowWarning(
			"conf/battle/feature.conf:feature.auction change value to '2' to silence this warning "
			"and maintain it enabled\n");
		battle_config.feature_auction = 0;
	...
	if (battle_config.feature_banking) {
		ShowWarning(
			"conf/battle/feature.conf banking is enabled but it requires PACKETVER 2013-07-24 or "
			"newer, disabling...\n");
	...
	if (battle_config.mvp_exp_reward_message) {
		ShowWarning(
			"conf/battle/client.conf MVP EXP reward message is enabled but it requires PACKETVER "
			"2013-12-23 or newer, disabling...\n");
	...
	if (battle_config.feature_roulette) {
		ShowWarning(
			"conf/battle/feature.conf roulette is enabled but it requires PACKETVER 2014-10-22 or "
			"newer, disabling...\n");
	...
	if (battle_config.feature_achievement) {
		ShowWarning(
			"conf/battle/feature.conf achievement is enabled but it requires PACKETVER 2015-05-13 "
			"or newer, disabling...\n");
	...
	if (battle_config.feature_petevolution) {
		ShowWarning(
			"conf/battle/feature.conf petevolution is enabled but it requires PACKETVER 2014-10-08 "
			"or newer, disabling...\n");
		battle_config.feature_petevolution = 0;
	}
	if (battle_config.feature_pet_autofeed) {
		ShowWarning(
			"conf/battle/feature.conf pet auto feed is enabled but it requires PACKETVER "
			"2014-10-08 or newer, disabling...\n");
	...
	if (battle_config.feature_refineui) {
		ShowWarning(
			"conf/battle/feature.conf refine UI is enabled but it requires PACKETVER 2016-10-12 or "
			"newer, disabling...\n");
	...
	if (battle_config.feature_equipswitch) {
		ShowWarning(
			"conf/battle/feature.conf equip switch is enabled but it requires PACKETVER 2017-02-08 "
			"or newer, disabling...\n");
	...
	if (battle_config.feature_homunculus_autofeed) {
		ShowWarning(
			"conf/battle/feature.conf homunculus autofeeding is enabled but it requires PACKETVER "
			"2017-09-20 or newer, disabling...\n");
	...
	if (battle_config.feature_attendance) {
		ShowWarning(
			"conf/battle/feature.conf attendance system is enabled but it requires PACKETVER "
			"2018-03-07 or newer, disabling...\n");
	...
	if (battle_config.feature_privateairship) {
		ShowWarning(
			"conf/battle/feature.conf private airship system is enabled but it requires PACKETVER "
			"2018-03-21 or newer, disabling...\n");
		battle_config.feature_privateairship = 0;
	}
#endif

#if !(PACKETVER_MAIN_NUM >= 20190116 || PACKETVER_RE_NUM >= 20190116 || \
	  PACKETVER_ZERO_NUM >= 20181226)
	if (battle_config.feature_barter) {
		ShowWarning(
			"conf/battle/feature.conf barter shop system is enabled but it requires PACKETVER "
			"2019-01-16 or newer, disabling...\n");
		battle_config.feature_barter = 0;
	}
#endif

#if !(PACKETVER_MAIN_NUM >= 20191120 || PACKETVER_RE_NUM >= 20191106 || \
	  PACKETVER_ZERO_NUM >= 20191127)
	if (battle_config.feature_barter_extended) {
		ShowWarning(
			"conf/battle/feature.conf extended barter shop system is enabled but it requires "
			"PACKETVER 2019-11-06 or newer, disabling...\n");
	...
	if (battle_config.custom_cell_stack_limit != 1)
		ShowWarning(
			"Battle setting 'custom_cell_stack_limit' takes no effect as this server was compiled "
			"without Cell Stack Limit support.\n");
#endif
...
void do_init_battle(void) {
	delay_damage_ers =
		ers_new(sizeof(struct delay_damage), "battle.cpp::delay_damage_ers", ERS_OPT_CLEAR);
