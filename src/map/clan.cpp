...
int clan_getMemberIndex(struct clan* clan, uint32 account_id) {
	...
	ARR_FIND(0, clan->max_member, i,
			 clan->members[i] != NULL && clan->members[i]->status.account_id == account_id);
...
bool clan_member_leave(struct map_session_data* sd, int clan_id, uint32 account_id,
					   uint32 char_id) {
	struct clan* clan;
	...
	if (sd->status.account_id != account_id || sd->status.char_id != char_id ||
		sd->status.clan_id != clan_id || (clan = sd->clan) == NULL) {
		return false;
	}
...
void clan_send_message(struct map_session_data* sd, const char* mes, int len) {
	...
	log_chat(LOG_CHAT_CLAN, sd->status.clan_id, sd->status.char_id, sd->status.account_id,
