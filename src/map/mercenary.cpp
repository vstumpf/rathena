...
void merc_contract_init(s_mercenary_data *md) {
	if (md->contract_timer == INVALID_TIMER)
		md->contract_timer =
			add_timer(gettick() + md->mercenary.life_time, merc_contract_end, md->master->bl.id, 0);
...
void mercenary_killbonus(s_mercenary_data *md) {
	std::vector<sc_type> scs = {SC_MERC_FLEEUP, SC_MERC_ATKUP, SC_MERC_HPUP, SC_MERC_SPUP,
								SC_MERC_HITUP};
...
uint64 MercenaryDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "Race")) {
		...
		if (!script_get_constant(race_constant.c_str(), &constant)) {
			this->invalidWarning(
				node["Race"], "Unknown mercenary race %s, defaulting to Formless.\n", race.c_str());
			constant = RC_FORMLESS;
		}

		if (!CHK_RACE(constant)) {
			this->invalidWarning(
				node["Race"], "Invalid mercenary race %s, defaulting to Formless.\n", race.c_str());
			constant = RC_FORMLESS;
		}
	...
	if (this->nodeExists(node, "Element")) {
		...
		if (!script_get_constant(ele_constant.c_str(), &constant)) {
			this->invalidWarning(node["Element"],
								 "Unknown mercenary element %s, defaulting to Neutral.\n",
								 ele.c_str());
		...
		if (!CHK_ELEMENT(constant)) {
			this->invalidWarning(node["Element"],
								 "Invalid mercenary element %s, defaulting to Neutral.\n",
								 ele.c_str());
			constant = ELE_NEUTRAL;
		}
	...
	if (this->nodeExists(node, "ElementLevel")) {
		...
		if (!CHK_ELEMENT_LEVEL(level)) {
			this->invalidWarning(node["ElementLevel"],
								 "Invalid mercenary element level %hu, defaulting to 1.\n", level);
			level = 1;
		}
	...
	if (this->nodeExists(node, "WalkSpeed")) {
		...
		if (speed < MIN_WALK_SPEED || speed > MAX_WALK_SPEED) {
			this->invalidWarning(node["WalkSpeed"],
								 "Invalid mercenary walk speed %hu, capping...\n", speed);
 ...
 **/
void do_final_mercenary(void) { mercenary_db.clear(); }
