...
typedef enum e_log_filter {
	...
	LOG_FILTER_PETITEM = 0x080,	  // Pet Accessories(8) + Eggs(7) (well, monsters don't drop 'em but
								  // we'll use the same system for ALL logs)
...
static bool should_log_item(t_itemid nameid, int amount, int refine) {
	...
	if ((filter & LOG_FILTER_ALL) || (filter & LOG_FILTER_HEALING && id->type == IT_HEALING) ||
		...
		(filter & LOG_FILTER_CHANCE &&
		 ((id->maxchance != -1 && id->maxchance <= log_config.rare_items_log) ||
		  id->nameid == ITEMID_EMPERIUM)))
		return true;
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(stmt,
								LOG_QUERY " INTO `%s` (`branch_date`, `account_id`, `char_id`, "
										  "`char_name`, `map`) VALUES (NOW(), '%d', '%d', ?, '%s')",
								log_config.log_branch, sd->status.account_id, sd->status.char_id,
								mapindex_id2name(sd->mapindex)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, sd->status.name,
		...
		fprintf(logfp, "%s - %s[%d:%d]\t%s\n", timestring, sd->status.name, sd->status.account_id,
				sd->status.char_id, mapindex_id2name(sd->mapindex));
		fclose(logfp);
	}
}
...
void log_pick(int id, int16 m, e_log_pick_type type, int amount, struct item* itm) {
	...
	if (log_config.sql_logs) {
		...
		StringBuf_Printf(&buf,
						 "%s INTO `%s` (`time`, `char_id`, `type`, `nameid`, `amount`, `refine`, "
						 "`map`, `unique_id`, `bound`, `enchantgrade`",
		...
		StringBuf_Printf(&buf,
						 ") VALUES(NOW(),'%u','%c','%u','%d','%d','%s','%" PRIu64 "','%d','%d'", id,
						 log_picktype2char(type), itm->nameid, amount, itm->refine,
						 map_getmapdata(m)->name[0] ? map_getmapdata(m)->name : "", itm->unique_id,
						 itm->bound, itm->enchantgrade);

		for (i = 0; i < MAX_SLOTS; i++) StringBuf_Printf(&buf, ",'%u'", itm->card[i]);
		for (i = 0; i < MAX_ITEM_RDM_OPT; i++)
			StringBuf_Printf(&buf, ",'%d','%d','%d'", itm->option[i].id, itm->option[i].value,
							 itm->option[i].param);
		StringBuf_Printf(&buf, ")");
		...
		fprintf(logfp, "%s - %d\t%c\t%u,%d,%d,%u,%u,%u,%u,%s,'%" PRIu64 "',%d,%d\n", timestring, id,
				log_picktype2char(type), itm->nameid, amount, itm->refine, itm->card[0],
				itm->card[1], itm->card[2], itm->card[3],
				map_getmapdata(m)->name[0] ? map_getmapdata(m)->name : "", itm->unique_id,
				itm->bound, itm->enchantgrade);
		fclose(logfp);
	}
...
void log_zeny(struct map_session_data* sd, e_log_pick_type type, struct map_session_data* src_sd,
			  int amount) {
	nullpo_retv(sd);
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR ==
			Sql_Query(logmysql_handle,
					  LOG_QUERY " INTO `%s` (`time`, `char_id`, `src_id`, `type`, `amount`, `map`) "
								"VALUES (NOW(), '%d', '%d', '%c', '%d', '%s')",
					  log_config.log_zeny, sd->status.char_id, src_sd->status.char_id,
					  log_picktype2char(type), amount, mapindex_id2name(sd->mapindex))) {
		...
		time(&curtime);
		strftime(timestring, sizeof(timestring), log_timestamp_format, localtime(&curtime));
		fprintf(logfp, "%s - %s[%d]\t%s[%d]\t%d\t\n", timestring, src_sd->status.name,
				src_sd->status.account_id, sd->status.name, sd->status.account_id, amount);
		fclose(logfp);
	}
}
...
void log_mvpdrop(struct map_session_data* sd, int monster_id, t_itemid nameid, t_exp exp) {
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR == Sql_Query(logmysql_handle,
								   LOG_QUERY
								   " INTO `%s` (`mvp_date`, `kill_char_id`, `monster_id`, `prize`, "
								   "`mvpexp`, `map`) VALUES (NOW(), '%d', '%d', '%u', '%" PRIu64
								   "', '%s') ",
								   log_config.log_mvpdrop, sd->status.char_id, monster_id, nameid,
								   exp, mapindex_id2name(sd->mapindex))) {
			Sql_ShowDebug(logmysql_handle);
			return;
		}
	} else {
		...
		time(&curtime);
		strftime(timestring, sizeof(timestring), log_timestamp_format, localtime(&curtime));
		fprintf(logfp, "%s - %s[%d:%d]\t%d\t%u,%" PRIu64 "\n", timestring, sd->status.name,
				sd->status.account_id, sd->status.char_id, monster_id, nameid, exp);
		fclose(logfp);
	}
}
...
void log_atcommand(struct map_session_data* sd, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(
					stmt,
					LOG_QUERY " INTO `%s` (`atcommand_date`, `account_id`, `char_id`, `char_name`, "
							  "`map`, `command`) VALUES (NOW(), '%d', '%d', ?, '%s', ?)",
					log_config.log_gm, sd->status.account_id, sd->status.char_id,
					mapindex_id2name(sd->mapindex)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, sd->status.name,
											 strnlen(sd->status.name, NAME_LENGTH)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (char*)message,
											 safestrnlen(message, 255)) ||
			SQL_SUCCESS != SqlStmt_Execute(stmt)) {
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
			return;
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(
					stmt,
					LOG_QUERY
					" INTO `%s` (`npc_date`, `char_name`, `map`, `mes`) VALUES (NOW(), ?, '%s', ?)",
					log_config.log_npc, map_mapid2mapname(nd->bl.m)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, nd->name,
											 strnlen(nd->name, NAME_LENGTH)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (char*)message,
											 safestrnlen(message, 255)) ||
			SQL_SUCCESS != SqlStmt_Execute(stmt)) {
			SqlStmt_ShowDebug(stmt);
...
void log_npc(struct map_session_data* sd, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(stmt,
								LOG_QUERY
								" INTO `%s` (`npc_date`, `account_id`, `char_id`, `char_name`, "
								"`map`, `mes`) VALUES (NOW(), '%d', '%d', ?, '%s', ?)",
...
/// logs chat
void log_chat(e_log_chat_type type, int type_id, int src_charid, int src_accid, const char* mapname,
			  int x, int y, const char* dst_charname, const char* message) {
	...
	if (log_config.sql_logs) {
		...
		if (SQL_SUCCESS !=
				SqlStmt_Prepare(
					stmt,
					LOG_QUERY
					" INTO `%s` (`time`, `type`, `type_id`, `src_charid`, `src_accountid`, "
					"`src_map`, `src_map_x`, `src_map_y`, `dst_charname`, `message`) VALUES "
					"(NOW(), '%c', '%d', '%d', '%d', '%s', '%d', '%d', ?, ?)",
					log_config.log_chat, log_chattype2char(type), type_id, src_charid, src_accid,
					mapname, x, y) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, (char*)dst_charname,
											 safestrnlen(dst_charname, NAME_LENGTH)) ||
			SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, (char*)message,
											 safestrnlen(message, CHAT_SIZE_MAX)) ||
			SQL_SUCCESS != SqlStmt_Execute(stmt)) {
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
			return;
		...
		time(&curtime);
		strftime(timestring, sizeof(timestring), log_timestamp_format, localtime(&curtime));
		fprintf(logfp, "%s - %c,%d,%d,%d,%s,%d,%d,%s,%s\n", timestring, log_chattype2char(type),
				type_id, src_charid, src_accid, mapname, x, y, dst_charname, message);
		fclose(logfp);
	}
}

/// logs cash transactions
void log_cash(struct map_session_data* sd, e_log_pick_type type, e_log_cash_type cash_type,
			  int amount) {
	nullpo_retv(sd);
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR ==
			Sql_Query(logmysql_handle,
					  LOG_QUERY " INTO `%s` ( `time`, `char_id`, `type`, `cash_type`, `amount`, "
								"`map` ) VALUES ( NOW(), '%d', '%c', '%c', '%d', '%s' )",
		...
		fprintf(logfp, "%s - %s[%d]\t%d(%c)\t\n", timestring, sd->status.name,
				sd->status.account_id, amount, log_cashtype2char(cash_type));
		fclose(logfp);
	}
}
...
void log_feeding(struct map_session_data* sd, e_log_feeding_type type, t_itemid nameid) {
	...
	if (log_config.sql_logs) {
		if (SQL_ERROR ==
			Sql_Query(logmysql_handle,
					  LOG_QUERY
					  " INTO `%s` (`time`, `char_id`, `target_id`, `target_class`, `type`, "
					  ...
					  log_feedingtype2char(type), intimacy, nameid, mapindex_id2name(sd->mapindex),
					  sd->bl.x, sd->bl.y)) {
			Sql_ShowDebug(logmysql_handle);
			return;
		}
		...
		fprintf(logfp, "%s - %s[%d]\t%d\t%d(%c)\t%d\t%u\t%s\t%hu,%hu\n", timestring,
				sd->status.name, sd->status.char_id, target_id, target_class,
				log_feedingtype2char(type), intimacy, nameid, mapindex_id2name(sd->mapindex),
				sd->bl.x, sd->bl.y);
		fclose(logfp);
	}
