...
static void deactivate_pcreset(struct npc_data* nd, int64 setid) {
	...
	if (setid == -1) {
		while (npcParse->active != NULL) deactivate_pcreset(nd, npcParse->active->setid);
		return;
	}
	pcreset = npcParse->active;
	while (pcreset != NULL) {
 ...
 */
void npc_chat_def_pattern(struct npc_data* nd, int64 setid, const char* pattern,
						  const char* label) {
	const char* err;
	int erroff;
...
void npc_chat_finalize(struct npc_data* nd) {
	...
	while (npcParse->active) delete_pcreset(nd, npcParse->active->setid);

	while (npcParse->inactive) delete_pcreset(nd, npcParse->inactive->setid);
...
int npc_chat_sub(struct block_list* bl, va_list ap) {
	...
	for (pcreset = npcParse->active; pcreset != NULL; pcreset = pcreset->next) {
		...
		for (e = pcreset->head; e != NULL; e = e->next) {
			...
			int r =
				pcre_exec(e->pcre_, e->pcre_extra_, msg, len, 0, 0, offsets, ARRAYLENGTH(offsets));
			if (r > 0) {
				...
				ARR_FIND(0, nd->u.scr.label_list_num, i,
						 strncmp(lst[i].name, e->label, sizeof(lst[i].name)) == 0);
