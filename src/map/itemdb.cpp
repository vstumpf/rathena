...
uint64 ItemDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Type")) {
		...
		if (!script_get_constant(type_constant.c_str(), &constant) || constant < IT_HEALING ||
			constant >= IT_MAX) {
	...
	if (this->nodeExists(node, "SubType")) {
		...
		if (item->type == IT_WEAPON) {
			...
			if (!script_get_constant(type_constant.c_str(), &constant) || constant < W_FIST ||
				...
				this->invalidWarning(node["SubType"],
									 "Invalid weapon type %s, defaulting to W_FIST.\n",
									 type.c_str());
				item->subtype = W_FIST;
			}
			...
			if (!script_get_constant(type_constant.c_str(), &constant) || constant <= AMMO_NONE ||
				...
				this->invalidWarning(node["SubType"],
									 "Invalid ammo type %s, defaulting to AMMO_NONE.\n",
									 type.c_str());
			...
			if (!script_get_constant(type_constant.c_str(), &constant) || constant < CARD_NORMAL ||
				...
				this->invalidWarning(node["SubType"],
									 "Invalid card type %s, defaulting to CARD_NORMAL.\n",
									 type.c_str());
			...
			this->invalidWarning(node["SubType"],
								 "Item sub type is not supported for this item type.\n");
			return 0;
		}
	} else {
		if (!exists)
	...
	if (this->nodeExists(node, "Range")) {
		...
		if (range > AREA_SIZE) {
			this->invalidWarning(
				node["Range"],
				"Item attack range %d exceeds AREA_SIZE (%d), capping to AREA_SIZE.\n", range,
				AREA_SIZE);
			range = AREA_SIZE;
		}
	...
	if (this->nodeExists(node, "Slots")) {
		...
		if (slots > MAX_SLOTS) {
			this->invalidWarning(node["Slots"],
								 "Item slots %d exceeds MAX_SLOTS (%d), capping to MAX_SLOTS.\n",
								 slots, MAX_SLOTS);
			slots = MAX_SLOTS;
		}
	...
	if (this->nodeExists(node, "Jobs")) {
		...
		for (const auto& jobit : jobNode) {
			...
			if (!script_get_constant(jobName_constant.c_str(), &constant)) {
				this->invalidWarning(jobNode[jobit.key()],
									 "Invalid item job %s, defaulting to All.\n", jobName.c_str());
	...
	if (this->nodeExists(node, "Classes")) {
		...
		for (const auto& classit : classNode) {
			...
			if (!script_get_constant(className_constant.c_str(), &constant)) {
				this->invalidWarning(classNode[classit.key()],
									 "Invalid class upper %s, defaulting to All.\n",
									 className.c_str());
				item->class_upper |= ITEMJ_ALL;
				break;
			}
	...
	if (this->nodeExists(node, "Gender")) {
		...
		if (!script_get_constant(gender_constant.c_str(), &constant) || constant < SEX_FEMALE ||
			constant > SEX_BOTH) {
	...
	if (this->nodeExists(node, "Locations")) {
		...
		for (const auto& locit : locationNode) {
			...
			if (!script_get_constant(equipName_constant.c_str(), &constant)) {
				this->invalidWarning(locationNode[locit.key()],
			...
			if (active) {
				if (constant & EQP_SHADOW_GEAR && item->type != IT_SHADOWGEAR) {
					this->invalidWarning(node,
										 "Invalid item equip location %s as it's not a Shadow Gear "
										 "item type, defaulting to IT_ETC.\n",
										 equipName.c_str());
					item->type = IT_ETC;
				}
		...
		if (!exists) {
			if (itemdb_isequip2(item.get())) {
				this->invalidWarning(node,
									 "Invalid item equip location as it has no equip location, "
									 "defaulting to IT_ETC.\n");
				item->type = IT_ETC;
			} else
				item->equip = 0;
		}
	...
	if (this->nodeExists(node, "WeaponLevel")) {
		...
		if (item->type != IT_WEAPON) {
			this->invalidWarning(node["WeaponLevel"],
								 "Item type is not a weapon, defaulting to 0.\n");
			lv = 0;
		}
	...
	if (this->nodeExists(node, "ArmorLevel")) {
		...
		if (item->type != IT_ARMOR) {
			this->invalidWarning(node["ArmorLevel"],
								 "Item type is not an armor, defaulting to 0.\n");
			level = 0;
		}
	...
	if (this->nodeExists(node, "EquipLevelMin")) {
		...
		if (lv > MAX_LEVEL) {
			this->invalidWarning(
				node["EquipLevelMin"],
				"Minimum equip level %d exceeds MAX_LEVEL (%d), capping to MAX_LEVEL.\n", lv,
				MAX_LEVEL);
			lv = MAX_LEVEL;
		}
	...
	if (this->nodeExists(node, "EquipLevelMax")) {
		...
		if (lv < item->elv) {
			this->invalidWarning(node["EquipLevelMax"],
								 "Maximum equip level %d is less than minimum equip level %d, "
								 "capping to minimum equip level.\n",
								 lv, item->elv);
			lv = item->elv;
		}
		...
		if (lv > MAX_LEVEL) {
			this->invalidWarning(
				node["EquipLevelMax"],
				"Maximum equip level %d exceeds MAX_LEVEL (%d), capping to MAX_LEVEL.\n", lv,
				MAX_LEVEL);
			lv = MAX_LEVEL;
		}
	...
	if (this->nodeExists(node, "AliasName")) {
		...
		if (view_data == nullptr) {
			this->invalidWarning(node["AliasName"],
								 "Unable to change the alias because %s is an unknown item.\n",
								 view.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "Flags")) {
		...
		if (this->nodeExists(flagNode, "BuyingStore")) {
			...
			if (!itemdb_isstackable2(item.get()) && active) {
				this->invalidWarning(flagNode["BuyingStore"],
									 "Non-stackable item cannot be enabled for buying store.\n");
				active = false;
			}
		...
		if (this->nodeExists(flagNode, "UniqueId")) {
			...
			if (!itemdb_isstackable2(item.get()) && active) {
				this->invalidWarning(flagNode["UniqueId"],
									 "Non-stackable item cannot be enabled for UniqueId.\n");
				active = false;
			}
		...
		if (this->nodeExists(flagNode, "DropEffect")) {
			...
			if (!script_get_constant(effect_constant.c_str(), &constant) ||
				constant < DROPEFFECT_NONE || constant > DROPEFFECT_MAX) {
				this->invalidWarning(
					flagNode["DropEffect"],
					"Invalid item drop effect %s, defaulting to DROPEFFECT_NONE.\n",
					effect.c_str());
	...
	if (this->nodeExists(node, "Delay")) {
		...
		if (this->nodeExists(delayNode, "Status")) {
			...
			if (!script_get_constant(status_constant.c_str(), &constant) || constant < SC_NONE ||
				constant >= SC_MAX) {
				this->invalidWarning(delayNode[c4::to_csubstr(status)],
									 "Invalid item delay status %s, defaulting to SC_NONE.\n",
									 status.c_str());
				constant = SC_NONE;
			}
	...
	if (this->nodeExists(node, "Stack")) {
		...
		if (this->nodeExists(stackNode, "Amount")) {
			...
			if (!itemdb_isstackable2(item.get())) {
				this->invalidWarning(stackNode["Amount"],
									 "Non-stackable item cannot be enabled for stacking.\n");
				amount = 0;
			}
	...
	if (this->nodeExists(node, "NoUse")) {
		...
		if (this->nodeExists(nouseNode, "Override")) {
			...
			if (override > 100) {
				this->invalidWarning(nouseNode["Override"],
									 "Item no use override level %d exceeds 100, capping to 100.\n",
									 override);
				override = 100;
			}
	...
	if (this->nodeExists(node, "Trade")) {
		...
		if (this->nodeExists(tradeNode, "Override")) {
			...
			if (override > 100) {
				this->invalidWarning(tradeNode["Override"],
									 "Item trade override level %d exceeds 100, capping to 100.\n",
									 override);
				override = 100;
			}
	...
	if (this->nodeExists(node, "EquipScript")) {
		...
		item->equip_script =
			parse_script(script.c_str(), this->getCurrentFile().c_str(),
						 this->getLineNumber(node["EquipScript"]), SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	} else {
		if (!exists)
			item->equip_script = nullptr;
	}
	...
	if (this->nodeExists(node, "UnEquipScript")) {
		...
		item->unequip_script = parse_script(script.c_str(), this->getCurrentFile().c_str(),
											this->getLineNumber(node["UnEquipScript"]),
...
void ItemDatabase::loadingFinished() {
	for (auto& tmp_item : item_db) {
		...
		if (item->type == IT_DELAYCONSUME) {
			...
			item->flag.delay_consume &=
				~DELAYCONSUME_TEMP;	 // Remove delayed consumption flag if switching types
		}

		if (item->type == IT_WEAPON) {
			if (item->weapon_level == 0) {
				ShowWarning(
					"Item %s is a weapon, but does not have a weapon level. Consider adding it. "
					"Defaulting to 1.\n",
					item->name.c_str());
				item->weapon_level = 1;
			}
			...
			if (item->armor_level == 0) {
				ShowWarning(
					"Item %s is an armor, but does not have an armor level. Consider adding it. "
					"Defaulting to 1.\n",
					item->name.c_str());
				item->armor_level = 1;
			}
		...
		if (item->value_buy / 124. < item->value_sell / 75.) {
			ShowWarning(
				"Buying/Selling [%d/%d] price of %s (%u) allows Zeny making exploit through "
		...
		// Shields need to have a view ID to be able to be recognized by ST_SHIELD check in
		// skill.cpp
...
e_sex ItemDatabase::defaultGender(const ryml::NodeRef& node, std::shared_ptr<item_data> id) {
	...
	if (id->type == IT_WEAPON) {
		if (id->subtype == W_MUSICAL) {
			if (id->sex != SEX_MALE) {
				this->invalidWarning(
					node, "Musical instruments are always male-only, defaulting to SEX_MALE.\n");
			}
		...
		if (id->subtype == W_WHIP) {
			if (id->sex != SEX_FEMALE) {
				this->invalidWarning(node,
									 "Whips are always female-only, defaulting to SEX_FEMALE.\n");
			}
...
uint64 LaphineSynthesisDatabase::parseBodyNode(const ryml::NodeRef& node) {
		...
		for (const ryml::NodeRef& requirementNode : node["Requirements"]) {
			...
			if (this->nodeExists(requirementNode, "Amount")) {
				...
				if (amount > MAX_AMOUNT) {
					this->invalidWarning(requirementNode["Amount"],
	...
	if (this->nodeExists(node, "MinimumRefine")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["MinimumRefine"],
	...
	if (this->nodeExists(node, "MaximumRefine")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["MaximumRefine"],
								 "Maximum refine %hu is too high, capping to MAX_REFINE...\n",
								 refine);
			refine = MAX_REFINE;
		}
...
uint64 LaphineUpgradeDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "ResultRefine")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["ResultRefine"],
								 "Result refine %hu is too high, capping to MAX_REFINE...\n",
								 refine);
			refine = MAX_REFINE;
		}
	...
	if (this->nodeExists(node, "ResultRefineMinimum")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(
				node["ResultRefineMinimum"],
				"Result refine minimum %hu is too high, capping to MAX_REFINE...\n", refine);
			refine = MAX_REFINE;
		}
	...
	if (this->nodeExists(node, "ResultRefineMaximum")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(
				node["ResultRefineMaximum"],
				"Result refine maximum %hu is too high, capping to MAX_REFINE...\n", refine);
			refine = MAX_REFINE;
		}
	...
	if (this->nodeExists(node, "BaseItems")) {
		for (const ryml::NodeRef& baseNode : node["BaseItems"]) {
			...
			std::shared_ptr<s_item_reform_base> base =
				util::umap_find(entry->base_items, base_itemid);
			...
			if (this->nodeExists(baseNode, "RequiredRandomOptions")) {
				...
				if (amount > MAX_ITEM_RDM_OPT) {
					this->invalidWarning(baseNode["RequiredRandomOptions"],
										 "Required random option amount %hu is too high, capping "
										 "to MAX_ITEM_RDM_OPT...\n",
										 amount);
					amount = MAX_ITEM_RDM_OPT;
				}
			...
			if (this->nodeExists(baseNode, "ChangeRefine")) {
				...
				if (refine > MAX_REFINE) {
					this->invalidWarning(
						baseNode["MaximumRefine"],
						"Refine change %hu is too high, capping to MAX_REFINE...\n", refine);
					refine = MAX_REFINE;
				} else if (refine < -MAX_REFINE) {
					this->invalidWarning(
						baseNode["MaximumRefine"],
						"Refine change %hu is too low, capping to -MAX_REFINE...\n", refine);
					refine = -MAX_REFINE;
				}
			...
			if (this->nodeExists(baseNode, "RandomOptionGroup")) {
				...
				if (!random_option_group.option_get_id(name, id)) {
					this->invalidWarning(baseNode["RandomOptionGroup"],
										 "Unknown random option group \"%s\".\n", name.c_str());
					return 0;
				}
...
bool ItemEnchantDatabase::parseMaterials(const ryml::NodeRef& node,
	...
	if (this->nodeExists(node, "Materials")) {
		for (const ryml::NodeRef& materialNode : node["Materials"]) {
			...
			if (this->nodeExists(materialNode, "Amount")) {
				...
				if (amount > MAX_AMOUNT) {
					this->invalidWarning(materialNode["Amount"],
										 "Amount %hu is too high, capping to MAX_AMOUNT...\n",
										 amount);
					amount = MAX_AMOUNT;
				}
...
uint64 ItemEnchantDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "MinimumRefine")) {
		...
		if (refine > MAX_REFINE) {
			this->invalidWarning(node["MinimumRefine"],
								 "Minimum refine %hu exceeds MAX_REFINE. Capping...\n", refine);
			refine = MAX_REFINE;
		}
	...
	if (this->nodeExists(node, "MinimumEnchantgrade")) {
		...
		if (enchantgrade > MAX_ENCHANTGRADE) {
			this->invalidWarning(node["MinimumEnchantgrade"],
								 "Minimum enchantgrade %hu exceeds MAX_ENCHANTGRADE. Capping...\n",
								 enchantgrade);
	...
	if (this->nodeExists(node, "Slots")) {
		for (const ryml::NodeRef& slotNode : node["Slots"]) {
			...
			std::shared_ptr<s_item_enchant_slot> enchant_slot =
				util::umap_find(enchant->slots, slot);
			...
			if (this->nodeExists(slotNode, "Price")) {
				...
				if (zeny > MAX_ZENY) {
					this->invalidWarning(slotNode["Price"],
										 "Price %u exceeds MAX_ZENY. Capping...\n", zeny);
					zeny = MAX_ZENY;
				}
			...
			if (this->nodeExists(slotNode, "EnchantgradeBonus")) {
				for (const ryml::NodeRef& enchantgradeNode : slotNode["EnchantgradeBonus"]) {
					...
					if (enchantgrade > MAX_ENCHANTGRADE) {
						this->invalidWarning(enchantgradeNode["Enchantgrade"],
											 "Enchant grade %hu exceeds MAX_ENCHANTGRADE.\n",
											 enchantgrade);
						return 0;
					}
			...
			if (this->nodeExists(slotNode, "Enchants")) {
				for (const ryml::NodeRef& enchantNode : slotNode["Enchants"]) {
					...
					if (enchantgrade > MAX_ENCHANTGRADE) {
						this->invalidWarning(enchantNode["Enchantgrade"],
											 "Enchant grade %hu exceeds MAX_ENCHANTGRADE...\n",
											 enchantgrade);
						return 0;
					}
					...
					if (this->nodeExists(enchantNode, "Items")) {
						for (const ryml::NodeRef& itemNode : enchantNode["Items"]) {
							...
							std::shared_ptr<item_data> enchant_item =
								item_db.search_aegisname(enchant_name.c_str());
							...
							std::shared_ptr<s_item_enchant_normal_sub> enchant = util::umap_find(
								enchants_for_enchantgrade->enchants, enchant_item->nameid);
			...
			if (this->nodeExists(slotNode, "PerfectEnchants")) {
				for (const ryml::NodeRef& enchantNode : slotNode["PerfectEnchants"]) {
					...
					if (this->nodeExists(enchantNode, "Price")) {
						...
						if (zeny > MAX_ZENY) {
							this->invalidWarning(enchantNode["Price"],
												 "Price %u exceeds MAX_ZENY. Capping...\n", zeny);
							zeny = MAX_ZENY;
						}
			...
			if (this->nodeExists(slotNode, "Upgrades")) {
				for (const ryml::NodeRef& upgradeNode : slotNode["Upgrades"]) {
					...
					if (this->nodeExists(upgradeNode, "Upgrade")) {
						...
						std::shared_ptr<item_data> upgrade_item =
							item_db.search_aegisname(upgrade_name.c_str());
					...
					if (this->nodeExists(upgradeNode, "Price")) {
						...
						if (zeny > MAX_ZENY) {
							this->invalidWarning(upgradeNode["Price"],
												 "Price %u exceeds MAX_ZENY. Capping...\n", zeny);
							zeny = MAX_ZENY;
						}
					...
					if (!enchant_upgrade_exists) {
						enchant_slot->upgrade.enchants[enchant_upgrade->enchant_item_id] =
							enchant_upgrade;
					}
				}
			}
...
uint64 ItemPackageDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Groups")) {
		for (const ryml::NodeRef& groupNode : node["Groups"]) {
			...
			std::shared_ptr<s_item_package_group> group =
				util::umap_find(entry->groups, groupIndex);
			...
			for (const ryml::NodeRef& itemNode : groupNode["Items"]) {
				...
				std::shared_ptr<s_item_package_item> package_item =
				...
				if (this->nodeExists(itemNode, "Amount")) {
					...
					if (amount > MAX_AMOUNT) {
						this->invalidWarning(itemNode["Amount"],
											 "Amount %hu is too high, capping to MAX_AMOUNT...\n",
											 amount);
						amount = MAX_AMOUNT;
					} else if (amount == 0) {
						if (!package_item_exists) {
							this->invalidWarning(itemNode["Amount"],
												 "Trying to remove non existant item \"%s\".\n",
												 name.c_str());
							return 0;
						} else {
				...
				if (this->nodeExists(itemNode, "Refine")) {
					...
					if (refine > MAX_REFINE) {
						this->invalidWarning(itemNode["Refine"],
				...
				if (this->nodeExists(itemNode, "RandomOptionGroup")) {
					...
					if (!random_option_group.option_get_id(name, option_group_id)) {
						this->invalidWarning(itemNode["RandomOptionGroup"],
											 "Unknown random option group \"%s\".\n", name.c_str());
						return 0;
					}
 ...
 *------------------------------------------*/
uint16 itemdb_searchname_array(std::map<t_itemid, std::shared_ptr<item_data>>& data, uint16 size,
							   const char* str) {
...
std::shared_ptr<s_item_group_entry> get_random_itemsubgroup(
	std::shared_ptr<s_item_group_random> random) {
	if (random == nullptr)
		return nullptr;
	...
	for (size_t j = 0, max = random->data.size() * 3; j < max; j++) {
		...
		if (entry->rate == 0 || rnd() % random->total_rate <
									entry->rate)  // always return entry for rate 0 ('must' item)
			return entry;
	}
 ...
 */
std::shared_ptr<s_item_group_entry> ItemGroupDatabase::get_random_entry(uint16 group_id,
																		uint8 sub_group) {
	std::shared_ptr<s_item_group_db> group = this->find(group_id);
 ...
 */
static void itemdb_pc_get_itemgroup_sub(map_session_data* sd, bool identify,
										std::shared_ptr<s_item_group_entry> data) {
	if (data == nullptr)
		return;
	...
	for (uint16 i = 0; i < data->amount; i += get_amt) {
		...
		if ((flag = pc_additem(sd, &tmp, get_amt, LOG_TYPE_SCRIPT))) {
			...
			intif_broadcast_obtain_special_item(sd, data->nameid, sd->itemid,
												ITEMOBTAIN_TYPE_BOXITEM);
	}
}
...
uint8 ItemGroupDatabase::pc_get_itemgroup(uint16 group_id, bool identify, map_session_data* sd) {
	...
	if (random != nullptr && !random->data.empty()) {
		for (const auto& it : random->data) itemdb_pc_get_itemgroup_sub(sd, identify, it.second);
	}
 ...
 */
std::shared_ptr<item_data> itemdb_exists(t_itemid nameid) { return item_db.find(nameid); }
...
bool itemdb_cantrade_sub(struct item_data* item, int gmlv, int gmlv2) {
	return (item && (!(item->flag.trade_restriction.trade) || gmlv >= item->gm_lv_trade_override ||
					 gmlv2 >= item->gm_lv_trade_override));
}

bool itemdb_canpartnertrade_sub(struct item_data* item, int gmlv, int gmlv2) {
	return (item && (item->flag.trade_restriction.trade_partner ||
					 gmlv >= item->gm_lv_trade_override || gmlv2 >= item->gm_lv_trade_override));
}
...
bool itemdb_canstore_sub(struct item_data* item, int gmlv, int unused) {
	return (item &&
			(!(item->flag.trade_restriction.storage) || gmlv >= item->gm_lv_trade_override));
}

bool itemdb_canguildstore_sub(struct item_data* item, int gmlv, int unused) {
	return (item &&
			(!(item->flag.trade_restriction.guild_storage) || gmlv >= item->gm_lv_trade_override));
}
...
bool itemdb_canauction_sub(struct item_data* item, int gmlv, int unused) {
	return (item &&
			(!(item->flag.trade_restriction.auction) || gmlv >= item->gm_lv_trade_override));
}

bool itemdb_isrestricted(struct item* item, int gmlv, int gmlv2,
						 bool (*func)(struct item_data*, int, int)) {
...
uint64 ItemGroupDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (!script_get_constant(group_name_constant.c_str(), &constant) || constant < IG_BLUEBOX) {
		if (strncasecmp(group_name.c_str(), "IG_", 3) != 0)
			...
			this->invalidWarning(
				node["Group"],
				"Invalid group %s. Note that 'IG_' is automatically appended to the group name.\n",
				group_name.c_str());
		return 0;
	}
		...
		for (const auto& subit : subNode) {
			if (this->nodeExists(subit, "Clear")) {
				...
				if (group->random.erase(id) == 0)
					this->invalidWarning(
						subit["Clear"],
						"The SubGroup %hu doesn't exist in the group %s. Clear failed.\n", id,
						group_name.c_str());
			...
			for (const auto& listit : listNode) {
				if (this->nodeExists(listit, "Clear")) {
					...
					if (random->data.erase(item->nameid) == 0)
						this->invalidWarning(listit["Clear"],
											 "Item %hu doesn't exist in the SubGroup %hu (group "
											 "%s). Clear failed.\n",
				...
				std::shared_ptr<s_item_group_entry> entry =
					util::umap_find(random->data, item->nameid);
				...
				if (subgroup == 0 && entry->rate > 0) {
					this->invalidWarning(listit["Item"],
										 "SubGroup 0 is reserved for item without Rate ('must' "
										 "item). Defaulting Rate to 0.\n");
					entry->rate = 0;
				}
				if (subgroup != 0 && entry->rate == 0) {
					this->invalidWarning(listit["Item"],
										 "Entry must have a Rate for group above 0, skipping.\n");
					continue;
				}
				...
				if (this->nodeExists(listit, "Bound")) {
					...
					if (!script_get_constant(bound_name_constant.c_str(), &bound) ||
						bound < BOUND_NONE || bound >= BOUND_MAX) {
				...
				if (this->nodeExists(listit, "RandomOptionGroup")) {
					...
					if (!random_option_group.option_get_id(name, id)) {
						this->invalidWarning(listit["RandomOptionGroup"],
											 "Unknown random option group \"%s\".\n", name.c_str());
						return 0;
					}
...
uint64 ComboDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	for (const auto& comboit : combosNode) {
		...
		if (items.size() < 2) {
			this->invalidWarning(comboNode,
								 "Not enough item to make a combo (need at least 2). Skipping.\n");
			return 0;
		}
...
bool itemdb_parse_roulette_db(void) {
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) rd.items[i] = 0;
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
		...
		for (k = 0; k < limit && SQL_SUCCESS == Sql_NextRow(mmysql_handle); k++) {
			...
			if (amount < 1 || amount > MAX_AMOUNT) {
				ShowWarning(
					"itemdb_parse_roulette_db: Unsupported amount '%hu' for item ID '%u' in level "
					"'%d'\n",
			...
			if (flag < 0 || flag > 1) {
				ShowWarning(
					"itemdb_parse_roulette_db: Unsupported flag '%d' for item ID '%u' in level "
					"'%d'\n",
					flag, item_id, level);
				continue;
			}
	...
	for (i = 0; i < MAX_ROULETTE_LEVEL; i++) {
		...
		ShowWarning(
			"itemdb_parse_roulette_db: Level %d has %d items, %d are required. Filling with "
			"Apples...\n",
			i + 1, rd.items[i], limit);
	...
	for (uint8 fi = 0; fi < ARRAYLENGTH(item_db_name); ++fi) {
		...
		if (SQL_ERROR ==
			Sql_Query(mmysql_handle,
					  "SELECT "
					  "`id`,`name_aegis`,`name_english`,`type`,`subtype`,`price_buy`,`price_sell`,`"
					  "weight`,`attack`,`defense`,`range`,`slots`,"
					  "`job_all`,`job_acolyte`,`job_alchemist`,`job_archer`,`job_assassin`,`job_"
					  "barddancer`,`job_blacksmith`,`job_crusader`,`job_gunslinger`,`job_hunter`,`"
					  "job_knight`,`job_mage`,`job_merchant`,"
					  "`job_monk`,`job_ninja`,`job_novice`,`job_priest`,`job_rogue`,`job_sage`,`"
					  "job_soullinker`,`job_stargladiator`,`job_supernovice`,`job_swordman`,`job_"
					  "taekwon`,`job_thief`,`job_wizard`,"
					  "`class_all`,`class_normal`,`class_upper`,`class_baby`,`gender`,"
					  "`location_head_top`,`location_head_mid`,`location_head_low`,`location_armor`"
					  ",`location_right_hand`,`location_left_hand`,`location_garment`,`location_"
					  "shoes`,`location_right_accessory`,`location_left_accessory`,"
					  "`location_costume_head_top`,`location_costume_head_mid`,`location_costume_"
					  "head_low`,`location_costume_garment`,`location_ammo`,`location_shadow_armor`"
					  ",`location_shadow_weapon`,`location_shadow_shield`,`location_shadow_shoes`,`"
					  "location_shadow_right_accessory`,`location_shadow_left_accessory`,"
					  "`weapon_level`,`armor_level`,`equip_level_min`,`equip_level_max`,`"
					  "refineable`,`view`,`alias_name`,"
					  "`flag_buyingstore`,`flag_deadbranch`,`flag_container`,`flag_uniqueid`,`flag_"
					  "bindonequip`,`flag_dropannounce`,`flag_noconsume`,`flag_dropeffect`,"
					  "`delay_duration`,`delay_status`,`stack_amount`,`stack_inventory`,`stack_"
					  "cart`,`stack_storage`,`stack_guildstorage`,`nouse_override`,`nouse_sitting`,"
					  "`trade_override`,`trade_nodrop`,`trade_notrade`,`trade_tradepartner`,`trade_"
					  "nosell`,`trade_nocart`,`trade_nostorage`,`trade_noguildstorage`,`trade_"
					  "nomail`,`trade_noauction`,`script`,`equip_script`,`unequip_script`"
#ifdef RENEWAL
					  ",`magic_attack`,`class_third`,`class_third_upper`,`class_third_baby`,`class_"
					  "fourth`,`job_kagerouoboro`,`job_rebellion`,`job_summoner`,`job_spirit_"
					  "handler`,`gradable`"
#endif
					  " FROM `%s`",
					  item_db_name[fi])) {
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			ShowStatus("Loading [%" PRIu64 "/%" PRIu64 "] rows from '" CL_WHITE "%s" CL_RESET
					   "'" CL_CLL "\r",
					   ++rows, total_rows, item_db_name[fi]);
...
uint64 RandomOptionDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Option")) {
		...
		if (randopt->name.compare(name) != 0 && random_option_db.option_exists(name)) {
			this->invalidWarning(node["Option"],
								 "Found duplicate random option name for %s, skipping.\n",
								 name.c_str());
			return 0;
		}
...
bool RandomOptionGroupDatabase::add_option(const ryml::NodeRef& node,
	...
	if (entry->min_value > entry->max_value) {
		this->invalidWarning(
			node["MaxValue"],
			"MinValue %d is greater than MaxValue %d, setting MaxValue to MinValue + 1.\n",
...
void s_random_opt_group::apply(struct item& item) {
	auto apply_sub = [](s_item_randomoption& item_option,
						const std::shared_ptr<s_random_opt_group_entry>& option) {
	...
	for (size_t i = 0; i < this->slots.size(); i++) {
		...
		for (size_t j = 0, max = this->slots[static_cast<uint16>(i)].size() * 3; j < max; j++) {
			std::shared_ptr<s_random_opt_group_entry> option =
				util::vector_random(this->slots[static_cast<uint16>(i)]);
	...
	if (this->max_random > 0) {
		for (size_t i = 0; i < min(this->max_random, MAX_ITEM_RDM_OPT); i++) {
			...
			std::shared_ptr<s_random_opt_group_entry> option =
				util::vector_random(this->random_options);
...
uint64 RandomOptionGroupDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Group")) {
		...
		if (randopt->name.compare(name) != 0 && random_option_group.option_exists(name)) {
			this->invalidWarning(node["Group"],
								 "Found duplicate random option group name for %s, skipping.\n",
								 name.c_str());
			return 0;
		}
	...
	if (this->nodeExists(node, "Slots")) {
		...
		for (const ryml::NodeRef& slotNode : slotsNode) {
			if (randopt->slots.size() >= MAX_ITEM_RDM_OPT) {
				this->invalidWarning(slotNode,
									 "Reached maximum of %d Random Option group options. Skipping "
									 "the remaining slots...\n",
									 MAX_ITEM_RDM_OPT);
				break;
			}
			...
			if (slot < 1 || slot > MAX_ITEM_RDM_OPT) {
				this->invalidWarning(
					slotNode["Slot"],
					"Invalid Random Opton Slot number %hu given, must be between 1~%d, skipping.\n",
			...
			if (!this->nodeExists(slotNode, "Options")) {
				this->invalidWarning(
					slotNode, "Random option slot does not contain Options node, skipping.\n");
				return 0;
			}
	...
	if (this->nodeExists(node, "MaxRandom")) {
		...
		if (max > MAX_ITEM_RDM_OPT) {
			this->invalidWarning(node["MaxRandom"],
								 "Exceeds the maximum of %d Random Option group options, capping "
								 "to MAX_ITEM_RDM_OPT.\n",
								 MAX_ITEM_RDM_OPT);
 ...
 */
void do_init_itemdb(void) { itemdb_read(); }
