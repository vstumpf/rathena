...
#define CAPTCHA_BMP_SIZE \
	(2 + 52 + (3 * 220 * 90))  // sizeof("BM") + sizeof(BITMAPV2INFOHEADER) + 24bits 220x90 BMP
#endif
...
struct map_session_data {
	...
	// NOTE: When deciding to add a flag to state or special_state, take into consideration that
	// state is preserved in status_calc_pc, while special_state is recalculated in each call.
	// [Skotlex]
	struct s_state {
		unsigned int active : 1;  // Marks active player (not active is logging in/out, or changing
								  // map servers)
		...
		unsigned int rest : 1;
		unsigned int storage_flag : 3;	// 0: closed, 1: Normal Storage open, 2: guild storage open
										// [Skotlex], 3: Premium Storage
		unsigned int snovice_dead_flag : 1;	 // Explosion spirits on death: 0 off, 1 used.
		unsigned int abra_flag : 2;			 // Abracadabra bugfix by Aru
		unsigned int autocast : 1;			 // Autospell flag [Inkfish]
		unsigned int autotrade : 3;	 // By Fantik. &2 Requested by vending autotrade; &4 Requested
									 // by buyingstore autotrade
		...
		unsigned int
			debug_remove_map : 1;  // temporary state to track double remove_map's [FlavioJS]
		...
		unsigned int
			prevend : 1;  // used to flag wheather you've spent 40sp to open the vending or not.
		unsigned int warping : 1;  // states whether you're in the middle of a warp processing
		unsigned int
			permanent_speed : 1;  // When 1, speed cannot be changed through status_calc_pc().
		bool hold_recalc;
		unsigned int banking : 1;  // 1 when we using the banking system 0 when closed
		unsigned int hpmeter_visible : 1;
		unsigned disable_atcommand_on_npc : 1;	// Prevent to use atcommand while talking with NPC
												// [Kichi]
		...
		bool pc_loaded;		// Ensure inventory data and status data is loaded before we calculate
							// player stats
	...
	uint64 class_;	// This is the internal job ID used by the map server to simplify
					// comparisons/queries/etc. [Skotlex]
	int group_id;
	std::shared_ptr<s_player_group> group;
	std::bitset<PC_PERM_MAX> permissions;  // group permissions have to be copied, because they
										   // might be adjusted by atcommand addperm
	...
	struct item_data *inventory_data[MAX_INVENTORY];  // direct pointers to itemdb entries (faster
													  // than doing item_id lookups)
	...
	std::vector<s_item_bonus> skillatk, skillusesprate, skillusesp, skillheal, skillheal2,
		skillblown, skillcastrate, skillfixcastrate, subskill, skillcooldown, skillfixcast,
		skillvarcast, skilldelay, itemhealrate, add_def, add_mdef, add_mdmg, reseff,
		itemgrouphealrate, itemsphealrate, itemgroupsphealrate;
	...
	struct s_bonus {
		...
		unsigned int setitem_hash, setitem_hash2;  // Split in 2 because shift operations only work
												   // on int ranges. [Skotlex]

		short splash_range, splash_add_range;
		short add_steal_rate;
		int add_heal_rate, add_heal2_rate;
		int sp_gain_value, hp_gain_value, magic_sp_gain_value, magic_hp_gain_value,
			long_sp_gain_value, long_hp_gain_value;
	...
	int stellar_mark[MAX_STELLAR_MARKS];   // Stores the account ID's of character's with a stellar
										   // mark.
	int united_soul[MAX_UNITED_SOULS];	   // Stores the account ID's of character's who's soul is
										   // united.
	int servant_sign[MAX_SERVANT_SIGN];	   // Stores the account ID's of character's with a servant
										   // sign.
	...
	int party_invite,
		party_invite_account;  // for handling party invitation (holds party id and account id)
	...
	uint16 change_level_2nd;  // job level when changing from 1st to 2nd class [jobchange_level in
							  // global_reg_value]
	uint16 change_level_3rd;  // job level when changing from 2nd to 3rd class [jobchange_level_3rd
							  // in global_reg_value]
	uint16 change_level_4th;  // job level when changing from 3rd to 4th class [jobchange_level_4rd
							  // in global_reg_value]
	...
	int avail_quests;  ///< Number of Q_ACTIVE and Q_INACTIVE entries in quest log (index of the
					   ///< first Q_COMPLETE entry)
	struct quest *quest_log;  ///< Quest log entries (note: Q_COMPLETE quests follow the first
							  ///< <avail_quests>th enties
	...
	struct s_bonus_script_list {
		struct linkdb_node *head;  ///< Bonus script head node. data: struct s_bonus_script_entry
								   ///< *entry, key: (intptr_t)entry
		uint16 count;
	} bonus_script;
	...
	int instance_id;
	e_instance_mode instance_mode;	///< Mode of instance player last leaves from (used for instance
									///< destruction button)
...
#define EQP_COSTUME \
	(EQP_COSTUME_HEAD_TOP | EQP_COSTUME_HEAD_MID | EQP_COSTUME_HEAD_LOW | EQP_COSTUME_GARMENT)
#define EQP_COSTUME_HELM (EQP_COSTUME_HEAD_TOP | EQP_COSTUME_HEAD_MID | EQP_COSTUME_HEAD_LOW)
#define EQP_SHADOW_GEAR                                                            \
	(EQP_SHADOW_ARMOR | EQP_SHADOW_WEAPON | EQP_SHADOW_SHIELD | EQP_SHADOW_SHOES | \
	 EQP_SHADOW_ACC_R | EQP_SHADOW_ACC_L)
...
#define pc_istrading(sd) \
	((sd)->npc_id || (sd)->state.vending || (sd)->state.buyingstore || (sd)->state.trading)
static bool pc_cant_act2(struct map_session_data *sd) {
	return sd->state.vending || sd->state.buyingstore ||
		   (sd->sc.opt1 && sd->sc.opt1 != OPT1_BURNING) || sd->state.trading ||
		   sd->state.storage_flag || sd->state.prevend || sd->state.refineui_open ||
...
#define pc_iscloaking(sd) \
	(!((sd)->sc.option & OPTION_CHASEWALK) && ((sd)->sc.option & OPTION_CLOAK))
...
#define pc_is50overweight(sd) \
	((sd)->weight * 100 >= (sd)->max_weight * battle_config.natural_heal_weight_rate)
#define pc_is70overweight(sd) \
	((sd)->weight * 100 >= (sd)->max_weight * battle_config.natural_heal_weight_rate_renewal)
...
#define pcdb_checkid_sub(class_)                                                        \
	(((class_) < JOB_MAX_BASIC) ||                                                      \
	 ...
	 ((class_) >= JOB_KAGEROU && (class_) <= JOB_OBORO) || (class_) == JOB_REBELLION || \
	 (class_) == JOB_SUMMONER || (class_) == JOB_BABY_SUMMONER ||                       \
...
#define pc_rightside_atk(sd) \
	((sd)->battle_status.watk + (sd)->battle_status.watk2 + (sd)->battle_status.eatk)
...
enum e_additem_result pc_cart_additem(struct map_session_data *sd, struct item *item_data,
									  int amount, e_log_pick_type log_type);
...
bool pc_addautobonus(std::vector<std::shared_ptr<s_autobonus>> &bonus, const char *script,
					 short rate, unsigned int dur, uint16 atk_type, const char *o_script,
...
enum e_addskill_type {
	...
	ADDSKILL_TEMP =
		1,	///< Temporary skill. If player learned the skill and the given level is higher, level
			///< will be replaced and learned level will be palced in skill flag. `flag = learned +
			///< SKILL_FLAG_REPLACED_LV_0; learned_level = level;`
	ADDSKILL_TEMP_ADDLEVEL =
		2,	///< Like PCSKILL_TEMP, except the level will be stacked. `learned_level += level`. The
...
uint16 pc_level_penalty_mod(struct map_session_data *sd, e_penalty_type type,
							std::shared_ptr<s_mob_db> mob, mob_data *md = nullptr);
#endif
