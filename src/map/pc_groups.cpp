...
bool PlayerGroupDatabase::parseCommands(const ryml::NodeRef& node,
	...
	for (const auto& it : node.children()) {
		...
		if (allowed) {
			if (util::vector_exists(commands, command)) {
				this->invalidWarning(it,
									 "Group already has command \"%s\". Please check your data.\n",
									 command.c_str());
				return false;
			} else {
...
uint64 PlayerGroupDatabase::parseBodyNode(const ryml::NodeRef& node) {
	...
	if (this->nodeExists(node, "Commands") &&
		!this->parseCommands(node["Commands"], group->commands)) {
		return 0;
	}

	if (this->nodeExists(node, "CharCommands") &&
		!this->parseCommands(node["CharCommands"], group->char_commands)) {
		return 0;
	}
	...
	if (this->nodeExists(node, "Inherit")) {
		...
		for (const auto& it : inherits) {
			...
			if (enable) {
				...
				if (!util::vector_exists(inheritanceVector, inherit)) {
					this->invalidWarning(it,
										 "Trying to remove inheritance of non-inherited group %s\n",
										 inherit.c_str());
					return 0;
				}
				...
				for (const auto& it : *this) {
					...
					if (otherName == otherGroupName) {
						...
						auto* otherGroupInheritance =
							util::map_find(this->inheritance, otherGroup->id);
...
void pc_group_pc_load(struct map_session_data* sd) {
	...
	if (group == nullptr) {
		ShowWarning(
			...
			sd->status.name, sd->status.account_id, sd->group_id);
		set_eof(sd->fd);
		return;
	}
 ...
 */
bool s_player_group::should_log_commands() { return this->log_commands; }

/**
 * Initialize PC Groups and read config.
 */
void do_init_pc_groups(void) { player_group_db.load(); }

/**
 * Finalize PC Groups
 */
void do_final_pc_groups(void) { player_group_db.clear(); }
...
void pc_groups_reload(void) {
	...
	for (struct map_session_data* sd = (struct map_session_data*)mapit_first(iter);
		 mapit_exists(iter); sd = (struct map_session_data*)mapit_next(iter)) {
		pc_group_pc_load(sd);
	}
	mapit_free(iter);
}
