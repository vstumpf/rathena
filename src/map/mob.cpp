...
#define ACTIVE_AI_RANGE \
	2  // Distance added on top of 'AREA_SIZE' at which mobs enter active AI mode.

#define IDLE_SKILL_INTERVAL \
	10	// Active idle skills should be triggered every 1 second (1000/MIN_MOBTHINKTIME)
...
static bool mobdb_searchname_sub(uint16 mob_id, const char *const str, bool full_cmp) {
	...
	if (!mob->base_exp && !mob->job_exp && !mob_has_spawn(mob_id))
		return false;  // Monsters with no base/job exp and no spawn point are, by this criteria,
					   // considered "slave mobs" and excluded from search results
	if (full_cmp) {
		...
		if (stristr(mob->name.c_str(), str) != NULL || stristr(mob->jname.c_str(), str) != NULL ||
...
uint16 mobdb_searchname(const char *const str) { return mobdb_searchname_(str, true); }
...
int mobdb_checkid(const int id) {
	if (!mob_db.exists(id))
		return 0;
	if (mob_is_clone(id))  // checkid is used mostly for random ID based code, therefore clone mobs
...
struct mob_data *mob_spawn_dataset(struct spawn_data *data) {
	...
	if (status_has_mode(&md->db->status, MD_LOOTER))
		md->lootitems =
			(struct s_mob_lootitem *)aCalloc(LOOTITEM_SIZE, sizeof(struct s_mob_lootitem));
	...
	for (size_t i = 0, max = summon->list.size() * 3; i < max; i++) {
		...
		if (mob == nullptr || mob_is_clone(entry->mob_id) ||
			...
			(flag & RMF_MOB_NOT_PLANT &&
			 status_has_mode(&mob->status,
							 MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED | MD_IGNOREMISC))) {
			continue;
		}
...
bool mob_ksprotected(struct block_list *src, struct block_list *target) {
	...
	do {
		...
		if (sd->bl.id == sce->val1 ||  // Same Owner
			(sce->val2 == 2 && sd->status.party_id &&
			 sd->status.party_id == sce->val3) ||  // Party KS allowed
			(sce->val2 == 3 && sd->status.guild_id &&
	...
	status_change_start(src, target, SC_KSPROTECTED, 10000, sd->bl.id, sd->state.noks,
						sd->status.party_id, sd->status.guild_id, battle_config.ksprotection,
						SCSTART_NOAVOID);

	return false;
}

struct mob_data *mob_once_spawn_sub(struct block_list *bl, int16 m, int16 x, int16 y,
									const char *mobname, int mob_id, const char *event,
									unsigned int size, enum mob_ai ai) {
	struct spawn_data data;

	memset(&data, 0,
		   sizeof(struct spawn_data));	// why ? this might screw attribute object and cause leak..
	data.m = m;
	data.num = 1;
	data.id = mob_id;
 ...
 *------------------------------------------*/
int mob_once_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *mobname,
				   int mob_id, int amount, const char *event, unsigned int size, enum mob_ai ai) {
	...
	for (count = 0; count < amount; count++) {
		...
		if (mob_id == MOBID_EMPERIUM) {
			...
			if (gc) {
				md->guardian_data =
					(struct guardian_data *)aCalloc(1, sizeof(struct guardian_data));
				...
				if (g) {
					...
					add_timer(gettick() + 5000, mob_spawn_guardian_sub, md->bl.id,
 ...
 *------------------------------------------*/
int mob_once_spawn_area(struct map_session_data *sd, int16 m, int16 x0, int16 y0, int16 x1,
						int16 y1, const char *mobname, int mob_id, int amount, const char *event,
						unsigned int size, enum mob_ai ai) {
	int i, max, id = 0;
	int lx = -1, ly = -1;
...
static TIMER_FUNC(mob_spawn_guardian_sub) {
	...
	if (bl == nullptr)	// It is possible mob was already removed from map when the castle has no
						// owner. [Skotlex]
		return 0;
	...
	if (g == nullptr) {	 // Liberate castle, if the guild is not found this is an error! [Skotlex]
		...
		if (md->mob_id == MOBID_EMPERIUM) {	 // Not sure this is the best way, but otherwise we'd be
											 // invoking this for ALL guardians spawned later on.
			...
			if (md->guardian_data->number >= 0 && md->guardian_data->number < MAX_GUARDIANS &&
				...
				guild_castledatasave(md->guardian_data->castle->castle_id,
									 CD_ENABLED_GUARDIAN00 + md->guardian_data->number, 0);
 ...
 *------------------------------------------*/
int mob_spawn_guardian(const char *mapname, int16 x, int16 y, const char *mobname, int mob_id,
					   const char *event, int guardian, bool has_index) {
	struct mob_data *md = nullptr;
	struct spawn_data data;
	...
	if ((x <= 0 || y <= 0) && !map_search_freecell(NULL, m, &x, &y, -1, -1, 1)) {
		ShowWarning(
			"mob_spawn_guardian: Couldn't locate a spawn cell for guardian class %d (index %d) at "
			"castle map %s\n",
	...
	if (has_index &&
		gc->guardian[guardian].id) {  // Check if guardian already exists, refuse to spawn if so.
		struct mob_data *md2 = (TBL_MOB *)map_id2bl(gc->guardian[guardian].id);
		if (md2 && md2->bl.type == BL_MOB && md2->guardian_data &&
			md2->guardian_data->number == guardian) {
			ShowError(
				"mob_spawn_guardian: Attempted to spawn guardian in position %d which already has "
				"a guardian (castle map %s)\n",
				guardian, mapname);
			return 0;
		}
	}
 ...
 *------------------------------------------*/
int mob_spawn_bg(const char *mapname, int16 x, int16 y, const char *mobname, int mob_id,
				 const char *event, unsigned int bg_id) {
	struct mob_data *md = nullptr;
	struct spawn_data data;
	int16 m;
	...
	if ((x <= 0 || y <= 0) && !map_search_freecell(NULL, m, &x, &y, -1, -1, 1)) {
		ShowWarning(
			"mob_spawn_bg: Couldn't locate a spawn cell for guardian class %d (bg_id %d) at map "
			"%s\n",
...
int mob_setdelayspawn(struct mob_data *md) {
	...
	} else if (status_has_mode(&db->status, MD_IGNOREMELEE | MD_IGNOREMAGIC | MD_IGNORERANGED |
												MD_IGNOREMISC)) {  // Plant type
...
int mob_spawn(struct mob_data *md) {
	...
	if (md->spawn) {  // Respawn data
		...
		if ((md->bl.x == 0 && md->bl.y == 0) || md->spawn->xs ||
			md->spawn->ys) {  // Monster can be spawned on an area.
			if (!map_search_freecell(
					&md->bl, -1, &md->bl.x, &md->bl.y, md->spawn->xs, md->spawn->ys,
					battle_config.no_spawn_on_player ? 4 : 0)) {  // retry again later
				if (md->spawn_timer != INVALID_TIMER)
					delete_timer(md->spawn_timer, mob_delayspawn);
				...
				return 1;
				   ...
				   map_foreachinallrange(mob_count_sub, &md->bl, AREA_SIZE,
										 BL_PC)) {	// retry again later (players on sight)
			if (md->spawn_timer != INVALID_TIMER)
				delete_timer(md->spawn_timer, mob_delayspawn);
			md->spawn_timer = add_timer(tick + 5000, mob_delayspawn, md->bl.id, 0);
			return 1;
	...
	for (i = 0; i < MAX_MOBSKILL; i++) md->skilldelay[i] = c;
	for (i = 0; i < DAMAGELOG_SIZE; i++) md->spotted_log[i] = 0;
	...
	switch (md->state.skillstate) {
		case MSS_BERSERK:
			...
			if (!(battle_config.mob_ai & 0x80) && md->norm_attacked_id != target->id)
				return 0;
			return (battle_config.mob_ai & 0x4 ||
					check_distance_bl(&md->bl, target, md->status.rhw.range + 1));
		case MSS_RUSH:
...
int mob_target(struct mob_data *md, struct block_list *bl, int dist) {
	...
	md->min_chase =
		cap_value(dist + md->db->range3 - md->status.rhw.range, md->db->range3, MAX_MINCHASE);
	return 0;
}
...
static int mob_ai_sub_hard_activesearch(struct block_list *bl, va_list ap) {
	...
	switch (bl->type) {
		...
		default:
			if (battle_config.hom_setting & HOMSET_FIRST_TARGET && (*target) &&
				(*target)->type == BL_HOM && bl->type != BL_HOM)
			...
			if (((*target) == NULL || !check_distance_bl(&md->bl, *target, dist)) &&
				...
				if (!path_search(&wpd, md->bl.m, md->bl.x, md->bl.y, bl->x, bl->y, 0,
								 CELL_CHKWALL))	 // Count walk path cells
					return 0;
				...
				(*target) = bl;
				md->target_id = bl->id;
				md->min_chase = cap_value(dist + md->db->range3 - md->status.rhw.range,
										  md->db->range3, MAX_MINCHASE);
				return 1;
			}
			break;
	}
	...
	if (mob_can_reach(md, bl, md->db->range3) &&
		((*target) == nullptr ||
		 ...
		 (!battle_config.monster_loot_search_type &&
		  !check_distance_bl(&md->bl, *target, dist))  // New target closer than previous one.
		 )) {
		(*target) = bl;
		md->target_id = bl->id;
		md->min_chase = md->db->range3;
...
static int mob_ai_sub_hard_slavemob(struct mob_data *md, t_tick tick) {
	...
	if (status_has_mode(&md->status,
						MD_CANMOVE)) {	// If the mob can move, follow around. [Check by Skotlex]
		...
		if (battle_config.slave_stick_with_master || md->special_state.ai == AI_ABR ||
			...
			// Since the master was in near immediately before, teleport is carried out and it
			// pursues.
			if (bl->m != md->bl.m || (old_dist < 10 && md->master_dist > 18) ||
		...
		// Approach master if within view range, chase back to Master's area also if standing on top
		// of the master.
		if ((md->master_dist > MOB_SLAVEDISTANCE || md->master_dist == 0) &&
			unit_can_move(&md->bl)) {
			int16 x = bl->x, y = bl->y;

			if (map_search_freecell(&md->bl, bl->m, &x, &y, MOB_SLAVEDISTANCE, MOB_SLAVEDISTANCE,
									1)) {
				if (unit_walktoxy(&md->bl, x, y, 0) ==
					0) {  // Slave is too far from master (outside of battle_config.max_walk_path
						  // range), stay put
	...
	if (DIFF_TICK(tick, md->last_linktime) >= MIN_MOBLINKTIME && !md->target_id) {
		...
		if (ud) {
			...
			if (tbl && status_check_skilluse(&md->bl, tbl, 0, 0)) {
				md->target_id = tbl->id;
				md->min_chase =
					cap_value(distance_bl(&md->bl, tbl) + md->db->range3 - md->status.rhw.range,
							  md->db->range3, MAX_MINCHASE);
				return 1;
			}
		}
...
int mob_unlocktarget(struct mob_data *md, t_tick tick) {
	...
	switch (md->state.skillstate) {
		case MSS_WALK:
			...
			md->state.skillstate = MSS_IDLE;
		case MSS_IDLE:
			if (md->ud.walktimer == INVALID_TIMER && md->idle_event[0] &&
				npc_event_do_id(md->idle_event, md->bl.id) > 0) {
				md->idle_event[0] = 0;
				break;
			}
			// Idle skill.
			if (!(++md->ud.walk_count % IDLE_SKILL_INTERVAL) && mobskill_use(md, tick, -1))
				break;
			// Random walk.
			if (!md->master_id && DIFF_TICK(md->next_walktime, tick) <= 0 &&
	...
	if (!md->ud.state.ignore_cell_stack_limit && battle_config.official_cell_stack_limit > 0 &&
		...
		map_count_oncell(md->bl.m, md->bl.x, md->bl.y, BL_CHAR | BL_NPC, 1) >
...
int mob_randomwalk(struct mob_data *md, t_tick tick) {
	...
	r = rnd();
	rdir = rnd() % 4;  // Randomize direction in which we iterate to prevent monster cluttering up
					   // in one corner
	dx = r % (d * 2 + 1) - d;
	dy = r / (d * 2 + 1) % (d * 2 + 1) - d;
	max = (d * 2 + 1) * (d * 2 + 1);
		...
		if (battle_config.mob_stuck_warning) {
			...
			if (md->move_fail_count > 1000) {
				ShowWarning("MOB can't move. random spawn %d, class = %d, at %s (%d,%d)\n",
							md->bl.id, md->mob_id, map_getmapdata(md->bl.m)->name, md->bl.x,
							md->bl.y);
				md->move_fail_count = 0;
				mob_spawn(md);
			}
...
int mob_warpchase(struct mob_data *md, struct block_list *target) {
	...
	map_foreachinallrange(mob_warpchase_sub, &md->bl, md->db->range2, BL_NPC, target, &warp,
						  &distance);
...
static bool mob_ai_sub_hard(struct mob_data *md, t_tick tick) {
	...
	if (md->attacked_id && mode & MD_CANATTACK) {
				...
				(  // Can't attack back and can't reach back.
					(!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0 &&
					 (battle_config.mob_ai & 0x2 || md->sc.data[SC_SPIDERWEB] ||
					  md->sc.data[SC_BITE] || md->sc.data[SC_VACUUM_EXTREME] ||
					  md->sc.data[SC_THORNSTRAP] ||
					  md->sc.data[SC__MANHOLE]	// Not yet confirmed if boss will teleport once it
												// can't reach target.
		...
		} else if ((abl = map_id2bl(md->attacked_id)) &&
				   (!tbl || mob_can_changetarget(md, abl, mode))) {
			int dist;
			if (md->bl.m != abl->m || abl->prev == NULL ||
				(dist = distance_bl(&md->bl, abl)) >=
					MAX_MINCHASE  // Attacker longer than visual area
				||
				battle_check_target(&md->bl, abl, BCT_ENEMY) <= 0  // Attacker is not enemy of mob
				|| (battle_config.mob_ai & 0x2 &&
					!status_check_skilluse(&md->bl, abl, 0,
										   0))	// Cannot normal attack back to Attacker
				|| (!battle_check_range(&md->bl, abl,
										md->status.rhw.range)  // Not on Melee Range and ...
					&& (									   // Reach check
						   (!can_move && DIFF_TICK(tick, md->ud.canmove_tick) > 0 &&
							(battle_config.mob_ai & 0x2 || md->sc.data[SC_SPIDERWEB] ||
							 md->sc.data[SC_BITE] || md->sc.data[SC_VACUUM_EXTREME] ||
							 md->sc.data[SC_THORNSTRAP] ||
							 md->sc.data[SC__MANHOLE]  // Not yet confirmed if boss will teleport
													   // once it can't reach target.
							 || md->walktoxy_fail_count > 0)) ||
						   !mob_can_reach(md, abl, dist + md->db->range3)))) {	// Rude attacked
				if (abl->id != md->bl.id  // Self damage does not cause rude attack
					&& md->state.attacked_count++ >= RUDE_ATTACKED_COUNT &&
					!mobskill_use(md, tick, MSC_RUDEATTACKED) && can_move && !tbl &&
					unit_escape(&md->bl, abl, rnd() % 10 + 1)) {  // Escaped.
					// TODO: Maybe it shouldn't attempt to run if it has another, valid target?
					md->attacked_id = md->norm_attacked_id = 0;
					return true;
				}
			} else if (!(battle_config.mob_ai & 0x2) &&
					   !status_check_skilluse(&md->bl, abl, 0, 0)) {
	...
	if (!tbl) {	 // No targets available.
		...
		if (md->bg_id && mode & MD_CANATTACK) {
			...
			map_foreachinallrange(mob_ai_sub_hard_bg_ally, &md->bl, view_range, BL_PC, md, &tbl,
								  mode);
			if (tbl) {
				if (distance_blxy(&md->bl, tbl->x, tbl->y) <= 3 ||
					unit_walktobl(&md->bl, tbl, 1, 1))
	...
	if (tbl->type == BL_ITEM) {	 // Loot time.
		...
		if (md->lootitem_count < LOOTITEM_SIZE) {
			memcpy(&md->lootitems[md->lootitem_count].item, &fitem->item,
				   sizeof(md->lootitems[0].item));
			md->lootitems[md->lootitem_count].mob_id = fitem->mob_id;
			md->lootitem_count++;
		} else {  // Destroy first looted item...
			if (md->lootitems[0].item.card[0] == CARD0_PET)
				intif_delete_petdata(
					MakeDWord(md->lootitems[0].item.card[1], md->lootitems[0].item.card[2]));
			memmove(&md->lootitems[0], &md->lootitems[1],
					(LOOTITEM_SIZE - 1) * sizeof(md->lootitems[0]));
			memcpy(&md->lootitems[LOOTITEM_SIZE - 1].item, &fitem->item,
				   sizeof(md->lootitems[0].item));
			md->lootitems[LOOTITEM_SIZE - 1].mob_id = fitem->mob_id;
		}

		if (pcdb_checkid(
				md->vd->class_)) {	// Give them walk act/delay to properly mimic players. [Skotlex]
	...
	if (battle_check_range(&md->bl, tbl, md->status.rhw.range) &&
		!(md->sc.option &
		  OPTION_HIDE)) {  // Target within range and able to use normal attack, engage
		if (md->ud.target != tbl->id ||
			md->ud.attacktimer == INVALID_TIMER) {	// Only attack if no more attack delay left
			if (tbl->type == BL_PC)
				mob_log_damage(md, tbl,
							   0);	// Log interaction (counts as 'attacker' for the exp bonus)

			if (!(mode & MD_RANDOMTARGET))
				unit_attack(&md->bl, tbl->id, 1);
			else {	// Attack once and find a new random target
				int search_size =
					(view_range < md->status.rhw.range) ? view_range : md->status.rhw.range;
	...
	if (md->ud.walktimer == INVALID_TIMER &&
		(md->state.skillstate == MSS_BERSERK || md->state.skillstate == MSS_ANGRY)) {
		if (DIFF_TICK(md->ud.canmove_tick, tick) <= MIN_MOBTHINKTIME &&
			DIFF_TICK(md->ud.canact_tick, tick) <
				-MIN_MOBTHINKTIME *
					IDLE_SKILL_INTERVAL) {	// Only use skill if able to walk on next tick and not
											// used a skill the last second
	...
	if (md->ud.walktimer != INVALID_TIMER && md->ud.target == tbl->id &&
		(!(battle_config.mob_ai & 0x1) ||
		 check_distance_blxy(
			 tbl, md->ud.to_x, md->ud.to_y,
			 md->status.rhw.range)))  // Current target tile is still within attack range.
		return true;
...
static int mob_ai_sub_foreachclient(struct map_session_data *sd, va_list ap) {
	t_tick tick = va_arg(ap, t_tick);
	map_foreachinallrange(mob_ai_sub_hard_timer, &sd->bl, AREA_SIZE + ACTIVE_AI_RANGE, BL_MOB,
						  sd->status.char_id, tick);
...
static int mob_ai_sub_lazy(struct mob_data *md, va_list args) {
	...
	if (battle_config.mob_active_time && md->last_pcneartime &&
		!status_has_mode(&md->status, MD_STATUSIMMUNE) &&
	...
	if (battle_config.boss_active_time && md->last_pcneartime &&
		status_has_mode(&md->status, MD_STATUSIMMUNE) &&
		DIFF_TICK(tick, md->last_thinktime) > MIN_MOBTHINKTIME) {
	...
	if (DIFF_TICK(md->next_walktime, tick) < 0 && status_has_mode(&md->status, MD_CANMOVE) &&
		...
		if (mob_is_spotted(md) && ((!status_has_mode(&md->status, MD_STATUSIMMUNE) &&
									rnd() % 100 < battle_config.mob_nopc_move_rate) ||
								   (status_has_mode(&md->status, MD_STATUSIMMUNE) &&
									rnd() % 100 < battle_config.boss_nopc_move_rate)))
			mob_randomwalk(md, tick);
	} else if (md->ud.walktimer == INVALID_TIMER) {
		// Probability for mobs far from players from doing their IDLE skill.
		// In Aegis, this is 100% for mobs that have been activated by players and none otherwise.
		if (mob_is_spotted(md) && ((!status_has_mode(&md->status, MD_STATUSIMMUNE) &&
									rnd() % 100 < battle_config.mob_nopc_idleskill_rate) ||
								   (status_has_mode(&md->status, MD_STATUSIMMUNE) &&
									rnd() % 100 < battle_config.boss_nopc_idleskill_rate)))
 ...
 *------------------------------------------*/
static struct item_drop *mob_setdropitem(struct s_mob_drop *mobdrop, int qty,
...
static TIMER_FUNC(mob_delay_item_drop) {
	...
	while (ditem) {
		...
		map_addflooritem(&ditem->item_data, ditem->item_data.amount, list->m, list->x, list->y,
 ...
 *------------------------------------------*/
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist,
						  struct item_drop *ditem, int loot, int drop_rate, bool flag) {
	TBL_PC *sd;
	bool test_autoloot;
	// Logs items, dropped by mobs [Lupus]
	log_pick_mob(md, loot ? LOG_TYPE_LOOT : LOG_TYPE_PICKDROP_MONSTER, -ditem->item_data.amount,
				 &ditem->item_data);
	...
	test_autoloot =
		...
		(flag ? ((battle_config.homunculus_autoloot
					  ? (battle_config.hom_idle_no_share == 0 || !pc_isidle_hom(sd))
					  : 0) ||
				 (battle_config.mercenary_autoloot
					  ? (battle_config.mer_idle_no_share == 0 || !pc_isidle_mer(sd))
					  : 0))
	...
	if (test_autoloot) {  // Autoloot.
		...
		if ((itemdb_search(ditem->item_data.nameid))->flag.broadcast &&
			...
			intif_broadcast_obtain_special_item(sd, ditem->item_data.nameid, md->mob_id,
												ITEMOBTAIN_TYPE_MONSTER_ITEM);

		if (party_share_loot(party_search(sd->status.party_id), sd, &ditem->item_data,
							 sd->status.char_id) == 0) {
	...
	if (char_id) {	// Log damage...
		...
		for (i = 0, minpos = DAMAGELOG_SIZE - 1, mindmg = UINT_MAX; i < DAMAGELOG_SIZE; i++) {
			...
			if (md->dmglog[i].dmg < mindmg &&
				i) {  // Never overwrite first hit slot (he gets double exp bonus)
				minpos = i;
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage) {
	if (src != nullptr && md->special_state.ai == AI_SPHERE &&
		!md->dmglog[0].id) {  // LOne WOlf explained that ANYONE can trigger the marine countdown
							  // skill. [Skotlex]
	...
	if (src && damage > 0) {  // Store total damage...
		...
		if ((src != &md->bl) &&
			md->state.aggressive)  // No longer aggressive, change to retaliate AI.
			md->state.aggressive = 0;
		// Log damage
 ...
 */
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate,
					int drop_modifier) {
	...
	if (src) {
		...
		if (battle_config.drops_by_luk2)  // Drops affected by luk as a % increase [Skotlex]
			drop_rate +=
				(int)(0.5 + drop_rate * status_get_luk(src) * battle_config.drops_by_luk2 / 10000.);

		if (src->type == BL_PC) {  // Player specific drop rate adjustments
			struct map_session_data *sd = (struct map_session_data *)src;
			int drop_rate_bonus = 100;

			// In PK mode players get an additional drop chance bonus of 25% if there is a 20 level
			// difference
			...
			drop_rate_bonus += sd->indexed_bonus.dropaddclass[mob->status.class_] +
							   sd->indexed_bonus.dropaddclass[CLASS_ALL];
			drop_rate_bonus += sd->indexed_bonus.dropaddrace[mob->status.race] +
							   sd->indexed_bonus.dropaddrace[RC_ALL];
			...
			// Now limit the drop rate to never be exceed the cap (default: 90%), unless it is
			// originally above it already.
...
int mob_dead(struct mob_data *md, struct block_list *src, int type) {
	...
	int dmgbltypes = 0;	 // bitfield of all bl types, that caused damage to the mob and are
						 // elligible for exp distribution
	...
	if (md->guardian_data && md->guardian_data->number >= 0 &&
		md->guardian_data->number < MAX_GUARDIANS)
		guild_castledatasave(md->guardian_data->castle->castle_id,
							 CD_ENABLED_GUARDIAN00 + md->guardian_data->number, 0);
	...
	if (!(type & 2) &&											  // No exp
		...
		(!md->master_id ||
		 !md->special_state.ai) &&	// Only player-summoned mobs do not give exp. [Skotlex]
		...
		for (i = 0; i < DAMAGELOG_SIZE && md->dmglog[i].id; i++) {
			...
			if (base_exp && tmpsd[i] && tmpsd[i]->hd)
				hom_gainexp(tmpsd[i]->hd, base_exp * battle_config.homunculus_exp_gain /
											  100);	 // Homunculus only receive 10% of EXP
#else
			if (base_exp &&
				md->dmglog[i].flag == MDLF_HOMUN)  // tmpsd[i] is null if it has no homunc.
	...
	if (!(type & 1) && !map_getmapflag(m, MF_NOMOBLOOT) && !md->state.rebirth &&
		(!md->special_state.ai ||						// Non special mob
		 ...
		 (md->special_state.ai == AI_SPHERE &&
		  battle_config.alchemist_summon_reward == 1)  // Marine Sphere Drops items.
		 )) {										   // Item Drop
		...
		drop_modifier = pc_level_penalty_mod(
			mvp_sd != nullptr ? mvp_sd : second_sd != nullptr ? second_sd : third_sd, PENALTY_DROP,
			nullptr, md);
#endif
		dlist->m = md->bl.m;
		dlist->x = md->bl.x;
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (mvp_sd && md->db->dropitem[i].rate <= battle_config.rare_drop_announce) {
				char message[128];
				sprintf(message, msg_txt(NULL, 541), mvp_sd->status.name, md->name,
						it->ename.c_str(), (float)drop_rate / 100);
				// MSG: "'%s' won %s's %s (chance: %0.02f%%)"
				intif_broadcast(message, strlen(message) + 1, BC_DEFAULT);
			}
			// Announce first, or else ditem will be freed. [Lance]
			// By popular demand, use base drop rate for autoloot code. [Skotlex]
			mob_item_drop(md, dlist, ditem, 0,
						  battle_config.autoloot_adjust ? drop_rate : md->db->dropitem[i].rate,
		...
		if (sd) {
			...
			for (const auto &it : sd->add_drop) {
				if (!&it || (!it.nameid && !it.group))
					continue;
				if ((it.race < RC_NONE_ &&
					 it.race == -md->mob_id) ||	 // Race < RC_NONE_, use mob_id
					(it.race == RC_ALL || it.race == status->race) ||		  // Matched race
					(it.class_ == CLASS_ALL || it.class_ == status->class_))  // Matched class
				{
					// Check if the bonus item drop rate should be multiplied with mob level/10
					// [Lupus]
					if (it.rate < 0) {
						...
						drop_rate =
							cap_value(drop_rate, max(battle_config.item_drop_adddrop_min, 1),
					...
					if (rnd() % 10000 >= drop_rate)
						continue;
					dropid =
						(it.nameid > 0) ? it.nameid : itemdb_group.get_random_item_id(it.group, 1);
					struct s_mob_drop mobdrop = {};
					mobdrop.nameid = dropid;

					mob_item_drop(md, dlist, mob_setdropitem(&mobdrop, 1, md->mob_id), 0, drop_rate,
								  homkillonly || merckillonly);
				}
			}
			...
			if (sd->bonus.get_zeny_num && rnd() % 100 < sd->bonus.get_zeny_rate) {
				i = sd->bonus.get_zeny_num > 0 ? sd->bonus.get_zeny_num
											   : -md->level * sd->bonus.get_zeny_num;
				if (!i)
					i = 1;
		...
		if (md->lootitems) {
			for (i = 0; i < md->lootitem_count; i++)
				mob_item_drop(md, dlist, mob_setlootitem(&md->lootitems[i], md->mob_id), 1, 10000,
							  homkillonly || merckillonly);
		}
		...
		if (mapdrops != nullptr) {
			...
			for (const auto &it : mapdrops->globals) {
				if (rnd_chance(it.second->rate, 10000)) {
					mob_item_drop(md, dlist, mob_setdropitem(it.second.get(), 1, md->mob_id), 0,
								  it.second->rate, homkillonly || merckillonly);
				}
			}
		...
		if (dlist->item)  // There are drop items.
			add_timer(tick + (!battle_config.delay_battle_damage ? 500 : 0), mob_delay_item_drop, 0,
					  (intptr_t)dlist);
		else  // No drops
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				if ((temp = pc_additem(mvp_sd, &item, 1, LOG_TYPE_PICKDROP_PLAYER)) != 0) {
					...
					map_addflooritem(&item, 1, mvp_sd->bl.m, mvp_sd->bl.x, mvp_sd->bl.y,
									 ...
									 (third_sd ? third_sd->status.char_id : 0), 1, 0, true);
				}

				if (i_data->flag.broadcast)
					intif_broadcast_obtain_special_item(mvp_sd, item.nameid, md->mob_id,
														ITEMOBTAIN_TYPE_MONSTER_ITEM);
	...
	rebirth = (md->sc.data[SC_KAIZEL] || md->sc.data[SC_ULTIMATE_S] ||
			   (md->sc.data[SC_REBIRTH] && !md->state.rebirth));
	if (!rebirth) {	 // Only trigger event on final kill
		...
		if (sd) {
			std::shared_ptr<s_mob_db> mission_mdb = mob_db.find(sd->mission_mobid),
									  mob = mob_db.find(md->mob_id);

			if ((sd->mission_mobid == md->mob_id) ||
				(mission_mdb != nullptr &&
				 ((battle_config.taekwon_mission_mobname == 1 &&
				...
				if (++(sd->mission_count) >= 100 &&
					(temp = mob_get_random_id(
						 ...
						 static_cast<e_random_monster_flags>(RMF_CHECK_MOB_LV | RMF_MOB_NOT_BOSS |
															 RMF_MOB_NOT_SPAWN),
			...
			if (sd->status.party_id)
				map_foreachinallrange(quest_update_objective_sub, &md->bl, AREA_SIZE, BL_PC,
									  sd->status.party_id, md);
			...
			if (achievement_db.mobexists(md->mob_id)) {
				if (battle_config.achievement_mob_share > 0 && sd->status.party_id > 0)
					map_foreachinallrange(achievement_update_objective_sub, &md->bl, AREA_SIZE,
										  BL_PC, sd->status.party_id, md->mob_id);
				else
					achievement_update_objective(sd, AG_BATTLE, 1, md->mob_id);
			}

			// The master or Mercenary can increase the kill count
			if (sd->md && src && (src->type == BL_PC || src->type == BL_MER) &&
				mob->lv > sd->status.base_level / 2)
	...
	if (!rebirth) {
		if (pcdb_checkid(
				md->vd->class_)) {	// Player mobs are not removed automatically by the client.
			...
			/**
			 * We give the client some time to breath and this allows it to display anything it'd
			 *like with the dead corpose For example, this delay allows it to display soul drain
			 *effect
			 **/
	...
	if (battle_config.mvp_tomb_enabled && md->spawn->state.boss &&
		map_getmapflag(md->bl.m, MF_NOTOMB) != 1)
...
void mob_revive(struct mob_data *md, unsigned int hp) {
	...
	md->last_linktime = tick;
	md->last_pcneartime = 0;
	// We reset the damage log and then set the already lost damage as self damage so players don't
	// get exp for it [Playtester]
	memset(md->dmglog, 0, sizeof(md->dmglog));
...
int mob_guardian_guildchange(struct mob_data *md) {
	...
	if (md->guardian_data->castle->guild_id == 0) {	 // Castle with no owner? Delete the guardians.
		if (md->mob_id ==
			MOBID_EMPERIUM) {  // But don't delete the emperium, just clear it's guild-data
			md->guardian_data->guild_id = 0;
...
int mob_getfriendstatus_sub(struct block_list *bl, va_list ap) {
	...
	if (cond2 == -1) {
		...
		for (j = SC_COMMON_MIN; j <= SC_COMMON_MAX && !flag; j++) {
			if ((flag = (md->sc.data[j] != NULL)))	// Once an effect was found, break out.
													// [Skotlex]
				break;
		}
	} else
...
int mobskill_use(struct mob_data *md, t_tick tick, int event, int64 damage) {
	struct block_list *fbl = NULL;	// Friend bl, which can either be a BL_PC or BL_MOB depending on
									// the situation. [Skotlex]
	struct block_list *bl;
	...
	for (n = 0; n < ms.size(); i++, n++) {
		...
		if (ms[i]->state != md->state.skillstate) {
			if (md->state.skillstate != MSS_DEAD &&
				(ms[i]->state == MSS_ANY ||
				 (ms[i]->state == MSS_ANYTARGET && md->target_id &&
				  md->state.skillstate != MSS_LOOT)))  // ANYTARGET works with any state as long as
													   // there's a target. [Skotlex]
				;
			else
				continue;
		}
		...
		if (ms[i]->cond1 == event)
			...
			flag = ((event & 0xffff) == MSC_SKILLUSED &&
					skill_get_inf((event >> 16)) & INF_GROUND_SKILL);
		else if (ms[i]->cond1 == MSC_DAMAGEDGT && damage > 0 &&
				 !((event & 0xffff) ==
				   MSC_SKILLUSED))	// Avoid double check if skill has been used [datawulf]
			flag = (damage > c2);
		else if (event == -1) {
			// Avoid entering on defined events to avoid "hyper-active skill use" due to the
			// overflow of calls to this function in battle.
			switch (ms[i]->cond1) {
				case MSC_ALWAYS:
					flag = 1;
					break;
				...
				case MSC_RUDEATTACKED:
					...
					if (flag)
						md->state.attacked_count =
							0;	// Rude attacked count should be reset after the skill condition is
				...
				case MSC_MASTERATTACKED:
					flag = (md->master_id > 0 && (fbl = map_id2bl(md->master_id)) &&
							unit_counttargeted(fbl) > 0);
					break;
				case MSC_ALCHEMIST:
		...
		// Execute skill
		skill_target =
			status_has_mode(&md->db->status, MD_RANDOMTARGET) ? MST_RANDOM : ms[i]->target;
		if (skill_get_casttype(ms[i]->skill_id) == CAST_GROUND) {  // Ground skill.
			short x, y;
			switch (skill_target) {
				case MST_RANDOM:  // Pick a random enemy within skill range.
					bl = battle_getenemy(
						&md->bl, DEFAULT_ENEMY_TYPE(md),
			...
			if (skill_target >= MST_AROUND5) {
				j = skill_target >= MST_AROUND1 ? (skill_target - MST_AROUND1) + 1
			...
			if (!battle_check_range(
					&md->bl, bl,
					skill_get_range2(&md->bl, ms[i]->skill_id, ms[i]->skill_lv, true)) ||
				!unit_skilluse_pos2(&md->bl, x, y, ms[i]->skill_id, ms[i]->skill_lv,
			...
			if (!battle_check_range(
					&md->bl, bl,
					skill_get_range2(&md->bl, ms[i]->skill_id, ms[i]->skill_lv, true)) ||
				!unit_skilluse_id2(&md->bl, bl->id, ms[i]->skill_id, ms[i]->skill_lv,
								   ms[i]->casttime, ms[i]->cancel)) {
				map_freeblock_unlock();
				if (battle_config.mob_ai & 0x1000)
					continue;
...
int mobskill_event(struct mob_data *md, struct block_list *src, t_tick tick, int flag,
	...
	else if (damage > 0)  // Trigger for any damage dealt from other attack types without affecting
						  // other triggers [datawulf]
...
int mob_clone_spawn(struct map_session_data *sd, int16 m, int16 x, int16 y, const char *event,
					int master_id, enum e_mode mode, int flag, unsigned int duration) {
	int mob_id;
	int inf, fd;
	struct mob_data *md;
	struct status_data *status;
	...
	/**
	 * We temporarily disable sd's fd so it doesn't receive the messages from
	...
	if (tree != nullptr && !tree->skills.empty()) {
		...
		for (const auto &it : tree->skills) skill_list.push_back(it.first);
		...
		for (const auto &it : skill_list) {
			...
			if (!(flag & 1) && skill_get_unit_id(skill_id) &&
			...
			ms->state = MSS_ANY;
			ms->permillage =
				500 * battle_config.mob_skill_rate / 100;  // Default chance of all skills: 5%
			ms->emotion = -1;
			ms->cancel = 0;
			...
			if (inf & INF_ATTACK_SKILL) {
				...
				if (skill_get_inf2(skill_id, INF2_NOTARGETSELF)) {	// auto-select target skill.
					...
					ms->delay += -5000 + (skill_get_time(skill_id, ms->skill_lv) +
										  skill_get_time2(skill_id, ms->skill_lv)) /
											 2;
					if (ms->delay < 5000)
				...
				if (db->skill.size() <
					MAX_MOBSKILL) {	 // duplicate this so it also triggers on self.
					ms->target = MST_SELF;
					ms->cond1 = MSC_MYHPLTMAXRATE;
					db->skill.push_back(ms);
				}
			} else {
				switch (skill_id) {	 // Certain Special skills that are passive, and thus, never
									 // triggered.
					...
					case GS_CHAINACTION:
						...
						ms->target = MST_TARGET;
						ms->cond1 = MSC_ALWAYS;
						ms->permillage = skill_id == MO_TRIPLEATTACK ? (3000 - ms->skill_lv * 100)
																	 : (ms->skill_lv * 500);
						ms->delay -=
							5000;  // Remove the added delay as these could trigger on "all hits".
						break;
...
static unsigned int mob_drop_adjust(int baserate, int rate_adjust, unsigned short rate_min,
									unsigned short rate_max) {
...
static void item_dropratio_adjust(t_itemid nameid, int mob_id, int *rate_adjust) {
	...
	if (item_ratio) {
		// If it is empty it is applied to all monsters, if not it is only applied if the monster is
		// in the vector
		if (item_ratio->mob_ids.empty() ||
			util::vector_exists(item_ratio->mob_ids, static_cast<uint16>(mob_id)))
...
bool MobDatabase::parseDropNode(std::string nodeName, const ryml::NodeRef &node, uint8 max,
								s_mob_drop *drops) {
	...
	for (const auto &dropit : dropNode) {
		...
		if (this->nodeExists(dropit, "Index")) {
			...
			if (index >= max) {
				this->invalidWarning(
					dropit["Index"],
					"Invalid monster %s index %hu. Must be between 0~%hu, skipping.\n",
...
uint64 MobDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (!((mob_id > MIN_MOB_DB && mob_id < MAX_MOB_DB) ||
		  (mob_id > MIN_MOB_DB2 && mob_id < MAX_MOB_DB2))) {
		this->invalidWarning(node["Id"],
							 "Invalid monster ID %d, must be in range %d-%d or %d-%d.\n", mob_id,
							 MIN_MOB_DB, MAX_MOB_DB, MIN_MOB_DB2, MAX_MOB_DB2);
		return false;
	}
	...
	if (this->nodeExists(node, "JapaneseName")) {
		...
		if (name.size() > NAME_LENGTH) {
			this->invalidWarning(
				node["JapaneseName"],
				"JapaneseName \"%s\" exceeds maximum of %d characters, capping...\n", name.c_str(),
				NAME_LENGTH - 1);
		}
	...
	if (this->nodeExists(node, "BaseExp")) {
		...
		mob->base_exp = static_cast<t_exp>(
			cap_value((double)exp * (double)battle_config.base_exp_rate / 100., 0, MAX_EXP));
	} else {
		if (!exists)
			mob->base_exp = 0;
	}
	...
	if (this->nodeExists(node, "JobExp")) {
		...
		mob->job_exp = static_cast<t_exp>(
			cap_value((double)exp * (double)battle_config.job_exp_rate / 100., 0, MAX_EXP));
	} else {
		if (!exists)
			mob->job_exp = 0;
	}
	...
	if (this->nodeExists(node, "MvpExp")) {
		...
		mob->mexp = static_cast<t_exp>(
			cap_value((double)exp * (double)battle_config.mvp_exp_rate / 100., 0, MAX_EXP));
	} else {
		if (!exists)
			mob->mexp = 0;
	}
	...
	if (this->nodeExists(node, "MagicDefense")) {
		...
		if (def < DEFTYPE_MIN || def > DEFTYPE_MAX) {
			this->invalidWarning(node["MagicDefense"],
								 "Invalid monster magic defense %d, capping...\n", def);
			def = cap_value(def, DEFTYPE_MIN, DEFTYPE_MAX);
		}
	...
	if (this->nodeExists(node, "RaceGroups")) {
		...
		for (const auto &raceit : raceNode) {
			...
			if (!CHK_RACE2(constant)) {
				this->invalidWarning(raceNode[raceit.key()],
									 "Invalid monster race group %s, skipping.\n",
									 raceName.c_str());
				continue;
			}
	...
	if (this->nodeExists(node, "Element")) {
		...
		if (!script_get_constant(ele_constant.c_str(), &constant)) {
			this->invalidWarning(node["Element"],
								 "Unknown monster element %s, defaulting to Neutral.\n",
								 ele.c_str());
		...
		if (!CHK_ELEMENT(constant)) {
			this->invalidWarning(node["Element"],
								 "Invalid monster element %s, defaulting to Neutral.\n",
								 ele.c_str());
			constant = ELE_NEUTRAL;
		}
	...
	if (this->nodeExists(node, "ElementLevel")) {
		...
		if (!CHK_ELEMENT_LEVEL(level)) {
			this->invalidWarning(node["ElementLevel"],
								 "Invalid monster element level %hu, defaulting to 1.\n", level);
			level = 1;
		}
...
void MobDatabase::loadingFinished() {
	for (auto &mobdata : *this) {
		...
		switch (mob->status.class_) {
			case CLASS_BOSS:
				mob->status.mode = static_cast<e_mode>(
					mob->status.mode | (MD_DETECTOR | MD_STATUSIMMUNE | MD_KNOCKBACKIMMUNE));
				break;
			case CLASS_GUARDIAN:
			...
			case CLASS_BATTLEFIELD:
				mob->status.mode =
					static_cast<e_mode>(mob->status.mode | (MD_STATUSIMMUNE | MD_SKILLIMMUNE));
				break;
			case CLASS_EVENT:
		...
		// On aegis there is no real visible effect of having a recharge-time less than amotion
		// anyway.
	...
	for (uint8 fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi) {
		...
		if (SQL_ERROR ==
			Sql_Query(
				mmysql_handle,
				"SELECT "
				"`id`,`name_aegis`,`name_english`,`name_japanese`,`level`,`hp`,`sp`,`base_exp`,`"
				"job_exp`,`mvp_exp`,`attack`,`attack2`,`defense`,`magic_defense`,`str`,`agi`,`vit`,"
				"`int`,`dex`,`luk`,`attack_range`,`skill_range`,`chase_range`,`size`,`race`,"
				"`racegroup_goblin`,`racegroup_kobold`,`racegroup_orc`,`racegroup_golem`,`"
				"racegroup_guardian`,`racegroup_ninja`,`racegroup_gvg`,`racegroup_battlefield`,`"
				"racegroup_treasure`,`racegroup_biolab`,`racegroup_manuk`,`racegroup_splendide`,`"
				"racegroup_scaraba`,`racegroup_ogh_atk_def`,`racegroup_ogh_hidden`,`racegroup_bio5_"
				"swordman_thief`,`racegroup_bio5_acolyte_merchant`,`racegroup_bio5_mage_archer`,`"
				"racegroup_bio5_mvp`,`racegroup_clocktower`,`racegroup_thanatos`,`racegroup_"
				"faceworm`,`racegroup_hearthunter`,`racegroup_rockridge`,`racegroup_werner_lab`,`"
				"racegroup_temple_demon`,`racegroup_illusion_vampire`,`racegroup_malangdo`,"
				"`element`,`element_level`,`walk_speed`,`attack_delay`,`attack_motion`,`damage_"
				"motion`,`damage_taken`,`ai`,`class`,"
				"`mode_canmove`,`mode_looter`,`mode_aggressive`,`mode_assist`,`mode_castsensoridle`"
				",`mode_norandomwalk`,`mode_nocast`,`mode_canattack`,`mode_castsensorchase`,`mode_"
				"changechase`,`mode_angry`,`mode_changetargetmelee`,`mode_changetargetchase`,`mode_"
				"targetweak`,`mode_randomtarget`,`mode_ignoremelee`,`mode_ignoremagic`,`mode_"
				"ignoreranged`,`mode_mvp`,`mode_ignoremisc`,`mode_knockbackimmune`,`mode_"
				"teleportblock`,`mode_fixeditemdrop`,`mode_detector`,`mode_statusimmune`,`mode_"
				"skillimmune`,"
				"`mvpdrop1_item`,`mvpdrop1_rate`,`mvpdrop1_option`,`mvpdrop1_index`,`mvpdrop2_item`"
				",`mvpdrop2_rate`,`mvpdrop2_option`,`mvpdrop2_index`,`mvpdrop3_item`,`mvpdrop3_"
				"rate`,`mvpdrop3_option`,`mvpdrop3_index`,"
				"`drop1_item`,`drop1_rate`,`drop1_nosteal`,`drop1_option`,`drop1_index`,`drop2_"
				"item`,`drop2_rate`,`drop2_nosteal`,`drop2_option`,`drop2_index`,`drop3_item`,`"
				"drop3_rate`,`drop3_nosteal`,`drop3_option`,`drop3_index`,`drop4_item`,`drop4_rate`"
				",`drop4_nosteal`,`drop4_option`,`drop4_index`,`drop5_item`,`drop5_rate`,`drop5_"
				"nosteal`,`drop5_option`,`drop5_index`,`drop6_item`,`drop6_rate`,`drop6_nosteal`,`"
				"drop6_option`,`drop6_index`,`drop7_item`,`drop7_rate`,`drop7_nosteal`,`drop7_"
				"option`,`drop7_index`,`drop8_item`,`drop8_rate`,`drop8_nosteal`,`drop8_option`,`"
				"drop8_index`,`drop9_item`,`drop9_rate`,`drop9_nosteal`,`drop9_option`,`drop9_"
				"index`,`drop10_item`,`drop10_rate`,`drop10_nosteal`,`drop10_option`,`drop10_index`"
#ifdef RENEWAL
		...
		while (SQL_SUCCESS == Sql_NextRow(mmysql_handle)) {
			ShowStatus("Loading [%" PRIu64 "/%" PRIu64 "] rows from '" CL_WHITE "%s" CL_RESET
					   "'" CL_CLL "\r",
					   ++rows, total_rows, mob_db_name[fi]);
...
uint64 MobAvailDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "HairStyle")) {
		if (pcdb_checkid(mob->vd.class_) == 0) {
			this->invalidWarning(node["HairStyle"],
								 "HairStyle is only applicable to Job sprites.\n");
			return 0;
		}
		...
		if (hair_style < MIN_HAIR_STYLE || hair_style > MAX_HAIR_STYLE) {
			this->invalidWarning(node["HairStyle"],
								 "HairStyle %d is out of range %d~%d. Setting to MIN_HAIR_STYLE.\n",
	...
	if (this->nodeExists(node, "HairColor")) {
		if (pcdb_checkid(mob->vd.class_) == 0) {
			this->invalidWarning(node["HairColor"],
								 "HairColor is only applicable to Job sprites.\n");
			return 0;
		}
		...
		if (hair_color < MIN_HAIR_COLOR || hair_color > MAX_HAIR_COLOR) {
			this->invalidWarning(node["HairColor"],
								 "HairColor %d is out of range %d~%d. Setting to MIN_HAIR_COLOR.\n",
	...
	if (this->nodeExists(node, "ClothColor")) {
		if (pcdb_checkid(mob->vd.class_) == 0) {
			this->invalidWarning(node["ClothColor"],
								 "ClothColor is only applicable to Job sprites.\n");
			return 0;
		}
		...
		if (cloth_color < MIN_CLOTH_COLOR || cloth_color > MAX_CLOTH_COLOR) {
			this->invalidWarning(
				node["ClothColor"],
				"ClothColor %d is out of range %d~%d. Setting to MIN_CLOTH_CLOR.\n", cloth_color,
	...
	if (this->nodeExists(node, "PetEquip")) {
		...
		if (pet_db_ptr == nullptr) {
			this->invalidWarning(node["PetEquip"],
								 "PetEquip is only applicable to defined pets.\n");
			return 0;
		}
	...
	if (this->nodeExists(node, "Options")) {
		...
		for (const auto &it : optionNode) {
			...
			if (constant & OPTION_CART) {
				this->invalidWarning(optionNode,
									 "OPTION_CART was replace by SC_PUSH_CART, skipping.\n");
				continue;
			}
#endif
		...
		mob->option &= ~(OPTION_HIDE | OPTION_CLOAK | OPTION_INVISIBLE |
						 OPTION_CHASEWALK);	 // Remove hiding types
	}
...
uint64 MobSummonDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (!script_get_constant(group_name_constant.c_str(), &constant) ||
		constant < MOBG_BRANCH_OF_DEAD_TREE || constant >= MOBG_MAX) {
		...
		for (const auto &mobit : MobNode) {
			...
			if (rate == 0) {
				if (summon->list.erase(mob_id) == 0)
					this->invalidWarning(
						mobit["Rate"],
						"Failed to remove %s, the monster doesn't exist in group %s.\n",
...
static bool mob_parse_row_mobskilldb(char **str, int columns, int current) {
	...
	static const struct {
	  ...
	  target[] = {
		  ...
		  {"target", MST_TARGET},	{"randomtarget", MST_RANDOM}, {"self", MST_SELF},
		  {"friend", MST_FRIEND},	{"master", MST_MASTER},		  {"around5", MST_AROUND5},
		  {"around6", MST_AROUND6}, {"around7", MST_AROUND7},	  {"around8", MST_AROUND8},
		  {"around1", MST_AROUND1}, {"around2", MST_AROUND2},	  {"around3", MST_AROUND3},
	...
	ms->skill_lv = j > battle_config.mob_max_skilllvl ? battle_config.mob_max_skilllvl
													  : j;	// we strip max skill level
	...
	if (skill_get_casttype(ms->skill_id) == CAST_GROUND) {	// Ground skill.
		if (ms->target > MST_AROUND) {
			ShowWarning(
				"mob_parse_row_mobskilldb: Wrong mob skill target for ground skill %d (%s) for "
				"%s.\n",
		...
		ShowWarning(
			"mob_parse_row_mobskilldb: Wrong mob skill target 'around' for non-ground skill %d "
			"(%s) for %s.\n",
			ms->skill_id, skill_get_name(ms->skill_id),
			mob_id < 0 ? "all mobs" : mob->sprite.c_str());
		ms->target = MST_TARGET;
...
static void mob_drop_ratio_adjust(void) {
	for (auto &pair : mob_db) {
		...
		for (j = 0; j < MAX_MVP_DROP_TOTAL; j++) {
			...
			rate = mob_drop_adjust(rate, rate_adjust, battle_config.item_drop_mvp_min,
			...
			if (rate) {
				...
				if (id->maxchance == -1 || (id->maxchance < rate / 10 + 1)) {
					...
					id->maxchance =
						rate / 10 + 1;	// reduce MVP drop info to not spoil common drop rate
				}
			}
				...
				switch (id->type) {
					case IT_HEALING:
						rate_adjust = is_mvp ? battle_config.item_rate_heal_mvp
											 : (is_boss ? battle_config.item_rate_heal_boss
					...
					case IT_ARMOR:
					case IT_PETARMOR:
						rate_adjust = is_mvp ? battle_config.item_rate_equip_mvp
											 : (is_boss ? battle_config.item_rate_equip_boss
					...
					case IT_CARD:
						rate_adjust = is_mvp ? battle_config.item_rate_card_mvp
											 : (is_boss ? battle_config.item_rate_card_boss
														: battle_config.item_rate_card);
						ratemin = battle_config.item_drop_card_min;
						ratemax = battle_config.item_drop_card_max;
						break;
					default:
						rate_adjust = is_mvp ? battle_config.item_rate_common_mvp
											 : (is_boss ? battle_config.item_rate_common_boss
			...
			if (rate && !is_treasurechest) {
				...
				if (k != MAX_SEARCH) {
					if (id->mob[k].id != mob_id) {
						memmove(&id->mob[k + 1], &id->mob[k],
								(MAX_SEARCH - k - 1) * sizeof(id->mob[0]));
					}
...
uint64 MapDropDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (this->nodeExists(node, "SpecificDrops")) {
		...
		for (const auto &monsterNode : specificNode) {
			...
			std::unordered_map<uint16, std::shared_ptr<s_mob_drop>> &specificDrops =
				mapdrops->specific[mob->id];
...
bool MapDropDatabase::parseDrop(const ryml::NodeRef &node,
								std::unordered_map<uint16, std::shared_ptr<s_mob_drop>> &drops) {
	uint16 index;
 ...
 **/
static void mob_skill_db_set_single_sub(std::shared_ptr<s_mob_db> mob,
										struct s_mob_skill_db *skill) {
	nullpo_retv(skill);
	...
	// Global skill
		...
		for (auto &pair : mob_db) {
			...
			if ((!(id & 1) && status_has_mode(&pair.second->status, MD_STATUSIMMUNE))  // Bosses
				|| (!(id & 2) &&
					!status_has_mode(&pair.second->status, MD_STATUSIMMUNE))  // Normal monsters
			)
				continue;
...
void mob_db_load(bool is_reload) {
	if (!is_reload) {
		...
		item_drop_list_ers =
			ers_new(sizeof(struct item_drop_list), "mob.cpp::item_drop_list_ers", ERS_OPT_NONE);
	}
	mob_load();
}
...
static int mob_reload_sub(struct mob_data *md, va_list args) {
	...
	if (!md->vd_changed) {
		...
		if (md->bl.prev != nullptr) {
			// Respawn all mobs on client side so that they are displayed correctly(if their view id
			// changed)
...
static int mob_reload_sub_npc(struct npc_data *nd, va_list args) {
	...
	if (mobdb_checkid(nd->class_)) {
		...
		if (nd->bl.prev)			// If they are spawned right now
			unit_refresh(&nd->bl);	// Respawn all NPCs on client side so that they are displayed
									// correctly(if their view id changed)
	}
