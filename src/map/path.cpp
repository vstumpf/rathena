...
static BHEAP_STRUCT_VAR(
	node_heap, g_open_set);	 // use static heap for all path calculations
...
#define heuristic(x0, y0, x1, y1) \
	(MOVE_COST * (abs((x1) - (x0)) + abs((y1) - (y0))))	 // Manhattan distance
/// @}
...
void do_init_path() {
	BHEAP_INIT(g_open_set);	 // [fwi]: BHEAP_STRUCT_VAR already initialized the heap, this is
							 // rudendant & just for code-conformance/readability
}  //

void do_final_path() { BHEAP_CLEAR(g_open_set); }  //
 ...
 *------------------------------------------*/
bool path_search_long(struct shootpath_data *spd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1,
					  cell_chk cell) {
	int dx, dy;
	int wx = 0, wy = 0;
	int weight;
...
static int add_path(struct node_heap *heap, struct path_node *tp, int16 x, int16 y, int g_cost,
					struct path_node *parent, int h_cost) {
 ...
 *------------------------------------------*/
bool path_search(struct walkpath_data *wpd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1,
				 int flag, cell_chk cell) {
	...
	if (x0 < 0 || x0 >= mapdata->xs || y0 < 0 ||
		y0 >= mapdata->ys /*|| map_getcellp(mapdata,x0,y0,cell)*/)
		return false;

	// Check destination cell
	if (x1 < 0 || x1 >= mapdata->xs || y1 < 0 || y1 >= mapdata->ys ||
		map_getcellp(mapdata, x1, y1, cell))
		return false;
	...
	if (flag & 1) {
		...
		for (;;) {
			...
			BHEAP_POP2(g_open_set, NODE_MINTOPCMP,
					   swap_ptrcast_pathnode);	// Remove it from 'open' set
...
#define chk_dir(d) ((allowed_dirs & (d)) == (d))
			...
			if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_EAST) &&
				!map_getcellp(mapdata, x + 1, y - 1, cell))
				e += add_path(&g_open_set, tp, x + 1, y - 1, g_cost + MOVE_DIAGONAL_COST, current,
							  heuristic(x + 1, y - 1, x1, y1));	 // (x+1, y-1) 5
			if (chk_dir(PATH_DIR_EAST))
				e += add_path(&g_open_set, tp, x + 1, y, g_cost + MOVE_COST, current,
							  heuristic(x + 1, y, x1, y1));	 // (x+1, y) 6
			if (chk_dir(PATH_DIR_NORTH | PATH_DIR_EAST) &&
				!map_getcellp(mapdata, x + 1, y + 1, cell))
				e += add_path(&g_open_set, tp, x + 1, y + 1, g_cost + MOVE_DIAGONAL_COST, current,
							  heuristic(x + 1, y + 1, x1, y1));	 // (x+1, y+1) 7
			if (chk_dir(PATH_DIR_NORTH))
				e += add_path(&g_open_set, tp, x, y + 1, g_cost + MOVE_COST, current,
							  heuristic(x, y + 1, x1, y1));	 // (x, y+1) 0
			if (chk_dir(PATH_DIR_NORTH | PATH_DIR_WEST) &&
				!map_getcellp(mapdata, x - 1, y + 1, cell))
				e += add_path(&g_open_set, tp, x - 1, y + 1, g_cost + MOVE_DIAGONAL_COST, current,
							  heuristic(x - 1, y + 1, x1, y1));	 // (x-1, y+1) 1
			if (chk_dir(PATH_DIR_WEST))
				e += add_path(&g_open_set, tp, x - 1, y, g_cost + MOVE_COST, current,
							  heuristic(x - 1, y, x1, y1));	 // (x-1, y) 2
			if (chk_dir(PATH_DIR_SOUTH | PATH_DIR_WEST) &&
				!map_getcellp(mapdata, x - 1, y - 1, cell))
				e += add_path(&g_open_set, tp, x - 1, y - 1, g_cost + MOVE_DIAGONAL_COST, current,
							  heuristic(x - 1, y - 1, x1, y1));	 // (x-1, y-1) 3
			if (chk_dir(PATH_DIR_SOUTH))
				e += add_path(&g_open_set, tp, x, y - 1, g_cost + MOVE_COST, current,
							  heuristic(x, y - 1, x1, y1));	 // (x, y-1) 4
#undef chk_dir
			if (e) {
				return false;
			}
...
unsigned int distance(int dx, int dy) {
	...
	if (dy < 0)
		dy = -dy;
	// There appears to be something wrong with the approximation below when either dx/dy is 0!
	// [Skotlex]
	if (dx == 0)
		return dy;
	if (dy == 0)
	...
	return (((max << 8) + (max << 3) - (max << 4) - (max << 1) + (min << 7) - (min << 5) +
			 (min << 3) - (min << 1)) >>
			8);
#else
	if (dx < 0)
