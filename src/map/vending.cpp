 ...
 */
DBMap *vending_getdb() { return vending_db; }

/**
 * Create an unique vending shop id.
 * @return the next vending_id
 */
static int vending_getuid(void) { return ++vending_nextid; }
...
void vending_closevending(struct map_session_data *sd) {
	...
	if (sd->state.vending) {
		if (Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE vending_id = %d;", vending_items_table,
					  sd->vender_id) != SQL_SUCCESS ||
			Sql_Query(mmysql_handle, "DELETE FROM `%s` WHERE `id` = %d;", vendings_table,
					  sd->vender_id) != SQL_SUCCESS) {
...
void vending_vendinglistreq(struct map_session_data *sd, int id) {
	...
	if (!pc_can_give_items(sd) ||
		!pc_can_give_items(vsd)) {	// check if both GMs are allowed to trade
 ...
 */
void vending_purchasereq(struct map_session_data *sd, int aid, int uid, const uint8 *data,
						 int count) {
	...
	for (i = 0; i < count; i++) {
		...
		// if they try to add packets (example: get twice or more 2 apples if marchand has only 3
		// apples). here, we check cumulative amounts
	...
	if (vsd->state.autotrade) {
		...
		if (i == vsd->vend_num) {
			// Close Vending (this was automatically done by the client, we have to do it manually
			// for autovenders) [Skotlex]
 ...
 * @return 0 If success, 1 - Cannot open (die, not state.prevend, trading), 2 - No cart, 3 - Count
 *issue, 4 - Cart data isn't saved yet, 5 - No valid item found
 */
int8 vending_openvending(struct map_session_data *sd, const char *message, const uint8 *data,
						 int count, struct s_autotrader *at) {
	int i, j;
	...
	if (pc_isdead(sd) || !sd->state.prevend || pc_istrading(sd)) {
		return 1;  // can't open vendings lying dead || didn't use via the skill (wpe/hack) || can't
				   // have 2 shops at once
	}
	...
	for (j = 0; j < count; j++) {
		...
		if (index < 0 || index >= MAX_CART				  // invalid position
			...
			|| sd->cart.u.items_cart[index]
				   .expire_time	 // It should not be in the cart but just in case
			|| (sd->cart.u.items_cart[index].bound &&
				!pc_can_give_bounded_items(
					sd))  // can't trade account bound items and has no permission
			|| !itemdb_cantrade(&sd->cart.u.items_cart[index], pc_get_group_level(sd),
								pc_get_group_level(sd)))  // untradeable item
			continue;
	...
	if (Sql_Query(mmysql_handle,
				  ...
				  "`title`, `autotrade`, `body_direction`, `head_direction`, `sit`) "
				  "VALUES( %d, %d, %d, '%c', '%s', %d, %d, '%s', %d, '%d', '%d', '%d' );",
				  vendings_table, sd->vender_id, sd->status.account_id, sd->status.char_id,
				  sd->status.sex == SEX_FEMALE ? 'F' : 'M', map_getmapdata(sd->bl.m)->name,
				  sd->bl.x, sd->bl.y, message_sql, sd->state.autotrade, at ? at->dir : sd->ud.dir,
	...
	StringBuf_Init(&buf);
	StringBuf_Printf(
		&buf, "INSERT INTO `%s`(`vending_id`,`index`,`cartinventory_id`,`amount`,`price`) VALUES",
		vending_items_table);
	for (j = 0; j < i; j++) {
		StringBuf_Printf(&buf, "(%d,%d,%d,%d,%d)", sd->vender_id, j,
						 sd->cart.u.items_cart[sd->vending[j].index].id, sd->vending[j].amount,
						 sd->vending[j].value);
		if (j < i - 1)
...
bool vending_searchall(struct map_session_data *sd, const struct s_search_store_search *s) {
	...
	for (idx = 0; idx < s->item_count; idx++) {
		ARR_FIND(0, sd->vend_num, i,
				 sd->cart.u.items_cart[sd->vending[i].index].nameid == s->itemlist[idx].itemId);
		if (i == sd->vend_num) {  // not found
			continue;
		}
...
void vending_reopen(struct map_session_data *sd) {
	...
	if ((at = (struct s_autotrader *)uidb_get(vending_autotrader_db, sd->status.char_id)) &&
		...
		for (j = 0, p = data, count = at->count; j < at->count; j++) {
			...
			ARR_FIND(0, MAX_CART, entry->index,
					 sd->cart.u.items_cart[entry->index].id == entry->cartinventory_id);
			...
			*index = entry->index + 2;
			*amount =
				itemdb_isstackable(sd->cart.u.items_cart[entry->index].nameid) ? entry->amount : 1;
			*value = entry->price;
		...
		if ((fail = vending_openvending(sd, at->title, data, count, at)) == 0) {
			...
			ShowInfo("Vending loaded for '" CL_WHITE "%s" CL_RESET "' with '" CL_WHITE "%d" CL_RESET
	...
	if (fail != 0) {
		ShowError("vending_reopen: (Error:%d) Load failed for autotrader '" CL_WHITE "%s" CL_RESET
				  "' (CID=%d/AID=%d)\n",
...
void do_init_vending_autotrade(void) {
	if (battle_config.feature_autotrade) {
		if (Sql_Query(mmysql_handle,
					  "SELECT `id`, `account_id`, `char_id`, `sex`, `title`, `body_direction`, "
					  "`head_direction`, `sit` "
					  "FROM `%s` "
					  "WHERE `autotrade` = 1 AND (SELECT COUNT(`vending_id`) FROM `%s` WHERE "
					  "`vending_id` = `id`) > 0 "
					  "ORDER BY `id`;",
		...
		if (Sql_NumRows(mmysql_handle) > 0) {
			...
			ShowStatus("Done loading '" CL_WHITE "%d" CL_RESET
					   "' vending autotraders with '" CL_WHITE "%d" CL_RESET "' items.\n",
...
void vending_update(map_session_data &sd) {
	if (Sql_Query(mmysql_handle,
				  ...
				  vendings_table, map_getmapdata(sd.bl.m)->name, sd.bl.x, sd.bl.y, sd.ud.dir,
				  sd.head_dir, pc_issit(&sd), sd.state.autotrade, sd.vender_id) != SQL_SUCCESS) {
		Sql_ShowDebug(mmysql_handle);
	}
}
