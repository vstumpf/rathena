...
static struct s_homun_intimacy_grade intimacy_grades[] = {
	{/*"Hate with passion",*/ 100},	  {/*"Hate",             */ 400},
	{/*"Awkward",          */ 1100},  {/*"Shy",              */ 10100},
	{/*"Neutral",          */ 25100}, {/*"Cordial",          */ 75100},
	{/*"Loyal",            */ 91100},
};
...
uint64 HomExpDatabase::parseBodyNode(const ryml::NodeRef &node) {
	...
	if (level >= MAX_LEVEL) {
		this->invalidWarning(node["Level"],
							 "Homunculus level %d exceeds maximum level %d, skipping.\n", level,
							 MAX_LEVEL);
		return 0;
	}
			...
			if (!battle_config.skillfree) {
				...
				for (j = 0; j < MAX_HOM_SKILL_REQUIRE; j++) {
					if (hskill_tree[c][i].need[j].id &&
						hom_checkskill(hd, hskill_tree[c][i].need[j].id) <
...
int hom_levelup(struct homun_data *hd) {
	...
	if (m_class & HOM_S && hd->homunculus.level < battle_config.hom_S_growth_level) {
		...
		if (!hd->homunculus.prev_class) {
			/// We also need to be sure that the previous class exists, otherwise give it something
			/// to work with
	...
	// Needed to update skill list for mutated homunculus so unlocked skills will appear when the
	// needed level is reached.
	status_calc_homunculus(hd, SCO_NONE);
	clif_hominfo(hd->master, hd, 0);
	clif_homskillinfoblock(hd->master);
	...
	if (hd->master && battle_config.homunculus_show_growth) {
		...
		sprintf(output,
				...
				growth_max_hp, growth_max_sp, growth_str / 10.0, growth_agi / 10.0,
				growth_vit / 10.0, growth_int / 10.0, growth_dex / 10.0, growth_luk / 10.0);
...
int hom_evolution(struct homun_data *hd) {
	...
	if (!hom_change_class(hd, hd->homunculusDB->evo_class)) {
		ShowError("hom_evolution: Can't evolve homunc from %d to %d", hd->homunculus.class_,
				  hd->homunculusDB->evo_class);
		return 0;
	}
...
static TIMER_FUNC(hom_hungry) {
	...
	hd->hungry_timer =
		add_timer(tick + hunger_delay, hom_hungry, sd->bl.id, 0);  // simple Fix albator
	return 0;
}
	...
	for (i = 0; i < homunculus_count; i++) {
		...
		switch (type) {
			case HOMUNCULUS_CLASS:
				if (homunculus_db[i].base_class == key || homunculus_db[i].evo_class == key)
					return i;
				break;
...
int hom_recv_data(uint32 account_id, struct s_homunculus *sh, int flag) {
	...
	if (hd && hd->homunculus.hp && !hd->homunculus.vaporize && hd->bl.prev == NULL &&
		sd->bl.prev != NULL) {
		if (map_addblock(&hd->bl))
			return 0;
		clif_spawn(&hd->bl);
 ...
 * @param hd
 */
void hom_reset_stats(struct homun_data *hd) {  // Resets a homunc stats back to zero (but doesn't
											   // touches hunger or intimacy)
 ...
 * <hom class>,<skill id>,<max level>,<need level>,<req id1>,<req lv1>,<req id2>,<req lv2>,<req
 * id3>,<req lv3>,<req id4>,<req lv4>,<req id5>,<req lv5>,<intimacy lv req>
 */
static bool read_homunculus_skilldb_sub(char *split[], int columns, int current) {
	...
	ARR_FIND(0, MAX_HOM_SKILL_TREE, idx,
			 !hskill_tree[class_idx][idx].id || hskill_tree[class_idx][idx].id == skill_id);
	if (idx == MAX_HOM_SKILL_TREE) {
		ShowWarning(
			"Unable to load skill %d into homunculus %d's tree. Maximum number of skills per class "
			"has been reached.\n",
...
void hom_reload_skill(void) { read_homunculus_skilldb(); }
