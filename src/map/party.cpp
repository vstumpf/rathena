...
static DBMap *party_booking_db;	 // uint32 char_id -> struct party_booking_ad_info* (releases data)
...
static TBL_PC *party_sd_check(int party_id, uint32 account_id, uint32 char_id) {
	...
	if (sd->status.party_id != party_id) {	// If player belongs to a different party, kick him out.
		intif_party_leave(party_id, account_id, char_id, sd->status.name,
...
void do_init_party(void) {
	...
	add_timer_interval(gettick() + battle_config.party_update_interval, party_send_xy_timer, 0, 0,
...
struct party_data *party_searchname(const char *str) {
	...
	for (p = (struct party_data *)dbi_first(iter); dbi_exists(iter);
		 p = (struct party_data *)dbi_next(iter)) {
...
int party_create(struct map_session_data *sd, char *name, int item, int item2) {
	...
	if (sd->status.party_id > 0 || sd->party_joining ||
		sd->party_creating) {  // already associated with a party
...
void party_created(uint32 account_id, uint32 char_id, int fail, int party_id, char *name) {
	...
	if (!sd || sd->status.char_id != char_id ||
		!sd->party_creating) {	// Character logged off before creation ack?
	...
	if (!fail) {
		...
		// We don't do any further work here because the char-server sends a party info packet right
		// after creating the party
		if (party_create_byscript) {  // returns party id in $@party_create_id if party is created
									  // by script
...
int party_recv_info(struct party *sp, uint32 char_id) {
	...
	while (added_count > 0) {  // new in party
		...
		// Only send this on party creation, otherwise it will be sent by party_send_movemap
		// [Lemongrass]
...
int party_invite(struct map_session_data *sd, struct map_session_data *tsd) {
	...
	if (!pc_has_permission(sd, PC_PERM_PARTY) || (tsd && !pc_has_permission(tsd, PC_PERM_PARTY))) {
		clif_displaymessage(sd->fd,
							msg_txt(sd, 81));  // "Your GM level doesn't authorize you to perform
...
int party_reply_invite(struct map_session_data *sd, int party_id, int flag) {
	...
	// The character is already in a party, possibly left a party invite open and created his own
	// party
	if (sd->status.party_id != 0) {
...
int party_removemember(struct map_session_data *sd, uint32 account_id, char *name) {
	...
	intif_party_leave(p->party.party_id, account_id, p->party.member[i].char_id,
					  p->party.member[i].name, PARTY_MEMBER_WITHDRAW_EXPEL);
...
int party_removemember2(struct map_session_data *sd, uint32 char_id, int party_id) {
	if (sd) {
		...
		intif_party_leave(sd->status.party_id, sd->status.account_id, sd->status.char_id,
						  sd->status.name, PARTY_MEMBER_WITHDRAW_EXPEL);
		return 1;
	} else {
		int i;
		struct party_data *p;
		...
		if (i >= MAX_PARTY)
			return -1;
		intif_party_leave(party_id, p->party.member[i].account_id, char_id, p->party.member[i].name,
						  PARTY_MEMBER_WITHDRAW_EXPEL);
		return 1;
	}
}
...
int party_leave(struct map_session_data *sd) {
	...
	party_trade_bound_cancel(sd);
	intif_party_leave(p->party.party_id, sd->status.account_id, sd->status.char_id, sd->status.name,
					  PARTY_MEMBER_WITHDRAW_LEAVE);
	return 1;
}

/// Invoked (from char-server) when a party member leaves the party.
int party_member_withdraw(int party_id, uint32 account_id, uint32 char_id, char *name,
	...
	if (p) {
		...
		ARR_FIND(
			0, MAX_PARTY, i,
			p->party.member[i].account_id == account_id && p->party.member[i].char_id == char_id);
		if (i < MAX_PARTY) {
...
int party_broken(int party_id) {
	...
	for (i = 0; i < MAX_PARTY; i++) {
		if (p->data[i].sd != nullptr) {
			clif_party_withdraw(*p->data[i].sd, p->party.member[i].account_id,
								p->party.member[i].name, PARTY_MEMBER_WITHDRAW_EXPEL, SELF);
...
int party_setoption(struct party_data *party, int option, int flag) {
	...
	switch (option) {
		case 0:
			intif_party_changeoption(party->party.party_id, party->party.member[i].account_id, flag,
									 party->party.item);
			break;
		case 1:
			if (flag)
				...
				flag = party->party.item & ~1;
			intif_party_changeoption(party->party.party_id, party->party.member[i].account_id,
									 party->party.exp, flag);
			break;
		case 2:
			if (flag)
...
int party_changeleader(struct map_session_data *sd, struct map_session_data *tsd,
					   struct party_data *p) {
	int mi, tmi;
	...
	if (!p) {
		...
		if (battle_config.change_party_leader_samemap &&
			p->party.member[mi].map != p->party.member[tmi].map) {
	...
	// Update members
	clif_party_leaderchanged(p->data[mi].sd, p->data[mi].sd->status.account_id,
							 p->data[tmi].sd->status.account_id);

	// Update info.
	intif_party_leaderchange(p->party.party_id, p->party.member[tmi].account_id,
							 p->party.member[tmi].char_id);
	clif_party_info(*p);
...
int party_recv_movemap(int party_id, uint32 account_id, uint32 char_id, unsigned short map_idx,
					   int online, int lv) {
	...
	ARR_FIND(0, MAX_PARTY, i,
			 p->party.member[i].account_id == account_id && p->party.member[i].char_id == char_id);
	if (i == MAX_PARTY) {
		ShowError("party_recv_movemap: char %d/%d not found in party %s (id:%d)", account_id,
				  char_id, p->party.name, party_id);
		return 0;
	}
...
int party_send_message(struct map_session_data *sd, const char *mes, int len) {
	...
	log_chat(LOG_CHAT_PARTY, sd->status.party_id, sd->status.char_id, sd->status.account_id,
		...
		switch (skill_id) {
			...
			case MO_COMBOFINISH:  // Increase Counter rate of Star Gladiators
				if ((p_sd->class_ & MAPID_UPPERMASK) == MAPID_STAR_GLADIATOR &&
					p_sd->sc.data[SC_READYCOUNTER] && pc_checkskill(p_sd, SG_FRIEND)) {
	...
	for (p = (struct party_data *)dbi_first(iter); dbi_exists(iter);
		...
		for (i = 0; i < MAX_PARTY; i++) {
			...
			if (battle_config.party_hp_mode &&
				p->data[i].hp != sd->battle_status.hp) {  // perform hp update
				clif_party_hp(*sd);
 ...
 * @author Valaris
 **/
void party_exp_share(struct party_data *p, struct block_list *src, t_exp base_exp, t_exp job_exp,
					 int zeny) {
...
// Does party loot. first_charid holds the charid of the player who has time priority to take the
// item.
int party_share_loot(struct party_data *p, struct map_session_data *sd, struct item *item,
					 int first_charid) {
	TBL_PC *target = NULL;
	int i;
	...
	if (p && p->party.item & 2 && (first_charid || !(battle_config.party_share_type & 1))) {
		...
		if (battle_config.party_share_type & 2) {  // Round Robin
			...
			do {
				i++;
				if (i >= MAX_PARTY)
					i = 0;	// reset counter to 1st person in party so it'll stop when it reaches
							// "itemc"
			...
			while (count > 0) {	 // Pick a random member.
				...
				if (pc_additem(psd[i], item, item->amount,
							   LOG_TYPE_PICKDROP_PLAYER)) {	 // Discard this receiver.
					psd[i] = psd[count - 1];
					count--;
...
int party_foreachsamemap(int (*func)(struct block_list *, va_list), struct map_session_data *sd,
						 int range, ...) {
	struct party_data *p;
	int i;
	int x0, y0, x1, y1;
...
void party_booking_search(struct map_session_data *sd, short level, short mapid, short job,
						  unsigned long lastindex, short resultcount) {
	struct party_booking_ad_info *pb_ad;
	int i, count = 0;
...
bool party_booking_delete(struct map_session_data *sd) {
	...
	if ((pb_ad = (struct party_booking_ad_info *)idb_get(party_booking_db, sd->status.char_id)) !=
		NULL) {
