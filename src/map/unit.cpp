...
const short dirx[DIR_MAX] = {0, -1, -1, -1,
							 0, 1,	1,	1};	 /// lookup to know where will move to x according dir
const short diry[DIR_MAX] = {1,	 1,	 0, -1,
...
int unit_walktoxy_sub(struct block_list *bl) {
	...
	if (bl->type != BL_NPC	  // If type is a NPC, please disregard.
		&& wpd.path_len > 14  // Official number of walkable cells is 14 if and only if there is an
							  // obstacle between. [malufett]
		&& !path_search_long(nullptr, bl->m, bl->x, bl->y, ud->to_x, ud->to_y,
	...
	if (ud->target_to && ud->chaserange > 1) {
		...
		for (i = (ud->chaserange * 10) - 10; i > 0 && ud->walkpath.path_len > 1;) {
			...
			if (direction_diagonal(dir) && bl->type != BL_MOB)
				i -= MOVE_COST * 2;	 // When chasing, units will target a diamond-shaped area in
									 // range [Playtester]
			else
				i -= MOVE_COST;
			ud->to_x -= dirx[dir];
...
TIMER_FUNC(unit_step_timer) {
	...
	// Flush target_to as it might contain map coordinates which should not be used by other
	// functions
	target_id = ud->target_to;
	ud->target_to = 0;

	// If stepaction is set then we remembered a client request that should be executed on the next
	// step Execute request now if target is in attack range
	if (ud->stepskill_id && skill_get_inf(ud->stepskill_id) & INF_GROUND_SKILL) {
		// Execute ground skill
		struct map_data *md = &map[bl->m];
		unit_skilluse_pos(bl, target_id % md->xs, target_id / md->xs, ud->stepskill_id,
						  ud->stepskill_lv);
	} else {
			...
			for (size_t i = 0; i < sd->areanpc.size(); i++) {
				...
				if (!nd || nd->subtype != NPCTYPE_SCRIPT ||
					!(nd->bl.m == bl->m && bl->x >= nd->bl.x - nd->u.scr.xs &&
					  bl->x <= nd->bl.x + nd->u.scr.xs && bl->y >= nd->bl.y - nd->u.scr.ys &&
...
static TIMER_FUNC(unit_walktoxy_timer) {
	...
	if (md && !ud->state.force_walk && md->walktoxy_fail_count < icewall_walk_block &&
		...
		clif_fixpos(bl);
		// Monsters in this situation first use a chase skill, then unlock target and then use an
		// idle skill
	...
	ud->walktimer = CLIF_WALK_TIMER;  // Arbitrary non-INVALID_TIMER value to make the clif code
	...
	switch (bl->type) {
		case BL_PC:
			...
			if (map_getcell(bl->m, x, y, CELL_CHKNPC)) {
				...
				if (bl->prev ==
					NULL)  // Script could have warped char, abort remaining of the function.
					return 0;
			} else
				sd->areanpc.clear();
			pc_cell_basilica(sd);
		...
		case BL_MOB:
			...
			if (!ud->state.force_walk && tid != INVALID_TIMER &&
				!(ud->walk_count % WALK_SKILL_INTERVAL) && map[bl->m].users > 0 &&
				mobskill_use(md, tick, -1)) {
	...
	if (tid == INVALID_TIMER)  // A directly invoked timer is from battle_stop_walking, therefore
...
int unit_walktoxy(struct block_list *bl, short x, short y, unsigned char flag) {
	...
	if ((flag & 8) &&
		!map_closest_freecell(bl->m, &x, &y, BL_CHAR | BL_NPC, 1))	// This might change x and y
		return 0;
	...
	if (ud->walktimer != INVALID_TIMER) {
		// When you come to the center of the grid because the change of destination while you're
		// walking right now Call a function from a timer unit_walktoxy_sub
...
int unit_walktobl(struct block_list *bl, struct block_list *tbl, int range, unsigned char flag) {
	...
	if (!unit_can_reach_bl(bl, tbl, distance_bl(bl, tbl) + 1, flag & 1, &ud->to_x, &ud->to_y)) {
		...
		// Note: Easy walking is no longer used by default, but we keep this to prevent endless
		// loops [Playtester]
		flag &= ~1;
	}
 ...
 */
void unit_run_hit(struct block_list *bl, struct status_change *sc, struct map_session_data *sd,
				  enum sc_type type) {
...
int unit_escape(struct block_list *bl, struct block_list *target, short dist, uint8 flag) {
	...
	while (dist > 0 &&
		   map_getcell(bl->m, bl->x + dist * dirx[dir], bl->y + dist * diry[dir], CELL_CHKNOREACH))
		dist--;

	return (dist > 0 &&
			unit_walktoxy(bl, bl->x + dist * dirx[dir], bl->y + dist * diry[dir], flag));
}
...
bool unit_movepos(struct block_list *bl, short dst_x, short dst_y, int easy, bool checkpath) {
	...
	if (sd) {
		...
		if (sd->status.pet_id > 0 && sd->pd && sd->pd->pet.intimate > PET_INTIMATE_NONE) {
			...
			if (!checkpath &&
				!path_search(NULL, pbl->m, pbl->x, pbl->y, dst_x, dst_y, 0, CELL_CHKNOPASS))
				flag = 1;
 ...
 * This function checks for "no warp" map flags, so it's safe to call without doing nowarpto/nowarp
 * checks
...
int unit_warp(struct block_list *bl, short m, short x, short y, clr_type type) {
	...
	switch (bl->type) {
		case BL_MOB:
			...
			if (m != bl->m && map_getmapflag(m, MF_NOBRANCH) && battle_config.mob_warp & 4 &&
				!(((TBL_MOB *)bl)->master_id))
				return 1;
			break;
		case BL_PC:
...
int unit_stop_walking(struct block_list *bl, int type) {
	...
	if ((type & USW_MOVE_ONCE && !ud->walkpath.path_pos)  // Force moving at least one cell.
		|| (type & USW_MOVE_FULL_CELL && td &&
			DIFF_TICK(td->tick, tick) <= td->data / 2)	// Enough time has passed to cover half-cell
	) {
	...
	// Re-added, the check in unit_set_walkdelay means dmg during running won't fall through to this
	// place in code [Kevin]
	if (ud->state.running) {
 ...
 * @return Success(1); Fail(0);
 */
int unit_skilluse_id2(struct block_list *src, int target_id, uint16 skill_id, uint16 skill_lv,
					  int casttime, int castcancel) {
	struct unit_data *ud;
	...
	// Normally not needed because clif.cpp checks for it, but the at/char/script commands don't!
	// [Skotlex]
	...
	if (src->type == BL_NPC)  // NPC-objects can override cast distance
		...
		range =
			skill_get_range2(src, skill_id, skill_lv, true);  // Skill cast distance from database
	...
	switch (skill_id) {
...
#endif
		case GD_EMERGENCYCALL:	// Emergency Call double cast when the user has learned Leap
								// [Daegaladh]
		...
		case DK_SERVANT_W_PHANTOM: {  // Stops servants from being consumed on unmarked targets.
			...
			if (tsc == nullptr || tsc->data[SC_SERVANT_SIGN] == nullptr ||
	...
	if (!ud->state
			 .running)	// Need TK_RUN or WUGDASH handler to be done before that, see bugreport:6026
		unit_stop_walking(src, 1);	// Even though this is not how official works but this will do
									// the trick. bugreport:6829
	...
	if (!sd || sd->skillitem != skill_id || skill_get_cast(skill_id, skill_lv))
		ud->canact_tick =
			tick +
			i64max(casttime, max(status_get_amotion(src), battle_config.min_skill_delay_limit)) +
			SECURITY_CASTTIME;
 ...
 */
int unit_skilluse_pos(struct block_list *src, short skill_x, short skill_y, uint16 skill_id,
					  uint16 skill_lv) {
 ...
 * @return Success(1); Fail(0);
 */
int unit_skilluse_pos2(struct block_list *src, short skill_x, short skill_y, uint16 skill_id,
	...
	if (ud->skilltimer != INVALID_TIMER)  // Normally not needed since clif.cpp checks for it, but
...
int unit_set_target(struct unit_data *ud, int target_id) {
	...
	if (ud->target != target_id) {
		...
		if (ud->target && (target = map_id2bl(ud->target)) && (ux = unit_bl2ud(target)) &&
			ux->target_count > 0)
			ux->target_count--;
 ...
 * @return true or false
 */
bool unit_can_reach_bl(struct block_list *bl, struct block_list *tbl, int range, int easy, short *x,
					   short *y) {
	...
	if (map_getcell(tbl->m, tbl->x - dx, tbl->y - dy,
		...
		for (i = 0;
			 i < 8 && map_getcell(tbl->m, tbl->x - dirx[i], tbl->y - diry[i], CELL_CHKNOPASS); i++)
			;
	...
	if (bl->type != BL_NPC	  // If type is a NPC, please disregard.
		&& wpd.path_len > 14  // Official number of walkable cells is 14 if and only if there is an
							  // obstacle between. [malufett]
		&& !path_search_long(nullptr, bl->m, bl->x, bl->y, tbl->x - dx, tbl->y - dy,
							 CELL_CHKNOPASS))  // Check if there is an obstacle between
		return false;
#endif
	...
	if (DIFF_TICK(ud->attackabletime, tick) <= 0) {
		...
		if (md) {
			...
			if (status_has_mode(sstatus, MD_ASSIST) &&
				...
				map_foreachinrange(mob_linksearch, src, md->db->range2, BL_MOB, md->mob_id, target,
								   tick);
			}
		}
...
bool unit_can_attack(struct block_list *bl, int target_id) {
	...
	if (sc->cant.attack ||
		(sc->data[SC_VOICEOFSIREN] && sc->data[SC_VOICEOFSIREN]->val2 == target_id))
		return false;
		...
		if (sd &&
			(sd->special_state.no_castcancel2 ||
			 ((sd->sc.data[SC_UNLIMITEDHUMMINGVOICE] || sd->special_state.no_castcancel) &&
			  ...
			  !map_getmapflag(
				  bl->m,
				  MF_BATTLEGROUND))))  // fixed flags being read the wrong way around [blackhole89]
			return 0;
	}
 ...
 */
int unit_remove_map_(struct block_list *bl, clr_type clrtype, const char *file, int line,
					 const char *func) {
	struct unit_data *ud = unit_bl2ud(bl);
	...
	if (sc && sc->count) {	// map-change/warp dispells.
		...
		// Ensure the bl is a PC; if so, we'll handle the removal of cloaking and cloaking exceed
		// later
		if (bl->type != BL_PC) {
		...
		case BL_PC: {
			...
			if (map[bl->m].users <= 0 || sd->state.debug_remove_map) {
				...
				ShowDebug(
					...
					sd->state.debug_remove_map, map[bl->m].name, map[bl->m].users, sd->debug_file,
...
void unit_remove_map_pc(struct map_session_data *sd, clr_type clrtype) {
	...
	// CLR_RESPAWN is the warp from logging out, CLR_TELEPORT is the warp from teleporting, but
	// pets/homunc need to just 'vanish' instead of showing the warping animation.
...
int unit_free(struct block_list *bl, clr_type clrtype) {
	...
	switch (bl->type) {
		case BL_PC: {
			...
			map_foreachpc(clif_friendslist_toggle_sub, sd->status.account_id, sd->status.char_id,
						  0);
		...
		case BL_MOB: {
			...
			if (md->spawn) {
				...
				if (!md->spawn->state.dynamic) {  // Permanently remove the mob
					if (--md->spawn->num == 0) {  // Last freed mob is responsible for deallocating
												  // the group's spawn data.
						aFree(md->spawn);
						md->spawn = NULL;
					}
				}
...
void unit_addshadowscar(unit_data &ud, int interval) {
	if (ud.shadow_scar_timer.size() >= MAX_SHADOW_SCAR) {
		ShowWarning(
			"unit_addshadowscar: Unit %s (%d) has reached the maximum amount of Shadow Scars "
			"(%d).\n",
			status_get_name(ud.bl), ud.bl->id, MAX_SHADOW_SCAR);
		return;
	}

	ud.shadow_scar_timer.push_back(
		add_timer(gettick() + interval, unit_shadowscar_timer, ud.bl->id, 0));
