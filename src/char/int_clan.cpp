...
int inter_clan_removemember_tosql(uint32 account_id, uint32 char_id) {
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `clan_id` = '0' WHERE `char_id` = '%d'",
...
struct clan* inter_clan_fromsql(int clan_id) {
	...
	if (clan->max_member > MAX_CLAN) {
		ShowWarning("Clan %d:%s specifies higher capacity (%d) than MAX_CLAN (%d)\n", clan_id,
					clan->name, clan->max_member, MAX_CLAN);
...
int mapif_clan_info(int fd) {
	...
	for (clan = (struct clan*)dbi_first(iter); dbi_exists(iter);
		 clan = (struct clan*)dbi_next(iter)) {
...
int inter_clan_init(void) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "SELECT `clan_id` FROM `%s`", schema_config.clan_table)) {
		Sql_ShowDebug(sql_handle);
		return 1;
	}
...
void inter_clan_final() { db_destroy(clan_db); }
