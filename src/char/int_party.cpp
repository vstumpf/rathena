...
int mapif_parse_PartyLeave(int fd, int party_id, uint32 account_id, uint32 char_id, char *name,
...
void int_party_calc_state(struct party_data *p) {
	...
	if (p->size == 2 && (char_child(p->party.member[0].char_id, p->party.member[1].char_id) ||
		...
		// Check Family State.
		p->family = char_family(p->party.member[0].char_id, p->party.member[1].char_id,
...
int inter_party_tosql(struct party *p, int flag, int index) {
	...
	if (flag & PS_BREAK) {	// Break the party
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d'",
								   schema_config.char_db, party_id))
			Sql_ShowDebug(sql_handle);
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `party_id`='%d'",
	...
	if (flag & PS_CREATE) {	 // Create party
		if (SQL_ERROR == Sql_Query(sql_handle,
								   ...
								   "VALUES ('%s', '%d', '%d', '%d', '%d')",
								   schema_config.party_db, esc_name, p->exp, p->item,
								   p->member[index].account_id, p->member[index].char_id)) {
			Sql_ShowDebug(sql_handle);
			return 0;
		}
	...
	if (flag & PS_LEADER) {	 // Update leader
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "UPDATE `%s`  SET `leader_id`='%d', `leader_char`='%d' WHERE `party_id`='%d'",
					  schema_config.party_db, p->member[index].account_id, p->member[index].char_id,
					  party_id))
			Sql_ShowDebug(sql_handle);
	}
	...
	if (flag & PS_ADDMEMBER) {	// Add one party member.
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "UPDATE `%s` SET `party_id`='%d' WHERE `account_id`='%d' AND `char_id`='%d'",
					  schema_config.char_db, party_id, p->member[index].account_id,
	...
	if (flag & PS_DELMEMBER) {	// Remove one party member.
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "UPDATE `%s` SET `party_id`='0' WHERE `party_id`='%d' AND "
...
struct party_data *inter_party_fromsql(int party_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `party_id`, `name`,`exp`,`item`, `leader_id`, `leader_char` "
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `account_id`,`char_id`,`name`,`base_level`,`last_map`,`online`,`class` "
				  "FROM `%s` WHERE `party_id`='%d'",
				  schema_config.char_db, party_id)) {
		Sql_ShowDebug(sql_handle);
		return NULL;
...
int inter_party_sql_init(void) {
	...
	/* Uncomment the following if you want to do a party_db cleanup (remove parties with no members)
	on startup.[Skotlex] ShowStatus("cleaning party table...\n"); if( SQL_ERROR ==
	Sql_Query(sql_handle, "DELETE FROM `%s` USING `%s` LEFT JOIN `%s` ON `%s`.leader_id
	=`%s`.account_id AND `%s`.leader_char = `%s`.char_id WHERE `%s`.account_id IS NULL", party_db,
	party_db, char_db, party_db, char_db, party_db, char_db, char_db) ) Sql_ShowDebug(sql_handle);
	*/
	return 0;
}
...
struct party_data *search_partyname(char *str) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `party_id` FROM `%s` WHERE `name`='%s'",
							   schema_config.party_db, esc_name))
		Sql_ShowDebug(sql_handle);
...
int mapif_party_withdraw(int party_id, uint32 account_id, uint32 char_id, char *name,
						 enum e_party_member_withdraw type) {
...
int mapif_parse_PartyAddMember(int fd, int party_id, struct party_member *member) {
	...
	p->size++;
	if (p->size == 2 ||
		p->size == 3)  // Check family state. And also accept either of their Parents. [RoM]
		int_party_calc_state(p);
	...
	if (p->party.member[i].leader) {
		...
		for (j = 0; j < MAX_PARTY; j++) {
			if (!p->party.member[j].account_id)
				continue;
			mapif_party_withdraw(party_id, p->party.member[j].account_id,
								 p->party.member[j].char_id, p->party.member[j].name, type);
...
int mapif_parse_PartyChangeMap(int fd, int party_id, uint32 account_id, uint32 char_id,
							   unsigned short map, int online, unsigned int lv) {
	struct party_data *p;
	int i;
	...
	for (i = 0; i < MAX_PARTY && (p->party.member[i].account_id != account_id ||
...
int mapif_parse_PartyShareLevel(int fd, unsigned int share_lvl) {
	...
	for (p = (struct party_data *)dbi_first(iter); dbi_exists(iter);
		 p = (struct party_data *)dbi_next(iter)) {	 // Update online parties
		if (p->party.count > 1)
			int_party_calc_state(p);
	}
	dbi_destroy(iter);
...
int inter_party_parse_frommap(int fd) {
	...
	switch (RFIFOW(fd, 0)) {
		case 0x3020:
			mapif_parse_CreateParty(fd, RFIFOCP(fd, 4), RFIFOB(fd, 28), RFIFOB(fd, 29),
									(struct party_member *)RFIFOP(fd, 30));
			break;
		case 0x3021:
		...
		case 0x3022:
			...
			break;
		case 0x3023:
			mapif_parse_PartyChangeOption(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOW(fd, 10),
										  RFIFOW(fd, 12));
			break;
		case 0x3024:
			...
			break;
		case 0x3025:
			mapif_parse_PartyChangeMap(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10),
									   RFIFOW(fd, 14), RFIFOB(fd, 16), RFIFOW(fd, 17));
			break;
		case 0x3026:
			mapif_parse_BreakParty(fd, RFIFOL(fd, 2));
			break;
		case 0x3027:
			mapif_parse_PartyMessage(fd, RFIFOL(fd, 4), RFIFOL(fd, 8), RFIFOCP(fd, 12),
									 RFIFOW(fd, 2) - 12);
			break;
		case 0x3029:
...
int inter_party_leave(int party_id, uint32 account_id, uint32 char_id, char *name) {
	return mapif_parse_PartyLeave(-1, party_id, account_id, char_id, name,
...
int inter_party_CharOnline(uint32 char_id, int party_id) {
	...
	if (party_id == -1) {  // Get party_id from the database
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT party_id FROM `%s` WHERE char_id='%d'",
...
int inter_party_CharOffline(uint32 char_id, int party_id) {
	...
	if (!p->party.count)
		// Parties don't have any data that needs be saved at this point... so just remove it from
		// memory.
		idb_remove(party_db_, party_id);
	return 1;
}
