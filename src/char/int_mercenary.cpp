...
bool mercenary_owner_fromsql(uint32 char_id, struct mmo_charstatus* status) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `merc_id`, `arch_calls`, `arch_faith`, `spear_calls`, `spear_faith`, "
				  "`sword_calls`, `sword_faith` FROM `%s` WHERE `char_id` = '%d'",
...
bool mercenary_owner_tosql(uint32 char_id, struct mmo_charstatus* status) {
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "`spear_calls`, `spear_faith`, `sword_calls`, `sword_faith`) VALUES ('%d', '%d', "
				  "'%d', '%d', '%d', '%d', '%d', '%d')",
				  schema_config.mercenary_owner_db, char_id, status->mer_id, status->arch_calls,
				  status->arch_faith, status->spear_calls, status->spear_faith, status->sword_calls,
				  status->sword_faith)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
bool mercenary_owner_delete(uint32 char_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.mercenary_db, char_id))
		Sql_ShowDebug(sql_handle);
...
bool mapif_mercenary_save(struct s_mercenary* merc) {
	...
	if (merc->mercenary_id == 0) {	// Create new DB entry
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "INSERT INTO `%s` (`char_id`,`class`,`hp`,`sp`,`kill_counter`,`life_time`) "
					  "VALUES ('%d','%d','%u','%u','%u','%" PRtf "')",
			...
			merc->mercenary_id = (int)Sql_LastInsertId(sql_handle);
			   ...
			   Sql_Query(
				   ...
				   schema_config.mercenary_db, merc->char_id, merc->class_, merc->hp, merc->sp,
				   merc->kill_count, merc->life_time, merc->mercenary_id)) {  // Update DB entry
		Sql_ShowDebug(sql_handle);
		flag = false;
	}
...
bool mapif_mercenary_load(int merc_id, uint32 char_id, struct s_mercenary* merc) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `class`, `hp`, `sp`, `kill_counter`, `life_time` FROM `%s` "
							   "WHERE `mer_id` = '%d' AND `char_id` = '%d'",
	...
	if (charserv_config.save_log)
		ShowInfo("Mercenary loaded (ID: %d / Class: %d / CID: %d).\n", merc->mercenary_id,
				 merc->class_, merc->char_id);

	return true;
}

bool mapif_mercenary_delete(int merc_id) {
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `mer_id` = '%d'",
							   schema_config.mercenary_db, merc_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
int inter_mercenary_sql_init(void) { return 0; }
void inter_mercenary_sql_final(void) { return; }
