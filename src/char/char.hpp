...
struct Char_Config {
	...
	bool name_ignoring_case;  // Allow or not identical name for characters but with a different
							  // case by [Yor]
	char
		unknown_char_name[NAME_LENGTH];	 // Name to use when the requested name cannot be determined
	char char_name_letters[1024];  // list of letters/symbols allowed (or not) in a character name.
								   // by [Yor]
	int char_name_option;  // Option to know which letters/symbols are authorised in the name of a
						   // character (0: all, 1: only those in char_name_letters, 2: all EXCEPT
						   // those in char_name_letters) by [Yor]
	int char_del_option;   // Character deletion type, email = 1, birthdate = 2 (default)
	int char_del_restriction;  // Character deletion restriction (0: none, 1: if the character is in
							   // a party, 2: if the character is in a guild, 3: if the character is
...
#define TRIM_CHARS \
	"\255\xA0\032\t\x0A\x0D "  // The following characters are trimmed regardless because they cause
							   // confusion and problems on the servers. [Skotlex]
...
struct auth_node {
	...
	uint32 ip;
	int sex;
	time_t expiration_time;	 // # of seconds 1/1/1970 (timestamp): Validity limit of the account (0
							 // = unlimited)
	int group_id;
...
#define char_unset_session_flag(account_id, val) \
	(char_set_session_flag_((account_id), (val), false))
