...
void mapif_Mail_return(int fd, uint32 char_id, int mail_id, uint32 account_id_receiver = 0,
					   uint32 account_id_sender = 0);
	...
	for (i = 0; i < md->amount; i++) {
		...
		if (msg->status == MAIL_NEW) {
			if (SQL_ERROR == Sql_Query(sql_handle,
									   ...
									   schema_config.mail_db, MAIL_UNREAD, msg->id))
				Sql_ShowDebug(sql_handle);
...
int mail_savemessage(struct mail_message* msg) {
	...
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`send_name`, `send_id`, `dest_name`, `dest_id`, `title`, "
					 "`message`, `time`, `status`, `zeny`,`type`",
					 schema_config.mail_db);
	StringBuf_Printf(&buf, ") VALUES (?, '%d', ?, '%d', ?, ?, '%lu', '%d', '%d', '%d'",
					 msg->send_id, msg->dest_id, (unsigned long)msg->timestamp, msg->status,
					 msg->zeny, msg->type);
	...
	if (SQL_SUCCESS != SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, msg->send_name,
										 strnlen(msg->send_name, NAME_LENGTH)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, msg->dest_name,
										 strnlen(msg->dest_name, NAME_LENGTH)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 2, SQLDT_STRING, msg->title,
										 strnlen(msg->title, MAIL_TITLE_LENGTH)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 3, SQLDT_STRING, msg->body,
										 strnlen(msg->body, MAIL_BODY_LENGTH)) ||
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s` (`id`, `index`, `amount`, `nameid`, `refine`, `attribute`, "
					 "`identify`, `unique_id`, `bound`, `enchantgrade`",
	...
	for (i = 0; i < MAIL_MAX_ITEM; i++) {
		...
		StringBuf_Printf(
			&buf, "('%" PRIu64 "', '%hu', '%d', '%u', '%d', '%d', '%d', '%" PRIu64 "', '%d', '%d'",
			(uint64)msg->id, i, msg->item[i].amount, msg->item[i].nameid, msg->item[i].refine,
			msg->item[i].attribute, msg->item[i].identify, msg->item[i].unique_id,
			msg->item[i].bound, msg->item[i].enchantgrade);
		for (j = 0; j < MAX_SLOTS; j++) StringBuf_Printf(&buf, ", '%u'", msg->item[i].card[j]);
		for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
...
bool mail_loadmessage(int mail_id, struct mail_message* msg) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "`id`,`send_name`,`send_id`,`dest_name`,`dest_id`,`title`,`message`,"
							   "`time`,`status`,`zeny`,`type` FROM `%s` WHERE `id` = '%d'",
		...
		if (msg->type == MAIL_INBOX_NORMAL && charserv_config.mail_return_days > 0) {
			msg->scheduled_deletion =
				msg->timestamp + charserv_config.mail_return_days * 24 * 60 * 60;
		} else if (msg->type == MAIL_INBOX_RETURNED && charserv_config.mail_delete_days > 0) {
			msg->scheduled_deletion =
				msg->timestamp + charserv_config.mail_delete_days * 24 * 60 * 60;
		} else {
	...
	StringBuf_AppendStr(
		&buf,
		"SELECT "
		"`amount`,`nameid`,`refine`,`attribute`,`identify`,`unique_id`,`bound`,`enchantgrade`");
	for (j = 0; j < MAX_SLOTS; j++) StringBuf_Printf(&buf, ",`card%d`", j);
	for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
		StringBuf_Printf(&buf, ", `option_id%d`", j);
		StringBuf_Printf(&buf, ", `option_val%d`", j);
	...
	for (i = 0; i < MAIL_MAX_ITEM && SQL_SUCCESS == Sql_NextRow(sql_handle); i++) {
		...
		for (j = 0; j < MAX_ITEM_RDM_OPT; j++) {
			...
			Sql_GetData(sql_handle, 10 + MAX_SLOTS + j * 3, &data, NULL);
...
int mail_timer_sub(int limit, enum mail_inbox_type type) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s`WHERE `type` = '%d' AND `send_id` = '0' AND `time` "
							   "<= UNIX_TIMESTAMP( NOW() - INTERVAL %d DAY )",
	...
	for (int i = 0; i < MAIL_ITERATION_SIZE; i++) {
		...
		if (type == MAIL_INBOX_NORMAL) {
			mapif_Mail_return(0, mails[i].char_id, mails[i].mail_id, mails[i].account_id,
							  mails[i].account_id_sender);
...
void mapif_parse_Mail_read(int fd) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `status` = '%d' WHERE `id` = '%d'",
							   schema_config.mail_db, MAIL_READ, mail_id))
		Sql_ShowDebug(sql_handle);
}
...
bool mail_DeleteAttach(int mail_id) {
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'",
							   schema_config.mail_attachment_db, mail_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
void mapif_Mail_getattach(int fd, uint32 char_id, int mail_id, int type) {
	...
	if (type & MAIL_ATT_ZENY) {
		if (msg.zeny > 0) {
			if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `zeny` = 0 WHERE `id` = '%d'",
									   schema_config.mail_db, mail_id)) {
				Sql_ShowDebug(sql_handle);
				return;
			}
...
bool mapif_Mail_delete(int fd, uint32 char_id, int mail_id, uint32 account_id) {
	...
	if (SQL_ERROR == Sql_QueryStr(sql_handle, "START TRANSACTION") ||
		SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'",
							   schema_config.mail_db, mail_id) ||
		SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `id` = '%d'",
							   schema_config.mail_attachment_db, mail_id) ||
	...
	if (fd <= 0) {
		...
		if ((character = (struct online_char_data*)idb_get(char_get_onlinedb(), account_id)) !=
				nullptr &&
			character->server >= 0) {
...
void mapif_parse_Mail_delete(int fd) { mapif_Mail_delete(fd, RFIFOL(fd, 2), RFIFOL(fd, 6)); }
 ...
 * Return Mail
 *------------------------------------------*/
void mapif_Mail_return(int fd, uint32 char_id, int mail_id, uint32 account_id_receiver,
					   uint32 account_id_sender) {
	struct mail_message msg;
	...
	// If we do not want to return mails without any attachments and the request was not sent by a
	// user
	...
	if (fd <= 0) {
		...
		// Check for online players
		if ((character = (struct online_char_data*)idb_get(char_get_onlinedb(),
														   account_id_sender)) != nullptr &&
			character->server >= 0) {
			fd = map_server[character->server].fd;
		} else {
			// The request was triggered inside the character server or the player is offline now
...
void mapif_parse_Mail_return(int fd) { mapif_Mail_return(fd, RFIFOL(fd, 2), RFIFOL(fd, 6)); }
...
bool mail_sendmail(int send_id, const char* send_name, int dest_id, const char* dest_name,
				   const char* title, const char* body, int zeny, struct item* item, int amount) {
	struct mail_message msg;
...
void mapif_Mail_receiver_send(int fd, int requesting_char_id, int char_id, int class_,
							  int base_level, const char* name) {
	WFIFOHEAD(fd, 38);
	WFIFOW(fd, 0) = 0x384e;
...
void mapif_parse_Mail_receiver_check(int fd) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "SELECT `char_id`,`class`,`base_level` FROM `%s` WHERE `name` = '%s'",
				  schema_config.char_db, esc_name)) {
		Sql_ShowDebug(sql_handle);
	} else if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		char* data;
...
int inter_mail_sql_init(void) { return 1; }

void inter_mail_sql_final(void) { return; }
