...
struct quest *mapif_quests_fromsql(uint32 char_id, int *count) {
	...
	if (SQL_ERROR == SqlStmt_Prepare(stmt,
									 "SELECT `quest_id`, `state`, `time`, `count1`, `count2`, "
									 "`count3` FROM `%s` WHERE `char_id`=? ",
...
bool mapif_quest_add(uint32 char_id, struct quest qd) {
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "INSERT INTO `%s`(`quest_id`, `char_id`, `state`, `time`, `count1`, `count2`, "
				  "`count3`) VALUES ('%d', '%d', '%d', '%u', '%d', '%d', '%d')",
				  schema_config.quest_db, qd.quest_id, char_id, qd.state, qd.time, qd.count[0],
				  qd.count[1], qd.count[2])) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
bool mapif_quest_update(uint32 char_id, struct quest qd) {
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "UPDATE `%s` SET `state`='%d', `count1`='%d', `count2`='%d', "
							   "`count3`='%d' WHERE `quest_id` = '%d' AND `char_id` = '%d'",
							   schema_config.quest_db, qd.state, qd.count[0], qd.count[1],
							   qd.count[2], qd.quest_id, char_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
