	...
	if (pet_id == -1) {	 // New pet.
		if (SQL_ERROR ==
			Sql_Query(
				sql_handle,
				"INSERT INTO `%s` "
				"(`class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`"
				"hungry`,`rename_flag`,`incubate`,`autofeed`) "
				"VALUES ('%d', '%s', '%d', '%d', '%d', '%u', '%u', '%d', '%d', '%d', '%d', '%d')",
				schema_config.pet_db, p->class_, esc_name, p->account_id, p->char_id, p->level,
		...
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "UPDATE `%s` SET "
					  "`class`='%d',`name`='%s',`account_id`='%d',`char_id`='%d',`level`='%d',`egg_"
					  "id`='%u',`equip`='%u',`intimate`='%d',`hungry`='%d',`rename_flag`='%d',`"
					  "incubate`='%d',`autofeed`='%d' WHERE `pet_id`='%d'",
					  schema_config.pet_db, p->class_, esc_name, p->account_id, p->char_id,
					  p->level, p->egg_id, p->equip, p->intimate, p->hungry, p->rename_flag,
					  p->incubate, p->autofeed, p->pet_id)) {
			Sql_ShowDebug(sql_handle);
			return 0;
		}
	}
...
int inter_pet_fromsql(int pet_id, struct s_pet *p) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `pet_id`, "
				  "`class`,`name`,`account_id`,`char_id`,`level`,`egg_id`,`equip`,`intimate`,`"
				  "hungry`,`rename_flag`,`incubate`,`autofeed` FROM `%s` WHERE `pet_id`='%d'",
...
int inter_pet_delete(int pet_id) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `pet_id`='%d'", schema_config.pet_db, pet_id))
		Sql_ShowDebug(sql_handle);
	return 0;
}
//------------------------------------------------------
...
int mapif_create_pet(int fd, uint32 account_id, uint32 char_id, short pet_class, short pet_lv,
					 t_itemid pet_egg_id, t_itemid pet_equip, short intimate, short hungry,
					 char rename_flag, char incubate, char *pet_name) {
...
int mapif_parse_CreatePet(int fd) {
	RFIFOHEAD(fd);
	mapif_create_pet(fd, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOW(fd, 10), RFIFOW(fd, 12),
					 RFIFOL(fd, 14), RFIFOL(fd, 18), RFIFOW(fd, 22), RFIFOW(fd, 24), RFIFOB(fd, 26),
					 RFIFOB(fd, 27), RFIFOCP(fd, 28));
	return 0;
}
