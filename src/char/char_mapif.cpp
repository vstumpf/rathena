 ...
 * - Default map
 * HZ 0x2afb <size>.W <status>.B <whisper name>.24B <mapname>.11B <map_x>.W <map_y>.W <server
 *name>.24B
 * @param fd
 **/
void chmapif_send_misc(int fd) {
	...
	// Default map
	safestrncpy(WBUFCP(buf, (offs += NAME_LENGTH)), charserv_config.default_map,
				MAP_NAME_LENGTH);											// 29
	WBUFW(buf, (offs += MAP_NAME_LENGTH)) = charserv_config.default_map_x;	// 41
	WBUFW(buf, (offs += 2)) = charserv_config.default_map_y;				// 43
	offs += 2;
	safestrncpy(WBUFCP(buf, offs), charserv_config.server_name,
				sizeof(charserv_config.server_name));  // 45
	offs += NAME_LENGTH;
 ...
 * Map-serv sent us his map indexes so we can transfert a player from a map-serv to another when
 * necessary We reply by sending back the char_serv_wisp_name  fame list and
...
int chmapif_parse_getmapname(int fd, int id) {
	...
	map_server[id].map = {};
	for (i = 4; i < RFIFOW(fd, 2); i += 4) map_server[id].map.push_back(RFIFOW(fd, i));

	mapbuf = RFIFOP(fd, 4);
	RFIFOSKIP(fd, RFIFOW(fd, 2));

	ShowStatus("Map-Server %d connected: %" PRIuPTR " maps, from IP %d.%d.%d.%d port %d.\n", id,
			   map_server[id].map.size(), CONVIP(map_server[id].ip), map_server[id].port);
...
int chmapif_parse_askscdata(int fd) {
	if (RFIFOREST(fd) < 10)
		...
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "SELECT type, tick, val1, val2, val3, val4 from `%s` WHERE "
								   "`account_id` = '%d' AND `char_id`='%d'",
		...
		if (Sql_NumRows(sql_handle) > 0) {
			...
			for (count = 0; count < 50 && SQL_SUCCESS == Sql_NextRow(sql_handle); ++count) {
				...
				memcpy(WFIFOP(fd, 14 + count * sizeof(struct status_change_data)), &scdata,
	...
	if (RFIFOREST(fd) < 6 || RFIFOREST(fd) < RFIFOW(fd, 2))
		...
		for (i = 0; i < map_server[id].users; i++) {
			...
			if (character->server > -1 && character->server != id) {
				ShowNotice(
					"Set map user: Character (%d:%d) marked on map server %d, but map server %d "
					"claims to have (%d:%d) online!\n",
...
int chmapif_parse_reqsavechar(int fd, int id) {
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
		...
		// Check account only if this ain't final save. Final-save goes through because of the
		// char-map reconnect
		...
		} else {  // This may be valid on char-server reconnection, when re-sending characters that
				  // already logged off.
			ShowError(
				"parse_from_map (save-char): Received data for non-existant/offline character "
				"(%d:%d).\n",
				aid, cid);
...
int chmapif_parse_authok(int fd) {
	if (RFIFOREST(fd) < 18)
		...
		if (runflag != CHARSERVER_ST_RUNNING) {
			...
			// node->sex = 0;
			node->ip = ntohl(ip);
			// node->expiration_time = 0; // unlimited/unknown time by default (not display in
			// map-server) node->gmlevel = 0;
			...
			{
				struct online_char_data* character =
					(struct online_char_data*)idb_get(online_char_db, account_id);
				if (character != NULL) {
...
int chmapif_parse_req_saveskillcooldown(int fd) {
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
		...
		if (count > 0) {
			...
			for (i = 0; i < count; ++i) {
				memcpy(&data, RFIFOP(fd, 14 + i * sizeof(struct skill_cooldown_data)),
					   sizeof(struct skill_cooldown_data));
				if (i > 0)
					StringBuf_AppendStr(&buf, ", ");
				StringBuf_Printf(&buf, "('%d','%d','%d','%" PRtf "')", aid, cid, data.skill_id,
								 data.tick);
			}
...
int chmapif_parse_req_skillcooldown(int fd) {
	if (RFIFOREST(fd) < 10)
		...
		if (Sql_NumRows(sql_handle) > 0) {
			...
			WFIFOL(fd, 4) = aid;
			WFIFOL(fd, 8) = cid;
			for (count = 0; count < MAX_SKILLCOOLDOWN && SQL_SUCCESS == Sql_NextRow(sql_handle);
				 ++count) {
				Sql_GetData(sql_handle, 0, &data, NULL);
				scd.skill_id = atoi(data);
				Sql_GetData(sql_handle, 1, &data, NULL);
				scd.tick = strtoll(data, nullptr, 10);
				memcpy(WFIFOP(fd, 14 + count * sizeof(struct skill_cooldown_data)), &scd,
					   sizeof(struct skill_cooldown_data));
			}
...
int chmapif_parse_reqchangemapserv(int fd) {
	if (RFIFOREST(fd) < 39)
		...
		map_id = char_search_mapserver(RFIFOW(fd, 18), ntohl(RFIFOL(fd, 24)),
									   ntohs(RFIFOW(fd, 28)));	// Locate mapserver by ip and port.
		if (map_id >= 0)
		...
		if (runflag == CHARSERVER_ST_RUNNING && session_isActive(map_fd) &&
			...
			// Update the "last map" as this is where the player must be spawned on the new map
			// server.
			...
			node->expiration_time = 0;	// FIXME (this thing isn't really supported we could as well
										// purge it instead of fixing)
...
int chmapif_parse_reqnewemail(int fd) {
	...
	if (chlogif_isconnected()) {  // don't send request if no login-server
		...
		memcpy(WFIFOP(login_fd, 0), RFIFOP(fd, 0),
			   86);	 // 0x2722 <account_id>.L <actual_e-mail>.40B <new_e-mail>.40B
...
int chmapif_parse_fwlog_changestatus(int fd) {
	if (RFIFOREST(fd) < 44)
		...
		int result = 0;	 // 0-login-server request done, 1-player not found, 2-gm level too low,
						 // 3-login-server offline, 4-current group level > VIP group level
			...
			//		result = 2; // 2-gm level too low
				...
				switch (operation) {
					...
					case CHRIF_OP_LOGIN_VIP:  // vip
						answer = (val1 & 4);  // vip_req val1=type, &1 login send return, &2 update
											  // timestamp, &4 map send answer
...
int chmapif_parse_save_scdata(int fd) {
		...
		return 0;
	{
		...
		if (Sql_Query(sql_handle, "DELETE FROM `%s` where `account_id` = %d and `char_id` = %d;",
					  schema_config.scdata_db, aid, cid) != SQL_SUCCESS) {
			Sql_ShowDebug(sql_handle);
		} else if (count > 0) {
			...
			StringBuf_Init(&buf);
			StringBuf_Printf(&buf,
							 "INSERT INTO `%s` (`account_id`, `char_id`, `type`, `tick`, `val1`, "
							 "`val2`, `val3`, `val4`) VALUES ",
							 schema_config.scdata_db);
			for (i = 0; i < count; ++i) {
				memcpy(&data, RFIFOP(fd, 14 + i * sizeof(struct status_change_data)),
					   sizeof(struct status_change_data));
				if (i > 0)
					StringBuf_AppendStr(&buf, ", ");
				StringBuf_Printf(&buf, "('%d','%d','%hu','%" PRtf "','%ld','%ld','%ld','%ld')", aid,
								 cid, data.type, data.tick, data.val1, data.val2, data.val3,
								 data.val4);
			}
			if (SQL_ERROR == Sql_QueryStr(sql_handle, StringBuf_Value(&buf)))
				Sql_ShowDebug(sql_handle);
...
int chmapif_parse_reqauth(int fd, int id) {
	if (RFIFOREST(fd) < 20)
		...
		} else if (runflag == CHARSERVER_ST_RUNNING && cd != NULL && node != NULL &&
		...
		//&& node->ip == ip
			...
			WFIFOL(fd, 16) = (uint32)node->expiration_time;	 // FIXME: will wrap to negative after
	...
	if (RFIFOREST(fd) < 10 + NAME_LENGTH)
		...
		if (SQL_ERROR ==
			...
			if (SQL_SUCCESS !=
				...
				SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_LONG, (void*)&unban_time,
												 sizeof(unban_time)) ||
				SQL_SUCCESS !=
					SqlStmt_BindParam(stmt, 1, SQLDT_INT, (void*)&t_cid, sizeof(t_cid)) ||
...
int chmapif_bonus_script_get(int fd) {
	if (RFIFOREST(fd) < 6)
		...
		if (SQL_ERROR == SqlStmt_Prepare(stmt,
										 "SELECT `script`, `tick`, `flag`, `type`, `icon` FROM "
										 "`%s` WHERE `char_id` = '%d' LIMIT %d",
										 schema_config.bonus_script_db, cid, MAX_PC_BONUS_SCRIPT) ||
			SQL_ERROR == SqlStmt_Execute(stmt) ||
			SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &tmp_bsdata.script_str,
											sizeof(tmp_bsdata.script_str), NULL, NULL) ||
			SQL_ERROR ==
				SqlStmt_BindColumn(stmt, 1, SQLDT_INT64, &tmp_bsdata.tick, 0, NULL, NULL) ||
			SQL_ERROR ==
				SqlStmt_BindColumn(stmt, 2, SQLDT_UINT16, &tmp_bsdata.flag, 0, NULL, NULL) ||
			SQL_ERROR ==
				SqlStmt_BindColumn(stmt, 3, SQLDT_UINT8, &tmp_bsdata.type, 0, NULL, NULL) ||
			SQL_ERROR ==
				SqlStmt_BindColumn(stmt, 4, SQLDT_INT16, &tmp_bsdata.icon, 0, NULL, NULL)) {
			SqlStmt_ShowDebug(stmt);
			SqlStmt_Free(stmt);
			return 1;
		}
		...
		if ((num_rows = (uint8)SqlStmt_NumRows(stmt)) > 0) {
			...
			for (i = 0; i < num_rows && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
				...
				safestrncpy(bsdata.script_str, tmp_bsdata.script_str,
							strlen(tmp_bsdata.script_str) + 1);
				bsdata.tick = tmp_bsdata.tick;
				bsdata.flag = tmp_bsdata.flag;
				bsdata.type = tmp_bsdata.type;
				bsdata.icon = tmp_bsdata.icon;
				memcpy(WFIFOP(fd, 9 + i * sizeof(struct bonus_script_data)), &bsdata,
			...
			if (SQL_ERROR == SqlStmt_Prepare(stmt, "DELETE FROM `%s` WHERE `char_id`='%d'",
...
int chmapif_bonus_script_save(int fd) {
	if (RFIFOREST(fd) < 4 || RFIFOREST(fd) < RFIFOW(fd, 2))
		...
		if (count > 0) {
			...
			for (i = 0; i < count; ++i) {
				memcpy(&bsdata, RFIFOP(fd, 9 + i * sizeof(struct bonus_script_data)),
					   sizeof(struct bonus_script_data));
				Sql_EscapeString(sql_handle, esc_script, bsdata.script_str);
				if (i > 0)
					StringBuf_AppendStr(&buf, ", ");
				StringBuf_Printf(&buf, "('%d','%s','%" PRtf "','%d','%d','%d')", cid, esc_script,
								 bsdata.tick, bsdata.flag, bsdata.type, bsdata.icon);
			}
			if (SQL_ERROR == Sql_QueryStr(sql_handle, StringBuf_Value(&buf)))
				Sql_ShowDebug(sql_handle);
...
int chmapif_init(int fd) { return inter_mapif_init(fd); }
...
void do_init_chmapif(void) {
	int i;
	for (i = 0; i < ARRAYLENGTH(map_server); ++i) chmapif_server_init(i);
}
...
void chmapif_server_reset(int id) {
	...
	online_char_db->foreach (online_char_db, char_db_setoffline,
							 id);  // Tag relevant chars as 'in disconnected' server.
...
void do_final_chmapif(void) {
	int i;
	for (i = 0; i < ARRAYLENGTH(map_server); ++i) chmapif_server_destroy(i);
}
