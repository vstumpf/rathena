...
void char_set_charselect(uint32 account_id) {
	...
	character =
		(struct online_char_data*)idb_ensure(online_char_db, account_id, char_create_online_data);
...
void char_set_char_online(int map_id, uint32 char_id, uint32 account_id) {
	...
	if (character->char_id != -1 && character->server > -1 && character->server != map_id) {
		ShowNotice(
			"set_char_online: Character %d:%d marked in map server %d, but map server %d claims to "
			"have (%d:%d) online!\n",
			character->account_id, character->char_id, character->server, map_id, account_id,
			char_id);
		mapif_disconnectplayer(map_server[character->server].fd, character->account_id,
							   character->char_id, 2);
	}
	...
	// Set char online in guild cache. If char is in memory, use the guild id on it, otherwise seek
	// it.
...
void char_set_char_offline(uint32 char_id, uint32 account_id) {
	...
	if (char_id == -1) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   ...
								   schema_config.char_db, account_id))
			Sql_ShowDebug(sql_handle);
	} else {
	...
	if ((character = (struct online_char_data*)idb_get(online_char_db, account_id)) !=
		NULL) {	 // We don't free yet to avoid aCalloc/aFree spamming during char change. [Skotlex]
		if (character->server > -1)
			if (map_server[character->server].users > 0)  // Prevent this value from going negative.
				map_server[character->server].users--;
		...
		if (character->char_id == char_id) {
			character->char_id = -1;
			character->server = -1;
			// needed if player disconnects completely since Skotlex did not want to free the
			// session
			character->pincode_success = false;
		}

		// FIXME? Why Kevin free'd the online information when the char was effectively in the
		// map-server?
	}
...
void char_set_all_offline_sql(void) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `online` = '0'", schema_config.char_db))
		Sql_ShowDebug(sql_handle);
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "UPDATE `%s` SET `connect_member` = '0'", schema_config.guild_db))
		Sql_ShowDebug(sql_handle);
}
...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p) {
	...
	if ((p->base_exp != cp->base_exp) || (p->base_level != cp->base_level) ||
		(p->job_level != cp->job_level) || (p->job_exp != cp->job_exp) || (p->zeny != cp->zeny) ||
		(p->last_point.map != cp->last_point.map) || (p->last_point.x != cp->last_point.x) ||
		(p->last_point.y != cp->last_point.y) || (p->max_hp != cp->max_hp) || (p->hp != cp->hp) ||
		(p->max_sp != cp->max_sp) || (p->sp != cp->sp) || (p->status_point != cp->status_point) ||
		(p->skill_point != cp->skill_point) || (p->str != cp->str) || (p->agi != cp->agi) ||
		(p->vit != cp->vit) || (p->int_ != cp->int_) || (p->dex != cp->dex) ||
		(p->luk != cp->luk) || (p->option != cp->option) || (p->party_id != cp->party_id) ||
		(p->guild_id != cp->guild_id) || (p->pet_id != cp->pet_id) || (p->weapon != cp->weapon) ||
		(p->hom_id != cp->hom_id) || (p->ele_id != cp->ele_id) || (p->shield != cp->shield) ||
		(p->head_top != cp->head_top) || (p->head_mid != cp->head_mid) ||
		(p->head_bottom != cp->head_bottom) || (p->delete_date != cp->delete_date) ||
		(p->rename != cp->rename) || (p->robe != cp->robe) ||
		(p->character_moves != cp->character_moves) || (p->unban_time != cp->unban_time) ||
		(p->font != cp->font) || (p->uniqueitem_counter != cp->uniqueitem_counter) ||
		(p->hotkey_rowshift != cp->hotkey_rowshift) || (p->clan_id != cp->clan_id) ||
		(p->title_id != cp->title_id) || (p->show_equip != cp->show_equip) ||
		(p->hotkey_rowshift2 != cp->hotkey_rowshift2) || (p->max_ap != cp->max_ap) ||
		(p->ap != cp->ap) || (p->trait_point != cp->trait_point) || (p->pow != cp->pow) ||
		...
		if (SQL_ERROR ==
			Sql_Query(
				...
				"`max_hp`='%u',`hp`='%u',`max_sp`='%u',`sp`='%u',`status_point`='%d',`skill_point`="
				"'%d',"
				"`str`='%d',`agi`='%d',`vit`='%d',`int`='%d',`dex`='%d',`luk`='%d',"
				"`option`='%d',`party_id`='%d',`guild_id`='%d',`pet_id`='%d',`homun_id`='%d',`"
				"elemental_id`='%d',"
				"`weapon`='%d',`shield`='%d',`head_top`='%d',`head_mid`='%d',`head_bottom`='%d',"
				"`last_map`='%s',`last_x`='%d',`last_y`='%d',`save_map`='%s',`save_x`='%d',`save_y`"
				"='%d', `rename`='%d',"
				...
				schema_config.char_db, p->base_level, p->job_level, p->base_exp, p->job_exp,
				p->zeny, p->max_hp, p->hp, p->max_sp, p->sp, p->status_point, p->skill_point,
				p->str, p->agi, p->vit, p->int_, p->dex, p->luk, p->option, p->party_id,
				p->guild_id, p->pet_id, p->hom_id, p->ele_id, p->weapon, p->shield, p->head_top,
				p->head_mid, p->head_bottom, mapindex_id2name(p->last_point.map), p->last_point.x,
				p->last_point.y, mapindex_id2name(p->save_point.map), p->save_point.x,
				p->save_point.y, p->rename,
				(unsigned long)p->delete_date,	// FIXME: platform-dependent size
				p->robe, p->character_moves, p->font, p->uniqueitem_counter, p->hotkey_rowshift,
				p->clan_id, p->title_id, p->show_equip, p->hotkey_rowshift2, p->max_ap, p->ap,
				p->trait_point, p->pow, p->sta, p->wis, p->spl, p->con, p->crt, p->account_id,
				p->char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		} else
	...
	if ((p->hair != cp->hair) || (p->hair_color != cp->hair_color) ||
		...
		(p->class_ != cp->class_) || (p->partner_id != cp->partner_id) ||
		(p->father != cp->father) || (p->mother != cp->mother) || (p->child != cp->child) ||
		(p->karma != cp->karma) || (p->manner != cp->manner) || (p->fame != cp->fame) ||
		(p->inventory_slots != cp->inventory_slots) || (p->body_direction != cp->body_direction) ||
		(p->disable_call != cp->disable_call)) {
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  ...
					  " WHERE  `account_id`='%d' AND `char_id` = '%d'",
					  schema_config.char_db, p->class_, p->hair, p->hair_color, p->clothes_color,
					  p->body, p->partner_id, p->father, p->mother, p->child, p->karma, p->manner,
					  p->fame, p->inventory_slots, p->body_direction, p->disable_call,
	...
	if (memcmp(p->memo_point, cp->memo_point, sizeof(p->memo_point))) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'",
								   schema_config.memo_db, p->char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		}
		...
		// insert here.
		StringBuf_Clear(&buf);
		StringBuf_Printf(&buf, "INSERT INTO `%s`(`char_id`,`map`,`x`,`y`) VALUES ",
		...
		for (i = 0, count = 0; i < MAX_MEMOPOINTS; ++i) {
			if (p->memo_point[i].map) {
				...
				StringBuf_Printf(&buf, "('%d', '%s', '%d', '%d')", char_id, esc_mapname,
	...
	if (memcmp(p->skill, cp->skill, sizeof(p->skill))) {
		//`skill` (`char_id`, `id`, `lv`)
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'",
								   schema_config.skill_db, p->char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		}

		StringBuf_Clear(&buf);
		StringBuf_Printf(&buf, "INSERT INTO `%s`(`char_id`,`id`,`lv`,`flag`) VALUES ",
	...
	if (diff == 1) {  // Save friends
		if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'",
								   schema_config.friend_db, char_id)) {
			Sql_ShowDebug(sql_handle);
			errors++;
		}

		StringBuf_Clear(&buf);
		StringBuf_Printf(&buf, "INSERT INTO `%s` (`char_id`, `friend_id`) VALUES ",
	...
	for (i = 0; i < ARRAYLENGTH(p->hotkeys); i++) {
		if (memcmp(&p->hotkeys[i], &cp->hotkeys[i], sizeof(struct hotkey))) {
			...
			StringBuf_Printf(&buf, "('%d','%u','%u','%u','%u')", char_id, (unsigned int)i,
...
int char_memitemdata_to_sql(const struct item items[], int max, int id,
							enum storage_type tableswitch, uint8 stor_id) {
	StringBuf buf;
	SqlStmt* stmt;
	...
	StringBuf_Init(&buf);
	StringBuf_AppendStr(&buf,
						"SELECT `id`, `nameid`, `amount`, `equip`, `identify`, `refine`, "
						"`attribute`, `expire_time`, `bound`, `unique_id`, `enchantgrade`");
	if (tableswitch == TABLE_INVENTORY) {
		StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
		offset = 2;
	}

	for (i = 0; i < MAX_SLOTS; ++i) StringBuf_Printf(&buf, ", `card%d`", i);
	...
	for (i = 0; i < MAX_ITEM_RDM_OPT; ++i) {
		SqlStmt_BindColumn(stmt, 11 + offset + MAX_SLOTS + i * 3, SQLDT_SHORT, &item.option[i].id,
						   0, NULL, NULL);
		SqlStmt_BindColumn(stmt, 12 + offset + MAX_SLOTS + i * 3, SQLDT_SHORT,
						   &item.option[i].value, 0, NULL, NULL);
		SqlStmt_BindColumn(stmt, 13 + offset + MAX_SLOTS + i * 3, SQLDT_CHAR, &item.option[i].param,
						   0, NULL, NULL);
	}
	...
	while (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
			...
			if (items[i].nameid == item.nameid && items[i].card[0] == item.card[0] &&
				...
				if (j == MAX_SLOTS && k == MAX_ITEM_RDM_OPT && items[i].amount == item.amount &&
					...
					(tableswitch != TABLE_INVENTORY || (items[i].favorite == item.favorite &&
														items[i].equipSwitch == item.equipSwitch)))
					;  // Do nothing.
				else {
					// update all fields.
					StringBuf_Clear(&buf);
					StringBuf_Printf(&buf,
									 "UPDATE `%s` SET `amount`='%d', `equip`='%u', "
									 "`identify`='%d', `refine`='%d',`attribute`='%d', "
									 "`expire_time`='%u', `bound`='%d', `unique_id`='%" PRIu64
									 "', `enchantgrade`='%d'",
									 tablename, items[i].amount, items[i].equip, items[i].identify,
									 items[i].refine, items[i].attribute, items[i].expire_time,
									 items[i].bound, items[i].unique_id, items[i].enchantgrade);
					if (tableswitch == TABLE_INVENTORY)
						StringBuf_Printf(&buf, ", `favorite`='%d', `equip_switch`='%u'",
										 items[i].favorite, items[i].equipSwitch);
		...
		if (!found) {  // Item not present in inventory, remove it.
			if (SQL_ERROR == Sql_Query(sql_handle, "DELETE from `%s` where `id`='%d' LIMIT 1",
									   tablename, item.id)) {
				Sql_ShowDebug(sql_handle);
				errors++;
			}
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf,
					 "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, "
					 "`attribute`, `expire_time`, `bound`, `unique_id`, `enchantgrade`",
					 tablename, selectoption);
	if (tableswitch == TABLE_INVENTORY)
		StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
	for (j = 0; j < MAX_SLOTS; ++j) StringBuf_Printf(&buf, ", `card%d`", j);
	for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
	...
	for (i = 0; i < max; ++i) {
		...
		StringBuf_Printf(
			&buf, "('%d', '%u', '%d', '%u', '%d', '%d', '%d', '%u', '%d', '%" PRIu64 "', '%d'", id,
			items[i].nameid, items[i].amount, items[i].equip, items[i].identify, items[i].refine,
		...
		for (j = 0; j < MAX_SLOTS; ++j) StringBuf_Printf(&buf, ", '%u'", items[i].card[j]);
		for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
...
bool char_memitemdata_from_sql(struct s_storage* p, int max, int id, enum storage_type tableswitch,
	...
	StringBuf_AppendStr(&buf,
						"SELECT "
						"`id`,`nameid`,`amount`,`equip`,`identify`,`refine`,`attribute`,`expire_"
						"time`,`bound`,`unique_id`,`enchantgrade`");
	if (tableswitch == TABLE_INVENTORY) {
		StringBuf_Printf(&buf, ", `favorite`, `equip_switch`");
		offset = 2;
	}
 ...
 *
 * If the per-character sex is defined but not supported by the current packetver, the account
 * gender is returned.
 *
...
int char_mmo_gender(const struct char_session_data* sd, const struct mmo_charstatus* p, char sex) {
	...
	// Per character gender is not supported before 2014-10-16
			...
			if (sd == nullptr) {
				...
				if (i >= fd_max) {
					ShowWarning(
						"Session was not found for character '%s' (CID: %d, AID: %d), defaulting "
						"gender to male...\n",
...
#if PACKETVER >= 20141016
			ShowWarning(
				"Found unknown gender '%c' for character '%s' (CID: %d, AID: %d), returning "
				"account gender...\n",
...
int char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count) {
	...
	if (SQL_ERROR ==
			SqlStmt_Prepare(
				stmt,
				"SELECT "
				"`char_id`,`char_num`,`name`,`class`,`base_level`,`job_level`,`base_exp`,`job_exp`,"
				"`zeny`,"
				"`str`,`agi`,`vit`,`int`,`dex`,`luk`,`max_hp`,`hp`,`max_sp`,`sp`,"
				"`status_point`,`skill_point`,`option`,`karma`,`manner`,`hair`,`hair_color`,"
				"`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`head_mid`,`head_bottom`,`"
				"last_map`,`rename`,`delete_date`,"
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 2, SQLDT_STRING, &p.name, sizeof(p.name), NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 33, SQLDT_STRING, &last_map, sizeof(last_map), NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 40, SQLDT_UINT, &p.uniqueitem_counter, 0, NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 45, SQLDT_UCHAR, &p.hotkey_rowshift2, 0, NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 55, SQLDT_UINT16, &p.inventory_slots, 0, NULL, NULL) ||
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if (SQL_ERROR ==
			SqlStmt_Prepare(stmt,
							"SELECT "
							"`char_id`,`account_id`,`char_num`,`name`,`class`,`base_level`,`job_"
							"level`,`base_exp`,`job_exp`,`zeny`,"
							"`str`,`agi`,`vit`,`int`,`dex`,`luk`,`max_hp`,`hp`,`max_sp`,`sp`,"
							"`status_point`,`skill_point`,`option`,`karma`,`manner`,`party_id`,`"
							"guild_id`,`pet_id`,`homun_id`,`elemental_id`,`hair`,"
							"`hair_color`,`clothes_color`,`body`,`weapon`,`shield`,`head_top`,`"
							"head_mid`,`head_bottom`,`last_map`,`last_x`,`last_y`,"
							"`save_map`,`save_x`,`save_y`,`partner_id`,`father`,`mother`,`child`,`"
							"fame`,`rename`,`delete_date`,`robe`, `moves`,"
							"`unban_time`,`font`,`uniqueitem_counter`,`sex`,`hotkey_rowshift`,`"
							"clan_id`,`title_id`,`show_equip`,`hotkey_rowshift2`,"
							"`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
							"`inventory_slots`,`body_direction`,`disable_call`"
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 3, SQLDT_STRING, &p->name, sizeof(p->name), NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 39, SQLDT_STRING, &last_map, sizeof(last_map), NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 42, SQLDT_STRING, &save_map, sizeof(save_map), NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 53, SQLDT_UINT32, &p->character_moves, 0, NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 56, SQLDT_UINT, &p->uniqueitem_counter, 0, NULL, NULL) ||
		SQL_ERROR == SqlStmt_BindColumn(stmt, 57, SQLDT_ENUM, &sex, sizeof(sex), NULL, NULL) ||
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 58, SQLDT_UCHAR, &p->hotkey_rowshift, 0, NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 62, SQLDT_UCHAR, &p->hotkey_rowshift2, 0, NULL, NULL) ||
		...
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 72, SQLDT_UINT16, &p->inventory_slots, 0, NULL, NULL) ||
	...
	if (SQL_ERROR ==
		...
		SQL_ERROR == SqlStmt_Execute(stmt) ||
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &point_map, sizeof(point_map), NULL, NULL) ||
	...
	//`skill` (`char_id`, `id`, `lv`)
	if (SQL_ERROR ==
			SqlStmt_Prepare(stmt, "SELECT `id`, `lv`,`flag` FROM `%s` WHERE `char_id`=? LIMIT %d",
							schema_config.skill_db, MAX_SKILL) ||
		SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) ||
		SQL_ERROR == SqlStmt_Execute(stmt) ||
	...
	for (i = 0; skill_count < MAX_SKILL && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
		if (tmp_skill.id > 0 && tmp_skill.id < MAX_SKILL_ID) {
			...
			ShowWarning(
				"mmo_char_fromsql: ignoring invalid skill (id=%u,lv=%u) of character %s "
				"(AID=%d,CID=%d)\n",
	...
	if (SQL_ERROR ==
			SqlStmt_Prepare(stmt,
							"SELECT c.`account_id`, c.`char_id`, c.`name` FROM `%s` c LEFT JOIN "
							"`%s` f ON f.`friend_id` = c.`char_id` WHERE f.`char_id`=? LIMIT %d",
							schema_config.char_db, schema_config.friend_db, MAX_FRIENDS) ||
		SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) ||
		SQL_ERROR == SqlStmt_Execute(stmt) ||
		SQL_ERROR ==
			SqlStmt_BindColumn(stmt, 0, SQLDT_INT, &tmp_friend.account_id, 0, NULL, NULL) ||
		SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_INT, &tmp_friend.char_id, 0, NULL, NULL) ||
		SQL_ERROR == SqlStmt_BindColumn(stmt, 2, SQLDT_STRING, &tmp_friend.name,
										sizeof(tmp_friend.name), NULL, NULL))
		SqlStmt_ShowDebug(stmt);
	...
	if (SQL_ERROR ==
			SqlStmt_Prepare(
				stmt,
				"SELECT `hotkey`, `type`, `itemskill_id`, `skill_lvl` FROM `%s` WHERE `char_id`=?",
				schema_config.hotkey_db) ||
		SQL_ERROR == SqlStmt_BindParam(stmt, 0, SQLDT_INT, &char_id, 0) ||
		SQL_ERROR == SqlStmt_Execute(stmt) ||
	...
	while (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
		if (hotkey_num >= 0 && hotkey_num < MAX_HOTKEYS_DB)
			...
			ShowWarning(
				"mmo_char_fromsql: ignoring invalid hotkey (hotkey=%d,type=%u,id=%u,lv=%u) of "
				"character %s (AID=%d,CID=%d)\n",
				hotkey_num, tmp_hotkey.type, tmp_hotkey.id, tmp_hotkey.lv, p->name, p->account_id,
				p->char_id);
	}
	...
	if (charserv_config.log_char) {
		if (SQL_ERROR ==
			Sql_Query(
				sql_handle,
				"INSERT INTO `%s` (`time`, "
				"`char_msg`,`account_id`,`char_num`,`name`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`"
				"hair`,`hair_color`)"
				"VALUES (NOW(), '%s', '%d', '%d', '%s', '0', '0', '0', '0', '0', '0', '0', '0')",
				schema_config.charlog_db, "change char name", sd->account_id, char_dat.slot,
				esc_name))
			Sql_ShowDebug(sql_handle);
	}
...
int char_check_char_name(char* name, char* esc_name) {
	...
	if (charserv_config.char_config.char_name_option ==
		1) {  // only letters/symbols in char_name_letters are authorised
	...
	if (charserv_config.char_config.name_ignoring_case) {
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "SELECT 1 FROM `%s` WHERE `name` = '%s' LIMIT 1",
								   schema_config.char_db, esc_name)) {
			Sql_ShowDebug(sql_handle);
			return -2;
		}
...
//-----------------------------------
int char_make_new_char(struct char_session_data* sd, char* name_, int str, int agi, int vit,
					   int int_, int dex, int luk, int slot, int hair_color, int hair_style,
					   short start_job, int sex) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT 1 FROM `%s` WHERE `account_id` = '%d'",
							   schema_config.char_db, sd->account_id))
		Sql_ShowDebug(sql_handle);
#ifdef VIP_ENABLE
	...
	if (charserv_config.log_char) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s` (`time`, "
								   "`char_msg`,`account_id`,`char_num`,`name`,`str`,`agi`,`vit`,`"
								   "int`,`dex`,`luk`,`hair`,`hair_color`)"
								   "VALUES (NOW(), '%s', '%d', '%d', '%s', '%d', '%d', '%d', '%d', "
								   "'%d', '%d', '%d', '%d')",
								   schema_config.charlog_db, "make new char", sd->account_id, slot,
								   esc_name, str, agi, vit, int_, dex, luk, hair_style, hair_color))
			Sql_ShowDebug(sql_handle);
	}
	...
	if (start_job == JOB_SUMMONER) {  // Check for just this job for now.
		memset(tmp_start_point, 0, MAX_STARTPOINT * sizeof(struct point));
		memset(tmp_start_items, 0, MAX_STARTITEM * sizeof(struct startitem));
		memcpy(tmp_start_point, charserv_config.start_point_doram,
			   MAX_STARTPOINT * sizeof(struct point));
		memcpy(tmp_start_items, charserv_config.start_items_doram,
			   MAX_STARTITEM * sizeof(struct startitem));
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "INSERT INTO `%s` (`account_id`, `char_num`, `name`, `class`, `zeny`, "
				  "`status_point`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, `max_hp`, `hp`,"
				  "`max_sp`, `sp`, `hair`, `hair_color`, `last_map`, `last_x`, `last_y`, "
				  "`save_map`, `save_x`, `save_y`, `sex`) VALUES ("
				  "'%d', '%d', '%s', '%d', '%d',  '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', "
				  "'%u', '%u', '%u', '%d', '%d', '%s', '%d', '%d', '%s', '%d', '%d', '%c')",
				  schema_config.char_db, sd->account_id, slot, esc_name, start_job,
				  charserv_config.start_zeny, status_points, str, agi, vit, int_, dex, luk,
				  (40 * (100 + vit) / 100), (40 * (100 + vit) / 100), (11 * (100 + int_) / 100),
				  (11 * (100 + int_) / 100), hair_style, hair_color,
				  mapindex_id2name(tmp_start_point[start_point_idx].map),
				  tmp_start_point[start_point_idx].x, tmp_start_point[start_point_idx].y,
				  mapindex_id2name(tmp_start_point[start_point_idx].map),
	...
	for (k = 0; k <= MAX_STARTITEM && tmp_start_items[k].nameid != 0; k++) {
		if (SQL_ERROR == Sql_Query(sql_handle,
								   "INSERT INTO `%s` (`char_id`,`nameid`, `amount`, `equip`, "
								   "`identify`) VALUES ('%d', '%u', '%hu', '%u', '%d')",
								   schema_config.inventory_db, char_id, tmp_start_items[k].nameid,
								   tmp_start_items[k].amount, tmp_start_items[k].pos, 1))
			Sql_ShowDebug(sql_handle);
	}
...
int char_divorce_char_sql(int partner_id1, int partner_id2) {
	if (SQL_ERROR ==
		...
		Sql_ShowDebug(sql_handle);
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE (`nameid`='%u' OR `nameid`='%u') AND "
							   "(`char_id`='%d' OR `char_id`='%d') LIMIT 2",
							   schema_config.inventory_db, WEDDING_RING_M, WEDDING_RING_F,
...
enum e_char_del_response char_delete(struct char_session_data* sd, uint32 char_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "`name`,`account_id`,`party_id`,`guild_id`,`base_level`,`homun_id`,`"
							   "partner_id`,`father`,`mother`,`elemental_id`,`delete_date` FROM "
							   "`%s` WHERE `account_id`='%u' AND `char_id`='%u'",
	...
	if ((charserv_config.char_config.char_del_level > 0 &&
		 base_level >= charserv_config.char_config.char_del_level) ||
		(charserv_config.char_config.char_del_level < 0 &&
		 base_level <= -charserv_config.char_config.char_del_level)) {
	...
	if (charserv_config.char_config.char_del_delay > 0 &&
		(!delete_date || delete_date > time(NULL))) {  // not queued or delay not yet passed
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `char_id`='%d' AND `incubate` = '0'",
							   schema_config.pet_db, char_id))
		Sql_ShowDebug(sql_handle);

	// Delete all pets that are stored in eggs (inventory + cart)
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = "
							   "`card1`|`card2`<<16 WHERE `%s`.char_id = '%d' AND card0 = 256",
							   schema_config.pet_db, schema_config.pet_db,
							   schema_config.inventory_db, schema_config.inventory_db, char_id))
		Sql_ShowDebug(sql_handle);
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` USING `%s` JOIN `%s` ON `pet_id` = "
							   "`card1`|`card2`<<16 WHERE `%s`.char_id = '%d' AND card0 = 256",
							   schema_config.pet_db, schema_config.pet_db, schema_config.cart_db,
							   schema_config.cart_db, char_id))
		Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `friend_id` = '%d'",
							   schema_config.friend_db, char_id))
		Sql_ShowDebug(sql_handle);
	...
	/* delete hotkeys */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'",
							   schema_config.hotkey_db, char_id))
		Sql_ShowDebug(sql_handle);
#endif
	...
	/* delete skills */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'",
							   schema_config.skill_db, char_id))
		Sql_ShowDebug(sql_handle);
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `dest_id`='%d'",
							   schema_config.mail_db, char_id))
		Sql_ShowDebug(sql_handle);

	/* mark mails as sent from server, if a character gets deleted */
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `send_id`='0' WHERE `send_id`='%d'",
							   schema_config.mail_db, char_id))
		Sql_ShowDebug(sql_handle);
	...
	/* bonus_scripts */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.bonus_script_db, char_id))
		Sql_ShowDebug(sql_handle);

	/* Quest Data */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.quest_db, char_id))
		Sql_ShowDebug(sql_handle);

	/* Achievement Data */
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.achievement_table, char_id))
		Sql_ShowDebug(sql_handle);

	if (charserv_config.log_char) {
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`char_msg`,`name`) VALUES "
					  "(NOW(), '%d', '%d', 'Deleted char (CID %d)', '%s')",
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `char_id`='%d'", guild_member_db,
	char_id) ) Sql_ShowDebug(sql_handle);
	*/

	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id` FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.guild_db, char_id))
		Sql_ShowDebug(sql_handle);
...
int char_mmo_char_tobuf(uint8* buffer, struct mmo_charstatus* p) {
	...
	info->weapon = p->option & (0x20 | 0x80000 | 0x100000 | 0x200000 | 0x400000 | 0x800000 |
	...
	info->chr_name_changeCnt =
		(p->rename > 0) ? 1 : 0;  // (0 = disabled, otherwise displays "Add-Ons" sidebar)
#endif
#if PACKETVER >= 20141016
...
int char_married(int pl1, int pl2) {
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `partner_id` FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.char_db, pl1))
		Sql_ShowDebug(sql_handle);
...
int char_child(int parent_id, int child_id) {
	if (parent_id == 0 ||
		child_id == 0)	// Failsafe, avoild querys and fix EXP bug dividing with lower level chars
		return 0;
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `child` FROM `%s` WHERE `char_id` = '%d'",
							   schema_config.char_db, parent_id))
		Sql_ShowDebug(sql_handle);
	else if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		char* data;
...
int char_family(int cid1, int cid2, int cid3) {
	if (cid1 == 0 || cid2 == 0 || cid3 == 0)  // Failsafe, and avoid querys where there is no sense
											  // to keep executing if any of the inputs are 0
		return 0;
	if (SQL_ERROR ==
		Sql_Query(
			sql_handle,
		...
		while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
			...
			if ((cid1 == charid && ((cid2 == partnerid && cid3 == childid) ||
									(cid2 == childid && cid3 == partnerid))) ||
				(cid1 == partnerid &&
				 ((cid2 == charid && cid3 == childid) || (cid2 == childid && cid3 == charid))) ||
				(cid1 == childid && ((cid2 == charid && cid3 == partnerid) ||
									 (cid2 == partnerid && cid3 == charid)))) {
				Sql_FreeResult(sql_handle);
				return childid;
			}
		}
...
void char_auth_ok(int fd, struct char_session_data* sd) {
	...
	if ((character = (struct online_char_data*)idb_get(online_char_db, sd->account_id)) !=
		NULL) {						   // check if character is not online already. [Skotlex]
		if (character->server > -1) {  // Character already online. KICK KICK KICK
			mapif_disconnectplayer(map_server[character->server].fd, character->account_id,
								   character->char_id, 2);
			if (character->waiting_disconnect == INVALID_TIMER)
				character->waiting_disconnect =
					add_timer(gettick() + AUTH_TIMEOUT, char_chardb_waiting_disconnect,
							  character->account_id, 0);
			chclif_send_auth_result(fd, 8);
			return;
		}
		if (session_isValid(character->fd) &&
			character->fd != fd) {	// There's already a connection from this account that hasn't
									// picked a char yet.
			chclif_send_auth_result(fd, 8);
			return;
		}
...
void char_read_fame_list(void) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "`class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR "
				  "`class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
				  schema_config.char_db, JOB_BLACKSMITH, JOB_WHITESMITH, JOB_BABY_BLACKSMITH,
				  JOB_MECHANIC, JOB_MECHANIC_T, JOB_BABY_MECHANIC, JOB_MEISTER,
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "`class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR `class`='%d' OR "
				  "`class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
				  schema_config.char_db, JOB_ALCHEMIST, JOB_CREATOR, JOB_BABY_ALCHEMIST,
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT `char_id`,`fame`,`name` FROM `%s` WHERE `fame`>0 AND "
							   "(`class`='%d' OR `class`='%d') ORDER BY `fame` DESC LIMIT 0,%d",
							   schema_config.char_db, JOB_TAEKWON, JOB_BABY_TAEKWON,
...
int char_loadName(uint32 char_id, char* name) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `name` FROM `%s` WHERE `char_id`='%d'",
							   schema_config.char_db, char_id))
		Sql_ShowDebug(sql_handle);
	else if (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
		Sql_GetData(sql_handle, 0, &data, &len);
...
// FIXME to be remove (moved to cnslif / will be done once map/char/login, all have their cnslif
// interface ready)
int parse_console(const char* buf) { return cnslif_parse(buf); }
...
TIMER_FUNC(char_chardb_waiting_disconnect) {
	struct online_char_data* character;
	if ((character = (struct online_char_data*)idb_get(online_char_db, id)) != NULL &&
		character->waiting_disconnect == tid) {	 // Mark it offline due to timeout.
		character->waiting_disconnect = INVALID_TIMER;
		char_set_char_offline(character->char_id, character->account_id);
	}
	return 0;
...
TIMER_FUNC(char_clan_member_cleanup) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "UPDATE `%s` SET `clan_id`='0' WHERE `online`='0' AND `clan_id`<>'0' AND "
				  "`last_login` IS NOT NULL AND `last_login` <= NOW() - INTERVAL %d DAY",
...
int char_lan_config_read(const char* lancfgName) {
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (strcmpi(w1, "subnet") == 0) {
			if (subnet_count >= s_subnet) {	 // We skip instead of break in case we want to add
											 // other conf in that file.
			...
			if ((subnet[subnet_count].char_ip & subnet[subnet_count].mask) !=
				...
				ShowError(
					"%s: Configuration Error: The char server (%s) and map server (%s) belong to "
...
bool char_checkdb(void) {
	...
	for (i = 0; i < ARRAYLENGTH(sqltable);
		 i++) {	 // check if they all exist and we can acces them in sql-server
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   ...
							   "`base_level`,`job_level`,`base_exp`,`job_exp`,`zeny`,`str`,`agi`,`"
							   "vit`,`int`,`dex`,`luk`,"
							   "`max_hp`,`hp`,`max_sp`,`sp`,`status_point`,`skill_point`,`option`,`"
							   "karma`,`manner`,`party_id`,"
							   "`guild_id`,`pet_id`,`homun_id`,`elemental_id`,`hair`,`hair_color`,`"
							   "clothes_color`,`weapon`,"
							   "`shield`,`head_top`,`head_mid`,`head_bottom`,`robe`,`last_map`,`"
							   "last_x`,`last_y`,`save_map`,"
							   "`save_x`,`save_y`,`partner_id`,`online`,`father`,`mother`,`child`,`"
							   "fame`,`rename`,`delete_date`,"
							   "`moves`,`unban_time`,`font`,`sex`,`hotkey_rowshift`,`clan_id`,`"
							   "last_login`,`title_id`,`show_equip`,"
							   "`hotkey_rowshift2`,"
							   "`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
							   "`inventory_slots`,`body_direction`,`disable_call`"
							   " FROM `%s` LIMIT 1;",
	...
	// checking skill_db
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `char_id`,`id`,`lv`,`flag` FROM `%s` LIMIT 1;",
							   schema_config.skill_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	// checking interlog_db
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `time`,`log` FROM `%s` LIMIT 1;",
							   schema_config.interlog_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	// checking memo_db
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `memo_id`,`char_id`,`map`,`x`,`y` FROM `%s` LIMIT 1;",
							   schema_config.memo_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   ...
							   "`connect_member`,`max_member`,`average_lv`,`exp`,`next_exp`,`skill_"
							   "point`,`mes1`,`mes2`,"
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   ...
							   "`triggerD`,`nextTime`,`payTime`,`createTime`,`visibleC`,`visibleG0`"
							   ",`visibleG1`,`visibleG2`,"
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT  `guild_id`,`account_id`,`name`,`mes` FROM `%s` LIMIT 1;",
							   schema_config.guild_expulsion_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `guild_id`,`id`,`lv` FROM `%s` LIMIT 1;",
							   schema_config.guild_skill_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	// checking friend_db
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `char_id`,`friend_id` FROM `%s` LIMIT 1;",
							   schema_config.friend_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`"
				  "option_id1`,`option_val1`,`option_parm1`,`option_id2`,`option_val2`,`option_"
				  "parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_id4`,`option_val4`,`"
				  "option_parm4`,`unique_id`, `bound`, `enchantgrade`"
				  " FROM `%s` LIMIT 1;",
				  schema_config.mail_attachment_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR ==
		Sql_Query(
			...
			"`card0`,`card1`,"
			"`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`option_val1`,`"
			"option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`"
			"option_parm3`,`option_id4`,`option_val4`,`option_parm4`,`unique_id`, `enchantgrade` "
			"FROM `%s` LIMIT 1;",
			schema_config.auction_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR ==
		Sql_Query(
			sql_handle,
			"SELECT  "
			"`homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`,"
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `homun_id`,`id`,`lv` FROM `%s` LIMIT 1;",
							   schema_config.skill_homunculus_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT  `mer_id`,`char_id`,`class`,`hp`,`sp`,`kill_counter`,`life_time` FROM "
				  "`%s` LIMIT 1;",
				  schema_config.mercenary_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  ...
				  "parm0`,`option_id1`,`option_val1`,`option_parm1`,`option_id2`,`option_val2`,`"
				  "option_parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_id4`,`option_"
				  "val4`,`option_parm4`,`expire_time`,`bound`,`unique_id`,`enchantgrade`"
				  " FROM `%s` LIMIT 1;",
				  schema_config.cart_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "`char_id`,`id`,`count1`,`count2`,`count3`,`count4`,`count5`,`"
							   "count6`,`count7`,`count8`,`count9`,`count10`,`completed`,`rewarded`"
							   " FROM `%s` LIMIT 1;",
							   schema_config.achievement_table)) {
		Sql_ShowDebug(sql_handle);
		return false;
...
void char_sql_config_read(const char* cfgName) {
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (!strcmpi(w1, "char_db"))
			...
			safestrncpy(schema_config.guild_alliance_db, w2,
						sizeof(schema_config.guild_alliance_db));
			...
			safestrncpy(schema_config.guild_expulsion_db, w2,
						sizeof(schema_config.guild_expulsion_db));
			...
			safestrncpy(schema_config.guild_position_db, w2,
						sizeof(schema_config.guild_position_db));
			...
			safestrncpy(schema_config.mail_attachment_db, w2,
						sizeof(schema_config.mail_attachment_db));
			...
			safestrncpy(schema_config.skill_homunculus_db, w2,
						sizeof(schema_config.skill_homunculus_db));
			...
			safestrncpy(schema_config.mercenary_owner_db, w2,
						sizeof(schema_config.mercenary_owner_db));
			...
			safestrncpy(schema_config.char_reg_num_table, w2,
						sizeof(schema_config.char_reg_num_table));
		else if (!strcmpi(w1, "char_reg_str_table"))
			safestrncpy(schema_config.char_reg_str_table, w2,
						sizeof(schema_config.char_reg_str_table));
		else if (!strcmpi(w1, "acc_reg_str_table"))
			safestrncpy(schema_config.acc_reg_str_table, w2,
						sizeof(schema_config.acc_reg_str_table));
		else if (!strcmpi(w1, "acc_reg_num_table"))
			safestrncpy(schema_config.acc_reg_num_table, w2,
						sizeof(schema_config.acc_reg_num_table));
			...
			safestrncpy(schema_config.clan_alliance_table, w2,
						sizeof(schema_config.clan_alliance_table));
		else if (!strcmpi(w1, "achievement_table"))
			safestrncpy(schema_config.achievement_table, w2,
						sizeof(schema_config.achievement_table));
...
void char_set_default_sql() {
	...
	safestrncpy(schema_config.guild_alliance_db, "guild_alliance",
				sizeof(schema_config.guild_alliance_db));
	safestrncpy(schema_config.guild_castle_db, "guild_castle",
				sizeof(schema_config.guild_castle_db));
	safestrncpy(schema_config.guild_expulsion_db, "guild_expulsion",
				sizeof(schema_config.guild_expulsion_db));
	safestrncpy(schema_config.guild_member_db, "guild_member",
				sizeof(schema_config.guild_member_db));
	safestrncpy(schema_config.guild_position_db, "guild_position",
				sizeof(schema_config.guild_position_db));
	safestrncpy(schema_config.guild_skill_db, "guild_skill", sizeof(schema_config.guild_skill_db));
	safestrncpy(schema_config.guild_storage_db, "guild_storage",
				sizeof(schema_config.guild_storage_db));
	...
	safestrncpy(schema_config.auction_db, "auction",
				sizeof(schema_config.auction_db));	// Auctions System
	...
	safestrncpy(schema_config.skill_homunculus_db, "skill_homunculus",
				sizeof(schema_config.skill_homunculus_db));
	safestrncpy(schema_config.mercenary_db, "mercenary", sizeof(schema_config.mercenary_db));
	safestrncpy(schema_config.mercenary_owner_db, "mercenary_owner",
				sizeof(schema_config.mercenary_owner_db));
	safestrncpy(schema_config.skillcooldown_db, "skillcooldown",
				sizeof(schema_config.skillcooldown_db));
	safestrncpy(schema_config.bonus_script_db, "bonus_script",
				sizeof(schema_config.bonus_script_db));
	safestrncpy(schema_config.char_reg_num_table, "char_reg_num",
				sizeof(schema_config.char_reg_num_table));
	safestrncpy(schema_config.char_reg_str_table, "char_reg_str",
				sizeof(schema_config.char_reg_str_table));
	safestrncpy(schema_config.acc_reg_str_table, "acc_reg_str",
				sizeof(schema_config.acc_reg_str_table));
	safestrncpy(schema_config.acc_reg_num_table, "acc_reg_num",
				sizeof(schema_config.acc_reg_num_table));
	safestrncpy(schema_config.clan_table, "clan", sizeof(schema_config.clan_table));
	safestrncpy(schema_config.clan_table, "clan_alliance",
				sizeof(schema_config.clan_alliance_table));
	safestrncpy(schema_config.achievement_table, "achievement",
				sizeof(schema_config.achievement_table));
}
...
void char_set_defaults() {
	...
	charserv_config.char_config.char_per_account =
		0;	// Maximum chars per account (default unlimited) [Sirius]
	charserv_config.char_config.char_del_level =
		0;	// From which level u can delete character [Lupus]
	...
	safestrncpy(charserv_config.wisp_server_name, "Server",
				sizeof(charserv_config.wisp_server_name));
	...
	charserv_config.char_config.char_name_option =
		0;	// Option to know which letters/symbols are authorised in the name of a character (0:
			// all, 1: only those in char_name_letters, 2: all EXCEPT those in char_name_letters) by
			// [Yor]
	safestrncpy(
		charserv_config.char_config.unknown_char_name, "Unknown",
		sizeof(charserv_config.char_config.unknown_char_name));	 // Name to use when the requested
																 // name cannot be determined
	safestrncpy(
		charserv_config.char_config.char_name_letters, "",
		sizeof(
			charserv_config.char_config.char_name_letters));  // list of letters/symbols allowed (or
															  // not) in a character name. by [Yor]
 ...
 */
void char_config_split_startpoint(char* w1_value, char* w2_value,
								  struct point start_point[MAX_STARTPOINT], short* count) {
	...
	while (lineitem != NULL && (*count) < MAX_STARTPOINT) {
		int n = sv_split(lineitem, strlen(lineitem), 0, ',', fields, fields_length,
 ...
 */
void char_config_split_startitem(char* w1_value, char* w2_value,
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (normal) {
			if (strcmpi(w1, "userid") == 0) {
				...
				if (strlen(w2) >= 4) {
					safestrncpy(charserv_config.wisp_server_name, w2,
								sizeof(charserv_config.wisp_server_name));
				}
				...
				if (charserv_config.login_ip) {
					char ip_str[16];
					safestrncpy(charserv_config.login_ip_str, w2,
								sizeof(charserv_config.login_ip_str));
					ShowStatus("Login server IP address : %s -> %s\n", w2,
							   ip2str(charserv_config.login_ip, ip_str));
				}
				...
				if (charserv_config.char_ip) {
					char ip_str[16];
					safestrncpy(charserv_config.char_ip_str, w2,
								sizeof(charserv_config.char_ip_str));
					ShowStatus("Character server IP address : %s -> %s\n", w2,
							   ip2str(charserv_config.char_ip, ip_str));
				}
			} else if (strcmpi(w1, "bind_ip") == 0) {
				charserv_config.bind_ip = host2ip(w2);
				if (charserv_config.bind_ip) {
					char ip_str[16];
					safestrncpy(charserv_config.bind_ip_str, w2,
								sizeof(charserv_config.bind_ip_str));
		...
		if (strcmpi(w1, "timestamp_format") == 0) {
...
#endif
			char_config_split_startpoint(w1, w2, charserv_config.start_point,
										 &charserv_config.start_point_count);
#if PACKETVER >= 20151001
		} else if (strcmpi(w1, "start_point_doram") == 0) {
			char_config_split_startpoint(w1, w2, charserv_config.start_point_doram,
										 &charserv_config.start_point_count_doram);
#endif
			...
			safestrncpy(charserv_config.char_config.unknown_char_name, w2,
			...
			safestrncpy(charserv_config.char_config.char_name_letters, w2,
						sizeof(charserv_config.char_config.char_name_letters));
		} else if (strcmpi(w1, "char_del_level") ==
				   0) {	 // disable/enable char deletion by its level condition [Lupus]
...
void char_config_adjust() {
	...
	if (charserv_config.char_config.char_del_option & CHAR_DEL_BIRTHDATE) {
		ShowWarning(
			"conf/char_athena.conf:char_del_option birthdate is enabled but it requires PACKETVER "
			"2010-08-03 or newer, defaulting to email...\n");
		charserv_config.char_config.char_del_option &= ~CHAR_DEL_BIRTHDATE;
	}
#endif
}
...
const char* char_msg_txt(int msg_number) { return _msg_txt(msg_number, CHAR_MAX_MSG, msg_table); }
void char_do_final_msg(void) { _do_final_msg(CHAR_MAX_MSG, msg_table); }
...
void set_server_type(void) { SERVER_TYPE = ATHENA_SERVER_CHAR; }
...
void do_shutdown(void) {
	if (runflag != CHARSERVER_ST_SHUTDOWN) {
		...
		for (id = 0; id < ARRAYLENGTH(map_server); ++id) chmapif_server_reset(id);
...
int do_init(int argc, char** argv) {
	...
	if (runflag != CORE_ST_STOP && strcmp(charserv_config.userid, "s1") == 0 &&
		strcmp(charserv_config.passwd, "p1") == 0) {
		ShowWarning("Using the default user/password s1/p1 is NOT RECOMMENDED.\n");
		ShowNotice(
			"Please edit your 'login' table to create a proper inter-server user/password (gender "
			"'S')\n");
		ShowNotice(
			"And then change the user/password to use in conf/char_athena.conf (or "
	...
	add_timer_interval(gettick() + 1000, char_clan_member_cleanup, 0, 0,
	...
	if (charserv_config.char_check_db && char_checkdb() == 0) {
		ShowFatalError(
			"char : A tables is missing in sql-server, please fix it, see (sql-files main.sql for "
			"structure) \n");
		exit(EXIT_FAILURE);
	}
	...
	// Chardb clean
	if (SQL_ERROR ==
		Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `account_id` = '0'", schema_config.char_db))
		Sql_ShowDebug(sql_handle);

	// guilddb clean
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `guild_lv` = '0' AND `max_member` = '0' AND "
							   "`exp` = '0' AND `next_exp` = '0' AND `average_lv` = '0'",
							   schema_config.guild_db))
		Sql_ShowDebug(sql_handle);
	...
	ShowInfo("Default map: '" CL_WHITE "%s %d,%d" CL_RESET "'\n", charserv_config.default_map,
			 charserv_config.default_map_x, charserv_config.default_map_y);

	ShowStatus("The char-server is " CL_GREEN "ready" CL_RESET
			   " (Server is listening on the port %d).\n\n",
			   charserv_config.char_port);
