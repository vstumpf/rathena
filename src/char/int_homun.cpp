...
void inter_homunculus_sql_final(void) { return; }

void mapif_homunculus_created(int fd, uint32 account_id, struct s_homunculus* sh,
							  unsigned char flag) {
...
void mapif_homunculus_renamed(int fd, uint32 account_id, uint32 char_id, unsigned char flag,
							  char* name) {
	...
	if (hd->hom_id == 0) {	// new homunculus
		if (SQL_ERROR ==
			Sql_Query(
				sql_handle,
				"INSERT INTO `%s` "
				"(`char_id`, `class`,`prev_class`,`name`,`level`,`exp`,`intimacy`,`hunger`, `str`, "
				"`agi`, `vit`, `int`, `dex`, `luk`, `hp`,`max_hp`,`sp`,`max_sp`,`skill_point`, "
				...
				hd->level, hd->exp, hd->intimacy, hd->hunger, hd->str, hd->agi, hd->vit, hd->int_,
				hd->dex, hd->luk, hd->hp, hd->max_hp, hd->sp, hd->max_sp, hd->skillpts,
				hd->rename_flag, hd->vaporize, hd->autofeed)) {
		...
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  ...
					  "',`intimacy`='%u',`hunger`='%d', `str`='%d', `agi`='%d', `vit`='%d', "
					  "`int`='%d', `dex`='%d', `luk`='%d', "
					  "`hp`='%u',`max_hp`='%u',`sp`='%u',`max_sp`='%u',`skill_point`='%d', "
					  "`rename_flag`='%d', `vaporize`='%d', `autofeed`='%d' WHERE `homun_id`='%d'",
					  schema_config.homunculus_db, hd->char_id, hd->class_, hd->prev_class,
					  esc_name, hd->level, hd->exp, hd->intimacy, hd->hunger, hd->str, hd->agi,
					  hd->vit, hd->int_, hd->dex, hd->luk, hd->hp, hd->max_hp, hd->sp, hd->max_sp,
					  hd->skillpts, hd->rename_flag, hd->vaporize, hd->autofeed, hd->hom_id)) {
			Sql_ShowDebug(sql_handle);
			flag = false;
		} else {
			SqlStmt* stmt;
			...
			if (SQL_ERROR ==
				SqlStmt_Prepare(stmt,
								"REPLACE INTO `%s` (`homun_id`, `id`, `lv`) VALUES (%d, ?, ?)",
...
bool mapif_homunculus_load(int homun_id, struct s_homunculus* hd) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "SELECT "
							   "`homun_id`,`char_id`,`class`,`prev_class`,`name`,`level`,`exp`,`"
							   "intimacy`,`hunger`, `str`, `agi`, `vit`, `int`, `dex`, `luk`, "
							   "`hp`,`max_hp`,`sp`,`max_sp`,`skill_point`,`rename_flag`, "
							   "`vaporize`, `autofeed` FROM `%s` WHERE `homun_id`='%u'",
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `id`,`lv` FROM `%s` WHERE `homun_id`=%d",
							   schema_config.skill_homunculus_db, homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
	...
	if (charserv_config.save_log)
		ShowInfo("Homunculus loaded (ID: %d - %s / Class: %d / CID: %d).\n", hd->hom_id, hd->name,
				 hd->class_, hd->char_id);

	return true;
}

bool mapif_homunculus_delete(int homun_id) {
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%u'",
							   schema_config.homunculus_db, homun_id) ||
		SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `homun_id` = '%u'",
							   schema_config.skill_homunculus_db, homun_id)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
int inter_homunculus_parse_frommap(int fd) {
	...
	switch (cmd) {
		case 0x3090:
			mapif_parse_homunculus_create(fd, (int)RFIFOW(fd, 2), (int)RFIFOL(fd, 4),
										  (struct s_homunculus*)RFIFOP(fd, 8));
			break;
		case 0x3091:
			mapif_parse_homunculus_load(fd, (int)RFIFOL(fd, 2), (int)RFIFOL(fd, 6));
			break;
		case 0x3092:
			mapif_parse_homunculus_save(fd, (int)RFIFOW(fd, 2), (int)RFIFOL(fd, 4),
										(struct s_homunculus*)RFIFOP(fd, 8));
			break;
		case 0x3093:
			...
			break;
		case 0x3094:
			mapif_parse_homunculus_rename(fd, (int)RFIFOL(fd, 2), (int)RFIFOL(fd, 6),
										  RFIFOCP(fd, 10));
			break;
		default:
			return 0;
