 ...
 * @return Array of found entries. It has *count entries, and it is care of the caller to aFree() it
 * afterwards.
 */
struct achievement *mapif_achievements_fromsql(uint32 char_id, int *count) {
	...
	StringBuf_AppendStr(&buf,
						"SELECT `id`, COALESCE(UNIX_TIMESTAMP(`completed`),0), "
						"COALESCE(UNIX_TIMESTAMP(`rewarded`),0)");
	for (i = 0; i < MAX_ACHIEVEMENT_OBJECTIVES; ++i) StringBuf_Printf(&buf, ", `count%d`", i + 1);
	StringBuf_Printf(&buf, " FROM `%s` WHERE `char_id` = '%u'", schema_config.achievement_table,
					 char_id);
...
bool mapif_achievement_delete(uint32 char_id, int achievement_id) {
	if (SQL_ERROR == Sql_Query(sql_handle,
							   "DELETE FROM `%s` WHERE `id` = '%d' AND `char_id` = '%u'",
...
bool mapif_achievement_add(uint32 char_id, struct achievement *ad) {
	...
	StringBuf_Init(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s` (`char_id`, `id`, `completed`, `rewarded`",
					 schema_config.achievement_table);
	for (i = 0; i < MAX_ACHIEVEMENT_OBJECTIVES; ++i) StringBuf_Printf(&buf, ", `count%d`", i + 1);
	StringBuf_AppendStr(&buf, ")");
	StringBuf_Printf(&buf, " VALUES ('%u', '%d',", char_id, ad->achievement_id,
					 (uint32)ad->completed, (uint32)ad->rewarded);
	if (ad->completed) {
	...
	for (i = 0; i < MAX_ACHIEVEMENT_OBJECTIVES; ++i) StringBuf_Printf(&buf, ", '%d'", ad->count[i]);
	StringBuf_AppendStr(&buf, ")");
...
int mapif_parse_achievement_reward(int fd) {
	...
	if (Sql_Query(sql_handle,
				  "UPDATE `%s` SET `rewarded` = FROM_UNIXTIME('%u') WHERE `char_id`='%u' AND `id` "
				  "= '%d' AND `completed` IS NOT NULL AND `rewarded` IS NULL",
				  schema_config.achievement_table, (uint32)current, char_id,
				  achievement_id) == SQL_ERROR ||
		...
		safesnprintf(mail_title, MAIL_TITLE_LENGTH,
					 char_msg_txt(228));  // 228: Achievement Reward Mail
		safesnprintf(mail_text, MAIL_BODY_LENGTH, char_msg_txt(229),
					 RFIFOCP(fd, 16 + NAME_LENGTH));  // 229: [%s] Achievement Reward.

		if (!mail_sendmail(0, mail_sender, char_id, mail_receiver, mail_title, mail_text, 0, &item,
						   1)) {
			current = 0;
		}
	}
