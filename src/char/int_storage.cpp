...
bool guild_storage_tosql(int guild_id, struct s_storage* p) {
	...
	return char_memitemdata_to_sql(p->u.items_guild, inter_guild_storagemax(guild_id), guild_id,
								   TABLE_GUILD_STORAGE, p->stor_id);
}
...
bool guild_storage_fromsql(int guild_id, struct s_storage* p) {
	return char_memitemdata_from_sql(p, inter_guild_storagemax(guild_id), guild_id,
									 TABLE_GUILD_STORAGE, p->stor_id);
}
	...
	for (auto storage_table : interServerDb) {
		if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  ...
					  "`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`"
					  "option_parm0`,`option_id1`,`option_val1`,`option_parm1`,"
					  "`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`"
					  "option_parm3`,`option_id4`,`option_val4`,`option_parm4`,"
...
void inter_storage_sql_final(void) { return; }
...
bool mapif_load_guild_storage(int fd, uint32 account_id, int guild_id, char flag) {
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `guild_id` FROM `%s` WHERE `guild_id`='%d'",
 ...
 * @author [Cydh]
 */
void mapif_itembound_store2gstorage(int fd, int guild_id, struct item items[],
...
bool mapif_parse_itembound_retrieve(int fd) {
	...
	StringBuf_AppendStr(&buf,
						...
						"`attribute`, `expire_time`, `bound`, `enchantgrade`");
	...
	StringBuf_Printf(&buf, " FROM `%s` WHERE `char_id`='%d' AND `bound` = %d",
					 schema_config.inventory_db, char_id, BOUND_GUILD);
	...
	for (j = 0; j < MAX_ITEM_RDM_OPT; ++j) {
		SqlStmt_BindColumn(stmt, 11 + MAX_SLOTS + j * 3, SQLDT_SHORT, &item.option[j].id, 0, NULL,
						   NULL);
		SqlStmt_BindColumn(stmt, 12 + MAX_SLOTS + j * 3, SQLDT_SHORT, &item.option[j].value, 0,
						   NULL, NULL);
		SqlStmt_BindColumn(stmt, 13 + MAX_SLOTS + j * 3, SQLDT_CHAR, &item.option[j].param, 0, NULL,
						   NULL);
	}
	...
	ShowInfo("Found '" CL_WHITE "%d" CL_RESET "' guild bound item(s) from CID = " CL_WHITE
			 "%d" CL_RESET ", AID = %d, Guild ID = " CL_WHITE "%d" CL_RESET ".\n",
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "DELETE FROM `%s` WHERE `char_id` = %d AND `bound` = %d",
					 schema_config.inventory_db, char_id, BOUND_GUILD);
	if (SQL_ERROR == SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
		SQL_ERROR == SqlStmt_Execute(stmt)) {
		SqlStmt_ShowDebug(stmt);
...
#undef CHECK_REMOVE
	...
	if (j) {
		StringBuf buf2;
		StringBuf_Init(&buf2);
		StringBuf_Printf(&buf2, "UPDATE `%s` SET %s WHERE `char_id`='%d'", schema_config.char_db,
						 StringBuf_Value(&buf), char_id);
 ...
 * @param result
 */
void mapif_storage_data_loaded(int fd, uint32 account_id, char type, struct s_storage* entries,
							   bool result) {
