...
// 0: success
// 1: failed
void chclif_moveCharSlotReply(int fd, struct char_session_data* sd, unsigned short index,
							  short reason) {
	WFIFOHEAD(fd, 8);
...
int chclif_parse_moveCharSlot(int fd, struct char_session_data* sd) {
	...
	if ((charserv_config.charmove_config.char_move_enabled) == 0 ||
		((charserv_config.charmove_config.char_moves_unlimited) == 0 &&
		 sd->char_moves[from] <= 0)) {
	...
	if (sd->found_char[to] > 0) {
		...
		if (charserv_config.charmove_config
				.char_movetoused) {	 // TODO: check if the target is in deletion process
			// Admin is friendly and uses triangle exchange
			if (SQL_ERROR == Sql_QueryStr(sql_handle, "START TRANSACTION") ||
				SQL_ERROR == Sql_Query(sql_handle,
									   "UPDATE `%s` SET `char_num`='%d' WHERE `char_id` = '%d'",
	...
	if ((charserv_config.charmove_config.char_moves_unlimited) == 0) {
		sd->char_moves[from]--;
		Sql_Query(sql_handle, "UPDATE `%s` SET `moves`='%d' WHERE `char_id`='%d'",
				  schema_config.char_db, sd->char_moves[from], sd->found_char[from]);
	}
...
// This is needed on RE clients from october 2015 onwards
	...
	WFIFOL(fd, 2) = (sd->char_slots > 3) ? sd->char_slots / 3 : 1;	// int TotalCnt (nb page to
																	// load)
...
int chclif_mmo_send006b(int fd, struct char_session_data* sd) {
	...
	WFIFOB(fd, 6) =
		MIN_CHARS + sd->chars_vip;	// Premium slots. (Any existent chars past sd->char_slots but
									// within MAX_CHARS will show a 'Premium Service' in red)
#endif
...
int chclif_parse_char_delete2_req(int fd, struct char_session_data* sd) {
	FIFOSD_CHECK(6) {
		...
		if (SQL_SUCCESS !=
				Sql_Query(
					sql_handle,
					"SELECT `delete_date`,`party_id`,`guild_id` FROM `%s` WHERE `char_id`='%d'",
...
int chclif_parse_char_delete2_cancel(int fd, struct char_session_data* sd) {
	...
	if (SQL_SUCCESS != Sql_Query(sql_handle,
								 "UPDATE `%s` SET `delete_date`='0' WHERE `char_id`='%d'",
								 schema_config.char_db, char_id)) {
		Sql_ShowDebug(sql_handle);
		chclif_char_delete2_cancel_ack(fd, char_id, 2);
		return 1;
...
int chclif_parse_maplogin(int fd) {
	if (RFIFOREST(fd) < 60)
		...
		if (runflag != CHARSERVER_ST_RUNNING || i == ARRAYLENGTH(map_server) ||
...
int chclif_parse_charselect(int fd, struct char_session_data* sd, uint32 ipl) {
	FIFOSD_CHECK(3) {
		...
		ARR_FIND(0, ARRAYLENGTH(map_server), server_id,
				 session_isValid(map_server[server_id].fd) && !map_server[server_id].map.empty());
		// Map-server not available, tell the client to wait (client wont close, char select will
		// respawn)
		if (server_id == ARRAYLENGTH(map_server)) {
			WFIFOHEAD(fd, 24);
			WFIFOW(fd, 0) = 0x840;
			WFIFOW(fd, 2) = 24;
			strncpy(WFIFOCP(fd, 4), "0",
					20);  // we can't send it empty (otherwise the list will pop up)
			WFIFOSET(fd, 24);
			return 1;
		}
		...
		if (SQL_SUCCESS != Sql_Query(sql_handle,
									 "SELECT `char_id` FROM `%s` WHERE `account_id`='%d' AND "
									 "`char_num`='%d' AND `delete_date` = 0",
									 schema_config.char_db, sd->account_id, slot) ||
			SQL_SUCCESS != Sql_NextRow(sql_handle) ||
			SQL_SUCCESS !=
				Sql_GetData(sql_handle, 0, &data, NULL)) {	// Not found?? May be forged packet.
			Sql_ShowDebug(sql_handle);
		...
		/* set char as online prior to loading its data so 3rd party applications will realise the
		...
		if (charserv_config.log_char) {
			...
			Sql_EscapeStringLen(sql_handle, esc_name, char_dat.name,
								strnlen(char_dat.name, NAME_LENGTH));
			if (SQL_ERROR == Sql_Query(sql_handle,
									   "INSERT INTO `%s`(`time`, `account_id`,`char_num`,`name`) "
									   "VALUES (NOW(), '%d', '%d', '%s')",
		...
		if (i < 0 || !cd->last_point.map) {
			...
			ARR_FIND(0, ARRAYLENGTH(map_server), j,
					 session_isValid(map_server[j].fd) && !map_server[j].map.empty());
			...
			if ((i = char_search_mapserver((j = mapindex_name2id(MAP_PRONTERA)), -1, -1)) >= 0) {
				cd->last_point.x = 273;
				cd->last_point.y = 354;
			} else if ((i = char_search_mapserver((j = mapindex_name2id(MAP_GEFFEN)), -1, -1)) >=
					   0) {
				cd->last_point.x = 120;
				cd->last_point.y = 100;
			} else if ((i = char_search_mapserver((j = mapindex_name2id(MAP_MORROC)), -1, -1)) >=
					   0) {
				cd->last_point.x = 160;
				cd->last_point.y = 94;
			} else if ((i = char_search_mapserver((j = mapindex_name2id(MAP_ALBERTA)), -1, -1)) >=
					   0) {
				cd->last_point.x = 116;
				cd->last_point.y = 57;
			} else if ((i = char_search_mapserver((j = mapindex_name2id(MAP_PAYON)), -1, -1)) >=
					   0) {
				cd->last_point.x = 87;
				cd->last_point.y = 117;
			} else if ((i = char_search_mapserver((j = mapindex_name2id(MAP_IZLUDE)), -1, -1)) >=
					   0) {
				cd->last_point.x = 94;
				cd->last_point.y = 103;
			} else {
				ShowInfo(
					"Connection Closed. No map server available that has a major city, and unable "
					"to find map-server for '%s'.\n",
		...
		if ((map_fd = map_server[i].fd) < 1 || session[map_fd] == NULL) {
			ShowError(
				"parse_char: Attempting to write to invalid session %d! Map Server #%d "
				"disconnected.\n",
				map_fd, i);
			map_server[i].fd = -1;
...
// S 0067 <name>.24B <str>.B <agi>.B <vit>.B <int>.B <dex>.B <luk>.B <slot>.B <hair color>.W <hair
// style>.W S 0a39 <name>.24B <slot>.B <hair color>.W <hair style>.W <starting job ID>.W
// <Unknown>.(W or 2 B's)??? <sex>.B
int chclif_parse_createnewchar(int fd, struct char_session_data* sd, int cmd) {
	...
	if ((charserv_config.char_new) == 0)  // turn character creation on/off [Kevin]
		...
		char_id = char_make_new_char(sd, name, str, agi, vit, int_, dex, luk, slot, hair_color,
									 hair_style, start_job, sex);
	}
...
TIMER_FUNC(charblock_timer) {
	...
	if (sd == NULL ||
		sd->charblock_timer == INVALID_TIMER)  // has disconected or was required to stop
		return 0;
...
// character's
// name. 		4: Another user is using this character name, so please select another one. 		5: In
// order to change the character name, you must leave the guild. 		6: In order to change the
// character name, you
// must leave the party. 		7: Length exceeds the maximum size of the character name you want to
// change. 		8: Name contains invalid characters. Character name change failed. 		9: The name change
// is
// prohibited. Character name change failed. 		10: Character name change failed, due an unknown
// error.
...
int chclif_parse(int fd) {
	...
	if (session[fd]->flag.eof) {
		if (sd != NULL && sd->auth) {  // already authed client
			...
			struct online_char_data* data =
				(struct online_char_data*)idb_get(online_char_db, sd->account_id);
			if (data != NULL && data->fd == fd)
				data->fd = -1;
			if (data == NULL ||
				data->server == -1)	 // If it is not in any server, send it offline. [Skotlex]
	...
	while (RFIFOREST(fd) >= 2) {
		...
		switch (cmd) {
			...
			default:
				ShowError("parse_char: Received unknown packet " CL_WHITE "0x%x" CL_RESET
						  " from ip '" CL_WHITE "%s" CL_RESET "'! Disconnecting!\n",
