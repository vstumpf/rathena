	...
	if (charserv_config.pincode_config.pincode_enabled) {
		...
		if (sd->pincode[0] == '\0') {
			...
			if (!(charserv_config.pincode_config.pincode_changetime) ||
				(sd->pincode_change + charserv_config.pincode_config.pincode_changetime) >
					time(NULL)) {
				DBMap* online_char_db = char_get_onlinedb();
				struct online_char_data* node =
...
int chlogif_parse_ackconnect(int fd) {
	...
	if (RFIFOB(fd, 2)) {
		...
		ShowError(
			"Also, please make sure your login db has the correct communication username/passwords "
			"and the gender of the account is S.\n");
...
int chlogif_parse_ackaccreq(int fd) {
		...
		return 0;
	{
		...
		if (session_isActive(request_id) &&
			(sd = (struct char_session_data*)session[request_id]->session_data) && !sd->auth &&
			sd->account_id == account_id && sd->login_id1 == login_id1 &&
			sd->login_id2 == login_id2 && sd->sex == sex) {
 ...
 * AH 0x2717 <aid>.L <email>.40B <expiration_time>.L <group_id>.B <birthdate>.11B <pincode>.5B
 *<pincode_change>.L <isvip>.B <char_vip>.B <char_billing>.B
 **/
int chlogif_parse_reqaccdata(int fd) {
	...
	ARR_FIND(0, fd_max, u_fd,
			 session[u_fd] && (sd = (struct char_session_data*)session[u_fd]->session_data) &&
				 sd->auth && sd->account_id == RFIFOL(fd, 2));
	if (u_fd < fd_max) {
		...
		if (sd->char_slots > MAX_CHARS) {
			ShowError(
				"Account '%d' `character_slots` column is higher than supported MAX_CHARS (%d), "
				"update MAX_CHARS in mmo.hpp! capping to MAX_CHARS...\n",
		...
		if (((charserv_config.max_connect_user == 0 || charserv_config.char_maintenance == 1) ||
			 (charserv_config.max_connect_user > 0 &&
			  char_count_users() >= charserv_config.max_connect_user)) &&
...
void chlogif_parse_change_sex_sub(int sex, int acc, int char_id, int class_, int guild_id) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `equip` = '0' WHERE `char_id` = '%d'",
							   schema_config.inventory_db, char_id))
		Sql_ShowDebug(sql_handle);

	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "UPDATE `%s` SET `class` = '%d', `weapon` = '0', `shield` = '0', `head_top` = "
				  "'0', `head_mid` = '0', `head_bottom` = '0', `sex` = '%c' WHERE `char_id` = '%d'",
	...
	if (RFIFOREST(fd) < 7)
		...
		if (acc > 0) {	// TODO: Is this even possible?
			...
			if (SQL_ERROR ==
					SqlStmt_Prepare(
						stmt,
						"SELECT `char_id`, `class`, `guild_id` FROM `%s` WHERE `account_id` = '%d'",
...
int chlogif_parse_ackchangecharsex(int char_id, int sex) {
	...
	if (SQL_ERROR ==
		Sql_Query(sql_handle,
				  "SELECT `account_id`,`class`,`guild_id` FROM `%s` WHERE `char_id` = '%d'",
	...
	if (RFIFOREST(fd) < 6)
		...
		if (character != NULL) {		   // account is already marked as online!
			if (character->server > -1) {  // Kick it from the map server it is on.
				...
				ARR_FIND(0, fd_max, i,
						 session[i] &&
							 (tsd = (struct char_session_data*)session[i]->session_data) &&
							 tsd->account_id == aid);
				if (i < fd_max) {
					chclif_send_auth_result(
						i, 2);	// Send "Someone has already logged in with this id"
					set_eof(i);
 ...
 * @param flag : 0x1 Select info and update old_groupid, 0x2 VIP duration is changed by atcommand or
 * script, 0x8 First request on player login
...
int chlogif_parse_AccInfoAck(int fd) {
	if (RFIFOREST(fd) < 19)
		...
		if (type == 0 || RFIFOREST(fd) < 122 + NAME_LENGTH) {
			mapif_accinfo_ack(false, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14),
							  0, -1, 0, 0, NULL, NULL, NULL, NULL, NULL);
			RFIFOSKIP(fd, 19);
			return 1;
		}
		type >>= 1;
		mapif_accinfo_ack(true, RFIFOL(fd, 2), RFIFOL(fd, 6), RFIFOL(fd, 10), RFIFOL(fd, 14), type,
						  RFIFOL(fd, 19), RFIFOL(fd, 23), RFIFOL(fd, 27), RFIFOCP(fd, 31),
						  RFIFOCP(fd, 71), RFIFOCP(fd, 87), RFIFOCP(fd, 111), RFIFOCP(fd, 122));
...
TIMER_FUNC(chlogif_check_connect_logserver) {
	...
	WFIFOW(login_fd, 84) =
		charserv_config.char_new_display;  // only display (New) if they want to [Kevin]
	WFIFOSET(login_fd, 86);

	return 1;
}

int chlogif_isconnected() { return session_isActive(login_fd); }
...
void do_init_chlogif(void) {
	...
	add_timer_interval(gettick() + 1000, chlogif_send_acc_tologin, 0, 0,
					   3600 * 1000);  // Sync online accounts every hour
}

/// Resets all the data.
void chlogif_reset(void) {
	int id;
	// TODO kick everyone out and reset everything or wait for connect and try to reaquire locks
	// [FlavioJS]
...
void chlogif_on_disconnect(void) { ShowWarning("Connection to Login Server lost.\n\n"); }
...
void chlogif_on_ready(void) {
	...
	ARR_FIND(0, ARRAYLENGTH(map_server), i,
			 session_isValid(map_server[i].fd) && !map_server[i].map.empty());
