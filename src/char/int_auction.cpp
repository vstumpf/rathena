...
int auction_count(uint32 char_id, bool buy) {
	...
	for (auction = (struct auction_data *)dbi_first(iter); dbi_exists(iter);
...
void auction_save(struct auction_data *auction) {
	...
	StringBuf_Printf(
		&buf,
		"UPDATE `%s` SET `seller_id` = '%d', `seller_name` = ?, `buyer_id` = '%d', `buyer_name` = "
		"?, `price` = '%d', `buynow` = '%d', `hours` = '%d', `timestamp` = '%lu', `nameid` = '%u', "
		"`item_name` = ?, `type` = '%d', `refine` = '%d', `attribute` = '%d', `enchantgrade` ='%d'",
		schema_config.auction_db, auction->seller_id, auction->buyer_id, auction->price,
		auction->buynow, auction->hours, (unsigned long)auction->timestamp, auction->item.nameid,
		auction->type, auction->item.refine, auction->item.attribute, auction->item.enchantgrade);
	...
	if (SQL_SUCCESS != SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 0, SQLDT_STRING, auction->seller_name,
										 strnlen(auction->seller_name, NAME_LENGTH)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 1, SQLDT_STRING, auction->buyer_name,
										 strnlen(auction->buyer_name, NAME_LENGTH)) ||
		SQL_SUCCESS != SqlStmt_BindParam(stmt, 2, SQLDT_STRING, auction->item_name,
										 strnlen(auction->item_name, ITEM_NAME_LENGTH)) ||
...
unsigned int auction_create(struct auction_data *auction) {
	...
	if (SQL_SUCCESS != SqlStmt_PrepareStr(stmt, StringBuf_Value(&buf)) ||
		...
		auction->auction_end_timer =
			add_timer(gettick() + tick, auction_end_timer, auction->auction_id, 0);
...
TIMER_FUNC(auction_end_timer) {
	...
	if ((auction = (struct auction_data *)idb_get(auction_db_, id)) != NULL) {
		if (auction->buyer_id) {
			mail_sendmail(0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201),
						  msg_txt(202), 0, &auction->item, 1);
			mapif_Auction_message(auction->buyer_id, 6);  // You have won the auction
			mail_sendmail(0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201),
						  msg_txt(203), auction->price, NULL, 0);
		} else
			mail_sendmail(0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201),
						  msg_txt(204), 0, &auction->item, 1);
...
void auction_delete(struct auction_data *auction) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "DELETE FROM `%s` WHERE `auction_id` = '%d'",
							   schema_config.auction_db, auction_id))
...
void inter_auctions_fromsql(void) {
	...
	StringBuf_AppendStr(&buf,
						...
						"`price`,`buynow`,`hours`,`timestamp`,`nameid`,`item_name`,`type`,`refine`,"
						"`attribute`,`unique_id`,`enchantgrade`");
...
void mapif_parse_Auction_requestlist(int fd) {
	...
	for (auction = static_cast<auction_data *>(dbi_first(iter)); dbi_exists(iter);
		 auction = static_cast<auction_data *>(dbi_next(iter))) {
...
void mapif_parse_Auction_cancel(int fd) {
	...
	if (auction->buyer_id > 0) {
		mapif_Auction_cancel(fd, char_id,
							 3);  // An auction with at least one bidder cannot be canceled
		return;
	}

	mail_sendmail(0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201),
				  msg_txt(205), 0, &auction->item, 1);
	auction_delete(auction);
...
void mapif_parse_Auction_close(int fd) {
	...
	mail_sendmail(0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201),
				  msg_txt(206), auction->price, NULL, 0);
	// Send Item to Buyer
	mail_sendmail(0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201),
				  msg_txt(207), 0, &auction->item, 1);
	mapif_Auction_message(auction->buyer_id, 6);  // You have won the auction
	auction_delete(auction);
...
void mapif_parse_Auction_bid(int fd) {
	...
	if ((auction = (struct auction_data *)idb_get(auction_db_, auction_id)) == NULL ||
		auction->price >= bid || auction->seller_id == char_id) {
	...
	if (bid >= auction->buynow) {  // Automatic won the auction
		mapif_Auction_bid(fd, char_id, bid - auction->buynow,
						  1);  // You have successfully bid in the auction

		mail_sendmail(0, msg_txt(200), auction->buyer_id, auction->buyer_name, msg_txt(201),
					  msg_txt(210), 0, &auction->item, 1);
		mapif_Auction_message(char_id, 6);	// You have won the auction
		mail_sendmail(0, msg_txt(200), auction->seller_id, auction->seller_name, msg_txt(201),
					  msg_txt(211), auction->buynow, NULL, 0);
