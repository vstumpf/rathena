 ...
 **/
const char *geoip_countryname[253] = {"Unknown",
									  "Asia/Pacific Region",
									  "Europe",
									  "Andorra",
...
/* [Dekamaster/Nightroad] */
/* WHY NOT A DBMAP: There are millions of entries in GeoIP and it has its own algorithm to go
 * quickly through them, a DBMap wouldn't be efficient */
...
void mapif_parse_accinfo(int fd) {
	...
	if (account_id < START_ACCOUNT_NUM) {  // is string
		if (SQL_ERROR ==
				Sql_Query(sql_handle,
						  "SELECT `account_id`,`name`,`class`,`base_level`,`job_level`,`online` "
						  "FROM `%s` WHERE `name` LIKE '%s' LIMIT 10",
			...
			if (Sql_NumRows(sql_handle) == 1) {	 // we found a perfect match
				...
				while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
					...
					inter_to_fd(fd, u_fd, u_aid, (char *)msg_txt(215), account_id, name,
								job_name(class_), base_level, job_level,
	...
	/* it will only get here if we have a single match then ask login-server to fetch the `login`
	 * record */
...
void mapif_accinfo_ack(bool success, int map_fd, int u_fd, int u_aid, int account_id, int8 type,
					   int group_id, int logincount, int state, const char *email,
					   const char *last_ip, const char *lastlogin, const char *birthdate,
					   const char *userid) {
	...
	inter_to_fd(map_fd, u_fd, u_aid, (char *)msg_txt(222), last_ip,
				geoip_getcountry(str2ip(last_ip)));
	inter_to_fd(map_fd, u_fd, u_aid, (char *)msg_txt(223), logincount, lastlogin);
	inter_to_fd(map_fd, u_fd, u_aid, (char *)msg_txt(224));

	if (SQL_ERROR ==
			Sql_Query(sql_handle,
					  "SELECT `char_id`, `name`, `char_num`, `class`, `base_level`, `job_level`, "
					  "`online` FROM `%s` WHERE `account_id` = '%d' ORDER BY `char_num` LIMIT %d",
		...
		while (SQL_SUCCESS == Sql_NextRow(sql_handle)) {
			...
			inter_to_fd(map_fd, u_fd, u_aid, (char *)msg_txt(225), char_num, char_id, name,
						job_name(class_), base_level, job_level, online ? "Online" : "Offline");
		}
	}
	Sql_FreeResult(sql_handle);
}
 ...
 **/
void inter_savereg(uint32 account_id, uint32 char_id, const char *key, uint32 index,
				   int64 int_value, const char *string_value, bool is_string) {
	...
	if (key[0] == '#' && key[1] == '#') {  // global account reg
		if (session_isValid(login_fd))
			...
			ShowError(
				"Login server unavailable, can't perform update on '%s' variable for AID:%" PRIu32
				" CID:%" PRIu32 "\n",
		...
		if (is_string) {
			if (string_value) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle,
										   ...
										   schema_config.acc_reg_str_table, account_id, esc_key,
										   index))
					Sql_ShowDebug(sql_handle);
			}
		} else {
			if (int_value) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle,
										   "DELETE FROM `%s` WHERE `account_id` = '%" PRIu32
										   "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
										   schema_config.acc_reg_num_table, account_id, esc_key,
										   index))
					Sql_ShowDebug(sql_handle);
			}
		}
		...
		if (is_string) {
			if (string_value) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle,
										   ...
										   "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
										   schema_config.char_reg_str_table, char_id, esc_key,
										   index))
					Sql_ShowDebug(sql_handle);
			}
		} else {
			if (int_value) {
				...
				if (SQL_ERROR == Sql_Query(sql_handle,
										   "DELETE FROM `%s` WHERE `char_id` = '%" PRIu32
										   "' AND `key` = '%s' AND `index` = '%" PRIu32 "' LIMIT 1",
										   schema_config.char_reg_num_table, char_id, esc_key,
										   index))
					Sql_ShowDebug(sql_handle);
			}
		}
...
int inter_accreg_fromsql(uint32 account_id, uint32 char_id, int fd, int type) {
	...
	switch (type) {
		case 3:	 // char reg
			if (SQL_ERROR ==
				Sql_Query(sql_handle,
						  "SELECT `key`, `index`, `value` FROM `%s` WHERE `char_id`='%" PRIu32 "'",
		...
		case 1:	 // account2 reg
			ShowError(
				"inter_accreg_fromsql: Char server shouldn't handle type 1 registry values (##). "
				"That is the login server's job!\n");
			return 0;
		default:
...
int inter_log(const char *fmt, ...) {
	...
	if (SQL_ERROR == Sql_Query(sql_handle, "INSERT INTO `%s` (`time`, `log`) VALUES (NOW(),  '%s')",
							   schema_config.interlog_db, esc_str))
		Sql_ShowDebug(sql_handle);
...
int inter_init_sql(const char *file) {
	...
	if (SQL_ERROR == Sql_Connect(sql_handle, char_server_id.c_str(), char_server_pw.c_str(),
								 char_server_ip.c_str(), (uint16)char_server_port,
								 char_server_db.c_str())) {
		ShowError(
			"Couldn't connect with username = '%s', host = '%s', port = '%d', database = '%s'\n",
			char_server_id.c_str(), char_server_ip.c_str(), char_server_port,
			char_server_db.c_str());
		Sql_ShowDebug(sql_handle);
		Sql_Free(sql_handle);
		exit(EXIT_FAILURE);
...
// broadcast sending
int mapif_broadcast(unsigned char *mes, int len, unsigned long fontColor, short fontType,
					short fontSize, short fontAlign, short fontY, int sfd) {
...
int check_ttl_wisdata(void) {
	...
	do {
		...
		for (i = 0; i < wis_delnum; i++) {
			...
			// mapif_wis_reply(wd->fd, wd->src, 1); // flag: 0: success to send wisper, 1: target
			// character is not loged in?, 2: ignored by target
...
int mapif_parse_broadcast(int fd) {
	mapif_broadcast(RFIFOP(fd, 16), RFIFOW(fd, 2), RFIFOL(fd, 4), RFIFOW(fd, 8), RFIFOW(fd, 10),
					RFIFOW(fd, 12), RFIFOW(fd, 14), fd);
	return 0;
}
...
int mapif_parse_WisRequest(int fd) {
	...
	safestrncpy(name, RFIFOCP(fd, 8 + NAME_LENGTH),
				NAME_LENGTH);  // Received name may be too large and not contain \0! [Skotlex]

	Sql_EscapeStringLen(sql_handle, esc_name, name, strnlen(name, NAME_LENGTH));
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT `name` FROM `%s` WHERE `name`='%s'",
...
int mapif_parse_WisReply(int fd) {
	...
	if (wd == NULL)
		return 0;  // This wisp was probably suppress before, because it was timeout of because of
				   // target was found on another map-server

	if ((--wd->count) <= 0 || flag != 1) {
		mapif_wis_reply(wd->fd, wd->src, flag);	 // flag: 0: success to send wisper, 1: target
												 // character is not loged in?, 2: ignored by target
...
// Received wisp message from map-server for ALL gm (just copy the message and resends it to ALL
// map-servers)
int mapif_parse_WisToGM(int fd) {
	unsigned char
		buf[2048];	// 0x3003/0x3803 <packet_len>.w <wispname>.24B <permission>.L <message>.?B
	...
	if (count) {
		...
		for (i = 0; i < count; i++) {
			...
			switch (RFIFOB(fd, cursor++)) {
				// int
				case 0:
					inter_savereg(account_id, char_id, key.c_str(), index, RFIFOQ(fd, cursor),
								  nullptr, false);
					cursor += 8;
					break;
				case 1:
...
void mapif_namechange_ack(int fd, uint32 account_id, uint32 char_id, int type, int flag,
						  char *name) {
...
int inter_parse_frommap(int fd) {
	...
	switch (cmd) {
		...
		case 0x3009:
			...
			break;
		default:
			if (inter_party_parse_frommap(fd) || inter_guild_parse_frommap(fd) ||
				inter_storage_parse_frommap(fd) || inter_pet_parse_frommap(fd) ||
				inter_homunculus_parse_frommap(fd) || inter_mercenary_parse_frommap(fd) ||
				inter_elemental_parse_frommap(fd) || inter_mail_parse_frommap(fd) ||
				inter_auction_parse_frommap(fd) || inter_quest_parse_frommap(fd) ||
				inter_clan_parse_frommap(fd) || inter_achievement_parse_frommap(fd))
				break;
			else
				return 0;
	}
