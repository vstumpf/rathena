...
char* sErr(int code) {
	...
	if (FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, code,
					  MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT), (LPTSTR)&sbuf, sizeof(sbuf),
					  NULL) == 0)
...
#define sSetsockopt(fd, level, optname, optval, optlen) \
...
uint32 send_shortlist_set[(MAXCONN + 31) /
						  32];	// to know if specific fd's are already in the shortlist
#endif
...
void set_defaultparse(ParseFunc defaultparse) { default_func_parse = defaultparse; }
...
#endif
			session[fd]->wdata_size = 0;  // Clear the send queue as we can't send anymore.
										  // [Skotlex]
			set_eof(fd);
		}
		return 0;
...
void flush_fifos(void) {
	int i;
	for (i = 1; i < fd_max; i++) flush_fifo(i);
}
...
int connect_client(int listen_fd) {
	...
	if (epoll_ctl(epfd, EPOLL_CTL_ADD, fd, &epevent) == SOCKET_ERROR) {
		ShowError(
			"connect_client: Failed to add to epoll event dispatcher for new socket #%d: %s\n", fd,
			error_msg());
		sClose(fd);
		return -1;
	}
		...
		if (sErrno == S_EWOULDBLOCK) {
			...
			if (result == 0) {
				if (!silent) {
					...
					ShowError(
						"make_connection: connection failed (socket #%d, timeout after %ds)!\n", fd,
						timeout);
				}
...
int realloc_writefifo(int fd, size_t addition) {
	...
	if (session[fd]->wdata_size + addition >
		session[fd]->max_wdata) {  // grow rule; grow in multiples of WFIFO_SIZE
		newsize = WFIFO_SIZE;
		while (session[fd]->wdata_size + addition > newsize) newsize += WFIFO_SIZE;
	} else if (session[fd]->max_wdata >=
				   (size_t)2 * (session[fd]->flag.server ? FIFOSIZE_SERVERLINK : WFIFO_SIZE) &&
			   (session[fd]->wdata_size + addition) * 4 <
				   session[fd]->max_wdata) {  // shrink rule, shrink by 2 when only a quarter of the
...
int RFIFOSKIP(int fd, size_t len) {
	...
	if (s->rdata_size < s->rdata_pos + len) {
		ShowError("RFIFOSKIP: skipped past end of read buffer! Adjusting from %" PRIuPTR
				  " to %d (session #%d)\n",
				  len, RFIFOREST(fd), fd);
		len = RFIFOREST(fd);
	}
...
int WFIFOSET(int fd, size_t len) {
	...
	if (s->wdata_size + len > s->max_wdata) {  // actually there was a buffer overflow already
		...
		ShowFatalError(
			...
			"a %u/%u bytes buffer.\n",
			fd, CONVIP(ip), (unsigned int)len, (unsigned int)s->wdata_size,
	...
	if (!s->flag.server) {
		if (len >
			socket_max_client_packet) {	 // see declaration of socket_max_client_packet for details
			ShowError("WFIFOSET: Dropped too large client packet 0x%04x (length=%" PRIuPTR
					  ", max=%" PRIuPTR ").\n",
...
int do_sockets(t_tick next) {
	...
	for (i = 1; i < fd_max; i++) {
		...
		{  // Finally, even if there is no data to parse, connections signalled eof should be
		   // closed, so we call parse_func [Skotlex]
	...
	if (last_tick != socket_data_last_tick) {
		...
		sprintf(buf,
				...
				"kB) | RAM: %.03f MB",
				socket_data_i / 1024., socket_data_ci / 1024., socket_data_qi / 1024.,
				socket_data_o / 1024., socket_data_co / 1024., socket_data_qo / 1024.,
				malloc_usage() / 1024.);
#ifdef _WIN32
		SetConsoleTitle(buf);
#else
	...
	for (i = 0; i < 0x10000; ++i) {
		...
		while (hist) {
			if ((!hist->ddos && DIFF_TICK(tick, hist->tick) > ddos_interval * 3) ||
				(hist->ddos &&
				 DIFF_TICK(tick, hist->tick) > ddos_autoreset)) {  // Remove connection history
...
int access_ipmask(const char* str, AccessControl* acc) {
	...
	if (strcmp(str, "all") == 0) {
		...
		unsigned int m[4];
		int n;
		if (((n = sscanf(str, "%3u.%3u.%3u.%3u/%3u.%3u.%3u.%3u", a, a + 1, a + 2, a + 3, m, m + 1,
						 m + 2, m + 3)) != 8 &&	 // not an ip + standard mask
			 (n = sscanf(str, "%3u.%3u.%3u.%3u/%3u", a, a + 1, a + 2, a + 3, m)) !=
				 5 &&  // not an ip + bit mask
			 (n = sscanf(str, "%3u.%3u.%3u.%3u", a, a + 1, a + 2, a + 3)) != 4) ||	// not an ip
			a[0] > 255 ||
			a[1] > 255 || a[2] > 255 || a[3] > 255 ||  // invalid ip
			(n == 8 &&
			 (m[0] > 255 || m[1] > 255 || m[2] > 255 || m[3] > 255)) ||	 // invalid standard mask
	...
	while (fgets(line, sizeof(line), fp)) {
		...
		if (!strcmpi(w1, "stall_time")) {
			...
			if (epoll_maxevents < 16) {
				ShowWarning(
					"socket_config_read: epoll_maxevents is set too low. Defaulting to 16...\n");
				epoll_maxevents = 16;
			}
		}
#endif
...
int socket_getips(uint32* ips, int max) {
...
#ifdef WIN32
	{
		...
		if (gethostname(fullhost, sizeof(fullhost)) == SOCKET_ERROR) {
			...
			for (; num < max && a[num] != NULL; ++num) ips[num] = (uint32)ntohl(*a[num]);
		}
	}
#else  // not WIN32
	{
	...
	{  // set socket limit to MAXCONN
		...
		if (0 == getrlimit(RLIMIT_NOFILE, &rlp)) {
			...
			if (0 != setrlimit(RLIMIT_NOFILE,
							   &rlp)) {	 // failed, try setting the maximum too (permission to
...
bool session_isValid(int fd) { return (fd > 0 && fd < MAXCONN && session[fd] != nullptr); }

bool session_isActive(int fd) { return (session_isValid(fd) && !session[fd]->flag.eof); }
...
uint32 str2ip(const char* ip_str) { return ntohl(inet_addr(ip_str)); }

// Reorders bytes from network to little endian (Windows).
// Neccessary for sending port numbers to the RO client until Gravity notices that they forgot
// ntohs() calls.
uint16 ntows(uint16 netshort) { return ((netshort & 0xFF) << 8) | ((netshort & 0xFF00) >> 8); }
...
void send_shortlist_do_sends() {
	...
	for (i = send_shortlist_count - 1; i >= 0; --i) {
		...
		if (((send_shortlist_set[idx] >> bit) & 1) == 0) {
			ShowDebug(
				"send_shortlist_do_sends: fd is not set, why is it in the shortlist? (fd=%d)\n",
				fd);
			continue;
		}
