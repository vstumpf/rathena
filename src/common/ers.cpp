...
static void ers_free_cache(ers_cache_t *cache, bool remove) {
	...
	for (i = 0; i < cache->Used; i++) aFree(cache->Blocks[i]);
...
static void *ers_obj_alloc_entry(ERS *self) {
	...
	if (instance->Cache->ReuseList != NULL) {
		...
		ret = &instance->Cache->Blocks[instance->Cache->Used - 1]
									  [instance->Cache->Free * instance->Cache->ObjectSize +
		...
		CREATE(instance->Cache->Blocks[instance->Cache->Used], unsigned char,
...
static void ers_obj_free_entry(ERS *self, void *entry) {
	...
	if (instance->Cache->Options & ERS_OPT_CLEAN)
		memset((unsigned char *)reuse + sizeof(struct ers_list), 0,
