	...
	*/
	...
	while ((q = strchr(p, 0x1b)) != NULL) {								// find the escape character
		...
		if (q[1] != '[') {	// write the escape char (whatever purpose it has)
			...
			q = q + 2;
			for (;;) {
				if (ISDIGIT(*q)) {	// add number to number array, only accept 2digits, shift out
									// the rest
			...
			while (1) {
				if (ISDIGIT(*q)) {
					...
					continue;
						   ...
						   'F') {  // \033[#F - Cursor Preceding Line (CPL)
								   // Moves the cursor up the indicated # of rows, to column 1.
				} else if (*q == 'G') {	 // \033[#G - Cursor Horizontal Absolute (CHA)
										 // Moves the cursor to indicated column in current row.
				} else if (*q == 'L' || *q == 'M' || *q == '@' ||
...
int _vShowMessage(enum msg_type flag, const char *string, va_list ap) {
	...
	if ((flag == MSG_WARNING && console_msg_log & 1) ||
		...
		if ((log = fopen(console_log_filepath, "a+"))) {
			...
			fprintf(log, "(%s) [ %s ] : ", timestring,
					flag == MSG_WARNING
						? "Warning"
						: flag == MSG_ERROR
							  ? "Error"
							  : flag == MSG_SQL ? "SQL Error"
												: flag == MSG_DEBUG ? "Debug" : "Unknown");
			va_copy(apcopy, ap);
	...
	switch (flag) {
		...
		case MSG_STATUS:  // Bright Green (To inform about good things)
			...
			break;
		case MSG_SQL:  // Bright Violet (For dumping out anything related with SQL) <- Actually,
					   // this is mostly used for SQL errors with the database, as successes can as
					   // well just be anything else... [Skotlex]
