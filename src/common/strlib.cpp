...
int sv_parse(const char* str, int len, int startoff, char delim, int* out_pos, int npos,
			 enum e_svopt opt) {
	struct s_svstate sv;
	int count;
	...
	if (out_pos == NULL)
		npos = 0;
	for (count = 0; count < npos; ++count) out_pos[count] = -1;
	sv.str = str;
	sv.len = len;
	sv.off = startoff;
	sv.opt = opt;
...
/// @param opt Options that determine the parsing behaviour
/// @return Number of fields found in the string or -1 if an error occured
int sv_split(char* str, int len, int startoff, char delim, char** out_fields, size_t nfields,
			 enum e_svopt opt) {
	int pos[1024];
	int done;
	char* end;
 ...
 */
bool sv_readdb(const char* directory, const char* filename, char delim, int mincols, int maxcols,
			   int maxrows, bool (*parseproc)(char* fields[], int columns, int current),
			   bool silent) {
	FILE* fp;
	int lines = 0;
	int entries = 0;
	...
	while (fgets(line, maxcols * colsize, fp)) {
		...
		columns = sv_split(line, strlen(line), 0, delim, fields, nb_cols,
...
int _StringBuf_Printf(const char* file, int line, const char* func, StringBuf* self,
					  const char* fmt, ...) {
	int len;
	va_list ap;
...
int _StringBuf_Vprintf(const char* file, int line, const char* func, StringBuf* self,
...
int _StringBuf_Append(const char* file, int line, const char* func, StringBuf* self,
					  const StringBuf* sbuf) {
...
int _StringBuf_AppendStr(const char* file, int line, const char* func, StringBuf* self,
						 const char* str) {
	int available = self->max_ - (self->ptr_ - self->buf_);
...
int StringBuf_Length(StringBuf* self) { return (int)(self->ptr_ - self->buf_); }
...
void StringBuf_Clear(StringBuf* self) { self->ptr_ = self->buf_; }
