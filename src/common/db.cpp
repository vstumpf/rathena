	...
	for (i = 0; i < db->free_count; i++) {
		if (db->free_list[i].node == node) {
			if (i < db->free_count - 1)	 // copy the last item to where the removed one was
				memcpy(&db->free_list[i], &db->free_list[db->free_count - 1],
	...
	for (; it->ht_index < HASH_SIZE; ++(it->ht_index)) {
		...
		while (node) {			// next node
			if (node->right) {	// continue in the right subtree
				node = node->right;
				while (node->left) node = node->left;  // get leftmost node
	...
	for (; it->ht_index >= 0; --(it->ht_index)) {
		...
		while (node) {		   // next node
			if (node->left) {  // continue in the left subtree
				node = node->left;
				while (node->right) node = node->right;	 // get rightmost node
			} else {									 // continue to the next parent (recursive)
				parent = node->parent;
				while (parent) {
					if (parent->left != node)
 ...
 *       is fetched
 * Puts data of the removed entry in out_data, if out_data is not NULL (unless data has been
 * released)
 * @param self Iterator
 ...
 * @see DBMap#vgetall
 */
static unsigned int db_obj_vgetall(DBMap *self, DBData **buf, unsigned int max, DBMatcher match,
								   va_list args) {
	DBMap_impl *db = (DBMap_impl *)self;
	unsigned int i;
	DBNode *node;
...
static int db_obj_vdestroy(DBMap *self, DBApply func, va_list args) {
	...
	if (db->global_lock) {
		ShowError(
			"db_vdestroy: Database is already locked for destruction. Aborting second database "
			"destruction.\n"
			"Database allocated at %s:%d\n",
			db->alloc_file, db->alloc_line);
		return 0;
	...
	if (db->free_lock)
		ShowWarning(
			"db_vdestroy: Database is still in use, %u lock(s) left. Continuing database "
			"destruction.\n"
			"Database allocated at %s:%d\n",
 ...
 */
DBMap *db_alloc(const char *file, const char *func, int line, DBType type, DBOptions options,
				unsigned short maxlen) {
	DBMap_impl *db;
	unsigned int i;
	char ers_name[50];
	...
	for (i = 0; i < HASH_SIZE; i++) db->ht[i] = NULL;
	db->cache = NULL;
	db->type = type;
	db->options = options;
	db->item_count = 0;
...
void db_final(void) {
	...
	ShowInfo(CL_WHITE "Database types" CL_RESET
			 ...
			 stats.db_int_alloc, stats.db_int_destroy, stats.db_uint_alloc, stats.db_uint_destroy,
			 stats.db_string_alloc, stats.db_string_destroy, stats.db_istring_alloc,
			 stats.db_istring_destroy, stats.db_int64_alloc, stats.db_int64_destroy,
	...
	ShowInfo(
		...
		stats.db_rotate_left, stats.db_rotate_right, stats.db_rebalance, stats.db_rebalance_erase,
		stats.db_is_key_null, stats.db_dup_key, stats.db_dup_key_free, stats.db_free_add,
		stats.db_free_remove, stats.db_free_lock, stats.db_free_unlock, stats.db_int_cmp,
		stats.db_uint_cmp, stats.db_string_cmp, stats.db_istring_cmp, stats.db_int64_cmp,
		stats.db_uint64_cmp, stats.db_int_hash, stats.db_uint_hash, stats.db_string_hash,
		stats.db_istring_hash, stats.db_int64_hash, stats.db_uint64_hash, stats.db_release_nothing,
		stats.db_release_key, stats.db_release_data, stats.db_release_both, stats.dbit_first,
		stats.dbit_last, stats.dbit_next, stats.dbit_prev, stats.dbit_exists, stats.dbit_remove,
		stats.dbit_destroy, stats.db_iterator, stats.db_exists, stats.db_get, stats.db_getall,
		stats.db_vgetall, stats.db_ensure, stats.db_vensure, stats.db_put, stats.db_remove,
		stats.db_foreach, stats.db_vforeach, stats.db_clear, stats.db_vclear, stats.db_destroy,
		stats.db_vdestroy, stats.db_size, stats.db_type, stats.db_options, stats.db_fix_options,
		stats.db_default_cmp, stats.db_default_hash, stats.db_default_release,
		stats.db_custom_release, stats.db_alloc, stats.db_i2key, stats.db_ui2key, stats.db_str2key,
		stats.db_i642key, stats.db_ui642key, stats.db_i2data, stats.db_ui2data, stats.db_ptr2data,
		stats.db_data2i, stats.db_data2ui, stats.db_data2ptr, stats.db_init, stats.db_final);
#endif /* DB_ENABLE_STATS */
