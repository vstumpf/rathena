...
bool exists(const char* filename) { return !access(filename, F_OK); }
...
uint16 MakeWord(uint8 byte0, uint8 byte1) { return byte0 | (byte1 << 0x08); }
...
int32 GetLong(const unsigned char* buf) { return (int32)GetULong(buf); }
