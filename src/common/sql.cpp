 ...
 */
unsigned int Sql_GetError(Sql* self) { return mysql_errno(&self->handle); }
 ...
 * @return
 */
int Sql_Connect(Sql* self, const char* user, const char* passwd, const char* host, uint16 port,
				const char* db) {
	if (self == NULL)
		return SQL_ERROR;

	StringBuf_Clear(&self->buf);
	if (!mysql_real_connect(&self->handle, host, user, passwd, db, (unsigned int)port,
							NULL /*unix_socket*/, 0 /*clientflag*/)) {
...
static int Sql_P_Keepalive(Sql* self) {
	...
	return add_timer_interval(gettick() + ping_interval * 1000, Sql_P_KeepaliveTimer, 0,
							  (intptr_t)self, ping_interval * 1000);
}

/// Escapes a string.
size_t Sql_EscapeString(Sql* self, char* out_to, const char* from) {
	if (self)
		return (size_t)mysql_real_escape_string(&self->handle, out_to, from,
...
int Sql_QueryV(Sql* self, const char* query, va_list args) {
	...
	if (mysql_real_query(&self->handle, StringBuf_Value(&self->buf),
...
///
/// @private
static int Sql_P_BindSqlDataType(MYSQL_BIND* bind, enum SqlDataType buffer_type, void* buffer,
								 size_t buffer_len, unsigned long* out_length, int8* out_is_null) {
...
///
/// @private
static void Sql_P_ShowDebugMysqlFieldInfo(const char* prefix, enum enum_field_types type,
										  int is_unsigned, unsigned long length,
...
static void SqlStmt_P_ShowDebugTruncatedColumn(SqlStmt* self, size_t i) {
	...
	ShowDebug("column - %lu\n", (unsigned long)i);
	Sql_P_ShowDebugMysqlFieldInfo("data   - ", field->type, field->flags & UNSIGNED_FLAG,
								  self->column_lengths[i].length, "");
	column = &self->columns[i];
	if (column->buffer_type == MYSQL_TYPE_STRING)
		Sql_P_ShowDebugMysqlFieldInfo("buffer - ", column->buffer_type, column->is_unsigned,
									  column->buffer_length, "+1(nul-terminator)");
	else
		Sql_P_ShowDebugMysqlFieldInfo("buffer - ", column->buffer_type, column->is_unsigned,
...
int SqlStmt_PrepareV(SqlStmt* self, const char* query, va_list args) {
	...
	StringBuf_Clear(&self->buf);
	StringBuf_Vprintf(&self->buf, query, args);
	if (mysql_stmt_prepare(self->stmt, StringBuf_Value(&self->buf),
						   (unsigned long)StringBuf_Length(&self->buf))) {
...
int SqlStmt_BindParam(SqlStmt* self, size_t idx, enum SqlDataType buffer_type, void* buffer,
					  size_t buffer_len) {
	if (self == NULL)
		return SQL_ERROR;
	...
	if (!self->bind_params) {  // initialize the bindings
		...
		for (i = 0; i < count; ++i) self->params[i].buffer_type = MYSQL_TYPE_NULL;
		self->bind_params = true;
	}
	if (idx < self->max_params)
		return Sql_P_BindSqlDataType(self->params + idx, buffer_type, buffer, buffer_len, NULL,
									 NULL);
	else
...
int SqlStmt_BindColumn(SqlStmt* self, size_t idx, enum SqlDataType buffer_type, void* buffer,
					   size_t buffer_len, uint32* out_length, int8* out_is_null) {
	if (self == NULL)
		return SQL_ERROR;
	...
	if (!self->bind_columns) {	// initialize the bindings
		...
		for (i = 0; i < cols; ++i) self->columns[i].buffer_type = MYSQL_TYPE_NULL;
		self->bind_columns = true;
	}
	if (idx < self->max_columns) {
		self->column_lengths[idx].out_length = out_length;
		return Sql_P_BindSqlDataType(self->columns + idx, buffer_type, buffer, buffer_len,
									 &self->column_lengths[idx].length, out_is_null);
	} else {
		return SQL_SUCCESS;	 // out of range - ignore
	}
}
...
void Sql_Init(void) { Sql_inter_server_read(SQL_CONF_NAME, true); }
